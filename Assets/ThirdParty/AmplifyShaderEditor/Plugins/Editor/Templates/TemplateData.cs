...
namespace AmplifyShaderEditor
{
    ...
    public class VertexDataContainer
    {
        ...
        public List<TemplateVertexData> VertexData
        {
            get { return m_vertexData; }
            set { m_vertexData = value; }
        }
        public string VertexDataId
        {
            get { return m_vertexDataId; }
            set { m_vertexDataId = value; }
        }
        public int VertexDataStartIdx
        {
            get { return m_vertexDataStartIdx; }
            set { m_vertexDataStartIdx = value; }
        }
    }
    ...
    public sealed class TemplateData : TemplateDataParent
    {
        ...
        private Dictionary<string, TemplateProperty> m_propertyDict =
            new Dictionary<string, TemplateProperty>();

        [SerializeField]
        private List<TemplateInputData> m_inputDataList = new List<TemplateInputData>();
        private Dictionary<int, TemplateInputData> m_inputDataDict =
            new Dictionary<int, TemplateInputData>();
        ...
        void LoadTemplateBody(string body)
        {
            ...
            FetchCodeAreas(
                TemplatesManager.TemplateVertexCodeBeginArea,
                MasterNodePortCategory.Vertex
            );
            FetchCodeAreas(
                TemplatesManager.TemplateFragmentCodeBeginArea,
            ...
            TemplateHelperFunctions.FetchLocalVars(
                m_templateBody,
                ref m_localVarsList,
                m_vertexFunctionData,
                m_fragmentFunctionData
            );
        ...
        void FetchSubShaderProperties()
        {
            ...
            if (beginSubShader > 0 && endSubShader > 0 && endSubShader > beginSubShader)
            {
                ...
                if (passIndex < 0)
                {
                    ...
                    if (currIdx > 0)
                    {
                        m_templateBody = m_templateBody.Insert(
                            currIdx,
                            identation + TemplatesManager.TemplatePassTag
                        );
                    }
                }

                // GET ALL THE MODULES
                string subBody = m_templateBody.Substring(
                    beginSubShader,
                    endSubShader - beginSubShader
                );
                //CULL MODE
                {
                ...
                //COLOR MASK
                {
                    ...
                    if (colorMaskIdx > 0)
                    {
                        ...
                        string colorMaskParams = subBody.Substring(
                            colorMaskIdx,
                            end - colorMaskIdx
                        );
                        ...
                        TemplateHelperFunctions.CreateColorMask(
                            colorMaskParams,
                            ref m_colorMaskData,
                            TemplateHelperFunctions.ColorMaskPattern
                        );
                ...
                //BlEND MODE
                {
                    ...
                    if (blendModeIdx > 0)
                    {
                        ...
                        TemplateHelperFunctions.CreateBlendMode(
                            blendParams,
                            ref m_blendData,
                            TemplateHelperFunctions.BlendModePattern
                        );
                ...
                //BLEND OP
                {
                    ...
                    if (blendOpIdx > 0)
                    {
                        ...
                        TemplateHelperFunctions.CreateBlendOp(
                            blendOpParams,
                            ref m_blendData,
                            TemplateHelperFunctions.BlendOpPattern
                        );
                ...
                //STENCIL
                {
                    ...
                    if (stencilIdx > -1)
                    {
                        ...
                        if (stencilEndIdx > 0)
                        {
                            ...
                            TemplateHelperFunctions.CreateStencilOps(
                                stencilParams,
                                ref m_stencilData
                            );
                ...
                //ZTEST
                {
                    ...
                    if (zTestOpIdx > -1)
                    {
                        int zTestEndIdx = subBody.IndexOf(
                            TemplatesManager.TemplateNewLine,
                            zTestOpIdx
                        );
                        if (zTestEndIdx > 0)
        ...
        void FetchCommonTags()
        {
            ...
            try
            {
                ...
                int nameEnd = m_templateBody.IndexOf(
                    TemplatesManager.TemplateFullEndTag,
                    nameBegin
                );
                ...
                m_shaderNameId = m_templateBody.Substring(
                    nameBegin,
            ...
            // Vertex Data
            {
                int vertexDataTagBegin = m_templateBody.IndexOf(
                    TemplatesManager.TemplateVertexDataTag
                );
                if (vertexDataTagBegin > -1)
                {
                    m_vertexDataContainer.VertexDataStartIdx = vertexDataTagBegin;
                    int vertexDataTagEnd = m_templateBody.IndexOf(
                        TemplatesManager.TemplateEndOfLine,
                        vertexDataTagBegin
                    );
                    m_vertexDataContainer.VertexDataId = m_templateBody.Substring(
                        vertexDataTagBegin,
                        vertexDataTagEnd
                            + TemplatesManager.TemplateEndOfLine.Length
                            - vertexDataTagBegin
                    );
                    int dataBeginIdx = m_templateBody.LastIndexOf(
                        '{',
                        vertexDataTagBegin,
                        vertexDataTagBegin
                    );
                    string vertexData = m_templateBody.Substring(
                        dataBeginIdx + 1,
                        vertexDataTagBegin - dataBeginIdx
                    );

                    int parametersBegin =
                        vertexDataTagBegin + TemplatesManager.TemplateVertexDataTag.Length;
                    string parameters = m_templateBody.Substring(
                        parametersBegin,
                        vertexDataTagEnd - parametersBegin
                    );
                    m_vertexDataContainer.VertexData = TemplateHelperFunctions.CreateVertexDataList(
                        vertexData,
                        parameters
                    );
            ...
            try
            {
                int interpDataBegin = m_templateBody.IndexOf(
                    TemplatesManager.TemplateInterpolatorBeginTag
                );
                if (interpDataBegin > -1)
                {
                    int interpDataEnd = m_templateBody.IndexOf(
                        TemplatesManager.TemplateEndOfLine,
                        interpDataBegin
                    );
                    string interpDataId = m_templateBody.Substring(
                        interpDataBegin,
                        interpDataEnd + TemplatesManager.TemplateEndOfLine.Length - interpDataBegin
                    );
                    ...
                    string interpData = m_templateBody.Substring(
                        dataBeginIdx + 1,
                        interpDataBegin - dataBeginIdx
                    );

                    m_interpolatorDataContainer = TemplateHelperFunctions.CreateInterpDataList(
                        interpData,
                        interpDataId,
                        8
                    );
            ...
            try
            {
                ...
                for (int i = 0; i < TemplatesManager.CommonTags.Length; i++)
                {
                    ...
                    if (idx > -1)
                    {
                            ...
                            case TemplateCommonTagId.Property:
                                {
                                    TemplateHelperFunctions.CreateShaderPropertiesList(
                                        m_templateBody.Substring(
                                            0,
                                            idx + TemplatesManager.CommonTags[i].Id.Length
                                        ),
                        ...
                        AddId(
                            currentId,
                            TemplatesManager.CommonTags[i].SearchIndentation,
                            TemplatesManager.CommonTags[i].CustomIndentation
                        );
                    }
                }
        ...
        void FetchCodeAreas(string begin, MasterNodePortCategory category)
        {
            ...
            if (areaBeginIndexes > -1)
            {
                ...
                string id = m_templateBody.Substring(
                    areaBeginIndexes,
                    endIdx + TemplatesManager.TemplateEndOfLine.Length - areaBeginIndexes
                );
                ...
                {
                    m_fragmentFunctionData = new TemplateFunctionData(
                        -1,
                        string.Empty,
                        id,
                        areaBeginIndexes,
                ...
                {
                    m_vertexFunctionData = new TemplateFunctionData(
                        -1,
                        string.Empty,
                        id,
                        areaBeginIndexes,
        ...
        {
            string beginTag =
                (portCategory == MasterNodePortCategory.Fragment)
                    ? TemplatesManager.TemplateInputsFragBeginTag
                    : TemplatesManager.TemplateInputsVertBeginTag;
            ...
            if (inputBeginIndexes != null && inputBeginIndexes.Length > 0)
            {
                ...
                {
                    int inputEndIdx = m_templateBody.IndexOf(
                        TemplatesManager.TemplateEndSectionTag,
                        inputBeginIndexes[i]
                    );
                    int defaultValueBeginIdx =
                        inputEndIdx + TemplatesManager.TemplateEndSectionTag.Length;
                    int endLineIdx = m_templateBody.IndexOf(
                        TemplatesManager.TemplateFullEndTag,
                        defaultValueBeginIdx
                    );

                    string defaultValue = m_templateBody.Substring(
                        defaultValueBeginIdx,
                        endLineIdx - defaultValueBeginIdx
                    );
                    string tagId = m_templateBody.Substring(
                        inputBeginIndexes[i],
                        endLineIdx
                            + TemplatesManager.TemplateFullEndTag.Length
                            - inputBeginIndexes[i]
                    );
                        ...
                        try
                        {
                            ...
                            WirePortDataType dataType = (WirePortDataType)
                                Enum.Parse(
                                    ...
                                    inputDataArray[(int)TemplatePortIds.DataType].ToUpper()
                                );

                            int portUniqueIDArrIdx = (int)TemplatePortIds.UniqueId;
                            int portUniqueId =
                                (portUniqueIDArrIdx < inputDataArray.Length)
                                    ? Convert.ToInt32(inputDataArray[portUniqueIDArrIdx])
                                    : -1;
                            if (portUniqueId < 0)
                            ...
                            int portOrderId =
                                (portOrderArrayIdx < inputDataArray.Length)
                                    ? Convert.ToInt32(inputDataArray[portOrderArrayIdx])
                                    : -1;
                            if (portOrderId < 0)
            ...
            if (propertyIndex > -1)
            {
                if (searchIndentation)
                {
                    ...
                    if (indentationIndex > -1)
                    {
                        ...
                        string indentation =
                            (length > 0)
                                ? m_templateBody.Substring(indentationIndex, length)
                                : string.Empty;
        ...
        {
            TemplateInputData inputData = new TemplateInputData(
                tagStartIdx,
                tagStartIdx,
                tagId,
                portName,
        ...
        public void FillEmptyTags(ref string body)
        {
            ...
            for (int i = 0; i < m_propertyList.Count; i++)
            {
                if (!m_propertyList[i].Used)
                {
                    ...
                    {
                        body = body.Replace(
                            m_propertyList[i].Indentation + m_propertyList[i].Id,
                            string.Empty
                        );
                    }
                }
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    if (m_propertyDict[id].AutoLineFeed)
                    {
                        ...
                        for (int j = 0; j < valuesArr.Length; j++)
                        {
                            ...
                            finalValue +=
                                ((i == 0 && j == 0) ? string.Empty : m_propertyDict[id].Indentation)
                                + valuesArr[j];
                    ...
                    else
                    {
                        //first value will be automatically indented by the string replace
                        finalValue +=
                            (i == 0 ? string.Empty : m_propertyDict[id].Indentation) + values[i];
                    }
                }
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_vertexDataContainer.VertexData[i].DataInfo == info)
                {
                    return string.Format(
                        ...
                        m_vertexFunctionData.InVarName,
                        m_vertexDataContainer.VertexData[i].VarName
                    );
                }
            }
            return string.Empty;
        ...
        public string GetInterpolatedData(TemplateInfoOnSematics info)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_interpolatorDataContainer.Interpolators[i].DataInfo == info)
                {
                    return string.Format(
                        TemplateHelperFunctions.TemplateVarFormat,
                        m_fragmentFunctionData.InVarName,
                        m_interpolatorDataContainer.Interpolators[i].VarName
                    );
                }
            }
            return string.Empty;
        ...
        public string InterpDataId
        {
            get { return m_interpolatorDataContainer.InterpDataId; }
        }
        public string VertexDataId
        {
            get { return m_vertexDataContainer.VertexDataId; }
        }
        public string ShaderNameId
        {
            get { return m_shaderNameId; }
            set { m_shaderNameId = value; }
        }
        public string TemplateBody
        {
            get { return m_templateBody; }
            set { m_templateBody = value; }
        }
        public List<TemplateInputData> InputDataList
        {
            get { return m_inputDataList; }
            set { m_inputDataList = value; }
        }
        public List<TemplateLocalVarData> LocalVarsList
        {
            get { return m_localVarsList; }
        }
        public List<TemplateVertexData> VertexDataList
        {
            get { return m_vertexDataContainer.VertexData; }
        }
        public TemplateInterpData InterpolatorData
        {
            get { return m_interpolatorDataContainer; }
        }
        public TemplateFunctionData VertexFunctionData
        {
            get { return m_vertexFunctionData; }
            set { m_vertexFunctionData = value; }
        }
        public TemplateFunctionData FragmentFunctionData
        {
            get { return m_fragmentFunctionData; }
            set { m_fragmentFunctionData = value; }
        }
        public List<TemplateShaderPropertyData> AvailableShaderProperties
        {
            get { return m_availableShaderProperties; }
            set { m_availableShaderProperties = value; }
        }
        public TemplateBlendData BlendData
        {
            get { return m_blendData; }
            set { m_blendData = value; }
        }
        public TemplateAlphaToMaskData AlphaToMaskData
        {
            get { return m_alphaToMaskData; }
            set { m_alphaToMaskData = value; }
        }
        public TemplateCullModeData CullModeData
        {
            get { return m_cullModeData; }
            set { m_cullModeData = value; }
        }
        public TemplateColorMaskData ColorMaskData
        {
            get { return m_colorMaskData; }
            set { m_colorMaskData = value; }
        }
        public TemplateStencilData StencilData
        {
            get { return m_stencilData; }
            set { m_stencilData = value; }
        }
        public TemplateDepthData DepthData
        {
            get { return m_depthData; }
            set { m_depthData = value; }
        }
        ...
        public VertexDataContainer VertexDataContainer
        {
            get { return m_vertexDataContainer; }
            set { m_vertexDataContainer = value; }
        }
        public TemplateInterpData InterpolatorDataContainer
        {
            get { return m_interpolatorDataContainer; }
            set { m_interpolatorDataContainer = value; }
        }
    }
}
