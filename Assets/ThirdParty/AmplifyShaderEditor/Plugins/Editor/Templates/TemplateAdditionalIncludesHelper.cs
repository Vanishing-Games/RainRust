...
namespace AmplifyShaderEditor
{
    ...
    {
        public TemplateAdditionalIncludesHelper()
            : base("Additional Includes")
        {
            m_helpBoxMessage = "Please add your includes without the #include \"\" keywords";
        }

        public override void AddToDataCollector(
        ...
        )
        {
            ...
            {
                if (
                    !string.IsNullOrEmpty(m_additionalItems[i])
                    && !nativesContainer.HasInclude(m_additionalItems[i])
                )
            ...
            {
                if (
                    !string.IsNullOrEmpty(m_outsideItems[i])
                    && !nativesContainer.HasInclude(m_outsideItems[i])
                )
