...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplateMultiPassMasterNode : MasterNode
    {
        ...
        [SerializeField]
        private TemplateOptionsDefinesContainer m_optionsDefineContainer =
            new TemplateOptionsDefinesContainer();
        ...
        private const string SRPMaterialSubsurfaceScatteringKeyword =
            "_MATERIAL_FEATURE_SUBSURFACE_SCATTERING 1";
        ...
        public override void ReleaseResources()
        {
            ...
            if (!IsLODMainMasterNode)
                return;
            TemplateMultiPass template =
                (m_templateMultiPass == null)
                    ? m_containerGraph.ParentWindow.TemplatesManagerInstance.GetTemplate(
                        m_templateGUID
                    ) as TemplateMultiPass
                    : m_templateMultiPass;
            ...
            if (template != null && template.AvailableShaderProperties != null)
            {
                ...
                {
                    ContainerGraph.ParentWindow.DuplicatePrevBufferInstance.ReleaseUniformName(
                        passUniqueId,
                        template.AvailableShaderProperties[i].PropertyName
                    );
                }
            }
        }
        ...
        void RegisterProperties()
        {
            ...
            if (m_templateMultiPass != null)
            {
                ...
                {
                    m_containerGraph.AddInternalTemplateNode(
                        m_templateMultiPass.AvailableShaderProperties[i]
                    );
                    int nodeId =
                        ContainerGraph.ParentWindow.DuplicatePrevBufferInstance.CheckUniformNameOwner(
                            m_templateMultiPass.AvailableShaderProperties[i].PropertyName
                        );
                    if (nodeId > -1)
                    {
                        if (UniqueId != nodeId)
                        {
                            ...
                            if (node != null)
                            {
                                UIUtils.ShowMessage(
                                    string.Format(
                                        "Template requires property name {0} which is currently being used by {1}. Please rename it and reload template.",
                                        m_templateMultiPass
                                            .AvailableShaderProperties[i]
                                            .PropertyName,
                            ...
                            else
                            {
                                UIUtils.ShowMessage(
                                    string.Format(
                                        "Template requires property name {0} which is currently being on your graph. Please rename it and reload template.",
                                        m_templateMultiPass
                                            .AvailableShaderProperties[i]
                                            .PropertyName
                                    )
        ...
        public override void OnInputPortConnected(
            int portId,
            int otherNodeId,
            int otherPortId,
        ...
        public void ForceTemplateRefresh()
        {
            SetTemplate(
                ...
                m_subShaderIdx,
                m_passIdx,
                SetTemplateSource.HotCodeReload
            );
        }
        ...
        )
        {
            ...
            if (m_templateMultiPass != null)
            {
                string passName = string.IsNullOrEmpty(m_passUniqueId)
                    ? (
                ...
                m_templateMultiPass.GetSubShaderandPassFor(
                    passName,
                    ref newSubShaderIdx,
                    ref newPassIdx
                );
                ...
                else if (m_templateMultiPass.SubShaders[0].MainPass != m_passIdx)
                {
                    SetAdditonalTitleText(
                        string.Format(SubTitleFormatterStr, m_subShaderIdx, m_passIdx)
                    );
                }
                m_invalidNode = false;
                if (
                    m_subShaderIdx >= m_templateMultiPass.SubShaders.Count
                    || m_passIdx >= m_templateMultiPass.SubShaders[m_subShaderIdx].Passes.Count
                )
                {
                    if (DebugConsoleWindow.DeveloperMode)
                        Debug.LogFormat(
                            "Inexisting pass {0}. Cancelling template fetch",
                            m_originalPassName
                        );
                ...
                m_isMainOutputNode = m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .Passes[m_passIdx]
                    .IsMainPass;
                ...
                if (source != SetTemplateSource.HotCodeReload)
                {
                    ...
                    m_isInvisible = m_templateMultiPass
                        .SubShaders[m_subShaderIdx]
                        .Passes[m_passIdx]
                        .IsInvisible
                        ? InvisibilityStatus.LockedInvisible
                        : InvisibilityStatus.Visible;
                }
                else
                {
                    ...
                    if (
                        m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].IsInvisible
                    )
                    {
                ...
                m_invisibleOptions = m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .Passes[m_passIdx]
                    .InvisibleOptions;
                ...
                if (!hotCodeOrRead)
                {
                    if (m_templateMultiPass.SubShaders[m_subShaderIdx].LODContainer.Index > -1)
                    {
                        ...
                        ShaderLOD = Convert.ToInt32(
                            m_templateMultiPass.SubShaders[m_subShaderIdx].LODContainer.Data
                        );
                    }
                    else
                    {
                ...
                //{
                m_subShaderModule.FetchDataFromTemplate(
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Modules
                );
                m_passModule.FetchDataFromTemplate(
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].Modules
                );
                //}
                ...
                if (writeDefaultData)
                {
                    ...
                    m_passName = m_templateMultiPass
                        .SubShaders[m_subShaderIdx]
                        .Passes[m_passIdx]
                        .PassNameContainer
                        .Data;
                    if (
                        !m_templateMultiPass.IsSinglePass /*&& !m_shaderNameIsTitle*/
                    )
                    {
        ...
        bool UpdatePortInfo()
        {
            List<TemplateInputData> inputDataList = m_templateMultiPass
                .SubShaders[m_subShaderIdx]
                .Passes[m_passIdx]
                .InputDataList;
            ...
            if (count != m_inputPorts.Count)
            {
                ...
                {
                    InputPort port = AddInputPort(
                        inputDataList[i].DataType,
                        false,
                        inputDataList[i].PortName,
                        inputDataList[i].OrderId,
                        inputDataList[i].PortCategory,
                        inputDataList[i].PortUniqueId
                    );
                    port.ExternalLinkId = inputDataList[i].LinkId;
                    m_hasLinkPorts =
                        m_hasLinkPorts || !string.IsNullOrEmpty(inputDataList[i].LinkId);
                }
                return true;
            }
            else
        ...
        public void SetPropertyActionFromItem(
            bool actionFromUser,
            TemplateModulesHelper module,
            TemplateActionItem item
        )
        {
            ...
            {
                m_mainMasterNodeRef =
                    m_containerGraph.CurrentMasterNode as TemplateMultiPassMasterNode;
            }

            TemplateModulesHelper subShaderModule = m_isMainOutputNode
                ? m_subShaderModule
                : m_mainMasterNodeRef.SubShaderModule;
            switch (item.PropertyAction)
            {
                case PropertyActionsEnum.CullMode:
                    {
                        ...
                        {
                            module.CullModeHelper.CurrentCullMode = subShaderModule
                                .CullModeHelper
                                .CurrentCullMode;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.CullModeHelper.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.ColorMask:
                    {
                        if (item.CopyFromSubShader)
                            module.ColorMaskHelper.ColorMask = subShaderModule
                                .ColorMaskHelper
                                .ColorMask;
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.ColorMaskHelper.CustomEdited;
                            if (performAction)
                            {
                                module.ColorMaskHelper.CustomEdited = false;
                                module.ColorMaskHelper.ColorMask = item.ColorMask.GetColorMask(
                                    module.ColorMaskHelper.ColorMask
                                );
                            }
                        }
                    }
                ...
                case PropertyActionsEnum.ColorMask1:
                    {
                        if (item.CopyFromSubShader)
                            module.ColorMaskHelper1.ColorMask = subShaderModule
                                .ColorMaskHelper1
                                .ColorMask;
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.ColorMaskHelper1.CustomEdited;
                            if (performAction)
                            {
                                module.ColorMaskHelper1.CustomEdited = false;
                                module.ColorMaskHelper1.ColorMask = item.ColorMask1.GetColorMask(
                                    module.ColorMaskHelper1.ColorMask
                                );
                            }
                        }
                    }
                ...
                case PropertyActionsEnum.ColorMask2:
                    {
                        if (item.CopyFromSubShader)
                            module.ColorMaskHelper2.ColorMask = subShaderModule
                                .ColorMaskHelper2
                                .ColorMask;
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.ColorMaskHelper2.CustomEdited;
                            if (performAction)
                            {
                                module.ColorMaskHelper2.CustomEdited = false;
                                module.ColorMaskHelper2.ColorMask = item.ColorMask2.GetColorMask(
                                    module.ColorMaskHelper2.ColorMask
                                );
                            }
                        }
                    }
                ...
                case PropertyActionsEnum.ColorMask3:
                    {
                        if (item.CopyFromSubShader)
                            module.ColorMaskHelper3.ColorMask = subShaderModule
                                .ColorMaskHelper3
                                .ColorMask;
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.ColorMaskHelper3.CustomEdited;
                            if (performAction)
                            {
                                module.ColorMaskHelper3.CustomEdited = false;
                                module.ColorMaskHelper3.ColorMask = item.ColorMask3.GetColorMask(
                                    module.ColorMaskHelper3.ColorMask
                                );
                            }
                        }
                    }
                ...
                case PropertyActionsEnum.ZWrite:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.DepthOphelper.ZWriteModeValue = subShaderModule
                                .DepthOphelper
                                .ZWriteModeValue;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.DepthOphelper.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.ZTest:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.DepthOphelper.ZTestModeValue = subShaderModule
                                .DepthOphelper
                                .ZTestModeValue;
                        }
                        else
                ...
                case PropertyActionsEnum.ZOffsetFactor:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.DepthOphelper.OffsetFactorValue = subShaderModule
                                .DepthOphelper
                                .OffsetFactorValue;
                        }
                        else
                ...
                case PropertyActionsEnum.ZOffsetUnits:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.DepthOphelper.OffsetUnitsValue = subShaderModule
                                .DepthOphelper
                                .OffsetUnitsValue;
                        }
                        else
                ...
                case PropertyActionsEnum.BlendRGB:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper.SourceFactorRGB = subShaderModule
                                .BlendOpHelper
                                .SourceFactorRGB;
                            module.BlendOpHelper.DestFactorRGB = subShaderModule
                                .BlendOpHelper
                                .DestFactorRGB;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.BlendOpHelper.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.BlendRGB1:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper1.SourceFactorRGB = subShaderModule
                                .BlendOpHelper1
                                .SourceFactorRGB;
                            module.BlendOpHelper1.DestFactorRGB = subShaderModule
                                .BlendOpHelper1
                                .DestFactorRGB;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.BlendOpHelper1.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.BlendRGB2:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper2.SourceFactorRGB = subShaderModule
                                .BlendOpHelper2
                                .SourceFactorRGB;
                            module.BlendOpHelper2.DestFactorRGB = subShaderModule
                                .BlendOpHelper2
                                .DestFactorRGB;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.BlendOpHelper2.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.BlendRGB3:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper3.SourceFactorRGB = subShaderModule
                                .BlendOpHelper3
                                .SourceFactorRGB;
                            module.BlendOpHelper3.DestFactorRGB = subShaderModule
                                .BlendOpHelper3
                                .DestFactorRGB;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading || !module.BlendOpHelper3.CustomEdited;
                            if (performAction)
                            {
                ...
                case PropertyActionsEnum.BlendAlpha:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper.SourceFactorAlpha = subShaderModule
                                .BlendOpHelper
                                .SourceFactorAlpha;
                            module.BlendOpHelper.DestFactorAlpha = subShaderModule
                                .BlendOpHelper
                                .DestFactorAlpha;
                        }
                        else
                        {
                            ...
                            if (performAction)
                            {
                                ...
                                module.BlendOpHelper.SourceFactorAlpha =
                                    item.ActionBlendAlphaSource;
                ...
                case PropertyActionsEnum.BlendAlpha1:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper1.SourceFactorAlpha = subShaderModule
                                .BlendOpHelper1
                                .SourceFactorAlpha;
                            module.BlendOpHelper1.DestFactorAlpha = subShaderModule
                                .BlendOpHelper1
                                .DestFactorAlpha;
                        }
                        else
                        {
                            ...
                            if (performAction)
                            {
                                ...
                                module.BlendOpHelper1.SourceFactorAlpha =
                                    item.ActionBlendAlphaSource1;
                ...
                case PropertyActionsEnum.BlendAlpha2:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper2.SourceFactorAlpha = subShaderModule
                                .BlendOpHelper2
                                .SourceFactorAlpha;
                            module.BlendOpHelper2.DestFactorAlpha = subShaderModule
                                .BlendOpHelper2
                                .DestFactorAlpha;
                        }
                        else
                        {
                            ...
                            if (performAction)
                            {
                                ...
                                module.BlendOpHelper2.SourceFactorAlpha =
                                    item.ActionBlendAlphaSource2;
                ...
                case PropertyActionsEnum.BlendAlpha3:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper3.SourceFactorAlpha = subShaderModule
                                .BlendOpHelper3
                                .SourceFactorAlpha;
                            module.BlendOpHelper3.DestFactorAlpha = subShaderModule
                                .BlendOpHelper3
                                .DestFactorAlpha;
                        }
                        else
                        {
                            ...
                            if (performAction)
                            {
                                ...
                                module.BlendOpHelper3.SourceFactorAlpha =
                                    item.ActionBlendAlphaSource3;
                ...
                case PropertyActionsEnum.BlendOpRGB:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper.BlendOpRGB = subShaderModule
                                .BlendOpHelper
                                .BlendOpRGB;
                        }
                        else
                ...
                case PropertyActionsEnum.BlendOpAlpha:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.BlendOpHelper.BlendOpAlpha = subShaderModule
                                .BlendOpHelper
                                .BlendOpAlpha;
                        }
                        else
                ...
                case PropertyActionsEnum.StencilReference:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.ReferenceValue = subShaderModule
                                .StencilBufferHelper
                                .ReferenceValue;
                        }
                        else
                        {
                            bool performAction =
                                !ContainerGraph.IsLoading
                                || !module.StencilBufferHelper.CustomEdited;
                            if (performAction)
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.ReferenceValue =
                                    item.ActionStencilReference;
                            }
                        }
                    }
                    break;
                case PropertyActionsEnum.StencilReadMask:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.ReadMaskValue = subShaderModule
                                .StencilBufferHelper
                                .ReadMaskValue;
                        }
                        else
                        {
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.ReadMaskValue =
                                    item.ActionStencilReadMask;
                            }
                        }
                    }
                    break;
                case PropertyActionsEnum.StencilWriteMask:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.WriteMaskValue = subShaderModule
                                .StencilBufferHelper
                                .WriteMaskValue;
                        }
                        else
                        {
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.WriteMaskValue =
                                    item.ActionStencilWriteMask;
                            }
                        }
                    }
                    break;
                case PropertyActionsEnum.StencilComparison:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.ComparisonFunctionIdxValue = subShaderModule
                                .StencilBufferHelper
                                .ComparisonFunctionIdxValue;
                        }
                        else
                        {
                            bool performAction =
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.ComparisonFunctionIdxValue =
                ...
                case PropertyActionsEnum.StencilPass:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.PassStencilOpIdxValue = subShaderModule
                                .StencilBufferHelper
                                .PassStencilOpIdxValue;
                        }
                        else
                        {
                            bool performAction =
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.PassStencilOpIdxValue =
                                    item.ActionStencilPass;
                            }
                        }
                    }
                    break;
                case PropertyActionsEnum.StencilFail:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.FailStencilOpIdxValue = subShaderModule
                                .StencilBufferHelper
                                .FailStencilOpIdxValue;
                        }
                        else
                        {
                            bool performAction =
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.FailStencilOpIdxValue =
                                    item.ActionStencilFail;
                            }
                        }
                    }
                    break;
                case PropertyActionsEnum.StencilZFail:
                    {
                        if (item.CopyFromSubShader)
                        {
                            module.StencilBufferHelper.ZFailStencilOpIdxValue = subShaderModule
                                .StencilBufferHelper
                                .ZFailStencilOpIdxValue;
                        }
                        else
                        {
                            bool performAction =
                            ...
                            {
                                module.StencilBufferHelper.CustomEdited = false;
                                module.StencilBufferHelper.ZFailStencilOpIdxValue =
                                    item.ActionStencilZFail;
                            }
                        }
                    }
                    break;
        ...
        public void OnCustomPassOptionSelected(
            bool actionFromUser,
            bool isRefreshing,
            bool invertAction,
            TemplateOptionUIItem uiItem,
            int recursionLevel,
            params TemplateActionItem[] validActions
        )
        {
            m_passOptions.OnCustomOptionSelected(
                actionFromUser,
                isRefreshing,
                invertAction,
                this,
        ...
        public void OnCustomSubShaderOptionSelected(
            bool actionFromUser,
            bool isRefreshing,
            bool invertAction,
            TemplateOptionUIItem uiItem,
        ...
        {
            if (m_isMainOutputNode)
                m_subShaderOptions.OnCustomOptionSelected(
                    actionFromUser,
                    isRefreshing,
                    invertAction,
                    this,
        ...
        {
            TemplateMultiPassMasterNode mainMasterNode = masterNode.IsMainOutputNode
                ? masterNode
                : (m_containerGraph.CurrentMasterNode as TemplateMultiPassMasterNode);
            mainMasterNode.SubShaderOptions.SetSubShaderCustomOptionsPortsInfo(
                masterNode,
        ...
        void SetCategoryIdxFromTemplate()
        {
            ...
            {
                int idx = i + 1;
                TemplateMultiPass templateData =
                    m_containerGraph.ParentWindow.TemplatesManagerInstance.GetTemplate(i)
                    as TemplateMultiPass;
                if (
                    templateData != null
                    && m_templateMultiPass != null
                    && m_templateMultiPass.GUID.Equals(templateData.GUID)
                )
        ...
        public void CheckTemplateChanges()
        {
            ...
            {
                if (
                    m_containerGraph.MultiPassMasterNodes.Count
                    != m_templateMultiPass.MasterNodesRequired
                )
                {
                    ...
                    m_containerGraph.ParentWindow.ReplaceMasterNode(
                        m_availableCategories[m_masterNodeCategory],
                        true
                    );
                }
            }
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            {
                m_subShaderModule.OnLogicUpdate(
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Modules
                );
            }
            ...
            if (
                m_refreshLODValueMasterNodes
                && (EditorApplication.timeSinceStartup - m_refreshLODValueMasterNodesTimestamp)
                    > MaxLODEditTimestamp
            )
            {
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            if (m_invalidNode)
            {
                if (m_isMainOutputNode)
                {
                    ...
                    if (
                        m_containerGraph.ParentWindow.TemplatesManagerInstance.CheckIfDeprecated(
                            m_templateGUID,
                            out newGUID
                        )
                    )
                    ...
                    {
                        UIUtils.ShowMessage(
                            "Invalid current template. Switching to Standard Surface",
                            MessageSeverity.Error
                        );
                        m_shaderModelIdx = 0;
                        m_masterNodeCategory = 0;
                        m_containerGraph.ParentWindow.ReplaceMasterNode(
                            new MasterNodeCategoriesData(
                                AvailableShaderTypes.SurfaceShader,
                                m_shaderName
                            ),
                            false
                        );
            ...
            if (!IsMainOutputNode)
            {
                if (!IsInvisible && Docking)
                {
                    ...
                    TemplateMultiPassMasterNode master =
                        ContainerGraph.CurrentMasterNode as TemplateMultiPassMasterNode;
                    ...
                    if (index > masterIndex)
                    {
                        ...
                        {
                            if (
                                !ContainerGraph.MultiPassMasterNodes.NodesList[i].IsInvisible
                                && ContainerGraph.MultiPassMasterNodes.NodesList[i].Docking
                            )
                                backTracking++;
                        }
            ...
            GUILayout.BeginHorizontal();
            {
                if (GUILayout.Button(OpenTemplateStr) && m_templateMultiPass != null)
                {
                    try
                    {
                        ...
                        {
                            Shader selectedTemplate = AssetDatabase.LoadAssetAtPath<Shader>(
                                pathname
                            );
        ...
        public override void DrawProperties()
        {
            ...
            NodeUtils.DrawPropertyGroup(
                ref m_propertiesFoldout,
                CommonPropertiesStr,
                DrawCommonProperties
            );
            NodeUtils.DrawPropertyGroup(
                ref m_subStringFoldout,
                SubShaderModuleStr,
                DrawSubShaderProperties
            );
            ...
            {
                List<TemplateMultiPassMasterNode> mpNodes = ContainerGraph
        ...
        {
            List<TemplateMultiPassMasterNode> nodes = this.ContainerGraph
                .MultiPassMasterNodes
                .NodesList;
        ...
        void AddHDKeywords()
        {
            ...
            if (
                m_templateMultiPass.SubShaders[m_subShaderIdx].Modules.SRPType
                    != TemplateSRPType.HDRP
                || !m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .Passes[m_passIdx]
                    .Modules
        ...
        void DrawLODAddRemoveButtons(int index, bool showRemove)
        {
            if (GUILayoutButton(string.Empty, UIUtils.PlusStyle, GUILayout.Width(15)))
            {
                ...
                for (int lod = ContainerGraph.LodMultiPassMasternodes.Count - 1; lod >= 0; lod--)
                {
                    if (ContainerGraph.LodMultiPassMasternodes[lod].Count != 0)
                    {
                        minPos = ContainerGraph
                            .LodMultiPassMasternodes[lod]
                            .NodesList[m_passIdx]
                            .Vec2Position;
                        break;
                    }
            ...
            if (
                showRemove && GUILayoutButton(string.Empty, UIUtils.MinusStyle, GUILayout.Width(15))
            )
            {
        ...
        public void DrawLodRowItem(bool listMode)
        {
            ...
            if (listMode)
            {
                ...
                m_shaderLOD = EditorGUILayoutIntField(
                    string.Empty,
                    m_shaderLOD,
                    GUILayout.Width(50)
                );
            ...
            if (listMode)
            {
                m_mainLODName = EditorGUILayoutTextField(
                    string.Empty,
                    m_mainLODName,
                    GUILayout.Width(100)
                );
        ...
        void DrawLOD()
        {
            if (m_templateMultiPass.CanAddLODs && m_lodIndex == -1)
            {
                ...
                for (int i = 0; i < ContainerGraph.LodMultiPassMasternodes.Count; i++)
                {
                    ...
                    {
                        TemplateMultiPassMasterNode masterNode = m_containerGraph
                            .LodMultiPassMasternodes[i]
                            .NodesList[m_passIdx];
        ...
        void DrawCommonProperties()
        {
            if (m_isMainOutputNode)
            {
                ...
                {
                    if (
                        m_templateMultiPass.CustomTemplatePropertyUI
                        == CustomTemplatePropertyUIEnum.HDPBR
                    )
                    {
                        EditorGUI.BeginChangeCheck();
                        CurrentHDMaterialType = (HDSRPMaterialType)EditorGUILayoutEnumPopup(
                            HDSRPMaterialTypeStr,
                            m_hdSrpMaterialType
                        );
                        if (EditorGUI.EndChangeCheck())
                            ConfigHDPorts();
        ...
        public void DrawSubShaderProperties()
        {
            ...
            if (ShaderLOD > 0)
            {
                noValidData = false;
                if (
                    m_templateMultiPass.CanAddLODs
                    && m_containerGraph.LodMultiPassMasternodes[0].Count > 0
                )
                {
        ...
        bool CreateInstructionsForList(
            ...
            ref List<InputPort> ports,
            ref string shaderBody,
            ref List<string> vertexInstructions,
            ref List<string> fragmentInstructions
        )
        {
            if (ports.Count == 0)
                return true;
            ...
            for (int i = 0; i < ports.Count; i++)
            {
                ...
                if (ports[i].HasOwnOrLinkConnection)
                {
                    ...
                    string newPortInstruction = ports[i]
                        .GeneratePortInstructions(ref m_currentDataCollector);

                    if (m_currentDataCollector.DirtySpecialLocalVariables)
                    {
                        string cleanVariables =
                            m_currentDataCollector.SpecialLocalVariables.Replace(
                                "\t",
                                string.Empty
                            );
                    ...
                    {
                        string cleanVariables = m_currentDataCollector.VertexLocalVariables.Replace(
                            "\t",
                            string.Empty
                        );
                        m_currentDataCollector.AddVertexInstruction(
                            cleanVariables,
                            UniqueId,
                            false
                        );
                    ...
                    {
                        fragmentInstructions.Add(
                            m_currentDataCollector.InstructionsList[j].PropertyName
                        );
                    }

                    for (int j = 0; j < m_currentDataCollector.VertexDataList.Count; j++)
                    {
                        vertexInstructions.Add(
                            m_currentDataCollector.VertexDataList[j].PropertyName
                        );
                    }

                    m_templateMultiPass.SetPassInputData(
                        m_subShaderIdx,
                        m_passIdx,
                        ports[i].PortId,
                        newPortInstruction
                    ...
                    isValid =
                        m_templateMultiPass.FillTemplateBody(
                            m_subShaderIdx,
                            m_passIdx,
                            inputData.TagId,
                            ref shaderBody,
        ...
        public string BuildShaderBody(
            MasterNodeDataCollector inDataCollector,
            ref MasterNodeDataCollector outDataCollector
        )
        {
            ...
            m_templateMultiPass.SetShaderName(
                string.Format(TemplatesManager.NameFormatter, m_shaderName)
            );
            ...
            outDataCollector.TemplateDataCollectorInstance.CurrentSRPType =
                m_templateMultiPass.SRPtype;
            ...
            foundExcludePassName = CheckExcludeAllPassOptions(
                m_subShaderOptions,
                out excludePassName
            );
            for (int i = 0; i < count; i++)
            {
                bool removePass =
                    !m_passSelector.IsVisible(i)
                    || (foundExcludePassName && !list[i].OriginalPassName.Equals(excludePassName));
        ...
        public string BuildLOD(
            MasterNodeDataCollector inDataCollector,
            ref MasterNodeDataCollector outDataCollector
        )
        {
            UsageListTemplateMultiPassMasterNodes bufferNodesList =
                ContainerGraph.MultiPassMasterNodes;
            int bufferMasterNodeId = ContainerGraph.CurrentMasterNodeId;

            ContainerGraph.MultiPassMasterNodes = ContainerGraph.LodMultiPassMasternodes[
                m_lodIndex
            ];
        ...
        public override Shader Execute(string pathname, bool isFullPath)
        {
            ...
            if (m_templateMultiPass.CanAddLODs && ContainerGraph.HasLODs)
            {
                for (int lod = 0; lod < ContainerGraph.LodMultiPassMasternodes.Count; lod++)
                {
                    ...
                    TemplateMultiPassMasterNode newMasterNode = ContainerGraph
                        .LodMultiPassMasternodes[lod]
                        .NodesList.Find((x) => x.IsMainOutputNode);
        ...
        public void CollectData()
        {
            ...
            m_currentDataCollector.TemplateDataCollectorInstance.SetMultipassInfo(
                m_templateMultiPass,
                m_subShaderIdx,
                m_passIdx,
                m_templateMultiPass.SubShaders[m_subShaderIdx].Modules.SRPType
            );
            m_currentDataCollector.TemplateDataCollectorInstance.FillSpecialVariables(
                m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx]
            );
            SetupNodeCategories();
            if (m_containerGraph.IsInstancedShader)
            {
                string blockName = UIUtils.RemoveInvalidCharacters(
                    ContainerGraph.GetMainMasterNodeOfLOD(-1).ShaderName
                );
                m_currentDataCollector.SetupInstancePropertiesBlock(blockName);
            }
            TemplateData templateData = m_templateMultiPass.CreateTemplateData(
                m_shaderName,
                string.Empty,
                m_subShaderIdx,
                m_passIdx
            );
            m_currentDataCollector.TemplateDataCollectorInstance.BuildFromTemplateData(
            ...
            if (m_currentDataCollector.TemplateDataCollectorInstance.InterpData.DynamicMax)
            {
                ...
                {
                    TemplateModulesHelper subShaderModule = IsMainOutputNode
                        ? m_subShaderModule
                        : (
                            m_containerGraph.CurrentMasterNode as TemplateMultiPassMasterNode
                        ).SubShaderModule;
                ...
                {
                    m_currentDataCollector.TemplateDataCollectorInstance.InterpData.RecalculateAvailableInterpolators(
                        interpolatorAmount
                    );
                }
            }
            ...
            //Copy Properties
            {
                ...
                {
                    m_currentDataCollector.SoftRegisterUniform(
                        m_templateMultiPass.AvailableShaderProperties[i]
                    );
                }
            }
            ...
            {
                int subShaderGlobalAmount = m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .AvailableShaderGlobals
                    .Count;
                for (int i = 0; i < subShaderGlobalAmount; i++)
                {
                    m_currentDataCollector.SoftRegisterUniform(
                        m_templateMultiPass.SubShaders[m_subShaderIdx].AvailableShaderGlobals[i]
                    );
                }
            }
            //Copy Globals from Pass Level
            {
                int passGlobalAmount = m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .Passes[m_passIdx]
                    .AvailableShaderGlobals
                    .Count;
            ...
            validBody =
                CreateInstructionsForList(
                    ...
                    ref vertexPorts,
                    ref shaderBody,
                    ref vertexInstructions,
                    ref fragmentInstructions
                ) && validBody;
            validBody =
                CreateInstructionsForList(
                    templateData,
            ...
            TerrainDrawInstancedHelper drawInstanced = m_isMainOutputNode
                ? m_drawInstancedHelper
                : m_mainMasterNodeRef.DrawInstancedHelperInstance;
            drawInstanced.UpdateDataCollectorForTemplates(
                ref m_currentDataCollector,
                ref vertexInstructions
            );
            ...
            vertexInstructions.AddRange(
                m_currentDataCollector.TemplateDataCollectorInstance.GetInterpUnusedChannels()
            );

            //Fill common local variables and operations
            validBody =
                m_templateMultiPass.FillVertexInstructions(
                    m_subShaderIdx,
                    m_passIdx,
                    vertexInstructions.ToArray()
                ) && validBody;
            validBody =
                m_templateMultiPass.FillFragmentInstructions(
                    m_subShaderIdx,
                    m_passIdx,
                    fragmentInstructions.ToArray()
                ) && validBody;
            ...
            if (m_containerGraph.IsInstancedShader)
            {
                ...
                string cbufferBegin = m_currentDataCollector.IsSRP
                    ? string.Format(
                        IOUtils.LWSRPInstancedPropertiesBegin,
                        m_currentDataCollector.InstanceBlockName
                    )
                    : string.Format(
                        IOUtils.InstancedPropertiesBegin,
                        m_currentDataCollector.InstanceBlockName
                    );
                string cBufferEnd = m_currentDataCollector.IsSRP
                    ? (
                        string.Format(
                            IOUtils.LWSRPInstancedPropertiesEnd,
                            m_currentDataCollector.InstanceBlockName
                        )
                    )
                    : IOUtils.InstancedPropertiesEnd;

                m_currentDataCollector.InstancedPropertiesList.Insert(
                    0,
                    new PropertyDataCollector(-1, cbufferBegin)
                );
                m_currentDataCollector.InstancedPropertiesList.Add(
                    new PropertyDataCollector(-1, cBufferEnd)
                );
                m_currentDataCollector.UniformsList.AddRange(
                    m_currentDataCollector.InstancedPropertiesList
                );
            }

            if (m_currentDataCollector.DotsPropertiesList.Count > 0)
            {
                m_currentDataCollector.DotsPropertiesList.Insert(
                    0,
                ...
                m_currentDataCollector.DotsPropertiesList.Insert(
                    0,
                    new PropertyDataCollector(-1, "#ifdef UNITY_DOTS_INSTANCING_ENABLED")
                );
                m_currentDataCollector.DotsPropertiesList.Insert(
                    0,
                    new PropertyDataCollector(-1, "")
                );
                m_currentDataCollector.DotsPropertiesList.Add(
                    new PropertyDataCollector(
                        -1,
                ...
                m_currentDataCollector.UniformsList.AddRange(
                    m_currentDataCollector.DotsPropertiesList
                );
                m_currentDataCollector.UniformsList.AddRange(
                    m_currentDataCollector.DotsDefinesList
                );
            }

            TemplateShaderModelModule shaderModelModule = m_isMainOutputNode
                ? m_subShaderModule.ShaderModelHelper
                : m_mainMasterNodeRef.SubShaderModule.ShaderModelHelper;
            ...
            m_currentDataCollector.TemplateDataCollectorInstance.CheckInterpolatorOverflow(
                shaderModel,
                m_passName
            );
        }

        public bool CheckExcludeAllPassOptions(
            TemplateOptionsUIHelper optionsUI,
            out string passName
        )
        {
            ...
            for (int optionIdx = 0; optionIdx < options.Count; optionIdx++)
            {
                ...
                {
                    TemplateActionItem[] actionItems = options[optionIdx]
                        .CurrentOptionActions
                        .Columns;
                    for (int actionIdx = 0; actionIdx < actionItems.Length; actionIdx++)
                    {
                        if (
                            actionItems[actionIdx].ActionType
                            == AseOptionsActionType.ExcludeAllPassesBut
                        )
                        {
            ...
            for (int optionIdx = 0; optionIdx < options.Count; optionIdx++)
            {
                if (options[optionIdx].IsVisible)
                {
                    ...
                    for (int actionIdx = 0; actionIdx < actionItems.Length; actionIdx++)
                    {
                        if (
                            actionItems[actionIdx].ActionType
                                == AseOptionsActionType.SetShaderProperty
                            && !string.IsNullOrEmpty(actionItems[actionIdx].ActionBuffer)
                        )
                        {
                            TemplateShaderPropertyData data =
                                m_templateMultiPass.GetShaderPropertyData(
                                    actionItems[actionIdx].ActionData
                                );
                            if (data != null)
                            {
                                string newPropertyValue = data.CreatePropertyForValue(
                                    actionItems[actionIdx].ActionBuffer
                                );
                                CurrentTemplate.IdManager.SetReplacementText(
                                    data.FullValue,
                                    newPropertyValue
                                );
                                        ...
                                        case WirePortDataType.FLOAT:
                                            {
                                                float value = 0;
                                                if (
                                                    actionItems[actionIdx]
                                                        .GetFloatValueFromActionBuffer(out value)
                                                )
                                                {
                                        ...
                                        case WirePortDataType.INT:
                                            {
                                                ...
                                                if (
                                                    actionItems[actionIdx]
                                                        .GetIntValueFromActionBuffer(out value)
                                                )
                                                {
                                                    CurrentMaterial.SetInt(
                                                        data.PropertyName,
                                                        value
                                                    );
                                                }
                    ...
                    if (options[optionIdx].Options.Type == AseOptionsType.Field)
                    {
                        foreach (var item in CurrentTemplate.IdManager.RegisteredTags)
                        {
                            if (item.Output.Equals(options[optionIdx].Options.FieldInlineName))
                            {
                                ...
                                if (
                                    node != null
                                    && (node.IsConnected || node.AutoRegister || node.UniqueId < -1)
                                    && options[optionIdx].Options.FieldValue.Active
                                )
                                {
        ...
        {
            MasterNodeDataCollector currDataCollector =
                (dataCollector == null) ? m_currentDataCollector : dataCollector;

            // Temporary hack
            if (m_templateMultiPass.SRPtype != TemplateSRPType.BiRP)
            {
                if (
                    m_templateMultiPass.AvailableShaderProperties.Find(x =>
                        x.PropertyName.Equals("_AlphaCutoff")
                    ) == null
                )
                {
                    if (!currDataCollector.ContainsProperty("_AlphaCutoff"))
                    {
                        currDataCollector.AddToProperties(
                            UniqueId,
                ...
                if (
                    m_templateMultiPass.AvailableShaderProperties.Find(x =>
                        x.PropertyName.Equals("_EmissionColor")
                    ) == null
                )
                {
            ...
            m_templateMultiPass.SetPropertyData(
                currDataCollector.BuildUnformatedPropertiesStringArr()
            );
        }
        ...
        )
        {
            ...
            {
                string lodUniqueId =
                    m_templateMultiPass.SubShaders[m_subShaderIdx].UniquePrefix
                    + "Module"
                    + m_templateMultiPass.SubShaders[m_subShaderIdx].LODContainer.Id;
        ...
        public void FillPassData(
            TemplateMultiPassMasterNode masterNode,
            TemplateDataCollector mainTemplateDataCollector
        )
        {
            if (m_isInvisible != InvisibilityStatus.Visible)
            {
                ...
                for (int i = 0; i < inputCount; i++)
                {
                    ...
                    {
                        TemplateMultiPassMasterNode linkedNode =
                            m_inputPorts[i].ExternalLinkNode as TemplateMultiPassMasterNode;
                        if (linkedNode != null)
                        {
            ...
            if (m_currentDataCollector != null)
            {
                ...
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.PassVertexData,
                    m_subShaderIdx,
                    m_passIdx,
                    inputArray
                );
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.PassInterpolatorData,
                    m_subShaderIdx,
                    m_passIdx,
                    m_currentDataCollector.InterpolatorList.ToArray()
                );

                List<PropertyDataCollector> afterNativesIncludePragmaDefineList =
                    new List<PropertyDataCollector>();
                ...
                afterNativesIncludePragmaDefineList.AddRange(
                    m_currentDataCollector.AfterNativeDirectivesList
                );
                ...
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.ModulePragmaBefore,
                    m_subShaderIdx,
                    m_passIdx,
                    beforeNatives
                );
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.ModulePragma,
                    m_subShaderIdx,
                    m_passIdx,
                    afterNativesIncludePragmaDefineList
                );
                ...
                {
                    m_currentDataCollector.FunctionsList.InsertRange(
                        0,
                        m_currentDataCollector.TemplateDataCollectorInstance.LateDirectivesList
                    );
                    m_templateMultiPass.SetPassData(
                        TemplateModuleDataType.ModuleFunctions,
                        m_subShaderIdx,
                        m_passIdx,
                        m_currentDataCollector.FunctionsList
                    );
                }
                else
                {
                    m_currentDataCollector.UniformsList.InsertRange(
                        0,
                        m_currentDataCollector.TemplateDataCollectorInstance.LateDirectivesList
                    );
                    m_currentDataCollector.UniformsList.AddRange(
                ...
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.ModuleInputVert,
                    m_subShaderIdx,
                    m_passIdx,
                    m_currentDataCollector.TemplateDataCollectorInstance.VertexInputParamsStr
                );
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.ModuleInputFrag,
                    m_subShaderIdx,
                    m_passIdx,
                    m_currentDataCollector.TemplateDataCollectorInstance.FragInputParamsStr
                );

                if (
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].TessVControlTag
                        != null
                    && m_templateMultiPass
                        .SubShaders[m_subShaderIdx]
                        .Passes[m_passIdx]
                    ...
                    m_templateMultiPass.SetPassData(
                        TemplateModuleDataType.VControl,
                        m_subShaderIdx,
                        m_passIdx,
                        inputArray
                    );
                ...
                if (
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].TessControlData
                        != null
                    && m_templateMultiPass
                        .SubShaders[m_subShaderIdx]
                        .Passes[m_passIdx]
                    ...
                    m_templateMultiPass.SetPassData(
                        TemplateModuleDataType.ControlData,
                        m_subShaderIdx,
                        m_passIdx,
                        m_templateMultiPass
                            .SubShaders[m_subShaderIdx]
                            .Passes[m_passIdx]
                            .TessControlData.GenerateControl(
                                m_currentDataCollector.TemplateDataCollectorInstance.VertexDataDict,
                                m_currentDataCollector.VertexInputList
                            )
                    );

                if (
                    m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].TessDomainData
                        != null
                    && m_templateMultiPass
                        .SubShaders[m_subShaderIdx]
                        .Passes[m_passIdx]
                    ...
                    m_templateMultiPass.SetPassData(
                        TemplateModuleDataType.DomainData,
                        m_subShaderIdx,
                        m_passIdx,
                        m_templateMultiPass
                            .SubShaders[m_subShaderIdx]
                            .Passes[m_passIdx]
                            .TessDomainData.GenerateDomain(
                                m_currentDataCollector.TemplateDataCollectorInstance.VertexDataDict,
                                m_currentDataCollector.VertexInputList
                            )
                    );
            ...
            m_templateMultiPass.SetPassData(
                TemplateModuleDataType.PassNameData,
                m_subShaderIdx,
                m_passIdx,
                string.Format(PassNameFormateStr, m_passName)
            );
        }

        public List<PropertyDataCollector> CrossCheckSoftRegisteredUniformList(
            List<PropertyDataCollector> uniformList
        )
        {
        ...
        public void FillUniforms(TemplateDataCollector mainTemplateDataCollector)
        {
            if (m_currentDataCollector.IsSRP)
            {
                ...
                {
                    List<PropertyDataCollector> finalList = CrossCheckSoftRegisteredUniformList(
                        mainTemplateDataCollector.SrpBatcherPropertiesList
                    );
                    m_templateMultiPass.SetPassData(
                        TemplateModuleDataType.ModuleSRPBatcher,
                        m_subShaderIdx,
                        m_passIdx,
                        finalList
                    );
                ...
                {
                    List<PropertyDataCollector> finalList = CrossCheckSoftRegisteredUniformList(
                        mainTemplateDataCollector.FullSrpBatcherPropertiesList
                    );
            ...
            m_templateMultiPass.SetPassData(
                TemplateModuleDataType.ModuleGlobals,
                m_subShaderIdx,
                m_passIdx,
                m_currentDataCollector.UniformsList
            );
        }
        ...
        void SetLinkedModuleData(TemplateModulesHelper linkedModule)
        {
            ...
            if (linkedModule.AdditionalDirectives.ValidData)
            {
                ...
                linkedModule.AdditionalDirectives.AddAllToDataCollector(
                    ref m_currentDataCollector,
                    pass,
                    pass.Modules.IncludePragmaContainer
                );
            }
        }
        ...
        void SetModuleData(TemplateModulesHelper module, bool isSubShader)
        {
            if (isSubShader)
            {
                ...
                {
                    module.AdditionalDirectives.AddAllToDataCollector(
                        ref m_currentDataCollector,
                        null,
                        m_templateMultiPass
                            .SubShaders[m_subShaderIdx]
                            .Modules
                            .IncludePragmaContainer
                    );
                }

                if (module.TagsHelper.ValidData)
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleTag,
                        m_subShaderIdx,
                        module.TagsHelper.GenerateTags()
                    );
                }
                ...
                if (module.AllModulesMode)
                {
                    ...
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.AllModules,
                        m_subShaderIdx,
                        body.Split('\n')
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleShaderModel,
                        m_subShaderIdx,
                        module.ShaderModelHelper.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendMode,
                        m_subShaderIdx,
                        module.BlendOpHelper.CurrentBlendFactor
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendMode1,
                        m_subShaderIdx,
                        module.BlendOpHelper1.CurrentBlendFactor
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendMode2,
                        m_subShaderIdx,
                        module.BlendOpHelper2.CurrentBlendFactor
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendMode3,
                        m_subShaderIdx,
                        module.BlendOpHelper3.CurrentBlendFactor
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendOp,
                        m_subShaderIdx,
                        module.BlendOpHelper.CurrentBlendOp
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendOp1,
                        m_subShaderIdx,
                        module.BlendOpHelper1.CurrentBlendOp
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendOp2,
                        m_subShaderIdx,
                        module.BlendOpHelper2.CurrentBlendOp
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleBlendOp3,
                        m_subShaderIdx,
                        module.BlendOpHelper3.CurrentBlendOp
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleAlphaToMask,
                        m_subShaderIdx,
                        module.AlphaToMaskHelper.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleCullMode,
                        m_subShaderIdx,
                        module.CullModeHelper.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleColorMask,
                        m_subShaderIdx,
                        module.ColorMaskHelper.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleColorMask1,
                        m_subShaderIdx,
                        module.ColorMaskHelper1.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleColorMask2,
                        m_subShaderIdx,
                        module.ColorMaskHelper2.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleColorMask3,
                        m_subShaderIdx,
                        module.ColorMaskHelper3.GenerateShaderData(isSubShader)
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleZTest,
                        m_subShaderIdx,
                        module.DepthOphelper.CurrentZTestMode
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleZwrite,
                        m_subShaderIdx,
                        module.DepthOphelper.CurrentZWriteMode
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleZOffset,
                        m_subShaderIdx,
                        module.DepthOphelper.CurrentOffset
                    );
                }
                ...
                {
                    CullMode cullMode =
                        (module.CullModeHelper.ValidData)
                            ? module.CullModeHelper.CurrentCullMode
                            : CullMode.Back;
                    string value = module.StencilBufferHelper.CreateStencilOp(cullMode);
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleStencil,
                        m_subShaderIdx,
                        value.Split('\n')
                    );
                }
                ...
                {
                    m_templateMultiPass.SetSubShaderData(
                        TemplateModuleDataType.ModuleRenderPlatforms,
                        m_subShaderIdx,
                        module.RenderingPlatforms.CreateResult(true)
                    );
                }
            }
            else
            {
                ...
                m_templateMultiPass.SetPassData(
                    TemplateModuleDataType.ModulePass,
                    m_subShaderIdx,
                    m_passIdx,
                    aboveUsePass
                );
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (!m_invalidNode)
                try
                {
                    ...
                    m_subShaderModule.ReadFromString(
                        m_templateMultiPass.SubShaders[m_subShaderIdx].Modules,
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                    m_passModule.ReadFromString(
                        m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx].Modules,
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                    ...
                    if (UIUtils.CurrentShaderVersion() > 15308)
                    {
                        ...
                        m_dependenciesHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    {
                        m_hdSrpMaterialType = (HDSRPMaterialType)
                            Enum.Parse(typeof(HDSRPMaterialType), GetCurrentParam(ref nodeParams));
                    }
                    ...
                    if (UIUtils.CurrentShaderVersion() > 15501)
                    {
                        if (
                            m_isMainOutputNode
                            && UIUtils.CurrentShaderVersion() > PASS_SELECTOR_VERSION
                        )
                            m_subShaderOptions.ReadFromString(
                                ref m_currentReadParamIdx,
                                ref nodeParams
                            );
                    ...
                    {
                        m_drawInstancedHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    if (
                        m_isMainOutputNode
                        && UIUtils.CurrentShaderVersion() > LOD_SUBSHADER_VERSION
                    )
                    {
        ...
        void CheckLegacyCustomInspectors()
        {
            ...
            if (
                m_templateMultiPass.SubShaders[0].Modules.SRPType == TemplateSRPType.HDRP
                && ASEPackageManagerHelper.CurrentHDRPBaseline >= ASESRPBaseline.ASE_SRP_11
            )
            {
                if (Constants.CustomInspectorHDLegacyTo11.ContainsKey(m_customInspectorName))
                {
                    UIUtils.ShowMessage(
                        string.Format(
                            "Detected obsolete custom inspector '{0}' in shader meta. Converting to new one '{1}'",
                            m_customInspectorName,
                            Constants.CustomInspectorHDLegacyTo11[m_customInspectorName]
                        ),
                        MessageSeverity.Warning
                    );
                    m_customInspectorName = Constants.CustomInspectorHDLegacyTo11[
                        m_customInspectorName
                    ];
                }
            }
            ...
            if (
                m_templateMultiPass.SubShaders[0].Modules.SRPType == TemplateSRPType.URP
                && ASEPackageManagerHelper.CurrentURPBaseline >= ASESRPBaseline.ASE_SRP_12
            )
            {
            ...
            if (
                m_templateMultiPass.SubShaders[0].Modules.SRPType == TemplateSRPType.HDRP
                && ASEPackageManagerHelper.CurrentHDRPBaseline >= ASESRPBaseline.ASE_SRP_10
            )
            {
                if (Constants.CustomInspectorHD7To10.ContainsKey(m_customInspectorName))
                {
                    UIUtils.ShowMessage(
                        string.Format(
                            "Detected obsolete custom inspector '{0}' in shader meta. Converting to new one '{1}'",
                            m_customInspectorName,
                            Constants.CustomInspectorHD7To10[m_customInspectorName]
                        ),
                        MessageSeverity.Warning
                    );
        ...
        public override void ReadFromDeprecated(ref string[] nodeParams, Type oldType = null)
        {
            ...
            try
            {
                ...
                TemplateMultiPass template =
                    m_containerGraph.ParentWindow.TemplatesManagerInstance.GetTemplate(templateGUID)
                    as TemplateMultiPass;
                if (template != null)
                {
                ...
                {
                    template =
                        m_containerGraph.ParentWindow.TemplatesManagerInstance.GetTemplateByName(
                            templateShaderName
                        ) as TemplateMultiPass;
                    if (template != null)
                    {
                ...
                if (UIUtils.CurrentShaderVersion() > 13902)
                {
                    ...
                    {
                        m_subShaderModule.BlendOpHelper.ReadBlendModeFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.BlendData.ValidBlendMode
                    )
                    {
                        m_passModule.BlendOpHelper.ReadBlendModeFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    {
                        m_subShaderModule.BlendOpHelper.ReadBlendOpFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.BlendData.ValidBlendOp
                    )
                    {
                    ...
                    //CULL MODE
                    if (
                        m_templateMultiPass.SubShaders[0].Modules.CullModeData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_subShaderModule.CullModeHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.CullModeData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_passModule.CullModeHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    //COLOR MASK
                    if (
                        m_templateMultiPass.SubShaders[0].Modules.ColorMaskData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_subShaderModule.ColorMaskHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.ColorMaskData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_passModule.ColorMaskHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    //STENCIL BUFFER
                    if (
                        m_templateMultiPass.SubShaders[0].Modules.StencilData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_subShaderModule.StencilBufferHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.StencilData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_passModule.StencilBufferHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                ...
                if (UIUtils.CurrentShaderVersion() > 14202)
                {
                    ...
                    {
                        m_subShaderModule.DepthOphelper.ReadZWriteFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.DepthData.ValidZWrite
                    )
                    {
                        m_passModule.DepthOphelper.ReadZWriteFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    {
                        m_subShaderModule.DepthOphelper.ReadZTestFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.DepthData.ValidZTest
                    )
                    {
                        m_subShaderModule.DepthOphelper.ReadZTestFromString(
                            ref m_currentReadParamIdx,
                    ...
                    {
                        m_subShaderModule.DepthOphelper.ReadOffsetFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.DepthData.ValidOffset
                    )
                    {
                        m_passModule.DepthOphelper.ReadOffsetFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                ...
                {
                    if (
                        m_templateMultiPass.SubShaders[0].Modules.TagData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_subShaderModule.TagsHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    else if (
                        m_templateMultiPass.SubShaders[0].Passes[0].Modules.TagData.DataCheck
                        == TemplateDataCheck.Valid
                    )
                    {
                        m_passModule.TagsHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
        ...
        {
            TemplateMultiPassMasterNode node = m_containerGraph.GetMasterNodeOfPass(
                passName,
                m_lodIndex
            );
            if (node != null)
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (
                ...
                && m_templateMultiPass
                    ...
                    .Modules
                    .SRPIsPBRHD
                && UIUtils.CurrentShaderVersion() < 15410
            )
            {
                FetchHDPorts();
                m_hdSrpMaterialType =
                    (m_specularPort != null && m_specularPort.HasOwnOrLinkConnection)
                        ? HDSRPMaterialType.Specular
                        : HDSRPMaterialType.Standard;
                ConfigHDPorts();
            }
            ...
            if (m_isMainOutputNode)
            {
                List<TemplateMultiPassMasterNode> masterNodes =
                    (m_lodIndex == -1)
                        ? m_containerGraph.MultiPassMasterNodes.NodesList
                        : m_containerGraph.LodMultiPassMasternodes[m_lodIndex].NodesList;
                ...
                {
                    UIUtils.ShowMessage(
                        "Template master nodes amount was modified. Could not set correctly its visibility options."
                    );
                }
                else
                {
        ...
        public override void ReadInputDataFromString(ref string[] nodeParams)
        {
            ...
            for (
                int i = 0;
                i < count && i < nodeParams.Length && m_currentReadParamIdx < nodeParams.Length;
                i++
            )
            {
                if (UIUtils.CurrentShaderVersion() < 5003)
                {
                    ...
                    m_currentReadParamIdx++;
                    if (
                        m_inputPorts[newId].IsEditable
                        && UIUtils.CurrentShaderVersion() >= 3100
                        && m_currentReadParamIdx < nodeParams.Length
                    )
                    {
                        m_currentReadParamIdx++;
                    }
        ...
        {
            get { return m_templateMultiPass.SubShaders[m_subShaderIdx]; }
        }
        public TemplatePass Pass
        {
            get { return m_templateMultiPass.SubShaders[m_subShaderIdx].Passes[m_passIdx]; }
        }
        public int SubShaderIdx
        {
            get { return m_subShaderIdx; }
        }
        public int PassIdx
        {
            get { return m_passIdx; }
        }
        public TemplateMultiPass CurrentTemplate
        {
            get { return m_templateMultiPass; }
        }
        public TemplateModulesHelper SubShaderModule
        {
            get { return m_subShaderModule; }
        }
        public TemplateModulesHelper PassModule
        {
            get { return m_passModule; }
        }
        public string PassName
        {
            get
            {
                return m_templateMultiPass
                    .SubShaders[m_subShaderIdx]
                    .Passes[m_passIdx]
                    .PassNameContainer
                    .Data;
        ...
        public string OriginalPassName
        {
            get { return m_originalPassName; }
        }
        public bool HasLinkPorts
        {
            get { return m_hasLinkPorts; }
        }
        public bool IsInvisible
        {
            get { return m_isInvisible != InvisibilityStatus.Visible; }
            set
            {
        ...
        public TemplatePassSelectorHelper PassSelector
        {
            get { return m_passSelector; }
        }
        public TemplateOptionsUIHelper PassOptions
        {
            get { return m_passOptions; }
        }
        public TemplateOptionsUIHelper SubShaderOptions
        {
            get { return m_subShaderOptions; }
        }
        public TemplateOptionsDefinesContainer OptionsDefineContainer
        {
            get { return m_optionsDefineContainer; }
        }
        public TerrainDrawInstancedHelper DrawInstancedHelperInstance
        {
            get { return m_drawInstancedHelper; }
        }
        public bool InvalidNode
        {
            get { return m_invalidNode; }
        }
        ...
        public bool IsLODMainFirstPass
        {
            get { return m_passIdx == 0 && m_lodIndex == -1; }
        }
        public override AvailableShaderTypes CurrentMasterNodeCategory
        {
            get { return AvailableShaderTypes.Template; }
        }
    }
}
