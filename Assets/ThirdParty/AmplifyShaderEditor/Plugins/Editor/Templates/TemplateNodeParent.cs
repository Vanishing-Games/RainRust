...
namespace AmplifyShaderEditor
{
    ...
    {
        protected const string ErrorMessageStr =
            "This node cannot be used with Surface Shaders or Shader Functions.\n\nPlease change your Shader Type to a compatible template, such as Legacy, URP, HDRP or other.";
        ...
        protected void DrawSubShaderUI()
        {
            ...
            m_subShaderIdx = EditorGUILayoutPopup(
                SubShaderStr,
                m_subShaderIdx,
                m_availableSubshaders
            );
        ...
        {
            EditorGUI.BeginChangeCheck();
            m_passLocalArrayIdx = EditorGUILayoutPopup(
                PassStr,
                m_passLocalArrayIdx,
                m_availablePassesLabels
            );
            if (EditorGUI.EndChangeCheck())
            {
        ...
        protected virtual void CheckWarningState()
        {
        ...
        protected void FetchMultiPassTemplate(MasterNode masterNode = null)
        {
            ...
            {
                m_templateMPData = (
                    ((masterNode == null) ? m_containerGraph.CurrentMasterNode : masterNode)
        ...
        protected virtual bool ValidatePass(int passIdx)
        {
            return true;
        }
