    ...
    public class TemplateFragmentDataNode : TemplateNodeParent
    {
        ...
        )
        {
            if (dataCollector.MasterNodeCategory != AvailableShaderTypes.Template)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Template Fragmment Data node is only intended for templates use only",
            ...
            if (!dataCollector.IsFragmentCategory)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Template Fragment Data node node is only intended for fragment use use only",
                    MessageSeverity.Error
                );
                return m_outputPorts[0].ErrorValue;
            }
        ...
        {
            return (
                m_templateMPData.SubShaders[SubShaderIdx].Passes[passIdx].FragmentFunctionData
                    != null
                && m_templateMPData
                    .SubShaders[SubShaderIdx]
                    .Passes[passIdx]
                    .InterpolatorDataContainer != null
            );
        }
