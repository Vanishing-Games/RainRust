...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplateMultiPassSwitchNode : TemplateNodeParent
    {
                        ...
                        for (int passIdx = 0; passIdx < passCount; passIdx++)
                        {
                            if (
                                m_templateMPData
                                    ...
                                    .Passes[passIdx]
                                    .HasValidFunctionBody
                            )
                            {
                                m_inputPorts[index].Name = string.Format(
                                    InputLabelStr,
                                    subShaderIdx,
                                    passIdx
                                );
        ...
        )
        {
            ...
            {
                UIUtils.ShowMessage(
                    "Template Multi-Pass Switch Data node is only intended for templates use only",
                    MessageSeverity.Error
                );
                return m_outputPorts[0].ErrorValue;
            }

            int currSubShaderIdx = dataCollector
                .TemplateDataCollectorInstance
                .MultipassSubshaderIdx;
            ...
            {
                if (
                    m_inputHelper[i].SubShaderIdx == currSubShaderIdx
                    && m_inputHelper[i].PassIdx == currPassIdx
                )
            ...
            if (m_multiPassMode)
            {
                if (m_templateMPData != null)
                {
                    ...
                    else
                    {
                        ...
                        for (int subShaderIdx = 0; subShaderIdx < subShaderCount; subShaderIdx++)
                        {
                            ...
                            for (int passIdx = 0; passIdx < passCount; passIdx++)
                            {
                                ...
                                )
                                {
                                    ...
                                    m_inputHelper.Add(
                                        new InputSwitchMPHelper(subShaderIdx, passIdx)
                                    );
                                    index += 1;
                                }
                            }
                        ...
                        {
                            Debug.LogWarning(
                                "Something wrong occured in reading MultiPass Switch node"
                            );
                        }
                    }
                }
