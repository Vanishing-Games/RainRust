...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateOptionsUIHelper
    {
        ...
        [NonSerialized]
        private Dictionary<string, TemplateOptionUIItem> m_passCustomOptionsUIDict =
            new Dictionary<string, TemplateOptionUIItem>();
        ...
        [SerializeField]
        private List<TemplateOptionPortItem> m_passCustomOptionsPorts =
            new List<TemplateOptionPortItem>();
        ...
        public void DrawCustomOptions(TemplateMultiPassMasterNode owner)
        {
            ...
            if (m_passCustomOptionsUI.Count > 0)
            {
                NodeUtils.DrawNestedPropertyGroup(
                    ref m_passCustomOptionsFoldout,
                    m_passCustomOptionsLabel,
        ...
        public void DrawCustomOptionsBlock()
        {
            ...
            float size = Mathf.Max(
                UIUtils.CurrentWindow.ParametersWindow.TransformedArea.width * 0.385f,
                0
            );
        ...
        private InputPort MatchInputPortForTemplateAction(
            TemplateMultiPassMasterNode passMasterNode,
            TemplateActionItem action
        )
        {
            InputPort port;
        ...
        private bool TestActionItemConditional(TemplateActionItem actionItem)
        {
            ...
            {
                TemplateOptionUIItem referenceItem = m_passCustomOptionsUI.Find(x =>
                    (x.Options.Name.Equals(conditional.Option))
                );
                if (referenceItem != null)
                {
                    bool equal = conditional.Value.Equals(
                        referenceItem.Options.Options[referenceItem.CurrentOption]
                    );
                    if (conditional.Condition == TemplateActionItemConditional.Conditional.Equal)
                    {
                        ...
                        conditional.Condition == TemplateActionItemConditional.Conditional.NotEqual
                    )
                    {
                        succeeded = !equal;
                    }
        ...
        public void OnCustomOptionSelected(
            bool actionFromUser,
            bool isRefreshing,
            bool invertAction,
            TemplateMultiPassMasterNode owner,
        ...
        )
        {
            ...
            for (int i = 0; i < validActions.Length; i++)
            {
                ...
                {
                    if (
                        !TemplateOptionsToolsHelper.InvertAction(
                            validActions[i].ActionType,
                            ref actionType
                        )
                    )
                    {
                    ...
                    case AseOptionsActionType.RefreshOption:
                        {
                            ...
                            TemplateOptionUIItem item = m_passCustomOptionsUI.Find(x =>
                                (x.Options.Name.Equals(validActions[i].ActionData))
                            );
                            if (item != null)
                            {
                            ...
                            {
                                Debug.LogFormat(
                                    "Could not find Option {0} for action '{1}' on template {2}",
                                    validActions[i].ActionData,
                                    validActions[i].ActionType,
                                    owner.CurrentTemplate.DefaultShaderName
                                );
                            }
                        }
                        break;
                    case AseOptionsActionType.ShowOption:
                        {
                            ...
                            if (item != null)
                            {
                                ...
                                {
                                    string optionId =
                                        validActions[i].PassName
                                        + validActions[i].ActionData
                                        + "Option";
                                    owner.ContainerGraph.ParentWindow.TemplatesManagerInstance.SetOptionsValue(
                                        optionId,
                                        true
                                    );
                                }
                    ...
                    case AseOptionsActionType.HidePort:
                        {
                            ...
                            {
                                passMasterNode = owner.ContainerGraph.GetMasterNodeOfPass(
                                    validActions[i].PassName,
                                    owner.LODIndex
                                );
                            }
                            ...
                            {
                                InputPort port = MatchInputPortForTemplateAction(
                                    passMasterNode,
                                    validActions[i]
                                );
                                if (port != null)
                                ...
                                {
                                    Debug.LogFormat(
                                        "Could not find port {0},{1} for action '{2}' on template {3}",
                                        validActions[i].ActionDataIdx,
                                        validActions[i].ActionData,
                                        validActions[i].ActionType,
                                        owner.CurrentTemplate.DefaultShaderName
                                    );
                            ...
                            {
                                Debug.LogFormat(
                                    "Could not find pass {0} for action {1} '{2}' on template {3}",
                                    validActions[i].PassName,
                                    validActions[i].ActionType,
                                    validActions[i].ActionData,
                                    owner.CurrentTemplate.DefaultShaderName
                    ...
                    case AseOptionsActionType.SetPortName:
                        {
                            ...
                            {
                                Debug.LogFormat(
                                    "Could not find pass {0}, {1} for action '{2}' on template {3}",
                                    validActions[i].PassName,
                                    validActions[i].ActionType,
                                    validActions[i].ActionData,
                                    owner.CurrentTemplate.DefaultShaderName
                    ...
                    case AseOptionsActionType.SetDefine:
                        {
                            ...
                            if (validActions[i].AllPasses)
                            {
                                ...
                                List<TemplateMultiPassMasterNode> nodes =
                                    owner.ContainerGraph.GetMultiPassMasterNodes(owner.LODIndex);
                                int count = nodes.Count;
                                for (int nodeIdx = 0; nodeIdx < count; nodeIdx++)
                                {
                                    nodes[nodeIdx]
                                        .OptionsDefineContainer.AddDirective(
                                            defineValue,
                                            false,
                                            isPragma
                                        );
                            ...
                            {
                                TemplateMultiPassMasterNode passMasterNode =
                                    owner.ContainerGraph.GetMasterNodeOfPass(
                                        validActions[i].PassName,
                                        owner.LODIndex
                                    );
                                if (passMasterNode != null)
                                {
                                    ...
                                    passMasterNode.OptionsDefineContainer.AddDirective(
                                        defineValue,
                                        false,
                                        isPragma
                                    );
                    ...
                    case AseOptionsActionType.RemoveDefine:
                        {
                            ...
                            if (validActions[i].AllPasses)
                            {
                                ...
                                if (!flag)
                                {
                                    ...
                                    {
                                        nodes[nodeIdx]
                                            .OptionsDefineContainer.RemoveDirective(defineValue);
                                    }
                                }
                            }
                            else if (!string.IsNullOrEmpty(validActions[i].PassName))
                            {
                                ...
                                if (passMasterNode != null)
                                {
                                    ...
                                    if (!flag)
                                    {
                                        passMasterNode.OptionsDefineContainer.RemoveDirective(
                                            defineValue
                                        );
                                    }
                                }
                    ...
                    case AseOptionsActionType.SetUndefine:
                        {
                            ...
                            if (validActions[i].AllPasses)
                            {
                                ...
                                {
                                    nodes[nodeIdx]
                                        .OptionsDefineContainer.AddDirective(defineValue, false);
                                }
                            }
                            else if (!string.IsNullOrEmpty(validActions[i].PassName))
                            {
                    ...
                    case AseOptionsActionType.SetPropertyOnPass:
                        {
                            ...
                            if (!string.IsNullOrEmpty(validActions[i].PassName))
                            {
                                ...
                                if (passMasterNode != null)
                                {
                                    passMasterNode.SetPropertyActionFromItem(
                                        actionFromUser,
                                        passMasterNode.PassModule,
                                        validActions[i]
                                    );
                                }
                                else
                            ...
                            {
                                owner.SetPropertyActionFromItem(
                                    actionFromUser,
                                    owner.PassModule,
                                    validActions[i]
                                );
                    ...
                    case AseOptionsActionType.SetMaterialProperty:
                        {
                            ...
                            if (owner.ContainerGraph.CurrentMaterial != null)
                            {
                                ...
                                {
                                    if (
                                        uiItem.Options.UIWidget == AseOptionsUIWidget.Float
                                        || uiItem.Options.UIWidget == AseOptionsUIWidget.FloatRange
                                    )
                                        owner.ContainerGraph.CurrentMaterial.SetFloat(
                                            prop,
                                            uiItem.CurrentFieldValue
                                        );
                                    else
                                        owner.ContainerGraph.CurrentMaterial.SetInt(
                                            prop,
                                            (int)uiItem.CurrentFieldValue
                                        );
        ...
        public void SetupCustomOptionsFromTemplate(
            TemplateMultiPassMasterNode owner,
            bool newTemplate
        )
        {
            TemplateOptionsContainer customOptionsContainer = m_isSubShader
                ? owner.SubShader.CustomOptionsContainer
                : owner.Pass.CustomOptionsContainer;
            ...
            if (!newTemplate && customOptionsContainer.Body.Length == m_passCustomOptionsSizeCheck)
            {
                for (int i = 0; i < m_passCustomOptionsUI.Count; i++)
                {
                    if (m_passCustomOptionsUI[i].EmptyEvent)
                    {
                        if (m_isSubShader)
                        {
                            m_passCustomOptionsUI[i].OnActionPerformedEvt +=
                                owner.OnCustomSubShaderOptionSelected;
                        }
                        else
                        {
                            m_passCustomOptionsUI[i].OnActionPerformedEvt +=
                                owner.OnCustomPassOptionSelected;
                        }
                    }
                }
                return;
            ...
            m_passCustomOptionsLabel = string.IsNullOrEmpty(customOptionsContainer.Name)
                ? CustomOptionsLabel
                : " " + customOptionsContainer.Name;
                ...
                for (int i = 0; i < customOptionsContainer.Options.Length; i++)
                {
                    switch (customOptionsContainer.Options[i].Type)
                    {
                        case AseOptionsType.Option:
                            {
                                TemplateOptionUIItem item = new TemplateOptionUIItem(
                                    customOptionsContainer.Options[i]
                                );
                                if (m_isSubShader)
                                {
                                    item.OnActionPerformedEvt +=
                                        owner.OnCustomSubShaderOptionSelected;
                                }
                                else
                                {
                                ...
                                m_passCustomOptionsUIDict.Add(
                                    customOptionsContainer.Options[i].Id,
                                    item
                                );
                            }
                            break;
                        case AseOptionsType.Port:
                            {
                                TemplateOptionPortItem item = new TemplateOptionPortItem(
                                    owner,
                                    customOptionsContainer.Options[i]
                                );
        ...
        public void SetCustomOptionsInfo(
        ...
        public void SetSubShaderCustomOptionsPortsInfo(
            TemplateMultiPassMasterNode masterNode,
            ref MasterNodeDataCollector dataCollector
        )
        {
            ...
            for (int i = 0; i < m_passCustomOptionsPorts.Count; i++)
            {
                m_passCustomOptionsPorts[i]
                    .SubShaderFillDataCollector(masterNode, ref dataCollector);
            }
        }
        ...
        public void RefreshCustomOptionsDict()
        {
            if (m_passCustomOptionsUIDict.Count != m_passCustomOptionsUI.Count)
            {
                ...
                {
                    m_passCustomOptionsUIDict.Add(
                        m_passCustomOptionsUI[i].Options.Id,
                        m_passCustomOptionsUI[i]
                    );
                }
            }
        }
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            for (int i = 0; i < savedOptions; i++)
            {
                ...
                Int64 optionTimestamp =
                    (UIUtils.CurrentShaderVersion() > 18929)
                        ? Convert.ToInt64(nodeParams[index++])
                        : 0;
                m_readOptions.Add(
                    new ReadOptions()
                    {
            ...
            for (int i = 0; i < optionsCount; i++)
            {
                ...
                if (m_passCustomOptionsUI[i].Options.Type == AseOptionsType.Field)
                    IOUtils.AddFieldValueToString(
                        ref nodeInfo,
                        m_passCustomOptionsUI[i].FieldValue.WriteToSingle()
                    );
                else
                    IOUtils.AddFieldValueToString(
                        ref nodeInfo,
                        m_passCustomOptionsUI[i].CurrentOption
                    );

                IOUtils.AddFieldValueToString(
                    ref nodeInfo,
                    m_passCustomOptionsUI[i].LastClickedTimestamp
                );
            }
        }
        ...
        public void SetReadOptions()
        {
            if (m_readOptions != null)
            {
                for (int i = 0; i < m_readOptions.Count; i++)
                {
                    ...
                    {
                        m_passCustomOptionsUIDict[m_readOptions[i].Name].LastClickedTimestamp =
                            m_readOptions[i].Timestamp;
                        if (
                            m_passCustomOptionsUIDict[m_readOptions[i].Name].Options.Type
                            == AseOptionsType.Field
                        )
                        {
                            m_passCustomOptionsUIDict[m_readOptions[i].Name]
                                .FieldValue.ReadFromSingle(m_readOptions[i].Selection);
                            foreach (
                                var item in m_passCustomOptionsUIDict[m_readOptions[i].Name]
                                    .Options
                                    .ActionsPerOption
                                    .Rows
                            )
                            {
                                if (
                                    ...
                                    && item.Columns[0].ActionType
                                        == AseOptionsActionType.SetMaterialProperty
                                )
                                {
                                    if (UIUtils.CurrentWindow.CurrentGraph.CurrentMaterial != null)
                                    {
                                        if (
                                            UIUtils.CurrentWindow.CurrentGraph.CurrentMaterial.HasProperty(
                                                item.Columns[0].ActionData
                                            )
                                        )
                                        {
                                            m_passCustomOptionsUIDict[
                                                m_readOptions[i].Name
                                            ].CurrentFieldValue =
                                                UIUtils.CurrentWindow.CurrentGraph.CurrentMaterial.GetFloat(
                                                    item.Columns[0].ActionData
                                                );
                                        }
                                    }
                            ...
                            m_passCustomOptionsUIDict[m_readOptions[i].Name].CurrentOptionIdx =
                                Convert.ToInt32(m_readOptions[i].Selection);
                    }
                }
            }
        }
        ...
        public void Refresh()
        {
            ...
            //}
            List<TemplateOptionUIItem> sortedList = m_passCustomOptionsUI
                .OrderBy(item => item.LastClickedTimestamp)
                .ToList();
        ...
        public List<TemplateOptionUIItem> PassCustomOptionsUI
        {
            get { return m_passCustomOptionsUI; }
        }
    }
}
