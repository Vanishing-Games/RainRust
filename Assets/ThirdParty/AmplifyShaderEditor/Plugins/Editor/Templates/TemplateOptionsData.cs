...
namespace AmplifyShaderEditor
{
    ...
    */
    ...
    public class TemplateOptionsItem
    {
        ...
        {
            get { return m_defaultFieldValue; }
            set { m_defaultFieldValue = value; }
        }
    }
    ...
    public class TemplateOptionsContainer
    {
        ...
        public void CopyPortOptionsFrom(TemplateOptionsContainer container, string passName)
        {
            ...
            {
                if (
                    container.Options[i].Type == AseOptionsType.Port
    ...
    public class TemplateOptionsToolsHelper
    {
        ...
        public const string PassOptionsMainPattern =
            "\\/\\*ase_pass_options:([\\w:= ]*)[\n]([\\w: \t;\n&|,_\\+\\-\\(\\)\\[\\]\\\"\\=\\/\\.]*)\\*\\/";
        public const string SubShaderOptionsMainPattern =
            "\\/\\*ase_subshader_options:([\\w:= ]*)[\n]([\\w: \t?!;\n&|,_\\+\\-\\(\\)\\[\\]\\\"\\=\\/\\.]*)\\*\\/";
        public static readonly char OptionsDataSeparator = ',';
        public static Dictionary<string, AseOptionsSetup> AseOptionsSetupDict = new Dictionary<
            string,
            AseOptionsSetup
        >()
        {
        ...
        public static Dictionary<string, AseOptionsUIWidget> AseOptionsUITypeDict = new Dictionary<
            string,
            AseOptionsUIWidget
        >()
        {
        ...
        public static Dictionary<string, AseOptionsActionType> AseOptionsActionTypeDict =
            new Dictionary<string, AseOptionsActionType>()
            {
        ...
        public static Dictionary<string, AseOptionItemSetup> AseOptionItemSetupDict =
            new Dictionary<string, AseOptionItemSetup>
            {
                { "None", AseOptionItemSetup.None },
                { "InvertActionOnDeselection", AseOptionItemSetup.InvertActionOnDeselection },
            };

        public static bool InvertAction(
            AseOptionsActionType original,
            ref AseOptionsActionType inverted
        )
        {
            bool success = true;
            switch (original)
        ...
        public static TemplateOptionsContainer GenerateOptionsContainer(
            bool isSubShader,
            string data
        )
        {
            ...
            Match match = Regex.Match(
                data,
                isSubShader ? SubShaderOptionsMainPattern : PassOptionsMainPattern
            );
            ...
            if (match.Success)
            {
                try
                {
                    ...
                    List<List<TemplateActionItem>> actionItemsList =
                        new List<List<TemplateActionItem>>();
                    ...
                    for (int i = 0; i < setupLines.Length; i++)
                    {
                        if (AseOptionsSetupDict.ContainsKey(setupLines[i]))
                        {
                            ...
                            switch (setup)
                            {
                                case AseOptionsSetup.CopyOptionsFromMainPass:
                                    optionsContainer.CopyOptionsFromMainPass = true;
                                    break;
                            }
                        }
                        else
                        {
                            ...
                            if (args.Length > 1 && AseOptionsSetupDict.ContainsKey(args[0]))
                            {
                                ...
                                switch (setup)
                                {
                                    case AseOptionsSetup.Id:
                                        if (!int.TryParse(args[1], out optionsContainer.Id))
                                            optionsContainer.Id = -1;
                                        break;
                                    case AseOptionsSetup.Name:
                                        optionsContainer.Name = args[1];
                                        break;
                                }
                            }
                        }
                                ...
                                case "Option":
                                    {
                                        ...
                                        string[] optionItemSetup = optionItems[1]
                                            .Split(OptionsDataSeparator);
                                        currentOption.Name = optionItemSetup[0];
                                        if (optionItemSetup.Length > 1)
                                        {
                                            if (
                                                AseOptionItemSetupDict.ContainsKey(
                                                    optionItemSetup[1]
                                                )
                                            )
                                                currentOption.Setup = AseOptionItemSetupDict[
                                                    optionItemSetup[1]
                                                ];
                                        }

                                        currentOption.Id =
                                            itemIds.Length > 1 ? itemIds[1] : optionItems[1];
                                        currentOption.DisplayOptions = optionItems[2]
                                            .Split(OptionsDataSeparator);
                                        currentOption.DisableIdx = currentOption
                                            .DisplayOptions
                                            .Length;
                                        optionItems[2] += ",disable";
                                        currentOption.Options = optionItems[2]
                                            .Split(OptionsDataSeparator);
                                        currentOption.Count = currentOption.Options.Length;

                                        for (
                                            int opIdx = 0;
                                            opIdx < currentOption.Options.Length;
                                            opIdx++
                                        )
                                        {
                                            optionItemToIndex.Add(
                                                currentOption.Options[opIdx],
                                                opIdx
                                            );
                                        ...
                                        if (
                                            currentOption.Options.Length == 2
                                            || (
                                                currentOption.Options.Length == 3
                                                && currentOption.Options[2].Equals("disable")
                                            )
                                        )
                                        {
                                            if (
                                                (
                                                    currentOption.Options[0].Equals("true")
                                                    && currentOption.Options[1].Equals("false")
                                                )
                                                || (
                                                    currentOption.Options[0].Equals("false")
                                                    && currentOption.Options[1].Equals("true")
                                                )
                                            )
                                            {
                                        ...
                                        {
                                            Debug.LogWarning(
                                                "Detected an option with less than two items:"
                                                    + optionItems[1]
                                            );
                                        }
                                ...
                                case "Field":
                                    {
                                        ...
                                        if (optionItems.Length >= 3)
                                        {
                                            currentOption.DefaultFieldValue = Convert.ToSingle(
                                                optionItems[3],
                                        ...
                                        if (optionItems.Length >= 6)
                                        {
                                            if (currentOption.UIWidget == AseOptionsUIWidget.Int)
                                                ...
                                                currentOption.UIWidget =
                                                    AseOptionsUIWidget.FloatRange;

                                            currentOption.FieldMin = Convert.ToSingle(
                                                optionItems[4],
                                                System.Globalization.CultureInfo.InvariantCulture
                                            );
                                            currentOption.FieldMax = Convert.ToSingle(
                                                optionItems[5],
                                                System.Globalization.CultureInfo.InvariantCulture
                                            );
                                        }
                                        ...
                                        if (optionItems.Length == 5 || optionItems.Length == 7)
                                        {
                                            ...
                                            currentOption.FieldInlineName = optionItems[
                                                optionItems.Length - 1
                                            ];
                                        }

                                        currentOption.Options = new string[]
                                        {
                                            "Change",
                                            "Inline",
                                            "disable",
                                ...
                                default:
                                    {
                                        ...
                                        const string IsConditionalPattern =
                                            @"^\s*(.+?)\s*\?\s*(.+?)\s*(=|!=)\s*(.+?)\s*$";
                                        TemplateActionItemConditional condition = null;
                                        Match isConditionalMatch = Regex.Match(
                                            optionItems[0],
        ...
        static void FillOptionAction(
            TemplateOptionsItem currentOption,
            ref List<List<TemplateActionItem>> actionItemsList
        )
        {
        ...
        static TemplateActionItem CreateActionItem(
            bool isSubshader,
            string[] optionItems,
            TemplateActionItemConditional condition = null
        )
            ...
            try
            {
                ...
                switch (actionItem.ActionType)
                {
                    case AseOptionsActionType.RefreshOption:
                        {
                            ...
                            {
                                Debug.LogWarning(
                                    "RefreshOption should not have additional parameters other than Option name."
                                );
                            }
                    ...
                    case AseOptionsActionType.SetOption:
                        {
                            ...
                            if (arr.Length > 1)
                            {
                                ...
                                {
                                    Debug.LogWarning(
                                        "SetOption value must be a the selection index"
                                    );
                                }
                            }
                        }
                    ...
                    case AseOptionsActionType.SetShaderProperty:
                        {
                            ...
                            {
                                actionItem.ActionData = optionItems[optionsIdx]
                                    .Substring(0, optIndex);
                                actionItem.ActionBuffer = optionItems[optionsIdx]
                                    .Substring(
                                        optIndex + 1,
                                        optionItems[optionsIdx].Length - optIndex - 1
                                    );
                            }
                        }
                        break;
                    ...
                    case AseOptionsActionType.SetPropertyOnSubShader:
                        {
                            string[] arr = optionItems[optionsIdx].Split(OptionsDataSeparator);
                            actionItem.PropertyAction = (PropertyActionsEnum)
                                Enum.Parse(typeof(PropertyActionsEnum), arr[0]);
                            if (
                                arr.Length == 1
                                && actionItem.ActionType == AseOptionsActionType.SetPropertyOnPass
                            )
                            {
                            ...
                            else
                            {
                                switch (actionItem.PropertyAction)
                                {
                                    case PropertyActionsEnum.CullMode:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionCullMode = (CullMode)
                                                    Enum.Parse(typeof(CullMode), arr[1]);
                                        }
                                        break;
                                    ...
                                    case PropertyActionsEnum.ZWrite:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionZWrite = (ZWriteMode)
                                                    Enum.Parse(typeof(ZWriteMode), arr[1]);
                                        }
                                        break;
                                    case PropertyActionsEnum.ZTest:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionZTest = (ZTestMode)
                                                    Enum.Parse(typeof(ZTestMode), arr[1]);
                                        }
                                        break;
                                    case PropertyActionsEnum.ZOffsetFactor:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionZOffsetFactor = Convert.ToSingle(
                                                    arr[1]
                                                );
                                        }
                                        break;
                                    case PropertyActionsEnum.ZOffsetUnits:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionZOffsetUnits = Convert.ToSingle(
                                                    arr[1]
                                                );
                                        }
                                        break;
                                    case PropertyActionsEnum.BlendRGB:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendRGBSource =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendRGBDest =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendRGB1:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendRGBSource1 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendRGBDest1 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendRGB2:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendRGBSource2 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendRGBDest2 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendRGB3:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendRGBSource3 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendRGBDest3 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendAlpha:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendAlphaSource =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendAlphaDest =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendAlpha1:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendAlphaSource1 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendAlphaDest1 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendAlpha2:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendAlphaSource2 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendAlphaDest2 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendAlpha3:
                                        {
                                            if (arr.Length > 2)
                                            {
                                                actionItem.ActionBlendAlphaSource3 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[1]
                                                        );
                                                actionItem.ActionBlendAlphaDest3 =
                                                    (AvailableBlendFactor)
                                                        Enum.Parse(
                                                            typeof(AvailableBlendFactor),
                                                            arr[2]
                                    ...
                                    case PropertyActionsEnum.BlendOpRGB:
                                        {
                                            if (arr.Length > 1)
                                            {
                                                actionItem.ActionBlendOpRGB = (AvailableBlendOps)
                                                    Enum.Parse(typeof(AvailableBlendOps), arr[1]);
                                            }
                                        }
                                        break;
                                    case PropertyActionsEnum.BlendOpAlpha:
                                        {
                                            if (arr.Length > 1)
                                            {
                                                actionItem.ActionBlendOpAlpha = (AvailableBlendOps)
                                                    Enum.Parse(typeof(AvailableBlendOps), arr[1]);
                                            }
                                        }
                                        break;
                                    case PropertyActionsEnum.StencilReference:
                                        {
                                            if (arr.Length > 1)
                                            {
                                                int.TryParse(
                                                    arr[1],
                                                    out actionItem.ActionStencilReference
                                                );
                                            }
                                        }
                                        break;
                                    case PropertyActionsEnum.StencilReadMask:
                                        {
                                            if (arr.Length > 1)
                                            {
                                                int.TryParse(
                                                    arr[1],
                                                    out actionItem.ActionStencilReadMask
                                                );
                                            }
                                        }
                                        break;
                                    case PropertyActionsEnum.StencilWriteMask:
                                        {
                                            if (arr.Length > 1)
                                            {
                                                int.TryParse(
                                                    arr[1],
                                                    out actionItem.ActionStencilWriteMask
                                                );
                                            }
                                        }
                                        break;
                                    case PropertyActionsEnum.StencilComparison:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionStencilComparison =
                                                    StencilBufferOpHelper.StencilComparisonValuesDict[
                                                        arr[1]
                                                    ];
                                        }
                                        break;
                                    case PropertyActionsEnum.StencilPass:
                                        {
                                            if (arr.Length > 1)
                                                actionItem.ActionStencilPass =
                                                    StencilBufferOpHelper.StencilOpsValuesDict[
                                                        arr[1]
                                                    ];
                                        }
                                        break;
