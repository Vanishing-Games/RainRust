...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateTagsModule : TemplateModuleParent
    {
        ...
        private Dictionary<string, CustomTagData> m_availableTagsDict =
            new Dictionary<string, CustomTagData>();

        public TemplateTagsModule()
            : base("Tags") { }
        ...
        public void ConfigureFromTemplateData(TemplateTagsModuleData tagsData)
        {
            ...
            if (newValidData && newValidData != m_validData)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    CustomTagData tagData = new CustomTagData(
                        tagsData.Tags[i].Name,
                        tagsData.Tags[i].Value,
                        i
                    );
        ...
        public void OnLogicUpdate()
        {
            if (
                m_tagNameCheckFlag
                && (EditorApplication.timeSinceStartup - m_tagNameCheckTimestamp)
                    > TagNameCheckMaxInterval
            )
            {
                ...
                {
                    if (
                        m_availableTags[m_tagNameCheckItemId]
                            .TagName.Equals(Constants.RenderQueueHelperStr)
                    )
                    {
                        m_availableTags[m_tagNameCheckItemId].SpecialTag =
                            TemplateSpecialTags.Queue;
                    }
                    else if (
                        m_availableTags[m_tagNameCheckItemId]
                            .TagName.Equals(Constants.RenderTypeHelperStr)
                    )
                    {
                        m_availableTags[m_tagNameCheckItemId].SpecialTag =
                            TemplateSpecialTags.RenderType;
                    }
                    else if (
                        m_availableTags[m_tagNameCheckItemId]
                            .TagName.Equals(Constants.DisableBatchingHelperStr)
                    )
                    {
                        m_availableTags[m_tagNameCheckItemId].SpecialTag =
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    if (m_availableTags[i].TagFoldout)
                    {
                        ...
                        //Tag Name
                        EditorGUI.BeginChangeCheck();
                        m_availableTags[i].TagName = m_currentOwner.EditorGUILayoutTextField(
                            TagNameStr,
                        ...
                        switch (m_availableTags[i].SpecialTag)
                        {
                            case TemplateSpecialTags.DisableBatching:
                                {
                                    m_availableTags[i].Batching = (DisableBatching)
                                        m_currentOwner.EditorGUILayoutEnumPopup(
                                            RenderTypeLabelStr,
                                            m_availableTags[i].Batching
                                        );
                                    m_availableTags[i].TagValue = m_availableTags[i]
                                        .Batching.ToString();
                                }
                                break;
                            case TemplateSpecialTags.RenderType:
                                {
                                    m_availableTags[i].RenderType = (RenderType)
                                        m_currentOwner.EditorGUILayoutEnumPopup(
                                            RenderTypeLabelStr,
                                            m_availableTags[i].RenderType
                                        );
                                    if (m_availableTags[i].RenderType == RenderType.Custom)
                                    {
                                        m_availableTags[i].TagValue =
                                            m_currentOwner.EditorGUILayoutTextField(
                                                CustomRenderTypeLabelStr,
                                                m_availableTags[i].TagValue
                                            );
                                    }
                                }
                            ...
                            case TemplateSpecialTags.Queue:
                                {
                                    EditorGUI.BeginChangeCheck();
                                    m_availableTags[i].RenderQueue = (RenderQueue)
                                        m_currentOwner.EditorGUILayoutEnumPopup(
                                            QueueLabelStr,
                                            m_availableTags[i].RenderQueue,
                                            GUILayout.MinWidth(150)
                                        );
                                    m_availableTags[i].RenderQueueOffset =
                                        m_currentOwner.EditorGUILayoutIntField(
                                            QueueIndexStr,
                                            m_availableTags[i].RenderQueueOffset
                                        );
                                    if (EditorGUI.EndChangeCheck())
                            ...
                            case TemplateSpecialTags.None:
                                {
                                    ...
                                    {
                                        m_availableTags[i].TagValue =
                                            UIUtils.RemoveShaderInvalidCharacters(
                                                m_availableTags[i].TagValue
                                            );
                                    }
                                }
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_availableTags[i].SpecialTag == tag)
                {
                    switch (tag)
                    {
                        ...
                        {
                            m_availableTags[i].Batching =
                                TemplateHelperFunctions.StringToDisableBatching[item.ActionData];
                            return;
                        }
                        case TemplateSpecialTags.RenderType:
                        {
                            m_availableTags[i].RenderType =
                                TemplateHelperFunctions.StringToRenderType[item.ActionData];
                            return;
                        }
                        case TemplateSpecialTags.Queue:
                        {
                            m_availableTags[i].RenderQueue =
                                TemplateHelperFunctions.StringToRenderQueue[item.ActionData];
                ...
                case TemplateSpecialTags.DisableBatching:
                    {
                        ...
                        data.Batching = TemplateHelperFunctions.StringToDisableBatching[
                            item.ActionData
                        ];
                    }
                    break;
                case TemplateSpecialTags.RenderType:
                    {
                        ...
                        data.RenderType = TemplateHelperFunctions.StringToRenderType[
                            item.ActionData
                        ];
                    }
                    break;
                case TemplateSpecialTags.Queue:
                    {
                        ...
                        data.TagName = "Queue";
                        data.RenderQueue = TemplateHelperFunctions.StringToRenderQueue[
                            item.ActionData
                        ];
        ...
        void AddTagFromRead(string data)
        {
            ...
            if (arr.Length > 1)
            {
                ...
                {
                    if (
                        m_availableTagsDict[name].TagId > -1
                        && m_availableTagsDict[name].TagId < m_availableTags.Count
                    )
                    {
                        if (arr.Length == 4)
                        {
                            m_availableTags[m_availableTagsDict[name].TagId]
                                .SetTagValue(value, arr[3]);
                        }
                        else
                        {
        ...
        public List<CustomTagData> AvailableTags
        {
            get { return m_availableTags; }
        }
        ...
        public bool HasRenderInfo(ref RenderType renderType, ref RenderQueue renderQueue)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    if (
                        TemplateHelperFunctions.StringToRenderType.ContainsKey(
                            m_availableTags[i].TagValue
                        )
                    )
                    {
                        renderType = TemplateHelperFunctions.StringToRenderType[
                            m_availableTags[i].TagValue
                        ];
