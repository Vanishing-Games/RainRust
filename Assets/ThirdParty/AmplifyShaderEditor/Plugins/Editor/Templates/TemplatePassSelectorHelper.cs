...
namespace AmplifyShaderEditor
{
    ...
    public class TemplatePassSelectorHelper
    {
        ...
        public void Setup(TemplateSubShader subShader)
        {
            if (m_currentPasses == null)
            {
                ...
                for (int i = 0; i < m_currentPasses.Length; i++)
                {
                    ...
                    m_currentPasses[i] = new PassVisibleOptionsItems()
                    {
                        Name = subShader.Passes[i].PassNameContainer.Data,
                        Visible = true,
                        Idx = i,
                    };
            ...
            NodeUtils.DrawNestedPropertyGroup(
                ...
                () =>
                {
                    for (int i = 0; i < m_currentPasses.Length; i++)
                    {
                        ...
                        m_currentPasses[i].Visible = owner.EditorGUILayoutToggleLeft(
                            m_currentPasses[i].Name,
                            m_currentPasses[i].Visible
                        );
                        if (EditorGUI.EndChangeCheck())
                        {
                            owner
                                .ContainerGraph.GetMultiPassMasterNodes(owner.LODIndex)[
                                    m_currentPasses[i].Idx
                                ]
                                .IsInvisible = !m_currentPasses[i].Visible;
                        }
                    }
        ...
        {
            return m_currentPasses[passId].Visible;
        }

        private PassVisibleOptionsItems[] AvailablePasses
        {
            get { return m_currentPasses; }
        }
    }
}
