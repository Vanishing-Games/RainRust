...
namespace AmplifyShaderEditor
{
    ...
    {
        public TemplateAdditionalDefinesHelper()
            : base("Additional Defines")
        {
        ...
        )
        {
            for (int i = 0; i < m_additionalItems.Count; i++)
            {
                if (
                    ...
                    && !nativesContainer.HasDefine(m_additionalItems[i])
                )
            ...
            for (int i = 0; i < m_outsideItems.Count; i++)
            {
                if (
                    ...
                    && !nativesContainer.HasDefine(m_outsideItems[i])
                )
