...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateInputData
    {
        ...
        public TemplateInputData(
            int tagLocalStartIdx,
            int tagGlobalStartIdx,
            string tagId,
            string portName,
            string defaultValue,
    ...
    public class TemplatePropertyContainer
    {
            ...
            if (propertyIndex > -1)
            {
                if (searchIndentation)
                {
                    ...
                    if (identationIndex > -1)
                    {
                        ...
                        string indentation =
                            (length > 0) ? body.Substring(identationIndex, length) : string.Empty;
        ...
        )
        {
            ...
            if (searchIndentation && propertyIndex > -1 && propertyIndex < body.Length)
            {
                ...
                if (indentationIndex > -1)
                {
                    ...
                    string indentation =
                        (length > 0) ? body.Substring(indentationIndex, length) : string.Empty;
                    TemplateProperty templateProperty = new TemplateProperty(
                        ID,
                        indentation,
                        false
        ...
        public List<TemplateProperty> PropertyList
        {
    ...
    public class TemplateTessControlData
    {
        ...
        public string[] GenerateControl(
            Dictionary<TemplateSemantics, TemplateVertexData> vertexData,
        ...
        )
        {
            ...
            if (vertexData != null && vertexData.Count > 0)
            {
                ...
                {
                    if (
                        inputList.FindIndex(x =>
                        {
                            return x.Contains(item.Value.VarName);
                        }) > -1
                    )
                        value.Add(
                            string.Format(
            ...
            if (vertexData != null && vertexData.Count > 0)
            {
                foreach (var item in vertexData)
                {
                    ...
                    if (
                        ...
                        value.Add(
                            string.Format(
                                "{0}.{1} = {2}[0].{1} * {3}.x + {2}[1].{1} * {3}.y + {2}[2].{1} * {3}.z;",
                                OutVarName,
                                item.Value.VarName,
                                InVarName,
                                BaryVarName
    ...
    public class TemplateFunctionData
    {
        ...
        public TemplateFunctionData(
            int mainBodyLocalIdx,
            string mainBodyName,
            string id,
            int position,
            string inVarInfo,
    ...
    public class TemplatesManager : ScriptableObject
    {
        ...
        public static readonly string TemplateCodeSnippetAttribBegin =
            "#CODE_SNIPPET_ATTRIBS_BEGIN#";
        ...
        public static readonly TemplateTagData[] CommonTags =
        {
            new TemplateTagData(TemplatePropertyTag, true),
        ...
        public static Dictionary<string, string> DeprecatedTemplates = new Dictionary<
            string,
            string
        >()
        { };

        public static Dictionary<string, string> OfficialTemplates = new Dictionary<
            string,
            string
        >()
        {
        ...
        public static readonly string TemplateMenuItemsFileGUID =
            "da0b931bd234a1e43b65f684d4b59bfb";

        private Dictionary<string, TemplateDataParent> m_availableTemplates =
            new Dictionary<string, TemplateDataParent>();
        ...
        public static bool IsTestTemplate
        {
            get { return CurrTemplateGUIDLoaded.Equals("a95a019bbc760714bb8228af04c291d1"); }
        }
        ...
        public void Init()
        {
            if (!Initialized)
            {
                ...
                string templateMenuItems = IOUtils.LoadTextFileFromDisk(
                    AssetDatabase.GUIDToAssetPath(TemplateMenuItemsFileGUID)
                );
                ...
                foreach (var pair in templates)
                {
                    ...
                    if (
                        !desc.isCommunity
                        && !refreshTemplateMenuItems
                        && templateMenuItems.IndexOf(name) < 0
                    )
                    {
                ...
                for (int j = 0; j < i; j++)
                {
                    if (m_sortedTemplates[i].Name == m_sortedTemplates[j].Name)
                    {
                        ...
                        if (match.Success)
                        {
                            ...
                            string firstPart = m_sortedTemplates[i]
                                .Name.Substring(0, match.Groups[1].Index);
                            string secondPart = m_sortedTemplates[i]
                                .Name.Substring(match.Groups[1].Index + strNumber.Length);
        ...
        public bool CheckIfDeprecated(string guid, out string newGUID)
        {
            ...
            {
                UIUtils.ShowMessage(
                    "Shader using deprecated template which no longer exists on ASE. Pointing to new correct one, options and connections to master node were reset."
                );
        ...
        {
            get { return m_sortedTemplates.Count; }
        }
    }
}
