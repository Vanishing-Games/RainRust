...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplatesBlendModule : TemplateModuleParent
    {
        ...
        private List<CommonBlendTypes> m_commonBlendTypes = new List<CommonBlendTypes>
        {
            ...
            new CommonBlendTypes(
                "Alpha Blend",
                AvailableBlendFactor.SrcAlpha,
                AvailableBlendFactor.OneMinusSrcAlpha
            ),
            new CommonBlendTypes(
                "Premultiplied",
                AvailableBlendFactor.One,
                AvailableBlendFactor.OneMinusSrcAlpha
            ),
            new CommonBlendTypes("Additive", AvailableBlendFactor.One, AvailableBlendFactor.One),
            new CommonBlendTypes(
                "Soft Additive",
                AvailableBlendFactor.OneMinusDstColor,
                AvailableBlendFactor.One
            ),
            new CommonBlendTypes(
                "Multiplicative",
                AvailableBlendFactor.DstColor,
                AvailableBlendFactor.Zero
            ),
            new CommonBlendTypes(
                "2x Multiplicative",
                AvailableBlendFactor.DstColor,
                AvailableBlendFactor.SrcColor
            ),
            new CommonBlendTypes(
                "Particle Additive",
                AvailableBlendFactor.SrcAlpha,
        ...
        public void ConfigureFromTemplateData(TemplateBlendData blendData)
        {
            if (blendData.ValidBlendMode)
            {
                if (m_validBlendMode != blendData.ValidBlendMode)
                {
                    ...
                    {
                        m_sourceFactorAlphaInline.SetInlineByName(
                            blendData.SourceFactorAlphaInline
                        );
                    }
        ...
        public override void ShowUnreadableDataMessage(ParentNode owner)
        {
            ...
            owner.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedBlendModeModule =
                foldout;
        }
        ...
        void DrawBlock(UndoParentNode owner, bool style)
        {
            EditorGUI.BeginChangeCheck();
            {
                ...
                if (m_blendModeEnabled)
                {
                    // RGB
                    EditorGUI.BeginChangeCheck();
                    m_currentRGBIndex = owner.EditorGUILayoutPopup(
                        BlendModesRGBStr,
                        m_currentRGBIndex,
                        m_commonBlendTypesArr
                    );
                    ...
                    m_sourceFactorRGBInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_sourceFactorRGB = (AvailableBlendFactor)
                                x.EditorGUILayoutEnumPopup(SourceFactorStr, m_sourceFactorRGB);
                        },
                        SourceFactorStr
                    );
                    if (style)
                    ...
                    m_destFactorRGBInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_destFactorRGB = (AvailableBlendFactor)
                                x.EditorGUILayoutEnumPopup(DstFactorStr, m_destFactorRGB);
                        },
                        DstFactorStr
                    );
                    if (style)
                ...
                if (m_blendOpEnabled)
                {
                    ...
                    m_blendOpRGBInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_blendOpRGB = (AvailableBlendOps)
                                x.EditorGUILayoutPopup(
                                    BlendOpsRGBStr,
                                    (int)m_blendOpRGB,
                                    BlendOpsHelper.BlendOpsLabels
                                );
                        },
                        BlendOpsRGBStr
                    );
                ...
                if (m_blendModeEnabled)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    m_currentAlphaIndex = owner.EditorGUILayoutPopup(
                        BlendModesAlphaStr,
                        m_currentAlphaIndex,
                        m_commonBlendTypesArr
                    );
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        {
                            m_sourceFactorAlpha = m_commonBlendTypes[
                                m_currentAlphaIndex
                            ].SourceFactor;
                    ...
                    m_sourceFactorAlphaInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_sourceFactorAlpha = (AvailableBlendFactor)
                                x.EditorGUILayoutEnumPopup(SourceFactorStr, m_sourceFactorAlpha);
                        },
                        SourceFactorStr
                    );
                    if (style)
                    ...
                    m_destFactorAlphaInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_destFactorAlpha = (AvailableBlendFactor)
                                x.EditorGUILayoutEnumPopup(DstFactorStr, m_destFactorAlpha);
                        },
                        DstFactorStr
                    );
                    if (style)
                ...
                if (m_blendOpEnabled)
                {
                    ...
                    m_blendOpAlphaInline.CustomDrawer(
                        ref owner,
                        (x) =>
                        {
                            m_blendOpAlpha = (AvailableBlendOps)
                                x.EditorGUILayoutPopup(
                                    BlendOpsAlphaStr,
                                    (int)m_blendOpAlpha,
                                    BlendOpsHelper.BlendOpsLabels
        ...
        void CheckRGBIndex()
        {
            ...
            {
                if (
                    m_commonBlendTypes[i].SourceFactor == m_sourceFactorRGB
                    && m_commonBlendTypes[i].DestFactor == m_destFactorRGB
                )
                {
                    m_currentRGBIndex = i;
                    return;
        ...
        void CheckAlphaIndex()
        {
            ...
            {
                if (
                    m_commonBlendTypes[i].SourceFactor == m_sourceFactorAlpha
                    && m_commonBlendTypes[i].DestFactor == m_destFactorAlpha
                )
                {
        ...
        public void ReadBlendModeFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            if (validDataOnMeta)
            {
                if (UIUtils.CurrentShaderVersion() < 15304)
                {
                    ...
                    m_sourceFactorRGB = (AvailableBlendFactor)
                        Enum.Parse(typeof(AvailableBlendFactor), nodeParams[index++]);
                    m_destFactorRGB = (AvailableBlendFactor)
                        Enum.Parse(typeof(AvailableBlendFactor), nodeParams[index++]);
        ...
        public void ReadBlendOpFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            if (validDataOnMeta)
            {
                ...
                {
                    m_blendOpRGB = (AvailableBlendOps)
                        Enum.Parse(typeof(AvailableBlendOps), nodeParams[index++]);
                    m_blendOpAlpha = (AvailableBlendOps)
                        Enum.Parse(typeof(AvailableBlendOps), nodeParams[index++]);
                }
                else
        ...
        public void WriteBlendOpToString(ref string nodeInfo)
        {
            ...
            if (m_validBlendOp)
            {
                if (!m_blendOpRGBInline.IsValid)
                    m_blendOpRGBInline.IntValue = (int)m_blendOpRGB;
                m_blendOpRGBInline.WriteToString(ref nodeInfo);

                if (!m_blendOpAlphaInline.IsValid)
                    m_blendOpAlphaInline.IntValue = (int)m_blendOpAlpha;
        ...
        public string CurrentBlendFactorSingle
        {
            get
            {
                return (m_currentRGBIndex > 0)
                    ? string.Format(
                        SingleBlendFactorStr,
                        m_target,
                        m_sourceFactorRGBInline.GetValueOrProperty(m_sourceFactorRGB.ToString()),
                        m_destFactorRGBInline.GetValueOrProperty(m_destFactorRGB.ToString())
                    )
                    : BlendFactorOff;
            }
        }
        ...
        public string CurrentBlendFactorSeparate
        {
            get
            {
                return string.Format(
                    SeparateBlendFactorStr,
                    m_target,
                    m_sourceFactorRGBInline.GetValueOrProperty(
                        (
                            m_currentRGBIndex > 0
                                ? m_sourceFactorRGB.ToString()
                                : AvailableBlendFactor.One.ToString()
                        )
                    ),
                    m_destFactorRGBInline.GetValueOrProperty(
                        m_currentRGBIndex > 0
                            ? m_destFactorRGB.ToString()
                            : AvailableBlendFactor.Zero.ToString()
                    ),
        ...
        public string CurrentBlendFactor
        {
            get
            {
                return (
                    (m_currentAlphaIndex > 0)
                        ? CurrentBlendFactorSeparate
                        : CurrentBlendFactorSingle
                );
            }
        }
        ...
        public string CurrentBlendOpSingle
        {
            get
            {
                return (m_blendOpRGB != AvailableBlendOps.OFF || m_blendOpRGBInline.IsValid)
                    ? string.Format(
                        SingleBlendOpStr,
                        Target,
                        m_blendOpRGBInline.GetValueOrProperty(m_blendOpRGB.ToString())
                    )
                    : string.Empty;
            }
        }
        ...
        public string CurrentBlendOpSeparate
        {
            get
            {
                return string.Format(
                    SeparateBlendOpStr,
                    Target,
                    m_blendOpRGBInline.GetValueOrProperty(
                        (m_currentRGBIndex > 0 && m_blendOpRGB != AvailableBlendOps.OFF)
                            ? m_blendOpRGB.ToString()
                            : AvailableBlendOps.Add.ToString()
                    ),
                    m_blendOpAlphaInline.GetValueOrProperty(m_blendOpAlpha.ToString())
                );
            }
        }

        public string CurrentBlendOp
        {
            get
            {
                return (
                    (m_blendOpAlpha != AvailableBlendOps.OFF || m_blendOpAlphaInline.IsValid)
                        ? CurrentBlendOpSeparate
                        : CurrentBlendOpSingle
                );
            }
        }
        public bool Active
        {
            get { return m_blendModeEnabled && (m_currentRGBIndex > 0 || m_currentAlphaIndex > 0); }
        }
        ...
        public bool ValidBlendMode
        {
            get { return m_validBlendMode; }
        }
        public bool ValidBlendOp
        {
            get { return m_validBlendOp; }
        }
        public int CurrentRGBIndex
        {
            get { return m_currentRGBIndex; }
        }
        ...
        public int CurrentAlphaIndex
        {
            get { return m_currentAlphaIndex; }
            set { m_currentAlphaIndex = value; }
        }
        ...
        public bool BlendModeEnabled
        {
            get { return m_blendModeEnabled; }
        }
        public bool BlendOpEnabled
        {
            get { return m_blendOpEnabled; }
        }
        ...
        public InlineProperty SourceFactorRGBInline
        {
            get { return m_sourceFactorRGBInline; }
        }
        public InlineProperty DestFactorRGBInline
        {
            get { return m_destFactorRGBInline; }
        }
        public InlineProperty SourceFactorAlphaInline
        {
            get { return m_sourceFactorAlphaInline; }
        }
        public InlineProperty DestFactorAlphaInline
        {
            get { return m_destFactorAlphaInline; }
        }
        public InlineProperty BlendOpRGBInline
        {
            get { return m_blendOpRGBInline; }
        }
        public InlineProperty BlendOpAlphaInline
        {
            get { return m_blendOpAlphaInline; }
        }
        public bool IsAdditiveRGB
        {
            get
            {
                return m_validBlendMode
                    && m_blendModeEnabled
                    && (m_currentRGBIndex > 0)
                    && (m_sourceFactorRGB == AvailableBlendFactor.One)
                    && (m_destFactorRGB == AvailableBlendFactor.One);
            }
        }
        public bool IsAlphaBlendRGB
        {
            get
            {
                return m_validBlendMode
                    && m_blendModeEnabled
                    && (m_currentRGBIndex > 0)
                    && (m_sourceFactorRGB == AvailableBlendFactor.SrcAlpha)
                    && (m_destFactorRGB == AvailableBlendFactor.OneMinusSrcAlpha);
            }
        }
    }
}
