        ...
        public static TemplateShaderInfo CreateShaderData(string body)
        {
            ...
            if (shaderMatch.Count > 0)
            {
                ...
                if (subShaderAmount > 0)
                {
                    ...
                    shaderData.PropertyStartIdx = body.IndexOf(
                    ...
                    for (int subShaderIdx = 0; subShaderIdx < subShaderAmount; subShaderIdx++)
                    {
                        ...
                        int subShaderEndIndex =
                            (subShaderIdx == (subShaderAmount - 1))
                                ? body.Length - 1
                                : subShaderMatch[subShaderIdx + 1].Index;
                        subShaderData.Data = body.Substring(
                            subshaderBeginIndex,
                            subShaderEndIndex - subshaderBeginIndex
                        );
                        subShaderData.StartIdx = subshaderBeginIndex;

                        //PASS
                        MatchCollection passMatch = Regex.Matches(
                            subShaderData.Data,
                            TemplatesManager.TemplatePassTagPattern
                        );
                        if (passMatch.Count == 0)
                        {
                            passMatch = Regex.Matches(
                                subShaderData.Data,
                                TemplatesManager.TemplateMPPassTag
                            );
                        }
                        ...
                        if (passCount > 0)
                        {
                            int lastPassIndex = subShaderData.Data.LastIndexOf(
                                TemplatesManager.TemplatePassesEndTag
                            );
                            if (lastPassIndex < 0)
                            {
                            ...
                            subShaderData.Modules = subShaderData.Data.Substring(
                                0,
                                passMatch[0].Index
                            );
                            for (int passIdx = 0; passIdx < passCount; passIdx++)
                            {
                                ...
                                int passEndIdx =
                                    (passIdx == (passCount - 1))
                                        ? lastPassIndex
                                        : passMatch[passIdx + 1].Index;
                                TemplatePassInfo passData = new TemplatePassInfo();
                                passData.Data = subShaderData.Data.Substring(
                                    passBeginIndex,
                                    passEndIdx - passBeginIndex
                                );
