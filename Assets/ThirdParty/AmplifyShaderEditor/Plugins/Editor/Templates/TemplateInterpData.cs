...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateInterpElement
    {
        ...
        public TemplateInterpElement(TemplateSemantics semantic)
        {
            ...
            Name =
                (semanticId == 0)
                    ...
                    : TemplateHelperFunctions.BaseInterpolatorName + semanticId.ToString();
        }
        ...
        public TemplateVertexData RequestChannels(
        ...
        )
        {
            ...
            else
            {
                string[] swizzleArray =
                    (isColor)
                        ? TemplateHelperFunctions.ColorSwizzle
                        : TemplateHelperFunctions.VectorSwizzle;
                string channels = ".";
    ...
    public class TemplateInterpData
    {
        ...
        public List<TemplateInterpElement> AvailableInterpolators =
            new List<TemplateInterpElement>();
                ...
                if (AvailableInterpolators.Count > 0)
                {
                    int currMax =
                        ...
                        + TemplateHelperFunctions.SemanticToInt[
                            AvailableInterpolators[AvailableInterpolators.Count - 1].Semantic
                        ];
                    if (newMax > currMax)
                    {
                        ...
                        {
                            AvailableInterpolators.Add(
                                new TemplateInterpElement(
                                    TemplateHelperFunctions.IntToSemantic[currMax + i]
                                )
                            );
                        }
                    }
                    ...
                    {
                        int min = TemplateHelperFunctions.SemanticToInt[
                            AvailableInterpolators[0].Semantic
                        ];
                        if (newMax > min)
                        {
        ...
        {
            get { return m_interpDataId; }
            set { m_interpDataId = value; }
        }
        public int InterpDataStartIdx
        {
            get { return m_interpDataStartIdx; }
            set { m_interpDataStartIdx = value; }
        }
        public bool DynamicMax
        {
            get { return m_dynamicMax; }
            set { m_dynamicMax = value; }
        }
    }
}
