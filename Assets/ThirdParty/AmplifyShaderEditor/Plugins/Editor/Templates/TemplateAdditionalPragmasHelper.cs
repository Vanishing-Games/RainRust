...
namespace AmplifyShaderEditor
{
    ...
    {
        public TemplateAdditionalPragmasHelper()
            : base("Additional Pragmas")
        {
        ...
        )
        {
            for (int i = 0; i < m_additionalItems.Count; i++)
            {
                if (
                    ...
                    && !nativesContainer.HasPragma(m_additionalItems[i])
                )
            ...
            for (int i = 0; i < m_outsideItems.Count; i++)
            {
                if (
                    ...
                    && !nativesContainer.HasPragma(m_outsideItems[i])
                )
