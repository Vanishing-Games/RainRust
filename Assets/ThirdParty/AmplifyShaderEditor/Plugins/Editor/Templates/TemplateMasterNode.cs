...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplateMasterNode : MasterNode
    {
        ...
        [SerializeField]
        private TemplatesStencilBufferModule m_stencilBufferHelper =
            new TemplatesStencilBufferModule();
        ...
        public override void ReleaseResources()
        {
            if (m_currentTemplate != null && m_currentTemplate.AvailableShaderProperties != null)
            {
                ...
                {
                    UIUtils.ReleaseUniformName(
                        UniqueId,
                        m_currentTemplate.AvailableShaderProperties[i].PropertyName
                    );
                }
            }
        }
        ...
        {
            m_currentTemplate =
                m_containerGraph.ParentWindow.TemplatesManagerInstance.GetTemplate(m_templateGUID)
                as TemplateData;
        ...
        public override void RefreshAvailableCategories()
        {
            ...
            m_availableCategories[0] = new MasterNodeCategoriesData(
            ...
            for (int i = 0; i < templateCount; i++)
            {
                ...
                m_availableCategories[idx] = new MasterNodeCategoriesData(
        ...
        void SetCategoryIdxFromTemplate()
        {
            ...
            for (int i = 0; i < templateCount; i++)
            {
                ...
                if (
                    ...
                    && m_currentTemplate.GUID.Equals(templateData.GUID)
                )
                    m_masterNodeCategory = idx;
            }
        }
        ...
        void RegisterProperties()
        {
            if (m_currentTemplate != null)
            {
                ...
                {
                    int nodeId = UIUtils.CheckUniformNameOwner(
                        m_currentTemplate.AvailableShaderProperties[i].PropertyName
                    );
                    if (nodeId > -1)
                    {
        ...
        public override void DrawProperties()
        {
            ...
            {
                CullMode cullMode =
                    (m_currentTemplate.CullModeData.DataCheck == TemplateDataCheck.Valid)
                        ? m_cullModeHelper.CurrentCullMode
                        : CullMode.Back;
        ...
        )
        {
            ...
            for (int i = 0; i < ports.Count; i++)
            {
                ...
                if (ports[i].IsConnected)
                {
                    ...
                    isValid =
                        m_currentTemplate.FillTemplateBody(
                            inputData.TagId,
                            ref shaderBody,
                            newPortInstruction
                        ) && isValid;
        ...
        public override Shader Execute(string pathname, bool isFullPath)
        {
            ...
            for (int i = 0; i < shaderPropertiesAmount; i++)
            {
                m_currentDataCollector.SoftRegisterUniform(
                    m_currentTemplate.AvailableShaderProperties[i]
                );
            }
            ...
            validBody =
                m_currentTemplate.FillVertexInstructions(
                    ref shaderBody,
                    vertexInstructions.ToArray()
                ) && validBody;
            validBody =
                m_currentTemplate.FillFragmentInstructions(
                    ref shaderBody,
                ...
                m_currentDataCollector.InstancedPropertiesList.Insert(
                    ...
                    new PropertyDataCollector(
                        ...
                        string.Format(
                            ...
                            UIUtils.RemoveInvalidCharacters(m_shaderName)
                        )
                    )
                );
                m_currentDataCollector.InstancedPropertiesList.Add(
                    new PropertyDataCollector(-1, IOUtils.InstancedPropertiesEnd)
                );
            ...
            validBody =
                m_currentTemplate.FillTemplateBody(
                    m_currentTemplate.ShaderNameId,
                    ref shaderBody,
            ...
            validBody =
                m_currentTemplate.FillTemplateBody(
                    ...
                    ref shaderBody,
                    m_currentDataCollector.GrabPassList
                ) && validBody;
            validBody =
                m_currentTemplate.FillTemplateBody(
                    TemplatesManager.TemplatePragmaTag,
                    ref shaderBody,
                    m_currentDataCollector.IncludesList
                ) && validBody;
            ...
            validBody =
                m_currentTemplate.FillTemplateBody(
                    ...
                    ref shaderBody,
                    m_currentDataCollector.BuildUnformatedPropertiesStringArr()
                ) && validBody;
            validBody =
                m_currentTemplate.FillTemplateBody(
            ...
            validBody =
                m_currentTemplate.FillTemplateBody(
                    m_currentTemplate.VertexDataId,
                    ref shaderBody,
                    m_currentDataCollector.VertexInputList.ToArray()
                ) && validBody;
            validBody =
                m_currentTemplate.FillTemplateBody(
                    m_currentTemplate.InterpDataId,
                    ref shaderBody,
            ...
            if (m_currentTemplate.BlendData.ValidBlendMode)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.BlendData.BlendModeId,
                        ref shaderBody,
                        m_blendOpHelper.CurrentBlendFactor
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.BlendData.ValidBlendOp)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.BlendData.BlendOpId,
                        ref shaderBody,
                        m_blendOpHelper.CurrentBlendOp
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.AlphaToMaskData.DataCheck == TemplateDataCheck.Valid)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.AlphaToMaskData.AlphaToMaskId,
                        ref shaderBody,
                        m_alphaToMaskHelper.GenerateShaderData(false)
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.DepthData.ValidZWrite)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.DepthData.ZWriteModeId,
                        ref shaderBody,
                        m_depthOphelper.CurrentZWriteMode
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.DepthData.ValidZTest)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.DepthData.ZTestModeId,
                        ref shaderBody,
                        m_depthOphelper.CurrentZTestMode
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.DepthData.ValidOffset)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.DepthData.OffsetId,
                        ref shaderBody,
                        m_depthOphelper.CurrentOffset
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.CullModeData.DataCheck == TemplateDataCheck.Valid)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.CullModeData.CullModeId,
                        ref shaderBody,
                        m_cullModeHelper.GenerateShaderData(false)
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.ColorMaskData.DataCheck == TemplateDataCheck.Valid)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.ColorMaskData.ColorMaskId,
                        ref shaderBody,
                        m_colorMaskHelper.GenerateShaderData(false)
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.StencilData.DataCheck == TemplateDataCheck.Valid)
            {
                ...
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.StencilData.StencilBufferId,
                        ref shaderBody,
                        m_stencilBufferHelper.CreateStencilOp(cullMode)
                    ) && validBody;
            }
            ...
            if (m_currentTemplate.TagData.DataCheck == TemplateDataCheck.Valid)
            {
                validBody =
                    m_currentTemplate.FillTemplateBody(
                        m_currentTemplate.TagData.TagsId,
                        ref shaderBody,
                        m_tagsHelper.GenerateTags()
                    ) && validBody;
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            try
            {
                ...
                if (UIUtils.CurrentShaderVersion() > 13902)
                {
                    ...
                    if (m_currentTemplate.BlendData.ValidBlendMode)
                    {
                        m_blendOpHelper.ReadBlendModeFromString(
                    ...
                    if (m_currentTemplate.BlendData.ValidBlendOp)
                    {
                        m_blendOpHelper.ReadBlendOpFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    if (m_currentTemplate.StencilData.DataCheck == TemplateDataCheck.Valid)
                    {
                        m_stencilBufferHelper.ReadFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                ...
                if (UIUtils.CurrentShaderVersion() > 14202)
                {
                    ...
                    if (m_currentTemplate.DepthData.ValidZWrite)
                    {
                        m_depthOphelper.ReadZWriteFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    if (m_currentTemplate.DepthData.ValidZTest)
                    {
                        m_depthOphelper.ReadZTestFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
                    ...
                    if (m_currentTemplate.DepthData.ValidOffset)
                    {
                        m_depthOphelper.ReadOffsetFromString(
                            ref m_currentReadParamIdx,
                            ref nodeParams
                        );
                    }
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_currentTemplate != null) ? m_currentTemplate.GUID : string.Empty
            );
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_currentTemplate != null) ? m_currentTemplate.DefaultShaderName : string.Empty
            );
