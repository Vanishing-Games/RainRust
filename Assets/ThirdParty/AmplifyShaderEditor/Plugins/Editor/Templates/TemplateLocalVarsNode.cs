    ...
    public sealed class TemplateLocalVarsNode : TemplateNodeParent
    {
        ...
        void UpdateFromId()
        {
            if (m_localVarsData != null)
            {
                ...
                string category =
                    m_localVarsData[m_currentDataIdx].Category == MasterNodePortCategory.Fragment
                        ? "Surface Data"
                        : "Vertex Data";
        ...
        )
        {
            ...
            {
                UIUtils.ShowMessage(
                    "Template Local Vars node is only intended for templates use only",
            ...
            if (m_localVarsData[m_currentDataIdx].Category != dataCollector.PortCategory)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    string.Format(
                        "Local Var {0} can only work on ports of type {1}",
                        m_localVarsData[m_currentDataIdx].LocalVarName,
                        m_localVarsData[m_currentDataIdx].Category
                    )
                );
                return m_outputPorts[0].ErrorValue;
            }
        ...
        void FetchLocalVarData(MasterNode masterNode = null)
        {
            ...
            if (m_multiPassMode)
            {
                ...
                {
                    m_localVarsData = m_templateMPData
