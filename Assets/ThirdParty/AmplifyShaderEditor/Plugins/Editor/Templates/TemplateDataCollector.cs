...
namespace AmplifyShaderEditor
{
    ...
    public class InterpDataHelper
    {
        ...
        public InterpDataHelper(
            WirePortDataType varType,
            string varName,
            bool isSingleComponent = true,
            bool setAtCompileTime = false
        )
        {
            VarName = varName;
            VarType = varType;
    ...
    public class TemplateInputParameters
    {
        ...
        )
        {
            ...
            Declaration = string.Format(
                "{0} {1} : {2}",
                UIUtils.PrecisionWirePortToCgType(precision, type),
                Name,
                Semantic
            );
    ...
    public class TemplateDataCollector
    {
        ...
        private List<PropertyDataCollector> m_lateDirectivesList =
            new List<PropertyDataCollector>();
        private Dictionary<string, PropertyDataCollector> m_lateDirectivesDict =
            new Dictionary<string, PropertyDataCollector>();

        private List<PropertyDataCollector> m_srpBatcherPropertiesList =
            new List<PropertyDataCollector>();
        private List<PropertyDataCollector> m_fullSrpBatcherPropertiesList =
            new List<PropertyDataCollector>();
        private Dictionary<string, PropertyDataCollector> m_srpBatcherPropertiesDict =
            new Dictionary<string, PropertyDataCollector>();

        public void CopySRPPropertiesFromDataCollector(
            int nodeId,
            TemplateDataCollector dataCollector
        )
        {
            for (int i = 0; i < dataCollector.SrpBatcherPropertiesList.Count; i++)
            {
                AddSRPBatcherProperty(
                    nodeId,
                    dataCollector.SrpBatcherPropertiesList[i].PropertyName
                );
            }
        }
        ...
        public void SetUVUsage(int uv, WirePortDataType type)
        {
            if (uv >= 0 && uv < MaxUV)
            {
                m_UVUsage[uv] = Mathf.Max(
                    m_UVUsage[uv],
                    TemplateHelperFunctions.DataTypeChannelUsage[type]
                );
            }
        }
            ...
            if (!m_lateDirectivesDict.ContainsKey(value))
            {
                ...
                switch (type)
                {
                    case AdditionalLineType.Include:
                        formattedValue = string.Format(Constants.IncludeFormat, value);
                        break;
                    case AdditionalLineType.Define:
                        formattedValue = string.Format(Constants.DefineFormat, value);
                        break;
                    case AdditionalLineType.Pragma:
                        formattedValue = string.Format(Constants.PragmaFormat, value);
                        break;
        ...
        public void SetMultipassInfo(
            TemplateMultiPass currentTemplate,
            int subShaderIdx,
            int passIdx,
            TemplateSRPType currentSRPType
        )
        {
            ...
            switch (type)
            {
                case AdditionalLineType.Include:
                {
                    return m_currentTemplate
                            ...
                            .Modules.IncludePragmaContainer.HasInclude(value)
                        || m_currentTemplate
                            .SubShaders[m_multipassSubshaderIdx]
                            .Passes[m_multipassPassIdx]
                            .Modules.IncludePragmaContainer.HasInclude(value);
                }
                case AdditionalLineType.Define:
                {
                    return m_currentTemplate
                            .SubShaders[m_multipassSubshaderIdx]
                            .Modules.IncludePragmaContainer.HasDefine(value)
                        || m_currentTemplate
                            .SubShaders[m_multipassSubshaderIdx]
                            .Passes[m_multipassPassIdx]
                            .Modules.IncludePragmaContainer.HasDefine(value);
                ...
                case AdditionalLineType.Pragma:
                {
                    return m_currentTemplate
                            .SubShaders[m_multipassSubshaderIdx]
                            .Modules.IncludePragmaContainer.HasPragma(value)
                        || m_currentTemplate
                            .SubShaders[m_multipassSubshaderIdx]
                            .Passes[m_multipassPassIdx]
                            .Modules.IncludePragmaContainer.HasPragma(value);
        ...
        {
            m_specialVertexLocalVars =
                new Dictionary<TemplateInfoOnSematics, TemplateLocalVarData>();
            ...
            for (int i = 0; i < localVarAmount; i++)
            {
                if (currentPass.LocalVarsList[i].IsSpecialVar)
                {
                    ...
                    {
                        m_specialVertexLocalVars.Add(
                            currentPass.LocalVarsList[i].SpecialVarType,
                            currentPass.LocalVarsList[i]
                        );
                    }
                    else
                    {
                        m_specialFragmentLocalVars.Add(
                            currentPass.LocalVarsList[i].SpecialVarType,
                            currentPass.LocalVarsList[i]
                        );
                    }
        ...
        public void BuildFromTemplateData(
            MasterNodeDataCollector dataCollector,
        ...
        )
        {
            ...
            if (templateData.VertexDataList != null)
            {
                ...
                {
                    m_vertexDataDict.Add(
                        templateData.VertexDataList[i].Semantics,
                        new TemplateVertexData(templateData.VertexDataList[i])
                    );
                }
            }
                ...
                for (int i = 0; i < fragCount; i++)
                {
                    ...
                    else if (templateData.InterpolatorData.Interpolators[i].IsSingleComponent)
                    {
                        varName = string.Format(
                            TemplateHelperFunctions.TemplateVarFormat,
                    ...
                    m_availableFragData.Add(
                        templateData.InterpolatorData.Interpolators[i].DataInfo,
                        new InterpDataHelper(
                            templateData.InterpolatorData.Interpolators[i].SwizzleType,
                            varName,
            ...
            if (templateData.VertexFunctionData != null && templateData.VertexDataList != null)
            {
                ...
                for (int i = 0; i < vertCount; i++)
                {
                    ...
                    m_availableVertData.Add(
                        templateData.VertexDataList[i].DataInfo,
                        new InterpDataHelper(
                            templateData.VertexDataList[i].SwizzleType,
                            varName,
        ...
        public void RegisterFragInputParams(
            WirePortDataType type,
            PrecisionType precision,
            string name,
            TemplateSemantics semantic,
        ...
        )
        {
            if (m_fragmentInputParams == null)
                m_fragmentInputParams =
                    new Dictionary<TemplateSemantics, TemplateInputParameters>();

            m_fragmentInputParams.Add(
                semantic,
                new TemplateInputParameters(type, precision, name, semantic, custom)
            );
        }
        ...
        )
        {
            ...
            m_fragmentInputParams.Add(
                semantic,
                new TemplateInputParameters(type, precision, name, semantic)
            );
        }

        public void RegisterVertexInputParams(
            WirePortDataType type,
            PrecisionType precision,
            string name,
            TemplateSemantics semantic,
        ...
        public string GetInstanceId()
        {
            ...
            {
                InterpDataHelper info = GetInfo(
                    TemplateInfoOnSematics.INSTANCEID,
                    useMasterNodeCategory,
                    customCategory
                );
                return info.VarName;
            }
            ...
            {
                MasterNodePortCategory category = useMasterNodeCategory
                    ? m_currentDataCollector.PortCategory
                    : customCategory;
                string name = TemplateHelperFunctions.SemanticsDefaultName[
                    TemplateSemantics.SV_InstanceID
                ];
                string varName = RegisterInfoOnSemantic(
                    category,
                    TemplateInfoOnSematics.INSTANCEID,
                    TemplateSemantics.INSTANCEID_SEMANTIC,
                    name,
        ...
        public string GetVertexId()
        {
            ...
            {
                InterpDataHelper info = GetInfo(
                    TemplateInfoOnSematics.VERTEXID,
                    useMasterNodeCategory,
                    customCategory
                );
                return TemplateHelperFunctions.AutoSwizzleData(
                    info.VarName,
                    info.VarType,
                    type,
                    true
            ...
            {
                MasterNodePortCategory portCategory = useMasterNodeCategory
                    ? m_currentDataCollector.PortCategory
                    : customCategory;
                string name = TemplateHelperFunctions.SemanticsDefaultName[
                    TemplateSemantics.SV_VertexID
                ];
                return RegisterInfoOnSemantic(
                    portCategory,
                    TemplateInfoOnSematics.VERTEXID,
                    TemplateSemantics.VERTEXID_SEMANTIC,
                    name,
                    WirePortDataType.UINT,
                    precision,
                    true
        ...
        public string GetPrimitiveId()
        {
            if (
                ...
                && m_fragmentInputParams.ContainsKey(TemplateSemantics.SV_PrimitiveID)
            )
                return m_fragmentInputParams[TemplateSemantics.SV_PrimitiveID].Name;

            string name = TemplateHelperFunctions.SemanticsDefaultName[
                TemplateSemantics.SV_PrimitiveID
            ];
        ...
        public string GetURPMainLight(int uniqueId, string shadowCoords = null)
        {
            ...
            m_currentDataCollector.AddLocalVariable(
                uniqueId,
                string.Format("Light ase_mainLight = GetMainLight( {0} );", shadowCoords)
            );
        ...
        public string GetVFace(int uniqueId)
        {
            if (IsSRP)
            {
                ...
                if (
                    GetCustomInterpolatedData(
                        TemplateInfoOnSematics.VFACE,
                        WirePortDataType.FLOAT,
                        PrecisionType.Float,
                        ref result,
                        true,
                        MasterNodePortCategory.Fragment
                    )
                )
                {
                ...
                else
                {
                    if (
                        m_fragmentInputParams != null
                        && m_fragmentInputParams.ContainsKey(TemplateSemantics.SV_IsFrontFacing)
                    )
                    ...
                    string custom =
                        ...
                        + TemplateHelperFunctions.SemanticsDefaultName[
                            TemplateSemantics.SV_IsFrontFacing
                        ]
                        + " : SV_IsFrontFace";
                    RegisterFragInputParams(
                        WirePortDataType.FLOAT,
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.SHADOWCOORDS,
            ...
            string shadowCoordsValue = string.Format(
                "TransformWorldToShadowCoord( {0} )",
                worldPos
            );
            if (m_currentDataCollector.PortCategory == MasterNodePortCategory.Fragment)
            {
                ...
                RegisterCustomInterpolatedData(
                    varName,
                    WirePortDataType.FLOAT4,
                    precision,
                    string.Format("TransformWorldToShadowCoord( {0} )", worldPos),
                    false,
                    MasterNodePortCategory.Fragment
                );
                m_currentDataCollector.AddLocalVariable(
        ...
        {
            return (m_currentDataCollector.PortCategory == MasterNodePortCategory.Fragment)
                ? m_availableFragData.ContainsKey(
                    TemplateHelperFunctions.IntToUVChannelInfo[uvChannel]
                )
                : m_availableVertData.ContainsKey(
                    TemplateHelperFunctions.IntToUVChannelInfo[uvChannel]
                );
        }
        ...
        {
            InterpDataHelper info =
                (m_currentDataCollector.PortCategory == MasterNodePortCategory.Fragment)
                    ? m_availableFragData[TemplateHelperFunctions.IntToUVChannelInfo[uvChannel]]
                    : m_availableVertData[TemplateHelperFunctions.IntToUVChannelInfo[uvChannel]];
        ...
        )
        {
            ...
            else if (m_currentDataCollector.TemplateDataCollectorInstance.HasUV(uvChannel))
            {
            ...
            if (isVertex)
            {
                string value = string.Format(
                    Constants.TilingOffsetFormat,
                    uvName,
                    propertyHelperVar + ".xy",
                    propertyHelperVar + ".zw"
                );
                string lodLevel = "0";
        ...
        public string GenerateAutoUVs(
            int uvChannel,
            WirePortDataType size = WirePortDataType.FLOAT2
        )
        {
            string uvName = string.Empty;
            string result = string.Empty;
        ...
        public string GetUV(
            int uvChannel,
            MasterNodePortCategory category = MasterNodePortCategory.Fragment,
            WirePortDataType size = WirePortDataType.FLOAT4
        )
        {
            if (!HasUV(uvChannel))
            {
        ...
        public string RegisterUV(int UVChannel, WirePortDataType size = WirePortDataType.FLOAT2)
        {
            ...
            if (m_currentDataCollector.PortCategory == MasterNodePortCategory.Vertex)
            {
                ...
                string varName =
                    TemplateHelperFunctions.BaseInterpolatorName
                    + ((UVChannel > 0) ? UVChannel.ToString() : string.Empty);
                m_availableVertData.Add(
                    TemplateHelperFunctions.IntToUVChannelInfo[UVChannel],
                    new InterpDataHelper(
                        WirePortDataType.FLOAT4,
                ...
                m_currentDataCollector.AddToVertexInput(
                    string.Format(TemplateHelperFunctions.TexFullSemantic, varName, semantic)
                );
                RegisterOnVertexData(semantic, size, varName);
                string finalVarName = m_availableVertData[
                    TemplateHelperFunctions.IntToUVChannelInfo[UVChannel]
                ].VarName;
                switch (size)
                {
            ...
            else
            {
                ...
                foreach (
                    KeyValuePair<TemplateSemantics, TemplateVertexData> kvp in m_vertexDataDict
                )
                {
                ...
                if (vertexSemantics == TemplateSemantics.NONE)
                {
                    ...
                    if (!m_vertexDataDict.ContainsKey(vertexSemantics))
                    {
                        ...
                        m_currentDataCollector.AddToVertexInput(
                            string.Format(
                                TemplateHelperFunctions.TexFullSemantic,
                                varName,
                                vertexSemantics
                            )
                        );
                ...
                {
                    bool isPosition =
                        vertexSemantics == TemplateSemantics.POSITION
                        || vertexSemantics == TemplateSemantics.POSITION;

                    string interpVarName =
                        m_currentTemplateData.VertexFunctionData.OutVarName
                        + "."
                        + availableInterp.VarNameWithSwizzle;
                    InterpDataHelper vertInfo = m_availableVertData[
                        TemplateHelperFunctions.IntToUVChannelInfo[UVChannel]
                    ];
                    string interpDecl = string.Format(
                        TemplateHelperFunctions.TemplateVariableDecl,
                        interpVarName,
                        TemplateHelperFunctions.AutoSwizzleData(
                            vertInfo.VarName,
                            vertInfo.VarType,
                            size,
                            isPosition
                    ...
                    string finalVarName =
                        m_currentTemplateData.FragmentFunctionData.InVarName
                        + "."
                        + availableInterp.VarNameWithSwizzle;
                    m_availableFragData.Add(
                        TemplateHelperFunctions.IntToUVChannelInfo[UVChannel],
                        new InterpDataHelper(size, finalVarName)
                    );
                    if (size != originalSize)
                    {
                        ...
                        finalVarName =
                            m_availableFragData[
                                TemplateHelperFunctions.IntToUVChannelInfo[UVChannel]
                            ].VarName + UIUtils.GetAutoSwizzle(originalSize);
                    }
                    return finalVarName;
                }
            }
        ...
        )
        {
            MasterNodePortCategory category = useMasterNodeCategory
                ? m_currentDataCollector.PortCategory
                : customCategory;
            return (category == MasterNodePortCategory.Fragment)
                ? m_availableFragData.ContainsKey(info)
                : m_availableVertData.ContainsKey(info);
        }

        public InterpDataHelper GetInfo(
            TemplateInfoOnSematics info,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            else
            {
                if (!m_availableVertData[info].SetAtCompileTime)
                {
                    string defineValue = string.Empty;
                    if (
                        TemplateHelperFunctions.InfoToDefineVertex.TryGetValue(
                            info,
                            out defineValue
                        )
                    )
        ...
        public string RegisterInfoOnSemantic(
            TemplateInfoOnSematics info,
            TemplateSemantics semantic,
            string name,
            WirePortDataType dataType,
            PrecisionType precisionType,
            bool requestNewInterpolator,
        ...
        public string RegisterInfoOnSemantic(
            MasterNodePortCategory portCategory,
            TemplateInfoOnSematics info,
            TemplateSemantics semantic,
            string name,
            WirePortDataType dataType,
        ...
        )
        {
            if (portCategory == MasterNodePortCategory.Vertex)
            {
                ...
                string vertInputVarType = UIUtils.PrecisionWirePortToCgType(
                    precisionType,
                    dataType
                );
                ...
                m_currentDataCollector.AddToVertexInput(
                    string.Format(
                        TemplateHelperFunctions.InterpFullSemantic,
                        vertInputVarType,
                        name,
                        semantic
                    )
            ...
            else
            {
                ...
                if (availableInterp != null)
                {
                    ...
                    string interpDecl = string.Format(
                        ...
                        interpVarName,
                        TemplateHelperFunctions.AutoSwizzleData(
                            m_availableVertData[info].VarName,
                            m_availableVertData[info].VarType,
                            dataType,
                            isPosition
                        )
                    );
        ...
        TemplateVertexData RegisterOnInterpolator(
            TemplateSemantics semantics,
            WirePortDataType dataType,
            string vertexDataName = null
        )
        {
            ...
            string interpolator = string.Format(
                TemplateHelperFunctions.InterpFullSemantic,
                UIUtils.WirePortToCgType(dataType),
                data.VarName,
                data.Semantics
            );
        ...
        public void RegisterOnVertexData(
            TemplateSemantics semantics,
            WirePortDataType dataType,
            string varName
        )
        ...
        public TemplateVertexData RequestMacroInterpolator(string varName)
        {
            ...
            for (int i = 0; i < m_interpolatorData.AvailableInterpolators.Count; i++)
            {
                ...
                {
                    TemplateVertexData data = m_interpolatorData
                        .AvailableInterpolators[i]
                        .RequestChannels(WirePortDataType.FLOAT4, false, varName);
                    if (data != null)
                    {
                        ...
                        {
                            string interpolator = string.Format(
                                TemplateHelperFunctions.InterpMacro,
                                varName,
                                TemplateHelperFunctions.SemanticToInt[data.Semantics]
                            );
                            m_currentDataCollector.AddToInterpolators(interpolator);
                        }
                        return data;
        ...
        public TemplateVertexData RequestNewInterpolator(
            WirePortDataType dataType,
            bool isColor,
            string varName = null,
            bool noInterpolationFlag = false,
        ...
        )
        {
            ...
            for (int i = 0; i < m_interpolatorData.AvailableInterpolators.Count; i++)
            {
                ...
                {
                    if (
                        m_interpolatorData.AvailableInterpolators[i].Usage != 0
                        && (
                            m_interpolatorData.AvailableInterpolators[i].NoInterpolation
                                != noInterpolationFlag
                    ...
                    TemplateVertexData data = m_interpolatorData
                        .AvailableInterpolators[i]
                        .RequestChannels(dataType, isColor, varName);
                    if (data != null)
                    {
                        if (!m_registeredVertexData.ContainsKey(data.VarName))
                        {
            ...
            int newInterpId =
                1
                + TemplateHelperFunctions.SemanticToInt[
                    m_interpolatorData
                        .AvailableInterpolators[m_interpolatorData.AvailableInterpolators.Count - 1]
                        .Semantic
                ];
            if (TemplateHelperFunctions.IntToSemantic.ContainsKey(newInterpId))
            {
                TemplateInterpElement item = new TemplateInterpElement(
                    TemplateHelperFunctions.IntToSemantic[newInterpId]
                );
        ...
        public List<string> GetInterpUnusedChannels()
        {
            ...
            if (m_interpolatorData != null)
            {
                for (int i = 0; i < m_interpolatorData.AvailableInterpolators.Count; i++)
                {
                    if (
                        m_interpolatorData.AvailableInterpolators[i].Usage > 0
                        && !m_interpolatorData.AvailableInterpolators[i].IsFull
                    )
                    {
                        string channels = string.Empty;
                        bool[] availableChannels = m_interpolatorData
                            .AvailableInterpolators[i]
                            .AvailableChannels;
                        ...
                        resetInstrucctions.Add(
                            string.Format(
                                "{0}.{1}.{2} = 0;",
                                m_currentTemplateData.VertexFunctionData.OutVarName,
                                m_interpolatorData.AvailableInterpolators[i].Name,
                                channels
                            )
                        );
                    }
        ...
        public bool ContainsSpecialLocalFragVar(
            TemplateInfoOnSematics info,
            WirePortDataType type,
            ref string result
        )
        {
            if (m_specialFragmentLocalVars.ContainsKey(info))
            {
                ...
                {
                    result = TemplateHelperFunctions.AutoSwizzleData(
                        result,
                        m_specialFragmentLocalVars[info].DataType,
                        type,
                        false
                    );
                }
        ...
        public bool GetCustomInterpolatedData(
            TemplateInfoOnSematics info,
            WirePortDataType type,
            PrecisionType precisionType,
            ref string result,
            bool useMasterNodeCategory,
            MasterNodePortCategory customCategory
        )
        {
            bool isPosition =
                info == TemplateInfoOnSematics.POSITION
            ...
            if (category == MasterNodePortCategory.Vertex)
            {
                if (m_specialVertexLocalVars.ContainsKey(info))
                {
                    ...
                    if (m_specialVertexLocalVars[info].DataType != type)
                    {
                        result = TemplateHelperFunctions.AutoSwizzleData(
                            result,
                            m_specialVertexLocalVars[info].DataType,
                            type,
                            isPosition
                        );
                    }
                ...
                if (m_availableFragData.ContainsKey(info))
                {
                    if (m_availableFragData[info].IsSingleComponent)
                    {
                        ...
                        if (m_availableFragData[info].VarType != type)
                        {
                            result = TemplateHelperFunctions.AutoSwizzleData(
                                result,
                                m_availableFragData[info].VarType,
                                type,
                                isPosition
                            );
                        }
                        ...
                        TemplateHelperFunctions.InfoToLocalVar.ContainsKey(info)
                    ...
                    )
                    {
                        ...
                        m_currentDataCollector.AddLocalVariable(
                            -1,
                            precisionType,
                            TemplateHelperFunctions.InfoToWirePortType[info],
                            result,
                            m_availableFragData[info].VarName
                        );
                        return true;
                    }
                }
        ...
        {
            InterpDataHelper info = GetInfo(
                TemplateInfoOnSematics.POSITION,
                false,
                MasterNodePortCategory.Vertex
            );
            return info.VarType;
        }

        public string GetVertexPosition(
            WirePortDataType type,
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.OBJECT_POSITION,
        ...
        private const string InstancingLibSRP =
        ...
        public void SetupInstancing()
        {
            if (!HasInfo(TemplateInfoOnSematics.INSTANCE_ID))
            {
                ...
                m_currentDataCollector.AddToIncludes(
                    -1,
                    IsSRP ? InstancingLibSRP : InstancingLibStandard
                );
                ...
                m_currentDataCollector.AddToLocalVariables(
                    MasterNodePortCategory.Vertex,
                    -1,
                    string.Format(
                        "UNITY_SETUP_INSTANCE_ID( {0} );",
                        m_currentTemplateData.VertexFunctionData.InVarName
                    )
                );
                m_currentDataCollector.AddToLocalVariables(
                    ...
                    -1,
                    string.Format(
                        "UNITY_TRANSFER_INSTANCE_ID( {0}, {1} );",
                        m_currentTemplateData.VertexFunctionData.InVarName,
                        m_currentTemplateData.VertexFunctionData.OutVarName
                    )
                );
        ...
        public string GetVertexColor(PrecisionType precisionType)
        {
            ...
            else
            {
                ...
                return RegisterInfoOnSemantic(
                    TemplateInfoOnSematics.COLOR,
                    TemplateSemantics.COLOR,
                    name,
                    WirePortDataType.FLOAT4,
                    precisionType,
                    false
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.WORLD_NORMAL,
                    WirePortDataType.FLOAT3,
                    precisionType,
                    ref result,
                    useMasterNodeCategory,
            ...
            if (
                !GetCustomInterpolatedData(
                    ...
                    WirePortDataType.FLOAT3,
                    precisionType,
                    ref worldNormalValue,
                    false,
                    MasterNodePortCategory.Vertex
                )
            )
            {
                string vertexNormal = GetVertexNormal(
                    precisionType,
                    false,
                    MasterNodePortCategory.Vertex
                );
        ...
        )
        {
            ...
            m_currentDataCollector.AddLocalVariable(
                uniqueId,
                "float3 " + tanNormal + " = " + normal + ";"
            );
            return string.Format(
                "float3( dot( {1}, {0} ), dot( {2}, {0} ), dot( {3}, {0} ) )",
                tanNormal,
                tanToWorld0,
                tanToWorld1,
                tanToWorld2
        ...
        )
        {
            ...
            {
                InterpDataHelper info = GetInfo(
                    TemplateInfoOnSematics.TANGENT,
                    useMasterNodeCategory,
                    customCategory
                );
                if (type != WirePortDataType.OBJECT && type != info.VarType)
        ...
        )
        {
            ...
            {
                InterpDataHelper info = GetInfo(
                    TemplateInfoOnSematics.BLENDWEIGHTS,
                    useMasterNodeCategory,
                    customCategory
                );
                return info.VarName;
            ...
            else
            {
                ...
                string varName = RegisterInfoOnSemantic(
                    category,
                    TemplateInfoOnSematics.BLENDWEIGHTS,
                    TemplateSemantics.BLENDWEIGHTS,
                    name,
                    WirePortDataType.FLOAT4,
                    PrecisionType.Float,
                    false,
        ...
        )
        {
            ...
            {
                InterpDataHelper info = GetInfo(
                    TemplateInfoOnSematics.BLENDINDICES,
                    useMasterNodeCategory,
                    customCategory
                );
                return info.VarName;
            ...
            else
            {
                ...
                string varName = RegisterInfoOnSemantic(
                    category,
                    TemplateInfoOnSematics.BLENDINDICES,
                    TemplateSemantics.BLENDINDICES,
                    name,
                    WirePortDataType.UINT4,
                    PrecisionType.Float,
                    false,
                    name
                );
        ...
        )
        {
            ...
            string normalValue = GetVertexNormal(
                precisionType,
                false,
                MasterNodePortCategory.Vertex
            );

            string bitangentValue = string.Format(
                "cross( {0}, {1}.xyz ) * {1}.w * ( unity_WorldTransformParams.w >= 0.0 ? 1.0 : -1.0 )",
                normalValue,
                tangentValue
            );
            RegisterCustomInterpolatedData(
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.WORLD_TANGENT,
                    WirePortDataType.FLOAT3,
                    precisionType,
                    ref result,
                    useMasterNodeCategory,
            ...
            if (
                !GetCustomInterpolatedData(
                    ...
                    WirePortDataType.FLOAT3,
                    precisionType,
                    ref worldTangentValue,
                    false,
                    MasterNodePortCategory.Vertex
                )
            )
            {
                string vertexTangent = GetVertexTangent(
                    WirePortDataType.FLOAT4,
                    precisionType,
                    false,
                    MasterNodePortCategory.Vertex
        ...
        )
        {
            ...
            string tangentSignValue = string.Format(
                "{0}.w * ( unity_WorldTransformParams.w >= 0.0 ? 1.0 : -1.0 )",
                tangentValue
            );
            RegisterCustomInterpolatedData(
                varName,
        ...
        public string GetWorldBinormal(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.WORLD_BITANGENT,
                    WirePortDataType.FLOAT3,
                    precisionType,
                    ref result,
                    useMasterNodeCategory,
            ...
            )
            {
                ...
                string tangentSign = GetTangentSign(
                    precisionType,
                    false,
                    MasterNodePortCategory.Vertex
                );
                worldBinormal = string.Format(
                    "cross( {0}, {1} ) * {2}",
                    worldNormal,
                    worldtangent,
                    tangentSign
        ...
        public string GetWorldReflection(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment,
            bool normalize = false
        ...
        public string GetWorldReflection(PrecisionType precisionType, string normal)
        {
            ...
            return string.Format(
                "reflect( -{0}, float3( dot( {2}, {1} ), dot( {3}, {1} ), dot( {4}, {1} ) ) )",
                worldRefl,
                normal,
                tanToWorld0,
                tanToWorld1,
        ...
        )
        {
            ...
            m_currentDataCollector.AddLocalVariable(
                uniqueId,
                "UNITY_LIGHT_ATTENUATION( ase_atten, " + fInName + ", " + worldPos + " )"
            );
            return "ase_atten";
        }

        public string GenerateRotationIndependentObjectScale(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId
        )
        {
            string worldToObjectMatrix =
                (m_currentSRPType == TemplateSRPType.BiRP)
                    ? "unity_WorldToObject"
                    : "GetWorldToObjectMatrix()";
            string value = string.Format(
                "( 1.0 / float3( length( {0}[ 0 ].xyz ), length( {0}[ 1 ].xyz ), length( {0}[ 2 ].xyz ) ) )",
                worldToObjectMatrix
            );
            dataCollector.AddLocalVariable(
                ...
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ParentObjectScaleStr,
                value
            );
        ...
        public string GenerateObjectScale(ref MasterNodeDataCollector dataCollector, int uniqueId)
        {
            ...
            string value = string.Format(
                "float3( length( {0}[ 0 ].xyz ), length( {0}[ 1 ].xyz ), length( {0}[ 2 ].xyz ) )",
                worldToObjectMatrix
            );
            dataCollector.AddLocalVariable(
                uniqueId,
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ObjectScaleStr,
                value
            );
            return GeneratorUtils.ObjectScaleStr;
        }

        public string GenerateObjectPosition(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId
        )
        {
            ...
            dataCollector.AddLocalVariable(
                ...
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ObjectPositionStr,
                value
            );
            return GeneratorUtils.ObjectPositionStr;
        }

        public string GenerateObjectBoundsMin(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId
        )
        {
            ...
            if (
                m_currentSRPType != TemplateSRPType.BiRP
                && (
                    ASEPackageManagerHelper.CurrentHDRPBaseline >= ASESRPBaseline.ASE_SRP_14
            ...
            dataCollector.AddLocalVariable(
                ...
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ObjectBoundsMinStr,
                value
            );
            return GeneratorUtils.ObjectBoundsMinStr;
        }

        public string GenerateObjectBoundsMax(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId
        )
        {
            ...
            dataCollector.AddLocalVariable(
                ...
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ObjectBoundsMaxStr,
                value
            );
            return GeneratorUtils.ObjectBoundsMaxStr;
        }

        public string GenerateObjectBoundsSize(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId
        )
        {
            ...
            dataCollector.AddLocalVariable(
                ...
                PrecisionType.Float,
                WirePortDataType.FLOAT3,
                GeneratorUtils.ObjectBoundsSizeStr,
                value
            );
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.WORLD_POSITION,
                    WirePortDataType.FLOAT3,
                    precision,
                    ref result,
                    useMasterNodeCategory,
            ...
            else if (m_currentSRPType == TemplateSRPType.HDRP)
            {
                if (
                    GetCustomInterpolatedData(
                        TemplateInfoOnSematics.RELATIVE_WORLD_POS,
                        WirePortDataType.FLOAT3,
                        precision,
                        ref result,
                        useMasterNodeCategory,
                ...
                )
                {
                    ...
                    string relWorldPosConversion = string.Format(
                        "GetAbsolutePositionWS( {0} )",
                        result
                    );
                    m_currentDataCollector.AddLocalVariable(
                        -1,
            ...
            {
                UIUtils.ShowMessage(
                    "Attempting to access inexisting vertex position to calculate world pos"
                );
                return "half3(0,0,0)";
            }
            ...
            )
            {
                ...
                {
                    worldPosConversion = string.Format(
                        "GetAbsolutePositionWS( TransformObjectToWorld( ( {0} ).xyz ) )",
                        vertexPos
                    );
                }
                ...
                {
                    worldPosConversion = string.Format(
                        "mul( unity_ObjectToWorld, float4( ( {0} ).xyz, 1 ) ).xyz",
                        vertexPos
                    );
                }
            }
        ...
        public string GetRelativeWorldPos(
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (
                !m_currentDataCollector.HasLocalVariableByName(
                    GeneratorUtils.RelativeWorldPositionStr
                )
            )
            {
                string positionOS = GetVertexPosition(
                    WirePortDataType.FLOAT4,
                    precision,
                    false,
                    MasterNodePortCategory.Vertex
                ...
                string invProjMatrix = GeneratorUtils.GenerateInverseProjection(
                ...
                {
                    m_currentDataCollector.AddToVertexLocalVariables(
                        -1,
                        precision,
                        WirePortDataType.FLOAT4,
                        GeneratorUtils.ViewHPositionStr,
                        string.Format(
                            "mul( {0}, {1} )",
                            invProjMatrix,
                            GeneratorUtils.ClipPositionStr
                        )
                    );
                }
                ...
                m_currentDataCollector.AddToVertexLocalVariables(
                    ...
                    GeneratorUtils.RelativeWorldPositionStr,
                    string.Format(
                        "mul( ( float3x3 )UNITY_MATRIX_I_V, {0}.xyz / {0}.w )",
                        GeneratorUtils.ViewHPositionStr
                    )
                );
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.VIEW_POSITION,
                    WirePortDataType.FLOAT3,
                    precision,
                    ref result,
                    useMasterNodeCategory,
            ...
            {
                if (
                    portCategory == MasterNodePortCategory.Vertex
                    || portCategory == MasterNodePortCategory.Tessellation
                )
                {
                    string positionOS = GetVertexPosition(
                        WirePortDataType.FLOAT3,
                    ...
                    result = string.Format(
                        IsSRP
                            ? "TransformWorldToView( TransformObjectToWorld( {0} ) )"
                            : "UnityObjectToViewPos( {0} )",
                        positionOS
                    );
                }
                else
                {
                    ...
                    result = string.Format(
                        IsSRP ? "TransformWorldToView( {0} )" : "UnityWorldToViewPos( {0} )",
                        positionWS
                    );
                }
                ...
                m_currentDataCollector.AddLocalVariable(
                    ...
                    precision,
                    WirePortDataType.FLOAT3,
                    GeneratorUtils.ViewPositionStr,
                    result
                );
            }
        ...
        public string GetClipPosForValue(
            string customVertexPos,
            string outputId,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            {
                UIUtils.ShowMessage(
                    "Attempting to access inexisting vertex position to calculate clip pos"
                );
        ...
        public string GetScreenPosForValue(
            PrecisionType precision,
            string customVertexPos,
            string outputId,
            bool useMasterNodeCategory = true,
        ...
        )
        {
            ...
            string clipSpacePos = GetClipPosForValue(
                customVertexPos,
                outputId,
                false,
                MasterNodePortCategory.Vertex
            ...
            if (m_currentSRPType == TemplateSRPType.HDRP)
            {
                screenPosConversion = string.Format(
                    "ComputeScreenPos( {0}, _ProjectionParams.x )",
                    clipSpacePos
                );
            }
            else
        ...
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.SCREEN_POSITION,
                    WirePortDataType.FLOAT4,
                    precision,
                    ref result,
                    useMasterNodeCategory,
        ...
        public string GetScreenPosNormalized(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (
                GetCustomInterpolatedData(
                    TemplateInfoOnSematics.SCREEN_POSITION_NORMALIZED,
                    WirePortDataType.FLOAT4,
                    precision,
                    ref result,
                    useMasterNodeCategory,
            ...
            {
                string screenPos = GetScreenPosRaw(
                    precision,
                    useMasterNodeCategory,
                    customCategory
                );
                string clipPlaneTestOp = string.Format(
        ...
        public string GetScreenPosCenter(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            string screenPosNorm = GetScreenPosNormalized(
                precision,
                useMasterNodeCategory,
                customCategory
            );
            if (!m_currentDataCollector.HasLocalVariableByName(GeneratorUtils.ScreenPosCenterStr))
            {
                ...
                m_currentDataCollector.AddLocalVariable(
                    ...
                    precision,
                    WirePortDataType.FLOAT4,
                    GeneratorUtils.ScreenPosCenterStr,
                    value
                );
            }
        ...
        public string GetScreenPosTiled(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            if (!m_currentDataCollector.HasLocalVariableByName(GeneratorUtils.ScreenPosTiledStr))
            {
                ...
                string value = string.Format(
                    "frac( float4( ( {0}.x * 2 - 1 ) * _ScreenParams.x / _ScreenParams.y, {0}.y * 2 - 1, 0, 0 ) )",
                    screenPosNorm
                );
                m_currentDataCollector.AddLocalVariable(
                    -1,
                    precision,
                    WirePortDataType.FLOAT4,
                    GeneratorUtils.ScreenPosTiledStr,
                    value
                );
            }
        ...
        public string GetScreenPosPixel(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            if (!m_currentDataCollector.HasLocalVariableByName(GeneratorUtils.ScreenPosPixelStr))
            {
                ...
                m_currentDataCollector.AddLocalVariable(
                    ...
                    precision,
                    WirePortDataType.FLOAT4,
                    GeneratorUtils.ScreenPosPixelStr,
                    GeneratorUtils.GenerateScreenPosNormToPixelFunctionCall(screenPosNorm)
                );
            }
        ...
        )
        {
            ...
            switch (depthMode)
            {
                ...
                {
                    instruction = GeneratorUtils.ApplyLinearDepthModifier(
                        ref m_currentDataCollector,
                        instruction,
                        depthMode
                    );
                ...
                case DepthMode.DepthEye:
                {
                    instruction = string.Format(
                        "( {0} ) * ( _ProjectionParams.z - _ProjectionParams.y )",
                        instruction
                    );
        ...
        public string GetViewVector(
            PrecisionType precisionType = PrecisionType.Float,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment,
            ViewSpace space = ViewSpace.World
        )
        {
            string varName;
            switch (space)
            {
                case ViewSpace.Tangent:
                    varName = GeneratorUtils.TangentViewVectorStr;
                    break;
                case ViewSpace.Object:
                    varName = GeneratorUtils.ObjectViewVectorStr;
                    break;
                case ViewSpace.View:
                    varName = GeneratorUtils.ViewViewVectorStr;
                    break;
                case ViewSpace.World:
                default:
                    varName = GeneratorUtils.WorldViewVectorStr;
                    break;
            }
            if (HasCustomInterpolatedData(varName, useMasterNodeCategory, customCategory))
            {
        ...
        public string GetViewDir(
            ...
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment,
            NormalizeType normalizeType = NormalizeType.Regular,
            ViewSpace space = ViewSpace.World
        )
        {
            if (space == ViewSpace.World)
            {
                ...
                if (
                    GetCustomInterpolatedData(
                        TemplateInfoOnSematics.WORLD_VIEW_DIR,
                        WirePortDataType.FLOAT3,
                        precisionType,
                        ref result,
                        useMasterNodeCategory,
            ...
            switch (space)
            {
                case ViewSpace.Tangent:
                    varName =
                        (normalizeType == NormalizeType.Regular)
                            ? GeneratorUtils.TangentViewDirectionStr
                            : GeneratorUtils.TangentViewDirectionSafeStr;
                    break;
                case ViewSpace.Object:
                    varName =
                        (normalizeType == NormalizeType.Regular)
                            ? GeneratorUtils.ObjectViewDirectionStr
                            : GeneratorUtils.ObjectViewDirectionSafeStr;
                    break;
                case ViewSpace.View:
                    varName =
                        (normalizeType == NormalizeType.Regular)
                            ? GeneratorUtils.ViewViewDirectionStr
                            : GeneratorUtils.ViewViewDirectionSafeStr;
                    break;
                case ViewSpace.World:
                default:
                    varName =
                        (normalizeType == NormalizeType.Regular)
                            ? GeneratorUtils.WorldViewDirectionStr
                            : GeneratorUtils.WorldViewDirectionSafeStr;
                    break;
            }

            if (!m_currentDataCollector.HasLocalVariableByName(varName))
            {
                string viewVector = GetViewVector(
                    precisionType,
                    useMasterNodeCategory,
                    customCategory,
                    space
                ...
                {
                    viewVector = TemplateHelperFunctions.SafeNormalize(
        ...
        public void GetWorldTangentTf(
            PrecisionType precisionType,
            out string tanToWorld0,
            out string tanToWorld1,
            out string tanToWorld2,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (
                HasCustomInterpolatedData(tanToWorld0, useMasterNodeCategory, customCategory)
            ...
            string tanToWorldVar0 = string.Format(
                "float3( {0}.x, {1}.x, {2}.x )",
                worldTangent,
                worldBinormal,
                worldNormal
            );
            string tanToWorldVar1 = string.Format(
                "float3( {0}.y, {1}.y, {2}.y )",
                worldTangent,
                worldBinormal,
                worldNormal
            );
            string tanToWorldVar2 = string.Format(
                "float3( {0}.z, {1}.z, {2}.z )",
                worldTangent,
                worldBinormal,
                worldNormal
        ...
        public string GetTangentToWorldMatrixFast(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            string result = string.Format(
                "float3x3( {0}.x, {1}.x, {2}.x, {0}.y, {1}.y, {2}.y, {0}.z, {1}.z, {2}.z )",
                worldTangent,
                worldBinormal,
                worldNormal
            );
            m_currentDataCollector.AddLocalVariable(
                ...
                GeneratorUtils.TangentToWorldFastStr,
                result
            );
            return GeneratorUtils.TangentToWorldFastStr;
        }

        public string GetTangentToWorldMatrixPrecise(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            string worldToTangent = GetWorldToTangentMatrix(
                precisionType,
                useMasterNodeCategory,
                customCategory
            );
            GeneratorUtils.Add3x3InverseFunction(
                ref m_currentDataCollector,
                UIUtils.PrecisionWirePortToCgType(precisionType, WirePortDataType.FLOAT)
            );
            m_currentDataCollector.AddLocalVariable(
                -1,
                precisionType,
                WirePortDataType.FLOAT3x3,
                GeneratorUtils.TangentToWorldPreciseStr,
                string.Format(GeneratorUtils.Inverse3x3Header, worldToTangent)
            );
            return GeneratorUtils.TangentToWorldPreciseStr;
        }

        public string GetWorldToTangentMatrix(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (HasCustomInterpolatedData(varName, useMasterNodeCategory, customCategory))
                return varName;
            string worldTanMat = string.Format(
                "float3x3( {0}, {1}, {2} )",
                worldTangent,
                worldBinormal,
                worldNormal
        ...
        public string GetObjectToViewPos(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            if (HasCustomInterpolatedData(varName, useMasterNodeCategory, customCategory))
                return varName;
            string vertexPos = GetVertexPosition(
                WirePortDataType.FLOAT3,
                precision,
                false,
                MasterNodePortCategory.Vertex
        ...
        )
        {
            ...
            if (HasCustomInterpolatedData(varName, useMasterNodeCategory, customCategory))
                return varName;
            string objectToView = GetObjectToViewPos(
                precision,
                false,
                MasterNodePortCategory.Vertex
            );
        ...
        public string GetObjectSpaceLightDir(
            PrecisionType precisionType,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            switch (m_currentSRPType)
            {
                ...
                case TemplateSRPType.BiRP:
                    ...
                    break;
                case TemplateSRPType.HDRP:
                    string worldSpaceLightDir = GetWorldSpaceLightDir(
                        precisionType,
                        useMasterNodeCategory,
                        customCategory
                    );
                    objectSpaceLightDir = string.Format(
                        "mul( GetWorldToObjectMatrix(), {0} ).xyz",
                        worldSpaceLightDir
                    );
                    break;
                case TemplateSRPType.URP:
        ...
        public string GetWorldSpaceLightDir(
            PrecisionType precision,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment
        )
        {
            ...
            else
            {
                ...
                if (m_currentSRPType == TemplateSRPType.HDRP)
                {
                    ...
                    lightVar =
                        "-"
                        + string.Format(TemplateHelperFunctions.HDLightInfoFormat, "0", "forward");
                }
                else
                {
                    lightVar = "_MainLightPosition.xyz";
                }
                return m_currentDataCollector.SafeNormalizeLightDir
                    ? string.Format("SafeNormalize( {0} )", lightVar)
                    : lightVar;
            }
            ...
            if (m_currentDataCollector.SafeNormalizeLightDir)
            {
                ...
                else
                {
                    ...
                    worldSpaceLightDir = string.Format(
                        "Unity_SafeNormalize( {0} )",
                        worldSpaceLightDir
                    );
                }
            }
        ...
        public void RegisterCustomInterpolatedData(
            string name,
            WirePortDataType dataType,
            PrecisionType precision,
            string vertexInstruction,
            bool useMasterNodeCategory = true,
            MasterNodePortCategory customCategory = MasterNodePortCategory.Fragment,
            bool noInterpolationFlag = false,
            bool sampleFlag = false
        )
        {
            ...
            if (category == MasterNodePortCategory.Fragment)
            {
                if (!m_customInterpolatedData[name].IsFragment)
                {
                    ...
                    TemplateVertexData interpData = RequestNewInterpolator(
                        dataType,
                        false,
                        null,
                        noInterpolationFlag,
                    ...
                    if (interpData == null)
                    {
                        Debug.LogErrorFormat(
                            "Could not assign interpolator of type {0} to variable {1}",
                            dataType,
                            name
                        );
                        return;
        ...
        )
        {
            if (m_customInterpolatedData.ContainsKey(name))
            {
                MasterNodePortCategory category = useMasterNodeCategory
                    ...
                    : customCategory;
                return (category == MasterNodePortCategory.Fragment)
                    ? m_customInterpolatedData[name].IsFragment
                    : m_customInterpolatedData[name].IsVertex;
            }
            return false;
        }
        ...
        public void BuildCBuffer(int nodeId)
        {
            ...
            if (m_srpBatcherPropertiesList.Count > 0)
            {
                ...
                m_srpBatcherPropertiesList.Sort(
                    (a, b) =>
                    {
                        ...
                        if (matchB.Groups.Count > 1 && matchB.Groups[1].Value.Length > 0)
                            sizeB = Convert.ToInt32(
                                matchB.Groups[1].Value,
                                System.Globalization.CultureInfo.InvariantCulture
                            );
                ...
                m_fullSrpBatcherPropertiesList.Insert(
                    0,
                    new PropertyDataCollector(nodeId, IOUtils.SRPCBufferPropertiesBegin)
                );
                m_fullSrpBatcherPropertiesList.AddRange(m_srpBatcherPropertiesList);
                m_fullSrpBatcherPropertiesList.Add(
                    new PropertyDataCollector(nodeId, IOUtils.SRPCBufferPropertiesEnd)
                );
            }
        }
        ...
        public const string GlobalMaxInterpolatorReachedMsg =
            "Maximum amount of interpolators reached!\nPlease consider optmizing your shader!";
        public const string MaxInterpolatorSMReachedMsg =
            "Maximum amount of interpolators reached for current shader model on pass {0}! Please consider increasing the shader model to {1}!";

        public void CheckInterpolatorOverflow(string currShaderModel, string passName)
        {
            int maxInterpolatorAmount = TemplateHelperFunctions.AvailableInterpolators[
                currShaderModel
            ];
            int currInterpolatorAmount =
                1
                + TemplateHelperFunctions.SemanticToInt[
                    InterpData
                        .AvailableInterpolators[InterpData.AvailableInterpolators.Count - 1]
                        .Semantic
                ];
            if (currInterpolatorAmount > maxInterpolatorAmount)
            {
                ...
                if (
                    TemplateHelperFunctions.GetShaderModelForInterpolatorAmount(
                        currInterpolatorAmount,
                        ref shaderModel
                    )
                )
                {
                    UIUtils.ShowMessage(
                        string.Format(MaxInterpolatorSMReachedMsg, passName, shaderModel),
        ...
        public Dictionary<TemplateSemantics, TemplateInputParameters> FragInputParameters
        {
            get { return m_fragmentInputParams; }
        }
        ...
        public Dictionary<TemplateSemantics, TemplateInputParameters> VertexInputParameters
        {
            get { return m_vertexInputParams; }
        }
        public TemplateData CurrentTemplateData
        {
            get { return m_currentTemplateData; }
        }
        public int MultipassSubshaderIdx
        {
            get { return m_multipassSubshaderIdx; }
        }
        public int MultipassPassIdx
        {
            get { return m_multipassPassIdx; }
        }
        public TemplateSRPType CurrentSRPType
        {
            get { return m_currentSRPType; }
            set { m_currentSRPType = value; }
        }
        public bool IsHDRP
        {
            get { return m_currentSRPType == TemplateSRPType.HDRP; }
        }
        public bool IsLWRP
        {
            get { return m_currentSRPType == TemplateSRPType.URP; }
        }
        public bool IsSRP
        {
            get
            {
                return (
                    m_currentSRPType == TemplateSRPType.URP
                    || m_currentSRPType == TemplateSRPType.HDRP
                );
            }
        }
        public TemplateInterpData InterpData
        {
            get { return m_interpolatorData; }
        }
        public List<PropertyDataCollector> LateDirectivesList
        {
            get { return m_lateDirectivesList; }
        }
        public List<PropertyDataCollector> SrpBatcherPropertiesList
        {
            get { return m_srpBatcherPropertiesList; }
        }
        public List<PropertyDataCollector> FullSrpBatcherPropertiesList
        {
            get { return m_fullSrpBatcherPropertiesList; }
        }
        public Dictionary<TemplateSemantics, TemplateVertexData> VertexDataDict
        {
            get { return m_vertexDataDict; }
        }
    }
}
