...
namespace AmplifyShaderEditor
{
    ...
    {
        [SerializeField]
        private List<TemplateShaderPropertyData> m_availableShaderProperties =
            new List<TemplateShaderPropertyData>();
        ...
        [SerializeField]
        private CustomTemplatePropertyUIEnum m_customTemplatePropertyUI =
            CustomTemplatePropertyUIEnum.None;
        ...
        private Dictionary<string, TemplateUniquePassData> m_passUniqueIdData =
            new Dictionary<string, TemplateUniquePassData>();
        ...
        void LoadTemplateBody(string guid, string path)
        {
            ...
            // Insert Before Tag
            MatchCollection col = Regex.Matches(
                shaderBody,
            ...
            for (int i = col.Count - 1; i >= 0; i--)
            {
                ...
                {
                    shaderBody = shaderBody.Insert(
                        col[i].Groups[2].Index,
                        TemplatesManager.TemplatePragmaBeforeTag + "\n" + col[i].Groups[1].Value
                    );
                }
            }
            //Detect SRP Batcher
            MatchCollection srpMatch = Regex.Matches(
                shaderBody,
                TemplateHelperFunctions.SRPBatcherFindTag
            );
            for (int i = srpMatch.Count - 1; i >= 0; i--)
            {
                ...
                {
                    shaderBody = shaderBody.Insert(
                        srpMatch[i].Groups[0].Index + srpMatch[i].Groups[0].Length,
                        TemplatesManager.TemplateSRPBatcherTag + srpMatch[i].Groups[1].Value
                    );
                }
            }
            ...
            // It will be read and processed over the TemplateSubShader constructor
            {
                ...
                {
                    MatchCollection subShaderMatch = Regex.Matches(
                        shaderBody,
                        TemplatesManager.TemplateMPSubShaderTag
                    );
                    ...
                    for (int i = subShaderAmount - 1; i > -1; i--)
                    {
                        ...
                        {
                            shaderBody = shaderBody.Insert(
                                subShaderMatch[i].Index + subShaderMatch[i].Length,
                                "\n\t\t\tLOD 0\n"
                            );
                        }
                    }
            ...
            try
            {
                ...
                m_defaultShaderName = m_defaultShaderName.Substring(
                    nameIdx[0],
                    nameIdx[1] - nameIdx[0]
                );
                m_shaderNameId = shaderBody.Substring(
                    nameBegin,
                    nameEnd + TemplatesManager.TemplateFullEndTag.Length - nameBegin
                );
                ...
                shaderBody = shaderBody.Substring(
                    nameEnd + TemplatesManager.TemplateFullEndTag.Length
                );
            }
            catch (Exception e)
            {
            ...
            {
                int index = m_dependenciesContainer.Id.IndexOf(
                    TemplatesManager.TemplateDependenciesListTag
                );
                m_templateProperties.AddId(
                    new TemplateProperty(
                        m_dependenciesContainer.Id,
                        m_dependenciesContainer.Id.Substring(0, index),
                        true
                    )
                );
                m_templateIdManager.RegisterId(
                    m_dependenciesContainer.Index,
                    m_dependenciesContainer.Id,
                    m_dependenciesContainer.Id
                );
            }

            TemplateHelperFunctions.FetchCustomInspector(
                m_customInspectorContainer,
                ref m_shaderBody
            );
            if (m_customInspectorContainer.IsValid)
            {
                ...
                m_templateProperties.AddId(
                    new TemplateProperty(
                        m_customInspectorContainer.Id,
                        m_customInspectorContainer.Id.Substring(0, index),
                        true
                    )
                );
                m_templateIdManager.RegisterId(
                    m_customInspectorContainer.Index,
                    m_customInspectorContainer.Id,
                    m_customInspectorContainer.Id
                );
            }
            ...
            {
                int index = m_fallbackContainer.Id.IndexOf(
                    "Fallback",
                    StringComparison.InvariantCultureIgnoreCase
                );
                m_templateProperties.AddId(
                    new TemplateProperty(
                        m_fallbackContainer.Id,
                        m_fallbackContainer.Id.Substring(0, index),
                        true
                    )
                );
                m_templateIdManager.RegisterId(
                    m_fallbackContainer.Index,
                    m_fallbackContainer.Id,
                    m_fallbackContainer.Id
                );
            }
            ...
            m_propertyTag = new TemplateTagData(
                m_shaderData.PropertyStartIdx,
            ...
            Dictionary<string, TemplateShaderPropertyData> duplicatesHelper =
                new Dictionary<string, TemplateShaderPropertyData>();
            TemplateHelperFunctions.CreateShaderPropertiesList(
                m_shaderData.Properties,
                ref m_availableShaderProperties,
                ref duplicatesHelper,
                -1,
                -1
            );
            ...
            {
                m_templateIdManager.RegisterId(
                    m_availableShaderProperties[i].Index,
                    m_availableShaderProperties[i].FullValue,
                    m_availableShaderProperties[i].FullValue
                );
            }
            ...
            //SHADER MODEL
            {
                Match shaderModelMatch = Regex.Match(
                    m_shaderData.Properties,
                    TemplateHelperFunctions.ShaderModelPattern
                );
                if (shaderModelMatch != null && shaderModelMatch.Success)
                {
                    if (
                        TemplateHelperFunctions.AvailableInterpolators.ContainsKey(
                            shaderModelMatch.Groups[1].Value
                        )
                    )
                    {
                        ...
                        m_globalShaderModel.InterpolatorAmount =
                            TemplateHelperFunctions.AvailableInterpolators[
                                shaderModelMatch.Groups[1].Value
                            ];
            ...
            {
                TemplateSubShader subShader = new TemplateSubShader(
                    this,
                    i,
                    m_templateIdManager,
                    "SubShader" + i,
                    m_shaderData.SubShaders[i],
                    ref duplicatesHelper
                );
            ...
            for (int subShaderIdx = 0; subShaderIdx < subShaderCount; subShaderIdx++)
            {
                m_subShaders[subShaderIdx]
                    .Modules.RegisterInternalUnityInlines(
                        ref m_availableShaderProperties,
                        ref duplicatesHelper
                    );
                ...
                for (int passIdx = 0; passIdx < passCount; passIdx++)
                {
                    ...
                    m_subShaders[subShaderIdx].Passes[passIdx].IsMainPass = (
                        mainSubShaderIdx == subShaderIdx && mainPassIdx == passIdx
                    );
                }
            }
        ...
        public void ResetState()
        {
            ...
            for (int subShaderIdx = 0; subShaderIdx < subshaderCount; subShaderIdx++)
            {
                ...
                for (int passIdx = 0; passIdx < passCount; passIdx++)
                {
                    m_subShaders[subShaderIdx]
                        .Passes[passIdx]
                        .TemplateProperties.ResetTemplateUsageData();
                }
            }
        }
        ...
        public void SetSubShaderData(
            TemplateModuleDataType type,
            int subShaderId,
            List<PropertyDataCollector> list
        )
        {
            string id = GetSubShaderDataId(type, subShaderId, false);
            string body = string.Empty;
        ...
        public void SetSubShaderData(TemplateModuleDataType type, int subShaderId, string text)
        {
            ...
            switch (type)
            {
                case TemplateModuleDataType.AllModules:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + TemplatesManager.TemplateAllModulesTag,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleShaderModel:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.ShaderModel.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendMode:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData.BlendModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendMode1:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData1.BlendModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendMode2:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData2.BlendModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendMode3:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData3.BlendModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendOp:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData.BlendOpId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendOp1:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData1.BlendOpId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendOp2:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData2.BlendOpId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleBlendOp3:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.BlendData3.BlendOpId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleAlphaToMask:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix
                                + m_subShaders[subShaderId].Modules.AlphaToMaskData.AlphaToMaskId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleCullMode:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.CullModeData.CullModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleColorMask:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.ColorMaskData.ColorMaskId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleColorMask1:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.ColorMaskData1.ColorMaskId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleColorMask2:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.ColorMaskData2.ColorMaskId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleColorMask3:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.ColorMaskData3.ColorMaskId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleStencil:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.StencilData.StencilBufferId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleZwrite:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.DepthData.ZWriteModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleZTest:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.DepthData.ZTestModeId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleZOffset:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.DepthData.OffsetId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleTag:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.TagData.TagsId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleGlobals:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.GlobalsTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleSRPBatcher:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.SRPBatcherTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleFunctions:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.FunctionsTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModulePragma:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.PragmaTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModulePragmaBefore:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.PragmaBeforeTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModulePass:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.PassTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleInputVert:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.InputsVertTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleInputFrag:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.InputsFragTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleRenderPlatforms:
                    {
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Modules.RenderPlatformHelper.ID,
                            text
                        );
                    }
                    break;
        ...
        public string GetSubShaderDataId(
            TemplateModuleDataType type,
            int subShaderId,
            bool addPrefix
        )
        {
            ...
            switch (type)
            {
                ...
                {
                    prefix = addPrefix
                        ? m_subShaders[subShaderId].Modules.UniquePrefix
                        : string.Empty;
        ...
        {
            if (
                subShaderId >= m_subShaders.Count
            ...
            switch (type)
            {
                ...
                {
                    prefix = addPrefix
                        ? m_subShaders[subShaderId].Passes[passId].Modules.UniquePrefix
                        : string.Empty;
                    return prefix + TemplatesManager.TemplateAllModulesTag;
                }
                case TemplateModuleDataType.ModuleBlendMode:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData.BlendModeId;
                }
                case TemplateModuleDataType.ModuleBlendMode1:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData1.BlendModeId;
                }
                case TemplateModuleDataType.ModuleBlendMode2:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData2.BlendModeId;
                }
                case TemplateModuleDataType.ModuleBlendMode3:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData3.BlendModeId;
                }
                case TemplateModuleDataType.ModuleBlendOp:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData.BlendOpId;
                }
                case TemplateModuleDataType.ModuleBlendOp1:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData1.BlendOpId;
                }
                case TemplateModuleDataType.ModuleBlendOp2:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData2.BlendOpId;
                }
                case TemplateModuleDataType.ModuleBlendOp3:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.BlendData3.BlendOpId;
                }
                case TemplateModuleDataType.ModuleAlphaToMask:
                {
                    prefix = addPrefix
                ...
                case TemplateModuleDataType.ModuleCullMode:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.CullModeData.CullModeId;
                }
                case TemplateModuleDataType.ModuleColorMask:
                {
                    prefix = addPrefix
                ...
                case TemplateModuleDataType.ModuleZwrite:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.DepthData.ZWriteModeId;
                }
                case TemplateModuleDataType.ModuleZTest:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.DepthData.ZTestModeId;
                }
                case TemplateModuleDataType.ModuleZOffset:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.DepthData.OffsetId;
                }
                case TemplateModuleDataType.ModuleTag:
                {
                    prefix = addPrefix
                ...
                case TemplateModuleDataType.ModuleSRPBatcher:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.SRPBatcherTag.Id;
                }
                case TemplateModuleDataType.ModuleFunctions:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.FunctionsTag.Id;
                }
                case TemplateModuleDataType.ModulePragma:
                {
                    prefix = addPrefix
                ...
                case TemplateModuleDataType.ModulePragmaBefore:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.PragmaBeforeTag.Id;
                }
                case TemplateModuleDataType.ModulePass:
                {
                    prefix = addPrefix
                ...
                case TemplateModuleDataType.ModuleInputVert:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.InputsVertTag.Id;
                }
                case TemplateModuleDataType.ModuleInputFrag:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].Modules.InputsFragTag.Id;
                }
                case TemplateModuleDataType.PassVertexFunction:
                {
                    prefix = addPrefix
                        ? m_subShaders[subShaderId].Passes[passId].UniquePrefix
                        : string.Empty;
                ...
                case TemplateModuleDataType.PassFragmentFunction:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].FragmentFunctionData.Id;
                }
                case TemplateModuleDataType.PassVertexData:
                {
                    prefix = addPrefix
                        ...
                        : string.Empty;
                    return prefix
                        + m_subShaders[subShaderId].Passes[passId].VertexDataContainer.VertexDataId;
                }
                ...
                case TemplateModuleDataType.ModuleShaderModel:
                    {
                        ...
                        m_templateIdManager.SetReplacementText(
                            prefix
                                + m_subShaders[subShaderId].Passes[passId].Modules.ShaderModel.Id,
                            text
                        );
                    }
                    break;
                ...
                case TemplateModuleDataType.ModuleTag:
                    {
                        ...
                        m_templateIdManager.SetReplacementText(
                            prefix
                                + m_subShaders[subShaderId].Passes[passId].Modules.TagData.TagsId,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ModuleGlobals:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].Modules.UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].Modules.GlobalsTag.Id,
                            text
                        );
                    }
                    break;
                ...
                case TemplateModuleDataType.ModulePragma:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].Modules.UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].Modules.PragmaTag.Id,
                            text
                        );
                    }
                    break;
                ...
                case TemplateModuleDataType.ModulePass:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].Modules.UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].Modules.PassTag.Id,
                            text
                        );
                    }
                    break;
                ...
                case TemplateModuleDataType.PassVertexFunction:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].VertexFunctionData.Id,
                            text
                        );
                    }
                    break;
                ...
                case TemplateModuleDataType.PassNameData:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].PassNameContainer.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.VControl:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].TessVControlTag.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.ControlData:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].TessControlData.Id,
                            text
                        );
                    }
                    break;
                case TemplateModuleDataType.DomainData:
                    {
                        prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
                        m_templateIdManager.SetReplacementText(
                            prefix + m_subShaders[subShaderId].Passes[passId].TessDomainData.Id,
                            text
                        );
                    }
                    break;
        ...
        public void SetPassInputData(int subShaderId, int passId, int inputId, string text)
        {
            ...
            TemplateInputData inputData = m_subShaders[subShaderId]
                .Passes[passId]
                .InputDataFromId(inputId);
            if (inputData != null)
            {
            ...
            else
            {
                Debug.LogErrorFormat(
                    "Unable to find input data for port with id {0} on subshader {1} pass {2}",
                    inputId,
                    subShaderId,
                    passId
                );
        ...
        public void SetPassInputDataByArrayIdx(
            int subShaderId,
            int passId,
            int inputId,
            string text
        )
        {
            ...
            string prefix = m_subShaders[subShaderId].Passes[passId].UniquePrefix;
            m_templateIdManager.SetReplacementText(
                prefix + m_subShaders[subShaderId].Passes[passId].InputDataList[inputId].TagId,
                text
            );
        }
        ...
        )
        {
            ...
            templateData.VertexDataContainer = SubShaders[subShaderId]
                .Passes[passId]
                .VertexDataContainer;
            templateData.InterpolatorDataContainer = SubShaders[subShaderId]
                .Passes[passId]
                .InterpolatorDataContainer;
            templateData.AvailableShaderProperties = m_availableShaderProperties;
            templateData.VertexFunctionData = SubShaders[subShaderId]
                .Passes[passId]
                .VertexFunctionData;
            templateData.FragmentFunctionData = SubShaders[subShaderId]
                .Passes[passId]
            ...
            templateData.ColorMaskData = SubShaders[subShaderId]
                .Passes[passId]
                .Modules
                .ColorMaskData;
        ...
        public bool FillTemplateBody(
            int subShaderId,
            int passId,
            string id,
            ref string body,
            List<PropertyDataCollector> values
        )
        {
            if (values.Count == 0)
            {
        ...
        )
        {
            ...
            {
                string finalValue = propertyContainer.PropertyDict[id].UseIndentationAtStart
                    ? propertyContainer.PropertyDict[id].Indentation
                    : string.Empty;
                for (int i = 0; i < values.Length; i++)
                {
                    ...
                    else
                    {
                        //first value will be automatically indented by the string replace
                        finalValue +=
                            (i == 0 ? string.Empty : propertyContainer.PropertyDict[id].Indentation)
                            + values[i];
                    }
                }
            ...
            if (values.Length > 1 || !string.IsNullOrEmpty(values[0]))
            {
                UIUtils.ShowMessage(
                    string.Format(
                        "Attempting to write data into inexistant tag {0}. Please review the template {1} body and consider adding the missing tag.",
                        id,
                        m_defaultShaderName
                    ),
        ...
        {
            TemplateFunctionData vertexFunctionData = SubShaders[subShaderId]
                .Passes[passId]
                .VertexFunctionData;
            if (vertexFunctionData != null && !string.IsNullOrEmpty(vertexFunctionData.Id))
            {
                string body = string.Empty;
                bool isValid = FillTemplateBody(
                    subShaderId,
                    passId,
                    vertexFunctionData.Id,
                    ref body,
                    values
                );
            ...
            {
                UIUtils.ShowMessage(
                    "Attemping to add vertex instructions on a template with no assigned vertex code area",
                    MessageSeverity.Error
                );
                return false;
            }
        ...
        {
            TemplateFunctionData fragmentFunctionData = SubShaders[subShaderId]
                .Passes[passId]
                .FragmentFunctionData;
            ...
            {
                UIUtils.ShowMessage(
                    "Attemping to add fragment instructions on a template with no assigned vertex code area",
                    MessageSeverity.Error
                );
                return false;
            }
        ...
        public void SetCustomInspector(string customInspector)
        {
            if (m_customInspectorContainer.Index > -1)
            {
                m_templateIdManager.SetReplacementText(
                    m_customInspectorContainer.Id,
                    m_templateProperties.PropertyDict[m_customInspectorContainer.Id].Indentation
                        + customInspector
                );
            }
        }
        ...
        public void SetFallback(string fallback)
        {
            if (m_fallbackContainer.Index > -1)
            {
                m_templateIdManager.SetReplacementText(
                    m_fallbackContainer.Id,
                    m_templateProperties.PropertyDict[m_fallbackContainer.Id].Indentation + fallback
                );
            }
        }
        ...
        public bool GetSubShaderandPassFor(string passUniqueId, ref int subShaderId, ref int passId)
        {
            ...
            if (m_passUniqueIdData.Count == 0)
            {
                for (int subShaderIdx = 0; subShaderIdx < m_subShaders.Count; subShaderIdx++)
                {
                    for (
                        int passIdx = 0;
                        passIdx < m_subShaders[subShaderIdx].Passes.Count;
                        passIdx++
                    )
                    {
                        ...
                        {
                            if (
                                m_passUniqueIdData.ContainsKey(
                                    m_subShaders[subShaderIdx]
                                        .Passes[passIdx]
                                        .Modules
                                        .PassUniqueName
                            ...
                            {
                                Debug.LogErrorFormat(
                                    "Found duplicate pass name '{0}' over template. Please fix template as it will result in multiple errors.",
                                    m_subShaders[subShaderIdx]
                                        .Passes[passIdx]
                                        .Modules
                                        .PassUniqueName
                            ...
                            m_passUniqueIdData.Add(
                                m_subShaders[subShaderIdx].Passes[passIdx].Modules.PassUniqueName,
                                new TemplateUniquePassData()
                                {
                                    PassIdx = passIdx,
        ...
        public TemplateSRPType SRPtype
        {
            get { return m_subShaders[0].Modules.SRPType; }
        }

        //public bool SRPIsPBRHD { get { return m_subShaders[0].Modules.SRPIsPBRHD ; } }
        public List<TemplateSubShader> SubShaders
        {
            get { return m_subShaders; }
        }
        public List<TemplateShaderPropertyData> AvailableShaderProperties
        {
            get { return m_availableShaderProperties; }
        }
            ...
            get
            {
                if (m_allShaderProperties == null)
                {
                    ...
                    for (int subShaderIdx = 0; subShaderIdx < SubShaders.Count; subShaderIdx++)
                    {
                        ...
                        {
                            m_allShaderProperties.AddRange(
                                SubShaders[subShaderIdx].AvailableShaderGlobals
                            );
                        }

                        for (
                            int passIdx = 0;
                            passIdx < SubShaders[subShaderIdx].Passes.Count;
                            passIdx++
                        )
                        {
                            if (
                            ...
                            {
                                m_allShaderProperties.AddRange(
                                    SubShaders[subShaderIdx].Passes[passIdx].AvailableShaderGlobals
                                );
                            }
                        }
                    }
        ...
        public TemplateTagData PropertyTag
        {
            get { return m_propertyTag; }
        }
        public TemplateIdManager IdManager
        {
            get { return m_templateIdManager; }
        }
        public TemplatePropertyContainer TemplateProperties
        {
            get { return m_templateProperties; }
        }
        public TemplateInfoContainer CustomInspectorContainer
        {
            get { return m_customInspectorContainer; }
        }
        public TemplateInfoContainer FallbackContainer
        {
            get { return m_fallbackContainer; }
        }
        public TemplateInfoContainer BeforePragmaContainer
        {
            get { return m_beforePragmaContainer; }
        }
        public bool IsSinglePass
        {
            get { return m_isSinglePass; }
        }
        public int MasterNodesRequired
        {
            get { return m_masterNodesRequired; }
        }
        public CustomTemplatePropertyUIEnum CustomTemplatePropertyUI
        {
            get { return m_customTemplatePropertyUI; }
        }
        public bool CanAddLODs
        {
            get { return m_lodInjectorId > -1; }
        }
        public TemplateShaderModelData GlobalShaderModel
        {
            get { return m_globalShaderModel; }
        }
    }
}
