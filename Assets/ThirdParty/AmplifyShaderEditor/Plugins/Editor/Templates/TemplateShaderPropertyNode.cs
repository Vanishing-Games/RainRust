...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplateShaderPropertyNode : TemplateNodeParent
    {
        ...
        void FetchScope()
        {
            ...
            for (int i = 0; i < shaderScopeCount; i++)
            {
                if (
                    m_templateMPData
                        ...
                        .PropertyName.Equals(m_propertyName)
                )
                {
            ...
            int subShaderScopeCount = m_templateMPData
            ...
            int passScopeCount = m_templateMPData
                .SubShaders[SubShaderIdx]
                .Passes[PassIdx]
                .AvailableShaderGlobals
                .Count;
                ...
                if (m_advancedView)
                {
                    switch (m_currentScope)
                    {
                        ...
                        case ShaderPropertyScope.SubShader:
                            m_shaderProperties = m_templateMPData
                                .SubShaders[SubShaderIdx]
                                .AvailableShaderGlobals;
                            break;
        ...
        public override void DrawProperties()
        {
            ...
            if (m_currentPropertyIdx > -1)
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    m_currentPropertyIdx = EditorGUILayoutPopup(
                        PropertyLabelStr,
        ...
        {
            EditorGUI.BeginChangeCheck();
            m_currentScope = (ShaderPropertyScope)EditorGUILayoutEnumPopup(
                CurrentScopeStr,
                m_currentScope
            );
            if (EditorGUI.EndChangeCheck())
        ...
        protected override void CheckWarningState()
        {
            if (m_containerGraph.CurrentCanvasMode != NodeAvailability.TemplateShader)
            {
        ...
        public override void SetPreviewInputs()
        {
            ...
            if (
                m_shaderProperties == null
                || m_currentPropertyIdx >= m_shaderProperties.Count
                || m_currentPropertyIdx < 0
            )
                return;
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                m_currentPropertyIdx = m_upperLeftWidgetHelper.DrawWidget(
                    this,
                    m_currentPropertyIdx,
                    m_propertyLabels
                );
        ...
        void UpdateFromId()
        {
            if (m_shaderProperties != null)
            {
                ...
                bool areCompatible = TemplateHelperFunctions.CheckIfCompatibles(
                    m_outputPorts[0].DataType,
                    m_shaderProperties[m_currentPropertyIdx].PropertyDataType
                );
                switch (m_shaderProperties[m_currentPropertyIdx].PropertyDataType)
                {
                ...
                m_typeName =
                    TypeLabelStr + m_shaderProperties[m_currentPropertyIdx].PropertyType.ToString();
                if (m_shaderProperties[m_currentPropertyIdx].PropertyType != PropertyType.Global)
                {
                    m_propertyNameLabel =
                        PropertyNameStr + m_shaderProperties[m_currentPropertyIdx].PropertyName;
                }
        ...
        string GenerateTitle(params float[] values)
        {
            ...
            if (values.Length == 1)
            {
                finalResult += values[0]
                    .ToString(
                        Mathf.Abs(values[0]) > 1000
                            ? Constants.PropertyBigFloatFormatLabel
                            : Constants.PropertyFloatFormatLabel
                    );
            }
            else
            {
        ...
        )
        {
            if (dataCollector.MasterNodeCategory != AvailableShaderTypes.Template)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Template Parameter node is only intended for templates use only",
            ...
            if (m_shaderProperties == null || m_shaderProperties.Count == 0)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Attempting to fetch inexistant parameter on " + m_nodeAttribs.Name + " node",
                    MessageSeverity.Error
                );
                return m_outputPorts[outputId].ErrorValue;
            }
            ...
            if (m_multiPassMode)
            {
                switch (m_currentScope)
                {
                    case ShaderPropertyScope.SubShader:
                        {
                            if (
                                dataCollector.TemplateDataCollectorInstance.MultipassSubshaderIdx
                                != SubShaderIdx
                            )
                            {
                                UIUtils.ShowMessage(
                                    UniqueId,
                                    string.Format(
                                        "{0} is only intended for subshader {1}",
                                        m_propertyLabels[m_currentPropertyIdx],
                                        SubShaderIdx
                                    )
                                );
                                return m_outputPorts[outputId].ErrorValue;
                    ...
                    case ShaderPropertyScope.Pass:
                        {
                            if (
                                dataCollector.TemplateDataCollectorInstance.MultipassSubshaderIdx
                                    != SubShaderIdx
                                || dataCollector.TemplateDataCollectorInstance.MultipassPassIdx
                                    != PassIdx
                            )
                            {
                                UIUtils.ShowMessage(
                                    UniqueId,
                                    string.Format(
                                        "{0} is only intended for subshader {1} and pass {2}",
                                        m_propertyLabels[m_currentPropertyIdx],
                                        SubShaderIdx,
                                        PassIdx
                                    )
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_currentScope = (ShaderPropertyScope)
                    Enum.Parse(typeof(ShaderPropertyScope), GetCurrentParam(ref nodeParams));
            }
            else
            {
        ...
        bool SetTemplate(MasterNode newMasterNode)
        {
            if (m_containerGraph.MultiPassMasterNodes.NodesList.Count > 0)
            {
                ...
                TemplateMultiPassMasterNode templateMasterNode = (
                    newMasterNode as TemplateMultiPassMasterNode
                );
            ...
            else
            {
                ...
                if (templateMasterNode != null)
                {
                    m_shaderProperties = templateMasterNode
                        .CurrentTemplate
                        .AvailableShaderProperties;
                    return true;
                }
            }
            return false;
