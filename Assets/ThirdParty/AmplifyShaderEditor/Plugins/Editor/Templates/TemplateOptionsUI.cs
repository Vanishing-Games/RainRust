...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateOptionUIItem
    {
        ...
        public TemplateOptionUIItem(TemplateOptionsItem options)
        {
            ...
            m_invertActionOnDeselection =
                options.Setup == AseOptionItemSetup.InvertActionOnDeselection;
        }
            ...
            if (m_isVisible)
            {
                ...
                switch (m_options.UIWidget)
                {
                    case AseOptionsUIWidget.Dropdown:
                        {
                            m_currentOption = owner.EditorGUILayoutPopup(
                                m_options.Name,
                                m_currentOption,
                                m_options.DisplayOptions
                            );
                        }
                        break;
                    case AseOptionsUIWidget.Toggle:
                        {
                            m_currentOption = owner.EditorGUILayoutToggle(
                                m_options.Name,
                                m_currentOption == 1
                            )
                                ? 1
                    ...
                    case AseOptionsUIWidget.Float:
                        {
                            ...
                            {
                                m_options.FieldValue.FloatValue = owner.EditorGUILayoutFloatField(
                                    m_options.Name,
                                    m_options.FieldValue.FloatValue
                                );
                            }
                        }
                        break;
                    case AseOptionsUIWidget.Int:
                        {
                            if (m_options.FieldInline)
                            {
                                ...
                                m_options.FieldValue.FloatValue = owner.EditorGUILayoutIntField(
                                    m_options.Name,
                                    (int)m_options.FieldValue.FloatValue
                                );
                        }
                        break;
                    case AseOptionsUIWidget.FloatRange:
                        {
                            if (m_options.FieldInline)
                            {
                                m_options.FieldValue.SliderField(
                                    ref owner,
                                    m_options.Name,
                                    m_options.FieldMin,
                                    m_options.FieldMax
                                ...
                                m_options.FieldValue.FloatValue = owner.EditorGUILayoutSlider(
                                    m_options.Name,
                                    m_options.FieldValue.FloatValue,
                                    m_options.FieldMin,
                                    m_options.FieldMax
                    ...
                    case AseOptionsUIWidget.IntRange:
                        {
                            if (m_options.FieldInline)
                            {
                                m_options.FieldValue.IntSlider(
                                    ref owner,
                                    m_options.Name,
                                    (int)m_options.FieldMin,
                                    (int)m_options.FieldMax
                                );
                                if (m_options.FieldValue.Active)
                                    m_currentOption = 1;
                                else
                                ...
                                m_options.FieldValue.FloatValue = owner.EditorGUILayoutIntSlider(
                                    m_options.Name,
                                    (int)m_options.FieldValue.FloatValue,
                                    (int)m_options.FieldMin,
                                    (int)m_options.FieldMax
                    ...
                    if (OnActionPerformedEvt != null)
                    {
                        if (m_invertActionOnDeselection)
                            OnActionPerformedEvt(
                                true,
                                false,
                                lastOption != m_options.DisableIdx,
                                this,
                                0,
                                m_options.ActionsPerOption[lastOption]
                            );
                        ...
                        OnActionPerformedEvt(
                            ...
                            this,
                            0,
                            m_options.ActionsPerOption[m_currentOption]
                        );
                    }
                }
            }
            ...
            else if (m_wasVisible)
            {
                ...
                if (OnActionPerformedEvt != null)
                {
                    OnActionPerformedEvt(
                        ...
                        this,
                        0,
                        m_options.ActionsPerOption[m_options.DisableIdx]
                    );
                }
            }
        }
                ...
                for (int i = 0; i < m_options.ActionsPerOption[m_currentOption].Length; i++)
                {
                    switch (m_options.ActionsPerOption[m_currentOption][i].ActionType)
                    {
                        case AseOptionsActionType.SetDefine:
                            {
                                dataCollector.AddToDefines(
                                    -1,
                                    m_options.ActionsPerOption[m_currentOption][i].ActionData
                                );
                            }
                            break;
        ...
        public TemplateOptionsItem Options
        {
        ...
        {
            set { m_currentOption = Mathf.Clamp(value, 0, m_options.Options.Length - 1); }
        }
        public bool EmptyEvent
        {
            get { return OnActionPerformedEvt == null; }
        }
        public TemplateActionItemGrid.TemplateActionItemRow CurrentOptionActions
        {
            get
            {
                if (m_options.Type == AseOptionsType.Field && m_currentOption == 1)
                {
                    if (m_options.FieldValue.NodeId < -1)
                    {
                        ...
                        PropertyNode node =
                            UIUtils.CurrentWindow.OutsideGraph.GetInternalTemplateNode(
                                m_options.FieldValue.NodeId
                            ) as PropertyNode;
                        if (node != null)
                        {
        ...
        public bool InvertActionOnDeselection
        {
            get { return m_invertActionOnDeselection; }
        }
        public Int64 LastClickedTimestamp
        {
            get { return m_lastClickedTimestamp; }
            set { m_lastClickedTimestamp = value; }
        }
    }
}
