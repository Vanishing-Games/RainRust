...
namespace AmplifyShaderEditor
{
    ...
    public class TemplateIdManager
    {
        ...
        private Dictionary<string, TemplateId> m_registeredIdsDict =
            new Dictionary<string, TemplateId>();
        ...
        {
            m_registeredPassIds.Add(
                new TemplatePassId() { PassId = passId, RemoveFromShader = false }
            );
        }
        ...
        public void SetReplacementText(string uniqueId, string replacementText)
        {
            ...
            if (
                m_registeredIdsDict.ContainsKey(uniqueId)
                && m_registeredIdsDict[uniqueId].StartIdx >= 0
            )
        ...
        public string BuildShader()
        {
            if (!m_isSorted)
            {
                m_registeredIds.Sort(
                    (x, y) =>
                    {
                        return x.StartIdx.CompareTo(y.StartIdx);
                    }
                );
            }
            ...
            for (int i = 0; i < idCount; i++)
            {
                ...
                {
                    finalShaderBody = finalShaderBody.ReplaceAt(
                        m_registeredIds[i].Tag,
                        m_registeredIds[i].ReplacementText,
                        offset + m_registeredIds[i].StartIdx
                    );
                    offset += (
                        m_registeredIds[i].ReplacementText.Length - m_registeredIds[i].Tag.Length
                    );
                }
            }
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_registeredPassIds[i].RemoveFromShader)
                    finalShaderBody = finalShaderBody.Replace(
                        m_registeredPassIds[i].PassId,
                        string.Empty
                    );
            }
            ...
            {
                if (
                    !m_registeredIds[i].IsReplaced
                    && !m_registeredIds[i].Tag.Equals(m_registeredIds[i].ReplacementText)
                )
                {
                    finalShaderBody = finalShaderBody.Replace(
                        m_registeredIds[i].Tag,
        ...
        public List<TemplateTag> RegisteredTags
        {
            get { return m_registeredTags; }
            set { m_registeredTags = value; }
        }
    }
}
