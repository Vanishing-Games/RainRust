...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TemplateShaderModelModule : TemplateModuleParent
    {
        ...
        private const string ShaderModelEncapsulateFormatStr =
            "CGINCLUDE\n#pragma target {0}\nENDCG";
        ...
        public TemplateShaderModelModule()
            : base("Shader Model") { }

        public override void Draw(UndoParentNode owner, bool style = true)
        {
            EditorGUI.BeginChangeCheck();
            m_shaderModelIdx = owner.EditorGUILayoutPopup(
                ShaderModelStr,
                m_shaderModelIdx,
                TemplateHelperFunctions.AvailableShaderModels
            );
        ...
        )
        {
            ...
            if (validDataOnMeta)
            {
                ...
                int templateModule = TemplateHelperFunctions.ShaderModelToArrayIdx[
                    modulesData.ShaderModel.Value
                ];
                if (templateModule > m_shaderModelIdx)
                {
                    if (Preferences.User.ForceTemplateMinShaderModel)
                    {
                        ...
                        UIUtils.ShowMessage(
                            "Changing shader model to minimum set by template: "
                                + modulesData.ShaderModel.Value
                                + "\n"
                                + "To disable this behavior please toggle off\n"
                                + "Preferences > Amplify Shader Editor > Force Template Min. Shader Model",
        ...
        public override string GenerateShaderData(bool isSubShader)
        {
            if (m_encapsulateOnCGInlude)
            {
                return string.Format(
                    ShaderModelEncapsulateFormatStr,
                    TemplateHelperFunctions.AvailableShaderModels[m_shaderModelIdx]
                );
            }
            else
            {
                return ShaderModelFormatStr
                    + TemplateHelperFunctions.AvailableShaderModels[m_shaderModelIdx];
            }
        }
        ...
        public int CurrentShaderModelIdx
        {
            get { return m_shaderModelIdx; }
        }
        public string CurrentShaderModel
        {
            get { return TemplateHelperFunctions.AvailableShaderModels[m_shaderModelIdx]; }
        }
        public bool EncapsulateOnCGInlude
        {
            get { return m_encapsulateOnCGInlude; }
        }
        public int InterpolatorAmount
        {
            get
            {
                return TemplateHelperFunctions.AvailableInterpolators[
                    TemplateHelperFunctions.AvailableShaderModels[m_shaderModelIdx]
                ];
            }
        }
