        ...
        )
        {
            ...
            for (int i = 0; i < importedAssets.Length; i++)
            {
                if (TemplateHelperFunctions.CheckIfTemplate(importedAssets[i]))
                {
                    ...
                    {
                        refreshMenuItems =
                            templateData.Reload() || refreshMenuItems || firstTimeDummyFlag;
                        ...
                        {
                            if (
                                IOUtils.AllOpenedWindows[windowIdx].OutsideGraph.CurrentCanvasMode
                                == NodeAvailability.TemplateShader
                            )
                            {
                                if (
                                    IOUtils
                                        .AllOpenedWindows[windowIdx]
                                        .OutsideGraph
                                ...
                                )
                                {
                                    ...
                                    IOUtils
                                        .AllOpenedWindows[windowIdx]
                                        .OutsideGraph.ForceMultiPassMasterNodesRefresh();
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        string name = TemplatesManager.OfficialTemplates.ContainsKey(guid)
                            ? TemplatesManager.OfficialTemplates[guid]
                            : string.Empty;
                        TemplateMultiPass mp =
                            TemplateMultiPass.CreateInstance<TemplateMultiPass>();
                ...
                if (deletedAssets[0].IndexOf(Constants.InvalidPostProcessDatapath) < 0)
                {
                    for (int i = 0; i < deletedAssets.Length; i++)
                    {
                        ...
                        if (templateData != null)
                        {
                            ...
                            for (int windowIdx = 0; windowIdx < windowCount; windowIdx++)
                            {
                                TemplateMasterNode masterNode =
                                    IOUtils
                                        .AllOpenedWindows[windowIdx]
                                        .CurrentGraph
                                        .CurrentMasterNode as TemplateMasterNode;
                                if (
                                    masterNode != null
                                    && masterNode.CurrentTemplate.GUID.Equals(templateData.GUID)
                                )
                                {
