...
namespace AmplifyShaderEditor
{
    ...
    public class ParentGraph : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        private UsageListTexturePropertyNodes m_texturePropertyNodes =
            new UsageListTexturePropertyNodes();
        ...
        private UsageListRegisterLocalVarNodes m_localVarNodes =
            new UsageListRegisterLocalVarNodes();
        ...
        private Dictionary<int, PropertyNode> m_internalTemplateNodesDict =
            new Dictionary<int, PropertyNode>();
        ...
        public void Init()
        {
            ...
            m_lodMultiPassMasterNodes = new List<UsageListTemplateMultiPassMasterNodes>(
                MaxLodAmount
            );
        ...
        public void HighlightWiresStartingNode(ParentNode node)
        {
            for (int outputIdx = 0; outputIdx < node.OutputPorts.Count; outputIdx++)
            {
                for (
                    int extIdx = 0;
                    extIdx < node.OutputPorts[outputIdx].ExternalReferences.Count;
                    extIdx++
                )
                {
                    ...
                    if (nextNode && nextNode.ConnStatus == NodeConnectionStatus.Connected)
                    {
                        ...
                        if (
                            port.ExternalReferences.Count == 0
        ...
        void PropagateHighlightDeselection(ParentNode node, int portId = -1)
        {
            ...
            {
                if (
                    node.InputPorts[i].ExternalReferences.Count > 0
                    && node.InputPorts[i].ExternalReferences[0].WireStatus == WireStatus.Highlighted
                )
                {
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_nodes[i] != null)
                {
                    UndoUtils.RegisterCompleteObjectUndo(
                        m_nodes[i],
                        Constants.UndoRegisterFullGrapId
                    );
                }
            }
        }
        ...
        public void OnNodeFinishMoving(
            ParentNode node,
        ...
        public void OnNodeReOrderEvent(ParentNode node, int index)
        {
            ...
            {
                Debug.LogWarning(
                    "Reorder canceled: This is a specific method for when reordering needs to be done and a its original index is higher than the new one"
                );
            }
            else
            {
            ...
            if (m_internalTemplateNodesDict.Count != m_internalTemplateNodesList.Count)
            {
                ...
                for (int i = 0; i < m_internalTemplateNodesList.Count; i++)
                {
                    if (m_internalTemplateNodesList[i] != null)
                        m_internalTemplateNodesDict.Add(
                            m_internalTemplateNodesList[i].UniqueId,
                            m_internalTemplateNodesList[i]
                        );
                }
            }
        ...
        public void AddInternalTemplateNode(TemplateShaderPropertyData data)
        {
            ...
            switch (data.PropertyDataType)
            {
                case WirePortDataType.FLOAT:
                    propertyNode = CreateInstance<RangedFloatNode>();
                    break;
                ...
                case WirePortDataType.INT:
                    propertyNode = CreateInstance<IntNode>();
                    break;
            ...
            // create array
            if (
                data.PropertyDataType == WirePortDataType.FLOAT
        ...
        public bool Draw(DrawInfo drawInfo)
        {
            ...
            if (drawInfo.CurrentEventType == EventType.Repaint)
            {
                ...
                if (
                    newLevel != m_lodLevel
                    || (
                        UIUtils.MainSkin != null
                        && UIUtils.MainSkin.textField.border.left != referenceValue
                    )
                )
                {
                    m_lodLevel = newLevel;
            ...
            if (m_afterDeserializeFlag || m_lateOptionsRefresh)
            {
                ...
                if (CurrentCanvasMode == NodeAvailability.TemplateShader)
                {
                    ...
                    {
                        m_parentWindow.ClipboardInstance.AddMultiPassNodesToClipboard(
                            MultiPassMasterNodes.NodesList,
                            true,
                            -1
                        );
                        for (int i = 0; i < m_lodMultiPassMasterNodes.Count; i++)
                        {
                            if (m_lodMultiPassMasterNodes[i].Count > 0)
                                m_parentWindow.ClipboardInstance.AddMultiPassNodesToClipboard(
                                    m_lodMultiPassMasterNodes[i].NodesList,
                                    false,
                                    i
                                );
                        }
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                m_hasUnConnectedNodes =
                    m_hasUnConnectedNodes
                    || (
                        node.ConnStatus != NodeConnectionStatus.Connected
                        && node.ConnStatus != NodeConnectionStatus.Island
                    );
            ...
            if (drawInfo.CurrentEventType == EventType.Repaint)
                DrawWires(
                    ParentWindow.WireTexture,
                    drawInfo,
                    ParentWindow.WindowContextPallete.IsActive,
                    ParentWindow.WindowContextPallete.CurrentPosition
                );
        ...
        public void DrawWires(
            Texture2D wireTex,
            DrawInfo drawInfo,
            bool contextPaletteActive,
            Vector3 contextPalettePos
        )
        {
            //Handles.BeginGUI();
            //Debug.Log(GUI.depth);
            ...
            for (int nodeIdx = 0; nodeIdx < m_nodes.Count; nodeIdx++)
            {
                ...
                for (int inputPortIdx = 0; inputPortIdx < node.InputPorts.Count; inputPortIdx++)
                {
                    ...
                    if (inputPort.ExternalReferences.Count > 0 && inputPort.Visible)
                    {
                        ...
                        for (
                            int wireIdx = 0;
                            wireIdx < inputPort.ExternalReferences.Count;
                            wireIdx++
                        )
                        {
                            ...
                            if (reference.NodeId != -1 && reference.PortId != -1)
                            {
                                ...
                                if (outputNode != null)
                                {
                                    OutputPort outputPort = outputNode.GetOutputPortByUniqueId(
                                        reference.PortId
                                    );
                                    ...
                                    if (isVisible)
                                    {
                                        Rect bezierBB = DrawBezier(
                                            drawInfo.InvertedZoom,
                                            startPos,
                                            endPos,
                                            inputPort.DataType,
                                            outputPort.DataType,
                                            node.GetInputPortVisualDataTypeByArrayIdx(inputPortIdx),
                                            outputNode.GetOutputPortVisualDataTypeById(
                                                reference.PortId
                                            ),
                                            reference.WireStatus,
                                            wireTex,
                                            node,
                                            outputNode
                                        );
                                        ...
                                        {
                                            m_bezierReferences[m_wireBezierCount]
                                                .UpdateInfo(
                                                    ref bezierBB,
                                                    inputPort.NodeId,
                                                    inputPort.PortId,
                                        ...
                                        {
                                            m_bezierReferences.Add(
                                                new WireBezierReference(
                                                    ref bezierBB,
                                                    inputPort.NodeId,
                                                    inputPort.PortId,
                                                    outputPort.NodeId,
                                ...
                                else
                                {
                                    if (DebugConsoleWindow.DeveloperMode)
                                        UIUtils.ShowMessage(
                                            "Detected Invalid connection from node "
                                                + node.UniqueId
                                                + " port "
                                                + inputPortIdx
                                                + " to Node "
            ...
            if (m_parentWindow.WireReferenceUtils.ValidReferences())
            {
                ...
                {
                    InputPort inputPort = GetNode(
                            m_parentWindow.WireReferenceUtils.InputPortReference.NodeId
                        )
                        .GetInputPortByUniqueId(
                            m_parentWindow.WireReferenceUtils.InputPortReference.PortId
                        );
                    Vector3 endPos = Vector3.zero;
                    if (m_parentWindow.WireReferenceUtils.SnapEnabled)
                    {
                        Vector2 pos =
                            (m_parentWindow.WireReferenceUtils.SnapPosition + drawInfo.CameraOffset)
                            * drawInfo.InvertedZoom;
                        endPos = new Vector3(pos.x, pos.y) + UIUtils.ScaledPortsDelta;
                    }
                    else
                    {
                        endPos = contextPaletteActive
                            ? contextPalettePos
                            : new Vector3(
                ...
                {
                    OutputPort outputPort = GetNode(
                            m_parentWindow.WireReferenceUtils.OutputPortReference.NodeId
                        )
                        .GetOutputPortByUniqueId(
                            m_parentWindow.WireReferenceUtils.OutputPortReference.PortId
                        );
                    Vector3 endPos = Vector3.zero;
                    if (m_parentWindow.WireReferenceUtils.SnapEnabled)
                    {
        ...
        Rect DrawBezier(
            ...
            Vector3 startPos,
            Vector3 endPos,
            WirePortDataType inputDataType,
            WirePortDataType outputDataType,
            WirePortDataType inputVisualDataType,
            WirePortDataType outputVisualDataType,
            WireStatus wireStatus,
            Texture2D wireTex,
            ParentNode inputNode = null,
        ...
        )
        {
            ...
            float resizedMag = Mathf.Min(
                mag * 0.66f,
                Constants.HORIZONTAL_TANGENT_SIZE * invertedZoom
            );
            ...
            if (ParentWindow.Options.MultiLinePorts)
            {
                ...
                WirePortDataType smallest = (
                    (int)outputDataType < (int)inputDataType ? outputDataType : inputDataType
                );
                smallest = (
                    (int)smallest < (int)outputVisualDataType ? smallest : outputVisualDataType
                );
                smallest = (
                    (int)smallest < (int)inputVisualDataType ? smallest : inputVisualDataType
                );
                ...
                wireThickness = Mathf.Lerp(
                    Constants.WIRE_WIDTH * (ty * invertedZoom * -0.05f + 0.15f),
                    Constants.WIRE_WIDTH * (ty * invertedZoom * 0.175f + 0.3f),
                    invertedZoom + 0.4f
                );
            }
            else
            {
                ...
                wireThickness = Mathf.Lerp(
                    Constants.WIRE_WIDTH * (invertedZoom * -0.05f + 0.15f),
                    Constants.WIRE_WIDTH * (invertedZoom * 0.175f + 0.3f),
                    invertedZoom + 0.4f
                );
            }
            ...
            if (ParentWindow.Options.ColoredPorts && wireStatus != WireStatus.Highlighted)
                boundBox = GLDraw.DrawBezier(
                    startPos,
                    startTangent,
                    endPos,
                    endTangent,
                    UIUtils.GetColorForDataType(outputVisualDataType, false, false),
                    UIUtils.GetColorForDataType(inputVisualDataType, false, false),
                    wireThickness,
                    segments,
                    ty
                );
                ...
                boundBox = GLDraw.DrawBezier(
                    ...
                    endPos,
                    endTangent,
                    UIUtils.GetColorFromWireStatus(wireStatus),
                    wireThickness,
                    segments,
                    ty
                );
        ...
        public void SetConnection(int InNodeId, int InPortId, int OutNodeId, int OutPortId)
        {
            ...
            if (inNode != null && outNode != null)
            {
                ...
                {
                    if (
                        inputPort.IsConnectedTo(OutNodeId, OutPortId)
                        || outputPort.IsConnectedTo(InNodeId, InPortId)
                    )
                    {
                        if (DebugConsoleWindow.DeveloperMode)
                            UIUtils.ShowMessage(
                                "Node/Port already connected " + InNodeId,
                                MessageSeverity.Error
                            );
                        return;
                    }
                    ...
                    if (!inputPort.Available || !outputPort.Available)
                    {
                        if (DebugConsoleWindow.DeveloperMode)
                            UIUtils.ShowMessage(
                                "Ports not available to connection",
                ...
                else if ((object)inputPort == null)
                {
                    if (DebugConsoleWindow.DeveloperMode)
                        UIUtils.ShowMessage(
                            "Input Port " + InPortId + " doesn't exist on node " + InNodeId,
                            MessageSeverity.Error
                        );
                }
                else
                {
                    if (DebugConsoleWindow.DeveloperMode)
                        UIUtils.ShowMessage(
                            "Output Port " + OutPortId + " doesn't exist on node " + OutNodeId,
                            MessageSeverity.Error
                        );
                }
            }
            ...
            if (outputNode != null)
            {
                ...
                if (outputPort != null)
                {
                    ...
                    if (
                        ...
                        inputNode.OnInputPortConnected(
                            inPortId,
                            outputNode.UniqueId,
                            outputPort.PortId
                        );
            ...
            {
                {
                    ...
                    {
                        if (
                            !m_nodes[nodeIdx].InputPorts[inputIdx].Visible
                        ...
                        )
                        {
                            DeleteConnection(
                                true,
                                m_nodes[nodeIdx].UniqueId,
                                m_nodes[nodeIdx].InputPorts[inputIdx].PortId,
                                true,
                                true
                            );
                        }
                ...
                }
                {
                    ...
                    {
                        if (
                            !m_nodes[nodeIdx].OutputPorts[outputIdx].Visible
                            && m_nodes[nodeIdx].OutputPorts[outputIdx].IsConnected
                        )
                        {
                            DeleteConnection(
                                false,
                                m_nodes[nodeIdx].UniqueId,
                                m_nodes[nodeIdx].OutputPorts[outputIdx].PortId,
                                true,
                                true
                            );
                        }
        ...
        public void DeleteAllConnectionFromNode(
            int nodeId,
            bool registerOnLog,
        ...
        )
        {
            ...
            if (registerUndo)
            {
                ...
                UndoUtils.RegisterCompleteObjectUndo(
                    ParentWindow,
                    Constants.UndoDeleteConnectionId
                );
            ...
            if (isInput)
            {
                ...
                if (inputPort != null && inputPort.IsConnected)
                {
                    ...
                    for (int i = 0; i < inputPort.ExternalReferences.Count; i++)
                    {
                        ...
                        outputNode
                            .GetOutputPortByUniqueId(inputReference.PortId)
                            .InvalidateConnection(inputPort.NodeId, inputPort.PortId);
                ...
                if (outputPort != null && outputPort.IsConnected)
                {
                    ...
                    for (int i = 0; i < outputPort.ExternalReferences.Count; i++)
                    {
                        ...
                        inputNode
                            ...
                            .InvalidateConnection(outputPort.NodeId, outputPort.PortId);
                        if (propagateCallback)
                        {
        ...
        public void MarkWireNodeSequence(WireNode node, bool isInput)
        {
            ...
            {
                MarkWireNodeSequence(
                    GetNode(node.InputPorts[0].ExternalReferences[0].NodeId) as WireNode,
                    isInput
                );
            }
            ...
            {
                MarkWireNodeSequence(
                    GetNode(node.OutputPorts[0].ExternalReferences[0].NodeId) as WireNode,
                    isInput
                );
            }
        }
        ...
        public void DestroyNode(
            ParentNode node,
            bool registerUndo = true,
            bool destroyMasterNode = false
        )
        {
            if (node == null)
            {
                UIUtils.ShowMessage(
                    "Attempting to destroying a inexistant node ",
                    MessageSeverity.Warning
                );
                return;
            }
            ...
            if (
                destroyMasterNode
                || (
                    node.UniqueId != m_masterNodeId && !(node is TemplateMultiPassMasterNode) /*!m_multiPassMasterNodes.HasNode( node.UniqueId )*/
                )
            )
            {
                ...
                for (int outputPortIdx = 0; outputPortIdx < node.OutputPorts.Count; outputPortIdx++)
                {
                    ...
                    if (outputPort.IsConnected)
                    {
                        for (
                            int wireIdx = 0;
                            wireIdx < outputPort.ExternalReferences.Count;
                            wireIdx++
                        )
                        {
            ...
            {
                TemplateMultiPassMasterNode templateMasterNode =
                    node as TemplateMultiPassMasterNode;
        ...
        private void ForceSignalPropagationOnMasterNodeInternal(
            UsageListTemplateMultiPassMasterNodes masterNodes
        )
        {
        ...
        public void UpdateShaderOnMasterNode(Shader newShader)
        {
            ...
            if (HasLODs)
            {
                ...
                {
                    if (
                        m_lodMultiPassMasterNodes.Count != 0
                        && m_lodMultiPassMasterNodes[i].NodesList.Count > 0
                    )
                    {
                        if (m_lodMultiPassMasterNodes[i].NodesList[passIdx] != null)
                        {
                            m_lodMultiPassMasterNodes[i]
                                .NodesList[passIdx]
                                .UpdateFromShader(newShader);
                        }
                        else
                        {
            ...
            if (HasLODs)
            {
                ...
                for (int i = 0; i < m_lodMultiPassMasterNodes.Count; i++)
                {
                    ...
                    )
                    {
                        m_lodMultiPassMasterNodes[i]
                            .NodesList[passIdx]
                            .UpdateMasterNodeMaterial(material);
                    }
                    else
                        break;
                }
            ...
            for (int i = 0; i < count; i++)
            {
                if (node.InputPorts[i].IsConnected)
                {
                    ResetNodesLocalVariables(
                        m_nodesDict[node.InputPorts[i].GetConnection().NodeId]
                    );
                }
            }
        }
        ...
        public TemplateMultiPassMasterNode CreateMultipassMasterNode(
            int lodId,
            bool registerUndo,
            int nodeId = -1,
            bool addLast = true
        ...
        {
            TemplateMultiPassMasterNode newNode =
                ScriptableObject.CreateInstance<TemplateMultiPassMasterNode>();
            if (newNode)
            {
                ...
                {
                    TemplateMultiPassMasterNode stubNode =
                        newNode.ExecuteStubCode() as TemplateMultiPassMasterNode;
        ...
        public void CrossCheckTemplateNodes(
            TemplateDataParent templateData,
            List<TemplateMultiPassMasterNode> mpNodesList,
            int lodId
        )
        {
            /*Paulo*/
            ...
            Dictionary<string, TemplateReplaceHelper> nodesDict =
                new Dictionary<string, TemplateReplaceHelper>();
            int mpNodeCount = mpNodesList.Count;
            for (int i = 0; i < mpNodeCount; i++)
            {
                string masterNodeId = mpNodesList[i].InvalidNode
                    ? mpNodesList[i].OriginalPassName + "ASEInvalidMasterNode" + i
                    : mpNodesList[i].OriginalPassName;
                nodesDict.Add(masterNodeId, new TemplateReplaceHelper(mpNodesList[i]));
            }

            TemplateMultiPassMasterNode currMasterNode =
                GetNode(m_masterNodeId) as TemplateMultiPassMasterNode;
            ...
            for (
                int subShaderIdx = 0;
                subShaderIdx < multipassData.SubShaders.Count;
                subShaderIdx++
            )
            {
                for (
                    int passIdx = 0;
                    passIdx < multipassData.SubShaders[subShaderIdx].Passes.Count;
                    passIdx++
                )
                {
                    string currPassName = multipassData
                        .SubShaders[subShaderIdx]
                        .Passes[passIdx]
                        .PassNameContainer
                        .Data;
                    if (nodesDict.ContainsKey(currPassName))
                    {
                        ...
                        currentPosition.y +=
                            nodesDict[currPassName].MasterNode.Position.height + 10;
                        nodesDict[currPassName].Used = true;
                        nodesDict[currPassName]
                            .MasterNode.SetTemplate(
                                multipassData,
                                false,
                                false,
                                subShaderIdx,
                                passIdx,
                                SetTemplateSource.NewShader
                            );
                        if (wasMainNode && !nodesDict[currPassName].MasterNode.IsMainOutputNode)
                        {
                            ...
                            !wasMainNode && nodesDict[currPassName].MasterNode.IsMainOutputNode
                        )
                        {
                    ...
                    else
                    {
                        ...
                        TemplateMultiPassMasterNode masterNode = CreateMultipassMasterNode(
                            lodId,
                            false
                        );
        ...
        private void OnRefreshLinkedPortsCompleteInternal(
            UsageListTemplateMultiPassMasterNodes masterNodes
        )
        {
            int mpCount = masterNodes.Count;
        ...
        private void RefreshLinkedMasterNodesInternal(
            UsageListTemplateMultiPassMasterNodes masterNodes,
            bool optionsUpdate
        )
        {
            ...
            if (mpCount > 1)
            {
                Dictionary<string, List<InputPort>> registeredLinks =
                    new Dictionary<string, List<InputPort>>();
                for (int i = 0; i < mpCount; i++)
                {
            ...
            masterNodes.NodesList.Sort(
                (x, y) =>
                    (x.SubShaderIdx * 1000 + x.PassIdx).CompareTo(y.SubShaderIdx * 1000 + y.PassIdx)
            );
        ...
        void CheckLinkedPorts(
            ref Dictionary<string, List<InputPort>> registeredLinks,
        ...
        public MasterNode ReplaceMasterNode(
            AvailableShaderTypes newType,
            bool writeDefaultData = false,
            TemplateDataParent templateData = null
        )
        {
            DeSelectAll();
            ResetNodeConnStatus();
                ...
                case AvailableShaderTypes.SurfaceShader:
                    {
                        ...
                        newMasterNode =
                            CreateNode(typeof(StandardSurfaceOutputNode), false) as MasterNode;
                    }
                    break;
                case AvailableShaderTypes.Template:
                    {
                        ...
                        {
                            newMasterNode =
                                CreateNode(typeof(TemplateMasterNode), false) as MasterNode;
                            (newMasterNode as TemplateMasterNode).SetTemplate(
                        ...
                        else
                        {
                            ...
                            )
                            {
                                ...
                                {
                                    TemplateMultiPassMasterNode masterNode =
                                        CreateNode(typeof(TemplateMultiPassMasterNode), false)
                                        as TemplateMultiPassMasterNode;
                                    if (
                                        multipassData
                                            .SubShaders[subShaderIdx]
                                            .Passes[passIdx]
        ...
        private void RepositionTemplateNodes(MasterNode newMasterNode)
        {
            ...
            for (int i = 0; i < MultiPassMasterNodes.Count; i++)
            {
                ...
                if (
                    !MultiPassMasterNodes.NodesList[i].IsInvisible
                    && MultiPassMasterNodes.NodesList[i].Docking
                )
                {
                    if (foundMaster)
                        dockedElementsAfter++;
            ...
            for (int i = masterIndex - 1; i >= 0; i--)
            {
                ...
                for (int j = i + 1; j <= masterIndex; j++)
                {
                    if (
                        !MultiPassMasterNodes.NodesList[i].IsInvisible
                        && !MultiPassMasterNodes.NodesList[j].Docking
                    )
                    {
                ...
                node.Vec2Position = new Vector2(
                    node.Vec2Position.x,
                    newMasterNode.Position.y - forwardTracking - 33 * (dockedElementsBefore)
                );
            }
            ...
            {
                if (
                    MultiPassMasterNodes.NodesList[i].UniqueId == newMasterNode.UniqueId
                    || MultiPassMasterNodes.NodesList[i].Docking
                )
                    continue;
                ...
                node.Vec2Position = new Vector2(
                    node.Vec2Position.x,
                    newMasterNode.Position.y + backTracking + 33 * (dockedElementsAfter)
                );
            }
        }
        ...
        {
            CleanNodes();
            TemplateDataParent templateData = m_parentWindow.TemplatesManagerInstance.GetTemplate(
                templateGUID
            );
            if (templateData.TemplateType == TemplateDataType.LegacySinglePass)
            {
                TemplateMasterNode newMasterNode =
                    CreateNode(typeof(TemplateMasterNode), false) as TemplateMasterNode;
                m_masterNodeId = newMasterNode.UniqueId;
            ...
            else
            {
                ...
                )
                {
                    ...
                    {
                        TemplateMultiPassMasterNode newMasterNode =
                            CreateNode(typeof(TemplateMultiPassMasterNode), false)
                            as TemplateMultiPassMasterNode;
                        if (multipassData.SubShaders[subShaderIdx].Passes[passIdx].IsMainPass)
                        {
        ...
        {
            CleanNodes();
            FunctionOutput newOutputNode =
                CreateNode(typeof(FunctionOutput), false) as FunctionOutput;
            m_masterNodeId = newOutputNode.UniqueId;
        ...
        public bool HasVirtualTexture
        {
            get { return m_virtualTextureCount > 0; }
        }
        ...
        public int InstancePropertyCount
        {
            get { return m_instancePropertyCount; }
            set { m_instancePropertyCount = value; }
        }

        public bool IsInstancedShader
        {
            get { return m_instancePropertyCount > 0; }
        }
        ...
        public void CreateLodMasterNodes(
            TemplateMultiPass templateMultiPass,
            int index,
        ...
        )
        {
            for (int lod = 0; lod < m_lodMultiPassMasterNodes.Count; lod++)
            {
                ...
                {
                    TemplateMultiPassMasterNode reference =
                        CurrentMasterNode as TemplateMultiPassMasterNode;
                    ...
                    else
                    {
                        ...
                        {
                            shaderLod =
                                m_lodMultiPassMasterNodes[lod - 1]
                                    .NodesList[reference.PassIdx]
                                    .ShaderLOD - MasterNodeLODIncrement;
                        }
                        //index == -1 is when user clicks on + button over the main lod master node
                        else if (index == -1)
                        {
                            int mainShaderLOD = m_lodMultiPassMasterNodes[0]
                                .NodesList[reference.PassIdx]
                                .ShaderLOD;
                        ...
                        else
                        {
                            if (m_lodMultiPassMasterNodes[index].Count > 0)
                            {
                                if (m_lodMultiPassMasterNodes[index + 1].Count > 0)
                                {
                                    shaderLod =
                                        (
                                            m_lodMultiPassMasterNodes[index]
                                                .NodesList[reference.PassIdx]
                                                .ShaderLOD
                                            + m_lodMultiPassMasterNodes[index + 1]
                                                .NodesList[reference.PassIdx]
                                                .ShaderLOD
                                        ) / 2;
                                }
                    ...
                    for (
                        int subShaderIdx = 0;
                        subShaderIdx < templateMultiPass.SubShaders.Count;
                        subShaderIdx++
                    )
                    {
                        for (
                            int passIdx = 0;
                            passIdx < templateMultiPass.SubShaders[subShaderIdx].Passes.Count;
                            passIdx++
                        )
                        {
                            TemplateMultiPassMasterNode masterNode =
                                ScriptableObject.CreateInstance(typeof(TemplateMultiPassMasterNode))
                                as TemplateMultiPassMasterNode;
                    ...
                    TemplateMultiPassMasterNode lodMainMasterNode =
                        CurrentMasterNode as TemplateMultiPassMasterNode;
        ...
        public void SortLODMasterNodes()
        {
            ...
            m_lodMultiPassMasterNodes.Sort(
                (x, y) =>
                {
                    if (x.Count > 0)
                    {
                        if (y.Count > 0)
                        {
                            return -x.NodesList[idx]
                                .ShaderLOD.CompareTo(y.NodesList[idx].ShaderLOD);
                        }
                        else
                        {
                            return -1;
        ...
        public bool IsNormalDependent
        {
            get { return m_normalDependentCount > 0; }
        }
        ...
        public NodeAvailability CurrentCanvasMode
        {
            get { return m_currentCanvasMode; }
            set
            {
                m_currentCanvasMode = value;
                ParentWindow.LateRefreshAvailableNodes();
            }
        }
        public OutputNode CurrentOutputNode
        {
            get { return GetNode(m_masterNodeId) as OutputNode; }
        }
        public FunctionOutput CurrentFunctionOutput
        {
            get { return GetNode(m_masterNodeId) as FunctionOutput; }
        }
        ...
        public AmplifyShaderFunction CurrentShaderFunction
        {
            get
            {
                if (CurrentFunctionOutput != null)
                    return CurrentFunctionOutput.Function;
                else
                    return null;
            }
            set
            {
                if (CurrentFunctionOutput != null)
                    CurrentFunctionOutput.Function = value;
            }
        }

        public bool HasUnConnectedNodes
        {
            get { return m_hasUnConnectedNodes; }
        }
        public UsageListSamplerNodes SamplerNodes
        {
            get { return m_samplerNodes; }
        }
        public UsageListFloatIntNodes FloatIntNodes
        {
            get { return m_floatNodes; }
        }
        public UsageListTexturePropertyNodes TexturePropertyNodes
        {
            get { return m_texturePropertyNodes; }
        }
        public UsageListTextureArrayNodes TextureArrayNodes
        {
            get { return m_textureArrayNodes; }
        }
        public UsageListPropertyNodes PropertyNodes
        {
            get { return m_propertyNodes; }
        }
        public UsageListPropertyNodes RawPropertyNodes
        {
            get { return m_rawPropertyNodes; }
        }
        public UsageListCustomExpressionsOnFunctionMode CustomExpressionOnFunctionMode
        {
            get { return m_customExpressionsOnFunctionMode; }
        }
        public UsageListStaticSwitchNodes StaticSwitchNodes
        {
            get { return m_staticSwitchNodes; }
        }
        public UsageListScreenColorNodes ScreenColorNodes
        {
            get { return m_screenColorNodes; }
        }
        public UsageListRegisterLocalVarNodes LocalVarNodes
        {
            get { return m_localVarNodes; }
        }
        public UsageListGlobalArrayNodes GlobalArrayNodes
        {
            get { return m_globalArrayNodes; }
        }
        public UsageListFunctionInputNodes FunctionInputNodes
        {
            get { return m_functionInputNodes; }
        }
        public UsageListFunctionNodes FunctionNodes
        {
            get { return m_functionNodes; }
        }
        public UsageListFunctionOutputNodes FunctionOutputNodes
        {
            get { return m_functionOutputNodes; }
        }
        public UsageListFunctionSwitchNodes FunctionSwitchNodes
        {
            get { return m_functionSwitchNodes; }
        }
        public UsageListFunctionSwitchCopyNodes FunctionSwitchCopyNodes
        {
            get { return m_functionSwitchCopyNodes; }
        }
        public UsageListTemplateMultiPassMasterNodes MultiPassMasterNodes
        {
            get { return m_multiPassMasterNodes; }
            set { m_multiPassMasterNodes = value; }
        }
        public List<UsageListTemplateMultiPassMasterNodes> LodMultiPassMasternodes
        {
            get { return m_lodMultiPassMasterNodes; }
        }
        ...
        public List<ParentNode> NodePreviewList
        {
            get { return m_nodePreviewList; }
            set { m_nodePreviewList = value; }
        }
        ...
        {
            get { return m_isDuplicating; }
            set { m_isDuplicating = value; }
        }
        ...
        public bool IsSRP
        {
            get
            {
                return m_currentSRPType == TemplateSRPType.URP
        ...
        public bool IsStandardSurface
        {
            get { return GetNode(m_masterNodeId) is StandardSurfaceOutputNode; }
        }
        ...
        public bool HasLODs
        {
            get { return m_lodMultiPassMasterNodes[0].Count > 0; }
        }
