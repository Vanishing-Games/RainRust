...
namespace AmplifyShaderEditor
{
    ...
    public class EditableIf : MaterialPropertyDrawer
    {
        ...
        public EditableIf(
            object fieldname,
            object comparison,
            object expectedvaluex,
            object expectedvaluey
        )
        {
        ...
        )
        {
            if (InputError)
            {
                EditorGUI.LabelField(
                    position,
                    "EditableIf Attribute Error: Input parameters are invalid!"
                );
                return;
            }
            ...
            {
                LHSprop = MaterialEditor.GetMaterialProperty(
                    prop.targets,
                    "_" + FieldName.Replace(" ", "")
                );
                if (string.IsNullOrEmpty(LHSprop.name))
                {
                    EditorGUI.LabelField(
                        position,
                        "EditableIf Attribute Error: " + FieldName + " Does not exist!"
                    );
                    return;
                }
            }
            ...
            switch (LHSprop.propertyType)
            {
                ...
                case UnityEngine.Rendering.ShaderPropertyType.Vector:
                    LHSVal =
                        LHSprop.propertyType == UnityEngine.Rendering.ShaderPropertyType.Color
                            ? (Vector4)LHSprop.colorValue
                            : LHSprop.vectorValue;
