    ...
    public class WirePort
    {
        ...
        public void AddPortForbiddenTypes(params WirePortDataType[] forbiddenTypes)
        {
            if (forbiddenTypes != null)
            {
                ...
                {
                    m_portRestrictions =
                        m_portRestrictions & (int.MaxValue - (int)forbiddenTypes[i]);
                }
            }
        }
        ...
        {
            m_externalReferences.Insert(
                0,
                new WireReference(nodeId, portId, WirePortDataType.OBJECT, false)
            );
            m_isDummy = true;
        }
        ...
        public void UpdateInfoOnExternalConn(int nodeId, int portId, WirePortDataType type)
        {
            ...
            {
                if (
                    m_externalReferences[i].NodeId == nodeId
                    && m_externalReferences[i].PortId == portId
                )
                {
        ...
        public void RemoveInvalidConnections()
        {
            ...
            for (int i = 0; i < m_externalReferences.Count; i++)
            {
                ...
                {
                    Debug.Log(
                        "Detected invalid connection on node " + m_nodeId + " port " + m_portId
                    );
                }
            }
            m_externalReferences.Clear();
        ...
        public bool NotFreeForAllTypes
        {
            get { return m_portRestrictions != -1; }
        }
    }
}
