        ...
        public override void OnInspectorGUI()
        {
            ...
            for (int i = 0; i < m_dependencies.Count; i++)
            {
                ...
                if (GUILayout.Button("edit", "minibuttonright", GUILayout.Width(100)))
                {
                    ...
                    {
                        var obj = AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(
                            m_dependencies[i].AssetPath
                        );
                        AmplifyShaderEditorWindow.LoadShaderFunctionToASE(obj, false);
                    }
                    else
                    {
                        var obj = AssetDatabase.LoadAssetAtPath<Shader>(
                            m_dependencies[i].AssetPath
                        );
            ...
            if (m_dependencies.Count > 0)
            {
                ...
                {
                    bool doit = EditorUtility.DisplayDialog(
                        "Open and Save All",
                        "This will try to open all shader function and shaders that use this shader function and save them in quick succession, this may irreversibly break your files if something goes wrong. Are you sure you want to try?",
