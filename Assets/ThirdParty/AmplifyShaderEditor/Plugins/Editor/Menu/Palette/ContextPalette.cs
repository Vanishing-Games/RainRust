...
namespace AmplifyShaderEditor
{
    public sealed class ContextPalette : PaletteParent
    {
        ...
        public ContextPalette(AmplifyShaderEditorWindow parentWindow)
            : base(parentWindow, 0, 0, 250, 250, string.Empty, MenuAnchor.NONE, MenuAutoSize.NONE)
        {
            m_isActive = false;
        ...
        public override void OnEnterPressed(int index = 0)
        {
            ...
            {
                FireNodeCreateEvent(
                    m_currentItems[index].NodeType,
                    m_currentItems[index].Name,
                    m_currentItems[index].Function
                );
            }
            else
            {
        ...
        public void Show(Vector2 position, Rect cameraInfo)
        {
            ...
            m_maximizedArea.x =
                (position.x + m_maximizedArea.width) > cameraInfo.width
                    ? (cameraInfo.width - 1.1f * m_maximizedArea.width)
                    : position.x;
            m_maximizedArea.y =
                (position.y + m_maximizedArea.height) > cameraInfo.height
                    ? (cameraInfo.height - 1.1f * m_maximizedArea.height)
                    : position.y;
