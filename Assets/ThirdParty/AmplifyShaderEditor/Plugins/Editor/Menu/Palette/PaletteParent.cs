...
namespace AmplifyShaderEditor
{
    ...
    public class PaletteParent : MenuParent
    {
        ...
        public delegate void OnPaletteNodeCreate(
            System.Type type,
            string name,
        ...
        public PaletteParent(
            ...
            float height,
            string name,
            MenuAnchor anchor = MenuAnchor.NONE,
            MenuAutoSize autoSize = MenuAutoSize.NONE
        )
            : base(parentWindow, x, y, width, height, name, anchor, autoSize)
        {
        ...
        public void FireNodeCreateEvent(
            System.Type type,
            string name,
            AmplifyShaderFunction function
        )
        ...
        )
        {
            ...
            GUILayout.BeginArea(m_transformedArea, m_content, m_style);
            {
                ...
                if (currenEvent.type == EventType.KeyDown)
                {
                    ...
                    if (
                        (
                            currenEvent.keyCode == KeyCode.KeypadEnter
                            || currenEvent.keyCode == KeyCode.Return
                        )
                ...
                m_currScrollBarDims.y =
                    m_transformedArea.height - 2 - 16 - 2 - 7 * m_initialSeparatorAmount - 2;
                ...
                );
                {
                    if (m_forceUpdate)
                    {
                        ...
                        else
                        {
                            for (int i = 0; i < allItems.Count; i++)
                            {
                                ...
                                {
                                    MatchCollection wordmatch = Regex.Matches(
                                        allItems[i].Tags,
                                        "\\b" + searchList[k]
                                    );
                                ...
                                if (searchList.Length == matchesFound)
                                {
                                    ...
                                    m_currentCategories[allItems[i].Category]
                                        .Contents.Add(allItems[i]);
                                }
                            }
                        }
                        ...
                        {
                            categoryEnumerator.Current.Value.Contents.Sort(
                                (x, y) => x.CompareTo(y, usingSearchFilter)
                            );
                        }
                    ...
                    if (
                        (
                            currenEvent.keyCode == KeyCode.DownArrow
                            || currenEvent.keyCode == KeyCode.UpArrow
                        )
                    ...
                    )
                    {
                        if (
                            GUI.GetNameOfFocusedControl()
                                .Equals(m_searchFilterControl + m_resizable)
                        )
                        {
                    ...
                    )
                    {
                        ...
                        int nextIndex =
                            m_currentItems.FindIndex(x =>
                                GUI.GetNameOfFocusedControl()
                                    .Equals(x.ItemUIContent.text + m_resizable)
                            ) + 1;
                        ...
                        if (m_searchFilter.Length > 0 || current.Value.Visible)
                        {
                            for (int i = 0; i < current.Value.Contents.Count; i++)
                            {
                                ...
                                //else
                                {
                                    ...
                                    //else
                                    {
                                        Rect thisRect = EditorGUILayout.GetControlRect(
                                            false,
                                            16f,
                                            EditorStyles.label
                                        );
                                        //if ( m_resizable )
                                        {
                                            ...
                                            )
                                            {
                                                ...
                                                OnPaletteNodeCreateEvt(
                                                    current.Value.Contents[i].NodeType,
                                                    current.Value.Contents[i].Name,
                                                    current.Value.Contents[i].Function
                                                );
                                        ...
                                        GUI.SetNextControlName(
                                            current.Value.Contents[i].ItemUIContent.text
                                                + m_resizable
                                        );
                                        ...
                                        {
                                            bool boundBottom =
                                                currPos - m_currentScrollPos.y
                                                > m_currScrollBarDims.y;
                                            bool boundTop = currPos - m_currentScrollPos.y - 4 <= 0;

                                            if (boundBottom)
                                                m_currentScrollPos.y =
                                                    currPos - m_currScrollBarDims.y + 2;
                                            else if (boundTop)
        ...
        public void DumpAvailableNodes(bool fromCommunity, string pathname)
        {
                ...
                if (fromCommunity && current.Value.HasCommunityData || !fromCommunity)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        if (
                            (
                                fromCommunity
                                && current.Value.Contents[i].NodeAttributes.FromCommunity
                            ) || !fromCommunity
                        ...
                        )
                        {
                            ...
                            string pageFilename = UIUtils.RemoveWikiInvalidCharacters(
                                pictureFilename
                            );

                            pictureFilename = UIUtils.RemoveInvalidCharacters(pictureFilename);

                            string nodeDescription = current
                                .Value
                                .Contents[i]
                                .ItemUIContent
                                .tooltip;
                            ...
                            if (current.Value.Contents[i].NodeAttributes.FromCommunity)
                                communityText =
                                    "<small class=\"cardauthor\">( originally by "
                                    + current.Value.Contents[i].NodeAttributes.Community
                                    + " )</small> ";

                            string nodeInfoBody = string.Format(
                                nodeInfoBodyFormat,
                                pictureFilename,
                                pageFilename,
                                nodeFullName,
            ...
            string finalText =
                noTOCHeader + nodesHeader + InitialCategories + nodesInfo + OverallFoot;
            ...
            string nodesPathname =
                pathname
                + (fromCommunity ? "AvailableNodesFromCommunity.txt" : "AvailableNodes.txt");
        ...
        {
            if (
                (currPos < m_currentScrollPos.y && (currPos + ItemSize) < m_currentScrollPos.y)
                || (
