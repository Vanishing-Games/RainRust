...
namespace AmplifyShaderEditor
{
    public sealed class ToolsMenuButton : ToolsMenuButtonParent
    {
        ...
        public ToolsMenuButton(
            ...
            bool drawOnFunction = true
        )
            : base(parentWindow, text, tooltip, buttonSpacing)
        {
        ...
        protected override void Init()
        {
            ...
            if (m_buttonTexture == null)
            {
                ...
                m_buttonTexture.Add(
                    AssetDatabase.LoadAssetAtPath(m_buttonTexturePath, typeof(Texture2D))
                        as Texture2D
                );
            }
            ...
            if (m_cachedStates.Count > 0)
            {
                for (int i = 0; i < m_cachedStates.Count; i++)
                {
                    ...
                    m_buttonTexture.Add(
                        AssetDatabase.LoadAssetAtPath(m_cachedStates[i], typeof(Texture2D))
                            as Texture2D
                    );
                }
        ...
        public override void Draw(float x, float y)
        {
            if (
                !(
                    ...
                    || m_parentWindow.CameraDrawInfo.CurrentEventType == EventType.Repaint
                )
            )
                return;
            ...
            if (
                m_parentWindow.CameraDrawInfo.CurrentEventType == EventType.MouseDown
                && m_buttonArea.Contains(m_parentWindow.CameraDrawInfo.MousePosition)
                && ToolButtonPressedEvt != null
            )
            {
                ToolButtonPressedEvt(m_buttonType);
