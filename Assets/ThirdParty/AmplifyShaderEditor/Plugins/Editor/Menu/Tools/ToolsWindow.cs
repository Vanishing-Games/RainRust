...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ToolsWindow : MenuParent
    {
        ...
        public ToolsWindow(AmplifyShaderEditorWindow parentWindow)
            : base(parentWindow, 0, 0, 0, 64, "Tools", MenuAnchor.TOP_LEFT, MenuAutoSize.NONE)
        {
            m_updateButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            m_liveButton = new ToolsMenuButton(
                m_parentWindow,
                ToolButtonType.Live,
                0,
                0,
            ...
            m_openSourceCodeButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            // middle right
            m_cleanUnusedNodesButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            m_focusOnMasterNodeButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            m_focusOnSelectionButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            // right
            m_shareButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            m_takeScreenshotButton = new ToolsMenuButton(
                m_parentWindow,
            ...
            m_showInfoWindowButton = new ToolsMenuButton(
                m_parentWindow,
                ToolButtonType.ShowInfoWindow,
                0,
                0,
                -1,
                -1,
                IOUtils.ShowInfoWindowGUID,
                string.Empty,
                "Open Helper Window."
            );
            ...
            // hidden
            m_showTipsWindowButton = new ToolsMenuButton(
                m_parentWindow,
                ToolButtonType.ShowTipsWindow,
                0,
                0,
                -1,
                -1,
                IOUtils.ShowTipsWindowGUID,
                string.Empty,
                "Open Quick Tips!"
            );
            m_showTipsWindowButton.ToolButtonPressedEvt += OnButtonPressedEvent;

            m_showConsoleWindowButton = new ToolsMenuButton(
                m_parentWindow,
                ToolButtonType.ShowConsole,
                0,
                0,
                -1,
                -1,
                IOUtils.ShowConsoleWindowGUID,
                string.Empty,
        ...
        )
        {
            ...
            //}
            GUI.color = m_updateButton.IsInside(mousePosition)
                ? LeftIconsColorOn
                : LeftIconsColorOff;
            ...
            GUI.color = m_openSourceCodeButton.IsInside(mousePosition)
                ? LeftIconsColorOn
                : LeftIconsColorOff;
            m_openSourceCodeButton.Draw(
                TabX + m_transformedArea.x + m_openSourceCodeButton.ButtonSpacing,
                TabY
            );
            ...
            if (m_searchBarVisible)
            {
                    ...
                    if (Event.current.shift)
                    {
                        if (
                            ...
                            || (
                                ...
                                && (
                                    currentFocus.Equals(SearchBarId)
                                    || string.IsNullOrEmpty(currentFocus)
                                )
                            )
                        )
                            SelectPrevious();
                ...
                if (
                    currentFocus.Equals(SearchBarId)
                    || (
                        m_parentWindow.CameraDrawInfo.CurrentEventType == EventType.MouseDown
                        && m_searchBarSize.Contains(m_parentWindow.CameraDrawInfo.MousePosition)
                    )
                    || m_selectSearchBarTextfield
                )
                {
                    EditorGUI.BeginChangeCheck();
                    {
                        GUI.SetNextControlName(SearchBarId);
                        m_searchBarValue = EditorGUI.TextField(
                            m_searchBarSize,
                            m_searchBarValue,
                            UIUtils.ToolbarSearchTextfield
                        );
                    }
                ...
                if (
                    m_refreshSearchResultList
                    && (m_parentWindow.CurrentInactiveTime > InactivityRefreshTime)
                )
                {
                    RefreshList();
                }
            ...
            GUI.color = m_shareButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_shareButton.Draw(m_transformedArea.x + m_transformedArea.width - 195 - TabX, TabY);

            GUI.color = m_takeScreenshotButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_takeScreenshotButton.Draw(
                m_transformedArea.x + m_transformedArea.width - 165 - TabX,
                TabY
            );

            GUI.color = m_focusOnSelectionButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_focusOnSelectionButton.Draw(
                m_transformedArea.x + m_transformedArea.width - 120 - TabX,
                TabY
            );

            GUI.color = m_focusOnMasterNodeButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_focusOnMasterNodeButton.Draw(
                m_transformedArea.x + m_transformedArea.width - 85 - TabX,
                TabY
            );

            GUI.color = m_cleanUnusedNodesButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_cleanUnusedNodesButton.Draw(
                m_transformedArea.x + m_transformedArea.width - 50 - TabX,
                TabY
            );

            GUI.color = m_showInfoWindowButton.IsInside(mousePosition)
                ? RightIconsColorOn
                : RightIconsColorOff;
            m_showInfoWindowButton.Draw(
                m_transformedArea.x + m_transformedArea.width - 25 - TabX,
                TabY
            );
        ...
        void SelectNext()
        {
            if (
                m_refreshSearchResultList
                || m_parentWindow.CurrentGraph.AllNodes.Count != m_previousNodeCount
            )
            {
                RefreshList();
            }
        ...
        public void DrawShaderTitle(
            MenuParent nodeParametersWindow,
            MenuParent paletteWindow,
            float availableCanvasWidth,
            float graphAreaHeight,
            string shaderName
        )
        {
            float leftAdjust = nodeParametersWindow.IsMaximized
                ? nodeParametersWindow.RealWidth
                : 0;
            ...
            m_boxRect.width += nodeParametersWindow.IsMaximized
                ? 0
                : nodeParametersWindow.RealWidth;
            ...
            UIUtils.ToolbarMainTitle.padding.right =
                (paletteWindow.IsMaximized ? 195 : 230) + extra;
            UIUtils.ToolbarMainTitle.padding.left = nodeParametersWindow.IsMaximized ? 110 : 145;

            if (m_borderStyle == null)
            {
                m_borderStyle =
                    (ParentWindow.CurrentGraph.CurrentMasterNode == null)
                        ? UIUtils.GetCustomStyle(CustomStyle.ShaderFunctionBorder)
                        : UIUtils.GetCustomStyle(CustomStyle.ShaderBorder);
            }
        ...
        public override bool IsInside(Vector2 position)
        {
            ...
            return m_boxRect.Contains(position)
                || m_areaLeft.Contains(position)
                || m_areaRight.Contains(position);
        }
