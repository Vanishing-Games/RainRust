    ...
    public class GraphContextMenu
    {
        ...
        public bool RefreshNodes(ParentGraph currentGraph)
        {
            ...
            try
            {
                ...
                Type[] availableTypes = IOUtils.GetTypesInNamespace(
                    mainAssembly,
                    "AmplifyShaderEditor"
                );
                ...
                try
                {
                    ...
                    {
                        Type[] extraTypes = IOUtils.GetTypesInNamespace(
                            editorAssembly,
                            "AmplifyShaderEditor"
                        );
                ...
                foreach (System.Type type in availableTypes)
                {
                    ...
                    {
                        foreach (
                            NodeAttributes attribute in Attribute
                                .GetCustomAttributes(type)
                                .OfType<NodeAttributes>()
                        )
                        {
                            if (attribute.Available && !attribute.Deprecated)
                            {
                                ...
                                if (
                                    attribute.CastType != null
                                    && attribute.CastType.Length > 0
                                    && type != null
                                )
                                {
                                ...
                                if (attribute.ShortcutKey != KeyCode.None && type != null)
                                    m_shortcutTypes.Add(
                                        ...
                                        new ShortcutKeyData(type, attribute.Name)
                                    );
                                ...
                                if (
                                    UIUtils.GetNodeAvailabilityInBitArray(
                                        attribute.NodeAvailabilityFlags,
                                    ...
                                    UIUtils.GetNodeAvailabilityInBitArray(
                                        attribute.NodeAvailabilityFlags,
                                        currentGraph.ParentWindow.CurrentNodeAvailability
                                    )
                                )
                                    m_items.Add(newItem);
                                else if (
                                    UIUtils.GetNodeAvailabilityInBitArray(
                                        attribute.NodeAvailabilityFlags,
                                        currentGraph.CurrentCanvasMode
                                    )
                                )
                                    m_items.Add(newItem);
            ...
            {
                allFunctions.Add(
                    AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(
            ...
            for (int i = 0; i < functionCount; i++)
            {
                ...
                {
                    NodeAttributes attribute = new NodeAttributes(
                        allFunctions[i].FunctionName,
                        allFunctions[i].CustomNodeCategory,
                        allFunctions[i].Description,
                        KeyCode.None,
                        true,
                        0,
                        int.MaxValue,
                    ...
                    ContextMenuItem newItem = new ContextMenuItem(
                        attribute,
                        type,
                        AddSpacesToSentence(attribute.Name),
                        attribute.Tags,
                        attribute.Category,
                        attribute.Description,
                        allFunctions[i],
        ...
        public static string AddSpacesToSentence(string text)
        {
            ...
            for (int i = 1; i < text.Length; i++)
            {
                ...
                if (i < text.Length - 1)
                {
                    ...
                    bool lastIsLower =
                        i < text.Length - 2
                            ? char.IsLower(text, i + 2) && char.IsLetter(text, i + 2)
                            : false;
        ...
        public ParentNode CreateNodeFromShortcutKey()
        {
            ...
            if (
                m_shortcutTypes.ContainsKey(m_lastKeyPressed)
                && m_shortcutTypes[m_lastKeyPressed].IsPressed
            )
            {
                ParentNode newNode = (ParentNode)
                    ScriptableObject.CreateInstance(m_shortcutTypes[m_lastKeyPressed].NodeType);
                return newNode;
            }
            return null;
        }
        ...
        public List<ContextMenuItem> ItemFunctions
        {
            get { return m_itemFunctions; }
        }
        ...
        public Dictionary<KeyCode, ShortcutKeyData> NodeShortcuts
        {
            get { return m_shortcutTypes; }
        }
        public bool CorrectlyLoaded
        {
            get { return m_correctlyLoaded; }
        }
    }
}
