    ...
    public class MenuParent
    {
        ...
        protected void InitDraw(Rect parentPosition, Vector2 mousePosition, int mouseButtonId)
        {
            if (m_style == null)
            {
                ...
                Texture minimizeTex = UIUtils
                    .GetCustomStyle(CustomStyle.MaximizeButton)
                    .normal.background;
                ...
                if (m_resizable)
                {
                    if (m_isResizing)
                    {
                        if (m_anchor == MenuAnchor.TOP_LEFT)
                            m_resizeDelta = (
                                ParentWindow.CurrentEvent.mousePosition.x - m_maximizedArea.width
                            );
                        else if (m_anchor == MenuAnchor.TOP_RIGHT)
                            m_resizeDelta =
                                ParentWindow.CurrentEvent.mousePosition.x
                                - (parentPosition.width - m_maximizedArea.width);
                    }
                }
            ...
            switch (m_anchor)
            {
                case MenuAnchor.TOP_LEFT:
                    {
                        ...
                        if (m_isMaximized)
                        {
                            m_minimizeButtonPos.x =
                                m_transformedArea.x
                                + m_transformedArea.width
                                - m_minimizeButtonPos.width
                                - MinimizeButtonXSpacing;
                        ...
                        else
                        {
                            ...
                            m_minimizeButtonPos.x =
                                m_transformedArea.x
                                + width * 0.5f
                                - m_minimizeButtonPos.width * 0.5f;
                            m_minimizeButtonPos.y =
                                m_transformedArea.height * 0.5f - m_minimizeButtonPos.height * 0.5f;
                        }
                    }
                    break;
                ...
                case MenuAnchor.TOP_RIGHT:
                    {
                        m_transformedArea.x =
                            parentPosition.width - currentArea.x - currentArea.width;
                        m_transformedArea.y = currentArea.y;
                        if (m_isMaximized)
                        {
                ...
                case MenuAnchor.BOTTOM_LEFT:
                    {
                        m_transformedArea.x = currentArea.x;
                        m_transformedArea.y =
                            parentPosition.height - currentArea.y - currentArea.height;
                    }
                    break;
        ...
        )
        {
            ...
            if (
                ParentWindow.CurrentEvent.type == EventType.MouseDrag
                && ParentWindow.CurrentEvent.button > 0 /*catches both middle and right mouse button*/
            )
            {
                m_isMouseInside = IsInside(mousePosition);
                if (m_isMouseInside)
                {
                    m_currentScrollPos.x +=
                        Constants.MenuDragSpeed * ParentWindow.CurrentEvent.delta.x;
                    if (m_currentScrollPos.x < 0)
                        m_currentScrollPos.x = 0;
                    m_currentScrollPos.y +=
                        Constants.MenuDragSpeed * ParentWindow.CurrentEvent.delta.y;
        ...
        public void PostDraw()
        {
            ...
            if (m_parentWindow.CameraDrawInfo.CurrentEventType == EventType.Repaint)
                GUI.Label(
                    ...
                    UIUtils.GetCustomStyle(
                        m_isMaximized ? CustomStyle.MinimizeButton : CustomStyle.MaximizeButton
                    )
                );

            if (
                m_parentWindow.CameraDrawInfo.CurrentEventType == EventType.MouseDown
                && buttonArea.Contains(m_parentWindow.CameraDrawInfo.MousePosition)
            )
            ...
            if (m_resizable && m_isMaximized)
            {
                ...
                if (
                    !m_isResizing && GUI.RepeatButton(m_resizeArea, string.Empty, m_resizeAreaStyle)
                )
                {
                    m_isResizing = true;
                }
                else
                {
                    ...
                    {
                        if (
                            ParentWindow.CurrentEvent.isMouse
                            && ParentWindow.CurrentEvent.type != EventType.MouseDrag
                        )
                        {
                            m_isResizing = false;
