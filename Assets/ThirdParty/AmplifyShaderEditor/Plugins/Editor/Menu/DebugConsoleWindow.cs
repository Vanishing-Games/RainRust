...
namespace AmplifyShaderEditor
{
    public sealed class DebugConsoleWindow : EditorWindow
    {
        ...
        public static DebugConsoleWindow OpenWindow()
        {
            if (DeveloperMode)
            {
                DebugConsoleWindow currentWindow = (DebugConsoleWindow)
                    DebugConsoleWindow.GetWindow(
                        typeof(DebugConsoleWindow),
                        false,
                        "ASE Debug Console"
                    );
                currentWindow.titleContent.tooltip =
                    "Debug Options for ASE. Intented only for ASE development team";
        ...
        void ShowWikiHelperFunctions()
        {
            ...
            {
                window.ShortcutManagerInstance.DumpShortcutsToDisk(
                    Application.dataPath + "/../NodesInfo/"
                );
            }
        }
            ...
            if (GUILayout.Button("Detect Infinite Loops"))
            {
                ...
                else
                {
                    if (window.OutsideGraph.MultiPassMasterNodes.Count > 0)
                    {
                        ...
                        List<TemplateMultiPassMasterNode> nodes = window
                            .OutsideGraph
                    ...
                    else
                    {
                        ...
                        UIUtils.DetectNodeLoopsFrom(
                            window.OutsideGraph.CurrentMasterNode,
