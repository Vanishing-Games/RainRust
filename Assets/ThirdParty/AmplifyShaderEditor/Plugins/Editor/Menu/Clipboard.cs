...
namespace AmplifyShaderEditor
{
    public class ClipboardData
    {
        ...
        {
            return Data
                + IOUtils.CLIPBOARD_DATA_SEPARATOR
                + Connections
                + IOUtils.CLIPBOARD_DATA_SEPARATOR
                + OldNodeId
                + IOUtils.CLIPBOARD_DATA_SEPARATOR
    ...
    public class Clipboard
    {
            ...
            for (int i = 0; i < templatesAmount; i++)
            {
                if (!masterNodes[i].InvalidNode)
                {
                    ...
                    ClipboardData clipboardData = new ClipboardData(
                        data,
                        connection,
                        masterNodes[i].UniqueId
                    );
                    m_multiPassMasterNodeData.Add(
                        masterNodes[i].PassUniqueName + lodId,
                        clipboardData
                    );
                }
            }
        ...
        public void GetMultiPassNodesFromClipboard(
            List<TemplateMultiPassMasterNode> masterNodes,
            int lodId
        )
        {
        ...
        )
        {
            ...
            {
                if (
                    UIUtils.CurrentWindow.IsShaderFunctionWindow
                    || !graph.IsMasterNode(selectedNodes[i])
                )
                {
        ...
        public Vector3 GetDataFromEditorPrefs()
        {
            ...
            if (!string.IsNullOrEmpty(clipboardData))
            {
                string[] clipboardDataArray = clipboardData.Split(
                    ClipboardTagId,
        ...
        public bool HasCachedMasterNodes
        {
            get { return m_multiPassMasterNodeData.Count > 0; }
        }
    }
}
