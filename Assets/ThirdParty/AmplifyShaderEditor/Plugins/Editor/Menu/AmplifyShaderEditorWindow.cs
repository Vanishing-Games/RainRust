...
namespace AmplifyShaderEditor
{
    ...
    public class AmplifyShaderEditorWindow : SearchableEditorWindow, ISerializationCallbackReceiver
    {
        ...
        public const string LiveShaderError =
            "Live Shader only works with an assigned Master Node on the graph";

        private const string OnlineSharingWarning =
            "Please enable \"Allow downloads over HTTP*\" in Player Settings to use the Online Sharing feature.";
        ...
        public ParentGraph MainGraphInstance
        {
        ...
        private static System.Diagnostics.Stopwatch m_batchTimer =
        ...
        public static void ConvertShaderToASE(Shader shader, bool OpenOnSeparateWindow = false)
        {
            if (UIUtils.IsUnityNativeShader(shader))
            {
                Debug.LogWarningFormat(
                    "Action not allowed. Attempting to load the native {0} shader into Amplify Shader Editor",
                    shader.name
                );
                return;
            }
            if (IOUtils.IsASEShader(shader))
            {
                ...
                else
                {
                    ...
                    if (IOUtils.AllOpenedWindows.Count > 0)
                    {
                        ...
                        {
                            EditorWindow openedWindow =
                                AmplifyShaderEditorWindow.GetWindow<AmplifyShaderEditorWindow>();
                            AmplifyShaderEditorWindow currentWindow = CreateTab();
            ...
            {
                Debug.LogWarning(
                    "[AmplifyShaderEditor] Can't open shader "
                        + shader.name
                        + " because it was not created in ASE."
                );
        ...
        public static void LoadShaderFunctionToASE(
            AmplifyShaderFunction shaderFunction,
            bool openedAssetFromNode,
            bool OpenOnSeparateWindow = false
        )
        {
            ...
            {
                AmplifyShaderEditorWindow currentWindow = CreateTab(
                    shaderFunction.FunctionName,
                    UIUtils.ShaderFunctionIcon
                );
                UIUtils.CurrentWindow = currentWindow;
                currentWindow.Show();
            }
            ...
            {
                UIUtils.CurrentWindow.titleContent.text = GenerateTabTitle(
                    shaderFunction.FunctionName
                );
        ...
        {
            AmplifyShaderEditorWindow currentWindow = (AmplifyShaderEditorWindow)
                AmplifyShaderEditorWindow.GetWindow(typeof(AmplifyShaderEditorWindow), false);
        ...
        {
            AmplifyShaderEditorWindow currentWindow =
                EditorWindow.CreateInstance<AmplifyShaderEditorWindow>();
            currentWindow.minSize = new Vector2((Constants.MINIMIZE_WINDOW_LOCK_SIZE - 150), 270);
            currentWindow.wantsMouseMove = true;
            if (title != null)
                currentWindow.titleContent.text = GenerateTabTitle(title);
        ...
        public override void OnEnable()
        {
            ...
            Shader.SetGlobalVector(
                PreviewSizeGlobalVariable,
                new Vector4(Constants.PreviewSize, Constants.PreviewSize, 0, 0)
            );
            ...
            m_paletteWindow = new PaletteWindow(this) { Resizable = true };
            ...
            m_shortcutManager.RegisterEditorShortcut(
                ...
                () =>
                {
                    ...
                    if (selectedNodes != null && selectedNodes.Count == 1)
                    {
                        ...
                        {
                            string url =
                                (string.IsNullOrEmpty(shaderFunctionNode.Function.URL))
                                    ? Constants.NodeCommonUrl
                                        + UIUtils.UrlReplaceInvalidStrings(
                                            shaderFunctionNode.Function.FunctionName
                                        )
                        ...
                        else
                        {
                            ...
                            {
                                UIUtils.ShowMessage(
                                    "Selected node doesn't have valid attibutes to get URL from."
                                );
                            }
                        }
                    }
            ...
            m_shortcutManager.RegisterEditorShortcut(
                ...
                () =>
                {
                    ...
                    CommentaryNode node =
                        m_mainGraphInstance.CreateNode(m_commentaryTypeNode, true, -1, false)
                        as CommentaryNode;
        ...
        public AmplifyShaderEditorWindow()
        {
            ...
            m_nodeParametersWindow = new NodeParametersWindow(this) { Resizable = true };
            ...
            m_autoPanArea[1] = new AutoPanData(
                AutoPanLocation.BOTTOM,
                25,
                autoPanSpeed * Vector2.down
            );
            m_autoPanArea[2] = new AutoPanData(
                AutoPanLocation.LEFT,
                25,
                autoPanSpeed * Vector2.right
            );
            m_autoPanArea[3] = new AutoPanData(
                AutoPanLocation.RIGHT,
                25,
                autoPanSpeed * Vector2.left
            );
        ...
        {
            TemplateDataParent templateData = m_templatesManager.GetTemplate(
                (
            ...
            m_mainGraphInstance.ReplaceMasterNode(
                AvailableShaderTypes.Template,
                writeDefaultData,
                templateData
            );
        }
        ...
        void Init()
        {
            ...
            m_graphBgTexture =
                AssetDatabase.LoadAssetAtPath(
                    AssetDatabase.GUIDToAssetPath(IOUtils.GraphBgTextureGUID),
                    typeof(Texture2D)
                ) as Texture2D;
            if (m_graphBgTexture != null)
            {
                m_graphFgTexture =
                    AssetDatabase.LoadAssetAtPath(
                        AssetDatabase.GUIDToAssetPath(IOUtils.GraphFgTextureGUID),
                        typeof(Texture2D)
                    ) as Texture2D;
                ...
                m_wireTexture =
                    AssetDatabase.LoadAssetAtPath(
                        AssetDatabase.GUIDToAssetPath(IOUtils.WireTextureGUID),
                        typeof(Texture2D)
                    ) as Texture2D;

                m_initialized =
                    m_graphBgTexture != null && m_graphFgTexture != null && m_wireTexture != null;
            }
        }
        ...
        static bool OnOpenAsset(int instanceID, int line)
        {
            ...
            if (ASEPackageManagerHelper.IsProcessing)
            {
                ...
                else
                {
                    ...
                    {
                        if (
                            !Preferences.User.DisableMaterialMode && IOUtils.IsASEShader(mat.shader)
                        )
                        {
        ...
        static void CreateConfirmationStandardShader()
        {
            ...
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(
                0,
                endNameEditAction,
                "New Amplify Shader.shader" /*assetPathAndName*/
                ,
        ...
        static void CreateNewShader(string customPath, string customShaderName)
        {
            ...
            if (string.IsNullOrEmpty(customPath))
            {
                ...
                {
                    UnityEngine.Object[] selection = Selection.GetFiltered(
        ...
        public static Shader CreateNewTemplateShader(
            string templateGUID,
            ...
            if (string.IsNullOrEmpty(customPath))
            {
                path =
                    Selection.activeObject == null
                        ...
                        : (IOUtils.dataPath + AssetDatabase.GetAssetPath(Selection.activeObject));
                if (path.IndexOf('.') > -1)
                {
                    path = path.Substring(0, path.LastIndexOf('/'));
                }
        ...
        static void CreateNewShaderFunction()
        {
            ...
            ProjectWindowUtil.StartNameEditingIfProjectWindowExists(
                asset.GetInstanceID(),
                endNameEditAction,
                "New ShaderFunction.asset" /*assetPathAndName*/
                ,
        ...
        public void OnProjectWindowChanged()
        {
            ...
            if (selectedShader != null)
            {
                if (
                    m_mainGraphInstance != null
                    && m_mainGraphInstance.CurrentMasterNode != null
                    && selectedShader == m_mainGraphInstance.CurrentMasterNode.CurrentShader
                )
                {
                ...
                case ASESelectionMode.Shader:
                    {
                        if (ShaderIsModified)
                        {
                            ...
                            bool savePrevious = UIUtils.DisplayDialog(
                                AssetDatabase.GetAssetPath(currShader)
                            );
                ...
                case ASESelectionMode.Material:
                    {
                        if (ShaderIsModified)
                        {
                            ...
                            OnSaveShader(
                                savePrevious,
                                currShader,
                                m_mainGraphInstance.CurrentMasterNode.CurrentMaterial,
                                null
                            );
                        }
                    }
                        ...
                        if (ShaderIsModified)
                        {
                            bool savePrevious = UIUtils.DisplayDialog(
                                AssetDatabase.GetAssetPath(
                                    m_mainGraphInstance.CurrentShaderFunction
                                )
                            );
        ...
        ShaderLoadResult OnSaveShader(
            bool value,
            Shader shader,
            Material material,
            AmplifyShaderFunction function
        )
        {
            if (value)
            {
        ...
        private void FinishedShaderCompileLog(double compileTimeInSeconds)
        {
            ...
            else if (m_mainGraphInstance.CurrentShaderFunction != null)
            {
                name =
                    " \""
                    + AssetDatabase.GetAssetPath(m_mainGraphInstance.CurrentShaderFunction)
                    + "\"";
            }
            Debug.Log(
                "[AmplifyShaderEditor] Finished compiling"
                    + name
                    + " in "
                    + compileTimeInSeconds.ToString("0.00")
                    + " seconds."
            );
        }
        ...
        public bool SaveToDisk(bool checkTimestamp)
        {
            ...
            if (m_mainGraphInstance.CurrentMasterNodeId == Constants.INVALID_NODE_ID)
            {
                Shader currentShader =
                    m_mainGraphInstance.CurrentMasterNode != null
                        ? m_mainGraphInstance.CurrentMasterNode.CurrentShader
                        : null;
                string newShader;
                ...
                {
                    newShader = EditorUtility.SaveFilePanel(
                        "Select Shader to save",
                        Application.dataPath,
                        "MyShader",
                        "shader"
                    );
                    ...
                    if (IOUtils.OnShaderSavedEvent != null)
                    {
                        ...
                        if (!m_mainGraphInstance.IsStandardSurface)
                        {
                            TemplateMultiPassMasterNode masterNode =
                                m_mainGraphInstance.GetMainMasterNodeOfLOD(-1);
                            if (masterNode != null)
                            {
                                info = masterNode.CurrentTemplate.GUID;
                            }
                        }
                        IOUtils.OnShaderSavedEvent(
                            currShader,
                            !m_mainGraphInstance.IsStandardSurface,
                            info
                        );
                    }
                    succeeded = true;
            ...
            else
            {
                ...
                m_mainGraphInstance.CurrentShaderFunction.FunctionInfo = IOUtils.AddAdditionalInfo(
                    m_mainGraphInstance.CurrentShaderFunction.FunctionInfo
                );
                ...
                case ToolButtonType.Live:
                    {
                        ...
                        if (
                            m_liveShaderEditing
                            && m_mainGraphInstance.CurrentMasterNode != null
                            && m_mainGraphInstance.CurrentMasterNode.CurrentShader == null
                        )
                        {
                ...
                case ToolButtonType.FocusOnMasterNode:
                    {
                        ...
                        FocusOnNode(
                            m_mainGraphInstance.CurrentMasterNode,
                            autoZoom ? 1 : m_cameraZoom,
                            true
                        );
                    }
                    break;
                ...
                case ToolButtonType.TakeScreenshot:
                    {
                        ...
                        bool takeit = EditorUtility.DisplayDialog(
                            "Take Screenshot",
                            "This is a work in progress feature that will undock itself if needed, increase the window outside of your screen resolution to take the shot, if something fails (ie: graph too big) you may need to restart Unity, do you wish to continue?",
                            "Yes",
                            "Cancel"
                        );
                        if (!takeit)
                        ...
                        int windowLong = WindowsUtil.GetWindowLong(
                            m_aseHandle,
                            WindowsUtil.GWL_STYLE
                        );
                        ...
                        for (int i = 0; i < selectedNodes.Count; i++)
                        {
                            ...
                            maxPos.x =
                                ((currPos.x + currPos.width) > maxPos.x)
                                    ? (currPos.x + currPos.width)
                                    : maxPos.x;
                            maxPos.y =
                                ((currPos.y + currPos.height) > maxPos.y)
                        ...
                        WindowsUtil.SetWindowLong(
                            m_aseHandle,
                            WindowsUtil.GWL_STYLE,
                            (int)(windowLong & ~(WindowsUtil.WS_SIZEBOX))
                        );
                        ...
                        WindowsUtil.SetWindowPos(
                            m_aseHandle,
                            0,
                            (int)m_prevWindowRect.xMin,
                            (int)m_prevWindowRect.yMin,
                            (int)centroid.x,
                            (int)centroid.y,
                            0x0040
                        );
                        ...
                        EditorUtility.DisplayDialog(
                            "Take Screenshot",
                            "This is a work in progress feature that only works in Windows environment",
                            "Ok"
                        );
#endif
                    }
        ...
        IntPtr FindASEWindowHandle()
        {
            ...
            {
                WindowsUtil.EnumChildWindows(
                    winPtrs[i],
                    delegate(System.IntPtr hwnd, System.IntPtr param)
                    {
                        ...
                        if (
                            Title
                                .ToString()
                                .Contains("AmplifyShaderEditor.AmplifyShaderEditorWindow")
                        )
                        {
                            if (!found)
                            {
        ...
        void TakeScreenShot()
        {
            ...
            Texture2D m_screenshotTex2D = new Texture2D(
                (int)position.width,
                (int)position.height,
                TextureFormat.RGB24,
                false
            );
            m_screenshotTex2D.ReadPixels(
                new Rect(0, 0, m_screenshotTex2D.width, m_screenshotTex2D.height),
                0,
                0
            );
            ...
            ShowMessage(
                "[AmplifyShaderEditor] Screenshot successfully taken and saved at: " + path,
                consoleLog: true
            );
            ...
            WindowsUtil.SetWindowPos(
                ...
                (int)m_prevWindowRect.xMin,
                (int)m_prevWindowRect.yMin,
                (int)m_prevWindowRect.width,
                (int)m_prevWindowRect.height,
                0x0040
            );
        ...
        {
            List<ParentNode> selectedNodes =
                (m_mainGraphInstance.SelectedNodes.Count > 0) && !forceAllNodes
                    ? m_mainGraphInstance.SelectedNodes
                    : m_mainGraphInstance.AllNodes;
            ...
            if (autoZoom)
            {
                ...
                if (centroid.x > canvasWidth || centroid.y > canvasHeight)
                {
        ...
        public void FocusOnNode(ParentNode node, float zoom, bool selectNode, bool late = false)
        {
            ...
            nodePoint.x =
                nodePoint.x
                - (m_nodeParametersWindow.RealWidth * 0.5f + m_paletteWindow.RealWidth * 0.5f)
                    * (zoom > 0.999f ? zoom : CameraZoom);
        ...
        public void FocusOnPoint(Vector2 point, float zoom, bool smooth = true)
        {
            ...
            if (smooth)
                SmoothCameraOffset(
                    -point
                        + new Vector2((m_cameraInfo.width) * 0.5f, m_cameraInfo.height * 0.5f)
                            * CameraZoom
                );
            else
                m_cameraOffset =
        ...
        void PreTestLeftMouseDown()
        {
            if (
                ...
                && m_currentEvent.button == ButtonClickId.LeftMouseButton
            )
            {
        ...
        void OnLeftMouseDown()
        {
            ...
            ParentNode node =
                (m_mainGraphInstance.NodeClicked < 0)
                    ? m_mainGraphInstance.CheckNodeAt(m_currentMousePos)
                    : m_mainGraphInstance.GetClickedNode();
            if (node != null)
            {
                m_mainGraphInstance.NodeClicked = node.UniqueId;
                ...
                else
                {
                    if (node.OnClick(m_currentMousePos2D))
                    {
                        if (!node.Selected)
                        {
                            m_mainGraphInstance.SelectNode(
                                node,
                                (
                                    m_currentEvent.modifiers == EventModifiers.Shift
                                    || m_currentEvent.modifiers == EventModifiers.Control
                                ),
                                true
                            );
                        }
                        ...
                        if (m_currentEvent.alt)
                        {
                            ...
                            if (
                                ...
                                && node.OutputPorts.Count > 0
                                && conn > 0
                                && node.OutputPorts[0].IsConnected
                            )
                            {
            ...
            else if (!m_multipleSelectionActive)
            {
                ...
                {
                    List<WireBezierReference> wireRefs = m_mainGraphInstance.GetWireBezierListInPos(
                        m_currentMousePos2D
                    );
                    if (wireRefs != null && wireRefs.Count > 0)
                    {
                        for (int i = 0; i < wireRefs.Count; i++)
                        {
                            ...
                            OutputPort outputPort = outNode.GetOutputPortByUniqueId(
                                wireRefs[i].OutPortId
                            );
                            InputPort inputPort = inNode.GetInputPortByUniqueId(
                                wireRefs[i].InPortId
                            );

                            // Calculate the 4 points for bezier taking into account wire nodes and their automatic tangents
                            Vector3 endPos = new Vector3(
                                inputPort.Position.x,
                                inputPort.Position.y
                            );
                            Vector3 startPos = new Vector3(
                                outputPort.Position.x,
                                outputPort.Position.y
                            );

                            float mag = (endPos - startPos).magnitude;
                            float resizedMag = Mathf.Min(
                                mag,
                                Constants.HORIZONTAL_TANGENT_SIZE * m_drawInfo.InvertedZoom
                            );
                            ...
                            if (inNode != null && inNode.GetType() == typeof(WireNode))
                                endTangent =
                                    endPos + ((inNode as WireNode).TangentDirection) * mag * 0.33f;

                            if (outNode != null && outNode.GetType() == typeof(WireNode))
                                startTangent =
                                    startPos
                                    - ((outNode as WireNode).TangentDirection) * mag * 0.33f;
                            ...
                            if (dist < 10)
                            {
                                ...
                                bool doubleTap =
                                    (doubleTapTime - m_wiredDoubleTapTimestamp)
                                    < WiredDoubleTapTime;
                                m_wiredDoubleTapTimestamp = doubleTapTime;

                                if (doubleTap)
                                {
                                    UndoUtils.RegisterCompleteObjectUndo(
                                        this,
                                        Constants.UndoCreateConnectionId
                                    );
                                    UndoUtils.RegisterCompleteObjectUndo(
                                        m_mainGraphInstance,
                                        Constants.UndoCreateConnectionId
                                    );
                                    UndoUtils.RecordObject(
                                        outNode,
                                        Constants.UndoCreateConnectionId
                                    );
                                    UndoUtils.RecordObject(
                                    ...
                                    ParentNode wireNode = m_mainGraphInstance.CreateNode(
                                        typeof(WireNode),
                                        true
                                    );
                                    if (wireNode != null)
                                    {
                                        ...
                                        m_mainGraphInstance.CreateConnection(
                                            wireNode.InputPorts[0].NodeId,
                                            wireNode.InputPorts[0].PortId,
                                            outputPort.NodeId,
                                            outputPort.PortId
                                        );
                                        m_mainGraphInstance.CreateConnection(
                                            inputPort.NodeId,
                                            inputPort.PortId,
                                            wireNode.OutputPorts[0].NodeId,
                                            wireNode.OutputPorts[0].PortId
                                        );
            ...
            if (
                !m_contextMenu.CheckShortcutKey()
                    && m_currentEvent.modifiers != EventModifiers.Shift
                    && m_currentEvent.modifiers != EventModifiers.Control
                || m_altBoxSelection
            )
        ...
        void OnLeftMouseDrag()
        {
            ...
            if (m_lastKeyPressed == KeyCode.Q)
            {
                ...
                {
                    ModifyZoom(
                        Constants.ALT_CAMERA_ZOOM_SPEED
                            * (m_currentEvent.delta.x + m_currentEvent.delta.y),
                        m_altKeyStartPos
                    );
                }
                else
            ...
            if (m_altDragStarted)
            {
                ...
                if (
                    m_currentEvent.modifiers == EventModifiers.Alt
                    && CurrentGraph.SelectedNodes.Count == 1
                )
                {
            ...
            if (!m_wireReferenceUtils.ValidReferences() && !m_altBoxSelection)
            {
                if (m_mouseDownOnValidArea && m_insideEditorWindow)
                {
                    ...
                    {
                        m_mainGraphInstance.MoveSelectedNodes(
                            m_cameraZoom * m_currentEvent.delta,
                            true
                        );
                    }
                    else
            ...
            {
                List<ParentNode> nodes = m_mainGraphInstance.GetNodesInGrid(
                    m_drawInfo.TransformedMousePos
                );
                if (nodes != null && nodes.Count > 0)
                {
                    ...
                    if (m_wireReferenceUtils.InputPortReference.IsValid)
                    {
                            ...
                            if (outputPorts != null)
                            {
                                for (int o = 0; o < outputPorts.Count; o++)
                                {
                                    if (outputPorts[o].Available)
                                    {
                                        ...
                                        currentPortPos =
                                            currentPortPos * m_cameraZoom - m_cameraOffset;
                        ...
                        if (
                            ...
                            && (
                                smallestDistance < Constants.SNAP_SQR_DIST
                                || currentPort.InsideActiveArea(
                                    (mousePos + m_cameraOffset) / m_cameraZoom
                                )
                            )
                        )
                        {
        ...
        public void OnLeftMouseUp()
        {
            ...
            if (m_multipleSelectionActive)
            {
                ...
                {
                    m_mainGraphInstance.MultipleSelection(
                        m_multipleSelectionArea,
                        !m_currentEvent.shift
                    );
                }
                else
                {
            ...
            if (m_wireReferenceUtils.ValidReferences())
            {
                ...
                ParentNode targetNode = m_wireReferenceUtils.SnapEnabled
                    ? m_mainGraphInstance.GetNode(m_wireReferenceUtils.SnapPort.NodeId)
                    : m_mainGraphInstance.CheckNodeAt(m_currentMousePos);
                if (targetNode != null && targetNode.ConnStatus != NodeConnectionStatus.Island)
                {
                    if (
                        m_wireReferenceUtils.InputPortReference.IsValid
                        && m_wireReferenceUtils.InputPortReference.NodeId != targetNode.UniqueId
                    )
                    {
                        OutputPort outputPort = m_wireReferenceUtils.SnapEnabled
                            ? targetNode.GetOutputPortByUniqueId(
                                m_wireReferenceUtils.SnapPort.PortId
                            )
                            : targetNode.CheckOutputPortAt(m_currentMousePos);
                        if (
                            outputPort != null
                            && !outputPort.Locked
                            && (
                                !m_wireReferenceUtils.InputPortReference.TypeLocked
                                || m_wireReferenceUtils.InputPortReference.DataType
                        ...
                        {
                            ParentNode originNode = m_mainGraphInstance.GetNode(
                                m_wireReferenceUtils.InputPortReference.NodeId
                            );
                            InputPort inputPort = originNode.GetInputPortByUniqueId(
                                m_wireReferenceUtils.InputPortReference.PortId
                            );
                            UIUtils.MarkUndoAction();
                            UndoUtils.RegisterCompleteObjectUndo(
                                this,
                            ...
                            if (inputPort.NotFreeForAllTypes && outputPort.NotFreeForAllTypes)
                            {
                                ...
                                {
                                    UIUtils.ShowIncompatiblePortMessage(
                                        true,
                                        originNode,
                                        inputPort,
                                        targetNode,
                            ...
                            if (
                                ...
                                targetNode.OnOutputPortConnected(
                                    outputPort.PortId,
                                    m_wireReferenceUtils.InputPortReference.NodeId,
                                    m_wireReferenceUtils.InputPortReference.PortId
                                );
                            ...
                            if (
                                ...
                                originNode.OnInputPortConnected(
                                    m_wireReferenceUtils.InputPortReference.PortId,
                                    targetNode.UniqueId,
                                    outputPort.PortId
                                );
                        ...
                        {
                            ShowMessage(
                                "Attempting to connect a port locked to type "
                                    + m_wireReferenceUtils.InputPortReference.DataType
                                    + " into a port of type "
                                    + outputPort.DataType
                            );
                    ...
                    if (
                        m_wireReferenceUtils.OutputPortReference.IsValid
                        && m_wireReferenceUtils.OutputPortReference.NodeId != targetNode.UniqueId
                    )
                    {
                        InputPort inputPort = m_wireReferenceUtils.SnapEnabled
                            ? targetNode.GetInputPortByUniqueId(
                                m_wireReferenceUtils.SnapPort.PortId
                            )
                            : targetNode.CheckInputPortAt(m_currentMousePos);
                        if (
                            inputPort != null
                            && !inputPort.Locked
                            && (
                                !inputPort.TypeLocked
                                || inputPort.DataType == WirePortDataType.OBJECT
                                || (
                                    inputPort.TypeLocked
                                    && inputPort.DataType
                                        == m_wireReferenceUtils.OutputPortReference.DataType
                                )
                            )
                        )
                        {
                            ParentNode originNode = m_mainGraphInstance.GetNode(
                                m_wireReferenceUtils.OutputPortReference.NodeId
                            );
                            OutputPort outputPort = originNode.GetOutputPortByUniqueId(
                                m_wireReferenceUtils.OutputPortReference.PortId
                            );
                            ...
                            {
                                if (
                                    m_currentEvent.control
                                    && m_wireReferenceUtils.SwitchPortReference.IsValid
                                )
                                {
                                    ParentNode oldOutputNode = UIUtils.GetNode(
                                        inputPort.GetConnection(0).NodeId
                                    );
                                    OutputPort oldOutputPort =
                                        oldOutputNode.GetOutputPortByUniqueId(
                                            inputPort.GetConnection(0).PortId
                                        );

                                    ParentNode switchNode = UIUtils.GetNode(
                                        m_wireReferenceUtils.SwitchPortReference.NodeId
                                    );
                                    InputPort switchPort = switchNode.GetInputPortByUniqueId(
                                        m_wireReferenceUtils.SwitchPortReference.PortId
                                    );

                                    switchPort.DummyAdd(oldOutputPort.NodeId, oldOutputPort.PortId);
                                    oldOutputPort.DummyAdd(switchPort.NodeId, switchPort.PortId);
                                    if (
                                        UIUtils.DetectNodeLoopsFrom(
                                            switchNode,
                                    ...
                                    ConnectInputToOutput(
                                        switchPort.NodeId,
                                        switchPort.PortId,
                                        oldOutputPort.NodeId,
                                        oldOutputPort.PortId
                                    );
                                }
                                else
                                {
                            ...
                            if (
                                ...
                                targetNode.OnInputPortConnected(
                                    inputPort.PortId,
                                    m_wireReferenceUtils.OutputPortReference.NodeId,
                                    m_wireReferenceUtils.OutputPortReference.PortId
                                );
                            ...
                            if (
                                ...
                                originNode.OnOutputPortConnected(
                                    m_wireReferenceUtils.OutputPortReference.PortId,
                                    targetNode.UniqueId,
                                    inputPort.PortId
                                );
            ...
            )
            {
                ...
                if (wireRefs != null && wireRefs.Count > 0)
                {
                    ...
                    for (int i = 0; i < wireRefs.Count; i++)
                    {
                        ...
                        if (
                            outNode == CurrentGraph.SelectedNodes[0]
                            || inNode == CurrentGraph.SelectedNodes[0]
                        )
                            continue;
                        ...
                        if (outNode != null && outNode.GetType() == typeof(WireNode))
                            startTangent =
                                startPos - ((outNode as WireNode).TangentDirection) * mag * 0.33f;

                        //Vector2 pos = ( CurrentGraph.SelectedNodes[0].CenterPosition + m_cameraOffset ) / m_cameraZoom;

                        float dist = HandleUtility.DistancePointBezier( /*pos*/
                            m_currentMousePos,
                            startPos,
                            endPos,
                            startTangent,
                    ...
                    if (closestDist < 40)
                    {
                        ParentNode outNode = m_mainGraphInstance.GetNode(
                            wireRefs[closestId].OutNodeId
                        );
                        ParentNode inNode = m_mainGraphInstance.GetNode(
                            wireRefs[closestId].InNodeId
                        );

                        OutputPort outputPort = outNode.GetOutputPortByUniqueId(
                            wireRefs[closestId].OutPortId
                        );
                        InputPort inputPort = inNode.GetInputPortByUniqueId(
                            wireRefs[closestId].InPortId
                        );
                        ...
                        if (selectedNode.InputPorts.Count > 0 && selectedNode.OutputPorts.Count > 0)
                        {
                            ...
                            m_mainGraphInstance.CreateConnection(
                                ...
                                selectedNode.InputPorts[0].PortId,
                                outputPort.NodeId,
                                outputPort.PortId
                            );
                            m_mainGraphInstance.CreateConnection(
                                ...
                                inputPort.PortId,
                                selectedNode.UniqueId,
                                selectedNode.OutputPorts[0].PortId
                            );
                        }
        ...
        public void OnValidObjectsDropped(UnityEngine.Object[] droppedObjs)
        {
            ...
            if (droppedObjs.Length == 1)
            {
                ...
                if (newShader == null)
                {
                    ...
                    {
                        if (
                            UIUtils.IsUnityNativeShader(
                                AssetDatabase.GetAssetPath(newMaterial.shader)
                            )
                        )
                        {
                            return;
                ...
                if (propagateDraggedObjsToNode)
                {
                    ...
                    else
                    {
                        ...
                        ParentNode newNode = m_contextMenu.CreateNodeFromCastType(
                            droppedObjs[0].GetType()
                        );
                        if (newNode)
                        {
                            newNode.ContainerGraph = m_mainGraphInstance;
                            ...
                            bool find = false;
                            if (
                                newNode is FunctionNode
                                && CurrentGraph.CurrentShaderFunction != null
                            )
                                find = SearchFunctionNodeRecursively(
                            ...
                            if (find)
                            {
                                ...
                                ShowMessage(
                                    "Shader Function loop detected, new node was removed to prevent errors."
                                );
                            }
                        }
                    }
        ...
        public ShaderLoadResult LoadDroppedObject(
            bool value,
            Shader shader,
            Material material,
            AmplifyShaderFunction shaderFunction = null
        )
        {
            UIUtils.CurrentWindow = this;
            ...
            if (shaderFunction != null)
            {
                ...
                UIUtils.LatestOpenedFolder = latestOpenedFolder.Substring(
                    0,
                    latestOpenedFolder.LastIndexOf('/') + 1
                );
            ...
            else if (value && shader != null)
            {
                ...
                switch (result)
                {
                    ...
                    case ShaderLoadResult.ASE_INFO_NOT_FOUND:
                        {
                            ShowMessage(
                                "Loaded shader wasn't created with ASE. Saving it will remove previous data."
                            );
                            UIUtils.CreateEmptyFromInvalid(shader);
                        }
                        break;
        ...
        {
            m_mainGraphInstance.DeleteConnection(
                isInput,
                nodeId,
                portId,
                registerOnLog,
        ...
        void DeleteSelectedNodes()
        {
            ...
            for (int selectedNodeIdx = 0; selectedNodeIdx < selectedNodes.Length; selectedNodeIdx++)
            {
                // Check inputs
                {
                    ...
                    if (inputIdxCount > 0)
                    {
                        for (int inputIdx = 0; inputIdx < inputIdxCount; inputIdx++)
                        {
                            ...
                            {
                                int nodeIdx = selectedNodes[selectedNodeIdx]
                                    .InputPorts[inputIdx]
                ...
                if (outputIdxCount > 0)
                {
                    ...
                    {
                        int inputIdxCount = selectedNodes[selectedNodeIdx]
                            .OutputPorts[outputIdx]
                            .ExternalReferences
                            .Count;
                        if (inputIdxCount > 0)
                        {
            ...
            for (int i = 0; i < selectedNodes.Length; i++)
            {
                ...
                m_mainGraphInstance.DeleteAllConnectionFromNode(
                    selectedNodes[i],
                    false,
                    true,
                    true
        ...
        void OnKeyboardUp()
        {
            ...
            if (
                m_shortcutManager.ActivateShortcut(
                    m_currentEvent.modifiers,
                    m_lastKeyPressed,
                    false
                )
            )
        ...
        void OnKeyboardDown()
        {
            ...
            if (
                m_currentEvent.control
                && m_currentEvent.shift
                && m_currentEvent.keyCode == KeyCode.V
            )
            {
            ...
            if (
                !m_altPressDown
                && (
                    OnKeyboardPress(KeyCode.LeftAlt)
                    || OnKeyboardPress(KeyCode.RightAlt)
                    || OnKeyboardPress(KeyCode.AltGr)
                )
            )
            {
                m_altPressDown = true;
            ...
            if (
                m_currentEvent.keyCode != KeyCode.None
                && m_currentEvent.modifiers == EventModifiers.None
            )
            {
        ...
        IEnumerator SendPostCoroutine(string url)
        {
            ...
            form.AddField(
                ...
                Clipboard.ClipboardId
                    + ";"
                    + EditorPrefs.GetString(Clipboard.ClipboardId, string.Empty)
            );
            ...
            if (!m_coroutine.MoveNext() && www != null)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            System.Text.RegularExpressions.Regex parser =
                                new System.Text.RegularExpressions.Regex(
                                    @".*(http:\/\/paste.amplify.pt\/view\/)([0-9a-z]*).*",
                            ...
                            Debug.Log(
                                "[AmplifyShaderEditor] Link copied to clipboard\n" + finalURL + "\n"
                            );
                        ...
                        {
                            Debug.Log(
                                "[AmplifyShaderEditor] Failed to generate paste:\n" + finalURL
                            );
                        }
                    }
                    EditorApplication.update -= PasteRequest;
        ...
        {
            float minCam = Mathf.Min(
                (
                    m_cameraInfo.width
                    - (m_nodeParametersWindow.RealWidth + m_paletteWindow.RealWidth)
                ),
                (m_cameraInfo.height - (m_toolsWindow.Height))
            );
            if (minCam < 1)
                minCam = 1;
            ...
            if (zoomIncrement < 0)
                CameraZoom = Mathf.Max(
                    m_cameraZoom + zoomIncrement * Constants.CAMERA_ZOOM_SPEED,
        ...
        void ModifyZoomSmooth(float zoomIncrement, Vector2 pivot)
        {
            ...
            if (m_targetZoomIncrement < 0)
                m_targetZoom = Mathf.Max(
                    m_cameraZoom + m_targetZoomIncrement * Constants.CAMERA_ZOOM_SPEED,
                    Constants.CAMERA_MIN_ZOOM
                );
            else if (CameraZoom < Mathf.Max(Constants.CAMERA_MAX_ZOOM, dynamicMaxZoom))
        ...
        void CopyToClipboard()
        {
            ...
            m_clipboard.AddToClipboard(
                m_mainGraphInstance.SelectedNodes,
                m_copyPasteInitialPos,
                m_mainGraphInstance
            );
        }
        ...
        {
            string[] parameters = m_clipboard
                .CurrentClipboardStrData[clipId]
                .Data.Split(IOUtils.FIELD_SEPARATOR);
            ...
            if (
                attributes != null
                && !UIUtils.GetNodeAvailabilityInBitArray(
                    attributes.NodeAvailabilityFlags,
                    m_mainGraphInstance.CurrentCanvasMode
                )
                && !UIUtils.GetNodeAvailabilityInBitArray(
                    attributes.NodeAvailabilityFlags,
                    m_currentNodeAvailability
                )
            )
                return null;
        ...
        void CreateConnectionsFromClipboardData(int clipId)
        {
            if (String.IsNullOrEmpty(m_clipboard.CurrentClipboardStrData[clipId].Connections))
                return;
            string[] lines = m_clipboard
                .CurrentClipboardStrData[clipId]
                .Connections.Split(IOUtils.LINE_TERMINATOR);
            ...
            if (!m_coroutine.MoveNext())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            data = www.downloadHandler.text.Replace(
                                Clipboard.ClipboardId + ";",
                                ""
                            );
                        ...
                        {
                            Debug.Log(
                                "[AmplifyShaderEditor] Error downloading, snippet might not exist anymore, clearing clipboard..."
                            );
                            EditorGUIUtility.systemCopyBuffer = string.Empty;
                        }
                    }
        ...
        void PasteFromClipboard(bool copyConnections)
        {
            ...
            for (int i = 0; i < m_clipboard.CurrentClipboardStrData.Count; i++)
            {
                ...
                if (node != null)
                {
                    ...
                    node.Vec2Position =
                        pos + deltaPos + m_copyPasteDeltaMul * Constants.CopyPasteDeltaPos;
        ...
        public string GenerateGraphInfo()
        {
            EditorPrefs.SetString(
                GUID,
                m_cameraInfo.x.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraInfo.y.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraInfo.width.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraInfo.height.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraOffset.x.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraOffset.y.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_cameraZoom.ToString()
                    + IOUtils.FIELD_SEPARATOR
                    + m_nodeParametersWindow.IsMaximized
                    + IOUtils.FIELD_SEPARATOR
        ...
        public static void LoadFromMeta(
            ref ParentGraph graph,
        ...
        )
        {
            ...
            if (checksumId > -1)
            {
                ...
                if (typeValuePair != null && typeValuePair.Length == 2)
                {
                    ...
                    if (
                        DebugConsoleWindow.DeveloperMode
                        && !currentChecksumValue.Equals(typeValuePair[1])
                    )
                    {
                    ...
                    if (shaderBodyId > -1)
                    {
                        ...
                        )
                        {
                            ...
                            string[] parameters = instructions[instructionIdx]
                                .Split(IOUtils.FIELD_SEPARATOR);
                                ...
                                case IOUtils.NodeParam:
                                    {
                                        ...
                                        typeStr = IOUtils.NodeTypeReplacer.ContainsKey(typeStr)
                                            ? IOUtils.NodeTypeReplacer[typeStr]
                                            : typeStr;
                                        ...
                                        if (type != null)
                                        {
                                            ...
                                            NodeAttributes attribs =
                                                contextMenu.GetNodeAttributesForType(type);
                                            if (attribs == null)
                                            {
                                                attribs =
                                                    contextMenu.GetDeprecatedNodeAttributesForType(
                                                        type
                                                    );
                                                if (attribs != null)
                                                {
                                                    if (
                                                        attribs.Deprecated
                                                        && attribs.DeprecatedAlternativeType != null
                                                    )
                                                    {
                                            ...
                                            if (newNode != null)
                                            {
                                                try
                                                {
                                                    ...
                                                    if (oldType != type)
                                                    {
                                                        newNode.ParentReadFromString(
                                                            ref parameters
                                                        );
                                                        newNode.ReadFromDeprecated(
                                                            ref parameters,
                                                            oldType
                                                        );
                                                    ...
                                                    if (oldType == type)
                                                    {
                                                        newNode.ReadInputDataFromString(
                                                            ref parameters
                                                        );
                                                        if (UIUtils.CurrentShaderVersion() > 5107)
                                                        {
                                                            newNode.ReadOutputDataFromString(
                                                                ref parameters
                                                            );
                                                        }
                                                    }
                                        ...
                                        else
                                        {
                                            UIUtils.ShowMessage(
                                                string.Format(
                                                    "{0} is not a valid ASE node ",
                                                    parameters[IOUtils.NodeTypeId]
                                                ),
                                ...
                                case IOUtils.WireConnectionParam:
                                    {
                                        ...
                                        try
                                        {
                                            InNodeId = Convert.ToInt32(
                                                parameters[IOUtils.InNodeId]
                                            );
                                            InPortId = Convert.ToInt32(
                                                parameters[IOUtils.InPortId]
                                            );
                                            OutNodeId = Convert.ToInt32(
                                                parameters[IOUtils.OutNodeId]
                                            );
                                            OutPortId = Convert.ToInt32(
                                                parameters[IOUtils.OutPortId]
                                            );
                                        }
                                        catch (Exception e)
                                        {
                                        ...
                                        if (inNode != null && outNode != null)
                                        {
                                            if (UIUtils.CurrentShaderVersion() < 5002)
                                            {
                                                InPortId = inNode.VersionConvertInputPortId(
                                                    InPortId
                                                );
                                                OutPortId = outNode.VersionConvertOutputPortId(
                                                    OutPortId
                                                );

                                                if (inNode.WasDeprecated)
                                                    InPortId = inNode.InputIdFromDeprecated(
                                                        InPortId
                                                    );
                                                if (outNode.WasDeprecated)
                                                    OutPortId = outNode.OutputIdFromDeprecated(
                                                        OutPortId
                                                    );

                                                inputPort = inNode.GetInputPortByArrayId(InPortId);
                                                outputPort = outNode.GetOutputPortByArrayId(
                                                    OutPortId
                                                );
                                            }
                                            else
                                            {
                                                ...
                                                inputPort = inNode.GetInputPortByUniqueId(InPortId);
                                                outputPort = outNode.GetOutputPortByUniqueId(
                                                    OutPortId
                                                );
                                            }
                                            ...
                                            if (inputPort != null && outputPort != null)
                                            {
                                                bool inputCompatible = inputPort.CheckValidType(
                                                    outputPort.DataType
                                                );
                                                bool outputCompatible = outputPort.CheckValidType(
                                                    inputPort.DataType
                                                );
                                                if (inputCompatible && outputCompatible)
                                                {
                                                    ...
                                                    inNode.OnInputPortConnected(
                                                        InPortId,
                                                        OutNodeId,
                                                        OutPortId,
                                                        false
                                                    );
                                                    outNode.OnOutputPortConnected(
                                                        OutPortId,
                                                        InNodeId,
                                                        InPortId
                                                    );
                                        ...
                                        else if (DebugConsoleWindow.DeveloperMode)
                                        {
                                            ...
                                            {
                                                UIUtils.ShowMessage(
                                                    "Input node " + InNodeId + " doesn't exist",
                                                    MessageSeverity.Error
                                                );
                                            }
                                            else
                                            {
                                                UIUtils.ShowMessage(
                                                    "Output node " + OutNodeId + " doesn't exist",
                                                    MessageSeverity.Error
                                                );
                                            }
                                        }
        ...
        public ShaderLoadResult LoadFromDisk(
            string pathname,
            AmplifyShaderFunction shaderFunction = null
        )
        {
                ...
                if (typeValuePair != null && typeValuePair.Length == 2)
                {
                    ...
                    if (shaderBodyId > -1)
                    {
                        ...
                        if (versionParams.Length == 2)
                        {
                            ...
                            {
                                ShowMessage(
                                    "This shader was created on a new ASE version\nPlease install v."
                                        + version
                                );
                            }
                            ...
                            if (DebugConsoleWindow.DeveloperMode)
                            {
                                ...
                                {
                                    ShowMessage(
                                        "This shader was created on a older ASE version\nSaving will update it to the new one."
                                    );
                                }
                            }
                                ...
                                case IOUtils.NodeParam:
                                    {
                                        ...
                                        if (type != null)
                                        {
                                            System.Type oldType = type;
                                            NodeAttributes attribs =
                                                m_contextMenu.GetNodeAttributesForType(type);
                                            if (attribs == null)
                                            {
                                                attribs =
                                                    m_contextMenu.GetDeprecatedNodeAttributesForType(
                                                        type
                                                    );
                                                if (attribs != null)
                                                {
                                                    ...
                                                    {
                                                        if (
                                                            attribs.DeprecatedAlternativeType
                                                            != null
                                                        )
                                                        {
                                                            type =
                                                            ...
                                                            ShowMessage(
                                                                string.Format(
                                                                    "Node {0} is deprecated and was replaced by {1} ",
                                                                    attribs.Name,
                                                        ...
                                                        else
                                                        {
                                                            if (
                                                                ...
                                                                ShowMessage(
                                                                    string.Format(
                                                                        Constants.DeprecatedNoAlternativeMessageStr,
                                                                        attribs.Name,
                                                                        attribs.DeprecatedAlternative
                                                                    ),
                                                                ...
                                                                ShowMessage(
                                                                    string.Format(
                                                                        Constants.DeprecatedMessageStr,
                                                                        attribs.Name,
                                                                        attribs.DeprecatedAlternative
                                                                    ),
                                                                    MessageSeverity.Normal,
            ...
            if (shaderFunction != null || shader != null)
            {
                cameraParams =
                    (cameraParams.Length != 0)
                        ? cameraParams
                        : EditorPrefs.GetString(GUID, "").Split(IOUtils.FIELD_SEPARATOR);

                if (cameraParams.Length == 9)
                {
                    try
                    {
                        var cameraInfo = new Rect(
                            Convert.ToSingle(cameraParams[0]),
                            Convert.ToSingle(cameraParams[1]),
                            Convert.ToSingle(cameraParams[2]),
                            Convert.ToSingle(cameraParams[3])
                        );
                        var cameraOffset = new Vector2(
                            Convert.ToSingle(cameraParams[4]),
                            Convert.ToSingle(cameraParams[5])
                        );
                        float cameraZoom = Convert.ToSingle(cameraParams[6]);

                        float centerWidth =
                            (this.position.width - cameraInfo.width) * 0.5f * cameraZoom;
                        float centerHeight =
                            (this.position.height - cameraInfo.height) * 0.5f * cameraZoom;
        ...
        public void ShowMessageImmediately(
            string message,
            MessageSeverity severity = MessageSeverity.Normal,
        ...
        void OnGUI()
        {
            ...
            if (
                m_currentEvent.type == EventType.ExecuteCommand
                || m_currentEvent.type == EventType.ValidateCommand
            )
            ...
            if (
                m_nodeParametersWindow != null
                && m_innerEditorVariables.NodeParametersMaximized
                    != m_nodeParametersWindow.IsMaximized
            )
            ...
            if (m_forcingMaterialUpdateFlag)
            {
                ...
                if (m_materialsToUpdate.Count > 0)
                {
                    float percentage =
                        100.0f
                        * (float)(UIUtils.TotalExampleMaterials - m_materialsToUpdate.Count)
                        / (float)UIUtils.TotalExampleMaterials;
                    if (m_forcingMaterialUpdateOp) // Read
                    {
                        Debug.Log(percentage + "% Recompiling " + m_materialsToUpdate[0].name);
                        LoadDroppedObject(
                            true,
                            m_materialsToUpdate[0].shader,
                            m_materialsToUpdate[0]
                        );
                    }
                    else // Write
                    {
            ...
            m_autoPanDirActive =
                ...
                || m_forceAutoPanDir
                || m_multipleSelectionActive
                || m_wireReferenceUtils.ValidReferences();
            ...
            //GUILayout.BeginArea( m_graphArea, "Nodes" );
            {
                ...
                GUI.DrawTextureWithTexCoords(
                    ...
                    m_graphBgTexture,
                    new Rect(
                        (-m_cameraOffset.x / m_graphBgTexture.width),
                        (m_cameraOffset.y / m_graphBgTexture.height)
            ...
            if (
                InsideMenus(m_currentMousePos2D) /*|| _confirmationWindow.IsActive*/
            )
            {
                ...
                if (
                    m_currentEvent.type == EventType.MouseUp
                    && m_currentEvent.button == ButtonClickId.LeftMouseButton
                )
                {
                    m_lmbPressed = false;
            ...
            //GUILayout.BeginArea( m_graphArea );
            {
                ...
                m_modeWindow.Draw(
                    m_graphArea,
                    m_currentMousePos2D,
                    m_mainGraphInstance.CurrentShader,
                    currentMaterial,
                    0.5f
                        * (
                            m_graphArea.width
                            - m_paletteWindow.RealWidth
                            - m_nodeParametersWindow.RealWidth
                        ),
                ...
                m_toolsWindow.SetStateOnButton(
                    ToolButtonType.CleanUnusedNodes,
                    hasUnusedConnNodes ? 1 : 0
                );
                ...
                if (masterNode != null)
                {
                    m_toolsWindow.DrawShaderTitle(
                        m_nodeParametersWindow,
                        m_paletteWindow,
                        AvailableCanvasWidth,
                        m_graphArea.height,
                        masterNode.CroppedShaderName
                    );
                }
            ...
            m_toolsWindow.Width =
                m_cameraInfo.width - (m_nodeParametersWindow.RealWidth + m_paletteWindow.RealWidth);
            ...
            if (
                position.width < m_lastWindowWidth
                && position.width < Constants.MINIMIZE_WINDOW_LOCK_SIZE
            )
            {
                autoMinimize = true;
            }
            ...
            ParentNode selectedNode =
                (m_mainGraphInstance.SelectedNodes.Count == 1)
                    ? m_mainGraphInstance.SelectedNodes[0]
                    : m_mainGraphInstance.CurrentMasterNode;
            m_repaintIsDirty =
                m_nodeParametersWindow.Draw(
                    ...
                    selectedNode,
                    m_currentMousePos2D,
                    m_currentEvent.button,
                    false
                ) || m_repaintIsDirty; //TODO: If multiple nodes from the same type are selected also show a parameters window which modifies all of them
            ...
            m_paletteWindow.Draw(
                m_cameraInfo,
                m_currentMousePos2D,
                m_currentEvent.button,
                !m_contextPalette.IsActive
            );
            ...
            if (m_palettePopup.IsActive)
            {
                ...
                if (m_currentEvent.GetTypeForControl(controlID) == EventType.MouseUp)
                {
                    if (m_currentEvent.button == ButtonClickId.LeftMouseButton)
                    {
                        ...
                        if (!InsideMenus(m_currentMousePos2D))
                        {
                            ParentNode newNode = CreateNode(
                                m_paletteChosenType,
                                TranformedMousePos,
                                m_paletteChosenFunction
                            );
            ...
            if (m_multipleSelectionActive)
            {
                ...
                transformedArea.position =
                    (transformedArea.position + m_cameraOffset) / m_cameraZoom;
            ...
            {
                m_autoPanArea[(int)AutoPanLocation.LEFT].AdjustInitialX =
                    m_nodeParametersWindow.IsMaximized ? m_nodeParametersWindow.RealWidth : 0;
                m_autoPanArea[(int)AutoPanLocation.RIGHT].AdjustInitialX =
                    m_paletteWindow.IsMaximized ? -m_paletteWindow.RealWidth : 0;
            ...
            if (m_liveShaderEditing)
            {
                if (m_saveIsDirty)
                {
                    if (
                        focusedWindow == this
                        && m_currentInactiveTime > InactivitySaveTime
                        && !(
                    ...
                    case OpenSaveState.CLOSE:
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Debug.Log(
                                        "[AmplifyShaderEditor] Finished batch compilation in "
                                            + m_batchTimer.Elapsed.TotalSeconds.ToString("0.00")
                                            + " seconds."
                                    );
                                }
                            }
        ...
        void OnInspectorUpdate()
        {
            ...
            if (
                (IsShaderFunctionWindow && CurrentGraph.CurrentShaderFunction == null)
                || m_markToClose
            )
            {
                Close();
        ...
        public void SetCtrlSCallback(bool imediate)
        {
            ...
            if ( /*node != null && node.CurrentShader != null && */
                m_shaderIsModified
            )
            {
                if (imediate)
        ...
        public void OnPaletteNodeCreate(
            System.Type type,
            string name,
            AmplifyShaderFunction function
        )
        ...
        public void OnContextPaletteNodeCreate(
            System.Type type,
            string name,
            AmplifyShaderFunction function
        )
        {
            ...
            ParentNode newNode = CreateNode(
                type,
                m_contextPalette.StartDropPosition * m_cameraZoom - m_cameraOffset,
                function
            );
            ...
            if (masterNode != null)
            {
                if (masterNode.CurrentMaterial)
                {
                    m_toolsWindow.SetStateOnButton(
                        ...
                        ShaderIsModified ? 0 : 2,
                        ShaderIsModified ? ShaderIsModifiedMessage : ShaderIsNotModified
                    );
                }
                else
                {
        ...
        {
            m_toolsWindow.SetStateOnButton(
                ToolButtonType.OpenSourceCode,
                masterNode.CurrentShader != null ? 1 : 0
            );
        }
        ...
        {
            m_mainGraphInstance.DestroyNode(node, registerUndo);
        }

        public ParentNode CreateNode(
            System.Type type,
            Vector2 position,
        ...
        )
        {
            ...
            if (m_wireReferenceUtils.ValidReferences())
            {
                if (m_wireReferenceUtils.InputPortReference.IsValid)
                {
                    ...
                    InputPort originPort = originNode.GetInputPortByUniqueId(
                        m_wireReferenceUtils.InputPortReference.PortId
                    );
                    OutputPort outputPort = node.GetFirstOutputPortOfType(
                        m_wireReferenceUtils.InputPortReference.DataType,
                        true
                    );
                    if (
                        outputPort != null
                        && originPort.CheckValidType(outputPort.DataType)
                        && (
                            !m_wireReferenceUtils.InputPortReference.TypeLocked
                            || m_wireReferenceUtils.InputPortReference.DataType
                                == WirePortDataType.OBJECT
                ...
                if (m_wireReferenceUtils.OutputPortReference.IsValid)
                {
                    ...
                    InputPort inputPort = node.GetFirstInputPortOfType(
                        m_wireReferenceUtils.OutputPortReference.DataType,
                        true
                    );
        ...
        public void UpdateNodePreviewListAndTime()
        {
            ...
            if (m_smoothOffset)
            {
                ...
                else
                {
                    ...
                    m_cameraOffset = Vector2.SmoothDamp(
                        m_cameraOffset,
                        m_targetOffset,
                        ref m_camVelocity,
                        0.1f,
        ...
        public void ForceUpdateFromMaterial()
        {
            m_forceUpdateFromMaterialFlag = true;
        }
            ...
            else
            {
                if (!string.IsNullOrEmpty(Lastpath))
                {
                    ...
                    if (shader == null)
                    {
                        ...
                        {
                            AmplifyShaderFunction function =
                                AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(Lastpath);
                            if (function != null)
                            {
        ...
        public void ForceMaterialsToUpdate(ref Dictionary<string, string> availableMaterials)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in availableMaterials)
            {
                Material material = AssetDatabase.LoadAssetAtPath<Material>(
                    AssetDatabase.GUIDToAssetPath(kvp.Value)
                );
                if (material != null)
                {
        ...
        public void ReplaceMasterNode(MasterNodeCategoriesData data, bool cacheMasterNodes)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_mainGraphInstance.CurrentMasterNode.InputPorts[i].IsConnected)
                {
                    ...
                    OutputPort op = m_mainGraphInstance
                        .CurrentMasterNode.InputPorts[i]
                        .GetOutputConnection();
            ...
            if (cacheMasterNodes)
            {
                m_clipboard.AddMultiPassNodesToClipboard(
                    m_mainGraphInstance.MultiPassMasterNodes.NodesList,
                    true,
                    -1
                );
                for (int i = 0; i < m_mainGraphInstance.LodMultiPassMasternodes.Count; i++)
                {
                    if (m_mainGraphInstance.LodMultiPassMasternodes[i].Count > 0)
                        m_clipboard.AddMultiPassNodesToClipboard(
                            m_mainGraphInstance.LodMultiPassMasternodes[i].NodesList,
                            false,
                            i
                        );
                }
        ...
        void CheckNodeReplacement()
        {
            if (m_replaceMasterNode)
            {
                ...
                switch (m_replaceMasterNodeType)
                {
                    ...
                    case AvailableShaderTypes.SurfaceShader:
                        {
                            ...
                            {
                                IOUtils.OnShaderTypeChangedEvent(
                                    m_mainGraphInstance.CurrentShader,
                                    false,
                                    string.Empty
                                );
                    ...
                    case AvailableShaderTypes.Template:
                        {
                            TemplateDataParent templateData = m_templatesManager.GetTemplate(
                                m_replaceMasterNodeData
                            );
                            if (m_replaceMasterNodeDataFromCache)
                            {
                                m_mainGraphInstance.CrossCheckTemplateNodes(
                                    templateData,
                                    m_mainGraphInstance.MultiPassMasterNodes.NodesList,
                                    -1
                                );
                                for (
                                    int i = 0;
                                    i < m_mainGraphInstance.LodMultiPassMasternodes.Count;
                                    i++
                                )
                                {
                                    if (m_mainGraphInstance.LodMultiPassMasternodes[i].Count > 0)
                                ...
                                m_clipboard.GetMultiPassNodesFromClipboard(
                                    m_mainGraphInstance.MultiPassMasterNodes.NodesList,
                                    -1
                                );
                                for (
            ...
            else if (m_outdatedShaderFromTemplateLoaded)
            {
                m_outdatedShaderFromTemplateLoaded = false;
                TemplateMultiPassMasterNode masterNode =
                    m_mainGraphInstance.CurrentMasterNode as TemplateMultiPassMasterNode;
                if (masterNode != null)
                {
            ...
            if (m_savedList.Count > 0)
            {
                foreach (var item in m_savedList)
                {
                    ...
                    InputPort ip = m_mainGraphInstance.CurrentMasterNode.InputPorts.Find(x =>
                        x.Name == name
                    );
        ...
        public void UpdateTabTitle()
        {
            ...
            {
                AmplifyShaderFunction openedShaderFunction =
                    m_mainGraphInstance.CurrentShaderFunction;
            ...
            else
            {
                ...
                {
                    this.titleContent.text = GenerateTabTitle(
                        m_mainGraphInstance.CurrentMaterial.name
                    );
                }
                else
                {
                    this.titleContent.text = GenerateTabTitle(
                        m_mainGraphInstance.CurrentShader.name
                    );
                }
            }
        }
        ...
        public bool ShaderIsModified
        {
            ...
            set
            {
                ...
                else
                {
                    m_toolsWindow.SetStateOnButton(
                        ToolButtonType.Update,
                        m_shaderIsModified ? 0 : 2
                    );
                    if (m_mainGraphInstance.CurrentShaderFunction != null)
                        UpdateTabTitle(
                            m_mainGraphInstance.CurrentShaderFunction.FunctionName,
                            m_shaderIsModified
                        );
                }
            }
        ...
        public GraphContextMenu ContextMenuInstance
        {
            get { return m_contextMenu; }
            set { m_contextMenu = value; }
        }
        public ShortcutsManager ShortcutManagerInstance
        {
            get { return m_shortcutManager; }
        }
        ...
        public ASESelectionMode CurrentSelection
        {
            ...
            set
            {
                ...
                switch (m_selectionMode)
                {
                    ...
                    case ASESelectionMode.Shader:
                        {
                            m_toolsWindow.BorderStyle = UIUtils.GetCustomStyle(
                                CustomStyle.ShaderBorder
                            );
                        }
                        break;
                    case ASESelectionMode.Material:
                    ...
                    case ASESelectionMode.ShaderFunction:
                        {
                            m_toolsWindow.BorderStyle = UIUtils.GetCustomStyle(
                                CustomStyle.ShaderFunctionBorder
                            );
                        }
                        break;
                }
        ...
        public string GUID
        {
            get
            {
                if (m_isShaderFunctionWindow)
                {
                    AmplifyShaderFunction openedShaderFunction =
                        m_mainGraphInstance.CurrentShaderFunction;
                    return openedShaderFunction != null
                        ? AssetDatabase.AssetPathToGUID(
                            AssetDatabase.GetAssetPath(openedShaderFunction)
                        )
                        : string.Empty;
                }
                else
                {
                    return m_mainGraphInstance.CurrentShader != null
                        ? AssetDatabase.AssetPathToGUID(
                            AssetDatabase.GetAssetPath(m_mainGraphInstance.CurrentShader)
                        )
                        : string.Empty;
                }
            }
        }
        public List<Toast> Messages
        {
            get { return m_messages; }
            set { m_messages = value; }
        }
        public float MaxMsgWidth
        {
            get { return m_maxMsgWidth; }
            set { m_maxMsgWidth = value; }
        }
        public bool MaximizeMessages
        {
            get { return m_maximizeMessages; }
            set { m_maximizeMessages = value; }
        }

        public void InvalidateAlt()
        {
            m_altAvailable = false;
        }

        public PaletteWindow CurrentPaletteWindow
        {
            get { return m_paletteWindow; }
        }
        ...
        public NodeExporterUtils CurrentNodeExporterUtils
        {
            get { return m_nodeExporterUtils; }
        }
        public AmplifyShaderFunction OpenedShaderFunction
        {
            get { return m_mainGraphInstance.CurrentShaderFunction; }
        }
        ...
        public string LastOpenedLocation
        {
            ...
            set { m_lastOpenedLocation = value; }
        }
        ...
        {
            get { return m_graphCount; }
            set { m_graphCount = value; }
        }
        ...
        public bool OpenedAssetFromNode
        {
            get { return m_openedAssetFromNode; }
            set { m_openedAssetFromNode = value; }
        }
        public bool IsShaderFunctionWindow
        {
            get { return m_isShaderFunctionWindow; }
            set { m_isShaderFunctionWindow = value; }
        }
        public bool NodesLoadedCorrectly
        {
            get { return m_nodesLoadedCorrectly; }
            set { m_nodesLoadedCorrectly = value; }
        }
        public double CurrentInactiveTime
        {
            get { return m_currentInactiveTime; }
        }
        public string ReplaceMasterNodeData
        {
            get { return m_replaceMasterNodeData; }
        }
        public AvailableShaderTypes ReplaceMasterNodeType
        {
            get { return m_replaceMasterNodeType; }
        }
        public NodeWireReferencesUtils WireReferenceUtils
        {
            get { return m_wireReferenceUtils; }
        }
        public ContextPalette WindowContextPallete
        {
            get { return m_contextPalette; }
        }

        // This needs to go to UIUtils
        public Texture2D WireTexture
        {
            get { return m_wireTexture; }
        }
        ...
        {
            get { return m_currentCommandName; }
        }
        public InnerWindowEditorVariables InnerWindowVariables
        {
            get { return m_innerEditorVariables; }
        }
        public TemplatesManager TemplatesManagerInstance
        {
            get { return m_templatesManager; }
        }
        public Material CurrentMaterial
        {
            get { return CurrentGraph.CurrentMaterial; }
        }
        public Shader CurrentShader
        {
            get { return CurrentGraph.CurrentShader; }
        }
