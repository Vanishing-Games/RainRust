...
namespace AmplifyShaderEditor
{
    ...
    public sealed class PortLegendInfo : EditorWindow
    {
        ...
        {
            PortLegendInfo currentWindow = (PortLegendInfo)
                PortLegendInfo.GetWindow(typeof(PortLegendInfo), false);
        ...
        public void Init()
        {
            ...
            if (!EditorGUIUtility.isProSkin)
            {
                ...
                m_nodeInfoLabelStyleBold.normal.textColor = m_labelStyle.normal.textColor =
                    Color.black;
                m_nodeInfoLabelStyle.normal.textColor = m_labelStyle.normal.textColor = Color.black;
            }

            m_availableArea = new Rect(
                WindowPosX,
                WindowPosY,
                WindowSizeX - 2 * WindowPosX,
                WindowSizeY - 2 * WindowPosY
            );
        }
        ...
        void DrawPort(WirePortDataType type)
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                GUILayout.Box(
                    string.Empty,
                    m_portStyle,
                    GUILayout.Width(UIUtils.PortsSize.x),
                    GUILayout.Height(UIUtils.PortsSize.y)
                );
        ...
        void OnGUI()
        {
            ...
            m_availableArea = new Rect(
                WindowPosX,
                WindowPosY,
                position.width - 2 * WindowPosX,
                position.height - 2 * WindowPosY
            );
            GUILayout.BeginArea(m_availableArea);
            {
                ...
                m_currentScrollPos = GUILayout.BeginScrollView(m_currentScrollPos);
                {
                    EditorGUILayout.BeginVertical();
                    {
                        ...
                        NodeUtils.DrawPropertyGroup(
                            ref m_editorShortcutAreaFoldout,
                            EditorShortcutsTitle,
                            DrawEditorShortcuts
                        );
                        NodeUtils.DrawPropertyGroup(
                            ref m_menuShortcutAreaFoldout,
                            MenuShortcutsTitle,
                            DrawMenuShortcuts
                        );
                        NodeUtils.DrawPropertyGroup(
                            ref m_nodesShortcutAreaFoldout,
                            NodesShortcutsTitle,
                            DrawNodesShortcuts
                        );
                        NodeUtils.DrawPropertyGroup(
                            ref m_compatibleAssetsFoldout,
                            CompatibleAssetsTitle,
                            DrawCompatibleAssets
                        );
                        NodeUtils.DrawPropertyGroup(
                            ref m_nodesDescriptionAreaFoldout,
                            NodesDescTitle,
        ...
        public void DrawCompatibleAssets()
        {
            ...
            if (window != null)
            {
                if (m_compatibleAssetsInfo == null)
                {
                    ...
                    {
                        if (
                            items[i].NodeAttributes != null
                            && items[i].NodeAttributes.CastType != null
                        )
                        {
                            ...
                            if (items[i].NodeAttributes.CastType.Length > 1)
                            {
                                ...
                                {
                                    types += CreateCompatibilityString(
                                        items[i].NodeAttributes.CastType[j].ToString()
                                    );
                            ...
                            {
                                types = CreateCompatibilityString(
                                    items[i].NodeAttributes.CastType[0].ToString()
                                );
                            }
                            m_compatibleAssetsInfo.Add(
                                new string[] { items[i].NodeAttributes.Name + ":   ", types }
                            );
                        }
                    }
                }
        ...
        public void DrawNodeDescriptions()
        {
            ...
            if (window != null)
            {
                if (m_nodeDescriptionsInfo == null)
                {
                    ...
                    Dictionary<string, PaletteFilterData> nodeData =
                        window.CurrentPaletteWindow.BuildFullList();
                ...
                //draw
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        m_nodeDescriptionsInfo[i].FoldoutValue = EditorGUILayout.Foldout(
                            m_nodeDescriptionsInfo[i].FoldoutValue,
                            m_nodeDescriptionsInfo[i].Category,
                            m_nodeInfoFoldoutStyle
                        );
                        if (m_nodeDescriptionsInfo[i].FoldoutValue)
                        {
                            ...
                            {
                                GUILayout.Label(
                                    m_nodeDescriptionsInfo[i].Contents[j, 0],
                                    m_nodeInfoLabelStyleBold
                                );
                                GUILayout.Label(
                                    m_nodeDescriptionsInfo[i].Contents[j, 1],
