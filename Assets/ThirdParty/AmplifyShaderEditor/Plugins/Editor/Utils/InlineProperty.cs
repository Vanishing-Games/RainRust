...
namespace AmplifyShaderEditor
{
    ...
    public class InlineProperty
    {
        ...
        public void CustomDrawer(
            ref UndoParentNode owner,
            DrawPropertySection Drawer,
            string content
        )
        {
            if (!m_active)
        ...
        {
            if (
                Preferences.User.ForceTemplateInlineProperties
                && !string.IsNullOrEmpty(m_nodePropertyName)
            )
            {
            ...
            else
            {
                ...
                if (int.TryParse(param, out int nodeId))
                {
                    ...
                    m_nodePropertyName = UIUtils.GetFloatIntNameByNodeId(
                        m_nodeId,
                        m_nodePropertyName
                    );
                }
        ...
        {
            m_value = isInt
                ? Convert.ToInt32(nodeParams[index++])
                : Convert.ToSingle(nodeParams[index++]);
