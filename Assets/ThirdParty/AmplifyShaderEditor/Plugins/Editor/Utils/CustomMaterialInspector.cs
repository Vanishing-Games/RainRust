...
namespace AmplifyShaderEditor
{
    public static class MaterialPropertyHandlerEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEditor.MaterialPropertyHandler, UnityEditor")
                    : type;
            }
        }
        ...
        {
            return MaterialPropertyHandlerEx.Type.InvokeMember(
                "GetHandler",
    ...
    internal class MaterialInspector : ShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
                    ...
                    GUILayout.BeginHorizontal();
                    {
                        if (GUILayout.Button(CopyButtonStr))
                        {
                            ...
                            for (int i = 0; i < propertyCount; i++)
                            {
                                UnityEditor.ShaderUtil.ShaderPropertyType type =
                                    UnityEditor.ShaderUtil.GetPropertyType(shader, i);
                        ...
                        if (GUILayout.Button(PasteButtonStr))
                        {
                            System.Threading.Thread.CurrentThread.CurrentCulture = System
                                ...
                                .CultureInfo
                                .InvariantCulture;
                            string propertiesStr = EditorPrefs.GetString(
                                IOUtils.MAT_CLIPBOARD_ID,
                                string.Empty
                            );
                            if (!string.IsNullOrEmpty(propertiesStr))
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        string[] valuesArr = propertyArr[i]
                                            .Split(IOUtils.FIELD_SEPARATOR);
                                        if (valuesArr.Length != 3)
                                        {
                                            Debug.LogWarning(
                                                "Material clipboard data is corrupted"
                                            );
                                            validData = false;
                                            break;
                                        }
                                        else if (mat.HasProperty(valuesArr[0]))
                                        {
                                            UnityEditor.ShaderUtil.ShaderPropertyType type =
                                                (UnityEditor.ShaderUtil.ShaderPropertyType)
                                                    Enum.Parse(
                                                        typeof(UnityEditor.ShaderUtil.ShaderPropertyType),
                                                        valuesArr[1]
                                                    );
                                            switch (type)
                                            {
                                                    ...
                                                    .TexEnv:
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            mat.SetTextureScale(
                                                                valuesArr[0],
                                                                new Vector2(
                                                                    Convert.ToSingle(texVals[1]),
                                                                    Convert.ToSingle(texVals[2])
                                                                )
                                                            );
                                                            mat.SetTextureOffset(
                                                                valuesArr[0],
                                                                new Vector2(
                                                                    Convert.ToSingle(texVals[3]),
                                                                    Convert.ToSingle(texVals[4])
                                                                )
                                                            );
                                                        }
                                                    }
            ...
            if (m_infoField == null)
            {
                m_infoField = typeof(MaterialEditor).GetField(
                    "m_InfoMessage",
            ...
            for (int i = 0; i < properties.Length; i++)
            {
                ...
                )
                {
                    ...
                    //else
                    //{
                    float propertyHeight = materialEditor.GetPropertyHeight(
                        properties[i],
            ...
            if (
                ...
                && m_lastRenderedTime + 0.032999999821186066 < EditorApplication.timeSinceStartup
            )
            {
        ...
        private void Init()
        {
            ...
            {
                m_targetMesh = AssetDatabase.LoadAssetAtPath<Mesh>(
        ...
        public override void OnMaterialPreviewSettingsGUI(MaterialEditor materialEditor)
        {
            ...
            if (UnityEditor.ShaderUtil.hardwareSupportsRectRenderTexture)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    if (m_targetMesh != null)
                    {
                        EditorPrefs.SetString(
                            PreviewModelPref,
                            AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(m_targetMesh))
                        );
                    }
                    else
                    {
                ...
                {
                    m_selectedField = typeof(MaterialEditor).GetField(
                        "m_SelectedMesh",
                        BindingFlags.Instance | BindingFlags.NonPublic
                    );
                }
                ...
                {
                    m_previewDirDefault = typeof(MaterialEditor).GetField(
                        "m_PreviewDir",
                        BindingFlags.Instance | BindingFlags.NonPublic
                    );
                }
        ...
        )
        {
            if (Event.current.type == EventType.DragExited)
            {
                if (DragAndDrop.objectReferences.Length > 0)
                {
                    ...
                    {
                        m_targetMesh = AssetDatabase.LoadAssetAtPath<Mesh>(
                            AssetDatabase.GetAssetPath(dropped)
                        );
                        EditorPrefs.SetString(
                            PreviewModelPref,
                            AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(m_targetMesh))
            ...
            {
                m_previewGUIType = Type.GetType(
                    "PreviewGUI, UnityEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
                );
                m_dragMethod = m_previewGUIType.GetMethod(
                    "Drag2D",
            ...
            m_previewDir = (Vector2)
                m_dragMethod.Invoke(m_previewGUIType, new object[] { m_previewDir, r });

            if (m_modelInspectorType == null)
            {
                m_modelInspectorType = Type.GetType(
                    "UnityEditor.MeshPreview, UnityEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
                );
                if (m_modelInspectorType == null)
                {
                    m_modelInspectorType = Type.GetType(
                        "UnityEditor.ModelInspector, UnityEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null"
                    );
                }
                m_renderMeshMethod = m_modelInspectorType.GetMethod(
                    "RenderMeshPreview",
            ...
            {
                m_previewSettingsType = m_modelInspectorType.GetNestedType(
                    "PreviewSettings",
            ...
            {
                shadedMaterialInfo = m_previewSettingsType.GetField(
                    "shadedPreviewMaterial",
                    BindingFlags.Instance | BindingFlags.Public
                );
                activeMaterialInfo = m_previewSettingsType.GetField(
                    "activeMaterial",
                    BindingFlags.Instance | BindingFlags.Public
                );
                previewDirInfo = m_previewSettingsType.GetField(
                    "previewDir",
                    BindingFlags.Instance | BindingFlags.Public
                );
            }
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                m_renderMeshMethod.Invoke(
                    m_modelInspectorType,
                    new object[]
                    {
                        m_targetMesh,
                        m_previewRenderUtility,
                        m_previewSettingsInstance,
                        -1,
                    }
                );
