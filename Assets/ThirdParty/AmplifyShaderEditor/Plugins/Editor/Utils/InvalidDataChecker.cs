...
namespace AmplifyShaderEditor
{
    ...
    {
        private static string[] m_invalidData =
        {
            "674ea7bed6b1cd94b8057074298096db", //"/Samples",
        ...
        static InvalidDataChecker()
        {
            ...
            if (foundInvalidData)
            {
                InvalidDataPopUp window = (InvalidDataPopUp)
                    EditorWindow.GetWindow(typeof(InvalidDataPopUp), true, "Found Invalid Data");
        ...
        static void Update()
        {
            ...
            if (!EditorApplication.isPlayingOrWillChangePlaymode)
            {
                ...
                else
                {
                    ...
                    {
                        show = (Preferences.ShowOption)
                            EditorPrefs.GetInt(Preferences.User.Keys.StartUp, 0);
                        // check version here
                        if (show == Preferences.ShowOption.OnNewVersion)
                        {
                            ASEStartScreen.StartBackgroundTask(
                                StartRequest(
                                    ASEStartScreen.ChangelogURL,
                                    () =>
                                    {
                                        var changeLog = ChangeLogInfo.CreateFromJSON(
        ...
        public static string InvalidDataCollected
        {
            get { return m_invalidDataCollected; }
        }
    }
    ...
    public class InvalidDataPopUp : EditorWindow
    {
        ...
        public void OnGUI()
        {
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.Label(
                    "\nAmplify Shader Editor " + VersionInfo.StaticToString(),
                    labelStyle,
                    GUILayout.ExpandWidth(true)
                );
                GUILayout.Space(5);
                GUILayout.Label(
                    "Invalid/Legacy Data was found on your previous ASE folder which needs to be removed in order for it to work correctly.",
                    labelStyle,
                    GUILayout.ExpandWidth(true)
                );
                GUILayout.Space(5);
                GUILayout.Label(
                    "Below are the detected files/folders which require to be removed.",
                    labelStyle,
                    GUILayout.ExpandWidth(true)
                );
                GUILayout.Space(5);
                ...
                GUILayout.Label(
                    "VERY IMPORTANT: If you have assets of yours inside these folders you need to move them to another location before hitting the button below or they will be PERMANENTLY DELETED",
                    labelStyle,
                    GUILayout.ExpandWidth(true)
                );
                GUILayout.Space(5);
