    ...
    public class ASESaveBundleAssetEditor : Editor
    {
        ...
        public void OnEnable()
        {
            ...
            if (m_listShaders == null)
            {
                ...
                m_listShaders.drawElementCallback = (
                    Rect rect,
                    int index,
                    bool isActive,
                    bool isFocused
                ...
                {
                    m_allShaders.GetArrayElementAtIndex(index).objectReferenceValue = (Shader)
                        EditorGUI.ObjectField(
                            rect,
                            "Shader " + index,
                            m_allShaders.GetArrayElementAtIndex(index).objectReferenceValue,
                            typeof(Shader),
                            false
                        );
                };
                ...
                m_listShaders.onAddCallback = (list) =>
                {
                    ...
                    m_allShaders
                        .GetArrayElementAtIndex(m_allShaders.arraySize - 1)
            ...
            if (m_listExtras == null)
            {
                ...
                m_listExtras.drawElementCallback = (
                    Rect rect,
                    int index,
                    bool isActive,
                    bool isFocused
                ...
                {
                    rect.width -= 55;
                    m_allExtras.GetArrayElementAtIndex(index).stringValue = (string)
                        EditorGUI.TextField(
                            rect,
                            "Path " + index,
                            m_allExtras.GetArrayElementAtIndex(index).stringValue
                        );
                ...
                m_listExtras.onAddCallback = (list) =>
                {
                    ...
                    m_allExtras.GetArrayElementAtIndex(m_allExtras.arraySize - 1).stringValue =
                        string.Empty;
                };
        ...
        void FetchValidShadersFromPath(string path, bool updateProperty)
        {
            ...
            if (AssetDatabase.IsValidFolder(path))
            {
                ...
                {
                    Shader internalShader = AssetDatabase.LoadAssetAtPath<Shader>(
                        AssetDatabase.GUIDToAssetPath(shaderInDir[shaderIdx])
                    );
                    if (internalShader != null && IOUtils.IsASEShader(internalShader))
                    {
                        ...
                        m_allShaders
                            .GetArrayElementAtIndex(m_allShaders.arraySize - 1)
                            .objectReferenceValue = internalShader;
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                EditorGUILayout.PropertyField(
                    m_packageContentsOrigin,
                    m_packageContentsOriginLabel
                );
                if (GUILayout.Button("Browse", GUILayout.MaxWidth(55)))
                {
                    m_packageContentsOrigin.stringValue = ASESaveBundleTool.FetchPath(
                        "Folder Path",
                        m_packageContentsOrigin.stringValue
                    );
                }
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Browse", GUILayout.MaxWidth(55)))
                    m_packageTargetPath.stringValue = EditorUtility.OpenFolderPanel(
                        "Folder Path",
                        m_packageTargetPath.stringValue,
                        ""
                    );
            }
            EditorGUILayout.EndHorizontal();
    ...
    public class ASESaveBundleTool : EditorWindow
    {
        ...
        private void ExportCurrent(ASESaveBundleAsset currentAsset)
        {
            ...
            AssetDatabase.ExportPackage(
                pathsList.ToArray(),
                currentAsset.PackageTargetPath
                    + "/"
                    + currentAsset.PackageTargetName
                    + ".unitypackage",
                ...
                );
                {
                    ...
                    EditorGUILayout.BeginVertical(m_contentStyle);
                    {
                        EditorGUI.BeginDisabledGroup(currentAsset.AllShaders.Count <= 0);
                        {
                            ...
                            if (
                                GUILayout.Button(
                                    UpdateAllStr /* , UpdateAllStyle , GUILayout.Height( 20 )*/
                                )
                            )
                            {
                                ...
                                {
                                    assetPaths[i] = AssetDatabase.GetAssetPath(
                                        currentAsset.AllShaders[i]
                                    );
                                }
                            ...
                            if (GUILayout.Button("Remove Custom Inspector"))
                            {
                                ...
                                for (int i = 0; i < count; i++)
                                {
                                    EditorUtility.DisplayProgressBar(
                                        ...
                                        currentAsset.AllShaders[i].name,
                                        i / (count - 1)
                                    );
                                    ...
                                    shaderBody = Regex.Replace(
                                        shaderBody,
                                        TemplateHelperFunctions.CustomInspectorPattern,
                                        string.Empty,
                        ...
                        EditorGUI.BeginDisabledGroup(
                            string.IsNullOrEmpty(currentAsset.PackageTargetName)
                                || string.IsNullOrEmpty(currentAsset.PackageTargetPath)
                        );
                        {
                        ...
                        m_asset =
                            EditorGUILayout.ObjectField(
                                "Asset Preset",
                                m_asset,
                                typeof(ASESaveBundleAsset),
                                false
                            ) as ASESaveBundleAsset;
                        if (
                            GUILayout.Button(
                        ...
                        )
                        {
                            ...
                            {
                                ASESaveBundleAsset outfile =
                                    AssetDatabase.LoadMainAssetAtPath(path) as ASESaveBundleAsset;
                                if (outfile != null)
                                {
                                ...
                                else
                                {
                                    ...
                                    {
                                        currentAsset =
                                            ScriptableObject.CreateInstance<ASESaveBundleAsset>();
                        ...
                        if (Event.current.type == EventType.Layout)
                        {
                            if (m_editor == null)
                            {
                                m_editor =
                                    Editor.CreateEditor(
                                        currentAsset,
                                        typeof(ASESaveBundleAssetEditor)
                                    ) as ASESaveBundleAssetEditor;
                            }
                            else
                            {
