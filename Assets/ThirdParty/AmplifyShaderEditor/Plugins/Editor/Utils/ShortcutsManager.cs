...
namespace AmplifyShaderEditor
{
    public class ShortcutItem
    {
        ...
        public ShortcutItem(
            string name,
            string description,
            ShortcutFunction myKeyDownFunctionPtr,
            ShortcutFunction myKeyUpFunctionPtr = null
        )
        {
            Name = name;
            Description = description;
    ...
    public class ShortcutsManager
    {
        ...
        private Dictionary<
            KeyCode,
            Dictionary<EventModifiers, ShortcutItem>
        > m_editorShortcutsDict =
            new Dictionary<KeyCode, Dictionary<EventModifiers, ShortcutItem>>();
        private Dictionary<KeyCode, ShortcutItem> m_editorNoModifiersShortcutsDict =
            new Dictionary<KeyCode, ShortcutItem>();
        private List<ShortcutItem> m_editorShortcutsList = new List<ShortcutItem>();

        private Dictionary<KeyCode, ShortcutItem> m_nodesShortcutsDict =
            new Dictionary<KeyCode, ShortcutItem>();
        ...
        public void DumpShortcutsToDisk(string pathname)
        {
            ...
            for (int i = 0; i < m_editorShortcutsList.Count; i++)
            {
                list += string.Format(
                    ItemWikiFormat,
                    m_editorShortcutsList[i].Name,
                    m_editorShortcutsList[i].Description
                );
            }
            list += "\n";
            list += "==== Nodes ====\n";
            for (int i = 0; i < m_nodesShortcutsList.Count; i++)
            {
                list += string.Format(
                    ItemWikiFormat,
                    m_nodesShortcutsList[i].Name,
                    m_nodesShortcutsList[i].Description
                );
            }
        ...
        public void RegisterEditorShortcut(
            bool showOnList,
            EventModifiers modifiers,
            KeyCode key,
            string description,
            ShortcutItem.ShortcutFunction myKeyDownFunctionPtr,
            ShortcutItem.ShortcutFunction myKeyUpFunctionPtr = null
        )
        {
            if (m_editorShortcutsDict.ContainsKey(key))
            {
                if (m_editorShortcutsDict[key].ContainsKey(modifiers))
                {
                    ...
                    {
                        Debug.Log(
                            "Attempting to register an already used editor shortcut key " + key
                        );
                    }
                    return;
                }
            ...
            ShortcutItem item = new ShortcutItem(
                (
                    (modifiers == EventModifiers.None || modifiers == EventModifiers.FunctionKey)
                        ? key.ToString()
                        : modifiers + " + " + key
                ),
                description,
                myKeyDownFunctionPtr,
                myKeyUpFunctionPtr
            );
        ...
        public bool ActivateShortcut(EventModifiers modifiers, KeyCode key, bool isKeyDown)
        {
            ...
            if (
                ...
                && m_editorNoModifiersShortcutsDict.ContainsKey(key)
            )
            {
                if (isKeyDown)
                {
        ...
        public List<ShortcutItem> AvailableEditorShortcutsList
        {
            get { return m_editorShortcutsList; }
        }
        public List<ShortcutItem> AvailableNodesShortcutsList
        {
            get { return m_nodesShortcutsList; }
        }
    }
}
