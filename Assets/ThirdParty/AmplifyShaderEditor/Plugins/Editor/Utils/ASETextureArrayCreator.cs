...
namespace AmplifyShaderEditor
{
    ...
    public class TextureArrayCreatorAssetEditor : Editor
    {
        ...
        public void OnEnable()
        {
            ...
            if (m_listTextures == null)
            {
                ...
                m_listTextures.drawElementCallback = (
                    Rect rect,
                    int index,
                    bool isActive,
                    bool isFocused
                ...
                {
                    m_allTextures.GetArrayElementAtIndex(index).objectReferenceValue = (Texture2D)
                        EditorGUI.ObjectField(
                            rect,
                            "Texture " + index,
                            m_allTextures.GetArrayElementAtIndex(index).objectReferenceValue,
                            typeof(Texture2D),
                            false
                        );
                };
                ...
                {
                    m_previewSize = EditorGUI.IntSlider(
                        rect,
                        "Texture List",
                        m_previewSize,
                        16,
                ...
                m_listTextures.onAddCallback = (list) =>
                {
                    ...
                    m_allTextures
                        .GetArrayElementAtIndex(m_allTextures.arraySize - 1)
                ...
                else
                {
                    ...
                    if (asset != null)
                    {
                        ...
                        if (AssetDatabase.IsValidFolder(path))
                        {
                            ...
                            {
                                Texture2D internalTex = AssetDatabase.LoadAssetAtPath<Texture2D>(
                                    AssetDatabase.GUIDToAssetPath(texInDir[texIdx])
                                );
                                if (internalTex != null)
                                {
                                    m_allTextures.InsertArrayElementAtIndex(
                                        m_allTextures.arraySize
                                    );
                                    m_allTextures
                                        .GetArrayElementAtIndex(m_allTextures.arraySize - 1)
                                        .objectReferenceValue = internalTex;
                                    m_so.ApplyModifiedProperties();
                                }
                            }
                        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                m_selectedSize.intValue = EditorGUILayout.Popup(
                    "X",
                    m_selectedSize.intValue,
                    m_sizesStr
                );
            ...
            m_lockRatio.boolValue = GUILayout.Toggle(
                m_lockRatio.boolValue,
                "L",
                "minibutton",
                GUILayout.Width(18)
            );
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    m_fileName.stringValue = m_tex3DMode.boolValue
                        ? Texture3DFilename
                        : ArrayFilename;
                }
            }
            ...
            m_anisoLevel.intValue = EditorGUILayout.IntSlider(
                "Aniso Level",
                m_anisoLevel.intValue,
                0,
                16
            );
            ...
            if (m_selectedFormatEnum.intValue == (int)TextureFormat.DXT1Crunched)
            {
                ...
                Debug.Log(
                    "Texture Array does not support crunched DXT1 format. Changing to DXT1..."
                );
            }
            else if (m_selectedFormatEnum.intValue == (int)TextureFormat.DXT5Crunched)
            {
                ...
                Debug.Log(
                    "Texture Array does not support crunched DXT5 format. Changing to DXT5..."
                );
            }

            m_quality.intValue = EditorGUILayout.IntSlider(
                "Format Quality",
                m_quality.intValue,
                0,
                100
            ...
            if (
                GUILayout.Button(
                    m_pathButtonContent,
                    m_pathButtonStyle,
                    GUILayout.MaxWidth(Mathf.Min(Screen.width * 0.5f, buttonSize.x))
                )
            )
            {
                string folderpath = EditorUtility.OpenFolderPanel(
                    "Save Texture Array to folder",
                    "Assets/",
                    ""
                );
            ...
            EditorGUI.BeginChangeCheck();
            m_fileName.stringValue = EditorGUILayout.TextField(
                m_fileName.stringValue,
                GUILayout.ExpandWidth(true)
            );
            if (EditorGUI.EndChangeCheck())
            {
                m_filenameChanged.boolValue =
                    m_fileName.stringValue == ArrayFilename ? false : true;
            }
    ...
    public class ASETextureArrayCreator : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            string buildButtonStr = currentAsset.Tex3DMode
                ? BuildTexture3DMessage
                : BuildArrayMessage;
            // build button
            EditorGUILayout.BeginHorizontal();
            ...
            if (GUILayout.Button(buildButtonStr, "prebutton", GUILayout.Height(20)))
            {
                ...
                {
                    if (
                        currentAsset.AllTextures[i].width != currentAsset.SizeX
                        || currentAsset.AllTextures[i].height != currentAsset.SizeY
                    )
                    {
                        showWarning = true;
                    }
                ...
                if (!showWarning)
                {
                    ...
                    EditorUtility.DisplayDialog(
                        "Warning!",
                        "Some textures need to be resized to fit the selected size. Do you want to continue?",
                        "Yes",
                        "No"
                    )
                )
            ...
            m_asset =
                EditorGUILayout.ObjectField(
                    "Asset Preset",
                    m_asset,
                    typeof(TextureArrayCreatorAsset),
                    false
                ) as TextureArrayCreatorAsset;
            if (
                GUILayout.Button(
                    m_asset != null ? "Save" : "Create",
                    "minibutton",
                    GUILayout.Width(50)
                )
            )
            {
                ...
                {
                    TextureArrayCreatorAsset outfile =
                        AssetDatabase.LoadMainAssetAtPath(path) as TextureArrayCreatorAsset;
                    if (outfile != null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            currentAsset =
                                ScriptableObject.CreateInstance<TextureArrayCreatorAsset>();
            ...
            if (Event.current.type == EventType.Layout)
            {
                ...
                {
                    m_editor =
                        Editor.CreateEditor(currentAsset, typeof(TextureArrayCreatorAssetEditor))
                        as TextureArrayCreatorAssetEditor;
                }
                else
                {
                    if (m_editor.Instance != currentAsset)
                    {
                        ...
                        m_editor =
                            Editor.CreateEditor(
                                currentAsset,
                                typeof(TextureArrayCreatorAssetEditor)
                            ) as TextureArrayCreatorAssetEditor;
                    }
                }
            }
        ...
        private void BuildTexture3D(TextureArrayCreatorAsset asset)
        {
            ...
            Texture3D texture3D = new Texture3D(
                sizeX,
                sizeY,
                asset.AllTextures.Count,
                asset.SelectedFormatEnum,
                asset.MipMaps
            );
            ...
            for (int i = 0; i < asset.AllTextures.Count; i++)
            {
                // build report
                int widthChanges =
                    asset.AllTextures[i].width < sizeX ? -1
                    : asset.AllTextures[i].width > sizeX ? 1
                    : 0;
                int heightChanges =
                    asset.AllTextures[i].height < sizeY ? -1
                    : asset.AllTextures[i].height > sizeY ? 1
                    : 0;
                if (
                    (widthChanges < 0 && heightChanges <= 0)
                    || (widthChanges <= 0 && heightChanges < 0)
                )
                ...
                bool isCompressed =
                    UncompressedFormats.FindIndex(x => x.Equals(asset.SelectedFormatEnum)) < 0;
                if (isCompressed)
                {
        ...
        private void BuildTexture3D(TextureArrayCreatorAsset asset)
        {
            ...
            for (int i = 0; i < asset.AllTextures.Count; i++)
            {
                ...
                bool isCompressed =
                    UncompressedFormats.FindIndex(x => x.Equals(asset.SelectedFormatEnum)) < 0;
                TextureFormat validReadPixelsFormat = isCompressed
                    ? TextureFormat.RGBAFloat
                    : asset.SelectedFormatEnum;
                Texture2D t2d = new Texture2D(
                    sizeX,
                    sizeY,
                    validReadPixelsFormat,
                    asset.MipMaps,
                    asset.LinearMode
                );
