...
namespace AmplifyShaderEditor
{
    ...
    public class ASEPPSHelperTool : EditorWindow
    {
        ...
        public static readonly string PPSPropertySetFormat =
            "\t\tsheet.properties.{0}( \"{1}\", settings.{1} );\n";
        public static readonly string PPSPropertySetNullPointerCheckFormat =
            "\t\tif(settings.{1}.value != null) sheet.properties.{0}( \"{1}\", settings.{1} );\n";
        public static readonly string PPSPropertyDecFormat =
            "\t[{0}Tooltip( \"{1}\" )]\n" + "\tpublic {2} {3} = new {2} {{ {4} }};\n";
        public static readonly Dictionary<WirePortDataType, string> WireToPPSType = new Dictionary<
            WirePortDataType,
            string
        >()
        {
        ...
        public static readonly Dictionary<WirePortDataType, string> WireToPPSValueSet =
        ...
        public static readonly Dictionary<
            ...
            string
        > ShaderPropertyToPPSType = new Dictionary<
            UnityEditor.ShaderUtil.ShaderPropertyType,
            string
        >()
        {
        ...
        public static readonly Dictionary<
            UnityEditor.ShaderUtil.ShaderPropertyType,
            string
        > ShaderPropertyToPPSSet = new Dictionary<
            UnityEditor.ShaderUtil.ShaderPropertyType,
            string
        >()
        {
        ...
        void FetchTooltips()
        {
            ...
            for (int i = 0; i < propertyCount; i++)
            {
                ...
                string description = UnityEditor.ShaderUtil.GetPropertyDescription(
                    m_currentShader,
                    i
                );
        ...
        void OnGUI()
        {
            ...
            if (
                GUILayout.Button(
                    m_pathButtonContent,
                    m_pathButtonStyle,
                    GUILayout.MaxWidth(Mathf.Min(position.width * 0.5f, buttonSize.x))
                )
            )
            {
            ...
            m_settingsClassName = EditorGUILayout.TextField(
                m_settingsClassName,
            ...
            if (m_tooltipsFoldout)
            {
                ...
                {
                    m_tooltips[i].Tooltip = EditorGUILayout.TextField(
                        m_tooltips[i].Name,
                        m_tooltips[i].Tooltip
                    );
                }
                EditorGUI.indentLevel--;
            }
        ...
        {
            List<PropertyNode> properties = UIUtils
                .CurrentWindow
                .OutsideGraph
                .PropertyNodes
                .NodesList;
        ...
        public void GetShaderInfoFromShaderAsset(
            ref string propertiesDecl,
            ref string propertiesSet
        )
        {
            ...
            if (m_currentShader != null)
            {
                ...
                for (int i = 0; i < propertyCount; i++)
                {
                    UnityEditor.ShaderUtil.ShaderPropertyType type =
                        UnityEditor.ShaderUtil.GetPropertyType(m_currentShader, i);
                    string name = UnityEditor.ShaderUtil.GetPropertyName(m_currentShader, i);
                        ...
                        case UnityEditor.ShaderUtil.ShaderPropertyType.Color:
                            {
                                ...
                                defaultValue = string.Format(
                                    "value = new Color({0}f,{1}f,{2}f,{3}f)",
                                    value.r,
                                    value.g,
                                    value.b,
                                    value.a
                        ...
                        case UnityEditor.ShaderUtil.ShaderPropertyType.Vector:
                            {
                                ...
                                defaultValue = string.Format(
                                    "value = new Vector4({0}f,{1}f,{2}f,{3}f)",
                                    value.x,
                                    value.y,
                                    value.z,
                                    value.w
                    ...
                    propertiesDecl += string.Format(
                        PPSPropertyDecFormat,
                        string.Empty,
                        m_tooltips[validIds].Tooltip,
                        ShaderPropertyToPPSType[type],
                        name,
                        defaultValue
                    );
                    propertiesSet += string.Format(
                        nullPointerCheck
                            ? PPSPropertySetNullPointerCheckFormat
                            : PPSPropertySetFormat,
                        ShaderPropertyToPPSSet[type],
                        name
                    );
                    validIds++;
                }
        ...
        private void GetInitialInfo()
        {
            ...
            m_menuEntry = masterNode
                .ShaderName.Replace("Hidden/", string.Empty)
        ...
        private void GetInitialInfo(Shader shader)
        {
            ...
            m_menuEntry = shader
                .name.Replace("Hidden/", string.Empty)
                .Replace(".shader", string.Empty);
            ...
            m_folderPath = m_folderPath.Replace(
                System.IO.Path.GetFileName(m_folderPath),
                string.Empty
            );
