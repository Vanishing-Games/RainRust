...
namespace AmplifyShaderEditor
{
    ...
    internal class CustomShaderInspector : Editor
    {
        ...
        {
            public static Texture2D errorIcon = EditorGUIUtilityEx.LoadIcon(
                "console.erroricon.sml"
            );

            public static Texture2D warningIcon = EditorGUIUtilityEx.LoadIcon(
                "console.warnicon.sml"
            );
#if UNITY_2020_1_OR_NEWER
            public static GUIContent togglePreprocess = EditorGUIUtilityEx.TextContent(
                "Preprocess only|Show preprocessor output instead of compiled shader code"
            );
#if UNITY_2020_2_OR_NEWER
            public static GUIContent toggleStripLineDirective = EditorGUIUtility.TrTextContent(
                "Strip #line directives",
                "Strip #line directives from preprocessor output"
            );
#endif
#endif
            public static GUIContent showSurface = EditorGUIUtilityEx.TextContent(
                "Show generated code|Show generated code of a surface shader"
            );

            public static GUIContent showFF = EditorGUIUtilityEx.TextContent(
                "Show generated code|Show generated code of a fixed function shader"
            );
            ...
            public static GUIContent builtinShader = EditorGUIUtilityEx.TextContent(
                "Built-in shader"
            );
                ...
                case EventType.MouseDrag:
                    {
                        if (GUIUtility.hotControl == controlID)
                        {
                            scrollPosition -=
                                DeltaMultiplier
                                * current.delta
                                * (float)((current.shift) ? ShiftMouseSpeed : DefaultMouseSpeed)
        ...
        public override void OnPreviewGUI(Rect r, GUIStyle background)
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                m_cameraTransform.rotation = Quaternion.Euler(
                    new Vector3(-m_mouseDelta.y, -m_mouseDelta.x, 0)
                );
        ...
        private static string GetPropertyType(Shader s, int index)
        {
            UnityEditor.ShaderUtil.ShaderPropertyType propertyType =
                UnityEditor.ShaderUtil.GetPropertyType(s, index);
            if (propertyType == UnityEditor.ShaderUtil.ShaderPropertyType.TexEnv)
            {
                return CustomShaderInspector.kTextureTypes[
                    (int)UnityEditor.ShaderUtil.GetTexDim(s, index)
                ];
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (shader.isSupported)
            {
                EditorGUILayout.LabelField(
                    "Cast shadows",
                    (!ShaderUtilEx.HasShadowCasterPass(shader)) ? "no" : "yes",
                    new GUILayoutOption[0]
                );
                EditorGUILayout.LabelField(
                    "Render queue",
                    ShaderUtilEx
                        .GetRenderQueue(shader)
                ...
                EditorGUILayout.LabelField(
                    "Ignore projector",
                    (!ShaderUtilEx.DoesIgnoreProjector(shader)) ? "no" : "yes",
                    new GUILayoutOption[0]
                );
                string label;
                ...
                int sRPBatcherCompatibilityCode = ShaderUtilEx.GetSRPBatcherCompatibilityCode(
                    shader,
                    shaderActiveSubshaderIndex
                );
                string label2 =
                    (sRPBatcherCompatibilityCode != 0) ? "not compatible" : "compatible";
                EditorGUILayout.LabelField("SRP Batcher", label2);
                if (sRPBatcherCompatibilityCode != 0)
                {
                    EditorGUILayout.HelpBox(
                        ShaderUtilEx.GetSRPBatcherCompatibilityIssueReason(
                            shader,
                            shaderActiveSubshaderIndex,
                            sRPBatcherCompatibilityCode
                        ),
                        MessageType.Info
                    );
                }
        ...
        private static void ShowShaderProperties(Shader s)
        {
            ...
            for (int i = 0; i < propertyCount; i++)
            {
                ...
                string label =
                    CustomShaderInspector.GetPropertyType(s, i)
                    + UnityEditor.ShaderUtil.GetPropertyDescription(s, i);
                EditorGUILayout.LabelField(propertyName, label, new GUILayoutOption[0]);
            }
        }

        internal static void ShaderErrorListUI(
            UnityEngine.Object shader,
            ShaderError[] errors,
        ...
        )
        {
            ...
            GUILayout.Space(5f);
            GUILayout.Label(
                string.Format("Errors ({0}):", num),
                EditorStyles.boldLabel,
                new GUILayoutOption[0]
            );
            int controlID = GUIUtility.GetControlID(
                CustomShaderInspector.kErrorViewHash,
                FocusType.Passive
            );
            float minHeight = Mathf.Min((float)num * 20f + 40f, 150f);
            scrollPosition = GUILayout.BeginScrollView(
                scrollPosition,
                GUISkinEx.GetCurrentSkin().box,
                new GUILayoutOption[] { GUILayout.MinHeight(minHeight) }
            );
            EditorGUIUtility.SetIconSize(new Vector2(16f, 16f));
            float height = CustomShaderInspector.Styles.messageStyle.CalcHeight(
                EditorGUIUtilityEx.TempContent(CustomShaderInspector.Styles.errorIcon),
                100f
            );
            Event current = Event.current;
            for (int i = 0; i < num; i++)
            {
                ...
                if (
                    ...
                    && current.button == 0
                    && controlRect.Contains(current.mousePosition)
                )
                {
                    ...
                    if (current.clickCount == 2)
                    {
                        ...
                        UnityEngine.Object @object =
                            (!string.IsNullOrEmpty(file))
                                ? AssetDatabase.LoadMainAssetAtPath(file)
                                : null;
                ...
                if (line > 0)
                {
                    ...
                    {
                        content = EditorGUIUtilityEx.TempContent(
                ...
                GUI.Label(
                    position2,
                    EditorGUIUtilityEx.TempContent(
                        message,
                        (!flag)
                            ? CustomShaderInspector.Styles.errorIcon
                            : CustomShaderInspector.Styles.warningIcon
                    ),
                    CustomShaderInspector.Styles.messageStyle
                );
            }
        ...
        private void ShowCompiledCodeButton(Shader s)
        {
            ...
            {
                s_PreprocessOnly = EditorGUILayout.Toggle(
                    Styles.togglePreprocess,
                    s_PreprocessOnly
                );
#if UNITY_2020_2_OR_NEWER
                if (s_PreprocessOnly)
                {
                    s_StripLineDirectives = EditorGUILayout.Toggle(
                        Styles.toggleStripLineDirective,
                        s_StripLineDirectives
                    );
                }
#endif
            }
            ...
            bool hasCode =
                ShaderUtilEx.HasShaderSnippets(s)
                || ShaderUtilEx.HasSurfaceShaders(s)
                || ShaderUtilEx.HasFixedFunctionShaders(s);
            if (hasCode)
            {
                    ...
                    PopupWindow.Show(
                        ...
                        (PopupWindowContent)
                            Activator.CreateInstance(
                                System.Type.GetType(
                                    "UnityEditor.ShaderInspectorPlatformsPopup, UnityEditor"
                                ),
                                new object[] { s }
                            )
                    );
                ...
                {
#if UNITY_2020_1
                    ShaderUtilEx.OpenCompiledShader(
                        s,
                        ShaderInspectorPlatformsPopupEx.GetCurrentMode(),
                        ShaderInspectorPlatformsPopupEx.GetCurrentPlatformMask(),
                        ShaderInspectorPlatformsPopupEx.GetCurrentVariantStripping() == 0,
                        s_PreprocessOnly
                    );
        ...
        private static void ShowSurfaceShaderButton(Shader s)
        {
            ...
            if (flag)
            {
                if (!(AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(s)) == null))
                {
                    if (
                        GUILayout.Button(
                            CustomShaderInspector.Styles.showSurface,
                            EditorStyles.miniButton,
                            new GUILayoutOption[] { GUILayout.ExpandWidth(false) }
                        )
                    )
                ...
                {
                    GUILayout.Button(
                        CustomShaderInspector.Styles.builtinShader,
                        GUI.skin.label,
                        new GUILayoutOption[0]
                    );
                }
            ...
            {
                GUILayout.Button(
                    CustomShaderInspector.Styles.no,
                    GUI.skin.label,
                    new GUILayoutOption[0]
                );
            }
        ...
        private static void ShowFixedFunctionShaderButton(Shader s)
        {
            ...
            if (flag)
            {
                if (!(AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(s)) == null))
                {
                    if (
                        GUILayout.Button(
                            CustomShaderInspector.Styles.showFF,
                            EditorStyles.miniButton,
                            new GUILayoutOption[] { GUILayout.ExpandWidth(false) }
                        )
                    )
    ...
    internal class KeywordsPopup : PopupWindowContent
    {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            m_GlobalKeywordsExpended = KeywordsFoldout(
                m_GlobalKeywordsExpended,
                "Global Keywords",
                m_GlobalKeywords
            );
            m_LocalKeywordsExpended = KeywordsFoldout(
                m_LocalKeywordsExpended,
                "Local Keywords",
                m_LocalKeywords
            );
    ...
    public static class EditorGUIUtilityEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEditor.EditorGUIUtility, UnityEditor")
                    : type;
            }
        }
        ...
        {
            return (Texture2D)
                EditorGUIUtilityEx.Type.InvokeMember(
                    "LoadIcon",
    ...
    public static class GUILayoutUtilityEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEngine.GUILayoutUtility, UnityEngine")
                    : type;
            }
        }
        ...
        {
            System.Type guiLayoutGroup = System.Type.GetType(
                "UnityEngine.GUILayoutGroup, UnityEngine"
            );
            var topLevel = GUILayoutUtilityEx
                .Type.GetProperty("topLevel", BindingFlags.NonPublic | BindingFlags.Static)
                .GetValue(null, null);
            return (Rect)
                guiLayoutGroup.InvokeMember(
                    "GetLast",
    ...
    public static class ShaderEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEngine.Shader, UnityEngine")
                    : type;
            }
        }
        ...
        {
            return (DisableBatchingType)
                ShaderEx
                    .Type.GetProperty(
                        "disableBatching",
                        BindingFlags.NonPublic | BindingFlags.Instance
                    )
                    .GetValue(s, new object[0]);
        }
    }
    ...
    public static class ShaderUtilEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEditor.ShaderUtil, UnityEditor")
                    : type;
            }
        }
        ...
        {
            ShaderUtilEx.Type.InvokeMember(
        ...
        {
            ShaderUtilEx.Type.InvokeMember(
                "OpenGeneratedFixedFunctionShader",
                BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.InvokeMethod,
                null,
                null,
                new object[] { s }
...
#if UNITY_2020_1
        public static void OpenCompiledShader(
            Shader shader,
            int mode,
            int customPlatformsMask,
        ...
        )
        {
            ShaderUtilEx.Type.InvokeMember(
                ...
                null,
                null,
                new object[] { shader, mode, customPlatformsMask, includeAllVariants }
            );
        }
#endif
        ...
        {
            return ShaderUtilEx.Type.InvokeMember(
        ...
        public static int GetAvailableShaderCompilerPlatforms()
        {
            return (int)
                ShaderUtilEx.Type.InvokeMember(
                    "GetAvailableShaderCompilerPlatforms",
                    BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.InvokeMethod,
                    null,
                    null,
                    new object[] { }
        ...
        {
            System.Type shaderErrorType = System.Type.GetType(
                "UnityEditor.ShaderError, UnityEditor"
            );
            var errorList = (System.Collections.IList)
                ShaderUtilEx.Type.InvokeMember(
                    "GetShaderErrors",
                    BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.InvokeMethod,
                    null,
            ...
            FieldInfo messageField = shaderErrorType.GetField(
                "message",
                BindingFlags.Public | BindingFlags.Instance
            );
            FieldInfo messageDetailsField = shaderErrorType.GetField(
                "messageDetails",
                BindingFlags.Public | BindingFlags.Instance
            );
            FieldInfo platformField = shaderErrorType.GetField(
                "platform",
                BindingFlags.Public | BindingFlags.Instance
            );
            FieldInfo fileField = shaderErrorType.GetField(
                "file",
                BindingFlags.Public | BindingFlags.Instance
            );
            FieldInfo lineField = shaderErrorType.GetField(
                "line",
                BindingFlags.Public | BindingFlags.Instance
            );
            FieldInfo warningField = shaderErrorType.GetField(
                "warning",
                BindingFlags.Public | BindingFlags.Instance
            );
        ...
        public static int GetSRPBatcherCompatibilityCode(Shader s, int subShaderIdx)
        {
            return (int)
                ShaderUtilEx.Type.InvokeMember(
                    ...
                    null,
                    null,
                    new object[] { s, subShaderIdx }
                );
        }

        public static string GetSRPBatcherCompatibilityIssueReason(
            Shader s,
            int subShaderIdx,
            int err
        )
    ...
    public static class FileUtilEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEditor.FileUtil, UnityEditor")
                    : type;
            }
        }
    ...
    public static class ShaderInspectorEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEditor.ShaderInspector, UnityEditor")
                    : type;
            }
        }

        public static void ShaderErrorListUI(
            UnityEngine.Object shader,
        ...
        )
        {
            Type.InvokeMember(
                ...
                null,
                null,
                new object[] { shader, messages, scrollPosition }
            );
        }
    }
    ...
    public static class GUISkinEx
    {
        ...
        public static System.Type Type
        {
            ...
            {
                return (type == null)
                    ? type = System.Type.GetType("UnityEngine.GUISkin, UnityEngine")
                    : type;
            }
        }
        ...
        public static GUISkin GetCurrentSkin()
        {
            return (GUISkin)
                GUISkinEx
                    .Type.GetField("current", BindingFlags.NonPublic | BindingFlags.Static)
                    .GetValue(null);
        }
    }
    ...
    public static class EditorGUIEx
    {
        ...
        public static bool ButtonMouseDown(
            Rect position,
            GUIContent content,
            FocusType focusType,
            GUIStyle style
        ...
        public static float kObjectFieldMiniThumbnailHeight
        {
            ...
            {
                return (float)
                    EditorGUIEx.Type.InvokeMember(
                        "kObjectFieldMiniThumbnailHeight",
    ...
    public static class ShaderInspectorPlatformsPopupEx
    {
        ...
        {
            return (int)
                ShaderInspectorPlatformsPopupEx
                    .Type.GetProperty("currentMode", BindingFlags.Public | BindingFlags.Static)
                    .GetValue(null, null);
        }
