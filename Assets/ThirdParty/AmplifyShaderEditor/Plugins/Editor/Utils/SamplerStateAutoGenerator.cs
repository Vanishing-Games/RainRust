...
namespace AmplifyShaderEditor
{
    ...
    public class SamplerStateAutoGenerator
    {
        ...
        [SerializeField]
        private List<InlineSamplerWrapOptions> m_additionalWrapOptions =
            new List<InlineSamplerWrapOptions>();
        ...
        public void Draw(UndoParentNode owner)
        {
            ...
            NodeUtils.DrawNestedPropertyGroup(
                ref m_visibleMainFoldout,
                InlineSamplerStateStr,
                DrawMain
            );
        }
        ...
        void DrawMain()
        {
            m_filterMode = (InlineSamplerFilteringMode)
                m_owner.EditorGUILayoutEnumPopup(m_filterMode);

            EditorGUILayout.BeginHorizontal();
            m_mainWrapMode.WrapMode = (InlineSamplerWrapMode)
                m_owner.EditorGUILayoutEnumPopup(m_mainWrapMode.WrapMode);
            m_mainWrapMode.Coordinates = (InlineSamplerWrapCoordinates)
                m_owner.EditorGUILayoutEnumPopup(m_mainWrapMode.Coordinates);
            EditorGUILayout.EndHorizontal();
            NodeUtils.DrawNestedPropertyGroup(
                ref m_visibleWrapsFoldout,
                AdditionalWrapsStr,
                DrawAdditionalWrapModes,
        ...
        void DrawAdditionalWrapModes()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                float maxWidth = 90;
                EditorGUILayout.BeginHorizontal();
                m_additionalWrapOptions[i].WrapMode = (InlineSamplerWrapMode)
                    m_owner.EditorGUILayoutEnumPopup(
                        m_additionalWrapOptions[i].WrapMode,
                        GUILayout.MaxWidth(maxWidth)
                    );
                m_additionalWrapOptions[i].Coordinates = (InlineSamplerWrapCoordinates)
                    m_owner.EditorGUILayoutEnumPopup(
                        m_additionalWrapOptions[i].Coordinates,
                        GUILayout.MaxWidth(maxWidth)
                    );
                EditorGUILayout.EndHorizontal();
            }
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            Enum.TryParse<InlineSamplerWrapCoordinates>(
                nodeParams[index++],
                out m_mainWrapMode.Coordinates
            );
