...
namespace AmplifyShaderEditor
{
    ...
    public sealed class StickyNoteNode : ParentNode, ISerializationCallbackReceiver
    {
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            m_noteTextArea.yMin +=
                m_innerTitleArea.height + NOTE_AREA_ADJUST * drawInfo.InvertedZoom;
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                if (!m_startEditingNoteText && !m_isEditingNoteText)
                {
                    if (
                        ...
                        && (
                            (
                                ...
                                && m_innerTitleArea.Contains(drawInfo.MousePosition)
                            )
                        )
                    )
                    {
                        if (
                            (EditorApplication.timeSinceStartup - m_clickTimeTitle)
                            < m_doubleClickTimeTitle
                        )
                    ...
                    if (m_isEditingInnerTitle || m_startEditingInnerTitle)
                    {
                        ...
                        m_innerTitleText = EditorGUITextField(
                            m_innerTitleArea,
                            string.Empty,
                            m_innerTitleText,
                ...
                if (!m_startEditingInnerTitle && !m_isEditingInnerTitle)
                {
                    if (
                        ...
                        && (
                            (
                                !ContainerGraph.ParentWindow.MouseInteracted
                                && drawInfo.CurrentEventType == EventType.MouseDown
                                && m_noteTextArea.Contains(drawInfo.MousePosition)
                            )
                        )
                    )
                    {
                        if (
                            (EditorApplication.timeSinceStartup - m_clickTimeNote)
                            < m_doubleClickTimeNote
                        )
                    ...
                    if (m_isEditingNoteText || m_startEditingNoteText)
                    {
                        ...
                        m_noteText = EditorGUITextArea(
                            m_noteTextArea,
                            m_noteText,
                            UIUtils.StickyNoteText
                        );
