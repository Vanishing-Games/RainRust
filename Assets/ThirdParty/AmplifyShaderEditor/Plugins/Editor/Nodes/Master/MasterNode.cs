...
namespace AmplifyShaderEditor
{
    ...
    public class MasterNodeCategoriesData
    {
        ...
        public MasterNodeCategoriesData(AvailableShaderTypes category, string name)
        {
            Category = category;
            Name = name;
        }
    ...
    public class MasterNode : OutputNode
    {
        ...
        protected readonly string[] ShaderModelTypeArr =
        {
            "2.0",
            "2.5",
            "3.0",
        ...
        public MasterNode(int uniqueId, float x, float y, float width, float height)
        ...
        protected void DrawCurrentShaderType()
        {
            ...
            m_masterNodeCategory = EditorGUILayoutPopup(
                m_categoryLabel,
                m_masterNodeCategory,
                m_availableCategoryLabels
            );
        ...
        protected void DrawCustomInspector(bool dropdown)
        {
            ...
            m_customInspectorName = EditorGUILayoutTextField(
                CustomInspectorStr,
            ...
            if (!dropdown)
            {
                if (
                    GUILayoutButton(
                        string.Empty,
                        UIUtils.GetCustomStyle(CustomStyle.ResetToDefaultInspectorButton),
                        GUILayout.Width(15),
                        GUILayout.Height(15)
                    )
                )
            ...
            else
            {
                ...
                )
                {
                    ...
                    if (foundHDRP && foundURP)
                    {
                        version =
                            (
                                ASEPackageManagerHelper.CurrentHDRPBaseline
                                > ASEPackageManagerHelper.CurrentURPBaseline
                            )
                                ? ASEPackageManagerHelper.CurrentHDRPBaseline
                                : ASEPackageManagerHelper.CurrentURPBaseline;
                    }
                    ...
                    if (foundHDRP)
                    {
                        ...
                        else if (version >= ASESRPBaseline.ASE_SRP_10)
                        {
        ...
        private void AddMenuItem(GenericMenu menu, string newClass)
        {
            menu.AddItem(
                ...
                m_customInspectorName.Equals(newClass),
                OnSelection,
                newClass
            );
        }
        ...
        public void DrawShaderKeywords()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                m_shaderKeywordsFoldout = EditorGUILayout.Foldout(
                    m_shaderKeywordsFoldout,
                    ShaderKeywordsStr
                );
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        if (
                            GUILayoutButton(
                                string.Empty,
                                m_smallAddShaderKeywordItemStyle,
                        ...
                        if (
                            GUILayoutButton(
                                string.Empty,
                                m_smallRemoveShaderKeywordStyle,
                                GUILayout.Width(ShaderKeywordButtonLayoutWidth)
                            )
                        )
                        {
        ...
        public override void OnNodeRepaint(DrawInfo drawInfo)
        {
            ...
            if (m_isMainOutputNode)
            {
                ...
                m_masterNodeIconCoords.x +=
                    m_globalPosition.width - m_masterNodeOffTex.width * drawInfo.InvertedZoom;
                m_masterNodeIconCoords.y +=
                    m_globalPosition.height - m_masterNodeOffTex.height * drawInfo.InvertedZoom;
        ...
        protected void DrawInstancedIcon(DrawInfo drawInfo)
        {
            ...
            m_masterNodeIconCoords = m_globalPosition;
            m_masterNodeIconCoords.x +=
                m_globalPosition.width - 5 - m_gpuInstanceOffTex.width * drawInfo.InvertedZoom;
        ...
        {
            List<CustomExpressionNode> nodes = m_containerGraph
        ...
        protected void SortInputPorts(
            ref List<InputPort> vertexPorts,
            ref List<InputPort> fragmentPorts
        )
        {
            for (int i = 0; i < m_inputPorts.Count; i++)
            {
                if (
                    m_inputPorts[i].Category == MasterNodePortCategory.Fragment
                    || m_inputPorts[i].Category == MasterNodePortCategory.Debug
                )
                {
        ...
        protected void UpdateShaderAsset(
            ref string pathname,
        ...
        )
        {
            ...
            if (m_currentShader != null)
            {
                ...
                if (m_currentMaterial != null)
                {
                    ...
                    UIUtils.CurrentWindow.OutsideGraph.UpdateMaterialOnPropertyNodes(
                        m_currentMaterial
                    );
        ...
        private void RefreshVisibleList(ref List<PropertyNode> allNodes)
        {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                if (
                    (rnode == null || !rnode.IsInside)
                    && (
                        !m_propertyNodesVisibleList.Exists(x =>
                            x.PropertyName.Equals(nodes[i].PropertyName)
                        )
                    )
                )
            ...
            m_propertyNodesVisibleList.Sort(
                (x, y) =>
                {
        ...
        public void DrawMaterialInputs(GUIStyle toolbarstyle, bool style = true)
        {
            ...
            if (style)
            {
                ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedProperties =
                    GUILayoutToggle(
                        ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedProperties,
                        PropertyOrderFoldoutStr,
            ...
            else
            {
                ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedProperties =
                    GUILayoutToggle(
                        ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedProperties,
                        PropertyOrderTemplateFoldoutStr,
                        UIUtils.MenuItemToggleStyle
                    );
            }
            ...
            {
                m_propertyReordableList = new ReorderableList(
                    m_propertyNodesVisibleList,
                    typeof(PropertyNode),
                    true,
                    false,
                    false,
                ...
                )
                {
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        ...
                        EditorGUI.LabelField(
                            first,
                            m_propertyNodesVisibleList[index].PropertyInspectorName
                        );
                        var second = rect;
                        second.width *= 0.4f;
                        ...
                        if (
                            GUI.Button(
                                second,
                                m_propertyNodesVisibleList[index].PropertyName,
                                new GUIStyle("AssetLabel Partial")
                            )
                        )
                        {
        ...
        {
            get { return m_customInspectorName; }
        }
        public string CustomInspectorFormatted
        {
            get { return string.Format(CustomInspectorFormat, m_customInspectorName); }
        }
        public string CroppedShaderName
        {
            get { return m_croppedShaderName; }
        }
        public virtual AvailableShaderTypes CurrentMasterNodeCategory
        {
            get
            {
                return (m_masterNodeCategory == 0)
                    ? AvailableShaderTypes.SurfaceShader
                    : AvailableShaderTypes.Template;
            }
        }
        public int CurrentMasterNodeCategoryIdx
        {
            get { return m_masterNodeCategory; }
        }
        public MasterNodeDataCollector CurrentDataCollector
        {
            get { return m_currentDataCollector; }
            set { m_currentDataCollector = value; }
        }
        public List<PropertyNode> PropertyNodesVisibleList
        {
            get { return m_propertyNodesVisibleList; }
        }
        public ReorderableList PropertyReordableList
        {
            get { return m_propertyReordableList; }
        }
        public int ReordableListLastCount
        {
            get { return m_lastCount; }
        }
        public MasterNodeCategoriesData CurrentCategoriesData
        {
            get { return m_availableCategories[m_masterNodeCategory]; }
        }
        public int ShaderLOD
        {
            get { return m_shaderLOD; }
            set { m_shaderLOD = Mathf.Max(0, value); }
        }
