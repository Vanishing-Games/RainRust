...
namespace AmplifyShaderEditor
{
    ...
    public class DependenciesData
    {
        ...
        public bool IsValid
        {
            ...
            {
                return (
                    !string.IsNullOrEmpty(DependencyValue) && !string.IsNullOrEmpty(DependencyName)
                );
            }
        }
    }
    ...
    public class DependenciesHelper
    {
        ...
        void DrawMainBody()
        {
            ...
            {
                m_availableDependencies[i].DependencyFoldout =
                    m_currentOwner.EditorGUILayoutFoldout(
                        m_availableDependencies[i].DependencyFoldout,
                        string.Format("[{0}] - {1}", i, m_availableDependencies[i].DependencyName)
                    );
                if (m_availableDependencies[i].DependencyFoldout)
                {
                    ...
                    m_availableDependencies[i].DependencyName = EditorGUILayout.TextField(
                        DependencyNameStr,
                        m_availableDependencies[i].DependencyName
                    );
                    ...
                    //Dependency Value
                    EditorGUI.BeginChangeCheck();
                    m_availableDependencies[i].DependencyValue = EditorGUILayout.TextField(
                        DependencyValueStr,
                        m_availableDependencies[i].DependencyValue
                    );
                    if (EditorGUI.EndChangeCheck())
                    {
                        m_availableDependencies[i].DependencyValue =
        ...
        public string GenerateDependencies()
        {
            ...
            for (int i = 0; i < dependencyCount; i++)
            {
                ...
                {
                    result +=
                        UIUtils.ShaderIndentTabs + m_availableDependencies[i].GenerateDependency();
                }
            }
            UIUtils.ShaderIndentLevel--;
            return result;
        ...
        {
            get { return m_availableDependencies.Count > 0; }
        }
    }
}
