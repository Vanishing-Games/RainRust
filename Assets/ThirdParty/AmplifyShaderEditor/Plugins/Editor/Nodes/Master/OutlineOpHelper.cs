...
namespace AmplifyShaderEditor
{
    ...
    public sealed class OutlineOpHelper
    {
        ...
        private readonly string OutlineSurfaceConfig =
            "#pragma surface outlineSurf Outline {0} keepalpha noshadow noambient novertexlights nolightmap nodynlightmap nodirlightmap nometa noforwardadd vertex:outlineVertexDataFunc ";
        ...
        private readonly string OutlineDefaultUniformColorInstanced =
            "UNITY_DEFINE_INSTANCED_PROP( half4, _ASEOutlineColor )";
        private readonly string OutlineDefaultUniformWidthInstanced =
            "UNITY_DEFINE_INSTANCED_PROP( half, _ASEOutlineWidth )";

        private readonly string OutlineDefaultVertexHeader =
            "void outlineVertexDataFunc( inout appdata_full v, out Input o )\n\t\t{";
        private readonly string OutlineTessVertexHeader =
        ...
        private readonly string[] OutlineBodyDefaultSurfEnd = { "}", "ENDCG", "\n" };
        ...
        private const string WidthVariableAccessInstanced =
            "UNITY_ACCESS_INSTANCED_PROP( _ASEOutlineWidth )";
        ...
        private const string WidthPropertyNameInstanced =
            "UNITY_ACCESS_INSTANCED_PROP(_ASEOutlineWidth)";

        private const string ColorPropertyDec =
            "_ASEOutlineColor( \"Outline Color\", Color ) = ({0})";
        private const string OutlinePropertyDec =
        ...
        public void Draw(ParentNode owner, GUIStyle toolbarstyle, Material mat)
        {
            ...
            owner.ContainerGraph.ParentWindow.InnerWindowVariables.OutlineActiveMode =
                owner.GUILayoutToggle(
                    owner.ContainerGraph.ParentWindow.InnerWindowVariables.OutlineActiveMode,
                    EditorVariablesManager.OutlineActiveMode.LabelName,
                    UIUtils.MenuItemToggleStyle,
                    GUILayout.ExpandWidth(true)
                );
            EditorGUI.BeginChangeCheck();
            m_enabled = owner.EditorGUILayoutToggle(
                string.Empty,
                m_enabled,
                UIUtils.MenuItemEnableStyle,
                GUILayout.Width(16)
            );
            ...
            if (owner.ContainerGraph.ParentWindow.InnerWindowVariables.OutlineActiveMode)
            {
                ...
                EditorGUI.indentLevel += 1;
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    m_outlineColor = owner.EditorGUILayoutColorField(
                        OutlineColorLabel,
                        m_outlineColor
                    );
                    ...
                    EditorGUI.BeginChangeCheck();
                    m_outlineWidth = owner.EditorGUILayoutFloatField(
                        OutlineWidthLabel,
                        m_outlineWidth
                    );
                    if (EditorGUI.EndChangeCheck() && mat != null)
        ...
        public void AddToDataCollector(ref MasterNodeDataCollector dataCollector)
        {
            if (!dataCollector.UsingCustomOutlineColor)
                dataCollector.AddToProperties(
                    -1,
                    string.Format(ColorPropertyDec, IOUtils.ColorToString(m_outlineColor)),
                    -1
                );
            if (!dataCollector.UsingCustomOutlineWidth)
                dataCollector.AddToProperties(
                    -1,
                    string.Format(OutlinePropertyDec, m_outlineWidth),
                    -1
                );
        }
        ...
        public string[] OutlineFunctionBody(
            ref MasterNodeDataCollector dataCollector,
            bool instanced,
            bool isShadowCaster,
            string shaderName,
            string[] billboardInfo,
            ref TessellationOpHelper tessOpHelper,
            string target,
        ...
        )
        {
            ...
            bool customOutline =
                dataCollector.UsingCustomOutlineColor
                || dataCollector.UsingCustomOutlineWidth
                || dataCollector.UsingCustomOutlineAlpha;
            int outlineMode = customOutline
                ? m_offsetMode
                : (m_mode == OutlineMode.VertexOffset ? 0 : 1);
            string extraOptions =
                (customOutline ? m_customNoFog : m_noFog) ? "nofog " : string.Empty;
                ...
                if (isShadowCaster)
                {
                    for (int i = 0; i < InputList.Count; i++)
                    {
                        dataCollector.AddToInput(
                            InputList[i].NodeId,
                            InputList[i].PropertyName,
                            !InputList[i].IsDirective
                        );
                    }
                }
                else
            ...
            if (instanced)
            {
                    ...
                    if (isShadowCaster)
                    {
                        for (int i = 0; i < UniformList.Count; i++)
                        {
                            dataCollector.AddToUniforms(
                                UniformList[i].NodeId,
                                UniformList[i].PropertyName
                            );
                        }
                ...
                if (!dataCollector.UsingCustomOutlineColor)
                    body.Add(
                        precision == PrecisionType.Float
                            ? OutlineDefaultUniformColorInstanced.Replace("half", "float")
                            : OutlineDefaultUniformColorInstanced
                    );

                if (!dataCollector.UsingCustomOutlineWidth)
                    body.Add(
                        precision == PrecisionType.Float
                            ? OutlineDefaultUniformWidthInstanced.Replace("half", "float")
                            : OutlineDefaultUniformWidthInstanced
                    );
                ...
                if (tessOpHelper.EnableTesselation && !isShadowCaster)
                {
                    ...
                    body.Add(
                        tessOpHelper
                            .GetCurrentTessellationFunction(ref dataCollector)
                            .Trim('\t', '\n') + "\n"
                    );
                }
                ...
                switch (outlineMode)
                {
                    case 0:
                        body.Add(
                            string.Format(
                                OutlineVertexOffsetMode,
                                dataCollector.UsingCustomOutlineWidth
                                    ? "outlineVar"
                                    : WidthPropertyNameInstanced
                            )
                        );
                        break;
                    case 1:
                ...
                if (
                    dataCollector.UsingCustomOutlineColor
                    || dataCollector.CustomOutlineSelectedAlpha > 0
                )
                {
            ...
            else
            {
                ...
                if (!dataCollector.UsingCustomOutlineColor)
                    body.Add(
                        precision == PrecisionType.Float
                            ? OutlineDefaultUniformColor.Replace("half", "float")
                            : OutlineDefaultUniformColor
                    );

                if (!dataCollector.UsingCustomOutlineWidth)
                    body.Add(
                        precision == PrecisionType.Float
                            ? OutlineDefaultUniformWidth.Replace("half", "float")
                            : OutlineDefaultUniformWidth
                    );
        ...
        public bool UsingCullMode
        {
            get { return m_cullMode != CullMode.Front; }
        }
        public bool UsingZWrite
        {
            get { return m_zWriteMode != 0; }
        }
        public bool UsingZTest
        {
            get { return m_zTestMode != 0; }
        }
        public int ZWriteMode
        {
            get { return m_zWriteMode; }
            set { m_zWriteMode = value; }
        }
        public int ZTestMode
        {
            get { return m_zTestMode; }
            set { m_zTestMode = value; }
        }
        public CullMode OutlineCullMode
        {
        ...
        {
            get { return m_standardAdditionalDirectives; }
            set { m_standardAdditionalDirectives = value; }
        }
        ...
        public string GrabPasses
        {
            get { return m_grabPasses; }
            set { m_grabPasses = value; }
        }
        public List<PropertyDataCollector> InputList
        {
            get { return m_inputList; }
            set { m_inputList = value; }
        }
        public List<PropertyDataCollector> UniformList
        {
            get { return m_uniformList; }
            set { m_uniformList = value; }
        }
        public List<PropertyDataCollector> InstancedPropertiesList
        {
            get { return m_instancedPropertiesList; }
            set { m_instancedPropertiesList = value; }
        }

        public Dictionary<string, string> LocalFunctions
        {
            get { return m_localFunctions; }
            set { m_localFunctions = value; }
        }
        public bool DirtyInput
        {
            get { return m_dirtyInput; }
            set { m_dirtyInput = value; }
        }

        //public OutlineMode CustomMode { get { return m_customMode; } set { m_customMode = value; } }
        public int OffsetMode
        {
            get { return m_offsetMode; }
            set { m_offsetMode = value; }
        }
        public bool CustomNoFog
        {
            get { return m_customNoFog; }
            set { m_customNoFog = value; }
        }
    }
}
