                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        if (m_items[index] != null)
                        {
                            ...
                            Rect labelPos = new Rect(
                                rect.x + popupPos.width * labelWidthMultiplier,
                                rect.y,
                                0.59f * rect.width,
                                rect.height
                            );

                            Rect shaderSelectorPos = new Rect(
                                labelPos.x + labelPos.width * shaderSelectorMultiplier,
                                rect.y,
                                15,
                                rect.height
                            );
                            ...
                            Rect buttonPlusPos = new Rect(
                                shaderSelectorPos.x
                                    + shaderSelectorPos.width * buttonPlusPosMultiplier,
                                rect.y,
                                ShaderKeywordButtonLayoutWidth,
                                rect.height
                            );
                            Rect buttonMinusPos = new Rect(
                                buttonPlusPos.x + buttonPlusPos.width,
                                rect.y,
                                ShaderKeywordButtonLayoutWidth,
                                rect.height
                            );
                            ...
                            m_items[index].Location = (UsePassLocation)
                                owner.EditorGUIEnumPopup(popupPos, m_items[index].Location);

                            if (
                                EditorGUI.EndChangeCheck()
                                && m_items[index].Location == UsePassLocation.Below
                                && m_owner != null
                                && m_owner.ContainerGraph.CurrentCanvasMode
                                    == NodeAvailability.TemplateShader
                            )
                            {
                                m_items[index].Location = UsePassLocation.Above;
                                UIUtils.ShowMessage(
                                    "Below option still not available on templates"
                                );
                            }
                            m_items[index].Value = owner.EditorGUITextField(
                                labelPos,
                                string.Empty,
                                m_items[index].Value
                            );
                            ...
                            )
                            {
                                ...
                                DisplayShaderContext(
                                    owner,
                                    GUILayoutUtility.GetRect(GUIContent.none, EditorStyles.popup)
                                );
                            }
...
#pragma warning restore 0618
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    UsePassLocation location = (UsePassLocation)
                        Enum.Parse(typeof(UsePassLocation), locationValue);
        ...
        public void BuildUsePassInfo(
            MasterNodeDataCollector dataCollector,
            ref string aboveItems,
            ref string bellowItems,
            string tabs
        )
        {
            int count = 0;
            ...
            for (int i = 0; i < count; i++)
            {
                aboveItems +=
                    tabs
                    + string.Format(
                        UseGrabFormatNewLine,
                        dataCollector.AboveUsePassesList[i].PropertyName
                    );
            }
            ...
            for (int i = 0; i < count; i++)
            {
                bellowItems +=
                    ...
                    + string.Format(
                        UseGrabFormatNewLine,
                        dataCollector.BelowUsePassesList[i].PropertyName
                    );
            }
        ...
        public void BuildUsePassInfo(
            MasterNodeDataCollector dataCollector,
            ref List<PropertyDataCollector> aboveItems,
            ref List<PropertyDataCollector> bellowItems
        )
        {
            int count = 0;
            count = dataCollector.AboveUsePassesList.Count;
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_items[i].Location == UsePassLocation.Above)
                {
                    aboveItems.Add(
                        new PropertyDataCollector(
                            -1,
                            string.Format(UseGrabFormat, m_items[i].Value)
                        )
                    );
                }
                else
