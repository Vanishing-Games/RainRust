...
namespace AmplifyShaderEditor
{
    ...
    public class RenderingOptionsOpHelper
    {
        ...
        private static readonly GUIContent EmissionGIFlags = new GUIContent(
            "Emission GI Flag",
            "Modifies Emission GI flags"
        );
        private static readonly GUIContent LODCrossfadeContent = new GUIContent(
            " LOD Group Cross Fade",
            "Applies a dither crossfade to be used with LOD groups for smoother transitions. Uses one interpolator\nDefault: OFF"
        );
        private static readonly GUIContent DisableBatchingContent = new GUIContent(
            "Disable Batching",
            "\nDisables objects to be batched and used with DrawCallBatching Default: False"
        );
        private static readonly GUIContent IgnoreProjectorContent = new GUIContent(
            " Ignore Projector",
            "\nIf True then an object that uses this shader will not be affected by Projectors Default: False"
        );
        private static readonly GUIContent UseDefaultCasterContent = new GUIContent(
            " Use Default Shadow Caster",
            "\nIf True always use surface default shadow caster Default: False"
        );
        private static readonly GUIContent ForceNoShadowCastingContent = new GUIContent(
        ...
        private static readonly GUIContent ForceEnableInstancingContent = new GUIContent(
            " Force Enable Instancing",
            "\nIf True forces instancing on shader independent of having instanced properties"
        );
        private static readonly GUIContent ForceDisableInstancingContent = new GUIContent(
            " Force Disable Instancing",
            "\nIf True forces disable instancing on shader independent of having instanced properties"
        );
        private static readonly GUIContent SpecularHightlightsContent = new GUIContent(
        ...
        private static readonly GUIContent ReflectionsContent = new GUIContent(
        ...
        public RenderingOptionsOpHelper()
        {
            ...
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Exclude Deferred", "exclude_path:deferred")
            );
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Exclude Forward", "exclude_path:forward")
            );
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Exclude Legacy Deferred", "exclude_path:prepass")
            );
            ...
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Per Vertex Light", "novertexlights")
            );
            m_codeGenerationDataList.Add(new CodeGenerationData(" Lightmaps", "nolightmap "));
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Dynamic Global GI", "nodynlightmap")
            );
            m_codeGenerationDataList.Add(
                new CodeGenerationData(" Directional lightmaps", "nodirlightmap")
            );
        ...
        public void Draw(StandardSurfaceOutputNode owner)
        {
            ...
            NodeUtils.DrawPropertyGroup(
                ...
                () =>
                {
                    ...
                    {
                        m_codeGenerationDataList[i].IsActive = !owner.EditorGUILayoutToggleLeft(
                            m_codeGenerationDataList[i].Name,
                            !m_codeGenerationDataList[i].IsActive
                        );
                    }
                    m_lodCrossfade = owner.EditorGUILayoutToggleLeft(
                        LODCrossfadeContent,
                        m_lodCrossfade
                    );
                    m_ignoreProjector = owner.EditorGUILayoutToggleLeft(
                        IgnoreProjectorContent,
                        m_ignoreProjector
                    );
                    EditorGUI.BeginDisabledGroup(!owner.CastShadows);
                    m_useDefaultShadowCaster = owner.EditorGUILayoutToggleLeft(
                        UseDefaultCasterContent,
                        m_useDefaultShadowCaster
                    );
                    EditorGUI.EndDisabledGroup();
                    m_forceNoShadowCasting = owner.EditorGUILayoutToggleLeft(
                        ForceNoShadowCastingContent,
                    ...
                    {
                        m_forceEnableInstancing = owner.EditorGUILayoutToggleLeft(
                            ForceEnableInstancingContent,
                            m_forceEnableInstancing
                        );
                    }

                    m_forceDisableInstancing = owner.EditorGUILayoutToggleLeft(
                        ForceDisableInstancingContent,
                        m_forceDisableInstancing
                    );
                    m_specularHighlightToggle = owner.EditorGUILayoutToggleLeft(
                        SpecularHightlightsContent,
                    ...
                    m_disableBatching = (DisableBatchingTagValues)
                        owner.EditorGUILayoutEnumPopup(DisableBatchingContent, m_disableBatching);
        ...
        public void Build(ref string OptionalParameters)
        {
            ...
            for (int i = 0; i < codeGenCount; i++)
            {
                ...
                {
                    OptionalParameters +=
                        m_codeGenerationDataList[i].Value + Constants.OptionalParametersSep;
                }
            }
            ...
            {
                OptionalParameters +=
                    Constants.LodCrossFadeOption2017 + Constants.OptionalParametersSep;
            }
        }
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            {
                m_disableBatching = (DisableBatchingTagValues)
                    Enum.Parse(typeof(DisableBatchingTagValues), nodeParams[index++]);
