...
namespace AmplifyShaderEditor
{
    ...
    public class CommonBlendTypes
    {
        ...
        public CommonBlendTypes(
            string name,
            AvailableBlendFactor sourceFactor,
            AvailableBlendFactor destFactor
        )
        {
            Name = name;
    ...
    public class BlendOpsHelper
    {
        ...
        public void Draw(UndoParentNode owner, bool customBlendAvailable)
        {
            ...
            m_currentIndex = owner.EditorGUILayoutPopup(
                BlendModesRGBStr,
                m_currentIndex,
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (m_currentIndex > 1)
                {
                    m_sourceFactorRGB.IntValue = (int)
                        m_commonBlendTypes[m_currentIndex].SourceFactor;
            ...
            m_sourceFactorRGB.CustomDrawer(
                ref owner,
                (x) =>
                {
                    tempCast = (AvailableBlendFactor)
                        x.EditorGUILayoutEnumPopup(SourceFactorStr, tempCast);
                },
                SourceFactorStr
            );
            ...
            m_destFactorRGB.CustomDrawer(
                ref owner,
                (x) =>
                {
                    tempCast = (AvailableBlendFactor)
                        x.EditorGUILayoutEnumPopup(DstFactorStr, tempCast);
                },
                DstFactorStr
            );
            ...
            m_blendOpRGB.CustomDrawer(
                ref owner,
                (x) =>
                {
                    m_blendOpRGB.IntValue = x.EditorGUILayoutPopup(
                        BlendOpsRGBStr,
            ...
            {
                m_blendOpEnabled =
                    (!m_blendOpRGB.Active && m_blendOpRGB.IntValue > -1)
                    || (m_blendOpRGB.Active && m_blendOpRGB.NodeId > -1); //AvailableBlendOps.OFF;
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    m_sourceFactorAlpha.IntValue = (int)
                        m_commonBlendTypes[m_currentAlphaIndex].SourceFactor;
                    m_sourceFactorAlpha.SetInlineNodeValue();

                    m_destFactorAlpha.IntValue = (int)
                        m_commonBlendTypes[m_currentAlphaIndex].DestFactor;
            ...
            m_sourceFactorAlpha.CustomDrawer(
                ref owner,
                (x) =>
                {
                    tempCast = (AvailableBlendFactor)
            ...
            m_destFactorAlpha.CustomDrawer(
                ref owner,
                (x) =>
                {
                    tempCast = (AvailableBlendFactor)
            ...
            m_blendOpAlpha.CustomDrawer(
                ref owner,
                (x) =>
                {
                    m_blendOpAlpha.IntValue = x.EditorGUILayoutPopup(
                        BlendOpsAlphaStr,
                        m_blendOpAlpha.IntValue,
                        BlendOpsLabels
                    );
                },
                BlendOpsAlphaStr
        ...
        void CheckRGBIndex()
        {
            ...
            {
                if (
                    m_commonBlendTypes[i].SourceFactor
                        == (AvailableBlendFactor)m_sourceFactorRGB.IntValue
                    && m_commonBlendTypes[i].DestFactor
                        == (AvailableBlendFactor)m_destFactorRGB.IntValue
                )
                {
                    m_currentIndex = i;
                    return;
        ...
        void CheckAlphaIndex()
        {
            ...
            for (int i = 1; i < count; i++)
            {
                if (
                    m_commonBlendTypes[i].SourceFactor
                        == (AvailableBlendFactor)m_sourceFactorAlpha.IntValue
                    && m_commonBlendTypes[i].DestFactor
                        == (AvailableBlendFactor)m_destFactorAlpha.IntValue
                )
                {
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            else
            {
                ...
                m_destFactorAlpha.IntValue = (int)
                    (AvailableBlendFactor)
                        Enum.Parse(typeof(AvailableBlendFactor), nodeParams[index++]);
                m_blendOpRGB.IntValue = (int)
                    (AvailableBlendOps)Enum.Parse(typeof(AvailableBlendOps), nodeParams[index++]);
                m_blendOpAlpha.IntValue = (int)
                    (AvailableBlendOps)Enum.Parse(typeof(AvailableBlendOps), nodeParams[index++]);
            }
            ...
            m_blendOpEnabled =
                (!m_blendOpRGB.Active && m_blendOpRGB.IntValue > -1)
                || (m_blendOpRGB.Active && m_blendOpRGB.NodeId > -1);
        }
        ...
        public string CurrentBlendRGB
        {
            get { return m_commonBlendTypes[m_currentIndex].Name; }
        }
        ...
        public string CurrentBlendFactorSingle
        {
            get
            {
                return string.Format(
                    ...
                    m_sourceFactorRGB.GetValueOrProperty(
                        ((AvailableBlendFactor)m_sourceFactorRGB.IntValue).ToString()
                    ),
                    m_destFactorRGB.GetValueOrProperty(
                        ((AvailableBlendFactor)m_destFactorRGB.IntValue).ToString()
                    )
                );
            }
        }
        ...
        public string CurrentBlendFactorSeparate
        {
            get
            {
                ...
                string srca = m_sourceFactorAlpha.GetValueOrProperty(
                    ((AvailableBlendFactor)m_sourceFactorAlpha.IntValue).ToString()
                );
                string dsta = m_destFactorAlpha.GetValueOrProperty(
                    ((AvailableBlendFactor)m_destFactorAlpha.IntValue).ToString()
                );
        ...
        public string CurrentBlendOpSingle
        {
            get
            {
                string value = m_blendOpRGB.GetValueOrProperty(
                    ((AvailableBlendOps)m_blendOpRGB.IntValue).ToString()
                );
        ...
        public string CurrentBlendOpSeparate
        {
            get
            {
                string rgbValue = m_blendOpRGB.GetValueOrProperty(
                    ((AvailableBlendOps)m_blendOpRGB.IntValue).ToString()
                );
                ...
                string alphaValue = m_blendOpAlpha.GetValueOrProperty(
                    ((AvailableBlendOps)m_blendOpAlpha.IntValue).ToString()
                );
                return string.Format(
                    SeparateBlendOpStr,
                    (m_currentIndex > 0 ? rgbValue : AvailableBlendOps.Add.ToString()),
                    alphaValue
                );
            }
        }
        public string CurrentBlendOp
        {
            get
            {
                return (
                    (
                        m_currentAlphaIndex > 0
                        && m_blendOpAlpha.GetValueOrProperty(
                            ((AvailableBlendOps)m_blendOpAlpha.IntValue).ToString()
                        ) != AvailableBlendOps.OFF.ToString()
                    )
        ...
        public bool Active
        {
            get { return m_enabled && (m_currentIndex > 0 || m_currentAlphaIndex > 0); }
        }
    }
}
