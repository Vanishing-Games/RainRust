...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TessellationOpHelper
    {
        ...
        private const string TessProperty =
            "_TessValue( \"Max Tessellation\", Range( 1, 32 ) ) = {0}";
        ...
        private const string TessFunctionOpen =
            "\t\tfloat4 tessFunction( {0} v0, {0} v1, {0} v2 )\n\t\t{{\n";
        ...
        private const string EdgeLengthTessProperty =
            "_EdgeLength ( \"Edge length\", Range( 2, 50 ) ) = {0}";
        ...
        private const string EdgeLengthTessMaxDispProperty =
        ...
        )
        {
            ...
            )
            {
                ...
                if (m_phongEnabled)
                {
                    EditorGUI.indentLevel += 1;
                    EditorGUI.BeginChangeCheck();
                    m_phongStrength = owner.EditorGUILayoutSlider(
                        PhongStrengthContent,
                        m_phongStrength,
                        0.0f,
                        1.0f
                    );
                ...
                m_tessType = owner.EditorGUILayoutIntPopup(
                    TesselationTypeStr,
                    m_tessType,
                    TesselationTypeLabels,
                ...
                switch (m_tessType)
                {
                    case 0:
                        {
                            EditorGUI.BeginChangeCheck();
                            m_tessFactor = owner.EditorGUILayoutSlider(
                                TessFactorContent,
                                m_tessFactor,
                                1,
                                32
                            ...
                            EditorGUI.BeginChangeCheck();
                            m_tessMinDistance = owner.EditorGUILayoutFloatField(
                                TessMinDistanceContent,
                                m_tessMinDistance
                            );
                            if (EditorGUI.EndChangeCheck() && mat != null)
                            {
                            ...
                            EditorGUI.BeginChangeCheck();
                            m_tessMaxDistance = owner.EditorGUILayoutFloatField(
                                TessMaxDistanceContent,
                                m_tessMaxDistance
                            );
                            if (EditorGUI.EndChangeCheck() && mat != null)
                            {
                    ...
                    case 3:
                        {
                            ...
                            EditorGUI.BeginChangeCheck();
                            m_tessMaxDistance = owner.EditorGUILayoutFloatField(
                                EdgeLengthTessMaxDisplacementContent,
                                m_tessMaxDistance
                            );
                            if (EditorGUI.EndChangeCheck() && mat != null)
                            {
        ...
        public void AddToDataCollector(ref MasterNodeDataCollector dataCollector, int reorder)
        {
                ...
                case 0:
                    {
                        ...
                        if (!m_hasCustomFunction)
                        {
                            ...
                            dataCollector.AddToProperties(
                                -1,
                                string.Format(TessProperty, m_tessFactor),
                                orderIndex++
                            );
                            ...
                            dataCollector.AddToProperties(
                                -1,
                                string.Format(TessMinProperty, m_tessMinDistance),
                                orderIndex++
                            );
                            dataCollector.AddToUniforms(
                                -1,
                            ...
                            dataCollector.AddToProperties(
                                -1,
                                string.Format(TessMaxProperty, m_tessMaxDistance),
                                orderIndex++
                            );
                            dataCollector.AddToUniforms(
                                -1,
                ...
                case 2:
                    {
                        ...
                        if (!m_hasCustomFunction)
                        {
                            dataCollector.AddToProperties(
                                -1,
                                string.Format(EdgeLengthTessProperty, m_tessFactor),
                                orderIndex++
                            );
                            dataCollector.AddToUniforms(
                                -1,
                ...
                case 3:
                    {
                        ...
                        if (!m_hasCustomFunction)
                        {
                            ...
                            dataCollector.AddToProperties(
                                -1,
                                string.Format(EdgeLengthTessMaxDispProperty, m_tessMaxDistance),
                                orderIndex++
                            );
                            dataCollector.AddToUniforms(
                                -1,
            ...
            if (m_phongEnabled)
            {
                dataCollector.AddToProperties(
                    -1,
                    string.Format(PhongStrengthProperty, m_phongStrength),
                    orderIndex++
                );
                dataCollector.AddToUniforms(
                    -1,
        ...
        public void WriteToOptionalParams(ref string optionalParams)
        {
            ...
            {
                optionalParams +=
                    TessellationOpHelper.PhongStrengthParam + Constants.OptionalParametersSep;
            }
        }
        ...
        {
            string finalTessFunctionOpen = string.Format(
                TessFunctionOpen,
                dataCollector.SurfaceVertexStructure
            );

            if (m_hasCustomFunction)
            {
                return finalTessFunctionOpen + m_customFunction + TessFunctionClose;
            }
            ...
            switch (m_tessType)
            {
                case 0:
                    {
                        tessFunction =
                            finalTessFunctionOpen + DistBasedTessFunctionBody + TessFunctionClose;
                    }
                    break;
                case 1:
                    {
                        tessFunction =
                            FixedAmountTessFunctionOpen
                ...
                case 2:
                    {
                        tessFunction =
                            finalTessFunctionOpen + EdgeLengthTessFunctionBody + TessFunctionClose;
                    }
                    break;
                case 3:
                    {
                        tessFunction =
                            finalTessFunctionOpen
