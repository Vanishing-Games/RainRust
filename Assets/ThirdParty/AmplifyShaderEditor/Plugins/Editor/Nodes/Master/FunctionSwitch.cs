    ...
    public sealed class FunctionSwitch : ParentNode
    {
        ...
        )
        {
            ...
            if (
                ...
                && arrayPos == m_currentSelectedInput
            )
            {
        ...
        )
        {
            ...
            if (m_activeConnections == 1)
                if (m_inputPorts[m_currentSelectedInput].IsConnected)
                    m_inputPorts[m_currentSelectedInput]
                        .GetOutputNode()
        ...
        public void ChangeSignalPropagation()
        {
            ...
            )
            {
                if (m_inputPorts[m_previousSelectedInput].IsConnected)
                    m_inputPorts[m_previousSelectedInput]
                        .GetOutputNode()
                        .DeactivateNode(m_inputPorts[m_previousSelectedInput].PortId, false);
                ...
                if (m_inputPorts[m_currentSelectedInput].IsConnected)
                    m_inputPorts[m_currentSelectedInput]
                        ...
                        .ActivateNode(
                            UniqueId,
                            m_inputPorts[m_currentSelectedInput].PortId,
                            m_activeType
                        );
            }
        }
        ...
        public bool DrawOption(ParentNode owner, bool forceDraw = false)
        {
            ...
            if (m_toggleMode)
            {
                m_currentSelectedInput = owner.EditorGUILayoutToggle(
                    m_optionLabel,
                    (m_currentSelectedInput != 0 ? true : false)
                )
                    ? 1
                    : 0;
            ...
            {
                m_currentSelectedInput = owner.EditorGUILayoutIntPopup(
                    m_optionLabel,
                    m_currentSelectedInput,
                    AvailableInputsLabels,
                    AvailableInputsValues
                );
        ...
        public void CheckReference()
        {
            ...
            if (
                ...
                || m_refMaxInputs != m_functionSwitchReference.MaxAmountInputs
        ...
        void UpdateFromSelected()
        {
            ...
            if (m_functionSwitchReference != null)
            {
                ...
                SetCurrentSelectedInput(
                    m_functionSwitchReference.GetCurrentSelectedInput(),
                    m_currentSelectedInput
                );
            }
        ...
        public override void DrawProperties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                if (m_referenceType == TexReferenceType.Instance)
                {
                    ...
                    m_referenceUniqueId = EditorGUILayout.IntPopup(
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    m_maxAmountInputs = EditorGUILayoutIntSlider(
                        MaxAmountStr,
                        m_maxAmountInputs,
                        2,
                        MaxAllowedAmount
                    ...
                    for (int i = 0; i < m_maxAmountInputs; i++)
                    {
                        ...
                        if (EditorGUI.EndChangeCheck())
                        {
                            ...
                            m_inputPorts[i].Name = UIUtils.RemoveInvalidEnumCharacters(
                                m_inputPorts[i].Name
                            );
                ...
                if (m_toggleMode)
                {
                    ...
                    m_currentSelectedInput = EditorGUILayoutToggle(
                        CurrSelectedStr,
                        (m_currentSelectedInput != 0 ? true : false)
                    )
                        ? 1
                ...
                else
                {
                    EditorGUI.BeginChangeCheck();
                    int prevVal = m_currentSelectedInput;
                    m_currentSelectedInput = EditorGUILayoutIntPopup(
                        CurrSelectedStr,
                        m_currentSelectedInput,
                        AvailableInputsLabels,
                        AvailableInputsValues
            ...
            if (UIUtils.CurrentShaderVersion() > 14205)
            {
                if (m_referenceType == TexReferenceType.Instance)
                {
                    m_functionSwitchReference =
                        UIUtils.GetNode(m_referenceUniqueId) as FunctionSwitch;
                    UpdateFromSelected();
                }
            }
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_validReference)
            {
                SetAdditonalTitleTextOnCallback(
                    m_functionSwitchReference.OptionLabel,
            ...
            else
            {
                ...
                if (m_editing)
                {
                    ...
                    else
                    {
                        EditorGUI.BeginChangeCheck();
                        int prevVal = m_currentSelectedInput;
                        m_currentSelectedInput = EditorGUIIntPopup(
                            m_varRect,
                            m_currentSelectedInput,
                            AvailableInputsLabels,
                            AvailableInputsValues,
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD2)
            {
                if (m_validReference)
                {
                    ...
                    else
                    {
                        GUI.Label(
                            m_varRect,
                            m_functionSwitchReference.AvailableInputsLabels[m_currentSelectedInput],
                            UIUtils.GraphDropDown
                        );
                    }
                ...
                else
                {
                    if (!m_editing)
                    {
                        ...
                        else
                        {
                            GUI.Label(
                                m_varRect,
                                AvailableInputsLabels[m_currentSelectedInput],
                                UIUtils.GraphDropDown
                            );
        ...
        )
        {
            if (m_inputPorts[m_currentSelectedInput].IsConnected)
                m_inputPorts[m_currentSelectedInput]
                    .GetOutputNode()
                    .PropagateNodeData(nodeData, ref dataCollector);
        }
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_functionSwitchReference != null ? m_functionSwitchReference.UniqueId : -1)
            );
        }
