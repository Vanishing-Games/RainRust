...
namespace AmplifyShaderEditor
{
    ...
    public class RenderingPlatformOpHelper
    {
        ...
        public static readonly RenderPlatformInfo[] RenderingPlatformsInfo =
        {
            ...
            {
                Label = " Xbox Series X",
                Value = RenderPlatforms.xboxseries,
            },
            new RenderPlatformInfo()
            {
                Label = " PlayStation",
                Value = RenderPlatforms.playstation,
            },
            ...
            new RenderPlatformInfo()
            {
                ...
                Value = RenderPlatforms.@switch,
            },
        };

        // Values from this dictionary must be the indices corresponding from the list above
        public static readonly Dictionary<RenderPlatforms, int> PlatformToIndex = new Dictionary<
            RenderPlatforms,
            int
        >()
        {
        ...
        public static readonly List<RenderPlatforms> LegacyIndexToPlatform =
            new List<RenderPlatforms>()
            {
        ...
        public void Draw(ParentNode owner)
        {
            ...
            NodeUtils.DrawPropertyGroup(
                ref value,
                RenderingPlatformsStr,
                () =>
                {
                    for (int i = 0; i < m_renderingPlatformValues.Length; i++)
                    {
                        m_renderingPlatformValues[i] = owner.EditorGUILayoutToggleLeft(
                            RenderingPlatformsInfo[i].Label,
                            m_renderingPlatformValues[i]
                        );
                    }
                }
            );
            owner.ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedRenderingPlatforms =
                value;
        }
        ...
        public void ReadFromString(ref uint index, ref string[] nodeParams)
        {
            ...
            else
            {
                ...
                if (count > 0)
                {
                    RenderPlatforms firstPlatform = (RenderPlatforms)
                        Enum.Parse(typeof(RenderPlatforms), nodeParams[index++]);
                ...
                for (int i = 0; i < m_renderingPlatformValues.Length; i++)
                {
                    if (m_renderingPlatformValues[i])
                    {
                        IOUtils.AddFieldValueToString(
                            ref nodeInfo,
                            RenderingPlatformsInfo[i].Value
                        );
                    }
                }
            }
        ...
        public bool LoadedFromTemplate
        {
            get { return m_loadedFromTemplate; }
        }
