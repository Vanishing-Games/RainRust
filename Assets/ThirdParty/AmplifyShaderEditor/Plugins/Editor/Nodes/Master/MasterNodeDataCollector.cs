...
namespace AmplifyShaderEditor
{
    ...
    public class TextureDefaultsDataColector
    {
        ...
        {
            get { return m_names.ToArray(); }
        }
        public Texture[] ValuesArr
        {
    ...
    public class MasterNodeDataCollector
    {
        ...
        private TextureChannelUsage[] m_requireTextureProperty =
        {
            TextureChannelUsage.Not_Used,
            TextureChannelUsage.Not_Used,
            TextureChannelUsage.Not_Used,
            TextureChannelUsage.Not_Used,
        };
        ...
        public void OpenPerVertexHeader(bool includeCustomData)
        {
            string appData =
                "inout "
                + (m_dirtyAppData ? Constants.CustomAppDataFullName : Constants.AppDataFullName)
                + " ";
            ...
            else
            {
                m_vertexData =
                    ...
                    + (
                        includeCustomData
                            ? (string.Format(", out Input {0}", Constants.VertexShaderOutputStr))
                            : string.Empty
                    )
                    + " )\n\t\t{\n";
                if (includeCustomData)
                    m_vertexData += string.Format(
                        "\t\t\tUNITY_INITIALIZE_OUTPUT( Input, {0} );\n",
        ...
        public void AddToVertexNormal(string value)
        {
            ...
            m_vertexData +=
                "\t\t\t" + Constants.VertexShaderInputStr + ".normal = " + value + ";\n";
        }
        ...
        public void AddToVertexTangent(string value)
        {
            ...
            m_vertexData +=
                "\t\t\t" + Constants.VertexShaderInputStr + ".tangent = " + value + ";\n";
        }

        public void AddVertexInstruction(string value, int nodeId = -1, bool addDelimiters = true)
        {
            if (
                !m_dirtyPerVertexData && !IsOutlineDataCollector /*&& !(m_usingCustomOutlineColor || m_usingCustomOutlineWidth)*/
            )
            {
                OpenPerVertexHeader(true);
            }
        ...
        {
            string value =
                UIUtils.PrecisionWirePortToCgType(precision, dataType) + " " + interpName;
            ...
            if (!m_customShadowCoordsDict.ContainsKey(interpName))
            {
                ...
                m_customShadowCoordsDict.Add(
                    interpName,
                    new InputCoordsCollector(nodeId, interpName, dataType, precision, slot, index)
                );
        ...
        )
        {
            ...
            AddToInput(
                nodeId,
                UIUtils.GetInputDeclarationFromType(precision, surfaceInput),
                addSemiColon
            );
        }
        ...
        public void TabifyInstancedVars()
        {
            ...
            {
                m_instancedPropertiesList[i].PropertyName =
                    '\t' + m_instancedPropertiesList[i].PropertyName;
            }
        }
        ...
        public void OptimizeInstancedProperties()
        {
            if (m_instancedPropertiesList.Count > 0)
            {
                ...
                m_instancedPropertiesList.Sort(
                    (x, y) =>
                    {
                        return GetWeightForInstancedType(x.DataType)
                            .CompareTo(GetWeightForInstancedType(y.DataType));
                    }
                );
        ...
        public void SetupInstancePropertiesBlock(string blockName)
        {
            ...
            {
                m_instancedProperties =
                    string.Format(IOUtils.InstancedPropertiesBeginTabs, blockName)
                    + m_instancedProperties
                    + IOUtils.InstancedPropertiesEndTabs;
            }
        }

        public void AddToDotsProperties(
            WirePortDataType dataType,
            int nodeId,
            string value,
            int orderIndex,
        ...
        )
        {
            ...
            string prop = string.Format(
                IOUtils.DotsInstancedPropertiesData,
                UIUtils.PrecisionWirePortToCgType(precision, dataType),
                value
            );
            string define = string.Format(
                IOUtils.DotsInstancedDefinesData,
                UIUtils.PrecisionWirePortToCgType(precision, dataType),
                value
            );
            ...
            {
                PropertyDataCollector dataColl = new PropertyDataCollector(
                    nodeId,
                    prop,
                    orderIndex
                );
        ...
        public void AddToInstancedProperties(
            WirePortDataType dataType,
            int nodeId,
            string value,
            int orderIndex
        )
        {
            ...
            if (
                !m_instancedPropertiesDict.ContainsKey(value)
        ...
        public void CopyPropertiesFromDataCollector(MasterNodeDataCollector dataCollector)
        {
            ...
            {
                AddToProperties(
                    dataCollector.PropertiesList[i].NodeId,
            ...
            m_templateDataCollector.CopySRPPropertiesFromDataCollector(
                -1,
        ...
        {
            List<PropertyDataCollector> list = new List<PropertyDataCollector>(
                m_propertiesDict.Values
            );
        ...
        public void CleanUpList(ref List<PropertyDataCollector> list)
        {
            ...
            if (list[list.Count - 1].PropertyName.Contains("[Header("))
            {
                ...
                Match match = Regex.Match(
                    list[list.Count - 1].PropertyName,
                    TemplateHelperFunctions.PropertiesPatternI
                );
                if (!match.Success)
                {
        ...
        public void AddGrabPass(string value)
        {
            ...
            m_grabPassList.Add(
                new PropertyDataCollector(
                    -1,
                    m_grabPass.Replace("\t", string.Empty).Replace("\n", string.Empty)
                )
            );
        ...
        public string GenerateInstanced(
        ...
        )
        {
            if (IsSRP)
            {
                return string.Format(
                    IOUtils.LWSRPInstancedPropertiesElement,
                    UIUtils.PrecisionWirePortToCgType(precisionType, dataType),
                    propertyName
                );
            }
            else
            {
                return string.Format(
                    IOUtils.InstancedPropertiesElement,
                    UIUtils.PrecisionWirePortToCgType(precisionType, dataType),
                    propertyName
                );
            }
        ...
        public void SoftRegisterUniform(TemplateShaderPropertyData data)
        {
            bool excludeUniformKeyword =
                (data.PropertyType == PropertyType.InstancedProperty) || IsSRP;

            string uniformName = data.IsMacro
                ? data.PropertyName
                : UIUtils.GenerateUniformName(
                    excludeUniformKeyword,
            ...
            string instancedUniform = GenerateInstanced(
                PrecisionType.Float,
                data.PropertyDataType,
                data.PropertyName
            );
            ...
            instancedUniform = GenerateInstanced(
                PrecisionType.Half,
                data.PropertyDataType,
                data.PropertyName
            );
        ...
        public void AddToUniforms(
            int nodeId,
            string dataType,
            string dataName,
            bool checkSRPBatch = false,
            bool excludeUniform = false
        )
        {
        ...
        {
            if (
                Using2DMacrosMask == MacrosMask.NONE
                && Using3DMacrosMask == MacrosMask.NONE
                && UsingCUBEMacrosMask == MacrosMask.NONE
                && Using2DArrayMacrosMask == MacrosMask.NONE
            )
                return;
        ...
        public void AddToDirectives(
            string value,
            int orderIndex = -1,
            AdditionalLineType type = AdditionalLineType.Custom
        )
        {
            if (string.IsNullOrEmpty(value))
                return;
        ...
        public void AddToIncludes(int nodeId, string value)
        {
            ...
            {
                PropertyDataCollector data = new PropertyDataCollector(
                    nodeId,
                    "#include \"" + value + "\""
                );
            ...
            else
            {
                if (m_showDebugMessages)
                    UIUtils.ShowMessage(
                        "AddToIncludes:Attempting to add duplicate " + value,
        ...
        public void AddToPragmas(int nodeId, string value)
        {
            ...
            else
            {
                if (m_showDebugMessages)
                    UIUtils.ShowMessage(
                        "AddToPragmas:Attempting to add duplicate " + value,
                        MessageSeverity.Warning
                    );
            }
        }
        ...
        public void AddToDefines(int nodeId, string value, bool define = true)
        {
            ...
            if (!m_definesDict.ContainsKey(value))
            {
                ...
                PropertyDataCollector dataCollector = new PropertyDataCollector(
                    nodeId,
                    defineValue
                );
            ...
            else
            {
                if (m_showDebugMessages)
                    UIUtils.ShowMessage(
                        "AddToDefines:Attempting to add duplicate " + value,
                        MessageSeverity.Warning
                    );
            }
        }
        ...
        public int GetVirtualCoordinatesId(int nodeId, string coord, string lodBias)
        {
            if (!m_virtualCoordinatesDict.ContainsKey(coord))
            {
                ...
                AddLocalVariable(
                    nodeId,
                    "VirtualCoord "
                        + Constants.VirtualCoordNameStr
                        + nodeId
                        + " = VTComputeVirtualCoord"
                        + lodBias
                        + "("
                        + coord
                        + ");"
        ...
        public bool AddToLocalVariables(
        ...
        )
        {
            ...
            string value =
                UIUtils.PrecisionWirePortToCgType(precisionType, type)
                + " "
                + varName
                + " = "
                + varValue
        ...
        private bool ValidadeLocalVariable(
            PrecisionType precisionType,
            WirePortDataType type,
            string varName,
            string varValue,
        ...
        )
        {
            ...
            {
                string value =
                    UIUtils.PrecisionWirePortToTypeValue(currPrecision, type, varName)
                    + " = "
                    + varValue
                    + ";";
        ...
        {
            string finalValue = addSemiColon
                ? varName + " = " + varValue + ";"
                : varName + " = " + varValue;
        ...
        {
            if (
                m_portCategory == MasterNodePortCategory.Tessellation
                || m_portCategory == MasterNodePortCategory.Vertex
            )
            {
                AddToVertexLocalVariables(
                    0,
                    IOUtils.CreateCodeComments(forceForwardSlash, comments)
                );
            }
            else
            {
                AddToFragmentLocalVariables(
                    0,
                    IOUtils.CreateCodeComments(forceForwardSlash, comments)
                );
            }
        ...
        public bool AddToFragmentLocalVariables(
            int nodeId,
            string value,
            bool ignoreDuplicates = false,
            string varName = ""
        )
        {
            ...
            if (m_usingCustomOutput)
            {
                if (!m_customOutputDict.ContainsKey(value) || ignoreDuplicates)
                {
                    ...
                    if (
                        ...
                        && !m_customOutputByNameDict.ContainsKey(varName)
                    )
                    {
                ...
                else
                {
                    if (m_showDebugMessages)
                        UIUtils.ShowMessage(
                            "AddToLocalVariables:Attempting to add duplicate " + value,
                            MessageSeverity.Warning
                        );
                }
            }
            else
            {
                ...
                {
                    if (
                        !m_localVariablesDict.TryGetValue(value, out PropertyDataCollector property)
                    )
                    {
                        property = new PropertyDataCollector(nodeId, value);
                    ...
                    if (
                        !string.IsNullOrEmpty(varName)
                        && !m_localVariablesByNameDict.ContainsKey(varName)
                    )
                    {
        ...
        public void AddToSpecialLocalVariables(
            int nodeId,
            string value,
            bool ignoreDuplicates = false,
            string varName = ""
        )
        {
            ...
            if (m_usingCustomOutput)
            {
                ...
                else
                {
                    if (m_showDebugMessages)
                        UIUtils.ShowMessage(
                            "AddToSpecialLocalVariables:Attempting to add duplicate " + value,
                            MessageSeverity.Warning
                        );
                }
            }
            else
            {
                ...
                {
                    if (
                        !m_specialLocalVariablesDict.TryGetValue(
                            value,
                            out PropertyDataCollector property
                        )
                    )
                    {
                        property = new PropertyDataCollector(nodeId, value);
                    ...
                    if (
                        !string.IsNullOrEmpty(varName)
                        && !m_specialLocalVariablesByNameDict.ContainsKey(varName)
                    )
                    {
        ...
        public bool AddToVertexLocalVariables(
            int nodeId,
            PrecisionType precisionType,
            WirePortDataType type,
            string varName,
        ...
        )
        {
            ...
            {
                if (
                    !m_vertexLocalVariablesDict.TryGetValue(
                        value,
                        out PropertyDataCollector property
                    )
                )
                ...
                if (
                    !string.IsNullOrEmpty(varName)
                    && !m_vertexLocalVariablesByNameDict.ContainsKey(value)
                )
                {
            ...
            else
            {
                if (m_showDebugMessages)
                    UIUtils.ShowMessage(
                        "AddToVertexLocalVariables:Attempting to add duplicate " + value,
                        MessageSeverity.Warning
                    );
            }
        ...
        public string AddFunctions(string header, string body, params object[] inParams)
        {
            if (!m_localFunctions.ContainsKey(header))
            {
                ...
                m_functionsList.Add(
                    new PropertyDataCollector(-1, body.Replace("\t\t", string.Empty))
                );
                m_functions += "\n" + body + "\n";
        ...
        )
        {
            if (!m_localFunctions.ContainsKey(header))
            {
                ...
                {
                    body +=
                        (m_masterNodeCategory == AvailableShaderTypes.Template)
                            ? bodyLines[i]
                            : "\t\t" + bodyLines[i];
                    if (addNewLine)
                        body += '\n';
        ...
        {
            ShaderImporter importer = (ShaderImporter)
                ShaderImporter.GetAtPath(AssetDatabase.GetAssetPath(shader));
            if (m_propertyNodes.Count > 0)
            {
                try
                {
                    ...
                    {
                        hasContents =
                            kvp.Value.UpdateShaderDefaults(ref shader, ref defaultCol)
                            || hasContents;
                    }
        ...
        public string SurfaceVertexStructure
        {
            get
            {
                return (
        ...
        {
            get { return m_customInput; }
        }
        public string Properties
        {
            get { return m_properties; }
        }
        public string InstanceBlockName
        {
            get { return m_instanceBlockName; }
        }
        public string InstancedProperties
        {
            get { return m_instancedProperties; }
        }
        public string Uniforms
        {
            get { return m_uniforms; }
        }
        ...
        public string SpecialLocalVariables
        {
            get { return m_specialLocalVariables; }
        }
        public string VertexLocalVariables
        {
            get { return m_vertexLocalVariables; }
        }
        ...
        {
            get { return m_grabPass; }
        }
        public bool DirtyAppData
        {
        ...
        public bool DirtyUniforms
        {
            get { return m_dirtyUniforms; }
        }
        public bool DirtyProperties
        {
            get { return m_dirtyProperties; }
        }
        public bool DirtyInstancedProperties
        {
            get { return m_dirtyInstancedProperties; }
        }
        public bool DirtyInputs
        {
            get { return m_dirtyInputs; }
        }
        public bool DirtyCustomInput
        {
            get { return m_dirtyCustomInputs; }
        }
        public bool DirtyIncludes
        {
            get { return m_dirtyIncludes; }
        }
        public bool DirtyPragmas
        {
            get { return m_dirtyPragmas; }
        }
        public bool DirtyDefines
        {
            get { return m_dirtyDefines; }
        }
        public bool DirtyAdditionalDirectives
        {
            get { return m_dirtyAdditionalDirectives; }
        }
        public bool DirtyLocalVariables
        {
            get { return m_dirtyLocalVariables; }
        }
        public bool DirtyVertexVariables
        {
            get { return m_dirtyVertexLocalVariables; }
        }
        public bool DirtySpecialLocalVariables
        {
            get { return m_dirtySpecialLocalVariables; }
        }
        public bool DirtyPerVertexData
        {
            get { return m_dirtyPerVertexData; }
        }
        public bool DirtyFunctions
        {
            get { return m_dirtyFunctions; }
        }
        public bool DirtyGrabPass
        {
            get { return m_grabPassIsDirty; }
        }
        public int LocalVariablesAmount
        {
            get { return m_localVariablesDict.Count; }
        }
        public int SpecialLocalVariablesAmount
        {
            get { return m_specialLocalVariablesDict.Count; }
        }
        public int VertexLocalVariablesAmount
        {
            get { return m_vertexLocalVariablesDict.Count; }
        }
        public bool TesselationActive
        {
            set { m_tesselationActive = value; }
            get { return m_tesselationActive; }
        }

        public int AvailableVertexTempId
        {
            get { return m_availableVertexTempId++; }
        }
        public int AvailableFragTempId
        {
            get { return m_availableFragTempId++; }
        }
        ...
        public bool IsFragmentCategory
        {
            get
            {
                return m_portCategory == MasterNodePortCategory.Fragment
                    || m_portCategory == MasterNodePortCategory.Debug;
            }
        }
        ...
        {
            get { return m_masterNodeCategory == AvailableShaderTypes.Template; }
        }
        ...
        public bool IsSRP
        {
            ...
            {
                return (
                    TemplateDataCollectorInstance.CurrentSRPType == TemplateSRPType.URP
                    || TemplateDataCollectorInstance.CurrentSRPType == TemplateSRPType.HDRP
                );
            }
        }
        ...
        public bool CustomOutline
        {
            get
            {
                return UsingCustomOutlineColor
                    || UsingCustomOutlineWidth
                    || UsingCustomOutlineAlpha;
            }
        }
        public List<PropertyDataCollector> InputList
        {
            get { return m_inputList; }
        }
        public List<PropertyDataCollector> CustomInputList
        {
            get { return m_customInputList; }
        }
        public List<PropertyDataCollector> PropertiesList
        {
            get { return m_propertiesList; }
        }
        public List<PropertyDataCollector> InstancedPropertiesList
        {
            get { return m_instancedPropertiesList; }
        }
        public List<PropertyDataCollector> DotsPropertiesList
        {
            get { return m_dotsPropertiesList; }
        }
        public List<PropertyDataCollector> DotsDefinesList
        {
            get { return m_dotsDefinesList; }
        }
        public List<PropertyDataCollector> UniformsList
        {
            get { return m_uniformsList; }
        }
        public List<PropertyDataCollector> MiscList
        {
            get { return m_additionalDirectivesList; }
        }
        public List<PropertyDataCollector> BeforeNativeDirectivesList
        {
            get { return m_additionalDirectivesList.FindAll(obj => obj.OrderIndex < 0); }
        }
        public List<PropertyDataCollector> AfterNativeDirectivesList
        {
            get { return m_additionalDirectivesList.FindAll(obj => obj.OrderIndex > 0); }
        }
        public List<PropertyDataCollector> IncludesList
        {
            get { return m_includesList; }
        }

        //public List<PropertyDataCollector> TagsList { get { return m_tagsList; } }
        public List<PropertyDataCollector> PragmasList
        {
            get { return m_pragmasList; }
        }
        public List<PropertyDataCollector> DefinesList
        {
            get { return m_definesList; }
        }
        public List<PropertyDataCollector> InstructionsList
        {
            get { return m_instructionsList; }
        }
        public List<PropertyDataCollector> LocalVariablesList
        {
            get { return m_localVariablesList; }
        }
        public List<PropertyDataCollector> VertexLocalVariablesList
        {
            get { return m_vertexLocalVariablesList; }
        }
        public List<PropertyDataCollector> SpecialLocalVariablesList
        {
            get { return m_specialLocalVariablesList; }
        }
        public List<PropertyDataCollector> VertexDataList
        {
            get { return m_vertexDataList; }
        }
        public List<PropertyDataCollector> CustomOutputList
        {
            get { return m_customOutputList; }
        }
        public List<PropertyDataCollector> FunctionsList
        {
            get { return m_functionsList; }
        }
        public List<PropertyDataCollector> GrabPassList
        {
            get { return m_grabPassList; }
        }
        public Dictionary<string, string> GrabPassDict
        {
            get { return m_grabPassDict; }
        }
        public List<PropertyDataCollector> AboveUsePassesList
        {
            get { return m_aboveUsePassesList; }
        }
        public List<PropertyDataCollector> BelowUsePassesList
        {
            get { return m_belowUsePassesList; }
        }
        public Dictionary<string, string> AboveUsePassesDict
        {
            get { return m_usePassesDict; }
        }
        public List<InputCoordsCollector> CustomShadowCoordsList
        {
            get { return m_customShadowCoordsList; }
        }
        public List<int> PackSlotsList
        {
            get { return m_packSlotsList; }
        }
        public Dictionary<string, string> LocalFunctions
        {
            get { return m_localFunctions; }
        }

        //Templates
        public List<string> VertexInputList
        {
            get { return m_vertexInputList; }
        }
        public List<string> InterpolatorList
        {
            get { return m_interpolatorsList; }
        }
        public List<string> VertexInterpDeclList
        {
            get { return m_vertexInterpDeclList; }
        }
        public TemplateDataCollector TemplateDataCollectorInstance
        {
            get { return m_templateDataCollector; }
        }
        ...
        public Dictionary<string, PropertyDataCollector> PropertiesDict
        {
