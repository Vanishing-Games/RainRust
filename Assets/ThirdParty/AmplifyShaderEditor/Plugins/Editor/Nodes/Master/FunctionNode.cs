...
namespace AmplifyShaderEditor
{
    ...
    public class FunctionNode : ParentNode
    {
        ...
        private Dictionary<int, FunctionInput> m_allFunctionInputsDict =
            new Dictionary<int, FunctionInput>();

        [SerializeField]
        private List<FunctionOutput> m_allFunctionOutputs;
        private Dictionary<int, FunctionOutput> m_allFunctionOutputsDict =
            new Dictionary<int, FunctionOutput>();

        [SerializeField]
        private List<FunctionSwitch> m_allFunctionSwitches;
        private Dictionary<int, FunctionSwitch> m_allFunctionSwitchesDict =
            new Dictionary<int, FunctionSwitch>();
        ...
        [SerializeField]
        private List<AdditionalDirectiveContainer> m_directives =
        ...
        public Texture2D HeaderIcon
        {
            get
            {
                ...
                {
                    m_headerIcon = AssetDatabase.LoadAssetAtPath<Texture2D>(
                        AssetDatabase.GUIDToAssetPath("5b59dcde73b12b746af7ff7c47a1195d")
                    );
                }
                return m_headerIcon;
            }
        ...
        public void CommonInit(AmplifyShaderFunction function, int uniqueId)
        {
            ...
            {
                m_functionGUID = AssetDatabase.AssetPathToGUID(
                    AssetDatabase.GetAssetPath(m_function)
                );
            }

            m_functionGraphId = Mathf.Max(
                m_functionGraphId,
                ContainerGraph.ParentWindow.GraphCount
            );
            ...
            AmplifyShaderEditorWindow.LoadFromMeta(
                ref m_functionGraph,
                ContainerGraph.ParentWindow.ContextMenuInstance,
                Function.FunctionInfo
            );
            //m_functionCheckSum = LastLine( m_function.FunctionInfo );
            m_functionCheckSum = AssetDatabase
                .GetAssetDependencyHash(AssetDatabase.GetAssetPath(m_function))
                .ToString();
            ...
            m_allFunctionInputs.Sort(
                (x, y) =>
                {
                    return x.OrderIndex.CompareTo(y.OrderIndex);
                }
            );
            m_allFunctionOutputs.Sort(
                (x, y) =>
                {
                    return x.OrderIndex.CompareTo(y.OrderIndex);
                }
            );
            m_allFunctionSwitches.Sort(
                (x, y) =>
                {
                    return x.OrderIndex.CompareTo(y.OrderIndex);
                }
            ...
            for (int i = 0; i < inputCount; i++)
            {
                ...
                {
                    AddInputPort(
                        m_allFunctionInputs[i].SelectedInputType,
                        false,
                        m_allFunctionInputs[i].InputName
                    );
                }
                else
                {
                    AddInputPort(
                        ...
                        m_allFunctionInputs[i].UniqueId
                    );
                }
                ...
                if (!m_allFunctionInputs[i].InputPorts[0].IsConnected)
                {
                    ...
                    m_inputPorts[i].InternalData = m_allFunctionInputs[i]
                        .InputPorts[0]
                        .InternalData;
                }
            ...
            for (int i = 0; i < outputCount; i++)
            {
                ...
                {
                    AddOutputPort(
                        m_allFunctionOutputs[i].AutoOutputType,
                        m_allFunctionOutputs[i].OutputName
                    );
                }
                else
                {
            ...
            UIUtils.CurrentWindow.OutsideGraph.OnLODMasterNodesAddedEvent +=
                OnLODMasterNodesAddedEvent;
        }

        public InputPort GetInput(FunctionInput input)
        {
            int index = m_allFunctionInputs.FindIndex(
                (x) =>
                {
        ...
        private void AddShaderFunctionDirectivesInternal(int lod)
        {
            ...
            {
                nodes[i]
                    .PassModule.AdditionalDirectives.AddShaderFunctionItems(
                        OutputId,
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (surface != null)
            {
                ...
                //}
                surface.AdditionalDirectives.AddShaderFunctionItems(
                    OutputId,
                    Function.AdditionalDirectives.DirectivesList
                );
            }
            ...
            if (ContainerGraph.ParentWindow.CurrentGraph != m_functionGraph)
                ContainerGraph.ParentWindow.CurrentGraph.InstancePropertyCount +=
                    m_functionGraph.InstancePropertyCount;
            ...
            if (inputCount > 0)
            {
                for (int i = 0; i < inputCount; i++)
                {
                    if (
                        ...
                        && m_inputPorts[
                            i
                        ].AutoDrawInternalData /*&& ( m_inputPorts[ i ].AutoDrawInternalData || ( m_autoDrawInternalPortData && m_useInternalPortData ) )*/ /*&& m_inputPorts[ i ].AutoDrawInternalData*/
                    )
                    {
        ...
        public override void Destroy()
        {
            ...
            if (
                ...
                && ContainerGraph.ParentWindow.CurrentGraph != m_functionGraph
            )
                ContainerGraph.ParentWindow.CurrentGraph.InstancePropertyCount -=
                    m_functionGraph.InstancePropertyCount;
            ...
            if (ContainerGraph.ParentWindow.OutsideGraph.CurrentStandardSurface != null)
            {
                ...
                //}
                ContainerGraph.ParentWindow.OutsideGraph.CurrentStandardSurface.AdditionalDirectives.RemoveShaderFunctionItems(
                    OutputId /*, m_directives */
                );
            }
            else
            {
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (m_portsChanged)
            {
                ...
                {
                    m_outputPorts[i]
                        .ChangeType(m_allFunctionOutputs[i].InputPorts[0].DataType, false);
                }
        ...
        {
            if (
                (ContainerGraph.ParentWindow.CheckFunctions || forceCheck || forceChange)
                && m_function != null
            )
            {
                //string newCheckSum = LastLine( m_function.FunctionInfo );
                string newCheckSum = AssetDatabase
                    .GetAssetDependencyHash(AssetDatabase.GetAssetPath(m_function))
                    .ToString();
        ...
        public void DuplicateMe()
        {
            ...
            for (int i = 0; i < m_allFunctionSwitches.Count; i++)
            {
                allOptions +=
                    ","
                    + m_allFunctionSwitches[i].UniqueId
                    + ","
            ...
            for (int i = 0; i < m_outputPorts.Count; i++)
            {
                ...
                {
                    OutputPort newOutputPort = newNode.GetOutputPortByUniqueId(
                    ...
                    if (newNode.OutputPorts != null && newOutputPort != null)
                    {
                        ...
                        {
                            ContainerGraph.CreateConnection(
                                m_outputPorts[i].ExternalReferences[j].NodeId,
                                m_outputPorts[i].ExternalReferences[j].PortId,
                                newOutputPort.NodeId,
                ...
                if (m_inputPorts[i].IsConnected)
                {
                    ...
                    if (newNode.InputPorts != null && newInputPort != null)
                    {
                        ContainerGraph.CreateConnection(
                            newInputPort.NodeId,
                            newInputPort.PortId,
                            m_inputPorts[i].ExternalReferences[0].NodeId,
                            m_inputPorts[i].ExternalReferences[0].PortId
                        );
                    }
                }
            }
        ...
        private FunctionOutput GetFunctionOutputByUniqueId(int uniqueId)
        {
            ...
            if (m_allFunctionOutputsDict.Count != m_allFunctionOutputs.Count)
            {
                ...
                {
                    m_allFunctionOutputsDict.Add(
                        m_allFunctionOutputs[i].UniqueId,
                        m_allFunctionOutputs[i]
                    );
                }
            }
        ...
        private FunctionInput GetFunctionInputByUniqueId(int uniqueId)
        {
            ...
            if (m_allFunctionInputsDict.Count != m_allFunctionInputs.Count)
            {
                ...
                for (int i = 0; i < listCount; i++)
                {
                    m_allFunctionInputsDict.Add(
                        m_allFunctionInputs[i].UniqueId,
                        m_allFunctionInputs[i]
                    );
                }
            }
        ...
        )
        {
            ...
            m_functionGraph.CurrentPrecision = ContainerGraph
                .ParentWindow
                .CurrentGraph
                .CurrentPrecision;
            ParentGraph cachedGraph = ContainerGraph.ParentWindow.CustomGraph;
            ...
            if (
                m_reordenator != null
                && m_reordenator.RecursiveCount() > 0
                && m_reordenator.HasTitle
            )
            {
                dataCollector.AddToProperties(
                    UniqueId,
                    "[Header(" + m_reordenator.HeaderTitle.Replace("-", " ") + ")]",
                    m_reordenator.OrderIndex
                );
            }
#endif
            ...
            {
                if (
                    !m_allFunctionInputs[i].InputPorts[0].IsConnected || m_inputPorts[i].IsConnected
                )
                    m_allFunctionInputs[i].OnPortGeneration += FunctionNodeOnPortGeneration;
            }

            result += functionOutput.GenerateShaderForOutput(
                outputId,
                ref dataCollector,
                ignoreLocalvar
            );
        ...
        private string FunctionNodeOnPortGeneration(
            ref MasterNodeDataCollector dataCollector,
            int index,
            ParentGraph graph
        )
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                m_reordenator != null ? m_reordenator.RawOrderIndex : -1
            );
            ...
            int functionSwitchCount =
                m_allFunctionSwitches != null ? m_allFunctionSwitches.Count : 0;
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            AmplifyShaderFunction loaded = AssetDatabase.LoadAssetAtPath<AmplifyShaderFunction>(
                AssetDatabase.GUIDToAssetPath(m_functionGUID)
            );
            if (loaded != null)
            {
                ...
                if (guids.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        UIUtils.ShowMessage(
                            UniqueId,
                            string.Format(
                                "Error loading {0} shader function from project folder",
                                m_filename
                            ),
