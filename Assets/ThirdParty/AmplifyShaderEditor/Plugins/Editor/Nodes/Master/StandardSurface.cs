...
namespace AmplifyShaderEditor
{
    ...
    {
        private static readonly string[] VertexLitFunc =
        {
        ...
        private static readonly GUIContent RenderPathContent = new GUIContent(
            "Render Path",
            "Selects and generates passes for the supported rendering paths\nDefault: All"
        );
        private const string ShaderModelStr = "Shader Model";
        private static readonly GUIContent LightModelContent = new GUIContent(
            "Light Model",
            "Surface shader lighting model defines how the surface reflects light\nDefault: Standard"
        );
        private static readonly GUIContent ShaderLODContent = new GUIContent(
            "Shader LOD",
            "Shader LOD"
        );
        private static readonly GUIContent CullModeContent = new GUIContent(
            "Cull Mode",
        ...
        [SerializeField]
        private RenderingPlatformOpHelper m_renderingPlatformOpHelper =
            new RenderingPlatformOpHelper();

        [SerializeField]
        private RenderingOptionsOpHelper m_renderingOptionsOpHelper =
            new RenderingOptionsOpHelper();
        ...
        [SerializeField]
        private TemplateAdditionalDirectivesHelper m_additionalDirectives =
            new TemplateAdditionalDirectivesHelper(" Additional Directives");

        [SerializeField]
        private AdditionalSurfaceOptionsHelper m_additionalSurfaceOptions =
            new AdditionalSurfaceOptionsHelper();
        ...
        public override void AddMasterPorts()
        {
            ...
            m_inputPorts[m_inputPorts.Count - 1].Locked =
                (m_currentLightModel == StandardShaderLightModel.Standard)
                || (m_currentLightModel == StandardShaderLightModel.StandardSpecular)
                    ? false
                    : true;
            ...
            m_inputPorts[m_inputPorts.Count - 1].Locked = (
                ...
                || m_currentLightModel == StandardShaderLightModel.Unlit
            ...
            m_inputPorts[m_inputPorts.Count - 1].Locked = (
                m_alphaMode == AlphaMode.Opaque || m_alphaMode == AlphaMode.Masked
            );
            ...
            m_inputPorts[m_inputPorts.Count - 1].Locked = (
                m_alphaMode != AlphaMode.Masked && m_alphaMode != AlphaMode.Custom
            );
            ...
            AddInputPort(
                WirePortDataType.FLOAT3,
                false,
                (m_vertexMode == VertexMode.Relative ? VertexDisplacementStr : VertexPositionStr),
                VertexDataStr,
                0 /*index++*/
                ,
            ...
            AddInputPort(
                WirePortDataType.FLOAT4,
                false,
                TessellationOpHelper.TessellationPortStr,
                index++,
                MasterNodePortCategory.Tessellation,
                14
            );
        ...
        public void DrawInspectorProperty()
        {
            ...
            {
                m_inspectorDefaultStyle = UIUtils.GetCustomStyle(
        ...
        public void DrawGeneralOptions()
        {
            ...
            m_currentLightModel = (StandardShaderLightModel)EditorGUILayoutEnumPopup(
                LightModelContent,
                m_currentLightModel
            );
            if (EditorGUI.EndChangeCheck())
            {
                ...
                {
                    ContainerGraph.ParentWindow.CurrentNodeAvailability =
                        NodeAvailability.CustomLighting;
                    //ContainerGraph.CurrentCanvasMode = NodeAvailability.CustomLighting;
                }
                else
                {
                    ContainerGraph.ParentWindow.CurrentNodeAvailability =
                        NodeAvailability.SurfaceShader;
                    //ContainerGraph.CurrentCanvasMode = NodeAvailability.SurfaceShader;
                }
            }

            m_shaderModelIdx = EditorGUILayoutPopup(
                ShaderModelStr,
                m_shaderModelIdx,
                ShaderModelTypeArr
            );
            ...
            m_inlineCullMode.CustomDrawer(
                ref inst,
                (x) =>
                {
                    m_cullMode = (CullMode)EditorGUILayoutEnumPopup(CullModeContent, m_cullMode);
                },
                CullModeContent.text
            );
            ...
            if (EditorGUI.EndChangeCheck())
            {
                m_inputPorts[m_vertexPortId].Name =
                    m_vertexMode == VertexMode.Relative ? VertexDisplacementStr : VertexPositionStr;
                m_sizeIsDirty = true;
            }

            ShaderLOD = Mathf.Clamp(
                EditorGUILayoutIntField(ShaderLODContent, ShaderLOD),
                0,
        ...
        public void ShowOpacityMaskValueUI()
        {
            EditorGUI.BeginChangeCheck();
            UndoParentNode inst = this;
            m_inlineOpacityMaskClipValue.CustomDrawer(
                ref inst,
                (x) =>
                {
                    m_opacityMaskClipValue = EditorGUILayoutFloatField(
                        OpacityMaskClipValueContent,
                        m_opacityMaskClipValue
                    );
                },
                OpacityMaskClipValueContent.text
            );
            if (EditorGUI.EndChangeCheck())
            {
                m_checkChanges = true;
                if (
                    ...
                    && m_currentMaterial.HasProperty(IOUtils.MaskClipValueName)
                )
                {
        ...
        public override void DrawProperties()
        {
            ...
            if (
                m_inspectorTooldropdownStyle == null
                || EditorGUIUtility.isProSkin != m_usingProSkin
            )
            {
            ...
            EditorGUILayout.BeginVertical();
            {
                ...
                //  General
                bool generalIsVisible = ContainerGraph
                    .ParentWindow
                    .InnerWindowVariables
                    .ExpandedGeneralShaderOptions;
                NodeUtils.DrawPropertyGroup(
                    ref generalIsVisible,
                    GeneralFoldoutStr,
                    DrawGeneralOptions
                );
                ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedGeneralShaderOptions =
                    generalIsVisible;
                ...
                bool blendOptionsVisible = GUILayout.Toggle(
                    ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedBlendOptions,
                    AlphaModeContent,
                ...
                EditorGUI.BeginChangeCheck();
                m_alphaMode = (AlphaMode)EditorGUILayoutPopup(
                    string.Empty,
                    (int)m_alphaMode,
                    FadeModeOptions,
                    UIUtils.InspectorPopdropdownStyle,
                ...
                m_customBlendAvailable = (
                    m_alphaMode == AlphaMode.Custom || m_alphaMode == AlphaMode.Opaque
                );
                ...
                if (ContainerGraph.ParentWindow.InnerWindowVariables.ExpandedBlendOptions)
                {
                    ...
                    m_renderType = (RenderType)EditorGUILayoutEnumPopup(
                        RenderTypeContent,
                        m_renderType
                    );
                    ...
                    {
                        EditorGUI.BeginChangeCheck();
                        m_customRenderType = EditorGUILayoutTextField(
                            CustomRenderTypeStr,
                            m_customRenderType
                        );
                        if (EditorGUI.EndChangeCheck())
                        {
                            m_customRenderType = UIUtils.RemoveInvalidCharacters(
                                m_customRenderType
                            );
                        }
                    }

                    m_renderQueue = (RenderQueue)EditorGUILayoutEnumPopup(
                        RenderQueueContent,
                        m_renderQueue
                    );
                    ...
                    EditorGUI.BeginDisabledGroup(
                        !(
                            m_alphaMode == AlphaMode.Transparent
                            || m_alphaMode == AlphaMode.Premultiply
                            || m_alphaMode == AlphaMode.Translucent
                            || m_alphaMode == AlphaMode.Custom
                        )
                    );
                    ...
                    if (m_refractionPort.IsConnected)
                    {
                        ...
                        EditorGUI.BeginChangeCheck();
                        m_inlineChromaticAberration.RangedFloatField(
                            ref inst,
                            ChromaticAberrationStr,
                            0.0f,
                            0.3f
                        );
                        if (EditorGUI.EndChangeCheck())
                        {
                            if (
                                m_currentMaterial != null
                                && m_currentMaterial.HasProperty(
                                    IOUtils.ChromaticAberrationProperty
                                )
                            )
                            {
                                m_currentMaterial.SetFloat(
                                    IOUtils.ChromaticAberrationProperty,
                                    m_inlineChromaticAberration.FloatValue
                                );
                            }
                        }
                    }
                    ...
                    m_inlineAlphaToCoverage.CustomDrawer(
                        ref inst,
                        (x) =>
                        {
                            m_alphaToCoverage = EditorGUILayoutToggle(
                                AlphaToCoverageStr,
                                m_alphaToCoverage
                            );
                        },
                        AlphaToCoverageStr.text
                    );
                ...
                m_tessOpHelper.Draw(
                    this,
                    m_inspectorToolbarStyle,
                    m_currentMaterial,
                    m_tessellationPort.IsConnected
                );
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (
                ...
                && m_transmissionPort.IsConnected
                && m_renderPath != RenderPath.ForwardOnly
            )
            {
                m_renderPath = RenderPath.ForwardOnly;
                UIUtils.ShowMessage(
                    "Render Path changed to Forward Only since transmission only works in forward rendering"
                );
            }

            if (
                m_translucencyPort != null
                && m_translucencyPort.IsConnected
                && m_renderPath != RenderPath.ForwardOnly
            )
            {
                m_renderPath = RenderPath.ForwardOnly;
                UIUtils.ShowMessage(
                    "Render Path changed to Forward Only since translucency only works in forward rendering"
                );
            }
            ...
            if (
                (m_tessOpHelper.EnableTesselation && !m_tessellationPort.IsConnected)
                != m_previousTessellationOn
            )
                m_checkChanges = true;
            ...
            if (m_checkChanges)
            {
                ...
                if (
                    m_inputPorts[m_discardPortId].Available && !m_inlineOpacityMaskClipValue.IsValid
                )
                {
        ...
        public override void OnNodeRepaint(DrawInfo drawInfo)
        {
            ...
            if (
                m_containerGraph.IsInstancedShader
                || m_renderingOptionsOpHelper.ForceEnableInstancing
            )
            {
        ...
        private void CacheCurrentSettings()
        {
            ...
            for (int portId = 0; portId < m_inputPorts.Count; portId++)
            {
                if (m_inputPorts[portId].IsConnected)
                {
                    ...
                    m_cacheNodeConnections.Add(
                        m_inputPorts[portId].Name,
                        new NodeCache(connection.NodeId, connection.PortId)
                    );
                }
            }
        }
        ...
        private void ConnectFromCache()
        {
            for (int i = 0; i < m_inputPorts.Count; i++)
            {
                ...
                if (cache != null)
                {
                    UIUtils.SetConnection(
                        UniqueId,
                        m_inputPorts[i].PortId,
                        cache.TargetNodeId,
                        cache.TargetPortId
                    );
                }
        ...
        public override void SetMaterialMode(Material mat, bool fetchMaterialValues)
        {
            ...
            if (m_alphaMode == AlphaMode.Masked || m_alphaMode == AlphaMode.Custom)
            {
                if (
                    fetchMaterialValues
                    && m_materialMode
                    && mat.HasProperty(IOUtils.MaskClipValueName)
                )
                {
            ...
            if (m_refractionPort.IsConnected && !m_inlineChromaticAberration.Active)
            {
                if (
                    fetchMaterialValues
                    && m_materialMode
                    && mat.HasProperty(IOUtils.ChromaticAberrationProperty)
                )
                    m_inlineChromaticAberration.FloatValue = mat.GetFloat(
                        IOUtils.ChromaticAberrationProperty
                    );
            }
        }
        ...
        public override void ForceUpdateFromMaterial(Material material)
        {
            ...
            if (m_refractionPort.IsConnected && !m_inlineChromaticAberration.Active)
            {
                if (material.HasProperty(IOUtils.ChromaticAberrationProperty))
                    m_inlineChromaticAberration.FloatValue = material.GetFloat(
                        IOUtils.ChromaticAberrationProperty
                    );
            }
        }
        ...
        public string CreateInstructionsForVertexPort(InputPort port)
        {
            ...
            string vertexInstructions = node.GetValueFromOutputStr(
                connection.PortId,
                port.DataType,
            ...
            if (m_currentDataCollector.DirtySpecialLocalVariables)
            {
                m_currentDataCollector.AddVertexInstruction(
                    m_currentDataCollector.SpecialLocalVariables,
                    UniqueId,
                    false
                );
            ...
            {
                m_currentDataCollector.AddVertexInstruction(
                    m_currentDataCollector.VertexLocalVariables,
                    UniqueId,
                    false
                );
        ...
        public void CreateInstructionsForPort(
            InputPort port,
            string portName,
            bool addCustomDelimiters = false,
            string customDelimiterIn = null,
            string customDelimiterOut = null,
            bool ignoreLocalVar = false,
            bool normalIsConnected = false,
        ...
        )
        {
            ...
            string newInstruction = node.GetValueFromOutputStr(
                connection.PortId,
                port.DataType,
                ref m_currentDataCollector,
                ignoreLocalVar
            ...
            if (m_currentDataCollector.DirtySpecialLocalVariables)
            {
                m_currentDataCollector.AddInstructions(
                    m_currentDataCollector.SpecialLocalVariables
                );
                m_currentDataCollector.ClearSpecialLocalVariables();
            }
            ...
            m_currentDataCollector.AddInstructions(
                addCustomDelimiters ? customDelimiterIn : ("\t\t\t" + portName + " = ")
            );
            m_currentDataCollector.AddInstructions(newInstruction);
            m_currentDataCollector.AddInstructions(
                addCustomDelimiters ? customDelimiterOut : ((isDebugPort) ? " + 1E-5;\n" : ";\n")
            );
        }
        ...
        public override Shader Execute(string pathname, bool isFullPath)
        {
            ...
            bool isInstancedShader =
                m_renderingOptionsOpHelper.ForceEnableInstancing || UIUtils.IsInstancedShader();
            ...
            if (
                m_renderingOptionsOpHelper.SpecularHighlightToggle
                || m_renderingOptionsOpHelper.ReflectionsToggle
            )
                m_currentDataCollector.AddToProperties(
                    UniqueId,
            ...
            if (m_renderingOptionsOpHelper.SpecularHighlightToggle)
            {
                ...
                m_currentDataCollector.AddToPragmas(
                    UniqueId,
            ...
            if (m_renderingOptionsOpHelper.ReflectionsToggle)
            {
                ...
                m_currentDataCollector.AddToPragmas(
                    UniqueId,
                    "shader_feature _GLOSSYREFLECTIONS_OFF"
                );
            }
            ...
            string finalRenderType =
                (m_renderType == RenderType.Custom && m_customRenderType.Length > 0)
                    ? m_customRenderType
                    : m_renderType.ToString();
            tags = string.Format(
                tags,
                finalRenderType,
                (m_renderQueue + ((m_queueOrder >= 0) ? "+" : string.Empty) + m_queueOrder)
            );
            //if ( !m_customBlendMode )
            {
                if (m_alphaMode == AlphaMode.Transparent || m_alphaMode == AlphaMode.Premultiply)
                {
                    ...
                    if (!m_renderingOptionsOpHelper.IgnoreProjectorValue)
                    {
                        Debug.Log(
                            string.Format(
                                "Setting Ignore Projector to True since it's requires by Blend Mode {0}.",
                                m_alphaMode
                            )
                        );
            ...
            {
                case StandardShaderLightModel.CustomLighting:
                    outputStruct = "SurfaceOutputCustomLightingCustom";
                    break;
                case StandardShaderLightModel.Standard:
                    outputStruct = "SurfaceOutputStandard";
                    break;
                case StandardShaderLightModel.StandardSpecular:
                    outputStruct = "SurfaceOutputStandardSpecular";
                    break;
                case StandardShaderLightModel.Unlit:
                case StandardShaderLightModel.Lambert:
                case StandardShaderLightModel.BlinnPhong:
                    outputStruct = "SurfaceOutput";
                    break;
            }
            ...
            if (m_currentLightModel == StandardShaderLightModel.CustomLighting)
            {
                ...
                m_currentDataCollector.ChangeCustomInputHeader(
                    m_currentLightModel.ToString() + Constants.CustomLightStructStr
                );
            ...
            if (m_inputPorts[m_inputPorts.Count - 1].IsConnected)
            {
                ...
                if (debugIsUsingCustomLighting)
                {
                    ...
                    ParentNode node = UIUtils.GetNode(connection.NodeId);
                    customLightingCode = node.GetValueFromOutputStr(
                        connection.PortId,
                        WirePortDataType.FLOAT3,
                        ref m_currentDataCollector,
                        false
                ...
                {
                    CreateInstructionsForPort(
                        debugPort,
                        Constants.OutputVarStr + ".Emission",
                        false,
                        null,
                        null,
                        false,
            ...
            else
            {
                ...
                for (int i = 0; i < sortedPorts.Count; i++)
                {
                    ...
                    if (
                        m_currentLightModel == StandardShaderLightModel.CustomLighting
                        && sortedPorts[i].Name.Equals(AlphaStr)
                    )
                    ...
                    if (sortedPorts[i].IsConnected)
                    {
                        ...
                        if (hasRefraction)
                        {
                            ...
                            if (m_grabOrder != 0)
                            {
                                ...
                                m_currentDataCollector.AddToUniforms(
                                    UniqueId,
                                    "uniform sampler2D RefractionGrab" + m_grabOrder + ";"
                                );
                            }
                            else
                            {
                            ...
                            if (!m_inlineChromaticAberration.Active)
                            {
                                ...
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "[Header(Refraction)]",
                                    m_refractionReorder.OrderIndex
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_ChromaticAberration(\"Chromatic Aberration\", Range( 0 , 0.3)) = 0.1",
                                    m_refractionReorder.OrderIndex + 1
                                );
                            }

                            m_currentDataCollector.AddToPragmas(
                                UniqueId,
                                "multi_compile _ALPHAPREMULTIPLY_ON"
                            );
                        }
                        ...
                        if (hasTranslucency || hasTransmission)
                        {
                            ...
                            if (hasTranslucency)
                            {
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "[Header(Translucency)]",
                                    m_translucencyReorder.OrderIndex
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_Translucency(\"Strength\", Range( 0 , 50)) = 1",
                                    m_translucencyReorder.OrderIndex + 1
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_TransNormalDistortion(\"Normal Distortion\", Range( 0 , 1)) = 0.1",
                                    m_translucencyReorder.OrderIndex + 2
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_TransScattering(\"Scaterring Falloff\", Range( 1 , 50)) = 2",
                                    m_translucencyReorder.OrderIndex + 3
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_TransDirect(\"Direct\", Range( 0 , 1)) = 1",
                                    m_translucencyReorder.OrderIndex + 4
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_TransAmbient(\"Ambient\", Range( 0 , 1)) = 0.2",
                                    m_translucencyReorder.OrderIndex + 5
                                );
                                m_currentDataCollector.AddToProperties(
                                    UniqueId,
                                    "_TransShadow(\"Shadow\", Range( 0 , 1)) = 0.9",
                                    m_translucencyReorder.OrderIndex + 6
                                );
                            ...
                            switch (m_currentLightModel)
                            {
                                ...
                                case StandardShaderLightModel.StandardSpecular:
                                    outputStruct =
                                        "SurfaceOutput"
                                        + m_currentLightModel.ToString()
                                        + Constants.CustomLightStructStr;
                                    break;
                            }
                            ...
                            m_currentDataCollector.AddToCustomInput(UniqueId, "half3 Albedo", true);
                            m_currentDataCollector.AddToCustomInput(UniqueId, "half3 Normal", true);
                            m_currentDataCollector.AddToCustomInput(
                                UniqueId,
                                "half3 Emission",
                                true
                            );
                        ...
                        if (sortedPorts[i].Name.Equals(DiscardStr))
                        {
                            ...
                            if (m_currentLightModel == StandardShaderLightModel.CustomLighting)
                            {
                                ...
                                customLightingMaskCode = node.GetValueFromOutputStr(
                                    connection.PortId,
                                    WirePortDataType.FLOAT,
                                    ref m_currentDataCollector,
                                    false
                                );
                                customLightingMaskCode =
                                    "clip( "
                                    + customLightingMaskCode
                                    + " - "
                                    + m_inlineOpacityMaskClipValue.GetValueOrProperty(
                                        IOUtils.MaskClipValueName,
                                        false
                                    )
                                    + " )";
                                customLightingInstructions = m_currentDataCollector.CustomOutput;
                        ...
                        {
                            string vertexInstructions = CreateInstructionsForVertexPort(
                                sortedPorts[i]
                            );
                            m_currentDataCollector.AddToVertexDisplacement(
                                vertexInstructions,
                                m_vertexMode
                            );
                        }
                        ...
                        else if (sortedPorts[i].DataName.Equals(VertexTangentStr))
                        {
                            ...
                            m_tessOpHelper.IsTessellationPort(sortedPorts[i].PortId)
                            && sortedPorts[i].IsConnected /* && m_tessOpHelper.EnableTesselation*/
                        )
                        {
                            //Vertex displacement and per vertex custom data
                            WireReference connection = sortedPorts[i].GetConnection();
                            ...
                            if (m_currentDataCollector.DirtySpecialLocalVariables)
                            {
                                m_tessOpHelper.AddAdditionalData(
                                    m_currentDataCollector.SpecialLocalVariables
                                );
                                m_currentDataCollector.ClearSpecialLocalVariables();
                            }
                        ...
                        else if (sortedPorts[i].Name.Equals(RefractionStr))
                        {
                            ...
                            m_currentDataCollector.AddInstructions(
                                "\t\t\to.Normal = o.Normal" + refractionFix + ";\n"
                            );
                        ...
                        )
                        {
                            ...
                            customLightingAlphaCode = node.GetValueFromOutputStr(
                                connection.PortId,
                                WirePortDataType.FLOAT,
                                ref m_currentDataCollector,
                                false
                    ...
                    else if (sortedPorts[i].Name.Equals(AlphaStr))
                    {
                        if (m_currentLightModel != StandardShaderLightModel.CustomLighting)
                        {
                            m_currentDataCollector.AddInstructions(
                                string.Format(
                                    "\t\t\t{0}.{1} = 1;\n",
                                    Constants.OutputVarStr,
                                    sortedPorts[i].DataName
                                )
                            );
                        }
                    }
            ...
            for (int i = 0; i < 4; i++)
            {
                if (m_currentDataCollector.GetChannelUsage(i) == TextureChannelUsage.Required)
                {
                    ...
                    m_currentDataCollector.AddToProperties(
                        -1,
                        UIUtils.GetTex2DProperty(channelName, TexturePropertyValues.white),
                        -1
                    );
                }
            }
            ...
            if (
                m_inputPorts[m_discardPortId].Available && m_inputPorts[m_discardPortId].IsConnected
            )
            {
                if (m_inlineOpacityMaskClipValue.IsValid)
                {
                    RangedFloatNode fnode =
                        UIUtils.GetNode(m_inlineOpacityMaskClipValue.NodeId) as RangedFloatNode;
                    if (fnode != null)
                    {
                        m_currentDataCollector.AddToProperties(
                            fnode.UniqueId,
                            fnode.GetPropertyValue(),
                            fnode.OrderIndex
                        );
                        m_currentDataCollector.AddToUniforms(
                            fnode.UniqueId,
                            fnode.GetUniformValue()
                        );
                    }
                    else
                    {
                        IntNode inode =
                            UIUtils.GetNode(m_inlineOpacityMaskClipValue.NodeId) as IntNode;
                        m_currentDataCollector.AddToProperties(
                            inode.UniqueId,
                            inode.GetPropertyValue(),
                            inode.OrderIndex
                        );
                        m_currentDataCollector.AddToUniforms(
                            inode.UniqueId,
                            inode.GetUniformValue()
                        );
                    }
                }
                else
                {
                    m_currentDataCollector.AddToProperties(
                        -1,
                        string.Format(
                            IOUtils.MaskClipValueProperty,
                            OpacityMaskClipValueStr,
                            m_opacityMaskClipValue
                        ),
                        (m_maskClipReorder != null) ? m_maskClipReorder.OrderIndex : -1
                    );
                    m_currentDataCollector.AddToUniforms(
                        -1,
                        string.Format(IOUtils.MaskClipValueUniform, m_opacityMaskClipValue)
                    );
                }
            }
            ...
            if (m_currentLightModel == StandardShaderLightModel.BlinnPhong)
                m_currentDataCollector.AddToProperties(
                    ...
                    m_specColorReorder.OrderIndex
                );

            //Tesselation
            if (m_tessOpHelper.EnableTesselation)
            {
                m_tessOpHelper.AddToDataCollector(
                    ref m_currentDataCollector,
                    m_tessellationReorder != null ? m_tessellationReorder.OrderIndex : -1
                );
                if (!m_currentDataCollector.DirtyPerVertexData)
                {
            ...
            if (
                m_outlineHelper.EnableOutline
                || (
                    m_currentDataCollector.UsingCustomOutlineColor
                    || m_currentDataCollector.CustomOutlineSelectedAlpha > 0
                    || m_currentDataCollector.UsingCustomOutlineWidth
                )
            )
            {
                ...
                OpenSubShaderBody(ref ShaderBody);
                {
                    ...
                    )
                    {
                        if (!usingDebugPort)
                            AddMultilineBody(
                                ref ShaderBody,
                                m_outlineHelper.OutlineFunctionBody(
                                    ref m_currentDataCollector,
                                    isInstancedShader,
                                    m_customShadowCaster,
                                    UIUtils.RemoveInvalidCharacters(ShaderName),
                                    (
                                        m_billboardOpHelper.IsBillboard && !usingDebugPort
                                            ? m_billboardOpHelper.GetInternalMultilineInstructions()
                                            : null
                                    ),
                                    ref m_tessOpHelper,
                                    ShaderModelTypeArr[m_shaderModelIdx],
                    ...
                    AddRenderState(
                        ref ShaderBody,
                        "Cull",
                        m_inlineCullMode.GetValueOrProperty(m_cullMode.ToString())
                    );
                    m_customBlendAvailable = (
                        m_alphaMode == AlphaMode.Custom || m_alphaMode == AlphaMode.Opaque
                    );
                    if (
                        (m_zBufferHelper.IsActive && m_customBlendAvailable)
                        || m_outlineHelper.UsingZWrite
                        || m_outlineHelper.UsingZTest
                    )
                    {
                    ...
                    if (m_alphaToCoverage || m_inlineAlphaToCoverage.Active)
                    {
                        ShaderBody +=
                            "\t\tAlphaToMask "
                            + m_inlineAlphaToCoverage.GetValueOrProperty("On")
                            + "\n";
                    }

                    // Build Color Mask
                    bool forceDefault = m_outlineHelper.ActiveColorMask;
                    m_colorMaskHelper.BuildColorMask(
                        ref ShaderBody,
                        m_customBlendAvailable,
                        forceDefault
                    );
                        ...
                        OpenCGProgram(ref ShaderBody);
                    {
                        ...
                        {
                            UIUtils.ShowMessage(
                                "Automatically changing Shader Model to 3.5 since\nit's the minimum required by texture arrays."
                            );
                            m_shaderModelIdx = 3;
                        }
                        ...
                        {
                            UIUtils.ShowMessage(
                                "Automatically changing Shader Model to 4.6 since\nit's the minimum required by tessellation."
                            );
                            m_shaderModelIdx = 6;
                        }
                        ...
                        {
                            UIUtils.ShowMessage(
                                "Automatically changing Render Path to Forward Only since\ntranslucency only works in forward rendering."
                            );
                            m_renderPath = RenderPath.ForwardOnly;
                        }
                        ...
                        ShaderBody += string.Format(
                            IOUtils.PragmaTargetHeader,
                            ShaderModelTypeArr[m_shaderModelIdx]
                        );

                        //Add pragmas (needs check to see if all pragmas work with custom shadow caster)
                        if (
                            m_currentDataCollector.DirtyPragmas /* && !m_customShadowCaster */
                        )
                        ...
                        //if ( !m_customBlendMode )
                        {
                            switch (m_alphaMode)
                            {
                                ...
                                case AlphaMode.Transparent:
                                    {
                                        OptionalParameters +=
                                            "alpha:fade" + Constants.OptionalParametersSep;
                                    }
                                    break;
                                case AlphaMode.Premultiply:
                                    {
                                        OptionalParameters +=
                                            "alpha:premul" + Constants.OptionalParametersSep;
                                    }
                                    break;
                            }
                        }
                        ...
                        {
                            OptionalParameters +=
                                "finalcolor:RefractionF" + Constants.OptionalParametersSep;
                        }
                        ...
                        {
                            OptionalParameters +=
                                "fullforwardshadows" + Constants.OptionalParametersSep;
                        }
                        ...
                        if (
                            m_renderingOptionsOpHelper.IsOptionActive(" Add Pass") && usingDebugPort
                        )
                        {
                        ...
                        switch (m_renderPath)
                        {
                            case RenderPath.All:
                                break;
                            case RenderPath.DeferredOnly:
                                OptionalParameters +=
                                    "exclude_path:forward" + Constants.OptionalParametersSep;
                                break;
                            case RenderPath.ForwardOnly:
                                OptionalParameters +=
                                    "exclude_path:deferred" + Constants.OptionalParametersSep;
                                break;
                        }
                        ...
                        if (!m_customShadowCaster)
                        {
                            ...
                            if (m_currentDataCollector.DirtyPerVertexData)
                                OptionalParameters +=
                                    "vertex:"
                                    + Constants.VertexDataFunc
                                    + Constants.OptionalParametersSep;
                            ...
                            m_additionalSurfaceOptions.WriteToOptionalSurfaceOptions(
                                ref OptionalParameters
                            );

                            AddShaderPragma(
                                ref ShaderBody,
                                "surface surf "
                                    + standardCustomLighting
                                    + m_currentLightModel.ToString()
                                    + customLightSurface
                                    + Constants.OptionalParametersSep
                                    + OptionalParameters
                        ...
                        else
                        {
                            if ( /*m_currentDataCollector.UsingWorldNormal ||*/
                                m_currentDataCollector.UsingInternalData
                            )
                            {
                        ...
                        {
                            m_currentDataCollector.SetupInstancePropertiesBlock(
                                UIUtils.RemoveInvalidCharacters(ShaderName)
                            );
                        ...
                        {
                            ShaderBody +=
                                m_tessOpHelper.GetCurrentTessellationFunction(
                                    ref m_currentDataCollector
                                ) + "\n";
                        }
                        ...
                        if (m_currentLightModel == StandardShaderLightModel.CustomLighting)
                        {
                            ShaderBody +=
                                ...
                                + outputStruct
                                + " "
                                + Constants.CustomLightOutputVarStr
                                + ", half3 viewDir, UnityGI gi )\n\t\t{\n";
                            ...
                            ShaderBody +=
                                "\t\t\tc.rgb = "
                                + (
                                    !string.IsNullOrEmpty(customLightingCode)
                                        ? customLightingCode
                                        : "0"
                                )
                                + ";\n";
                            ShaderBody +=
                                "\t\t\tc.a = "
                                + (
                                    !string.IsNullOrEmpty(customLightingAlphaCode)
                                        ? customLightingAlphaCode
                                        : "1"
                                )
                                + ";\n";
                            if (
                                ...
                                || (
                                    ...
                                    && m_blendOpsHelper.CurrentBlendRGB.IndexOf("Premultiplied")
                                        > -1
                                )
                            )
                        ...
                        if (hasTranslucency || hasTransmission)
                        {
                            ...
                            if (hasTranslucency)
                            {
                                ...
                                ShaderBody +=
                                    ...
                                    + Constants.CustomLightOutputVarStr
                                    + ".Normal * _TransNormalDistortion;\n";
                                ShaderBody +=
                                    "\t\t\thalf transVdotL = pow( saturate( dot( viewDir, -lightDir ) ), _TransScattering );\n";
                                ShaderBody +=
                                    "\t\t\thalf3 translucency = lightAtten * (transVdotL * _TransDirect + gi.indirect.diffuse * _TransAmbient) * "
                                    + Constants.CustomLightOutputVarStr
                                    + ".Translucency;\n";
                                ShaderBody +=
                                    "\t\t\thalf4 c = half4( "
                                    + Constants.CustomLightOutputVarStr
                                    + ".Albedo * translucency * _Translucency, 0 );\n\n";
                            }

                            if (hasTransmission)
                            {
                                ShaderBody +=
                                    "\t\t\thalf3 transmission = max(0 , -dot("
                                    + Constants.CustomLightOutputVarStr
                                    + ".Normal, gi.light.dir)) * gi.light.color * "
                                    + Constants.CustomLightOutputVarStr
                                    + ".Transmission;\n";
                                ShaderBody +=
                                    "\t\t\thalf4 d = half4("
                                    + Constants.CustomLightOutputVarStr
                                    + ".Albedo * transmission , 0);\n\n";
                            }

                            ShaderBody +=
                                "\t\t\tSurfaceOutput" + m_currentLightModel.ToString() + " r;\n";
                            ShaderBody +=
                                "\t\t\tr.Albedo = "
                                + Constants.CustomLightOutputVarStr
                                + ".Albedo;\n";
                            ...
                            ShaderBody +=
                                "\t\t\t\tgi = UnityGlobalIllumination(data, "
                                + Constants.CustomLightOutputVarStr
                                + ".Occlusion, "
                                + Constants.CustomLightOutputVarStr
                                + ".Normal);\n";
                            ShaderBody += "\t\t\t#else\n";
                            ShaderBody +=
                                "\t\t\t\tUNITY_GLOSSY_ENV_FROM_SURFACE( g, "
                                + Constants.CustomLightOutputVarStr
                                + ", data );\n";
                            ShaderBody +=
                                "\t\t\t\tgi = UnityGlobalIllumination( data, "
                        ...
                        {
                            ShaderBody +=
                                "\t\tinline float4 Refraction( Input "
                                + Constants.InputVarStr
                                + ", "
                                + outputStruct
                                + " "
                                + Constants.OutputVarStr
                                + ", float indexOfRefraction, float chomaticAberration ) {\n";
                            ShaderBody +=
                                "\t\t\tfloat3 worldNormal = "
                                + Constants.OutputVarStr
                                + ".Normal;\n";
                            ...
                            ShaderBody +=
                                "\t\t\tfloat3 worldViewDir = normalize( UnityWorldSpaceViewDir( "
                                + Constants.InputVarStr
                                + ".worldPos ) );\n";
                            ShaderBody +=
                            ...
                            ShaderBody +=
                                "\t\t\tfloat4 redAlpha = tex2D( "
                                + grabpass
                                + ", ( projScreenPos + cameraRefraction ) );\n";
                            ShaderBody +=
                                "\t\t\tfloat green = tex2D( "
                                + grabpass
                                + ", ( projScreenPos + ( cameraRefraction * ( 1.0 - chomaticAberration ) ) ) ).g;\n";
                            ShaderBody +=
                                "\t\t\tfloat blue = tex2D( "
                                + grabpass
                                + ", ( projScreenPos + ( cameraRefraction * ( 1.0 + chomaticAberration ) ) ) ).b;\n";
                            ShaderBody +=
                                "\t\t\treturn float4( redAlpha.r, green, blue, redAlpha.a );\n";
                            ShaderBody += "\t\t}\n\n";

                            ShaderBody +=
                                "\t\tvoid RefractionF( Input "
                                + Constants.InputVarStr
                                + ", "
                                + outputStruct
                                + " "
                            ...
                            {
                                ShaderBody +=
                                    "\t\t\tcolor.rgb = color.rgb + Refraction( "
                                    + Constants.InputVarStr
                                    + ", "
                                    + Constants.OutputVarStr
                                    + ", "
                                    + refractionCode
                                    + ", "
                                    + m_inlineChromaticAberration.GetValueOrProperty(false)
                                    + " ) * ( 1 - color.a );\n";
                            }
                            else
                            {
                                ShaderBody +=
                                    ...
                                    + ", "
                                    + refractionCode
                                    + ", _ChromaticAberration ) * ( 1 - color.a );\n";
                            }
                    ...
                    {
                        OpenCGProgram(ref ShaderBody);
                        string customLightSurface =
                            hasTranslucency || hasTransmission ? "Custom" : "";
                        m_renderingPlatformOpHelper.SetRenderingPlatforms(ref ShaderBody);
                        ...
                        ShaderBody +=
                            "\t\t\t#pragma target " + ShaderModelTypeArr[m_shaderModelIdx] + "\n";
                        ...
                        if (
                            !(
                                (m_alphaToCoverage || m_inlineAlphaToCoverage.Active)
                                && hasOpacity
                                && hasOpacityMask
                            )
                        )
                        ...
                        if (!m_currentDataCollector.UsingInternalData)
                            ShaderBody +=
                                "\t\t\t\tfloat3 worldPos : TEXCOORD" + (texcoordIndex++) + ";\n";
                        if (m_currentDataCollector.UsingScreenPos)
                            ShaderBody +=
                                "\t\t\t\tfloat4 screenPos : TEXCOORD" + (texcoordIndex++) + ";\n";
                        if ( /*m_currentDataCollector.UsingWorldNormal || m_currentDataCollector.UsingWorldPosition ||*/
                            m_currentDataCollector.UsingInternalData
                            || m_currentDataCollector.DirtyNormal
                        )
                        {
                            ShaderBody +=
                                "\t\t\t\tfloat4 tSpace0 : TEXCOORD" + (texcoordIndex++) + ";\n";
                            ShaderBody +=
                                "\t\t\t\tfloat4 tSpace1 : TEXCOORD" + (texcoordIndex++) + ";\n";
                            ShaderBody +=
                                "\t\t\t\tfloat4 tSpace2 : TEXCOORD" + (texcoordIndex++) + ";\n";
                        }
                        else if (
                            !m_currentDataCollector.UsingInternalData
                            && m_currentDataCollector.UsingWorldNormal
                        )
                        {
                            ShaderBody +=
                                "\t\t\t\tfloat3 worldNormal : TEXCOORD" + (texcoordIndex++) + ";\n";
                        }
                        ...
                        ShaderBody +=
                            "\t\t\tv2f vert( "
                            + m_currentDataCollector.SurfaceVertexStructure
                            + " v )\n";
                        ShaderBody += "\t\t\t{\n";
                        ShaderBody += "\t\t\t\tv2f o;\n";
                        ...
                        if (
                            m_currentDataCollector.DirtyPerVertexData
                            || m_currentDataCollector.CustomShadowCoordsList.Count > 0
                        )
                        ...
                        if (m_currentDataCollector.DirtyPerVertexData)
                        {
                            ShaderBody +=
                                "\t\t\t\tvertexDataFunc( v"
                                + (
                                    m_currentDataCollector.TesselationActive
                                        ? ""
                                        : ", customInputData"
                                )
                                + " );\n";
                        ...
                        for (
                            int i = 0;
                            i < m_currentDataCollector.CustomShadowCoordsList.Count;
                            i++
                        )
                        {
                            int size = UIUtils.GetChannelsAmount(
                                m_currentDataCollector.CustomShadowCoordsList[i].DataType
                            );
                            string channels = string.Empty;
                            for (int j = 0; j < size; j++)
                            {
                                channels += Convert.ToChar(
                                    120
                                        + m_currentDataCollector
                                            .CustomShadowCoordsList[i]
                                            .TextureIndex
                                        + j
                                );
                            }
                            ...
                            ShaderBody +=
                                "\t\t\t\to.customPack"
                                + (m_currentDataCollector.CustomShadowCoordsList[i].TextureSlot + 1)
                                + "."
                                + channels
                                + " = customInputData."
                                + m_currentDataCollector.CustomShadowCoordsList[i].CoordName
                                + ";\n";
                            ...
                            if (
                                m_currentDataCollector
                                    .CustomShadowCoordsList[i]
                                    .CoordName.StartsWith("uv_")
                            )
                            {
                                ShaderBody +=
                                    "\t\t\t\to.customPack"
                                    + (
                                        m_currentDataCollector.CustomShadowCoordsList[i].TextureSlot
                                        + 1
                                    )
                                    + "."
                                    + channels
                        ...
                        if (
                            m_currentDataCollector.UsingViewDirection
                            && !m_currentDataCollector.DirtyNormal
                        )
                        ...
                        ShaderBody +=
                            "\t\t\t\tUNITY_INITIALIZE_OUTPUT( " + outputStruct + ", o )\n";
                        ShaderBody += "\t\t\t\tsurf( surfIN, o );\n";
                        if (
                            (hasOpacity || hasOpacityMask)
                            && m_currentLightModel == StandardShaderLightModel.CustomLighting
                        )
                        {
                        ...
                        /*if( ( ( m_alphaToCoverage || m_inlineAlphaToCoverage.Active ) && hasOpacity && m_inputPorts[ m_discardPortId ].IsConnected ) )
                        {
                            ...
                            !(
                                (m_alphaToCoverage || m_inlineAlphaToCoverage.Active)
                                && hasOpacity
                                && m_inputPorts[m_discardPortId].IsConnected
                            ) && hasOpacity
                        )
                        {
                            ShaderBody +=
            ...
            {
                IOUtils.StartSaveThread(
                    ShaderBody,
                    (isFullPath ? pathname : (IOUtils.dataPath + pathname))
                );
            }
            else
            {
                IOUtils.StartSaveThread(
                    ...
                    Application.dataPath
                        + "/AmplifyShaderEditor/Samples/Shaders/"
                        + m_shaderName
                        + ".shader"
                );
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            try
            {
                ...
                m_currentLightModel = (StandardShaderLightModel)
                    Enum.Parse(typeof(StandardShaderLightModel), GetCurrentParam(ref nodeParams));

                if (
                    CurrentMasterNodeCategory == AvailableShaderTypes.SurfaceShader
                    && m_currentLightModel == StandardShaderLightModel.CustomLighting
                )
                {
                    ContainerGraph.CurrentCanvasMode = NodeAvailability.CustomLighting;
                ...
                m_renderingOptionsOpHelper.ReadFromString(
                    ref m_currentReadParamIdx,
                    ref nodeParams
                );

                m_cullMode = (CullMode)
                    Enum.Parse(typeof(CullMode), GetCurrentParam(ref nodeParams));
                ...
                if (UIUtils.CurrentShaderVersion() > 11)
                {
                    ...
                    m_renderType = (RenderType)
                        Enum.Parse(typeof(RenderType), GetCurrentParam(ref nodeParams));
                    if (UIUtils.CurrentShaderVersion() > 14305)
                    {
                        m_customRenderType = GetCurrentParam(ref nodeParams);
                    }
                    m_renderQueue = (RenderQueue)
                        Enum.Parse(typeof(RenderQueue), GetCurrentParam(ref nodeParams));
                }
                if (UIUtils.CurrentShaderVersion() > 2402)
                {
                    m_renderPath = (RenderPath)
                        Enum.Parse(typeof(RenderPath), GetCurrentParam(ref nodeParams));
                }
                if (UIUtils.CurrentShaderVersion() > 2405)
                {
                    m_renderingPlatformOpHelper.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                }
                ...
                if (UIUtils.CurrentShaderVersion() > 6101)
                {
                    m_vertexMode = (VertexMode)
                        Enum.Parse(typeof(VertexMode), GetCurrentParam(ref nodeParams));
                }
                ...
                if (
                    UIUtils.CurrentShaderVersion() > 10010
                    && UIUtils.CurrentShaderVersion() < 15312
                )
                {
                ...
                if (
                    UIUtils.CurrentShaderVersion() > 13102
                    && UIUtils.CurrentShaderVersion() < 15312
                )
                {
                ...
                if (
                    UIUtils.CurrentShaderVersion() > 14005
                    && UIUtils.CurrentShaderVersion() < 15312
                )
                {
                ...
                {
                    m_inlineOpacityMaskClipValue.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                }
                ...
                {
                    m_additionalDirectives.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                    m_additionalSurfaceOptions.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
                }
                ...
                {
                    m_additionalDirectives.AddItems(
                        AdditionalLineType.Define,
                        m_additionalDefines.DefineList
                    );
                    m_additionalDirectives.AddItems(
                        AdditionalLineType.Include,
                        m_additionalIncludes.IncludeList
                    );
                    m_additionalDirectives.AddItems(
                        AdditionalLineType.Pragma,
                        m_additionalPragmas.PragmaList
                    );
                }
                ...
                if (UIUtils.CurrentShaderVersion() > 16204)
                    m_inlineChromaticAberration.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams,
                        false
                    );
                ...
                if (UIUtils.CurrentShaderVersion() > 16207)
                    m_inlineAlphaToCoverage.ReadFromString(
                        ref m_currentReadParamIdx,
                        ref nodeParams
                    );
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (
                m_currentLightModel == StandardShaderLightModel.CustomLighting
                && m_inputPorts[m_emissionPortId].IsConnected
                && UIUtils.CurrentShaderVersion() < 13802
            )
            {
                OutputPort port = m_inputPorts[m_emissionPortId].GetOutputConnection(0);
                m_inputPorts[m_emissionPortId].FullDeleteConnections();
                UIUtils.SetConnection(
                    m_inputPorts[m_customLightingPortId].NodeId,
                    m_inputPorts[m_customLightingPortId].PortId,
                    port.NodeId,
                    port.PortId
                );
            }
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_maskClipReorder != null) ? m_maskClipReorder.OrderIndex : -1
            );
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_translucencyReorder != null) ? m_translucencyReorder.OrderIndex : -1
            );
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_refractionReorder != null) ? m_refractionReorder.OrderIndex : -1
            );
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_tessellationReorder != null) ? m_tessellationReorder.OrderIndex : -1
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_specColorReorder != null) ? m_specColorReorder.OrderIndex : -1
            );
        ...
        private void UpdateFromBlendMode()
        {
            ...
            m_blendOpsHelper.SetBlendOpsFromBlendMode(
                m_alphaMode,
                (m_alphaMode == AlphaMode.Custom || m_alphaMode == AlphaMode.Opaque)
            );
        }
        ...
        public StandardShaderLightModel CurrentLightingModel
        {
            get { return m_currentLightModel; }
        }
        ...
        public OutlineOpHelper OutlineHelper
        {
            get { return m_outlineHelper; }
        }
        public float OpacityMaskClipValue
        {
            get { return m_opacityMaskClipValue; }
        }
        public InlineProperty InlineOpacityMaskClipValue
        {
            get { return m_inlineOpacityMaskClipValue; }
            set { m_inlineOpacityMaskClipValue = value; }
        }
                ...
                return (
                    ...
                    && (
                        ...
                        || (
                            ...
                            && (
                                m_currentDataCollector.UsingWorldNormal
                                || m_currentDataCollector.UsingWorldReflection
                                || m_currentDataCollector.UsingViewDirection
                            )
                        )
                        || (
                            m_castShadows
                            && m_inputPorts[m_discardPortId].Available
                            && m_inputPorts[m_discardPortId].IsConnected
                            && m_currentLightModel == StandardShaderLightModel.CustomLighting
                        )
                    )
        ...
        {
            get { return AvailableShaderTypes.SurfaceShader; }
        }
    }
}
