...
namespace AmplifyShaderEditor
{
    ...
    public class BillboardOpHelper
    {
            ...
            NodeUtils.DrawPropertyGroup(
                ...
                () =>
                {
                    m_billboardType = (BillboardType)
                        owner.EditorGUILayoutEnumPopup(BillboardTypeStr, m_billboardType);
                    m_rotationIndependent = owner.EditorGUILayoutToggle(
                        BillboardRotIndStr,
                        m_rotationIndependent
                    );
                    m_affectNormalTangent = owner.EditorGUILayoutToggle(
                        BillboardAffectNormalTangentStr,
        ...
        public static void FillDataCollector(
            ref MasterNodeDataCollector dataCollector,
            BillboardType billboardType,
            bool rotationIndependent,
            string vertexPosValue,
            string vertexNormalValue,
            string vertexTangentValue,
            bool vertexIsFloat3,
            bool affectNormalTangent
        )
        {
                ...
                case BillboardType.Cylindrical:
                    {
                        for (int i = 0; i < BillboardCylindricalInstructions.Length; i++)
                        {
                            dataCollector.AddVertexInstruction(
                                BillboardCylindricalInstructions[i]
                                    + (dataCollector.IsTemplate ? ";" : string.Empty),
                                -1,
                                true
                            );
                        }
                ...
                case BillboardType.Spherical:
                    {
                        ...
                        {
                            dataCollector.AddVertexInstruction(
                                BillboardSphericalInstructions[i]
                                    + (dataCollector.IsTemplate ? ";" : string.Empty),
                                -1,
                                true
                            );
            ...
            {
                string normalValue = string.Format(
                    BillboardCommonInstructions[3],
                    vertexNormalValue
                );
                dataCollector.AddVertexInstruction(
                    normalValue + (dataCollector.IsTemplate ? ";" : string.Empty),
                    -1,
                    true
                );

                string tangentValue = string.Format(
                    BillboardCommonInstructions[3],
                    vertexTangentValue
                );
                dataCollector.AddVertexInstruction(
                    tangentValue + (dataCollector.IsTemplate ? ";" : string.Empty),
                    -1,
                    true
                );
            }
            ...
            if (rotationIndependent)
            {
                for (int i = 0; i < BillboardRotIndependent.Length; i++)
                {
                    ...
                    dataCollector.AddVertexInstruction(
                        value + (dataCollector.IsTemplate ? ";" : string.Empty),
                        -1,
                        true
                    );
                }
            ...
            {
                string vertexPosConverted = vertexIsFloat3
                    ? string.Format("float4({0},0)", vertexPosValue)
                    : vertexPosValue;
        ...
        public string[] GetInternalMultilineInstructions()
        {
            ...
            return GetMultilineInstructions(
                m_billboardType,
                m_rotationIndependent,
                "v.vertex",
                "v.normal",
