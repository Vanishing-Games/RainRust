...
namespace AmplifyShaderEditor
{
    ...
    public class TerrainDrawInstancedHelper
    {
        ...
        private readonly string ApplyMeshModificationInstruction =
            "{0} = ApplyMeshModification({0});";
        ...
        public void UpdateDataCollectorForTemplates(
        ...
        )
        {
            if (m_enable)
            {
                ...
                string[] instancedPragmas = dataCollector.IsSRP
                    ? InstancedPragmasSRP
                    : InstancedPragmas;
                ...
                {
                    TemplateFunctionData functionData = dataCollector
                        .TemplateDataCollectorInstance
                        .CurrentTemplateData
                        .VertexFunctionData;
                    string uvCoord = dataCollector.TemplateDataCollectorInstance.GetUV(
                        0,
                        MasterNodePortCategory.Vertex
                    );
                    string vertexNormal =
                        dataCollector.TemplateDataCollectorInstance.GetVertexNormal(
                            PrecisionType.Float,
                            false,
                            MasterNodePortCategory.Vertex
                        );
                    ...
                    string functionHeader = string.Format(
                        ApplyMeshModificationFunctionSRP[0],
                        functionData.InVarType,
                        functionData.InVarName
                    );
                    ...
                    string vertexVarName = dataCollector
                        .TemplateDataCollectorInstance
                        .CurrentTemplateData
                        .VertexFunctionData
                        .InVarName;
                    vertexInstructions.Insert(
                        0,
                        string.Format(ApplyMeshModificationInstruction, vertexVarName)
                    );
                }
                else
                {
                    ...
                    string functionHeader = string.Format(
                        ApplyMeshModificationFunctionDefaultTemplate[0],
                        functionData.InVarType,
                        functionData.InVarName
                    );
                    string functionBody =
                        ...
                        + string.Format(
                            ApplyMeshModificationFunctionDefaultTemplate[10],
                            vertexNormal,
                            uvCoord
                        )
                        + ApplyMeshModificationFunctionDefaultTemplate[11]
                        + string.Format(
                            ApplyMeshModificationFunctionDefaultTemplate[12],
                            functionData.InVarName
                        )
        ...
        public void UpdateDataCollectorForStandard(ref MasterNodeDataCollector dataCollector)
        {
            if (m_enable)
            {
                ...
                string functionHeader = string.Format(
                    ApplyMeshModificationFunctionStandard[0],
                ...
                for (int i = 1; i < ApplyMeshModificationFunctionStandard.Length; i++)
                {
                    IOUtils.AddFunctionLine(
                        ref functionBody,
                        ApplyMeshModificationFunctionStandard[i]
                    );
                }
                ...
                dataCollector.AddVertexInstruction(
                    string.Format(ApplyMeshModificationInstructionStandard, "v")
                );
            }
        }
