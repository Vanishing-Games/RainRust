...
namespace AmplifyShaderEditor
{
    ...
    public sealed class LinearToGammaNode : HelperParentNode
    {
        ...
        public readonly static string[] ModeListStr =
        {
            "Fast Linear to sRGB",
            "Exact Linear to sRGB",
        };
        public static readonly int[] ModeListInt = { 0, 1 };

        public static readonly string[] ModeListStrLW =
        {
            "Fast Linear to sRGB",
            "Exact Linear to sRGB",
            "Linear to Gamma 2.0",
        ...
        public static readonly int[] ModeListIntLW = { 0, 1, 2, 3 };
            ...
            if (!dataCollector.IsSRP)
            {
                ...
                if (m_selectedMode == 1)
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "half3 " + m_localVarName + " = " + result + ";"
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        m_localVarName
                            + " = half3( LinearToGammaSpaceExact("
                            + m_localVarName
                            + ".r), LinearToGammaSpaceExact("
                            + m_localVarName
                            + ".g), LinearToGammaSpaceExact("
                            + m_localVarName
                            + ".b) );"
                    );
