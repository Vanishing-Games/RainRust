    ...
    public sealed class CameraDepthFade : ParentNode
    {
        ...
        )
        {
            ...
            )
            {
                ...
                value = string.Format(
                    CameraDepthFadeFormat,
                    "-UnityObjectToViewPos( " + vertexVarName + " ).z",
                    offset,
                    distance
                );
                RegisterLocalVariable(0, value, ref dataCollector, "cameraDepthFade" + OutputId);
            ...
            if (dataCollector.TesselationActive)
            {
                if (vertexPort.IsConnected)
                {
                    ...
                    RegisterLocalVariable(
                        0,
                        string.Format("-UnityObjectToViewPos( {0} ).z", vertexValue),
                        ref dataCollector,
                        eyeDepth
                    );
                }
                ...
                {
                    eyeDepth = GeneratorUtils.GenerateScreenDepthOnFrag(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType
                    );
