...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TriplanarNode : ParentNode
    {
        ...
        private readonly List<string> m_functionSamplingBodyReturnSphereNormalize =
            new List<string>()
            {
                "return normalize( xNorm.xyz * projNormal.x + yNorm.xyz * projNormal.y + zNorm.xyz * projNormal.z );",
            };

        private readonly List<string> m_functionSamplingBodyReturnCylinderNormalize =
            new List<string>()
            {
        ...
        public override void SetPreviewInputs()
        {
            ...
            if (m_selectedTriplanarType == TriplanarType.Cylindrical && m_midTexture != null)
            {
                if (m_midTexPort.IsConnected)
                    PreviewMaterial.SetTexture(
                        "_B",
                        m_midTexPort.InputPreviewTexture(ContainerGraph)
                    );
                else
                ...
                if (m_botTexPort.IsConnected)
                    PreviewMaterial.SetTexture(
                        "_C",
                        m_botTexPort.InputPreviewTexture(ContainerGraph)
                    );
                else
            ...
            PreviewMaterial.SetFloat(
                "_IsSpherical",
                (m_selectedTriplanarType == TriplanarType.Spherical ? 1 : 0)
            );
        }
                ...
                if (!m_topTexPort.CheckValidType(type))
                {
                    ...
                    UIUtils.ShowMessage(
                        ...
                        "Triplanar Sampler node only accepts SAMPLER2D and SAMPLER2DARRAY input types.\nTexture Object connected changed to "
                            + type
                            + ", connection was lost, please review and update accordingly.",
        ...
        public void ConfigurePorts()
        {
            ...
            if (
                m_midTexPort.DataType == WirePortDataType.SAMPLER2DARRAY
            ...
            if (
                m_botTexPort.DataType == WirePortDataType.SAMPLER2DARRAY
                && m_selectedTriplanarType == TriplanarType.Cylindrical
            )
        ...
        public override void DrawProperties()
        {
            ...
            if (m_selectedTriplanarType == TriplanarType.Cylindrical)
            {
                if (!m_midTexPort.IsConnected)
                    NodeUtils.DrawPropertyGroup(
                        ref m_midTextureFoldout,
                        "Middle Texture",
                        DrawMidTextureOptions
                    );
                if (!m_botTexPort.IsConnected)
                    NodeUtils.DrawPropertyGroup(
                        ref m_botTextureFoldout,
                        "Bottom Texture",
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_editing)
            {
                ...
                if (
                    commandName.Equals("ObjectSelectorUpdated")
                    || commandName.Equals("ObjectSelectorClosed")
                )
                {
                    ...
                    if (m_pickId == 2)
                    {
                        if (newValue != (UnityEngine.Object)m_botTexture.Value)
                        {
                            PreviewIsDirty = true;
                            UndoRecordObject(
                                "Changing value EditorGUIObjectField on node Triplanar Node"
                            );
            ...
            if (drawInfo.CurrentEventType == EventType.Repaint)
            {
                ...
                if (m_topTexPort.IsConnected)
                {
                    EditorGUI.DrawPreviewTexture(
                        startPickerClone,
                        m_topTexPort.GetOutputConnection(0).OutputPreviewTexture,
                        null,
                ...
                if (m_selectedTriplanarType == TriplanarType.Cylindrical)
                {
                    ...
                    if (m_midTexPort.IsConnected)
                    {
                        EditorGUI.DrawPreviewTexture(
                            startPickerClone,
                            m_midTexPort.GetOutputConnection(0).OutputPreviewTexture,
                            null,
                            ScaleMode.ScaleAndCrop
                        );
                    }
                    ...
                    if (m_botTexPort.IsConnected)
                    {
                        EditorGUI.DrawPreviewTexture(
                            startPickerClone,
                            m_botTexPort.GetOutputConnection(0).OutputPreviewTexture,
                            null,
                            ScaleMode.ScaleAndCrop
                        );
                    }
        ...
        public override void DrawTitle(Rect titlePos)
        {
            if (m_editPropertyNameMode)
            {
                ...
                m_propertyInspectorName = GUITextField(
                    titlePos,
        ...
        )
        {
            if (m_outputPorts[0].IsLocalValue(dataCollector.PortCategory))
                return GetOutputColorItem(
                    0,
                    outputId,
                    m_outputPorts[0].LocalValue(dataCollector.PortCategory)
                );
            ...
            else
            {
                ...
                dataCollector.AddToProperties(
                    UniqueId,
                    m_topTexture.GetTexture2DPropertyValue(),
            ...
            if (m_selectedTriplanarType == TriplanarType.Spherical)
            {
                ...
                if (
                    (
                        outsideGraph.SamplingMacros
                        || m_topTexPort.DataType == WirePortDataType.SAMPLER2DARRAY
                    )
                )
                    ssTop = GeneratorUtils.GenerateSamplerState(
                        ref dataCollector,
                        UniqueId,
                        texTop,
                        VariableMode.Create
            ...
            else
            {
                ...
                else
                {
                    ...
                    dataCollector.AddToProperties(
                        UniqueId,
                        m_midTexture.GetTexture2DPropertyValue(),
                ...
                else
                {
                    ...
                    dataCollector.AddToProperties(
                        UniqueId,
                        m_botTexture.GetTexture2DPropertyValue(),
                ...
                if (
                    ...
                    ssMid = GeneratorUtils.GenerateSamplerState(
                        ref dataCollector,
                        UniqueId,
                        texMid,
                        VariableMode.Create
                ...
                if (
                    ...
                    ssBot = GeneratorUtils.GenerateSamplerState(
                        ref dataCollector,
                        UniqueId,
                        texBot,
                        VariableMode.Create
            ...
            {
                samplers =
                    GeneratorUtils.GetPropertyDeclaraction("topTexMap", m_topTexPort.DataType, ", ")
                    + GeneratorUtils.GetSamplerDeclaraction(
                        "samplertopTexMap",
                        m_topTexPort.DataType,
                        ", "
                    );
                ...
                triplanarBody.Add(
                    "xNorm = "
                        + GeneratorUtils.GenerateSamplingCall(
                            ref dataCollector,
                            m_topTexPort.DataType,
                            "topTexMap",
                            "samplertopTexMap",
                            string.Format(
                                "{0}tiling * worldPos.zy * float2(  nsign.x, 1.0 ){1}",
                                array1,
                                array2
                            ),
                            mip,
                ...
                triplanarBody.Add(
                    "yNorm = "
                        + GeneratorUtils.GenerateSamplingCall(
                            ...
                            "samplertopTexMap",
                            string.Format(
                                "{0}tiling * worldPos.xz * float2(  nsign.y, 1.0 ){1}",
                                array1,
                                array2
                            ),
                            mip,
                ...
                triplanarBody.Add(
                    "zNorm = "
                        + GeneratorUtils.GenerateSamplingCall(
                            ...
                            "samplertopTexMap",
                            string.Format(
                                "{0}tiling * worldPos.xy * float2( -nsign.z, 1.0 ){1}",
                                array1,
                                array2
                            ),
                            mip,
                ...
                if (m_normalCorrection)
                {
                    if (scaleNormals)
                    {
                        ConvertListTo(
                            dataCollector,
                            true,
                            m_functionSamplingBodySignsSphereScale,
                            triplanarBody
                        );
                    }
                    else
                    {
                        ConvertListTo(
                            dataCollector,
                            false,
                            m_functionSamplingBodySignsSphere,
                            triplanarBody
                        );
                    }
            ...
            else
            {
                ...
                string midArgs =
                    GeneratorUtils.GetPropertyDeclaraction("midTexMap", m_midTexPort.DataType, ", ")
                    + GeneratorUtils.GetSamplerDeclaraction(
                        "samplermidTexMap",
                        m_midTexPort.DataType,
                        ", "
                    );
                string botArgs =
                    GeneratorUtils.GetPropertyDeclaraction("botTexMap", m_botTexPort.DataType, ", ")
                    + GeneratorUtils.GetSamplerDeclaraction(
                        "samplerbotTexMap",
                        m_botTexPort.DataType,
                        ", "
                    );
                ...
                if (m_normalCorrection)
                {
                    if (scaleNormals)
                    {
                        if (!(dataCollector.IsTemplate && dataCollector.IsSRP))
                        {
                            dataCollector.AddToIncludes(
                                UniqueId,
                                Constants.UnityStandardUtilsLibFuncs
                            );
                        }
                        ...
                        ConvertListTo(
                            dataCollector,
                            true,
                            m_functionSamplingBodySignsCylinderScale,
                            triplanarBody
                        );
                    }
                    else
                    {
                        ...
                        ConvertListTo(
                            dataCollector,
                            false,
                            m_functionSamplingBodySignsCylinder,
                            triplanarBody
                        );
                    }
            ...
            callHeader = string.Format(
                m_functionCall,
                headerID,
                "{0}, {1}, {2}, {3}, {4}, {5}, {6}" + extraArguments
            );
            ...
            if (m_normalCorrection && m_normalSpace == ViewSpace.Tangent)
                worldToTangent = GeneratorUtils.GenerateWorldToTangentMatrix(
                    ref dataCollector,
                    UniqueId,
            ...
            if (m_selectedTriplanarSpace == TriplanarSpace.Object)
            {
                if (m_normalCorrection && m_normalSpace == ViewSpace.Tangent)
                {
                    string vt = GeneratorUtils.GenerateVertexTangent(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType,
                        WirePortDataType.FLOAT3
                    );
                    string vbt = GeneratorUtils.GenerateVertexBitangent(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType
                    );
                    norm = GeneratorUtils.GenerateVertexNormal(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType
                    );
                    ...
                    pos = GeneratorUtils.GenerateVertexPosition(
                        ref dataCollector,
                        UniqueId,
                        WirePortDataType.FLOAT3
                    );
            ...
            if (m_selectedTriplanarType == TriplanarType.Spherical)
                call = dataCollector.AddFunctions(
                    callHeader,
                    samplingTriplanar,
                    texTop + ssTop,
                    pos,
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_selectedTriplanarType = (TriplanarType)
                Enum.Parse(typeof(TriplanarType), GetCurrentParam(ref nodeParams));
            m_selectedTriplanarSpace = (TriplanarSpace)
                Enum.Parse(typeof(TriplanarSpace), GetCurrentParam(ref nodeParams));
            ...
            m_tempTopDefaultValue = (TexturePropertyValues)
            ...
            m_tempTopDefaultTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(
            ...
            if (UIUtils.CurrentShaderVersion() > 18201)
                m_normalSpace = (ViewSpace)
                    Enum.Parse(typeof(ViewSpace), GetCurrentParam(ref nodeParams));
