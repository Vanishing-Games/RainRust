...
namespace AmplifyShaderEditor
{
    ...
    public sealed class SurfaceDepthNode : ParentNode
    {
        ...
        private string ApplyLinearDepthModifier(
        ...
        )
        {
            ...
            if (dataCollector.IsTemplate)
            {
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    string clipPos = dataCollector.TemplateDataCollectorInstance.GetClipPosForValue(
                        vertexPos,
                        OutputId
                    );
            ...
            if (m_inputPorts[0].IsConnected)
            {
                ...
                else
                {
                    ...
                    string clipDepthOutput = string.Format(
                        "{0}.{1}",
                        Constants.VertexShaderOutputStr,
                        clipDepth
                    );
                    string clipDepthInput = string.Format(
                        "{0}.{1}",
                        Constants.InputVarStr,
                        clipDepth
                    );

                    string vertexInstruction = string.Format(
                        "UnityObjectToClipPos( {0} ).zw",
                        vertexPos
                    );
                    ...
                    fragmentInstruction = ApplyLinearDepthModifier(
                        ref dataCollector,
                        fragmentInstruction
                    );
            ...
            {
                return GeneratorUtils.GenerateSurfaceDepth(
                    ref dataCollector,
                    UniqueId,
                    CurrentPrecisionType,
                    m_depthMode
