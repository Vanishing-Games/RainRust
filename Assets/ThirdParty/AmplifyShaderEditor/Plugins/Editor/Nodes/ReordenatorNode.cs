...
namespace AmplifyShaderEditor
{
    ...
    public class ReordenatorNode : PropertyNode
    {
        ...
        public ReordenatorNode()
            : base() { }

        public void Init(
            string entryName,
            string entryInspectorName,
        ...
        {
            get { return m_headerTitle; }
            set { m_headerTitle = value; }
        }

        public bool HasTitle
        {
            get { return !string.IsNullOrEmpty(m_headerTitle); }
        }
        ...
        public int RecursiveSetOrderOffset(int offset, bool lockit, int order = -1)
        {
            ...
            if (m_propertyList != null)
                m_propertyList.Sort(
                    (x, y) =>
                    {
                        return (x as PropertyNode).OrderIndex.CompareTo(
                            (y as PropertyNode).OrderIndex
                        );
                    }
                );
        ...
        public void RecursiveLog()
        {
            ...
            for (int i = 0; i < m_propertyList.Count; i++)
            {
                if ((m_propertyList[i] is ReordenatorNode))
                    ...
                    Debug.Log(
                        (m_propertyList[i] as PropertyNode).OrderIndex
                            + " "
                            + (m_propertyList[i] as PropertyNode).PropertyName
                    );
            }
        }
