...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ScreenColorNode : PropertyNode
    {
        ...
        private string LWFetchOpaqueTexture =
            "float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( {0}.xy ), 1.0 )";

#if UNITY_2021_1_OR_NEWER
        private const string URP2DHelpBox =
            "For the Grab Screen Color to properly work a proper setup is required:"
        ...
        private readonly string[] URP2DDeclaration =
        {
            "TEXTURE2D_X( _CameraSortingLayerTexture );",
            "SAMPLER( sampler_CameraSortingLayerTexture );",
        };
        private readonly string URP2DFunctionHeader =
            "float4( ASESample2DSortingLayer({0}.xy), 1.0 )";
        ...
        public ScreenColorNode()
            : base() { }

        public ScreenColorNode(int uniqueId, float x, float y, float width, float height)
        ...
        private void OnReorderEventComplete()
        {
            ...
            {
                m_referenceArrayId = ContainerGraph.ScreenColorNodes.GetNodeRegisterIdx(
                    m_referenceNode.UniqueId
                );
            }
        }
        ...
        public override void DrawMainPropertyBlock()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                else
                {
                    ...
                    {
                        m_content.text =
                            m_referenceNode.TitleContent.text + Constants.InstancePostfixStr;
                    }
                }
                UpdateHeaderColor();
            }
...
#if UNITY_2021_1_OR_NEWER
            if (
                (ContainerGraph.IsLWRP || ContainerGraph.ParentWindow.IsShaderFunctionWindow)
            ...
            else if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                SetAdditonalTitleTextOnCallback(
                    ...
                    (instance, newSubTitle) =>
                        instance.AdditonalTitleContent.text = string.Format(
                            Constants.SubTitleVarNameFormatStr,
                            newSubTitle
                        )
                );
            ...
            if (dataCollector.IsSRP)
            {
                ...
                if (dataCollector.TemplateDataCollectorInstance.IsLWRP)
                {
                    ...
                    if (m_isURP2D)
                    {
                        ...
                        dataCollector.AddLocalVariable(
                            ...
                            valueName,
                            string.Format(URP2DFunctionHeader, uvCoords)
                        );
                    }
                    else
#endif
                ...
                else
                {
                    ...
                    dataCollector.AddFunction(
                        HDSampleSceneColorFunc5[0],
                        HDSampleSceneColorFunc5,
                        false
                    );
                    string exposureValue = m_exposure
                        ? "1.0"
                    ...
                    dataCollector.AddLocalVariable(
                        ...
                        WirePortDataType.FLOAT4,
                        valueName,
                        string.Format(HDSampleSceneColorHeader5, uvCoords, lod, exposureValue)
                    );
                }
            }
            else
        ...
        public string SetFetchedData(ref MasterNodeDataCollector dataCollector, bool ignoreLocalVar)
        {
            ...
            string samplerOp = string.Format(
                "UNITY_SAMPLE_SCREENSPACE_TEXTURE({0},{1})",
                propertyName,
                uvValue
            );
            dataCollector.AddLocalVariable(
        ...
        )
        {
            ...
            {
                result = m_inputPorts[0]
                    .GenerateShaderForOutput(
                        ref dataCollector,
                        (isProjecting ? WirePortDataType.FLOAT4 : WirePortDataType.FLOAT2),
                        ignoreLocalVar,
                        true
                    );
            }
            else
            {
                ...
                if (dataCollector.IsTemplate)
                    customScreenPos = dataCollector.TemplateDataCollectorInstance.GetScreenPosRaw(
                        CurrentPrecisionType
                    );

                if (isProjecting)
                    result = GeneratorUtils.GenerateGrabScreenPosition(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType,
                        !dataCollector.UsingCustomScreenPos,
                        customScreenPos
                    );
                else
                    result = GeneratorUtils.GenerateGrabScreenPositionNormalized(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType,
                        !dataCollector.UsingCustomScreenPos,
        ...
        public override void CheckIfAutoRegister(ref MasterNodeDataCollector dataCollector)
        {
            ...
            )
            {
                ...
                string propertyName = CurrentPropertyReference;
                bool emptyName =
                    string.IsNullOrEmpty(m_propertyInspectorName)
                    || propertyName == GrabTextureDefault;
                dataCollector.AddGrabPass(emptyName ? string.Empty : propertyName);
            }
        }
