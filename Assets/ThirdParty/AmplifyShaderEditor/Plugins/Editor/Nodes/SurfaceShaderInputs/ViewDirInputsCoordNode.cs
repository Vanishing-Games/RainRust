    ...
    public sealed class ViewDirInputsCoordNode : SurfaceShaderINParentNode
    {
        ...
        private void UpdateTitle()
        {
            m_additionalContent.text = string.Format(
                Constants.SubTitleSpaceFormatStr,
                m_viewDirSpace.ToString()
            );
            m_sizeIsDirty = true;
        }
        ...
        )
        {
            if (dataCollector.IsTemplate)
            {
                ...
                string varName = inst.GetViewDir(
                    useMasterNodeCategory: true,
                    customCategory: MasterNodePortCategory.Fragment,
                    normalizeType: m_safeNormalize ? NormalizeType.Safe : NormalizeType.Regular,
                    space: m_viewDirSpace
                );
            ...
            {
                string result = GeneratorUtils.GenerateViewDirection(
                    ref dataCollector,
                    UniqueId,
                    normalizeType: m_safeNormalize ? NormalizeType.Safe : NormalizeType.Regular,
                    space: m_viewDirSpace
            ...
            else
            {
                if (m_viewDirSpace == ViewSpace.Tangent)
                {
                    string result = base.GenerateShaderForOutput(
                        outputId,
                        ref dataCollector,
                        ignoreLocalVar
                    );
                ...
                else
                {
                    ...
                    string result = GeneratorUtils.GenerateViewDirection(
                        ref dataCollector,
                        UniqueId,
                        m_safeNormalize ? NormalizeType.Safe : NormalizeType.Regular,
                        space: m_viewDirSpace
                    );
                    return GetOutputVectorItem(0, outputId, result);
                }
