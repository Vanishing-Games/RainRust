...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ScreenDepthNode : ParentNode
    {
        ...
        private DepthMode m_depthMode
        {
            get { return (DepthMode)m_viewSpaceInt; }
        }
        ...
        private void UpdateAdditonalTitleText()
        {
            SetAdditonalTitleText(
                string.Format(
                    Constants.SubTitleModeFormatStr,
                    GeneratorUtils.DepthModeStr[m_viewSpaceInt]
                )
            );
        }
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            m_viewSpaceInt = m_upperLeftWidget.DrawWidget(
                this,
                m_viewSpaceInt,
                GeneratorUtils.DepthModeStr
            );
        ...
        )
        {
            ...
            )
            {
                if (dataCollector.IsTemplate && dataCollector.CurrentSRPType == TemplateSRPType.URP)
                {
                    dataCollector.AddToDirectives(
                        Constants.CameraDepthTextureLWEnabler,
                        -1,
            ...
            else
            {
                if (dataCollector.IsTemplate)
                {
                    ...
                    {
                        screenPosNorm = GeneratorUtils.GenerateScreenPositionNormalized(
                            ref dataCollector,
                            UniqueId,
            ...
            string screenDepthInstruction = TemplateHelperFunctions.CreateDepthFetch(
                dataCollector,
                screenPosNorm
            );
            ...
            {
                string viewSpace =
                    (m_depthMode == DepthMode.DepthLinearEye) ? "LinearEyeDepth" : "Linear01Depth";
                if (dataCollector.IsTemplate && dataCollector.IsSRP)
                {
            ...
            dataCollector.AddLocalVariable(
                ...
                GeneratorUtils.DepthModeVarNameStr[m_viewSpaceInt] + OutputId,
                screenDepthInstruction
            );
