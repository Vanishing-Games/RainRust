        ...
        )
        {
            ...
            if (isVertex)
            {
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    string tangent = GeneratorUtils.GenerateWorldTangent(
                        ref dataCollector,
                        UniqueId
                    );
                    ...
                    string inputTangent = m_inputPorts[0]
                        .GeneratePortInstructions(ref dataCollector);
                    dataCollector.AddToVertexLocalVariables(
                        UniqueId,
                        "float3 tangentNormal" + OutputId + " = " + inputTangent + ";"
                    );

                    string viewDir = GeneratorUtils.GenerateViewDirection(
                        ref dataCollector,
                        UniqueId
                    );
                    dataCollector.AddToVertexLocalVariables(
                        UniqueId,
                        "float3 modWorldNormal"
                            + OutputId
                            + " = ( tangentToWorld[0] * tangentNormal"
                            + OutputId
                            + ".x + tangentToWorld[1] * tangentNormal"
                            + OutputId
                            + ".y + tangentToWorld[2] * tangentNormal"
                            + OutputId
                            + ".z);"
                    );
                ...
                else
                {
                    ...
                    string worldNormal = GeneratorUtils.GenerateWorldNormal(
                        ref dataCollector,
                        UniqueId
                    );
                    string viewDir = GeneratorUtils.GenerateViewDirection(
            ...
            else
            {
                ...
                else
                {
                    ...
                    result = GeneratorUtils.GenerateWorldReflection(
                        ref dataCollector,
                        UniqueId,
                        m_normalize
                    );
