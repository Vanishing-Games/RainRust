        ...
        )
        {
            ...
            )
            {
                ...
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    if (connCount > 1)
                    {
                        dataCollector.AddToFragmentLocalVariables(
                            UniqueId,
                            string.Format(NormalVecDecStr, NormalVecValStr + OutputId, result)
                        );
                ...
                else
                {
                    ...
                    {
                        result = GeneratorUtils.GenerateWorldNormal(
                            ref dataCollector,
                            UniqueId,
                            m_normalize
                        );
            ...
            else
            {
                if (m_inputPorts[0].IsConnected)
                {
                    ...
                    dataCollector.AddToVertexLocalVariables(
                        UniqueId,
                        "float3 modWorldNormal" + OutputId + " = " + result + ";"
                    );
                    return GetOutputVectorItem(0, outputId, "modWorldNormal" + OutputId);
                }
                else
                {
                    string result = GeneratorUtils.GenerateWorldNormal(
                        ref dataCollector,
                        UniqueId,
                        m_normalize
                    );
