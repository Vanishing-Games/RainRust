...
namespace AmplifyShaderEditor
{
    ...
    [NodeAttributes(
        "Screen Position",
        "Camera And Screen",
        "Gives access to the screen coordinates of the mesh vertex or fragment, where the X and Y values represent the horizontal and vertical positions. Use the Mode dropdown to choose the desired output mode."
    )]
    public sealed class ScreenPosInputsNode : SurfaceShaderINParentNode
    {
        ...
        private readonly string[] m_outputTypeStr =
        {
            "Normalized",
            "Raw",
            "Center",
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            if (
                ...
                && UIUtils.CurrentWindow.CurrentGraph != null
        ...
        void ConfigureHeader()
        {
            SetAdditonalTitleText(
                string.Format(Constants.SubTitleModeFormatStr, m_outputTypeStr[m_outputTypeInt])
            );
        ...
        )
        {
            ...
            if (
                dataCollector.TesselationActive && dataCollector.IsFragmentCategory
            ...
            )
            {
                switch ((Mode)m_outputTypeInt)
                {
                    case Mode.Normalized:
                        screenPos = GeneratorUtils.GenerateScreenPositionNormalizedOnFrag(
                            ref dataCollector,
                            UniqueId,
                        ...
                        break;
                    case Mode.Raw:
                        screenPos = GeneratorUtils.GenerateScreenPositionRawOnFrag(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Center:
                        screenPos = GeneratorUtils.GenerateScreenPositionCenterOnFrag(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Tiled:
                        screenPos = GeneratorUtils.GenerateScreenPositionTiledOnFrag(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Pixel:
                        screenPos = GeneratorUtils.GenerateScreenPositionPixelOnFrag(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
            ...
            else
            {
                ...
                {
                    case Mode.Normalized:
                        screenPos = GeneratorUtils.GenerateScreenPositionNormalized(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Raw:
                        screenPos = GeneratorUtils.GenerateScreenPositionRaw(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Center:
                        screenPos = GeneratorUtils.GenerateScreenPositionCenter(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Tiled:
                        screenPos = GeneratorUtils.GenerateScreenPositionTiled(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
                        break;
                    case Mode.Pixel:
                        screenPos = GeneratorUtils.GenerateScreenPositionPixel(
                            ref dataCollector,
                            UniqueId,
                            CurrentPrecisionType
                        );
