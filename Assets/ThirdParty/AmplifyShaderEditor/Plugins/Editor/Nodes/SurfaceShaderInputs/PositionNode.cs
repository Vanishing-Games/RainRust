...
namespace AmplifyShaderEditor
{
    ...
    [NodeAttributes(
        "Position",
        "Surface Data",
        "Surface position. Absolute World-space, by default.\n\n"
    ...
    public sealed class PositionNode : SurfaceShaderINParentNode
    {
        ...
        private readonly string[] m_outputSpaceStr =
        {
            "Object",
            "World",
            "Relative World",
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            m_outputSpaceIndex = m_upperLeftWidget.DrawWidget(
                this,
                m_outputSpaceIndex,
                m_outputSpaceStr
            );
        ...
        public override void DrawProperties()
        {
            ...
            if (m_outputSpaceIndex == (int)Space.World)
            {
                EditorGUILayout.HelpBox(
                    "World refers to absolute World-Space coordinates, in full coordinates around the origin.",
                    MessageType.Info
                );
            }
            ...
            {
                EditorGUILayout.HelpBox(
                    "Relative World refers to Camera-relative World-space, where values are kept relative to the Camera position, making them less prone to the numerical precision degradation of large floating point values.",
                    MessageType.Info
                );
            }
        }
        ...
        void ConfigureHeader()
        {
            SetAdditonalTitleText(
                string.Format(Constants.SubTitleModeFormatStr, m_outputSpaceStr[m_outputSpaceIndex])
            );
        ...
        )
        {
            if (dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                ...
                return string.Format(
                    "{0}( 0, 0, 0 )",
                    UIUtils.PrecisionWirePortToCgType(CurrentPrecisionType, WirePortDataType.FLOAT3)
                );
            }

            if (dataCollector.IsTemplate)
            {
                string varName = dataCollector.TemplateDataCollectorInstance.GetPosition(
                    (Space)m_outputSpaceIndex
                );
                return GetOutputVectorItem(0, outputId, varName);
            }
            else
            {
                string worldPosition = GeneratorUtils.GeneratePosition(
                    ref dataCollector,
                    UniqueId,
                    (Space)m_outputSpaceIndex
                );
