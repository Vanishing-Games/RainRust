    ...
    public sealed class WorldNormalInputsNode : SurfaceShaderINParentNode
    {
        ...
        )
        {
            ...
            )
            {
                if (m_addInstruction)
                {
                    string precision = UIUtils.PrecisionWirePortToCgType(
                        CurrentPrecisionType,
                        WirePortDataType.FLOAT3
                    );
                    dataCollector.AddVertexInstruction(
                        precision
                            + " worldNormal = UnityObjectToWorldNormal("
            ...
            else
            {
                ...
                )
                {
                    ...
                    string result = string.Format(
                        Constants.WorldNormalLocalDecStr,
                        m_precisionString
                    );
                    int count = 0;
                ...
                {
                    return base.GenerateShaderForOutput(
                        outputId,
                        ref dataCollector,
                        ignoreLocalVar
                    );
