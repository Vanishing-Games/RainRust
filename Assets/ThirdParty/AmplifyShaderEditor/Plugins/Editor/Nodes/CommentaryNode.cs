...
namespace AmplifyShaderEditor
{
    ...
    public sealed class CommentaryNode : ParentNode, ISerializationCallbackReceiver
    {
        ...
        private Dictionary<int, ParentNode> m_nodesOnCommentaryDict =
            new Dictionary<int, ParentNode>();
        ...
        public void CreateFromSelectedNodes(
            Vector2 mousePosOnCanvasCoords,
        ...
        )
        {
            ...
            m_position = new Rect(
                minPos.x - BORDER_SIZE_X,
                minPos.y - BORDER_SIZE_Y,
                dims.x,
                dims.y
            );
        }
        ...
        public void NodeStoppedMoving(
            ParentNode node,
            bool testOnlySelected,
            InteractionMode useTargetInteraction
        )
        {
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            if (m_reRegisterNodes)
            {
                ...
                for (int i = 0; i < m_nodesOnCommentary.Count; i++)
                {
                    if (m_nodesOnCommentary[i] != null)
                    {
                        m_nodesOnCommentary[i].OnNodeStoppedMovingEvent += NodeStoppedMoving;
                        m_nodesOnCommentary[i].OnNodeDestroyedEvent += NodeDestroyed;
                        m_nodesOnCommentaryDict.Add(
                            m_nodesOnCommentary[i].UniqueId,
                            m_nodesOnCommentary[i]
                        );
                    }
                }
            }
            ...
            {
                m_resizeIconTex = UIUtils
                    .GetCustomStyle(CustomStyle.CommentaryResizeButton)
                    .normal.background;
            }
            ...
            m_resizeLeftIconCoords.y =
                m_globalPosition.y
                + m_globalPosition.height
                - 2
                - (m_resizeIconTex.height + ResizeButtonPos.y) * drawInfo.InvertedZoom;
            ...
            m_resizeRightIconCoords.x =
                m_globalPosition.x
                + m_globalPosition.width
                - 1
                - (m_resizeIconTex.width + ResizeButtonPos.x) * drawInfo.InvertedZoom;
            m_resizeRightIconCoords.y =
                m_globalPosition.y
                + m_globalPosition.height
                - 2
                - (m_resizeIconTex.height + ResizeButtonPos.y) * drawInfo.InvertedZoom;
        ...
        public override void OnNodeRepaint(DrawInfo drawInfo)
        {
            ...
            GUI.Label(
                m_globalPosition,
                string.Empty,
                UIUtils.GetCustomStyle(CustomStyle.CommentaryBackground)
            );
            ...
            // Buttons
            GUI.Label(
                m_resizeLeftIconCoords,
                string.Empty,
                UIUtils.GetCustomStyle(CustomStyle.CommentaryResizeButtonInv)
            );
            GUI.Label(
                m_resizeRightIconCoords,
                string.Empty,
                UIUtils.GetCustomStyle(CustomStyle.CommentaryResizeButton)
            );
            ...
            if (!string.IsNullOrEmpty(m_titleText))
            {
                ...
                GUI.Label(
                    titleRect,
                    m_titleText,
                    UIUtils.GetCustomStyle(CustomStyle.CommentarySuperTitle)
                );
            }
        }
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                if (
                    ...
                    && (
                        (
                            ...
                            && m_commentArea.Contains(drawInfo.MousePosition)
                        )
                    )
                )
                {
                ...
                if (m_isEditing || m_startEditing)
                {
                    ...
                    m_commentText = EditorGUITextField(
                        m_commentArea,
                        string.Empty,
                        m_commentText,
                        UIUtils.CommentaryTitle
                    );
                    if (EditorGUI.EndChangeCheck())
                    {
                        m_checkCommentText = true;
            ...
            if (drawInfo.CurrentEventType == EventType.MouseDown && drawInfo.LeftMouseButtonPressed)
            {
                // Left Button
                if (
                    m_resizeLeftIconCoords.Contains(drawInfo.MousePosition)
                    && ContainerGraph.ParentWindow.CurrentEvent.modifiers != EventModifiers.Shift
                )
                {
                    if (!m_isResizingLeft)
                    {
                ...
                // Right Button
                if (
                    m_resizeRightIconCoords.Contains(drawInfo.MousePosition)
                    && ContainerGraph.ParentWindow.CurrentEvent.modifiers != EventModifiers.Shift
                )
                {
            ...
            )
            {
                ...
                if (m_isResizingLeft)
                {
                    ...
                    {
                        Vector2 currSize = (
                            drawInfo.TransformedMousePos - m_resizeStartPoint
        ...
        public override bool OnNodeInteraction(ParentNode node)
        {
            ...
            for (int i = 0; i < m_nodesOnCommentary.Count; i++)
            {
                if (
                    m_nodesOnCommentary[i]
                    && m_nodesOnCommentary[i] != this
                    && m_nodesOnCommentary[i].OnNodeInteraction(node)
                )
                {
                    return false;
                }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                string[] colorChannels = GetCurrentParam(ref nodeParams)
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                m_frameColor.r.ToString()
                    + IOUtils.VECTOR_SEPARATOR
                    + m_frameColor.g.ToString()
                    + IOUtils.VECTOR_SEPARATOR
                    + m_frameColor.b.ToString()
                    + IOUtils.VECTOR_SEPARATOR
                    + m_frameColor.a.ToString()
            );
        }
        ...
        {
            get { return Event.current.alt ? m_position : m_auxHeaderPos; }
        }

        public override bool Contains(Vector3 pos)
        {
            return Event.current.alt
                ? m_globalPosition.Contains(pos)
                : (
                    m_headerPosition.Contains(pos)
                    || m_resizeRightIconCoords.Contains(pos)
                    || m_resizeLeftIconCoords.Contains(pos)
                );
        }
    }
}
