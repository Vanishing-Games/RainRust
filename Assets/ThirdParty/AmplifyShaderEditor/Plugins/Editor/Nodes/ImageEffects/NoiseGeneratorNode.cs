...
namespace AmplifyShaderEditor
{
    ...
    public sealed class NoiseGeneratorNode : ParentNode
    {
        ...
        // Simple
        private const string SimpleNoiseRandomValueFunc =
            "inline float noise_randomValue (float2 uv) { return frac(sin(dot(uv, float2(12.9898, 78.233)))*43758.5453); }";
        private const string SimpleNoiseInterpolateFunc =
            "inline float noise_interpolate (float a, float b, float t) { return (1.0-t)*a + (t*b); }";
        private const string SimpleValueNoiseHeader = "inline float valueNoise (float2 uv)";
        private readonly string[] SimpleValueNoiseBody =
        {
        ...
        private readonly string[] SimpleNoiseBody =
        {
        ...
        // Simplex 2D
        private const string Simplex2DFloat3Mod289Func =
            "float3 mod2D289( float3 x ) { return x - floor( x * ( 1.0 / 289.0 ) ) * 289.0; }";
        private const string Simplex2DFloat2Mod289Func =
            "float2 mod2D289( float2 x ) { return x - floor( x * ( 1.0 / 289.0 ) ) * 289.0; }";
        private const string Simplex2DPermuteFunc =
        ...
        private readonly string[] SimplexNoise2DBody =
        {
        ...
        private const string Simplex3DFloat3Mod289 =
            "float3 mod3D289( float3 x ) { return x - floor( x / 289.0 ) * 289.0; }";
        private const string Simplex3DFloat4Mod289 =
            "float4 mod3D289( float4 x ) { return x - floor( x / 289.0 ) * 289.0; }";
        private const string Simplex3DFloat4Permute =
            "float4 permute( float4 x ) { return mod3D289( ( x * 34.0 + 1.0 ) * x ); }";
        private const string TaylorInvSqrtFunc =
        ...
        )
        {
            ...
            switch (m_type)
            {
                case NoiseGeneratorType.Simplex2D:
                    {
                        ...
                        RegisterLocalVariable(
                            0,
                            string.Format(SimplexNoise2DFunc, size),
                            ref dataCollector,
                            ("simplePerlin2D" + OutputId)
                        );
                    }
                    break;
                case NoiseGeneratorType.Simplex3D:
                    {
                        ...
                        RegisterLocalVariable(
                            0,
                            string.Format(SimplexNoise3DFunc, size),
                            ref dataCollector,
                            ("simplePerlin3D" + OutputId)
                        );
                    }
                    break;
                ...
                case NoiseGeneratorType.Gradient:
                    {
                        string[] body = m_unityVersion
                            ? UnityGradientNoiseFunctionsBody
                            : GradientNoiseFunctionsBody;
                        string func = m_unityVersion ? UnityGradientNoiseFunc : GradientNoiseFunc;

                        dataCollector.AddFunction(body[0], body, false);
                        RegisterLocalVariable(
                            0,
                            string.Format(func, size, scale),
                            ref dataCollector,
                            ("gradientNoise" + OutputId)
                        );
                    }
                    break;
                ...
                case NoiseGeneratorType.Simple:
                    {
                        ...
                        RegisterLocalVariable(
                            0,
                            string.Format(SimpleNoiseFunc, size),
                            ref dataCollector,
                            ("simpleNoise" + OutputId)
                        );
                    }
                    break;
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_type = (NoiseGeneratorType)
                Enum.Parse(typeof(NoiseGeneratorType), GetCurrentParam(ref nodeParams));
