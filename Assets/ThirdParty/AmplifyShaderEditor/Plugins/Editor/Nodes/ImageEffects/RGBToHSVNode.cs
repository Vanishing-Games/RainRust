...
namespace AmplifyShaderEditor
{
    ...
    public sealed class RGBToHSVNode : ParentNode
    {
        ...
        public static readonly string[] RGBToHSVFunction =
        {
            "{0}3 RGBToHSV({0}3 c)\n",
            "{\n",
        ...
        public static readonly bool[] RGBToHSVFlags =
        {
            true,
            false,
            true,
        ...
        public static void AddRGBToHSVFunction(
            ...
            if (!dataCollector.HasFunction(RGBToHSVHeader))
            {
                ...
                for (int i = 0; i < RGBToHSVFunction.Length; i++)
                {
                    finalFunction +=
                        ...
                        + (
                            RGBToHSVFlags[i]
                                ? string.Format(RGBToHSVFunction[i], precisionString)
                                : RGBToHSVFunction[i]
                        );
                }
                UIUtils.ShaderIndentLevel--;
        ...
        )
        {
            ...
            RegisterLocalVariable(
                0,
                string.Format(RGBToHSVHeader, rgbValue),
                ref dataCollector,
                "hsvTorgb" + OutputId
            );
            return GetOutputVectorItem(
