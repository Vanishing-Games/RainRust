...
namespace AmplifyShaderEditor
{
    ...
    public sealed class VoronoiNode : ParentNode
    {
        ...
        private readonly string[] VoronoiHashBody =
        {
            "p = p - 2 * floor( p / 2 );",
        ...
        private const string VoronoiHeader =
            "float voronoi{0}( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )";
        ...
        private readonly string[] m_functionTypeStr =
        {
            "Cells",
            "Crystal",
            "Glass",
        ...
        private readonly string[] m_distanceFunctionStr =
        {
            "Euclidean\u00B2",
            "Euclidean",
            "Manhattan",
        ...
        {
            m_inputPorts[3].Visible =
                !m_useUnity && m_calculateSmoothValue && (m_functionType == 0);
            m_sizeIsDirty = true;
        }
        ...
        public override void RenderNodePreview()
        {
            ...
            if (!Preferences.User.DisablePreviews)
            {
                ...
                Graphics.Blit(
                    null,
                    m_outputPorts[0].OutputPreviewTexture,
                    PreviewMaterial,
                ...
                Graphics.Blit(
                    null,
                    m_outputPorts[1].OutputPreviewTexture,
                    PreviewMaterial,
                    m_previewMaterialPassId
                );
                ...
                Graphics.Blit(
                    null,
                    m_outputPorts[2].OutputPreviewTexture,
                    PreviewMaterial,
                    m_previewMaterialPassId
                );
        ...
        public override void DrawProperties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    m_functionType = EditorGUILayoutPopup(
                        FunctionTypeStr,
                        m_functionType,
                        m_functionTypeStr
                    );
                    ...
                    m_distanceFunction = EditorGUILayoutPopup(
                        DistanceFunctionLabelStr,
                        m_distanceFunction,
                        m_distanceFunctionStr
                    );
                    if (m_distanceFunction == 4)
                    {
                        m_minkowskiPower = EditorGUILayoutSlider(
                            "Minkowski Power",
                            m_minkowskiPower,
                            1,
                            5
                    ...
                    m_searchQuality = EditorGUILayoutPopup(
                        SearchQualityLabelStr,
                        m_searchQuality,
                        m_searchQualityStr
                    );
        ...
        void ChangeFunction(string scale)
        {
            ...
            switch (dFunction)
            {
                ...
                case 4:
                    VoronoiBody[11] =
                        "\t\tfloat d = "
                        + (1 / Mathf.Pow(2, 1 / m_minkowskiPower)).ToString("n3")
                        + " * pow( ( pow( abs( r.x ), "
                        + m_minkowskiPower
                        + " ) + pow( abs( r.y ), "
                        + m_minkowskiPower
                        + " ) ), "
                        + (1 / m_minkowskiPower).ToString("n3")
                        + " );";
                    break;
            }
            ...
            if (m_functionType == 0)
            {
                if (m_calculateSmoothValue)
                {
                    ...
                    VoronoiBody[16] = " //\t\t\tF2 = d;";
                    VoronoiBody[17] = m_applySmoothToIds
                        ? "\t\t\tfloat correctionFactor = smoothness * h * (1.0f - h) / ( 1.0f + 3.0f * smoothness ); smoothId = lerp( smoothId,id, h ) - correctionFactor;"
                        : string.Empty;
                    VoronoiBody[18] = " //\t\t}";
                }
                else
            ...
            {
                VoronoiHashBody[0] =
                    "p = p - " + m_tileScale + " * floor( p / " + m_tileScale + " );";
            }
            else
            {
                VoronoiHashBody[0] = "";
        ...
        )
        {
            ...
            if (m_useUnity)
            {
                ...
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("float2 uv{0} = 0;", OutputId)
                );
                dataCollector.AddFunction(
                    UnityVoroniNoiseFunctionsBody[0],
                    UnityVoroniNoiseFunctionsBody,
                    false
                );
                string varName = "unityVoronoy" + OutputId;
                string varValue = string.Format(
                    UnityVoronoiNoiseFunc,
                    uvValue,
                    angleOffset,
                    cellDensity,
                    "uv" + OutputId
            ...
            else
            {
                ...
                if (m_calculateSmoothValue)
                {
                    ...
                    string smoothnessValue = m_inputPorts[3]
                ...
                for (int i = 0; i < VoronoiBody.Length; i++)
                {
                    if (i == 9)
                    {
                        IOUtils.AddFunctionLine(
                            ref voronoiFunc,
                            string.Format(VoronoiBody[i], OutputId)
                        );
                    }
                    else
                    {
                ...
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("float2 coords{0} = {1} * {2};", OutputId, uvs, scaleValue)
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("float2 id{0} = 0;", OutputId)
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("float2 uv{0} = 0;", OutputId)
                ...
                else
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("float fade{0} = 0.5;", OutputId)
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("float voroi{0} = 0;", OutputId)
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("float rest{0} = 0;", OutputId)
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format(
                            "for( int it{0} = 0; it{0} <" + m_octaves + "; it{0}++ ){{",
                            OutputId
                        )
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format(
                            "voroi{0} += fade{0} * voronoi{0}( coords{0}, time{0}, id{0}, uv{0}, {1},{2} );",
                            OutputId,
                            smoothnessName,
                            smoothIdName
                        )
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("rest{0} += fade{0};", OutputId)
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("coords{0} *= 2;", OutputId)
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("fade{0} *= 0.5;", OutputId)
                    );
                    dataCollector.AddLocalVariable(UniqueId, "}" + "//Voronoi" + OutputId);
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("voroi{0} /= rest{0};", OutputId)
                    );
                }
                ...
                m_outputPorts[1]
                    .SetLocalValue(
                        (m_functionType == 0 && m_calculateSmoothValue && m_applySmoothToIds)
                            ? smoothIdName
                            : ("id" + OutputId),
