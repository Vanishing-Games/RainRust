    ...
    public sealed class SimplexNoiseNode : ParentNode
    {
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            IOUtils.AddFunctionHeader(
                ref m_simplex3dFractalFuncBody,
                "float3 Simplex3dFractal ( float3 m )"
            );
            IOUtils.AddFunctionLine(
                ref m_simplex3dFractalFuncBody,
                "return (0.5333333*Simplex3d ( m ) + 0.2666667*Simplex3d ( 2.0*m ) + 0.1333333*Simplex3d ( 4.0*m ) + 0.0666667*Simplex3d ( 8.0*m )).xxx;"
            );
        ...
        )
        {
            ...
            switch (m_type)
            {
                case NoiseType.Simplex3D:
                    {
                        string finalValue = dataCollector.AddFunctions(
                            Simplex3dfunctionHeader,
                            m_simplex3dFuncBody,
                            posValue + "*" + widthValue
                        );
                ...
                case NoiseType.Simplex3DFractal:
                    {
                        dataCollector.AddFunctions(
                            Simplex3dfunctionHeader,
                            m_simplex3dFuncBody,
                            posValue + "*" + widthValue
                        );
                        string finalValue = dataCollector.AddFunctions(
                            Simplex3dFractalfunctionHeader,
                            m_simplex3dFractalFuncBody,
                            posValue + "*" + widthValue + "+" + widthValue
                        );
                        result = finalValue + "* 0.5 + 0.5";
                    }
                    break;
