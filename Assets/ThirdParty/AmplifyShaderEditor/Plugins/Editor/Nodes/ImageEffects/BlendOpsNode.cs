...
namespace AmplifyShaderEditor
{
    ...
    public class BlendOpsNode : ParentNode
    {
        ...
        private const string ASEDarkerColorFunc =
            "inline float ASEDarkerColor{0}( float srcLocalVar, float dstLocalVar ){"
            + " return ({1} < {2}) ? s : d; }";

        private const string ASELighterColorCall = "ASELighterColor{}({0},{1})";
        private const string ASELighterColorFunc =
            "inline float ASELighterColor{0}( float srcLocalVar, float dstLocalVar ){"
            + " return ({1} > {2}) ? s : d; }";
        ...
        public override void SetPreviewInputs()
        {
            ...
            int lerpMode =
                (m_inputPorts[2].IsConnected || m_inputPorts[2].FloatInternalData < 1) ? 1 : 0;
        ...
        public override void DrawProperties()
        {
            ...
            m_currentBlendOp = (BlendOps)EditorGUILayoutEnumPopup(
                BlendOpsModeStr,
                m_currentBlendOp
            );
            if (EditorGUI.EndChangeCheck())
            {
                SetAdditonalTitleText(
                    string.Format(Constants.SubTitleTypeFormatStr, m_currentBlendOp)
                );
            }
        ...
        {
            x.SetAdditonalTitleText(
                string.Format(Constants.SubTitleTypeFormatStr, (x as BlendOpsNode).m_currentBlendOp)
            );
        };

        private string CreateMultiChannel(
        ...
        )
        {
            switch (m_outputPorts[0].DataType)
            {
                ...
                case WirePortDataType.FLOAT2:
                {
                    ...
                    string xChannelValue = string.Format(
                        function,
                        srcLocalVar + ".x",
                        dstLocalVar + ".x"
                    );
                    ...
                    string yChannelValue = string.Format(
                        function,
                        srcLocalVar + ".y",
                        dstLocalVar + ".y"
                    );
                ...
                case WirePortDataType.FLOAT3:
                {
                    ...
                    string zChannelValue = string.Format(
                        function,
                        srcLocalVar + ".z",
                        dstLocalVar + ".z"
                    );
                ...
                case WirePortDataType.COLOR:
                {
                    ...
                    string wChannelValue = string.Format(
                        function,
                        srcLocalVar + ".w",
                        dstLocalVar + ".w"
                    );
        ...
        )
        {
            ...
            string src = m_inputPorts[0]
                .GenerateShaderForOutput(ref dataCollector, m_mainDataType, false, true);
            string dst = m_inputPorts[1]
                .GenerateShaderForOutput(ref dataCollector, m_mainDataType, false, true);
            ...
            dataCollector.AddLocalVariable(
                UniqueId,
                UIUtils.PrecisionWirePortToCgType(CurrentPrecisionType, m_mainDataType)
                    + " "
                    + srcLocalVar,
                src + ";"
            );
            ...
            switch (m_currentBlendOp)
            {
                ...
                case BlendOps.VividLight:
                    {
                        result = string.Format(
                            "(( {0} > 0.5 ) ? ( {1} / max( ( 1.0 - {0} ) * 2.0 ,0.00001) ) : ( 1.0 - ( ( ( 1.0 - {1} ) * 0.5 ) / max( {0},0.00001) ) ) )",
                            srcLocalVar,
                            dstLocalVar
                        );
            ...
            if (m_inputPorts[2].IsConnected || m_inputPorts[2].FloatInternalData < 1.0)
            {
                ...
                string lerpResult = string.Format(
                    "lerp({0},{1},{2})",
                    dstLocalVar,
                    result,
                    opacity
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_currentBlendOp = (BlendOps)
                Enum.Parse(typeof(BlendOps), GetCurrentParam(ref nodeParams));
