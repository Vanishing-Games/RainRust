...
namespace AmplifyShaderEditor
{
    ...
    [NodeAttributes(
        ...
        tags: "If Ternary Compare Less Equal Not Greater"
    )]
    public sealed class Compare : ParentNode
    {
        private static readonly string[] LabelsSTR =
        {
            "Equal",
            "Not Equal",
            "Greater",
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            m_comparision = (Comparision)
                m_upperLeftWidget.DrawWidget(this, (int)m_comparision, LabelsSTR);
        ...
        public void UpdateConnection(int portId)
        {
            ...
            {
                otherPortId = (portId == 0) ? 1 : 0;
                otherPortType = m_inputPorts[otherPortId].IsConnected
                    ? m_inputPorts[otherPortId].DataType
                    : WirePortDataType.FLOAT;
                m_mainInputType =
                    UIUtils.GetPriority(m_inputPorts[portId].DataType)
                    > UIUtils.GetPriority(otherPortType)
        ...
        )
        {
            ...
            if (
                m_mainInputType != WirePortDataType.FLOAT
                && m_mainInputType != WirePortDataType.INT
                && m_mainInputType != m_mainOutputType
            )
            {
                compatibleInputType = m_mainOutputType;
            }

            string a = m_inputPorts[0]
            ...
            string T = m_inputPorts[2]
                .GenerateShaderForOutput(ref dataCollector, m_mainOutputType, ignoreLocalvar, true);
            string F = m_inputPorts[3]
                .GenerateShaderForOutput(ref dataCollector, m_mainOutputType, ignoreLocalvar, true);
            return CreateOutputLocalVariable(
                0,
                string.Format("( {0} {2} {1} ? {3} : {4} )", a, b, op, T, F),
                ref dataCollector
            );
        }
        ...
        public override void ReadFromDeprecated(ref string[] nodeParams, Type oldType = null)
        {
            ...
            else if (oldType == typeof(TFHCCompareNotEqual))
            {
                m_comparision = Comparision.NotEqual;
            }
            else if (oldType == typeof(TFHCCompareGreater))
            {
                m_comparision = Comparision.Greater;
            }
            else if (oldType == typeof(TFHCCompareGreaterEqual))
            {
                m_comparision = Comparision.GreaterOrEqual;
            }
            else if (oldType == typeof(TFHCCompareLower))
            {
                m_comparision = Comparision.Less;
            }
            else if (oldType == typeof(TFHCCompareLowerEqual))
            {
