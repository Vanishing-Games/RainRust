    ...
    public sealed class ConditionalIfNode : ParentNode
    {
        ...
        public override void DrawProperties()
        {
            ...
            if (!m_inputPorts[0].IsConnected)
                m_inputPorts[0].FloatInternalData = EditorGUILayoutFloatField(
                    m_inputPorts[0].Name,
                    m_inputPorts[0].FloatInternalData
                );
            if (!m_inputPorts[1].IsConnected)
                m_inputPorts[1].FloatInternalData = EditorGUILayoutFloatField(
                    m_inputPorts[1].Name,
        ...
        void TestMainOutputDataType()
        {
            ...
            for (int i = 2; i < 5; i++)
            {
                if (
                    ...
                    && (
                        UIUtils.GetPriority(m_inputPorts[i].DataType) > UIUtils.GetPriority(newType)
                    )
                )
                {
        ...
        )
        {
            ...
            m_results[0] = m_inputPorts[2]
                .GenerateShaderForOutput(
                    ref dataCollector,
                    m_outputMainDataType,
                    ignoreLocalvar,
                    true
                );
            m_results[1] = m_inputPorts[3]
                .GenerateShaderForOutput(
                    ref dataCollector,
                    m_outputMainDataType,
                    ignoreLocalvar,
            ...
            string localVarDec = string.Format(
                "{0} {1} = 0;",
            ...
            if (
                greater
                && m_inputPorts[2].GetOutputConnection() == m_inputPorts[3].GetOutputConnection()
            )
            {
                gequal = true;
            }
            ...
            if (
                lesser
                && m_inputPorts[4].GetOutputConnection() == m_inputPorts[3].GetOutputConnection()
            )
            {
                lequal = true;
            }
            ...
            if (lequal && gequal) // all equal
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("{0} = {1};", localVarName, m_results[1]),
                    true
                );
            }
            else if (!lequal && gequal) // greater or equal
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[3], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format("\t{0} = {1};", localVarName, m_results[0]),
                    true
                );
                ...
                if (welse)
                {
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("\t{0} = {1};", localVarName, m_results[2]),
                        true
                    );
                }
            }
            else if (lequal && !gequal) // lesser or equal
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[4], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
            ...
            else if (nequal) // not equal
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[5], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
            ...
            else if (equal) // equal
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[1], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
            ...
            else if (lesser && !midCon && !greater) // lesser
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[2], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
            ...
            else if (greater && !midCon && !lesser) // greater
            {
                dataCollector.AddLocalVariable(
                    UniqueId,
                    string.Format(IfOps[0], AValue, BValue),
                    true
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
            ...
            else // all different
            {
                ...
                if (lesser)
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "else " + string.Format(IfOps[2], AValue, BValue),
                        true
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
