...
namespace AmplifyShaderEditor
{
    public static class HDUtilsEx
    {
        ...
        public static System.Type Type
        {
            get
            {
                return (type == null)
                    ? type = System.Type.GetType(
                        "UnityEngine.Experimental.Rendering.HDPipeline.HDUtils, Unity.RenderPipelines.HighDefinition.Runtime"
                    )
                    : type;
            }
        }
    ...
    public static class DiffusionProfileSettingsEx
    {
        ...
        public static System.Type Type
        {
            get
            {
                return (type == null)
                    ? type = System.Type.GetType(
                        "UnityEngine.Experimental.Rendering.HDPipeline.DiffusionProfileSettings, Unity.RenderPipelines.HighDefinition.Runtime"
                    )
                    : type;
            }
        }
    ...
    public sealed class DiffusionProfileNode : PropertyNode
    {
        ...
        {
            m_defaultValue = EditorGUILayoutObjectField(
        ...
        public override void DrawMaterialProperties()
        {
            ...
            m_materialValue = EditorGUILayoutObjectField(
                Constants.MaterialValueLabel,
                m_materialValue,
                DiffusionProfileSettingsEx.Type,
                true
            ) /*as DiffusionProfileSettings*/
            ;
        ...
        public override string GetPropertyValue()
        {
            ...
            string assetVec =
                RoundTrip.ToRoundTrip(asset.x)
                + ", "
                + RoundTrip.ToRoundTrip(asset.y)
                + ", "
                + RoundTrip.ToRoundTrip(asset.z)
                + ", "
                + RoundTrip.ToRoundTrip(asset.w);
            ...
            if (m_defaultInspector)
            {
                lineOne =
                    PropertyAttributes
                    + "[HideInInspector]_DiffusionProfileAsset(\""
                    + m_propertyInspectorName
                    + "\", Vector) = ( "
                    + assetVec
                    + " )";
                lineTwo =
                    "\n[HideInInspector]_DiffusionProfileHash(\""
                    + m_propertyInspectorName
                    + "\", Float) = "
        ...
        public override void UpdateMaterial(Material mat)
        {
            ...
            if (UIUtils.IsProperty(m_currentParameterType) && !InsideShaderFunction)
            {
                ...
                {
                    Vector4 asset = HDUtilsEx.ConvertGUIDToVector4(
                    ...
                    if (m_defaultInspector)
                    {
                        ...
                        mat.SetFloat(
                            ...
                            HDShadowUtilsEx.Asfloat(MaterialHash)
                        );
                    }
                    else
                    {
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_defaultValue = AssetDatabase.LoadAssetAtPath(
                    AssetDatabase.GUIDToAssetPath(defaultGuid),
                    DiffusionProfileSettingsEx.Type
                );
            }
            if (UIUtils.CurrentShaderVersion() > 17004)
            {
                ...
                {
                    m_materialValue = AssetDatabase.LoadAssetAtPath(
        ...
        public override void ReadOutputDataFromString(ref string[] nodeParams)
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 17005)
                m_outputPorts[0]
                    .ChangeProperties(Constants.EmptyPortValue, WirePortDataType.FLOAT, false);
        }
        ...
        uint DefaultHash
        {
            get
            {
                return (m_defaultValue != null)
                    ? DiffusionProfileSettingsEx.Hash(m_defaultValue)
                    : 0;
            }
        }
        uint MaterialHash
        {
            get
            {
                return (m_materialValue != null)
                    ? DiffusionProfileSettingsEx.Hash(m_materialValue)
                    : 0;
            }
        }
