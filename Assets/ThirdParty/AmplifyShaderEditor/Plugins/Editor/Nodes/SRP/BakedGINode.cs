    ...
    public sealed class BakedGINode : ParentNode
    {
        ...
        public override void DrawProperties()
        {
            ...
            EditorGUILayout.HelpBox(
                "Screen Position input must be Normalized (xyz/w), which is the default setting for Screen Position node.",
                MessageType.Info
            );
        ...
        )
        {
            ...
            {
                UIUtils.ShowMessage(
                    "Node only intended to use on HDRP and URP rendering pipelines"
                );
            ...
            if (dataCollector.TemplateDataCollectorInstance.IsHDRP)
            {
                ...
                if (
                    ASEPackageManagerHelper.CurrentHDRPBaseline == ASESRPBaseline.ASE_SRP_14
                        && unityVersion >= 20220326
                    || ASEPackageManagerHelper.CurrentHDRPBaseline == ASESRPBaseline.ASE_SRP_16
                        && unityVersion >= 20230220
                    || ASEPackageManagerHelper.CurrentHDRPBaseline == ASESRPBaseline.ASE_SRP_17
                        && unityIsBeta
                        && betaVersion >= 15
                ...
                )
                {
                    dataCollector.AddToPragmas(
                        UniqueId,
                        "multi_compile_fragment _ PROBE_VOLUMES_L1 PROBE_VOLUMES_L2"
                    );
