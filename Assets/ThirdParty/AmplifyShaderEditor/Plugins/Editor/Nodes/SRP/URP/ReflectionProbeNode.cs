...
namespace AmplifyShaderEditor
{
    ...
    public class ReflectionProbeNode : ParentNode
    {
        ...
        public const string ErrorOnCompilationMsg =
            "Attempting to use URP specific node on incorrect SRP or Builtin RP.";
        ...
        )
        {
            ...
            if (dataCollector.IsSRP && dataCollector.CurrentSRPType == TemplateSRPType.URP)
            {
                if (ASEPackageManagerHelper.PackageSRPVersion >= (int)ASESRPBaseline.ASE_SRP_12)
                {
                    dataCollector.AddToPragmas(
                        UniqueId,
                        "multi_compile_fragment _ _REFLECTION_PROBE_BLENDING"
                    );
                    dataCollector.AddToPragmas(
                        UniqueId,
            ...
            {
                viewDirOS = dataCollector.TemplateDataCollectorInstance.GetViewDir(
                    CurrentPrecisionType,
                    space: ViewSpace.Object
                );
            }
            ...
            {
                normalOS = dataCollector.TemplateDataCollectorInstance.GetVertexNormal(
                    CurrentPrecisionType
                );
            }
            ...
            RegisterLocalVariable(
                outputId,
                string.Format(ReflectionProbeStr, viewDirOS, normalOS, lod),
                ref dataCollector
            );
            return m_outputPorts[0].LocalValue(dataCollector.PortCategory);
        }
