...
namespace AmplifyShaderEditor
{
    ...
    public class DynamicTypeNode : ParentNode
    {
        ...
        void UpdateDisconnectedConnection(int portId)
        {
            if (m_extensibleInputPorts || m_allowMatrixCheck)
            {
                ...
                if (groupOneType > 0 && m_mainDataType == WirePortDataType.FLOAT4x4)
                {
                    m_errorMessageTooltip =
                        "Doing this operation with FLOAT4x4 value only works against other FLOAT4x4 or FLOAT values";
                    m_showErrorMessage = true;
                }
                else if (groupTwoType > 0 && m_mainDataType == WirePortDataType.FLOAT3x3)
                {
                    m_errorMessageTooltip =
                        "Doing this operation with FLOAT3x3 value only works against other FLOAT3x3 or FLOAT values";
                    m_showErrorMessage = true;
                }
                else
                {
            ...
            else if (m_inputPorts[0].DataType != m_inputPorts[1].DataType)
            {
                ...
                {
                    if (
                        UIUtils.GetPriority(m_inputPorts[0].DataType)
                        > UIUtils.GetPriority(m_inputPorts[1].DataType)
                    )
                    {
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (
                m_previouslyDragging
                    != m_containerGraph.ParentWindow.WireReferenceUtils.OutputPortReference.IsValid
                && m_containerGraph.ParentWindow.WireReferenceUtils.OutputPortReference.NodeId
                    != UniqueId
            )
            {
                    ...
                    if (m_inputCount <= 10)
                    {
                        if (m_inputCount > m_lastInputCount)
                        {
                            UndoUtils.RegisterCompleteObjectUndo(
                                ...
                                Constants.UndoCreateDynamicPortId
                            );
                            ...
                            AddInputPort(
                                m_mainDataType,
                                false,
                                ((char)('A' + m_inputCount - 1)).ToString()
                            );
                            m_inputPorts[m_inputCount - 1]
                                .CreatePortRestrictions(m_dynamicRestrictions);
                        }
                ...
                m_previouslyDragging = m_containerGraph
                    .ParentWindow
                    .WireReferenceUtils
                    .OutputPortReference
                    .IsValid;
        ...
        private bool CheckValidConnections()
        {
            ...
            if (m_inputPorts.Count != m_beforePreviewCount)
            {
                if (hasMatrix)
                {
                    ...
                    if (showError)
                        m_containerGraph.ParentWindow.ShowMessage(
                            UniqueId,
                            "Matrix operations are only valid for the first two inputs to prevent errors"
                        );
                }
                else
                {
                    ...
                    {
                        if (
                            m_inputPorts[i].DataType == WirePortDataType.FLOAT3x3
                            || m_inputPorts[i].DataType == WirePortDataType.FLOAT4x4
                        )
                        {
                            m_containerGraph.ParentWindow.ShowMessage(
                                UniqueId,
        ...
        private void UpdateEmptyInputPorts(bool recordUndo)
        {
            if (!m_extensibleInputPorts)
                return;
            NodeWireReferencesUtils wireReferenceUtils = m_containerGraph
                .ParentWindow
                .WireReferenceUtils;
            if (!wireReferenceUtils.OutputPortReference.IsValid)
            {
                ...
                for (int i = m_inputPorts.Count - 1; i >= 2; i--)
                {
                    if (!m_inputPorts[i].IsConnected)
                    {
                        hasDeleted = true;
                        if (
                            wireReferenceUtils.InputPortReference.IsValid
                            && wireReferenceUtils.InputPortReference.NodeId == UniqueId
                            && wireReferenceUtils.InputPortReference.PortId
        ...
        public virtual string BuildResults(
            int outputId,
        ...
        )
        {
            ...
            {
                m_inputB = UIUtils.CastPortType(
                    ref dataCollector,
                    CurrentPrecisionType,
                    m_inputB,
                    m_inputPorts[1].DataType,
                    m_mainDataType,
                    m_inputB
                );
            }
        ...
        protected void SetExtensibleInputData(
            int outputId,
            ref MasterNodeDataCollector dataCollector,
            bool ignoreLocalvar
        )
