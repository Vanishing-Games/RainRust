...
namespace AmplifyShaderEditor
{
    ...
    public sealed class SamplerNode : TexturePropertyNode
    {
        ...
        public static readonly int[] AvailableAutoCast = { 0, 1, 2, 3, 4 };
        public static readonly string[] AvailableAutoCastStr =
        {
            "Auto",
            "Locked To Texture 1D",
            "Locked To Texture 2D",
            "Locked To Texture 3D",
            "Locked To Cube",
        };
        ...
        [SerializeField]
        private SamplerStateAutoGenerator m_samplerStateAutoGenerator =
            new SamplerStateAutoGenerator();
        ...
        public SamplerNode(int uniqueId, float x, float y, float width, float height)
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_errorMessageTooltip =
                "A texture object marked as normal map is connected to this sampler. Please consider turning on the Unpack Normal Map option";
        ...
        public override void SetPreviewInputs()
        {
            ...
            PreviewMaterial.SetInt(
                m_cachedLodId,
                (m_mipMode == MipType.MipLevel ? 1 : (m_mipMode == MipType.MipBias ? 2 : 0))
            );
        ...
        private void OnReorderEventComplete()
        {
            ...
            {
                m_referenceArrayId = ContainerGraph.SamplerNodes.GetNodeRegisterIdx(
                    m_referenceSampler.UniqueId
                );
            }
        }
        ...
        public override void DrawSubProperties()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                SetAdditonalTitleText(
                    string.Format(Constants.PropertyValueLabel, GetPropertyValStr())
                );
                ConfigureInputPorts();
        ...
        {
            m_defaultTextureValue = (TexturePropertyValues)EditorGUILayoutEnumPopup(
                DefaultTextureStr,
                m_defaultTextureValue
            );
            ...
            AutoCastType newAutoCast = (AutoCastType)EditorGUILayoutEnumPopup(
                AutoCastModeStr,
                m_autocastMode
            );
        ...
        public override void OnConnectedOutputNodeChanges(
            int portId,
            int otherNodeId,
            int otherPortId,
            string name,
        ...
        )
        {
            ...
            if (portId == m_texPort.PortId)
            {
                ...
                m_textureProperty =
                    m_texPort.GetOutputNodeWhichIsNotRelay(0) as TexturePropertyNode;
        ...
        )
        {
            ...
            if (portId == m_texPort.PortId)
            {
                ...
                {
                    if (
                        Constants.WireToTexture.TryGetValue(
                            m_texPort.ConnectionType(),
                            out m_currentType
                        )
                    )
                    {
        ...
        public override void ConfigureOutputPorts()
        {
            ...
            if (!AutoUnpackNormals)
            {
                ...
                m_outputPorts[m_colorPort.PortId + 1]
                    .ChangeProperties("R", WirePortDataType.FLOAT, false);
                m_outputPorts[m_colorPort.PortId + 2]
                    .ChangeProperties("G", WirePortDataType.FLOAT, false);
                m_outputPorts[m_colorPort.PortId + 3]
                    .ChangeProperties("B", WirePortDataType.FLOAT, false);
                m_outputPorts[m_colorPort.PortId + 4]
                    .ChangeProperties("A", WirePortDataType.FLOAT, false);
            }
            else
            {
                m_colorPort.ChangeProperties("XYZ", WirePortDataType.FLOAT3, false);
                m_outputPorts[m_colorPort.PortId + 1]
                    .ChangeProperties("X", WirePortDataType.FLOAT, false);
                m_outputPorts[m_colorPort.PortId + 2]
                    .ChangeProperties("Y", WirePortDataType.FLOAT, false);
                m_outputPorts[m_colorPort.PortId + 3]
                    .ChangeProperties("Z", WirePortDataType.FLOAT, false);
            }
        ...
        {
            m_headerColorModifier =
                (m_referenceType == TexReferenceType.Object) ? Color.white : ReferenceHeaderColor;
        }
        ...
        public void DrawSamplerOptions()
        {
            ...
            m_textureCoordSet = EditorGUILayoutIntPopup(
                Constants.AvailableUVSetsLabel,
                m_textureCoordSet,
                Constants.AvailableUVSetsStr,
                Constants.AvailableUVSets
            );
            ...
            if (!m_lodPort.IsConnected && m_lodPort.Visible)
            {
                m_lodPort.FloatInternalData = EditorGUILayoutFloatField(
                    newMipMode == MipType.MipBias ? "Mip Bias" : "Mip Level",
            ...
            if (
                ...
                && (newMipMode == MipType.Derivative || newMipMode == MipType.MipBias)
                && !UIUtils.CurrentWindow.OutsideGraph.IsSRP
            )
            {
            ...
            {
                m_normalPort.FloatInternalData = EditorGUILayoutFloatField(
                    NormalScaleStr,
                    m_normalPort.FloatInternalData
                );
            }
        ...
        {
            EditorGUI.BeginChangeCheck();
            m_referenceType = (TexReferenceType)EditorGUILayoutPopup(
                Constants.ReferenceTypeStr,
                (int)m_referenceType,
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (m_referenceType == TexReferenceType.Object)
                {
                    ...
                    m_textureProperty = m_texPort.IsConnected
                        ? m_texPort.GetOutputNodeWhichIsNotRelay(0) as TexturePropertyNode
                        : this;
                }
                else
            ...
            else
            {
                ...
                EditorGUI.BeginChangeCheck();
                m_referenceArrayId = EditorGUILayoutPopup(
                    Constants.AvailableReferenceStr,
                    m_referenceArrayId,
                    arr
                );
                if (EditorGUI.EndChangeCheck())
        ...
        public override void DrawGUIControls(DrawInfo drawInfo)
        {
            ...
            if (
                ...
                && m_previewRect.Contains(drawInfo.MousePosition)
                ...
                case ReferenceState.Connected:
                    {
                        ...
                        GUI.Label(
                            m_iconPos,
                            string.Empty,
                            UIUtils.GetCustomStyle(CustomStyle.SamplerTextureIcon)
                        );
                        ...
                        GUI.Label(
                            m_previewRect,
                            string.Empty,
                            UIUtils.GetCustomStyle(CustomStyle.SamplerFrame)
                        );
                    }
                    break;
                case ReferenceState.Instance:
                    {
                        ...
                        SetTitleTextOnCallback(
                            m_previewTextProp.PropertyInspectorName,
                            (instance, newTitle) =>
                                instance.TitleContent.text = newTitle + Constants.InstancePostfixStr
                        );
                        if (
                            m_previewTextProp.AdditonalTitleContent.text != m_additionalContent.text
                        )
                        {
                            PreviewIsDirty = true;
                        }
        ...
        private void DrawTexturePropertyPreview(DrawInfo drawInfo, bool instance)
        {
            ...
            SetTitleText(
                texProp.PropertyInspectorName
                    + (instance ? Constants.InstancePostfixStr : " (Input)")
            );
            ...
            if (m_referenceIconStyle == null || m_referenceIconStyle.normal == null)
            {
                ...
                if (
                    m_referenceIconStyle != null
                    && m_referenceIconStyle.normal != null
                    && m_referenceIconStyle.normal.background != null
                )
                {
        ...
        public override string GenerateSamplerPropertyName(
            int outputId,
        ...
        )
        {
            ...
            {
                generatedSamplerState = GeneratorUtils.GenerateSamplerState(
                    ref dataCollector,
                    UniqueId,
                    PropertyName,
                    m_variableMode
        ...
        )
        {
            if (dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    m_nodeAttribs.Name + " cannot be used on Master Node Tessellation port"
                );
                return "(-1)";
            }
            ...
            if (SoftValidReference)
            {
                ...
                {
                    portProperty = m_referenceSampler.TexPort.GeneratePortInstructions(
                        ref dataCollector
                    );
                }
                else
            ...
            string valueName = SetFetchedData(
                ref dataCollector,
                ignoreLocalVar,
                outputId,
                portProperty
        ...
        )
        {
            ...
            if (useMacros || m_currentType == TextureType.Texture2DArray)
            {
                ...
                {
                    TexturePropertyNode samplerNode = UIUtils.GetTexturePropertyNode(
                        m_useSamplerArrayIdx - 1
                    );
                    if (samplerNode != null)
                    {
                        if (samplerNode.IsConnected)
                        {
                            ...
                            samplerToUse = GeneratorUtils.GenerateSamplerState(
                                ref dataCollector,
                                UniqueId,
                                property,
                                varMode
                        ...
                        else
                        {
                            UIUtils.ShowMessage(
                                UniqueId,
                                string.Format(
                                    "{0} attempting to use sampler from unconnected {1} node. Reference Sampler nodes must be in use for their samplers to be created.",
                                    m_propertyName,
                                    samplerNode.PropertyName
                                ),
                            ...
                            dataCollector.AddToUniforms(
                                UniqueId,
                                string.Format(
                                    Constants.SamplerDeclarationSRPMacros[m_currentType],
                                    propertyName
                                )
                            );
                    ...
                    else
                    {
                        UIUtils.ShowMessage(
                            UniqueId,
                            m_propertyName + " attempting to use sampler from invalid node.",
                            MessageSeverity.Warning
                        );
                        dataCollector.AddToUniforms(
                            UniqueId,
                ...
                else
                {
                    ...
                    if (
                        m_samplerPort.IsConnected
                        && !string.IsNullOrEmpty(samplerState)
                        && !samplerState.Equals("0")
                    )
                    {
                ...
                if (outsideGraph.IsSRP)
                {
                    if (
                        ...
                        && (currMipMode == MipType.MipBias || currMipMode == MipType.Derivative)
                    )
                        GeneratorUtils.AddCustom3DSRPMacros(ref dataCollector);
                    samplerValue = string.Format(
                        Constants.TexSampleSRPMacros[m_currentType],
                        suffix,
                        propertyName,
                        samplerToUse,
                        uvCoords
                ...
                {
                    GeneratorUtils.AddCustomStandardSamplingMacros(
                        ref dataCollector,
                        m_currentType,
                        currMipMode
                    );
                    samplerValue = string.Format(
                        Constants.TexSampleSamplerStandardMacros[m_currentType],
                        suffix,
                        propertyName,
                        samplerToUse,
                        uvCoords
            ...
            else
            {
                ...
                samplerValue =
                    m_samplerType + mipType + "( " + propertyName + ", " + uvCoords + " )";
            }

            return samplerValue;
        }

        public string SetFetchedData(
            ref MasterNodeDataCollector dataCollector,
            bool ignoreLocalVar,
            int outputId,
        ...
        )
        {
            ...
            if (ignoreLocalVar)
            {
                ...
                string samplerValue = SampleTexture(
                    ref dataCollector,
                    ignoreLocalVar,
                    portProperty,
                    currMipMode,
            ...
            if (vtex != null)
            {
                ...
                if (string.IsNullOrEmpty(atPathname))
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        "Could not find Amplify Texture on your project folder. Please install it and re-compile the shader.",
                ...
                else
                {
                    ...
                    UnityEngine.Object obj = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(
                        atPathname
                    );
                    if (obj == null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            lodBias =
                                m_mipMode == MipType.MipLevel ? "Lod"
                                : m_mipMode == MipType.MipBias ? "Bias"
                                : "";
                        }
                        else
                        {
                        ...
                        int virtualCoordId = dataCollector.GetVirtualCoordinatesId(
                            UniqueId,
                            GetVirtualUVCoords(ref dataCollector, ignoreLocalVar, portProperty),
                            lodBias
                        );
                        string virtualSampler = SampleVirtualTexture(
                            vtex,
                            Constants.VirtualCoordNameStr + virtualCoordId
                        );
                        string virtualVariable = dataCollector.AddVirtualLocalVariable(
                            UniqueId,
                            "virtualNode" + OutputId,
                            virtualSampler
                        );
            ...
            string samplerOp = SampleTexture(
                ref dataCollector,
                ignoreLocalVar,
                portProperty,
                currMipMode,
                ...
                if (m_outputPorts[i].IsConnected)
                {
                    ...
                    if (connectedPorts > 1 || m_outputPorts[i].ConnectionCount > 1)
                    {
                        ...
                        if (
                            ...
                            dataCollector.AddToFragmentLocalVariables(
                                UniqueId,
                                m_precisionString
                                    + " "
                                    + textureFetchedValue
                        ...
                        m_outputPorts[m_colorPort.PortId + 1]
                            .SetLocalValue(textureFetchedValue + ".r", dataCollector.PortCategory);
                        m_outputPorts[m_colorPort.PortId + 2]
                            .SetLocalValue(textureFetchedValue + ".g", dataCollector.PortCategory);
                        m_outputPorts[m_colorPort.PortId + 3]
                            .SetLocalValue(textureFetchedValue + ".b", dataCollector.PortCategory);
                        m_outputPorts[m_colorPort.PortId + 4]
                            .SetLocalValue(textureFetchedValue + ".a", dataCollector.PortCategory);
        ...
        )
        {
            ...
            if (!m_uvPort.IsConnected)
            {
                ...
                string dummyPropUV =
                    "_tex" /*+ ( coordSize != 2 ? "" + coordSize : "" )*/
                    + "coord"
                    + (coordSet > 0 ? (coordSet + 1).ToString() : "");
                string dummyUV =
                    "uv" + (coordSet > 0 ? (coordSet + 1).ToString() : "") + dummyPropUV;
                ...
                if (scaleOffset)
                {
                    ...
                    texCoordsST = m_texCoordsHelper.GenerateShaderForOutput(
                        0,
                        ref dataCollector,
                        false
                    );
                ...
                {
                    coordInput = GeneratorUtils.GenerateAutoUVs(
                        ref dataCollector,
                        UniqueId,
                        coordSet,
                        null,
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        dataCollector.AddToInput(
                            UniqueId,
                            dummyUV,
                            dataCollector.GetMaxTextureChannelSize(coordSet)
                        );
                    }
                }
                ...
                if (dataCollector.MasterNodeCategory == AvailableShaderTypes.Template)
                {
                    ...
                    if (
                        dataCollector.TemplateDataCollectorInstance.GetCustomInterpolatedData(
                            TemplateHelperFunctions.IntToUVChannelInfo[m_textureCoordSet],
                            m_uvPort.DataType,
                            PrecisionType.Float,
                            ref result,
                            false,
                    ...
                    else if (dataCollector.TemplateDataCollectorInstance.HasUV(m_textureCoordSet))
                        coordInput = dataCollector.TemplateDataCollectorInstance.GetUVName(
                            m_textureCoordSet,
                            m_uvPort.DataType
                        );
                    else
                        coordInput = dataCollector.TemplateDataCollectorInstance.RegisterUV(
                            m_textureCoordSet,
                            m_uvPort.DataType
                        );
                }
                ...
                if (coordSize > 2)
                {
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "float" + coordSize + " " + uvName + " = " + coordInput + ";"
                    );
                    if (scaleOffset)
                    {
                        string scaleOffsetValue = GeneratorUtils.GenerateScaleOffsettedUV(
                            m_currentType,
                            coordInput + ".xy",
                            texCoordsST,
                            false
            ...
            if (m_currentType == TextureType.Texture2DArray)
            {
                ...
                if (
                    !(
                        (outsideGraph.SamplingMacros || m_currentType == TextureType.Texture2DArray)
                        && outsideGraph.IsSRP
                    )
                )
                {
            ...
            if (isVertex)
            {
                ...
                if (
                    ...
                    return UIUtils.PrecisionWirePortToCgType(
                            PrecisionType.Float,
                            WirePortDataType.FLOAT4
                        )
                        + "( "
            ...
            {
                if (
                    (m_mipMode == MipType.MipLevel || m_mipMode == MipType.MipBias) /*&& m_lodPort.IsConnected*/
                )
                {
                    string lodLevel = m_lodPort.GeneratePortInstructions(ref dataCollector);
                    if (
        ...
        public string GetVirtualUVCoords(
            ref MasterNodeDataCollector dataCollector,
            bool ignoreLocalVar,
            string portProperty
        )
        {
            string bias = "";
            if (
                !dataCollector.IsFragmentCategory
                || m_mipMode == MipType.MipBias
                || m_mipMode == MipType.MipLevel
            )
            {
                string lodLevel = m_lodPort.GeneratePortInstructions(ref dataCollector);
                bias += ", " + lodLevel;
            ...
            else
            {
                ...
                if (dataCollector.IsTemplate)
                {
                    ...
                    {
                        uvName = dataCollector.TemplateDataCollectorInstance.GetUVName(
                            m_textureCoordSet,
                            m_uvPort.DataType
                        );
                    }
                    ...
                    {
                        uvName = dataCollector.TemplateDataCollectorInstance.RegisterUV(
                            m_textureCoordSet,
                            m_uvPort.DataType
                        );
                    }
                    ...
                    else
                    {
                        ...
                        dataCollector.AddLocalVariable(
                            ...
                            WirePortDataType.FLOAT2,
                            uvChannelName,
                            GeneratorUtils.GenerateScaleOffsettedUV(
                                m_currentType,
                                uvName,
                                propertyName,
                                true
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        string dummyPropUV =
                            "_texcoord"
                            + (m_textureCoordSet > 0 ? (m_textureCoordSet + 1).ToString() : "");
                        string dummyUV =
                            "uv"
                            + (m_textureCoordSet > 0 ? (m_textureCoordSet + 1).ToString() : "")
                            + dummyPropUV;
                        ...
                        else
                        {
                            ...
                            string offsettedUV = GeneratorUtils.GenerateScaleOffsettedUV(
                                m_currentType,
                                dummyUV,
                                propertyName,
                                true
                            ...
                            dataCollector.AddToLocalVariables(
                                ...
                                WirePortDataType.FLOAT2,
                                uvChannelName,
                                Constants.InputVarStr + "." + offsettedUV
                            );
                        }
                        uvCoord = uvChannelName;
                    }
        ...
        private void AddNormalMapTag(ref MasterNodeDataCollector dataCollector, ref string value)
        {
            if (m_autoUnpackNormals)
            {
                ...
                string scaleValue = isScaledNormal
                    ? m_normalPort.GeneratePortInstructions(ref dataCollector)
                    : "1.0f";
                value = GeneratorUtils.GenerateUnpackNormalStr(
                    ref dataCollector,
                    CurrentPrecisionType,
                    UniqueId,
                    OutputId,
        ...
        public override void ReadFromDeprecated(ref string[] nodeParams, Type oldType = null)
        {
            ...
            if (oldType == typeof(TextureArrayNode))
            {
                ...
                m_referenceType = (TexReferenceType)
                    Enum.Parse(typeof(TexReferenceType), GetCurrentParam(ref nodeParams));
                ...
                if (UIUtils.CurrentShaderVersion() > 3202)
                    m_mipMode = (MipType)
                        Enum.Parse(typeof(MipType), GetCurrentParam(ref nodeParams));
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_defaultValue = AssetDatabase.LoadAssetAtPath<Texture>(
                    AssetDatabase.GUIDToAssetPath(defaultTextureGUID)
                );
                string materialTextureGUID = GetCurrentParam(ref nodeParams);
                m_materialValue = AssetDatabase.LoadAssetAtPath<Texture>(
                    AssetDatabase.GUIDToAssetPath(materialTextureGUID)
                );
            }
            else
            {
            ...
            m_defaultTextureValue = (TexturePropertyValues)
                Enum.Parse(typeof(TexturePropertyValues), GetCurrentParam(ref nodeParams));
            m_autocastMode = (AutoCastType)
                Enum.Parse(typeof(AutoCastType), GetCurrentParam(ref nodeParams));
            m_autoUnpackNormals = Convert.ToBoolean(GetCurrentParam(ref nodeParams));
            ...
            if (UIUtils.CurrentShaderVersion() > 3201)
                m_currentType = (TextureType)
                    Enum.Parse(typeof(TextureType), GetCurrentParam(ref nodeParams));
        ...
        public override void RefreshExternalReferences()
        {
            ...
            {
                m_useSamplerArrayIdx =
                    UIUtils.GetTexturePropertyNodeRegisterId(m_useSamplerArrayIdx) + 1;
            }
            else
            {
                m_useSamplerArrayIdx = 0;
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                ((m_referenceSampler != null) ? m_referenceSampler.UniqueId : -1)
            );
            IOUtils.AddFieldValueToString(ref nodeInfo, m_mipMode);
            IOUtils.AddFieldValueToString(ref nodeInfo, m_currentType);
        }

        public override void WriteAdditionalToString(
            ref string nodeInfo,
            ref string connectionsInfo
        ) { }
        ...
        public override string GetPropertyValStr()
        {
            return m_materialMode
                ? (m_materialValue != null ? m_materialValue.name : IOUtils.NO_TEXTURES)
                : (m_defaultValue != null ? m_defaultValue.name : IOUtils.NO_TEXTURES);
        }
        ...
        public override string GetPropertyValue()
        {
            if (SoftValidReference)
            {
                ...
                m_texPort.IsConnected
                && (m_texPort.GetOutputNodeWhichIsNotRelay(0) as TexturePropertyNode) != null
            )
            {
        ...
        public override bool GetUniformData(
            out string dataType,
            out string dataName,
            ref bool fullValue
        )
        {
            if (SoftValidReference)
            {
                if (m_referenceSampler.TexPort.IsConnected)
                {
                    ...
                    return m_referenceSampler.TextureProperty.GetUniformData(
                        out dataType,
                        out dataName,
                        ref fullValue
                    );
        ...
        public string UVCoordsName
        {
            get
            {
                return Constants.InputVarStr
                    + "."
                    + IOUtils.GetUVChannelName(CurrentPropertyReference, m_textureCoordSet);
            }
        }
        ...
        public override string CurrentPropertyReference
        {
            get
            {
                ...
                if (m_referenceType == TexReferenceType.Instance && m_referenceArrayId > -1)
                {
                    SamplerNode node = ContainerGraph.SamplerNodes.GetNode(m_referenceArrayId);
                    propertyName =
                        (node != null) ? node.TextureProperty.PropertyName : PropertyName;
                }
                else if (
                    m_texPort.IsConnected
                    && (m_texPort.GetOutputNodeWhichIsNotRelay(0) as TexturePropertyNode) != null
        ...
        public bool AutoUnpackNormals
        {
            ...
            set
            {
                if (value != m_autoUnpackNormals)
                {
                    ...
                    {
                        m_defaultTextureValue = value
                            ? TexturePropertyValues.bump
                            : TexturePropertyValues.white;
                    }
                }
            }
        }

        public override void PropagateNodeData(
            NodeData nodeData,
            ref MasterNodeDataCollector dataCollector
        )
        {
            ...
            {
                if (!m_texPort.IsConnected)
                    dataCollector.TemplateDataCollectorInstance.SetUVUsage(
                        m_textureCoordSet,
                        m_uvPort.DataType
                    );
            }
            else
            {
                ...
                {
                    dataCollector.AddCustomAppData(
                        string.Format(TemplateHelperFunctions.TexUVFullSemantic, m_textureCoordSet)
                    );
                }
            }
        }
        ...
        public bool IsObject
        {
            get
            {
                return (m_referenceType == TexReferenceType.Object) || (m_referenceSampler == null);
            }
        }
    }
}
