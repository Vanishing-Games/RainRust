...
namespace AmplifyShaderEditor
{
    ...
    {
        private const string DummyPropertyDec =
            "[HideInInspector] _DummyTex{0}( \"\", 2D ) = \"white\"";
        private const string DummyUniformDec = "uniform sampler2D _DummyTex{0};";
        private const string DummyTexCoordDef = "uv{0}_DummyTex{0}";
        private const string DummyTexCoordSurfDef =
            "float2 texCoordDummy{0} = {1}.uv{2}_DummyTex{2}*{3} + {4};";
        ...
        )
        {
            ...
            if (portId == 2)
            {
                ...
                m_inputReferenceNode =
                    m_texPort.GetOutputNodeWhichIsNotRelay() as TexturePropertyNode;
                UpdatePorts();
            }
            UpdateTitle();
        }
        ...
        void UpdateTitle()
        {
            ...
            {
                m_additionalContent.text = string.Format(
                    "Value( {0} )",
            ...
            else if (m_referenceArrayId > -1 && m_referenceNode != null)
            {
                m_additionalContent.text = string.Format(
                    "Value( {0} )",
                    m_referenceNode.PropertyInspectorName
                );
            }
            else
            {
        ...
        public override void DrawProperties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            m_texcoordSize = EditorGUILayoutIntPopup(
                Constants.AvailableUVSizesLabel,
                m_texcoordSize,
                Constants.AvailableUVSizesStr,
                Constants.AvailableUVSizes
            );
            if (EditorGUI.EndChangeCheck())
            {
                UpdateOutput();
            ...
            m_textureCoordChannel = EditorGUILayoutIntPopup(
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            {
                EditorGUI.BeginChangeCheck();
                m_texcoordSize = EditorGUIIntPopup(
                    m_dropdownRect,
                    m_texcoordSize,
                    Constants.AvailableUVSizesStr,
                    Constants.AvailableUVSizes,
            ...
            if (UIUtils.CurrentShaderVersion() > 2402)
            {
                if (UIUtils.CurrentShaderVersion() > 2404)
                {
                    ...
                    if (m_referenceNodeId > -1)
                        m_referenceArrayId = UIUtils.GetTexturePropertyNodeRegisterId(
                            m_referenceNodeId
                        );
                }
                else
        ...
        )
        {
            ...
            {
                dataCollector.SetTextureChannelSize(
                    m_textureCoordChannel,
                ...
                if (m_textureCoordChannel > 3)
                {
                    dataCollector.AddCustomAppData(
                        string.Format(
                            TemplateHelperFunctions.TexUVFullSemantic,
                            m_textureCoordChannel
                        )
                    );
                }
            ...
            MasterNodePortCategory propagateCategory =
                (
                    nodeData.Category != MasterNodePortCategory.Vertex
                    && nodeData.Category != MasterNodePortCategory.Tessellation
                )
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                ((m_referenceNode != null) ? m_referenceNode.UniqueId : -1)
            );
        ...
        )
        {
            ...
            else if (m_referenceArrayId > -1)
            {
                ...
                if (temp != null)
                {
                    portProperty = temp.BaseGenerateShaderForOutput(
                        outputId,
                        ref dataCollector,
                        ignoreLocalVar
                    );
            ...
            if (dataCollector.MasterNodeCategory == AvailableShaderTypes.Template)
            {
                ...
                string indexStr =
                    m_textureCoordChannel > 0 ? (m_textureCoordChannel + 1).ToString() : "";
                ...
                if (
                    dataCollector.TemplateDataCollectorInstance.GetCustomInterpolatedData(
                        TemplateHelperFunctions.IntToUVChannelInfo[m_textureCoordChannel],
                        m_outputPorts[0].DataType,
                        PrecisionType.Float,
                        ref result,
                        false,
                ...
                {
                    string finalTexCoordName =
                        "uv"
                        + indexStr
                        + (m_texcoordSize > 2 ? "s" + sizeDif : "")
                        + currPropertyName;
                    ...
                    dummyPropertyTexcoords = m_texCoordsHelper.GenerateShaderForOutput(
                        0,
                        ref dataCollector,
                        false
                    );
                    ...
                    if (m_texcoordSize > 2)
                    {
                        ...
                        dataCollector.AddLocalVariable(
                            UniqueId,
                            finalTexCoordName + ".xy",
                            string.Format(
                                Constants.TilingOffsetFormat,
                                uvName + ".xy",
                                dummyPropertyTexcoords + ".xy",
                                dummyPropertyTexcoords + ".zw"
                            ) + ";"
                        );
                        m_outputPorts[0]
                            .SetLocalValue(finalTexCoordName, dataCollector.PortCategory);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        RegisterLocalVariable(
                            0,
                            string.Format(Constants.TilingOffsetFormat, uvName, tiling, offset),
                            ref dataCollector,
                            finalTexCoordName
                        );
                    }
            ...
            if (
                !string.IsNullOrEmpty(propertyName) /*m_referenceArrayId > -1*/
            )
            {
                m_surfaceTexcoordName = GeneratorUtils.GenerateAutoUVs(
                    ref dataCollector,
                    UniqueId,
                    m_textureCoordChannel,
                    propertyName,
        ...
        public override void ReadInputDataFromString(ref string[] nodeParams)
        {
            ...
            else
            {
                for (
                    int i = 0;
                    i < 2 && i < nodeParams.Length && m_currentReadParamIdx < nodeParams.Length;
                    i++
                )
                {
