...
namespace AmplifyShaderEditor
{
    ...
    public class TexturePropertyNode : PropertyNode
    {
        ...
        protected readonly string[] AvailablePropertyTypeLabels =
        {
            PropertyType.Property.ToString(),
            PropertyType.Global.ToString(),
        };
        protected readonly int[] AvailablePropertyTypeValues =
        {
            (int)PropertyType.Property,
            (int)PropertyType.Global,
        };
        ...
        public TexturePropertyNode()
            : base() { }

        public TexturePropertyNode(int uniqueId, float x, float y, float width, float height)
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_availableAttribs.Add(
                new PropertyAttributes("Single Line Texture", "[SingleLineTexture]")
            );
            m_showPreview = true;
        ...
        {
            PropertyType parameterType = (PropertyType)EditorGUILayoutIntPopup(
                ParameterTypeStr,
                (int)m_currentParameterType,
                AvailablePropertyTypeLabels,
                AvailablePropertyTypeValues
            );
        ...
        public string GetTexture2DUniformValue()
        {
            if (PropertyName == "_CameraDepthTexture")
                ...
                return GeneratorUtils.GetPropertyDeclaraction(
                    PropertyName,
        ...
        protected void ConfigFromObject(
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            {
                PropertyType parameterType = (PropertyType)EditorGUIIntPopup(
                    m_dropdownRect,
                    (int)m_currentParameterType,
                    AvailablePropertyTypeLabels,
                    AvailablePropertyTypeValues,
            ...
            if (m_isEditingPicker && m_drawPicker && m_currentParameterType != PropertyType.Global)
            {
                ...
                Type currType =
                    (m_autocastMode == AutoCastType.Auto) ? typeof(Texture) : m_textureType;
                if (m_materialMode)
                {
                    m_materialValue =
                        EditorGUIObjectField(m_previewRect, m_materialValue, currType, false)
                        as Texture;
                }
                else
                {
                    m_defaultValue =
                        EditorGUIObjectField(m_previewRect, m_defaultValue, currType, false)
                        as Texture;
                }
        ...
        public override void CheckIfAutoRegister(ref MasterNodeDataCollector dataCollector)
        {
            ...
            )
            {
                ...
                {
                    GeneratorUtils.GenerateSamplerState(
                        ref dataCollector,
                        UniqueId,
                        PropertyName,
                        m_variableMode
        ...
        {
            return GeneratorUtils.GenerateSamplerState(
                ref dataCollector,
                UniqueId,
                PropertyName,
                m_variableMode
        ...
        public virtual string GenerateSamplerPropertyName(
            int outputId,
        ...
        public string BaseGenerateShaderForOutput(
            int outputId,
            ref MasterNodeDataCollector dataCollector,
            bool ignoreLocalVar
        )
        ...
        public override bool UpdateShaderDefaults(
            ref Shader shader,
            ref TextureDefaultsDataColector defaultCol /* ref string metaStr */
        )
        {
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            if (m_useSamplerArrayIdx > 0)
            {
                ...
                IOUtils.AddFieldValueToString(
                    ref nodeInfo,
                    (samplerNode != null ? samplerNode.UniqueId : -1)
                );
            }
            else
            {
        ...
        public override void WriteAdditionalClipboardData(ref string nodeInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_materialValue != null)
                    ? AssetDatabase.GetAssetPath(m_materialValue)
                    : Constants.NoStringValue
            );
        }
        ...
        )
        {
            ...
            if (outsideGraph.SamplingMacros || m_currentType == TextureType.Texture2DArray)
            {
                if (outsideGraph.IsSRP)
                {
                    ...
                    {
                        dataName = GeneratorUtils.GetPropertyDeclaraction(
                            PropertyName,
                            m_currentType,
                            ";"
                        );
            ...
            if (PropertyName == "_CameraDepthTexture")
            {
                ...
                {
                    UIUtils.ShowMessage(
                        "Use Sampling Macros flag on master node properties is required to be turned on in order to properly declare _CameraDepthTexture over an SRP shader!",
        ...
        public bool IsValid
        {
            get { return m_materialMode ? (m_materialValue != null) : (m_defaultValue != null); }
        }

        public virtual bool IsNormalMap
        {
            get { return m_isNormalMap; }
        }
        ...
        {
            Shader.SetGlobalTexture(m_propertyName, m_defaultValue);
        }

        public override void FetchGlobalValue()
        {
            m_materialValue = Shader.GetGlobalTexture(m_propertyName);
        }
        ...
        public bool ForceSamplingMacrosGen
        {
            set { m_forceSamplingMacrosGen = value; }
        }
    }
}
