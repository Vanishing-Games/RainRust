...
namespace AmplifyShaderEditor
{
    ...
    public sealed class InverseOpNode : SingleInputOp
    {
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_inputPorts[0]
                .CreatePortRestrictions(WirePortDataType.FLOAT3x3, WirePortDataType.FLOAT4x4);
        ...
        )
        {
            ...
            if (m_outputPorts[0].DataType == WirePortDataType.FLOAT3x3)
            {
                ...
                RegisterLocalVariable(
                    0,
                    string.Format(GeneratorUtils.Inverse3x3Header, value),
                    ref dataCollector,
                    "invertVal" + OutputId
                );
            }
            else
            {
                if (!dataCollector.HasFunction(Inverse4x4Header))
                {
                    ...
                    for (int i = 0; i < Inverse4x4Function.Length; i++)
                    {
                        finalFunction +=
                            ...
                            + (
                                Inverse4x4FunctionFlags[i]
                                    ? string.Format(Inverse4x4Function[i], precisionString)
                                    : Inverse4x4Function[i]
                            );
                    }
                ...
                RegisterLocalVariable(
                    0,
                    string.Format(Inverse4x4Header, value),
                    ref dataCollector,
                    "invertVal" + OutputId
                );
            }
