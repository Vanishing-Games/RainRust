...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ReciprocalOpNode : ParentNode
    {
        ...
        )
        {
            ...
            var localVarType = UIUtils.PrecisionWirePortToCgType(
            ...
            dataCollector.AddLocalVariable(
                UniqueId,
                string.Format("{0} {1} = rcp( {2} );", localVarType, localVarName, inputValue)
            );
            dataCollector.AddLocalVariable(UniqueId, "#else", true);
            dataCollector.AddLocalVariable(
                UniqueId,
                string.Format("{0} {1} = 1.0 / {2};", localVarType, localVarName, inputValue)
            );
