...
namespace AmplifyShaderEditor
{
    ...
    public sealed class BillboardNode : ParentNode
    {
        private const string ErrorMessage =
            "Billboard node should only be connected to vertex ports.";
        ...
        private readonly Action<ParentNode> OnWidgetUpdate = (x) =>
        {
            x.SetAdditonalTitleText(
                string.Format(Constants.SubTitleTypeFormatStr, (x as BillboardNode).Type)
            );
        };
        ...
        public override void DrawProperties()
        {
            ...
            NodeUtils.DrawPropertyGroup(
                ...
                () =>
                {
                    ...
                    m_billboardType = (BillboardType)EditorGUILayoutEnumPopup(
                        BillboardOpHelper.BillboardTypeStr,
                        m_billboardType
                    );
                    if (EditorGUI.EndChangeCheck())
                    {
                        SetAdditonalTitleText(
                            string.Format(Constants.SubTitleTypeFormatStr, m_billboardType)
                        );
                    }
                    m_rotationIndependent = EditorGUILayoutToggle(
                        BillboardOpHelper.BillboardRotIndStr,
                        m_rotationIndependent
                    );
                    m_affectNormalTangent = EditorGUILayoutToggle(
                        BillboardOpHelper.BillboardAffectNormalTangentStr,
        ...
        )
        {
            ...
            string vertexPosValue = dataCollector.IsTemplate
                ? dataCollector.TemplateDataCollectorInstance.GetVertexPosition(
                    WirePortDataType.OBJECT,
                    CurrentPrecisionType
                )
                : "v.vertex";
            string vertexNormalValue = dataCollector.IsTemplate
                ? dataCollector.TemplateDataCollectorInstance.GetVertexNormal(CurrentPrecisionType)
                : "v.normal";
            string vertexTangentValue = dataCollector.IsTemplate
                ? dataCollector.TemplateDataCollectorInstance.GetVertexTangent(
            ...
            {
                InterpDataHelper info = dataCollector.TemplateDataCollectorInstance.GetInfo(
            ...
            BillboardOpHelper.FillDataCollector(
                ref dataCollector,
                m_billboardType,
                m_rotationIndependent,
                vertexPosValue,
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_billboardType = (BillboardType)
                Enum.Parse(typeof(BillboardType), GetCurrentParam(ref nodeParams));
            SetAdditonalTitleText(string.Format(Constants.SubTitleTypeFormatStr, m_billboardType));
