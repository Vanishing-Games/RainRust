...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TexCoordVertexDataNode : VertexDataNode
    {
        ...
        public override void DrawProperties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            m_index = EditorGUILayoutIntPopup(
                Constants.AvailableUVChannelLabel,
                m_index,
                Constants.AvailableUVSetsStr,
                Constants.AvailableUVChannels
            );
            if (EditorGUI.EndChangeCheck())
            {
                m_currentVertexData =
                    (m_index == 0)
                        ? "texcoord"
                        : "texcoord" + Constants.AvailableUVChannelsStr[m_index];
            }
        }
        ...
        )
        {
            if (dataCollector.IsTemplate)
            {
                ...
                {
                    dataCollector.TemplateDataCollectorInstance.RegisterUV(
                        m_index,
                ...
                if (
                    dataCollector.TemplateDataCollectorInstance.GetCustomInterpolatedData(
                        TemplateHelperFunctions.IntToUVChannelInfo[m_index],
                        m_outputPorts[0].DataType,
                        PrecisionType.Float,
                        ref result,
                        false,
                ...
                else if (dataCollector.TemplateDataCollectorInstance.HasUV(m_index))
                {
                    InterpDataHelper info = dataCollector.TemplateDataCollectorInstance.GetUVInfo(
                        m_index
                    );
                    if (outputId == 0)
                    {
                        return dataCollector.TemplateDataCollectorInstance.GetUVName(
                            m_index,
                            m_outputPorts[0].DataType
                        );
                    }
            ...
            string texcoords = GeneratorUtils.GenerateAutoUVs(
                ref dataCollector,
                UniqueId,
                m_index,
                null,
        ...
        static public string GenerateFragUVs(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId,
            int index,
        ...
        public static string GenerateVertexUVs(
            ref MasterNodeDataCollector dataCollector,
            int uniqueId,
            int index,
            string propertyName = null,
            ...
            else
            {
                ...
                if (m_index > 3)
                {
                    dataCollector.AddCustomAppData(
                        string.Format(TemplateHelperFunctions.TexUVFullSemantic, m_index)
                    );
                }
            }
        }
