...
namespace AmplifyShaderEditor
{
    ...
    public sealed class RangedFloatNode : PropertyNode
    {
        ...
        public RangedFloatNode()
            : base() { }

        public RangedFloatNode(int uniqueId, float x, float y, float width, float height)
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_availableAttribs.Add(
                new PropertyAttributes("No Keyword Toggle", "[ToggleUI]", "[NoKeywordToggle]")
            );
        ...
        public override void DrawSubProperties()
        {
            ...
            if (m_floatMode)
            {
                m_defaultValue = EditorGUILayoutFloatField(
            ...
            {
                m_defaultValue = EditorGUILayoutSlider(
                    Constants.DefaultValueLabel,
                    m_defaultValue,
                    m_min,
                    m_max
        ...
        public override void DrawMaterialProperties()
        {
            ...
            if (m_floatMode)
            {
                m_materialValue = EditorGUILayoutFloatField(
            ...
            {
                m_materialValue = EditorGUILayoutSlider(
                    Constants.MaterialValueLabel,
                    m_materialValue,
                    m_min,
                    m_max
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            if (m_floatMode)
            {
                ...
                m_propertyDrawPos.width =
                    drawInfo.InvertedZoom * Constants.FLOAT_DRAW_WIDTH_FIELD_SIZE;
            ...
            else
            {
                m_propertyDrawPos = m_remainingBox;
                m_propertyDrawPos.width =
                    m_outputPorts[0].Position.x
                    - m_propertyDrawPos.x
                    - (
                        m_outputPorts[0].LabelSize.x
                        + (Constants.PORT_TO_LABEL_SPACE_X + 3) * drawInfo.InvertedZoom
                        + 2
                    );
                m_propertyDrawPos.height =
                    drawInfo.InvertedZoom * Constants.FLOAT_DRAW_HEIGHT_FIELD_SIZE;
        ...
        void DrawFakeSlider(ref float value, DrawInfo drawInfo)
        {
            ...
            sliderBackRect.center = new Vector2(
                sliderValRect.center.x,
                Mathf.Round(sliderValRect.center.y)
            );

            GUI.Label(
                sliderBackRect,
                string.Empty,
                UIUtils.GetCustomStyle(CustomStyle.SliderStyle)
            );
        ...
        void DrawSlider(ref float value, DrawInfo drawInfo)
        {
            ...
            value = GUIHorizontalSlider(
                sliderValRect,
                value,
                m_min,
                m_max,
                GUIStyle.none,
                UIUtils.RangedFloatSliderThumbStyle
            );
        }
        ...
        {
            Shader.SetGlobalFloat(m_propertyName, m_defaultValue);
        }

        public override void FetchGlobalValue()
        {
            m_materialValue = Shader.GetGlobalFloat(m_propertyName);
        }
        ...
        public override void GeneratePPSInfo(ref string propertyDeclaration, ref string propertySet)
        {
            ...
            propertyDeclaration += string.Format(
                ASEPPSHelperTool.PPSPropertyDecFormat,
                additionalHeaders,
                PropertyInspectorName,
                ASEPPSHelperTool.WireToPPSType[WirePortDataType.FLOAT],
                PropertyName,
                m_defaultValue
            );

            propertySet += string.Format(
                ASEPPSHelperTool.PPSPropertySetFormat,
                "Float",
                PropertyName
            );
        }
