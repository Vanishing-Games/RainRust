...
namespace AmplifyShaderEditor
{
    ...
    public sealed class StaticSwitch : PropertyNode
    {
        ...
        public static readonly string[] KeywordTypeList =
        {
            "Shader Feature",
            "Multi Compile",
            "Dynamic Branch",
        };
        public static readonly int[] KeywordTypeInt =
        {
            (int)KeywordType.ShaderFeature,
            (int)KeywordType.MultiCompile,
            (int)KeywordType.DynamicBranch,
        };

        [SerializeField]
        private string[] m_defaultKeywordNames =
        {
            "Key0",
            "Key1",
            "Key2",
        ...
        public const string KeywordModifierSurfaceWarning =
            "Keyword stage ignored in Surface Shaders due to Unity bug.";
        ...
        public override void SetPreviewInputs()
        {
            ...
            StaticSwitch node =
                (m_staticSwitchVarMode == StaticSwitchVariableMode.Reference && m_reference != null)
                    ? m_reference
                    : this;
        ...
        void OnReorderEventComplete()
        {
            if (CurrentVarMode == StaticSwitchVariableMode.Reference)
            {
                if (m_reference != null)
                {
                    m_referenceArrayId = ContainerGraph.StaticSwitchNodes.GetNodeRegisterIdx(
                        m_reference.UniqueId
                    );
                }
            }
        }
            ...
            if (m_createToggle)
            {
                ...
                if (m_keywordModeType == KeywordModeType.KeywordEnum && m_keywordEnumAmount > 0)
                {
                    return PropertyAttributes
                        + "["
                        + m_keywordModeType.ToString()
                        + "("
                        + GetKeywordEnumPropertyList()
                        + ")] "
                        + m_propertyName
                        + "(\""
        ...
        public string KeywordEnum(int index)
        {
            if (m_createToggle)
            {
                return string.IsNullOrEmpty(PropertyName)
                    ? KeywordEnumList(index)
                    : (PropertyName + "_" + KeywordEnumList(index));
            }
            else
            {
                return string.IsNullOrEmpty(PropertyName)
                    ? KeywordEnumList(index)
                    : (PropertyName + KeywordEnumList(index));
            }
        }
        ...
        public string KeywordEnumList(int index)
        {
            ...
            {
                return m_createToggle
                    ? m_keywordEnumList[index].ToUpper()
                    : m_keywordEnumList[index];
            }
        }
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            m_showErrorMessage = (
                m_multiCompile == (int)KeywordType.DynamicBranch
                && TemplateHelperFunctions.GetUnityVersion() < 20220100
            );
        }
        ...
        void DrawEnumList()
        {
            ...
            for (int i = 0; i < m_keywordEnumList.Length; i++)
            {
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    m_keywordEnumList[i] = UIUtils.RemoveInvalidEnumCharacters(
                        m_keywordEnumList[i]
                    );
        ...
        {
            int maxinputs =
                m_keywordModeType == KeywordModeType.KeywordEnum ? KeywordEnumAmount : 2;
        ...
        {
            EditorGUI.BeginChangeCheck();
            CurrentVarMode = (StaticSwitchVariableMode)EditorGUILayoutEnumPopup(
                ModeStr,
                CurrentVarMode
            );
            if (EditorGUI.EndChangeCheck())
            ...
            {
                EditorGUI.BeginChangeCheck();
                m_multiCompile = EditorGUILayoutIntPopup(
                    KeywordTypeStr,
                    m_multiCompile,
                    KeywordTypeList,
                    KeywordTypeInt
            ...
            {
                EditorGUI.BeginChangeCheck();
                m_keywordModeType = (KeywordModeType)EditorGUILayoutEnumPopup(
                    TypeStr,
                    m_keywordModeType
                );
                if (EditorGUI.EndChangeCheck())
                ...
                if (CurrentVarMode == StaticSwitchVariableMode.Create || m_createToggle)
                {
                    ...
                    if (CurrentVarMode == StaticSwitchVariableMode.Create)
                    {
                        ...
                        if (m_lockKeyword)
                            ...
                            m_currentKeyword = EditorGUILayoutTextField(
                                KeywordNameStr,
                                m_currentKeyword
                            );
                        GUI.enabled = guiEnabledBuffer;
                        m_lockKeyword = GUILayout.Toggle(
                            m_lockKeyword,
                            (m_lockKeyword ? UIUtils.LockIconOpen : UIUtils.LockIconClosed),
                            "minibutton",
                            GUILayout.Width(22)
                        );
            ...
            if (CurrentVarMode == StaticSwitchVariableMode.Fetch)
            {
                //ShowPropertyInspectorNameGUI();
                EditorGUI.BeginChangeCheck();
                m_currentKeywordId = EditorGUILayoutPopup(
                    KeywordStr,
                    m_currentKeywordId,
                    UIUtils.AvailableKeywords
                );
                if (EditorGUI.EndChangeCheck())
                {
            ...
            GUI.enabled = (m_multiCompile != (int)KeywordType.DynamicBranch);
            {
                ...
                bool isFunction = (
                    m_containerGraph.CurrentCanvasMode == NodeAvailability.ShaderFunction
                );
                bool isTemplate = (
                    m_containerGraph.CurrentCanvasMode == NodeAvailability.TemplateShader
                );
                if (
                    (!isTemplate || isFunction)
                    && (m_keywordModeType == KeywordModeType.KeywordEnum)
                    && (m_shaderStage != ShaderStage.All)
                )
                {
                    EditorGUILayout.HelpBox(
                        KeywordModifierSurfaceWarning,
                        isFunction ? MessageType.Info : MessageType.Warning
                    );
                }
            ...
            if (m_createToggle)
            {
                ...
                switch (m_propertyTab)
                {
                    ...
                    case 0:
                        {
                            EditorGUI.BeginChangeCheck();
                            if (m_keywordModeType != KeywordModeType.KeywordEnum)
                                m_materialValue = EditorGUILayoutToggle(
                                    ToggleMaterialValueStr,
                                    m_materialValue == 1
                                )
                                    ? 1
                                    : 0;
                            else
                                m_materialValue = EditorGUILayoutPopup(
                                    ToggleMaterialValueStr,
                                    m_materialValue,
                                    m_keywordEnumList
                                );
                    ...
                    case 1:
                        {
                            if (m_keywordModeType != KeywordModeType.KeywordEnum)
                                m_defaultValue = EditorGUILayoutToggle(
                                    ToggleDefaultValueStr,
                                    m_defaultValue == 1
                                )
                                    ? 1
                                    : 0;
                            else
                                m_defaultValue = EditorGUILayoutPopup(
                                    ToggleDefaultValueStr,
                                    m_defaultValue,
                                    m_keywordEnumList
                                );
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            if (m_staticSwitchVarMode == StaticSwitchVariableMode.Reference)
            {
                ...
                m_iconPos.x += /*m_globalPosition.width - m_iconPos.width - */
                    5 * drawInfo.InvertedZoom;
            }
        }
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_editing)
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    CurrentSelectedInput = EditorGUIPopup(
                        m_varRect,
            ...
            if (m_createToggle && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD2)
            {
                if (!m_editing)
                {
                    ...
                    else
                    {
                        GUI.Label(
                            m_varRect,
                            m_keywordEnumList[CurrentSelectedInput],
                            UIUtils.GraphDropDown
                        );
                ...
                if (allowStages)
                {
                    switch (m_shaderStage)
                    {
                        ...
                        case ShaderStage.Vertex:
                            staticSwitchType += "_vertex";
                            break;
                        case ShaderStage.Fragment:
                            staticSwitchType += "_fragment";
                            break;
                        case ShaderStage.Hull:
                            staticSwitchType += "_hull";
                            break;
                        case ShaderStage.Domain:
                            staticSwitchType += "_domain";
                            break;
                        case ShaderStage.Geometry:
                            staticSwitchType += "_geometry";
                            break;
                        case ShaderStage.Raytracing:
                            staticSwitchType += "_raytracing";
                            break;
                    }
                }
            }
        ...
        void RegisterPragmas(ref MasterNodeDataCollector dataCollector)
        {
            if (CurrentVarMode == StaticSwitchVariableMode.Create)
            {
                string staticSwitchType = GetStaticSwitchType(
                    allowStages: dataCollector.IsTemplate
                        || (m_keywordModeType != KeywordModeType.KeywordEnum)
                );
                ...
                if (m_keywordModeType == KeywordModeType.KeywordEnum)
                {
                    dataCollector.AddToPragmas(
                        UniqueId,
                        staticSwitchType + " " + GetKeywordEnumPragmaList()
                    );
                }
                else
                {
                    if (m_multiCompile == 1)
                        dataCollector.AddToPragmas(
                            UniqueId,
                            staticSwitchType + " __ " + CurrentKeyword
                        );
                    else
                        dataCollector.AddToPragmas(
                            UniqueId,
                            staticSwitchType + " " + CurrentKeyword
                        );
                }
            }
        }
        ...
        )
        {
            ...
            string outType = UIUtils.PrecisionWirePortToCgType(
            ...
            if (node.KeywordModeTypeValue == KeywordModeType.KeywordEnum)
            {
                ...
                if (m_multiCompile == (int)KeywordType.DynamicBranch)
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        outType + " dynamicSwitch" + OutputId + " = ( " + outType + " )0;",
                        true
                    );
                    for (int i = 0; i < node.KeywordEnumAmount; i++)
                    {
                        ...
                        dataCollector.AddLocalVariable(
                            UniqueId,
                            "\tdynamicSwitch" + OutputId + " = " + allOutputs[i] + ";",
                            true
                        );
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "\tdynamicSwitch" + OutputId + " = " + allOutputs[node.DefaultValue] + ";",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "}", true);
                }
            ...
            else
            {
                ...
                if (m_multiCompile == (int)KeywordType.DynamicBranch)
                {
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "UNITY_BRANCH if ( " + node.CurrentKeyword + " )",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "{", true);
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "\tdynamicSwitch" + OutputId + " = " + trueCode + ";",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "}", true);
                    dataCollector.AddLocalVariable(UniqueId, "else", true);
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "\tdynamicSwitch" + OutputId + " = " + falseCode + ";",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "}", true);
                    m_outputPorts[0]
                        .SetLocalValue("dynamicSwitch" + OutputId, dataCollector.PortCategory);
                }
                else
                {
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "\t" + outType + " staticSwitch" + OutputId + " = " + trueCode + ";",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "#else", true);
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "\t" + outType + " staticSwitch" + OutputId + " = " + falseCode + ";",
                        true
                    );
                    dataCollector.AddLocalVariable(UniqueId, "#endif", true);
                    m_outputPorts[0]
                        .SetLocalValue("staticSwitch" + OutputId, dataCollector.PortCategory);
                }
            }
        ...
        public override void DrawTitle(Rect titlePos)
        {
            ...
            SetAdditonalTitleTextOnCallback(
                subTitle,
                subTitleFormat,
                (instance, newSubTitle) =>
                    instance.AdditonalTitleContent.text = string.Format(subTitleFormat, newSubTitle)
            );
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_keywordModeType = (KeywordModeType)
                    Enum.Parse(typeof(KeywordModeType), GetCurrentParam(ref nodeParams));
            }
            ...
            if (UIUtils.CurrentShaderVersion() > 16304)
            {
                ...
                CurrentVarMode = (StaticSwitchVariableMode)
                    Enum.Parse(typeof(StaticSwitchVariableMode), currentVarMode);
                if (CurrentVarMode == StaticSwitchVariableMode.Reference)
                {
            ...
            if (UIUtils.CurrentShaderVersion() > 18928)
                m_shaderStage = (ShaderStage)
                    Enum.Parse(typeof(ShaderStage), GetCurrentParam(ref nodeParams));
        ...
        public bool IsStaticSwitchDirty
        {
            get { return m_isStaticSwitchDirty; }
        }
        public KeywordModeType KeywordModeTypeValue
        {
            get { return m_keywordModeType; }
        }
        ...
        public string CurrentKeyword
        {
            get
            {
                ...
                return (
                    m_lockKeyword || string.IsNullOrEmpty(m_currentKeyword)
                        ? PropertyName + OnOffStr
                        : m_currentKeyword
                );
            }
        }
        ...
        {
            get { return m_createToggle; }
        }

        public int KeywordEnumAmount
        {
            get { return m_keywordEnumAmount; }
            set
            {
