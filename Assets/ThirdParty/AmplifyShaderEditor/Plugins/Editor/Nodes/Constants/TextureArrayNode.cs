...
namespace AmplifyShaderEditor
{
    ...
    public class TextureArrayNode : PropertyNode
    {
            ...
            if (m_referenceType == TexReferenceType.Instance && m_referenceSampler != null)
            {
                if ((ParentNode)m_referenceSampler != m_referenceSampler.PreviewTextProp)
                {
                    ...
                    PreviewMaterial.SetTexture(
                        "_G",
                        m_referenceSampler.PreviewTextProp.PreviewTexture
                    );
                }
                else
                {
        ...
        private void OnReorderEventComplete()
        {
            if (m_referenceType == TexReferenceType.Instance && m_referenceSampler != null)
            {
                m_referenceArrayId = ContainerGraph.TextureArrayNodes.GetNodeRegisterIdx(
        ...
        {
            m_uvSet = EditorGUILayoutIntPopup(
        ...
        {
            m_requireMaterialUpdate = true;
            Texture2DArray texture = m_materialMode
                ? m_materialTextureArray
                : m_defaultTextureArray;
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_isEditingPicker && m_drawPicker)
            {
                ...
                if (
                    ...
                    && hitRect.Contains(drawInfo.MousePosition)
                )
                {
                    restoreMouse = true;
                    ...
                    if (drawInfo.CurrentEventType == EventType.Repaint)
                    {
                        ...
                        if (m_previewTextProp != null)
                        {
                            SetTitleTextOnCallback(
                                m_previewTextProp.TitleContent.text,
                                (instance, newTitle) =>
                                    instance.TitleContent.text = newTitle + " (Input)"
                            );
                ...
                case ReferenceState.Instance:
                    {
                        ...
                        {
                            SetTitleTextOnCallback(
                                m_referenceSampler.PreviewTextProp.TitleContent.text,
                                (instance, newTitle) =>
                                    instance.TitleContent.text =
                                        newTitle + Constants.InstancePostfixStr
                            );
                            SetAdditonalTitleText(
                                m_referenceSampler.PreviewTextProp.AdditonalTitleContent.text
                            );
                        }
        ...
        protected void DrawTexturePicker(DrawInfo drawInfo)
        {
            ...
            Texture2DArray currentValue = m_materialMode
                ? m_materialTextureArray
                : m_defaultTextureArray;
        ...
        )
        {
            ...
            {
                if (m_referenceSampler.TexPort.IsConnected)
                    propertyName = m_referenceSampler.TexPort.GeneratePortInstructions(
                        ref dataCollector
                    );
                else
            ...
            else
            {
                ...
                {
                    uvs = dataCollector.TemplateDataCollectorInstance.GetTextureCoord(
                        m_uvSet,
                        propertyName /*( instanced ? m_referenceSampler.PropertyName : PropertyName )*/
                        ,
                        UniqueId,
                ...
                {
                    if (isVertex)
                        uvs = TexCoordVertexDataNode.GenerateVertexUVs(
                            ref dataCollector,
                            UniqueId,
                            m_uvSet,
                            propertyName
                        ...
                        uvs = TexCoordVertexDataNode.GenerateFragUVs(
                            ref dataCollector,
                            UniqueId,
                            m_uvSet,
                            propertyName
            ...
            if (dataCollector.IsTemplate && dataCollector.IsSRP)
            {
                ...
                if (m_mipMode == MipType.MipBias)
                {
                    ...
                    result =
                        ...
                        + index
                        + "), "
                        + m_ddxPort.GeneratePortInstructions(ref dataCollector)
                        + ", "
                        + m_ddyPort.GeneratePortInstructions(ref dataCollector)
                        + ");";
                }
            ...
            else
            {
                ...
                else
                {
                    result =
                        ...
                        + (m_lodPort.Visible || isVertex ? "_LOD" : "")
                        + "("
                        + propertyName
                        + ", float3("
                        + uvs
                        ...
                        + index
                        + ") "
                        + (m_lodPort.Visible || isVertex ? ", " + level : "")
                        + " )";
                }
            }
            ...
            if (m_autoUnpackNormals)
            {
                ...
                string scaleValue = isScaledNormal
                    ...
                    : "1.0";
                result = GeneratorUtils.GenerateUnpackNormalStr(
                    ref dataCollector,
                    CurrentPrecisionType,
                    UniqueId,
                    OutputId,
        ...
        )
        {
            MasterNode currMasterNode =
                (m_containerGraph.CurrentMasterNode != null)
                    ? m_containerGraph.CurrentMasterNode
                    : m_containerGraph.ParentWindow.OutsideGraph.CurrentMasterNode;
            if (
                currMasterNode != null
                && currMasterNode.CurrentDataCollector.IsTemplate
                && currMasterNode.CurrentDataCollector.IsSRP
            )
            {
        ...
        public override void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ...
                (m_defaultTextureArray != null)
                    ? AssetDatabase.GetAssetPath(m_defaultTextureArray)
        ...
        public override void WriteAdditionalClipboardData(ref string nodeInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ...
                (m_materialTextureArray != null)
                    ? AssetDatabase.GetAssetPath(m_materialTextureArray)
                    : Constants.NoStringValue
            );
        }
        ...
        public override void UpdateMaterial(Material mat)
        {
            ...
            if (
                ...
                && !InsideShaderFunction
                && m_referenceType == TexReferenceType.Object
            )
            {
                OnPropertyNameChanged();
        ...
        public override bool UpdateShaderDefaults(
            ref Shader shader,
        ...
        public override string GetPropertyValStr()
        {
            return m_materialMode
                ? (
                    m_materialTextureArray != null
                        ? m_materialTextureArray.name
                        : IOUtils.NO_TEXTURES
                )
                : (
                    m_defaultTextureArray != null ? m_defaultTextureArray.name : IOUtils.NO_TEXTURES
                );
        }
        ...
        public ParentNode PreviewTextProp
        {
            get { return m_previewTextProp; }
        }
        public InputPort TexPort
        {
