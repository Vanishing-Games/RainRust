...
namespace AmplifyShaderEditor
{
    ...
    public sealed class GradientNode : ParentNode
    {
        ...
        public GradientNode(int uniqueId, float x, float y, float width, float height)
        ...
        )
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                if (i < m_gradient.colorKeys.Length)
                {
                    colors[i] =
                        "float4( "
                        + m_gradient.colorKeys[i].color.r
                        + ", "
                        + m_gradient.colorKeys[i].color.g
                        + ", "
                        + m_gradient.colorKeys[i].color.b
                        + ", "
            ...
            for (int i = 0; i < 8; i++)
            {
                if (i < m_gradient.alphaKeys.Length)
                {
                    alphas[i] =
                        "float2( "
                        + m_gradient.alphaKeys[i].alpha
                        + ", "
                        + m_gradient.alphaKeys[i].time
                        + " )";
                }
                else
                {
            ...
            string functionResult = dataCollector.AddFunctions(
                ...
                m_functionBody,
                (int)m_gradient.mode,
                m_gradient.colorKeys.Length,
                m_gradient.alphaKeys.Length,
                colors[0],
                colors[1],
                colors[2],
                colors[3],
            ...
            dataCollector.AddLocalVariable(
                UniqueId,
                "Gradient gradient" + OutputId + " = " + functionResult + ";"
            );
