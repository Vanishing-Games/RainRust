...
namespace AmplifyShaderEditor
{
    ...
    public class PropertyNode : ParentNode
    {
        ...
        private const string InvalidAttributeFormatter =
            "Attribute {0} not found on node {1}. Please click on this message to select node and review its attributes section";
        ...
        private const string FetchToCreateDuplicatesMsg =
            "Reverting property name from '{0}' to '{1}' as it is registered to another property node.";
        private const string FetchToCreateOnDuplicateNodeMsg =
            "Setting new property name '{0}' as '{1}' is registered to another property node.";
        ...
        public PropertyNode(int uniqueId, float x, float y, float width, float height)
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_availableAttribs.Add(
                new PropertyAttributes("Hide in Inspector", "[HideInInspector]")
            );
            m_availableAttribs.Add(new PropertyAttributes("HDR", "[HDR]"));
            m_availableAttribs.Add(new PropertyAttributes("Gamma", "[Gamma]"));
            m_availableAttribs.Add(
                new PropertyAttributes("Per Renderer Data", "[PerRendererData]")
            );
        ...
        public override void AfterCommonInit()
        {
            ...
            if (PaddingTitleLeft == 0 && m_freeType)
            {
                ...
                if (PaddingTitleRight == 0)
                    PaddingTitleRight =
                        Constants.PropertyPickerWidth + Constants.IconsLeftRightMargin;
            }
        ...
        public void CheckDelayedDirtyProperty()
        {
            ...
            {
                if (
                    (EditorApplication.timeSinceStartup - m_delayedDirtyPropertyTimestamp)
                    > MaxPropertyTimestamp
                )
                {
        ...
        public virtual void CheckPropertyFromInspector(bool forceUpdate = false)
        {
            if (m_propertyFromInspector)
            {
                if (
                    forceUpdate
                    || (EditorApplication.timeSinceStartup - m_propertyFromInspectorTimestamp)
                        > MaxTimestamp
                )
                {
                    m_propertyFromInspector = false;
                    bool autoGlobal =
                        m_autoGlobalName || m_currentParameterType == PropertyType.Global;
        ...
        {
            if (
                m_checkDuplicateProperty
                && (EditorApplication.timeSinceStartup - m_checkDuplicatePropertyTimestamp)
                    > MaxTimestamp
            )
            {
                m_checkDuplicateProperty = false;
                m_propertyName = UIUtils.GeneratePropertyName(
                    m_propertyName,
                    PropertyType.Global,
                    false
                );
                ...
                if (UIUtils.IsNumericName(m_propertyName))
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        string.Format(
                            "Invalid property name '{0}' as it cannot start with numbers. Reverting to last valid name '{1}'",
                            m_propertyName,
                            m_oldName
                        ),
                ...
                if (!m_propertyName.Equals(m_oldName))
                {
                    ...
                    else
                    {
                        ...
                        UIUtils.ShowMessage(
                            UniqueId,
                            string.Format(
                                "Property name '{0}' is already in use. Reverting to last valid name '{1}'",
                                m_propertyName,
                                m_oldName
                            )
                        );
        ...
        public void ChangeParameterType(PropertyType parameterType)
        {
            UndoUtils.RegisterCompleteObjectUndo(
                ...
                Constants.UndoChangePropertyTypeNodesId
            );
            UndoUtils.RegisterCompleteObjectUndo(
                m_containerGraph,
                Constants.UndoChangePropertyTypeNodesId
            ...
            if (
                (
                    parameterType == PropertyType.Property
                    || parameterType == PropertyType.InstancedProperty
                )
                && m_currentParameterType != PropertyType.Property
                && m_currentParameterType != PropertyType.InstancedProperty
            )
            {
        ...
        {
            m_enumModeInt = EditorGUILayout.IntPopup(
                "Mode",
                m_enumModeInt,
                EnumModesStr,
                EnumModeIntValues
            ...
            if (m_enumModeInt == 0)
            {
                ...
                for (int i = 0; i < m_enumNames.Count; i++)
                {
                    ...
                    m_enumValues[i] = Mathf.Max(
                        0,
                        EditorGUILayoutIntField("Value", m_enumValues[i], GUILayout.Width(100))
                    );
            ...
            else
            {
                ...
                if (
                    GUILayout.Button(
                        string.Empty,
                        UIUtils.InspectorPopdropdownFallback,
                        GUILayout.Width(17),
                        GUILayout.Height(19)
                    )
                )
        ...
        private void AddMenuItem(GenericMenu menu, string newClass)
        {
            menu.AddItem(
                ...
                m_enumClassName.Equals(newClass),
                OnSelection,
                newClass
            );
        }
        ...
        protected void DrawHeaderAttributes()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                m_headerAttributeValues[i] = EditorGUILayoutTextField(
                    "Header " + i,
                    m_headerAttributeValues[i]
                );
                if (EditorGUI.EndChangeCheck())
                {
                    m_headerAttributeValues[i] = UIUtils.RemoveHeaderAttrCharacters(
                        m_headerAttributeValues[i]
                    );
                }
            }
        ...
        public virtual void DrawAttributes()
        {
            ...
            for (int i = 0; i < m_availableAttribsArr.Length; i++)
            {
                m_selectedAttribsArr[i] = EditorGUILayoutToggleLeft(
                    m_availableAttribsArr[i],
                    m_selectedAttribsArr[i]
                );
            }
            if (EditorGUI.EndChangeCheck())
            {
            ...
            bool customAttr = EditorGUILayoutToggleLeft(
                "Custom",
                m_customAttrCount == 0 ? false : true
            );
            if (!customAttr)
            {
                m_customAttrCount = 0;
        ...
        public virtual void DrawMainPropertyBlock()
        {
            EditorGUILayout.BeginVertical();
            {
                if (m_freeType)
                {
                    PropertyType parameterType = (PropertyType)EditorGUILayoutEnumPopup(
                        ParameterTypeStr,
                        m_currentParameterType
                    );
        ...
        public override void DrawProperties()
        {
            ...
            if (m_freeType || m_freeName)
            {
                ...
                if (m_drawAttributes)
                    NodeUtils.DrawPropertyGroup(
                        ref m_visibleAttribsFoldout,
                        Constants.AttributesLaberStr,
                        DrawAttributes
                    );
                ...
                if (m_drawAttributes)
                {
                    if (m_hasHeaders)
                        NodeUtils.DrawPropertyGroup(
                            ref m_visibleHeaderAttrFoldout,
                            HeaderAttrStr,
                            DrawHeaderAttributes,
                            DrawHeaderAttrAddRemoveButtons
                        );

                    if (m_customAttrCount > 0)
                        NodeUtils.DrawPropertyGroup(
                            ref m_visibleCustomAttrFoldout,
                            CustomAttrStr,
        ...
        public void ShowPropertyNameGUI(bool isProperty)
        {
            ...
            if (isProperty)
            {
                ...
                EditorGUI.BeginChangeCheck();
                m_autoGlobalName = GUILayout.Toggle(
                    m_autoGlobalName,
                    (m_autoGlobalName ? UIUtils.LockIconOpen : UIUtils.LockIconClosed),
                    "minibutton",
                    GUILayout.Width(22)
                );
                if (EditorGUI.EndChangeCheck())
        ...
        {
            if (
                m_showHybridInstancedUI
                && CurrentParameterType == PropertyType.Property
                && (m_containerGraph.IsSRP || m_containerGraph.CurrentShaderFunction != null)
            )
            {
        ...
        public virtual string GetPropertyValStr()
        {
            return string.Empty;
        }

        public virtual string GetSubTitleVarNameFormatStr()
        {
            return Constants.SubTitleVarNameFormatStr;
        }
        ...
        {
            if (
                currentMousePos2D.y - m_globalPosition.y
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                ...
                if (m_isEditing || m_startEditing)
                {
                    ...
                    m_propertyInspectorName = EditorGUITextField(
                        m_titleClickArea,
                        string.Empty,
                        m_propertyInspectorName,
                ...
                if (m_freeType)
                {
                    ...
                    {
                        PropertyType parameterType = (PropertyType)EditorGUIEnumPopup(
                            m_dropdownRect,
                            m_currentParameterType,
                            UIUtils.PropertyPopUp
                        );
                        if (parameterType != m_currentParameterType)
                        {
                            ChangeParameterType(parameterType);
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            if (m_currentParameterType != m_lastParameterType || m_propertyNameIsDirty)
            {
                ...
                if (m_currentParameterType != PropertyType.Constant)
                {
                    ...
                    //{
                    string currValue =
                        (m_currentParameterType == PropertyType.Global && m_globalDefaultBehavior)
                            ? "<GLOBAL>"
                            : GetPropertyValStr();
                    SetClippedAdditionalTitle(
                        string.Format(
                            m_useVarSubtitle
                                ? GetSubTitleVarNameFormatStr()
                                : Constants.SubTitleValueFormatStr,
                            currValue
                        ),
                        m_longNameSize,
                        LongNameEnder
                ...
                else
                {
                    SetClippedTitle(m_propertyInspectorName, m_longNameSize);
                    SetClippedAdditionalTitle(
                        string.Format(Constants.SubTitleConstFormatStr, GetPropertyValStr()),
                        m_longNameSize,
                        LongNameEnder
                    );
                }
        ...
        public void RegisterFirstAvailablePropertyName(
            bool releaseOldOne,
            bool appendIndexToCurrOne = false
        )
        {
            if (releaseOldOne)
                UIUtils.ReleaseUniformName(UniqueId, m_oldName);
            ...
            if (m_isNodeBeingCopied || appendIndexToCurrOne)
            {
                ...
                else
                {
                    if (UIUtils.IsUniformNameAvailable(m_propertyName))
                        ...
                        UIUtils.GetFirstAvailableName(
                            UniqueId,
                            m_outputPorts[0].DataType,
                            out m_propertyName,
                            out m_propertyInspectorName,
                            !string.IsNullOrEmpty(m_customPrefix),
                            m_customPrefix
                        );
                }
            }
        ...
        public void RegisterPropertyName(
            bool releaseOldOne,
            string newName,
            bool autoGlobal = true,
        ...
        )
        {
            ...
            )
            {
                ...
                if (autoGlobal)
                    localPropertyName = UIUtils.GeneratePropertyName(
                        newName,
                        m_currentParameterType,
                        forceUnderscore
                    );
            ...
            if (autoGlobal)
                propertyName = UIUtils.GeneratePropertyName(
                    newName,
                    m_currentParameterType,
                    forceUnderscore
                );
        ...
        public virtual void CheckIfAutoRegister(ref MasterNodeDataCollector dataCollector)
        {
            ...
            if (
                CurrentParameterType != PropertyType.Constant
                && m_autoRegister
                && (m_connStatus != NodeConnectionStatus.Connected || InsideShaderFunction)
            )
            {
        ...
        protected virtual void RegisterProperty(ref MasterNodeDataCollector dataCollector)
        {
                ...
                case PropertyType.Property:
                    {
                        ...
                        if (
                            m_variableMode == VariableMode.Create
                            && GetUniformData(out dataType, out dataName, ref fullValue)
                        )
                        {
                            if (fullValue)
                            {
                        ...
                        {
                            dataCollector.AddToDotsProperties(
                                m_outputPorts[0].DataType,
                                UniqueId,
                                m_propertyName,
                                OrderIndex,
                ...
                case PropertyType.InstancedProperty:
                    {
                        ...
                        dataCollector.AddToInstancedProperties(
                            m_outputPorts[0].DataType,
                            UniqueId,
                            GetInstancedUniformValue(dataCollector.IsTemplate, dataCollector.IsSRP),
                            OrderIndex
                        );
                    }
                    break;
            ...
            if (
                m_currentParameterType == PropertyType.InstancedProperty
                && !m_outputPorts[0].IsLocalValue(dataCollector.PortCategory)
            )
            {
                string instancedVar = dataCollector.IsSRP
                    ...
                    //m_propertyName :
                    string.Format(
                        IOUtils.LWSRPInstancedPropertiesData,
                        dataCollector.InstanceBlockName,
                        m_propertyName
                    )
        ...
        {
            UIUtils.UpdatePropertyDataNode(UniqueId, PropertyInspectorName);
        }
        ...
        public string GetInstancedUniformValue(bool isTemplate, bool isSRP)
        {
            if (isTemplate)
            {
                ...
                return string.Format(
                    IOUtils.InstancedPropertiesElementTabs,
                    UIUtils.PrecisionWirePortToCgType(
                        CurrentPrecisionType,
                        m_outputPorts[0].DataType
                    ),
        ...
        public string GetInstancedUniformValue(
            bool isTemplate,
            bool isSRP,
        ...
        )
        {
            if (isTemplate)
            {
                if (isSRP)
                {
                    ...
                    return string.Format(
                        IOUtils.LWSRPInstancedPropertiesElement,
                        UIUtils.PrecisionWirePortToCgType(CurrentPrecisionType, dataType),
                        value
                    );
                }
                else
        ...
        public virtual bool GetUniformData(
            out string dataType,
            out string dataName,
            ref bool fullValue
        )
        {
            dataType = UIUtils.PrecisionWirePortToCgType(
                CurrentPrecisionType,
                m_outputPorts[0].DataType
            );
            ...
            if (attribCount > 0)
            {
                for (int i = 0; i < attribCount; i++)
                {
                    IOUtils.AddFieldValueToString(
                        ref nodeInfo,
                        m_availableAttribs[m_selectedAttribs[i]].Attribute
                    );
                }
            }
        ...
        int IdForAttrib(string name)
        {
            ...
            {
                if (
                    m_availableAttribs[i].Attribute.Equals(name)
                    || (
                        m_availableAttribs[i].HasDeprecatedValue
                        && m_availableAttribs[i].DeprecatedValue.Equals(name)
                    )
                )
                    return i;
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 2505)
            {
                ...
                m_currentParameterType = property.Equals("Uniform")
                    ? PropertyType.Global
                    : (PropertyType)Enum.Parse(typeof(PropertyType), property);
            }
            else
            {
                m_currentParameterType = (PropertyType)
                    Enum.Parse(typeof(PropertyType), GetCurrentParam(ref nodeParams));
            }
            ...
            if (UIUtils.CurrentShaderVersion() > 14003)
            {
                m_variableMode = (VariableMode)
                    Enum.Parse(typeof(VariableMode), GetCurrentParam(ref nodeParams));
            }
        ...
        {
            if (
                m_variableMode == VariableMode.Fetch /* && m_autoGlobalName */
            )
            {
        ...
        void UpdateTooltip()
        {
            ...
            m_tooltipText = string.Format(
                TooltipFormatter,
                m_nodeAttribs.Description,
                m_propertyInspectorName,
                currValue
            );
        }

        public override void SetClippedTitle(
            string newText,
            int maxSize = 170,
            string endString = "..."
        )
        ...
        public override void SetClippedAdditionalTitle(
            string newText,
            int maxSize = 170,
            string endString = "..."
        )
        ...
        public bool CanDrawMaterial
        {
            get { return m_materialMode && m_currentParameterType != PropertyType.Constant; }
        }
                        ...
                        else
                        {
                            ...
                            else if (UIUtils.CheckUniformNameOwner(m_propertyName) != UniqueId)
                            {
                                ...
                                UIUtils.ShowMessage(
                                    UniqueId,
                                    string.Format(
                                        FetchToCreateOnDuplicateNodeMsg,
                                        m_propertyName,
                                        oldProperty
                                    ),
                                    MessageSeverity.Warning
        ...
        {
            return (m_currentParameterType == PropertyType.InstancedProperty)
                ? m_outputPorts[0].LocalValue(portCategory)
                : m_propertyName;
        }
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (
                m_currentParameterType == PropertyType.Global
                && m_globalDefaultBehavior
                && (EditorApplication.timeSinceStartup - m_globalFetchTimestamp)
                    > MaxGlobalFetchTimestamp
            )
            {
                FetchGlobalValue();
        ...
        public override bool CheckFindText(string text)
        {
            return base.CheckFindText(text)
                ...
                || m_propertyInspectorName.IndexOf(text, StringComparison.CurrentCultureIgnoreCase)
                    >= 0;
        }
        ...
        public virtual void GeneratePPSInfo(
            ref string propertyDeclaration,
            ref string propertySet
        ) { }
        ...
        public virtual string PropertyInspectorName
        {
            get { return m_propertyInspectorName; }
        }
        public bool FreeType
        {
            get { return m_freeType; }
            set { m_freeType = value; }
        }
        public bool ReRegisterName
        {
            get { return m_reRegisterName; }
            set { m_reRegisterName = value; }
        }
        public string CustomPrefix
        {
            get { return m_customPrefix; }
            set { m_customPrefix = value; }
        }
        ...
        {
            get { return PropertyInspectorName; }
        }
        public bool RegisterPropertyOnInstancing
        {
            get { return m_registerPropertyOnInstancing; }
            set { m_registerPropertyOnInstancing = value; }
        }
        public bool SrpBatcherCompatible
        {
            get { return m_srpBatcherCompatible; }
        }
        public bool AddGlobalToSRPBatcher
        {
            get { return m_addGlobalToSRPBatcher; }
            set { m_addGlobalToSRPBatcher = value; }
        }
