...
namespace AmplifyShaderEditor
{
    ...
    public sealed class CustomStandardSurface : ParentNode
    {
        ...
        public override void DrawProperties()
        {
            ...
            m_workflow = (ASEStandardSurfaceWorkflow)EditorGUILayoutEnumPopup(
                WorkflowStr,
                m_workflow
            );
        ...
        )
        {
            ...
            dataCollector.AddLocalVariable(
                ...
                "s"
                    + OutputId
                    + ".Smoothness = "
                    + m_inputPorts[4].GeneratePortInstructions(ref dataCollector)
                    + ";"
            );
            dataCollector.AddLocalVariable(
                UniqueId,
            ...
            dataCollector.AddLocalVariable(
                ...
                "Unity_GlossyEnvironmentData g"
                    + OutputId
                    + " = UnityGlossyEnvironmentSetup( s"
                    + OutputId
                    + ".Smoothness, data.worldViewDir, s"
                    + OutputId
                    + ".Normal, float3(0,0,0));"
            );
            dataCollector.AddLocalVariable(
                ...
                "gi"
                    + OutputId
                    + " = UnityGlobalIllumination( data, s"
                    + OutputId
                    + ".Occlusion, s"
                    + OutputId
                    + ".Normal, g"
            ...
            dataCollector.AddLocalVariable(
                UniqueId,
                "surfResult" + OutputId + " += s" + OutputId + ".Emission;\n"
            );
            ...
            dataCollector.AddLocalVariable(
                UniqueId,
                string.Format("surfResult{0} -= s{0}.Emission;", OutputId)
            );
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_workflow = Convert.ToBoolean(GetCurrentParam(ref nodeParams))
                    ? ASEStandardSurfaceWorkflow.Specular
                    : ASEStandardSurfaceWorkflow.Metallic;
            }
            else
            {
                m_workflow = (ASEStandardSurfaceWorkflow)
                    Enum.Parse(typeof(ASEStandardSurfaceWorkflow), GetCurrentParam(ref nodeParams));
            }
            UpdateSpecularMetallicPorts();
