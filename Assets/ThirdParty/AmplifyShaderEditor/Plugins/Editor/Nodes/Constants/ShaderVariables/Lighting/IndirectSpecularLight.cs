...
namespace AmplifyShaderEditor
{
    ...
    public sealed class IndirectSpecularLight : ParentNode
    {
        ...
        private const string UpgradeErrorMessage =
            "Smoothness port was previously being used as Roughness, please check if you are correctly using it and save to confirm.";
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            {
                m_errorMessageTypeIsError = m_upgradeMessage
                    ? NodeMessageType.Warning
                    : NodeMessageType.Error;
                m_errorMessageTooltip = m_upgradeMessage
                    ? UpgradeErrorMessage
                    : DefaultErrorMessage;
                ...
                && (
                    ...
                    && ContainerGraph.CurrentSRPType != TemplateSRPType.BiRP
                )
            )
            {
        ...
        public override void DrawProperties()
        {
            ...
            if (!m_inputPorts[1].IsConnected)
                m_inputPorts[1].FloatInternalData = EditorGUILayout.FloatField(
                    m_inputPorts[1].Name,
                    m_inputPorts[1].FloatInternalData
                );
            if (!m_inputPorts[2].IsConnected)
                m_inputPorts[2].FloatInternalData = EditorGUILayout.FloatField(
                    m_inputPorts[2].Name,
                    m_inputPorts[2].FloatInternalData
                );
        ...
        )
        {
            if (dataCollector.IsTemplate)
            {
                if (!dataCollector.IsSRP)
                {
                    ...
                    string worldViewDir = dataCollector.TemplateDataCollectorInstance.GetViewDir(
                        useMasterNodeCategory: false,
                        customCategory: MasterNodePortCategory.Fragment
                    );
                    ...
                    if (m_inputPorts[0].IsConnected)
                    {
                        if (m_normalSpace == ViewSpace.Tangent)
                            ...
                            worldNormal = m_inputPorts[0]
                                .GeneratePortInstructions(ref dataCollector);
                    }
                    else
                    {
                        worldNormal = dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                            PrecisionType.Float,
                            false,
                    ...
                    string tempsmoothness = m_inputPorts[1]
                        .GeneratePortInstructions(ref dataCollector);
                    string tempocclusion = m_inputPorts[2]
                        .GeneratePortInstructions(ref dataCollector);
                    ...
                    dataCollector.AddLocalVariable(
                        ...
                        "indirectSpecular" + OutputId,
                        "UnityGI_IndirectSpecular( data, "
                            + tempocclusion
                            + ", "
                            + worldNormal
                            + ", g"
                ...
                else
                {
                    if (dataCollector.CurrentSRPType == TemplateSRPType.URP)
                    {
                        ...
                        else
                        {
                            worldNormal =
                                dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                                    precisionType: PrecisionType.Float,
                                    useMasterNodeCategory: false,
                                    customCategory: MasterNodePortCategory.Fragment
                                );
                        }
                        ...
                        )
                        {
                            ...
                            string normalizedScreenUV =
                                dataCollector.TemplateDataCollectorInstance.GetScreenPosNormalized(
                                    CurrentPrecisionType,
                                    useMasterNodeCategory: false,
                                    customCategory: MasterNodePortCategory.Fragment
                                );
                            ...
                            dataCollector.AddLocalVariable(
                                UniqueId,
                                string.Format(
                                    "float3 indirectSpecular{0} = GlossyEnvironmentReflection( reflectVector{0}, {1}, 1.0 - {2}, {3}, {4}.xy );",
                                    OutputId,
                                    worldPos,
                                    tempsmoothness,
                                    tempocclusion,
                        ...
                        else
                        {
                            dataCollector.AddLocalVariable(
                                ...
                                "float3 indirectSpecular"
                                    + OutputId
                                    + " = GlossyEnvironmentReflection( reflectVector"
                                    + OutputId
                                    + ", 1.0 - "
                                    + tempsmoothness
                                    + ", "
                    ...
                    else if (dataCollector.CurrentSRPType == TemplateSRPType.HDRP)
                    {
                        UIUtils.ShowMessage(
                            UniqueId,
                            "Indirect Specular Light node currently not supported on HDRP"
                        );
            ...
            if (m_inputPorts[0].IsConnected)
            {
                ...
                dataCollector.AddLocalVariable(
                    UniqueId,
                    "float3 indirectNormal" + OutputId + " = " + normal + ";"
                );
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 13804)
            {
                ...
                UIUtils.ShowMessage(
                    UniqueId,
                    "Indirect Specular Light node: Smoothness port was previously being used as Roughness, please check if you are correctly using it and save to confirm."
                );
            }
