...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TransformPositionNode : ParentNode
    {
        ...
        private const string AseObjectToWorldPosFormat =
            "mul( unity_ObjectToWorld, float4( {0}, 1 ) ).xyz";
        private const string AseHDObjectToWorldPosFormat =
            "mul( GetObjectToWorldMatrix(), float4( {0}, 1 ) ).xyz";
        ...
        private const string AseObjectToViewPosFormat =
            "mul( UNITY_MATRIX_MV, float4( {0}, 1 ) ).xyz";
        private const string AseHDObjectToViewPosFormat =
            "TransformWorldToView( TransformObjectToWorld({0}) )";

        private const string AseWorldToObjectPosVarName = "worldToObj";
        private const string AseWorldToObjectPosFormat =
            "mul( unity_WorldToObject, float4( {0}, 1 ) ).xyz";
        private const string AseSRPWorldToObjectPosFormat =
            "mul( GetWorldToObjectMatrix(), float4( {0}, 1 ) ).xyz";

        private const string AseWorldToViewPosVarName = "worldToView";
        private const string AseWorldToViewPosFormat =
            "mul( UNITY_MATRIX_V, float4( {0}, 1 ) ).xyz";

        private const string AseViewToObjectPosVarName = "viewToObj";
        private const string AseViewToObjectPosFormat =
            "mul( unity_WorldToObject, mul( UNITY_MATRIX_I_V, float4( {0}, 1 ) ) ).xyz";
        private const string AseHDViewToObjectPosFormat =
            "mul( GetWorldToObjectMatrix(), mul( UNITY_MATRIX_I_V, float4( {0}, 1 ) ) ).xyz";

        private const string AseViewToWorldPosVarName = "viewToWorld";
        private const string AseViewToWorldPosFormat =
            "mul( UNITY_MATRIX_I_V, float4( {0}, 1.0 ) ).xyz";
        ...
        public override void DrawProperties()
        {
            ...
            {
                m_perspectiveDivide = EditorGUILayoutToggle(
                    PerpectiveDivideStr,
                    m_perspectiveDivide
                );
            }
            ...
            if (m_to == TransformSpaceTo.Clip || m_to == TransformSpaceTo.Screen)
            {
                if (m_perspectiveDivide || m_to == TransformSpaceTo.Screen)
                {
                    ...
                    result = string.Format(
                        AseClipToNDC,
                        varName,
                        (m_to == TransformSpaceTo.Screen) ? ".xyz" : string.Empty
                    );
                    varName += "NDC";
                }
                else if (m_to != TransformSpaceTo.Clip)
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 17500 && from.Equals("Clip"))
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Clip Space no longer supported on From field over Transform Position node"
                );
            }
            else
            {
