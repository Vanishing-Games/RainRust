...
namespace AmplifyShaderEditor
{
    ...
    {
        public const string FaceOnVertexWarning =
            "Face type nodes generates extra instructions when used on vertex ports since it needs to manually calculate the value";
        ...
        )
        {
            if (dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    m_nodeAttribs.Name + " node does not work on Tessellation port"
                );
            ...
            if (dataCollector.PortCategory == MasterNodePortCategory.Vertex)
            {
                if (dataCollector.TesselationActive)
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        m_nodeAttribs.Name + " node does not work properly on Tessellation ports"
                    );
                    return m_outputPorts[0].ErrorValue;
                }
                else
                {
                    ...
                    string faceVariable = GeneratorUtils.GenerateVertexFace(
                        ref dataCollector,
                        UniqueId
                    );
                    return faceVariable;
