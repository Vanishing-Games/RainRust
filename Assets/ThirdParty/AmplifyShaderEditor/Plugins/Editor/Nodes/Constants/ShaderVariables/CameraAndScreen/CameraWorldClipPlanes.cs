...
namespace AmplifyShaderEditor
{
    ...
    public sealed class CameraWorldClipPlanes : ShaderVariablesNode
    {
        ...
        {
            base.Draw(drawInfo);
            m_upperLeftWidget.DrawWidget<BuiltInShaderClipPlanesTypes>(
                ref m_selectedType,
                this,
                OnWidgetUpdate
            );
        ...
        private readonly Action<ParentNode> OnWidgetUpdate = (x) =>
        {
            x.SetAdditonalTitleText(
                string.Format(Constants.SubTitleTypeFormatStr, (x as CameraWorldClipPlanes).Type)
            );
        };
        ...
        public override void DrawProperties()
        {
            ...
            m_selectedType = (BuiltInShaderClipPlanesTypes)EditorGUILayoutEnumPopup(
                LabelStr,
                m_selectedType
            );
            if (EditorGUI.EndChangeCheck())
            {
                SetAdditonalTitleText(
                    string.Format(Constants.SubTitleTypeFormatStr, m_selectedType)
                );
                SetSaveIsDirty();
            }
        }
        ...
        )
        {
            ...
            if (dataCollector.TemplateDataCollectorInstance.IsHDRP)
            {
                ...
                {
                    case BuiltInShaderClipPlanesTypes.Left:
                        return ValueHDRPStr + "[0]";
                    case BuiltInShaderClipPlanesTypes.Right:
                        return ValueHDRPStr + "[1]";
                    case BuiltInShaderClipPlanesTypes.Bottom:
                        return ValueHDRPStr + "[3]";
                    case BuiltInShaderClipPlanesTypes.Top:
                        return ValueHDRPStr + "[2]";
                    case BuiltInShaderClipPlanesTypes.Near:
                        return ValueHDRPStr + "[4]";
                    case BuiltInShaderClipPlanesTypes.Far:
                        return ValueHDRPStr + "[5]";
                }
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_selectedType = (BuiltInShaderClipPlanesTypes)
                Enum.Parse(typeof(BuiltInShaderClipPlanesTypes), GetCurrentParam(ref nodeParams));
            SetAdditonalTitleText(string.Format(Constants.SubTitleTypeFormatStr, m_selectedType));
        }
