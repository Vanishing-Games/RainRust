    ...
    public class InstanceIdNode : ParentNode
    {
        ...
        )
        {
            if (dataCollector.PortCategory == MasterNodePortCategory.Tessellation)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    m_nodeAttribs.Name + " does not work on Tessellation port"
                );
            ...
            if (m_procedural)
            {
                if (
                    dataCollector.IsSRP
                    && (
                        dataCollector.CurrentPassName.Contains("Forward")
                        || dataCollector.CurrentPassName.Contains("GBuffer")
                    )
                )
                {
                    dataCollector.AddToPragmas(
                        UniqueId,
                        "instancing_options renderinglayer procedural:ASEProceduralSetup"
                    );
                }
                else
                {
                    dataCollector.AddToPragmas(
                        UniqueId,
                        "instancing_options procedural:ASEProceduralSetup"
                    );
                }
            ...
            else
            {
                ...
                if (dataCollector.IsFragmentCategory)
                {
                    GenerateValueInVertex(
                        ...
                        Constants.VertexShaderInputStr + "." + name,
                        name,
                        true
                    );
