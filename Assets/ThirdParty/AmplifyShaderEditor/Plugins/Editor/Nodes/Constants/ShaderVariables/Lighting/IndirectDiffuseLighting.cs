...
namespace AmplifyShaderEditor
{
    ...
    [NodeAttributes(
        ...
        "Lighting",
        "Indirect Lighting",
        NodeAvailabilityFlags = (int)(
            NodeAvailability.CustomLighting | NodeAvailability.TemplateShader
        )
    )]
    public sealed class IndirectDiffuseLighting : ParentNode
    {
        ...
        private readonly string IndirectDiffuseHeaderURP15 =
            "ASEIndirectDiffuse( {0}, {1}, {2}, {3} )";
        ...
        private readonly string IndirectDiffuseHeaderURP17 =
            "ASEIndirectDiffuse( {0}, {1}, {2}, {3} )";
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_errorMessageTooltip =
                "This node only returns correct information using a custom light model, otherwise returns 0";
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (
                (
                    ContainerGraph.CurrentStandardSurface != null
                    && ContainerGraph.CurrentStandardSurface.CurrentLightingModel
        ...
        )
        {
            ...
            if (dataCollector.IsTemplate && dataCollector.IsFragmentCategory)
            {
                if (!dataCollector.IsSRP)
                {
                    ...
                    if (
                        dataCollector.TemplateDataCollectorInstance.HasInfo(
                            TemplateInfoOnSematics.TEXTURE_COORDINATES1,
                            false,
                            MasterNodePortCategory.Vertex
                        )
                    )
                        texcoord1 = dataCollector
                            .TemplateDataCollectorInstance.GetInfo(
                                TemplateInfoOnSematics.TEXTURE_COORDINATES1,
                                false,
                                MasterNodePortCategory.Vertex
                            )
                        ...
                        texcoord1 =
                            dataCollector.TemplateDataCollectorInstance.RegisterInfoOnSemantic(
                                MasterNodePortCategory.Vertex,
                                TemplateInfoOnSematics.TEXTURE_COORDINATES1,
                                TemplateSemantics.TEXCOORD1,
                                "texcoord1",
                    ...
                    string vOutName = dataCollector
                        .TemplateDataCollectorInstance
                        .CurrentTemplateData
                        .VertexFunctionData
                        .OutVarName;
                    string fInName = dataCollector
                        .TemplateDataCollectorInstance
                        .CurrentTemplateData
                        .FragmentFunctionData
                        .InVarName;
                    TemplateVertexData data =
                        dataCollector.TemplateDataCollectorInstance.RequestNewInterpolator(
                            WirePortDataType.FLOAT4,
                            false,
                            "ase_lmap"
                        );
                    ...
                    dataCollector.AddToVertexLocalVariables(
                        ...
                        vOutName
                            ...
                            + ".zw = "
                            + texcoord2
                            + ".xy * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;"
                    );
                    ...
                    dataCollector.AddToVertexLocalVariables(
                        ...
                        vOutName
                            ...
                            + ".xy = "
                            + texcoord1
                            + ".xy * unity_LightmapST.xy + unity_LightmapST.zw;"
                    );
                    dataCollector.AddToVertexLocalVariables(UniqueId, "#endif //stalm");

                    TemplateVertexData shdata =
                        dataCollector.TemplateDataCollectorInstance.RequestNewInterpolator(
                    ...
                    string worldNormal = dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                        PrecisionType.Float,
                        false,
                        MasterNodePortCategory.Vertex
                    );
                    ...
                    dataCollector.AddToVertexLocalVariables(
                        UniqueId,
                        outSH + " = ShadeSHPerVertex (" + worldNormal + ", " + outSH + ");"
                    );
                    ...
                    if (m_inputPorts[0].IsConnected)
                    {
                        if (m_normalSpace == ViewSpace.Tangent)
                            fragWorldNormal =
                                dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                                    UniqueId,
                            ...
                            fragWorldNormal = m_inputPorts[0]
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "UNITY_INITIALIZE_OUTPUT( UnityGIInput, data" + OutputId + " );"
                    );

                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "#if defined(LIGHTMAP_ON) || defined(DYNAMICLIGHTMAP_ON) //dylm" + OutputId
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "data" + OutputId + ".lightmapUV = " + fInName + "." + varName + ";"
                    );
                    dataCollector.AddLocalVariable(UniqueId, "#endif //dylm" + OutputId);

                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "#if UNITY_SHOULD_SAMPLE_SH //fsh" + OutputId
                    );
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        "data" + OutputId + ".ambient = " + fInName + "." + shVarName + ";"
                    );
                ...
                else
                {
                    if (dataCollector.CurrentSRPType == TemplateSRPType.URP)
                    {
                        ...
                        if (
                            !dataCollector.TemplateDataCollectorInstance.HasRawInterpolatorOfName(
                        ...
                        )
                        {
                            ...
                            dataCollector.AddToVertexLocalVariables(
                                ...
                                "OUTPUT_LIGHTMAP_UV( "
                                    ...
                                    + ", unity_LightmapST, "
                                    + vOutName
                                    + ".lightmapUVOrVertexSH.xy );",
                                true
                            );

                            if (
                                ASEPackageManagerHelper.PackageSRPVersion
                                >= (int)ASESRPBaseline.ASE_SRP_15
                            )
                            {
                                string worldPos =
                                    dataCollector.TemplateDataCollectorInstance.GetWorldPos(
                                        false,
                                        MasterNodePortCategory.Vertex
                                    );
                                dataCollector.AddToVertexLocalVariables(
                            ...
                            dataCollector.AddToPragmas(
                                UniqueId,
                                "multi_compile _ DIRLIGHTMAP_COMBINED"
                            );
                        ...
                        string result;
                        if (
                            ASEPackageManagerHelper.CurrentSRPVersion
                            >= (int)ASESRPBaseline.ASE_SRP_15
                        )
                        {
                            string positionWS =
                                dataCollector.TemplateDataCollectorInstance.GetWorldPos();
                            string viewDirWS =
                                dataCollector.TemplateDataCollectorInstance.GetViewDir();
                            ...
                            )
                            {
                                dataCollector.AddFunction(
                                    IndirectDiffuseBodyURP17[0],
                                    IndirectDiffuseBodyURP17,
                                    false
                                );
                                result = string.Format(
                                    IndirectDiffuseHeaderURP17,
                                    fInName,
                                    fragWorldNormal,
                                    positionWS,
                                    viewDirWS
                            ...
                            {
                                dataCollector.AddFunction(
                                    IndirectDiffuseBodyURP15[0],
                                    IndirectDiffuseBodyURP15,
                                    false
                                );
                                result = string.Format(
                                    IndirectDiffuseHeaderURP15,
                                    fInName,
                                    fragWorldNormal,
                                    positionWS,
                                    viewDirWS
                        ...
                        {
                            dataCollector.AddFunction(
                                IndirectDiffuseBodyURP12[0],
                                IndirectDiffuseBodyURP12,
                                false
                            );
                            result = string.Format(
                                IndirectDiffuseHeaderURP12,
                                fInName,
                                fragWorldNormal
                            );
                        }
                        ...
                        {
                            dataCollector.AddFunction(
                                IndirectDiffuseBodyURP10[0],
                                IndirectDiffuseBodyURP10,
                                false
                            );
                            result = string.Format(
                                IndirectDiffuseHeaderURP10,
                                fInName,
                                fragWorldNormal
                            );
                        }
                        ...
                        string mainLight =
                            dataCollector.TemplateDataCollectorInstance.GetURPMainLight(UniqueId);
                        dataCollector.AddLocalVariable(
                            UniqueId,
                            string.Format(
                                "MixRealtimeAndBakedGI( {0}, {1}, {2}, half4( 0, 0, 0, 0 ) );",
                                mainLight,
                                fragWorldNormal,
                                finalValue
                            )
                    ...
                    else if (dataCollector.CurrentSRPType == TemplateSRPType.HDRP)
                    {
                        ...
                        )
                        {
                            ...
                            string positionSS = string.Format(
                                "( uint2 )( {0}.xy / {0}.w * _ScreenSize.xy )",
                                screenPos
                            );
            ...
            if (
                dataCollector.GenType == PortGenType.NonCustomLighting
                || dataCollector.CurrentCanvasMode != NodeAvailability.CustomLighting
            )
                return "float3(0,0,0)";
            ...
            else
            {
                ...
                normal = GeneratorUtils.GenerateWorldNormal(
                    ref dataCollector,
                    UniqueId,
                    m_normalize
                );
            ...
            else
            {
                ...
                dataCollector.AddLocalVariable(
                    UniqueId,
                    "gi" + OutputId + " = UnityGI_Base( data, 1, diffNorm" + OutputId + " );"
                );
                dataCollector.AddLocalVariable(
                    UniqueId,
                    CurrentPrecisionType,
                    WirePortDataType.FLOAT3,
                    "indirectDiffuse" + OutputId,
                    "gi" + OutputId + ".indirect.diffuse + diffNorm" + OutputId + " * 0.0001"
                );
            }
