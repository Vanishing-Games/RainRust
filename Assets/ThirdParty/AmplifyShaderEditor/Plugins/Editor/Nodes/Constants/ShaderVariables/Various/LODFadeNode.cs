...
namespace AmplifyShaderEditor
{
    ...
    public sealed class LODFadeNode : ConstVecShaderVariable
    {
        ...
        private const string LegacyVarValue =
            "(( unity_LODFade.x < 0 ) ? ( 1 + unity_LODFade.x ) : ( unity_LODFade.x ))";

        private const string LegacyVarLabel = "Legacy Behavior";
        private const string LegacyVarInfo =
            "Prior to Unity 2019 values given by unity_LODFade.x/Fade[0...1] port were always positive and complemented each other between LOD Groups.\n"
