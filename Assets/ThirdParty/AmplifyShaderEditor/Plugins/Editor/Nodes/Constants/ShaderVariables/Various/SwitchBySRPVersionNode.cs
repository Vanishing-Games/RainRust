        ...
        private void UpdateConnections()
        {
            ...
            m_outputPorts[0]
                .ChangeTypeWithRestrictions(
                    activePort.DataType,
                    FunctionInput.PortCreateRestriction(activePort.DataType)
                );

            string srpTypeName =
                (ContainerGraph != null)
                    ? SRPTypeNames[(int)ContainerGraph.CurrentSRPType]
                    : "Unknown";
            SetAdditonalTitleText(
                string.Format(
                    Constants.SubTitleCurrentFormatStr,
                    srpTypeName + ", " + SRPVersionList[activePortIndex].Item1
                )
            );
        }
