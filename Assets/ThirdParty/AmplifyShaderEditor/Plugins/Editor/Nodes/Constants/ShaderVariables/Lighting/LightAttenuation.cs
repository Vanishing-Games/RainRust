...
namespace AmplifyShaderEditor
{
    [System.Serializable]
    [NodeAttributes(
        "Main Light Attenuation",
        "Lighting",
        "Attenuation/shadow of main Directional light.",
    ...
    {
        static readonly string SurfaceError =
            "This node only returns correct information using a custom light model, otherwise returns 1";
        static readonly string TemplateError =
        ...
        )
        {
            if (dataCollector.IsTemplate)
            {
                if (!dataCollector.IsSRP)
                {
                    ...
                    if (
                        dataCollector.TemplateDataCollectorInstance.ContainsSpecialLocalFragVar(
                ...
                else
                {
                    if (dataCollector.CurrentSRPType == TemplateSRPType.URP)
                    {
                        ...
                        if (ASEPackageManagerHelper.CurrentSRPVersion >= 140009)
                        {
                            if (isForward || isGBuffer)
                            {
                                ...
                                pragmas.Add(
                                    "multi_compile_fragment _ _SHADOWS_SOFT _SHADOWS_SOFT_LOW _SHADOWS_SOFT_MEDIUM _SHADOWS_SOFT_HIGH"
                                );
                            }
                        ...
                        )
                        {
                            ...
                            ASEPackageManagerHelper.CurrentURPBaseline >= ASESRPBaseline.ASE_SRP_11
                        )
                        {
                            if (isForward || isGBuffer)
                            {
                        ...
                        dataCollector.AddLocalVariable(
                            UniqueId,
                            string.Format(LightweightFragmentInstructions[1], mainLight)
                        );
                        return ASEAttenVarName;
                    }
                    else
                    {
                        UIUtils.ShowMessage(
                            UniqueId,
                            "Light Attenuation node currently not supported on HDRP"
                        );
                        return "1";
                    }
                }
            ...
            )
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Light Attenuation node currently not supported on non-custom lighting surface shaders"
                );
                return "1";
            }
