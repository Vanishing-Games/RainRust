...
namespace AmplifyShaderEditor
{
    ...
    public sealed class TransformDirectionNode : ParentNode
    {
        ...
        private const string AseObjectToWorldDirFormat =
            "mul( unity_ObjectToWorld, float4( {0}, 0.0 ) ).xyz";
        private const string AseSRPObjectToWorldDirFormat =
            "mul( GetObjectToWorldMatrix(), float4( {0}, 0.0 ) ).xyz";

        private const string AseObjectToViewDirVarName = "objToViewDir";
        private const string AseObjectToViewDirFormat =
            "mul( UNITY_MATRIX_IT_MV, float4( {0}, 0.0 ) ).xyz";
        private const string AseHDObjectToViewDirFormat =
            "TransformWorldToViewDir( TransformObjectToWorldDir( {0} ) )";

        private const string AseWorldToObjectDirVarName = "worldToObjDir";
        private const string AseWorldToObjectDirFormat =
            "mul( unity_WorldToObject, float4( {0}, 0.0 ) ).xyz";
        private const string AseSRPWorldToObjectDirFormat =
            "mul( GetWorldToObjectMatrix(), float4( {0}, 0.0 ) ).xyz";

        private const string AseWorldToViewDirVarName = "worldToViewDir";
        private const string AseWorldToViewDirFormat =
            "mul( UNITY_MATRIX_V, float4( {0}, 0.0 ) ).xyz";

        private const string AseViewToObjectDirVarName = "viewToObjDir";
        private const string AseViewToObjectDirFormat =
            "mul( UNITY_MATRIX_T_MV, float4( {0}, 0.0 ) ).xyz";

        private const string AseViewToWorldDirVarName = "viewToWorldDir";
        private const string AseViewToWorldDirFormat =
            "mul( UNITY_MATRIX_I_V, float4( {0}, 0.0 ) ).xyz";

        ///////////////////////////////////////////////////////////
        private const string AseObjectToClipDirVarName = "objectToClipDir";
        private const string AseObjectToClipDirFormat =
            "mul( UNITY_MATRIX_VP, mul( unity_ObjectToWorld, float4( {0}, 0.0 ) ) ).xyz";
        private const string AseSRPObjectToClipDirFormat =
            "TransformWorldToHClipDir( TransformObjectToWorldDir( {0} ) )";

        private const string AseWorldToClipDirVarName = "worldToClipDir";
        private const string AseWorldToClipDirFormat =
            "mul( UNITY_MATRIX_VP, float4( {0}, 0.0 ) ).xyz";
        ...
        private const string AseViewToClipDirFormat =
            "mul( UNITY_MATRIX_P, float4( {0}, 0.0 ) ).xyz";
        private const string AseSRPViewToClipDirFormat =
            "mul( GetViewToHClipMatrix(), float4( {0}, 1.0 ) ).xyz";

        //
        private const string AseClipToObjectDirVarName = "clipToObjectDir";

        private const string AseClipToObjectDirFormat =
            "mul( UNITY_MATRIX_IT_MV, mul( unity_CameraInvProjection, float4( {0}, 0.0 ) ) ).xyz";
        private const string AseClipToWorldDirFormat =
            "mul( UNITY_MATRIX_I_V, mul( unity_CameraInvProjection, float4( {0}, 0.0 ) ) ).xyz";
        private const string AseClipToViewDirFormat =
            " mul( unity_CameraInvProjection, float4( {0}, 0.0 ) ).xyz";
        private const string AseHDClipToObjectDirFormat =
            "mul( UNITY_MATRIX_I_M, mul( UNITY_MATRIX_I_VP, float4( {0}, 0.0) ) ).xyz";

        private const string AseClipToWorldDirVarName = "clipToWorldDir";
        private const string AseHDClipToWorldDirFormat =
            "mul( UNITY_MATRIX_I_VP, float4( {0}, 0.0 ) ).xyz";

        private const string AseClipToViewDirVarName = "clipToViewDir";
        private const string AseHDClipToViewDirFormat =
            " mul( UNITY_MATRIX_I_P,float4( {0}, 0.0 ) ).xyz";
        ...
        private readonly string[] m_spaceOptionsFrom = { "Object", "World", "View", "Tangent" };
        ...
        public override void DrawProperties()
        {
            ...
            m_from = (TransformSpaceFrom)EditorGUILayoutPopup(
                FromStr,
                (int)m_from,
                m_spaceOptionsFrom
            );
            ...
            {
                m_inverseTangentType = (InverseTangentType)EditorGUILayoutEnumPopup(
                    InverseTBNStr,
                    m_inverseTangentType
                );
            }
        ...
        )
        {
            ...
            if (
                (int)m_from != (int)m_to
                && (m_from == TransformSpaceFrom.Tangent || m_to == TransformSpaceTo.Tangent)
            )
                dataCollector.DirtyNormal = true;
        }

        void CalculateTransform(
            TransformSpaceFrom from,
            TransformSpaceTo to,
            ref MasterNodeDataCollector dataCollector,
            ref string varName,
            ref string result
                            ...
                            case TransformSpaceTo.World:
                                {
                                    if (
                                        ...
                                        && dataCollector
                                            ...
                                            .CurrentSRPType != TemplateSRPType.BiRP
                                    )
                                        result = string.Format(
                                            AseSRPObjectToWorldDirFormat,
                                            result
                                        );
                                    else
                            ...
                            case TransformSpaceTo.View:
                                {
                                    if (
                                        ...
                                        && dataCollector
                                            .TemplateDataCollectorInstance
                                            .CurrentSRPType == TemplateSRPType.HDRP
                                    )
        ...
        )
        {
                ...
                case TransformSpaceFrom.Object:
                    {
                        switch (m_to)
                        {
                            ...
                            case TransformSpaceTo.Tangent:
                                {
                                    GeneratorUtils.GenerateWorldToTangentMatrix(
                                        ref dataCollector,
                                        UniqueId,
                ...
                case TransformSpaceFrom.Tangent:
                    {
                        ...
                        if (m_inverseTangentType == InverseTangentType.Fast)
                            ...
                            matrixVal = GeneratorUtils.GenerateTangentToWorldMatrixPrecise(
                                ref dataCollector,
                                UniqueId,
                                CurrentPrecisionType
                            );
            ...
            if (m_normalize)
            {
                result = GeneratorUtils.NormalizeValue(
                    ref dataCollector,
                    m_safeNormalize,
                    m_inputPorts[0].DataType,
                    result
                );
            }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 17500 && from.Equals("Clip"))
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Clip Space no longer supported on From field over Transform Direction node"
                );
            }
            else
            {
            ...
            m_to = (TransformSpaceTo)
                Enum.Parse(typeof(TransformSpaceTo), GetCurrentParam(ref nodeParams));
            m_normalize = Convert.ToBoolean(GetCurrentParam(ref nodeParams));
            if (UIUtils.CurrentShaderVersion() > 15800)
            {
                m_inverseTangentType = (InverseTangentType)
                    Enum.Parse(typeof(InverseTangentType), GetCurrentParam(ref nodeParams));
            }
