...
namespace AmplifyShaderEditor
{
    ...
    public sealed class ColorNode : PropertyNode
    {
        ...
        public ColorNode(int uniqueId, float x, float y, float width, float height)
        ...
        {
            ChangeOutputProperties(
                0,
                m_useAlpha ? "RGBA" : "RGB",
                m_useAlpha ? WirePortDataType.COLOR : WirePortDataType.FLOAT3,
                false
            );
        ...
        public override void DrawSubProperties()
        {
            ...
            m_defaultValue = EditorGUILayoutColorField(
                Constants.DefaultValueLabelContent,
                m_defaultValue,
                false,
                m_useAlpha,
                m_isHDR,
            ...
            {
                m_autoGammaToLinearConversion = EditorGUILayoutToggle(
                    AutoGammaToLinearStr,
        ...
        public override void DrawMaterialProperties()
        {
            if (m_materialMode)
                EditorGUI.BeginChangeCheck();
            m_materialValue = EditorGUILayoutColorField(
                Constants.MaterialValueLabelContent,
                m_materialValue,
                false,
                m_useAlpha,
                m_isHDR
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            m_propertyDrawPos.height = Mathf.Min(
                m_remainingBox.height,
                ((maxWidth + (m_useAlpha ? 20 : 0)) * drawInfo.InvertedZoom)
            );

            m_frameDrawPos0 = new Rect(
                m_propertyDrawPos.x,
                m_propertyDrawPos.y - 1,
                m_propertyDrawPos.width + 1,
                m_propertyDrawPos.height + 1
            );
            m_frameDrawPos1 = new Rect(
                m_propertyDrawPos.x,
                m_propertyDrawPos.y + m_propertyDrawPos.height - 1,
                m_propertyDrawPos.width,
                1
            );
        }
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_isEditingFields && m_currentParameterType != PropertyType.Global)
            {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    m_materialValue = EditorGUIColorField(
                        m_propertyDrawPos,
                        m_dummyContent,
                        m_materialValue,
                        false,
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    m_defaultValue = EditorGUIColorField(
                        m_propertyDrawPos,
                        m_dummyContent,
                        m_defaultValue,
                        false,
            ...
            {
                var value =
                    (m_materialMode && m_currentParameterType != PropertyType.Constant)
                        ? m_materialValue
                        : m_defaultValue;
            ...
            dataCollector.AddLocalVariable(
                ...
                CreateLocalVarDec(
                    color.r
                        ...
                        + ","
                        + color.b
                        + (m_useAlpha ? "," + color.a : string.Empty)
                )
            );
        ...
        void CheckIfHDR()
        {
            ...
            {
                if (
                    m_selectedAttribs[i] == 1 /*HDR Property ID*/
                )
                {
                    m_isHDR = true;
                    break;
            ...
            if (hdrBuffer && !m_isHDR)
            {
                ...
                {
                    float defaultColorLength = Mathf.Sqrt(
                        m_defaultValue.r * m_defaultValue.r
                ...
                {
                    float materialColorLength = Mathf.Sqrt(
                        m_materialValue.r * m_materialValue.r
        ...
        public override string GetPropertyValStr()
        {
            if (m_materialMode && m_currentParameterType != PropertyType.Constant)
            {
                return m_materialValue.r.ToString(Constants.PropertyVectorFormatLabel)
                    + IOUtils.VECTOR_SEPARATOR
                    + m_materialValue.g.ToString(Constants.PropertyVectorFormatLabel)
                    + IOUtils.VECTOR_SEPARATOR
            ...
            {
                return m_defaultValue.r.ToString(Constants.PropertyVectorFormatLabel)
                    + IOUtils.VECTOR_SEPARATOR
                    + m_defaultValue.g.ToString(Constants.PropertyVectorFormatLabel)
                    + IOUtils.VECTOR_SEPARATOR
        ...
        private Color MaterialValue
        {
            set
            {
                ...
                {
                    float materialColorLength = Mathf.Sqrt(
                        value.r * value.r + value.g * value.g + value.b * value.b
                    );
