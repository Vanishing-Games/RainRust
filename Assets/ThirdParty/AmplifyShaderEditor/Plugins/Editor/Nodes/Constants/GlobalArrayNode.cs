...
namespace AmplifyShaderEditor
{
    ...
    public sealed class GlobalArrayNode : ParentNode
    {
        ...
        private readonly string[] AvailableTypesLabel =
        {
            "Float",
            "Color",
            "Vector4",
        ...
        private readonly WirePortDataType[] AvailableTypesValues =
        {
            ...
            else if (m_referenceNodeId > -1)
            {
                ...
                if (m_referenceNode != null)
                {
                    SetAdditonalTitleText(
                        string.Format(Constants.SubTitleValueFormatStr, m_referenceNode.DataToArray)
                    );
        ...
        public string GenerateInstancedShaderForOutput(
            int outputId,
        ...
        )
        {
            ...
            if (m_referenceNode != null)
            {
                ...
                {
                    InputPort indexYPort = GetInputPortByUniqueId(2);
                    string arrayIndexX = indexXPort.IsConnected
                        ? indexXPort.GeneratePortInstructions(ref dataCollector)
                        : m_indexX.ToString();
                    string arrayIndexY = indexYPort.IsConnected
                        ? indexYPort.GeneratePortInstructions(ref dataCollector)
                        : m_indexY.ToString();
        ...
        )
        {
            ...
            if (m_referenceType == TexReferenceType.Instance)
                return GenerateInstancedShaderForOutput(
                    outputId,
                    ref dataCollector,
                    ignoreLocalvar
                );

            string dataType = UIUtils.PrecisionWirePortToCgType(
                CurrentPrecisionType,
                AvailableTypesValues[m_type]
            );
            ...
            if (m_isJagged)
            {
                ...
                string arrayIndexX = indexXPort.IsConnected
                    ? indexXPort.GeneratePortInstructions(ref dataCollector)
                    : m_indexX.ToString();
                string arrayLengthX = arrayLengthXPort.IsConnected
                    ? arrayLengthXPort.GeneratePortInstructions(ref dataCollector)
                    : ArrayLengthX.ToString();

                string arrayIndexY = indexYPort.IsConnected
                    ? indexYPort.GeneratePortInstructions(ref dataCollector)
                    : m_indexY.ToString();
                string arrayLengthY = arrayLengthYPort.IsConnected
                    ? arrayLengthYPort.GeneratePortInstructions(ref dataCollector)
                    : ArrayLengthY.ToString();

                dataCollector.AddToUniforms(
                    UniqueId,
                    dataType,
                    string.Format(JaggedArrayFormatStr, m_name, arrayLengthX, arrayLengthY)
                );
                if (m_autoRangeCheck)
                {
                    arrayIndexX = string.Format(
                        "clamp({0},0,({1} - 1))",
                        arrayIndexX,
                        arrayLengthX
                    );
                    arrayIndexY = string.Format(
                        "clamp({0},0,({1} - 1))",
                        arrayIndexY,
                        arrayLengthY
                    );
            ...
            {
                string arrayIndex = indexXPort.IsConnected
                    ? indexXPort.GeneratePortInstructions(ref dataCollector)
                    : m_indexX.ToString();
                string arrayLength = arrayLengthXPort.IsConnected
                    ? arrayLengthXPort.GeneratePortInstructions(ref dataCollector)
                    : ArrayLengthX.ToString();
                ...
                dataCollector.AddToUniforms(
                    UniqueId,
                    dataType,
                    string.Format(ArrayFormatStr, m_name, arrayLength)
                );
            ...
            )
            {
                ...
                else
                {
                    dataCollector.AddToUniforms(
                        UniqueId,
                        dataType,
                        string.Format(ArrayFormatStr, m_name, ArrayLengthX)
                    );
                }
            }
        }
        ...
        public int ArrayLengthX
        {
            get
            {
                return (m_referenceNode != null) ? m_referenceNode.ArrayLengthX : m_arrayLengthX;
            }
        }
        public int ArrayLengthY
        {
            get
            {
                return (m_referenceNode != null) ? m_referenceNode.ArrayLengthY : m_arrayLengthY;
            }
        }
