...
namespace AmplifyShaderEditor
{
    ...
    public class ParentNode : UndoParentNode, ISerializationCallbackReceiver
    {
        ...
        public void RefreshUIHeaderColor()
        {
            ...
            if (functionNode != null && functionNode.Function != null)
            {
                ...
                else if (functionNode.Function.Header == AmplifyShaderFunction.HeaderStyle.Category)
                {
                    m_headerColor = UIUtils.GetColorFromCategory(
                        functionNode.Function.CustomNodeCategory
                    );
                }
                else
                {
            ...
            else if (m_nodeAttribs != null)
            {
                ...
                else
                {
                    ...
                    {
                        m_headerColor = UIUtils.AddColorCategory(
                            m_nodeAttribs.Category,
                            m_nodeAttribs.CustomCategoryColor
                        );
                    }
                }
            }
        ...
        protected virtual void CommonInit(int uniqueId)
        {
            ...
            m_nodeAttribs =
                info.GetCustomAttributes(typeof(NodeAttributes), true).FirstOrDefault()
                as NodeAttributes;
        ...
        public virtual void AfterCommonInit()
        {
            ...
            {
                m_paddingTitleRight +=
                    Constants.PreviewExpanderWidth + Constants.IconsLeftRightMargin;
                m_paddingTitleLeft =
                    Constants.PreviewExpanderWidth + Constants.IconsLeftRightMargin;
            }
        }
        ...
        public virtual bool UpdateShaderDefaults(
            ref Shader shader,
            ref TextureDefaultsDataColector defaultCol
        )
        {
            return false;
        }
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_inputPorts[i].IsConnected)
                {
                    ...
                    if (node != null)
                    {
                        ...
                        {
                            if (
                                outputPort.ExternalReferences[j].NodeId == m_uniqueId
        ...
        public void RemoveOutputPort(string name)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_outputPorts[i].Name.Equals(name))
                {
                    ...
                    {
                        m_containerGraph.DeleteConnection(
                            false,
                            m_uniqueId,
                            m_outputPorts[i].PortId,
                            false,
                            true
                        );
        ...
        )
        {
            InputPort port = new InputPort(
                m_uniqueId,
                (uniquePortId < 0 ? m_inputPorts.Count : uniquePortId),
                type,
                name,
                typeLocked,
                (orderId >= 0 ? orderId : m_inputPorts.Count),
                category
            );
        ...
        public InputPort AddInputPortAt(
            int idx,
            WirePortDataType type,
            bool typeLocked,
            string name,
        ...
        public void AddOutputPort(WirePortDataType type, string name, int uniquePortId = -1)
        {
            m_outputPorts.Add(
                new OutputPort(
                    this,
                    m_uniqueId,
                    (uniquePortId < 0 ? m_outputPorts.Count : uniquePortId),
                    type,
                    name
                )
            );
            m_outputPortsDict.Add(
                m_outputPorts[m_outputPorts.Count - 1].PortId,
                m_outputPorts[m_outputPorts.Count - 1]
            );
            SetSaveIsDirty();
            m_sizeIsDirty = true;
        }

        public void AddOutputPortAt(
            int idx,
            WirePortDataType type,
            string name,
            int uniquePortId = -1
        ...
        public void AddOutputColorPorts(
            string name = "RGBA",
            bool addAlpha = true,
            bool addRGB = false
        )
        {
            ...
            // RGBA port
            int startOrderId =
                (m_outputPorts.Count > 0) ? m_outputPorts[m_outputPorts.Count - 1].OrderId : 0;
            ...
            m_outputPorts.Add(
                new OutputPort(
                    ...
                    m_uniqueId,
                    m_outputPorts.Count,
                    addAlpha ? WirePortDataType.COLOR : WirePortDataType.FLOAT3,
                    name
                )
            );
            m_outputPortsDict.Add(
            ...
            // Red port
            m_outputPorts.Add(
                new OutputPort(this, m_uniqueId, m_outputPorts.Count, WirePortDataType.FLOAT, "R")
            );
            m_outputPortsDict.Add(
                m_outputPorts[m_outputPorts.Count - 1].PortId,
                m_outputPorts[m_outputPorts.Count - 1]
            ...
            // Green port
            m_outputPorts.Add(
                new OutputPort(this, m_uniqueId, m_outputPorts.Count, WirePortDataType.FLOAT, "G")
            );
            m_outputPortsDict.Add(
                m_outputPorts[m_outputPorts.Count - 1].PortId,
                m_outputPorts[m_outputPorts.Count - 1]
            ...
            // Blue port
            m_outputPorts.Add(
                new OutputPort(this, m_uniqueId, m_outputPorts.Count, WirePortDataType.FLOAT, "B")
            );
            m_outputPortsDict.Add(
                m_outputPorts[m_outputPorts.Count - 1].PortId,
                m_outputPorts[m_outputPorts.Count - 1]
        ...
        public void ChangeOutputProperties(
            int outputID,
            string newName,
            WirePortDataType newType,
            bool invalidateConnections = true
        )
        {
        ...
        public InputPort GetFirstInputPortOfType(
            WirePortDataType dataType,
            bool countObjectTypeAsValid
        )
        {
            int count = m_inputPorts.Count;
            for (int i = 0; i < count; i++)
            {
                if (
                    (m_inputPorts[i].CheckValidType(dataType))
                    || (
                        countObjectTypeAsValid
                        && m_inputPorts[i].DataType == WirePortDataType.OBJECT
                    )
                )
                    return m_inputPorts[i];
            }
        ...
        public OutputPort GetFirstOutputPortOfType(WirePortDataType dataType, bool checkForCasts)
        {
            ...
            {
                if (
                    (m_outputPorts[i].CheckValidType(dataType))
                    || (checkForCasts && UIUtils.CanCast(dataType, m_outputPorts[i].DataType))
                )
                    return m_outputPorts[i];
            }
            return null;
        ...
        protected virtual void ChangeSizeFinished()
        {
            m_firstPreviewDraw = true; /*MarkForPreviewUpdate();*/
        }
        ...
        protected void ChangeSize()
        {
            ...
            for (int i = 0; i < inputSize; i++)
            {
                if (m_inputPorts[i].Visible)
                {
                    if (m_inputPorts[i].DirtyLabelSize || m_inputPorts[i].LabelSize == Vector2.zero)
                    {
                        ...
                        m_inputPorts[i].UnscaledLabelSize = UIUtils.UnZoomedInputPortStyle.CalcSize(
                            m_sizeContentAux
                        );
                    }
            ...
            for (int i = 0; i < outputSize; i++)
            {
                ...
                {
                    if (
                        m_outputPorts[i].DirtyLabelSize
                        || m_outputPorts[i].LabelSize == Vector2.zero
                    )
                    {
                        m_outputPorts[i].DirtyLabelSize = false;
                        m_sizeContentAux.text = m_outputPorts[i].Name;
                        m_outputPorts[i].UnscaledLabelSize =
                            UIUtils.UnZoomedOutputPortPortStyle.CalcSize(m_sizeContentAux);
                    }
            ...
            float headerWidth = Mathf.Max(
                UIUtils.UnZoomedNodeTitleStyle.CalcSize(m_content).x
                    + m_paddingTitleLeft
                    + m_paddingTitleRight,
                UIUtils.UnZoomedPropertyValuesTitleStyle.CalcSize(m_additionalContent).x
                    + m_paddingTitleLeft
                    + m_paddingTitleRight
            );
            m_position.width = Mathf.Max(
                headerWidth,
                Mathf.Max(MinInsideBoxWidth, m_insideSize.x) + inSize.x + outSize.x
            ); /* + Constants.NODE_HEADER_LEFTRIGHT_MARGIN * 2;*/
            ...
            m_position.height =
                Mathf.Max(inputCount, outputCount) * (m_fontHeight + Constants.INPUT_PORT_DELTA_Y); // + Constants.INPUT_PORT_DELTA_Y;
            m_position.height = Mathf.Max(
                m_position.height,
                Mathf.Max(MinInsideBoxHeight, m_insideSize.y)
            );
            m_position.height +=
                UIUtils.HeaderMaxHeight
                + /*m_extraHeaderHeight +*/
                Constants.INPUT_PORT_DELTA_Y; // + m_extraSize.y;
        ...
        public virtual void OnInputPortConnected(
            int portId,
            int otherNodeId,
            int otherPortId,
        ...
        public virtual void OnNodeChange()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (m_outputPorts[i].IsConnected)
                {
                    ...
                    {
                        ContainerGraph
                            .GetNode(m_outputPorts[i].ExternalReferences[f].NodeId)
                            .OnNodeChange();
                    }
                }
            }
        ...
        public virtual void ActivateNode(
            int signalGenNodeId,
            int signalGenPortId,
            System.Type signalGenNodeType
        )
        {
            if (m_selfPowered)
                return;

            ConnStatus = m_restrictions.GetRestiction(signalGenNodeType, signalGenPortId)
            ...
            if (m_activeConnections == 1)
            {
                ...
                for (int i = 0; i < m_inputPorts.Count; i++)
                {
                    if (m_inputPorts[i].IsConnected)
                    {
                        m_inputPorts[i]
                            .GetOutputNode()
                            .ActivateNode(signalGenNodeId, signalGenPortId, signalGenNodeType);
                    }
                }
            }
        ...
        {
            GetInputPortByUniqueId(deactivatedPort)
                .GetOutputNode()
                .DeactivateNode(deactivatedPort, false);
        }
            ...
            if (forceComplete || m_activeConnections <= 0)
            {
                ...
                for (int i = 0; i < m_inputPorts.Count; i++)
                {
                    if (m_inputPorts[i].IsConnected)
                    {
                        ...
                        if (node != null)
                            node.DeactivateNode(
                                deactivatedPort == -1 ? m_inputPorts[i].PortId : deactivatedPort,
                                false
                            );
                    }
                }
        ...
        protected void CalculatePositionAndVisibility(DrawInfo drawInfo)
        {
            ...
            m_globalPosition.x =
                drawInfo.InvertedZoom * (m_globalPosition.x + drawInfo.CameraOffset.x);
            m_globalPosition.y =
                drawInfo.InvertedZoom * (m_globalPosition.y + drawInfo.CameraOffset.y);
            m_globalPosition.width *= drawInfo.InvertedZoom;
            m_globalPosition.height *= drawInfo.InvertedZoom;

            m_isVisible =
                (m_globalPosition.x + m_globalPosition.width > 0)
        ...
        {
            if (withInherit)
                m_currentPrecisionType = (PrecisionType)EditorGUILayoutPopup(
                    PrecisionContent.text,
                    (int)m_currentPrecisionType,
                    PrecisionLabelsExtra
                );
            else
                m_currentPrecisionType = (PrecisionType)EditorGUILayoutPopup(
                    PrecisionContent.text,
            ...
            if (m_outputPorts.Count > 0)
            {
                switch (m_outputPorts[0].DataType)
                {
                    ...
                    case WirePortDataType.SAMPLER2DARRAY:
                        m_channelNumber = 4;
                        mask.Set(
                            m_previewChannels[0] ? 1 : 0,
                            m_previewChannels[1] ? 1 : 0,
                            m_previewChannels[2] ? 1 : 0,
                            m_previewChannels[3] ? 1 : 0
                        );
                        break;
                    default:
                        m_channelNumber = 3;
        ...
        {
            if (
                rect.Contains(drawInfo.MousePosition)
                && m_channelNumber > 1
                && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD2
            )
            {
                ...
                {
                    m_previewChannels[0] = GUI.Toggle(
                        buttonRect,
                        m_previewChannels[0],
                        string.Empty,
                        GUIStyle.none
                    ...
                    m_previewChannels[1] = GUI.Toggle(
                        buttonRect,
                        m_previewChannels[1],
                        string.Empty,
                        GUIStyle.none
                ...
                else if (m_channelNumber == 3)
                {
                    ...
                    buttonRect.x += 14 * drawInfo.InvertedZoom;
                    m_previewChannels[2] = GUI.Toggle(
                        buttonRect,
                        m_previewChannels[2],
                        string.Empty,
                        GUIStyle.none
                ...
                else if (m_channelNumber == 4)
                {
                    ...
                    buttonRect.x += 14 * drawInfo.InvertedZoom;
                    m_previewChannels[3] = GUI.Toggle(
                        buttonRect,
                        m_previewChannels[3],
                        string.Empty,
                        GUIStyle.none
        ...
        protected void DrawPreviewMaskButtonsRepaint(DrawInfo drawInfo, Rect rect)
        {
            ...
            )
            {
                ...
                if (m_channelNumber == 2)
                {
                    ...
                    buttonRect.x += 14 * drawInfo.InvertedZoom;
                    UIUtils.MiniButtonTopRight.Draw(
                        buttonRect,
                        "G",
                        false,
                        false,
        ...
        {
            if (
                m_fireTimedUpdateRequest
                && (EditorApplication.timeSinceStartup - m_timedUpdateInitialValue)
                    > m_timedUpdateInterval
            )
            {
        ...
        public virtual void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            if (m_isVisible)
            {
                ...
                m_headerPosition.height =
                    UIUtils.CurrentHeaderHeight + m_extraHeaderHeight * drawInfo.InvertedZoom;
                ...
                m_titlePos.width -=
                    (m_paddingTitleLeft + m_paddingTitleRight) * drawInfo.InvertedZoom;
                ...
                if (m_hasLeftDropdown && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                {
                    ...
                    m_dropdownRect.x =
                        m_globalPosition.x
                        + (Constants.IconsLeftRightMargin + 1) * drawInfo.InvertedZoom;
                    m_dropdownRect.height = Constants.NodeButtonSizeY * drawInfo.InvertedZoom;
                    m_dropdownRect.y =
                        m_globalPosition.y
                        + m_headerPosition.height * 0.5f
                        - 14 * drawInfo.InvertedZoom * 0.5f;
                }
                ...
                if (m_drawPreviewExpander && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                {
                    ...
                    m_expandRect.x =
                        m_globalPosition.x
                        + m_globalPosition.width
                        - (Constants.IconsLeftRightMargin + Constants.PreviewExpanderWidth)
                            * drawInfo.InvertedZoom; //titlePos.x + titlePos.width;
                    m_expandRect.height = Constants.PreviewExpanderHeight * drawInfo.InvertedZoom;
                    m_expandRect.y =
                        m_globalPosition.y
                        + m_headerPosition.height * 0.5f
                        - Constants.PreviewExpanderHeight * drawInfo.InvertedZoom * 0.5f;
                }
            }

            if (m_anchorAdjust < 0)
            {
                m_anchorAdjust = UIUtils
                    .GetCustomStyle(CustomStyle.PortEmptyIcon)
            ...
            // Input Ports
            {
                ...
                m_currInputPortPos.y +=
                    drawInfo.InvertedZoom * Constants.PORT_INITIAL_Y
                    + m_extraHeaderHeight * drawInfo.InvertedZoom;
                ...
                for (int i = 0; i < inputCount; i++)
                {
                    if (m_sortedInputPorts[i].Visible)
                    {
                        ...
                        float deltaX =
                            1f
                            * drawInfo.InvertedZoom
                            * (UIUtils.PortsSize.x + Constants.PORT_TO_LABEL_SPACE_X);
                        ...
                        m_sortedInputPorts[i].LabelSize =
                            m_sortedInputPorts[i].UnscaledLabelSize * drawInfo.InvertedZoom;

                        m_lastInputBottomRight.x = Mathf.Max(
                            m_lastInputBottomRight.x,
                            initialX
                                + m_sortedInputPorts[i].UnscaledLabelSize.x
                                + Constants.PORT_INITIAL_X
                                + Constants.PORT_TO_LABEL_SPACE_X
                                + UIUtils.PortsSize.x
                        );
                        ...
                        if (!m_sortedInputPorts[i].Locked)
                        {
                            ...
                            m_auxRect.xMin -=
                                Constants.PORT_INITIAL_X * drawInfo.InvertedZoom
                                + scaledOverflow
                                + overflow;
                            if (
                                m_containerGraph
                                ...
                                m_auxRect.xMax +=
                                    m_sortedInputPorts[i].LabelSize.x
                                    + Constants.PORT_TO_LABEL_SPACE_X * drawInfo.InvertedZoom
                                    + scaledOverflow
                                    + overflow;
                            else
                                m_auxRect.xMax +=
                        ...
                        m_currInputPortPos.y +=
                            drawInfo.InvertedZoom * (m_fontHeight + Constants.INPUT_PORT_DELTA_Y);
                        //GUI.Label( m_sortedInputPorts[ i ].ActivePortArea, string.Empty, UIUtils.Box );
                    }
                }
                if (m_visibleInputs > 0)
                    m_lastInputBottomRight.y +=
                        m_fontHeight * m_visibleInputs
                        + Constants.INPUT_PORT_DELTA_Y * (m_visibleInputs - 1);
            }
            ...
            // Output Ports
            {
                ...
                m_currOutputPortPos.x += (
                    m_globalPosition.width
                    - drawInfo.InvertedZoom * (Constants.PORT_INITIAL_X + m_anchorAdjust)
                );
                m_currOutputPortPos.y +=
                    drawInfo.InvertedZoom * Constants.PORT_INITIAL_Y
                    + m_extraHeaderHeight * drawInfo.InvertedZoom;
                ...
                for (int i = 0; i < outputCount; i++)
                {
                    if (m_sortedOutputPorts[i].Visible)
                    {
                        ...
                        m_sortedOutputPorts[i].LabelSize =
                            m_sortedOutputPorts[i].UnscaledLabelSize * drawInfo.InvertedZoom;

                        m_lastOutputBottomLeft.x = Mathf.Min(
                            m_lastOutputBottomLeft.x,
                            initialX
                                - m_sortedOutputPorts[i].UnscaledLabelSize.x
                                - Constants.PORT_INITIAL_X
                                - Constants.PORT_TO_LABEL_SPACE_X
                                - UIUtils.PortsSize.x
                        );
                        ...
                        if (!m_sortedOutputPorts[i].Locked)
                        {
                            ...
                            if (
                                ...
                                m_auxRect.xMin -=
                                    m_sortedOutputPorts[i].LabelSize.x
                                    + Constants.PORT_TO_LABEL_SPACE_X * drawInfo.InvertedZoom
                                    + scaledOverflow
                                    + overflow;
                            else
                ...
                if (m_visibleOutputs > 0)
                    m_lastOutputBottomLeft.y +=
                        m_fontHeight * m_visibleOutputs
                        + Constants.INPUT_PORT_DELTA_Y * (m_visibleOutputs - 1);
            }
            ...
            m_unscaledRemainingBox.yMax = Mathf.Max(
                m_lastOutputBottomLeft.y,
                m_lastInputBottomRight.y
            );

            m_remainingBox.position =
                (m_unscaledRemainingBox.position + drawInfo.CameraOffset) * drawInfo.InvertedZoom;
            ...
            if (m_canExpand && (m_showPreview || m_globalShowPreview))
            {
                if (m_autoLocation == PreviewLocation.TopCenter)
                {
                    ...
                    m_unscaledPreviewRect.x =
                        m_unscaledRemainingBox.center.x - 0.5f * (previewSize + 2 * marginAround);
                }
                else if (m_autoLocation == PreviewLocation.BottomCenter)
                {
                    m_unscaledPreviewRect.y = Mathf.Max(
                        m_lastOutputBottomLeft.y,
                        m_lastInputBottomRight.y
                    );
                    m_unscaledPreviewRect.x =
                        m_position.x
                        + 0.5f * m_position.width
                        - 0.5f * (previewSize + 2 * marginAround);
                }
                ...
                if (m_autoLocation == PreviewLocation.BottomCenter)
                    ...
                    m_unscaledPreviewRect.height =
                        previewSize
                        + (
                            m_visibleInputs > 0 && m_visibleOutputs > 0
                                ? 2 * marginAround
                                : marginAround
                        );
                ...
                m_previewRect.position =
                    (m_previewRect.position + drawInfo.CameraOffset) * drawInfo.InvertedZoom;
                m_auxVector2.Set(
                    previewSize * drawInfo.InvertedZoom,
                    previewSize * drawInfo.InvertedZoom
                );
                ...
                {
                    m_auxVector2.Set(
                        m_previewRect.position.x + scaledMarginAround,
                        m_previewRect.position.y + scaledMarginAround
                    );
                ...
                else if (m_autoLocation == PreviewLocation.TopCenter)
                {
                    m_auxVector2.Set(
                        m_previewRect.position.x + scaledMarginAround,
                        m_previewRect.position.y
                    );
                    m_previewRect.position = m_auxVector2;
                }
                else
                {
                    m_previewRect.position += new Vector2(
                        scaledMarginAround,
                        (m_visibleInputs > 0 && m_visibleOutputs > 0 ? scaledMarginAround : 0)
                    );
                }
            }
            ...
            if (m_firstPreviewDraw)
            {
                ...
                if (m_canExpand && (m_showPreview || m_globalShowPreview))
                {
                    ...
                    {
                        float fillWidth =
                            m_unscaledRemainingBox.width - m_unscaledPreviewRect.width;
            ...
            if (m_showErrorMessage)
            {
                ...
                m_errorBox.y =
                    (m_globalPosition.yMax - 28 * drawInfo.InvertedZoom)
                    + 3 * drawInfo.InvertedZoom;
        ...
        public virtual void OnNodeRepaint(DrawInfo drawInfo)
        {
            ...
            // Background
            GUI.color = m_infiniteLoopDetected
                ? Constants.InfiniteLoopColor
                : Constants.NodeBodyColor;
            if (m_useSquareNodeTitle || ContainerGraph.LodLevel >= ParentGraph.NodeLOD.LOD2)
                ...
                GUI.Label(
                    m_globalPosition,
                    string.Empty,
                    UIUtils.GetCustomStyle(CustomStyle.NodeWindowOff)
                );
            ...
            if (m_useSquareNodeTitle || ContainerGraph.LodLevel >= ParentGraph.NodeLOD.LOD2)
                ...
                GUI.Label(
                    m_headerPosition,
                    string.Empty,
                    UIUtils.GetCustomStyle(CustomStyle.NodeHeader)
                );
            ...
            if (functionNode != null)
            {
                ...
                GUI.DrawTexture(
                    new Rect(m_globalPosition.x, m_globalPosition.y, scaledSize, scaledSize),
                    functionNode.HeaderIcon,
            ...
            if (m_hasSubtitle && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
                GUI.Label(
                    m_addTitlePos,
                    m_additionalContent,
                    UIUtils.GetCustomStyle(CustomStyle.PropertyValuesTitle)
                );
            ...
            if (m_drawPreviewExpander && ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                GUI.Label(
                    m_expandRect,
                    string.Empty,
                    (m_showPreview ? UIUtils.PreviewCollapser : UIUtils.PreviewExpander)
                );
                ...
                if (m_inputPorts[i].Visible)
                {
                    ...
                    if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                    {
                        if (m_inputPorts[i].Locked)
                            ...
                            GUI.color = UIUtils.GetColorForDataType(
                                m_inputPorts[i].DataType,
                                false,
                                true
                            );
                        else
                            GUI.color = m_inputPorts[i].HasCustomColor
                                ? m_inputPorts[i].CustomColor
                                : UIUtils.GetColorForDataType(m_inputPorts[i].DataType, true, true);

                        GUIStyle style = m_inputPorts[i].IsConnected
                            ? UIUtils.GetCustomStyle(CustomStyle.PortFullIcon)
                            : UIUtils.GetCustomStyle(CustomStyle.PortEmptyIcon);
                    ...
                    if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
                    {
                        if (m_inputPorts[i].Locked)
                        {
                            ...
                            GUI.Label(
                                m_inputPorts[i].LabelPosition,
                                m_inputPorts[i].Name,
                                UIUtils.InputPortLabel
                            );
                            GUI.color = m_colorBuffer;
                        }
                        else
                        {
                            if (
                                m_containerGraph.ParentWindow.GlobalShowInternalData
                                && !m_inputPorts[i].IsConnected
                                && UIUtils.InternalDataOnPort.fontSize > 1f
                                && (
                                    m_inputPorts[i].AutoDrawInternalData
                                    || (m_autoDrawInternalPortData && m_useInternalPortData)
                                )
                                && m_inputPorts[i].DisplayInternalData.Length > 4
                                && (
                                    (
                                        m_inputPorts[i].DataType >= WirePortDataType.FLOAT
                                        && m_inputPorts[i].DataType <= WirePortDataType.INT
                                    )
                                    || m_inputPorts[i].DataType == WirePortDataType.UINT
                                )
                            )
                            {
                                GUI.color =
                                    Constants.NodeBodyColor /* * new Color( 1f, 1f, 1f, 0.75f )*/
                                ;
                                Rect internalBox = m_inputPorts[i].LabelPosition;
                                m_sizeContentAux.text = m_inputPorts[i].DisplayInternalData;
                                Vector2 portText = UIUtils.InternalDataOnPort.CalcSize(
                                    m_sizeContentAux
                                );
                                internalBox.width = portText.x;
                                internalBox.height = portText.y;
                                internalBox.y =
                                    m_inputPorts[i].LabelPosition.center.y
                                    - internalBox.height * 0.5f;
                                internalBox.x =
                                    GlobalPosition.x
                                    - internalBox.width
                                    - 4 * drawInfo.InvertedZoom
                                    - 1;
                ...
                if (m_outputPorts[i].Visible)
                {
                    ...
                    if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                    {
                        if (m_outputPorts[i].Locked)
                            ...
                            GUI.color = UIUtils.GetColorForDataType(
                                m_outputPorts[i].DataType,
                                false,
                                false
                            );
                        else
                            GUI.color = m_outputPorts[i].HasCustomColor
                                ? m_outputPorts[i].CustomColor
                                : UIUtils.GetColorForDataType(
                                    m_outputPorts[i].DataType,
                                    true,
                                    false
                                );

                        GUIStyle style = m_outputPorts[i].IsConnected
                            ? UIUtils.GetCustomStyle(CustomStyle.PortFullIcon)
                            : UIUtils.GetCustomStyle(CustomStyle.PortEmptyIcon);
                    ...
                    if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
                    {
                        if (m_outputPorts[i].Locked)
                        {
                            GUI.color = Constants.PortLockedTextColor;
                            GUI.Label(
                                m_outputPorts[i].LabelPosition,
                                m_outputPorts[i].Name,
                                UIUtils.OutputPortLabel
                            );
                            GUI.color = m_colorBuffer;
                        }
                        else
            ...
            if (m_selected)
            {
                ...
                if (m_useSquareNodeTitle || ContainerGraph.LodLevel >= ParentGraph.NodeLOD.LOD2)
                    ...
                    GUI.Label(
                        m_globalPosition,
                        string.Empty,
                        UIUtils.GetCustomStyle(CustomStyle.NodeWindowOn)
                    );
                GUI.color = m_colorBuffer;
            }
        ...
        public bool DropdownEditing
        {
            get { return m_dropdownEditing; }
            set
            {
                m_dropdownEditing = value;
                PreviewIsDirty = true;
            }
        }
        ...
        public void DrawGuiPorts(DrawInfo drawInfo)
        {
            ...
            if (drawInfo.CurrentEventType == EventType.MouseDown)
            {
                ...
                {
                    if (
                        m_inputPorts[i].Visible
                        && !m_inputPorts[i].Locked
                        && m_isVisible
                        && m_inputPorts[i].ActivePortArea.Contains(drawInfo.MousePosition)
                        && drawInfo.LeftMouseButtonPressed
                    )
                    {
                        UIUtils.CurrentWindow.MouseInteracted = true;
                        ...
                        if (m_inputPorts[i].IsConnected)
                        {
                            ...
                            bool doubleTap =
                                (doubleTapTime - m_doubleClickTimestamp) < DoubleClickTime;
                        ...
                        {
                            m_containerGraph.ParentWindow.WireReferenceUtils.SetInputReference(
                                m_uniqueId,
                                m_inputPorts[i].PortId,
                                m_inputPorts[i].DataType,
                                m_inputPorts[i].TypeLocked
                            );
                        }
                ...
                {
                    if (
                        m_outputPorts[i].Visible
                        && m_outputPorts[i].ActivePortArea.Contains(drawInfo.MousePosition)
                        && drawInfo.LeftMouseButtonPressed
                    )
                    {
                        UIUtils.CurrentWindow.MouseInteracted = true;
                        ...
                        {
                            m_containerGraph.ParentWindow.WireReferenceUtils.SetOutputReference(
                                m_uniqueId,
                                m_outputPorts[i].PortId,
                                m_outputPorts[i].DataType,
                                false
            ...
            //Preview buttons
            if (
                m_drawPreviewMaskButtons
                && (
                    drawInfo.CurrentEventType == EventType.MouseDown
                    || drawInfo.CurrentEventType == EventType.MouseUp
                )
            )
        ...
        public virtual void SetPreviewInputs()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                if (
                    m_inputPorts[i].IsConnected
                    && m_inputPorts[i].InputNodeHasPreview(ContainerGraph)
                )
                {
        ...
        public bool ShowTooltip(DrawInfo drawInfo)
        {
            ...
            if (m_globalPosition.Contains(drawInfo.MousePosition) || m_linkVisibility)
            {
                if (m_tooltipTimestamp + 0.6f < Time.realtimeSinceStartup || m_linkVisibility)
                {
                    ...
                    if (
                        m_showErrorMessage
                        && m_errorBox.Contains(drawInfo.MousePosition)
                        && !string.IsNullOrEmpty(m_errorMessageTooltip)
                    )
                        errorTooltip = true;

                    Rect globalTooltipPos = m_globalPosition;
                    GUIContent temp = new GUIContent(
                        errorTooltip ? m_errorMessageTooltip : m_tooltipText
                    );
                    ...
                    if (
                        errorTooltip
                        && globalTooltipPos.y + globalTooltipPos.height > Screen.height - 32
                    )
                    ...
                    if (!errorTooltip && m_hasTooltipLink)
                    {
                        ...
                        if (link.Contains(drawInfo.MousePosition))
                        {
                            ...
                            {
                                UIUtils
                                    .MainSkin.customStyles[52]
                                    .Draw(link, WikiLinkStr, true, false, false, false);
                            }
                        }
                        else
                        {
        ...
        protected void DrawInternalDataGroup()
        {
            ...
            if (inputCount > 0)
            {
                ...
                {
                    if (
                        m_inputPorts[i].Available
                        && m_inputPorts[i].ValidInternalData
                        && !m_inputPorts[
                            i
                        ].IsConnected /*&& ( m_inputPorts[ i ].AutoDrawInternalData || ( m_autoDrawInternalPortData && m_useInternalPortData ) )*/ /*&& m_inputPorts[ i ].AutoDrawInternalData*/
                    )
                    {
            ...
            if (drawInternalDataUI)
                NodeUtils.DrawPropertyGroup(
                    ref m_internalDataFoldout,
                    Constants.InternalDataLabelStr,
                    () =>
                    {
                        for (int i = 0; i < m_inputPorts.Count; i++)
                        {
        ...
        protected void PickInput(InputPort port)
        {
            ...
            m_containerGraph.ParentWindow.WireReferenceUtils.OutputPortReference.SetReference(
                from.NodeId,
                from.PortId,
                from.DataType,
                connection.TypeLocked
            );
        ...
        protected bool ApplyModifierToPort(WirePort port, bool isInput)
        {
                ...
                case EventModifiers.Control:
                    {
                        ...
                        {
                            WireReference connection = port.GetConnection(0);
                            m_containerGraph.ParentWindow.WireReferenceUtils.InputPortReference.SetReference(
                                connection.NodeId,
                                connection.PortId,
                                connection.DataType,
                                connection.TypeLocked
            ...
            if (isInput)
                m_containerGraph.ParentWindow.WireReferenceUtils.SwitchPortReference.SetReference(
                    port.NodeId,
                    port.PortId,
                    port.DataType,
                    false
                ); //always save last connection
            else
                m_containerGraph.ParentWindow.WireReferenceUtils.SwitchPortReference.SetReference(
                    -1,
                    -1,
        ...
        public void DeleteAllInputConnections(
            bool alsoDeletePorts,
            bool inhibitWireNodeAutoDel = false
        )
        {
            int count = m_inputPorts.Count;
            for (int i = 0; i < count; i++)
        ...
        public string GetValueFromOutputStr(
            int outputId,
            WirePortDataType inputPortType,
        ...
        )
        {
            ...
            {
                if (
                    outPort.DataType != WirePortDataType.OBJECT
                    && outPort.DataType != inputPortType
                )
                {
                    return UIUtils.CastPortType(
                        ref dataCollector,
                        CurrentPrecisionType,
                        null,
                        outPort.DataType,
                        inputPortType,
                        outPort.LocalValue(dataCollector.PortCategory)
                    );
                }
                else
                {
        ...
        public virtual string GenerateShaderForOutput(
            int outputId,
            ref MasterNodeDataCollector dataCollector,
            bool ignoreLocalvar
        )
            ...
            if (dataCollector.IsSRP)
            {
                switch (dataCollector.CurrentSRPType)
                {
                    case TemplateSRPType.HDRP:
                        if (OnHDAction != null)
                            OnHDAction(outputId, ref dataCollector);
                        break;
                    case TemplateSRPType.URP:
                        if (OnLightweightAction != null)
                            OnLightweightAction(outputId, ref dataCollector);
                        break;
                }
            }
            return string.Empty;
        ...
        public string GenerateValueInVertex(
            ref MasterNodeDataCollector dataCollector,
            WirePortDataType dataType,
            string dataValue,
            string dataName,
        ...
        )
        {
            ...
            if (dataCollector.IsTemplate)
            {
                if (
                    createInterpolator
                    && dataCollector.TemplateDataCollectorInstance.HasCustomInterpolatedData(
                        dataName
                    )
                )
                    return dataName;
                ...
                {
                    dataCollector.TemplateDataCollectorInstance.RegisterCustomInterpolatedData(
                        dataName,
                        dataType,
                        CurrentPrecisionType,
                        dataValue
                ...
                {
                    dataCollector.AddToVertexLocalVariables(
                        -1,
                        CurrentPrecisionType,
                        dataType,
                        dataName,
            ...
            //SURFACE
            {
                if (dataCollector.TesselationActive)
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        "Unable to use Vertex to Frag when Tessellation is active"
                    );
                ...
                if (createInterpolator)
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        Constants.VertexShaderOutputStr + "." + dataName,
                        dataValue + ";"
                    );
                }
                else
        ...
        public string GenerateInputInVertex(
            ref MasterNodeDataCollector dataCollector,
            int inputPortUniqueId,
            string varName,
            bool createInterpolator,
        ...
        )
        {
            ...
            //SURFACE
            {
                ...
                if (createInterpolator)
                {
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        Constants.VertexShaderOutputStr + "." + varName,
                        vertexVarValue + ";"
                    );
                }
                else
        ...
        public string CreateOutputLocalVariable(
            int outputArrayId,
            string value,
            ref MasterNodeDataCollector dataCollector
        )
        ...
        public void RegisterLocalVariable(
            int outputArrayId,
            string value,
            ref MasterNodeDataCollector dataCollector,
        ...
        )
        {
            ...
            string localVar = port.ConfigOutputLocalValue(
                CurrentPrecisionType,
                value,
                customName,
        ...
        {
            ContainerGraph.ParentWindow.ParametersWindow.IsMaximized = !ContainerGraph
                .ParentWindow
                .ParametersWindow
                .IsMaximized;
        }
        ...
        public Vector2 CenterPosition
        {
            ...
            {
                return new Vector2(
                    m_position.x + m_position.width * 0.5f,
                    m_position.y + m_position.height * 0.5f
                );
                ;
            }
        }

        public Rect GlobalPosition
        {
        ...
        {
            get { return m_connStatus == NodeConnectionStatus.Connected; }
        }
        public NodeConnectionStatus ConnStatus
        {
            ...
            set
            {
                ...
                switch (m_connStatus)
                {
                    ...
                    case NodeConnectionStatus.Not_Connected:
                        m_statusColor = Constants.NodeDefaultColor;
                        break;
                    case NodeConnectionStatus.Connected:
                        m_statusColor = Constants.NodeConnectedColor;
                        break;
                    case NodeConnectionStatus.Error:
                        m_statusColor = Constants.NodeErrorColor;
                        break;
                }
            }
        }
        ...
        public virtual int InputIdFromDeprecated(int oldInputId)
        {
            return oldInputId;
        }

        public virtual int OutputIdFromDeprecated(int oldOutputId)
        {
            return oldOutputId;
        }
        ...
        public void ParentReadFromString(ref string[] nodeParams)
        {
            ...
            string[] posCoordinates = nodeParams[m_currentReadParamIdx++]
                .Split(IOUtils.VECTOR_SEPARATOR);
            ...
            if (UIUtils.CurrentShaderVersion() > 22)
            {
                ...
                if (m_customPrecision)
                {
                    if (val.Equals("Fixed"))
                        ...
                        m_currentPrecisionType = (PrecisionType)
                            Enum.Parse(typeof(PrecisionType), val);
                }
                else
                {
        ...
        public virtual void ReadInputDataFromString(ref string[] nodeParams)
        {
            ...
            )
            {
                if (UIUtils.CurrentShaderVersion() < 5003)
                {
                    ...
                    {
                        m_inputPorts[newId].DataType = (WirePortDataType)
                            Enum.Parse(
                                typeof(WirePortDataType),
                ...
                else
                {
                    ...
                    WirePortDataType DataType = (WirePortDataType)
        ...
        protected string GetCurrentParam(ref string[] nodeParams)
        {
            ...
            UIUtils.ShowMessage(
                UniqueId,
                "Invalid params number in node " + m_uniqueId + " of type " + GetType(),
        ...
        public virtual void WriteToString(ref string nodeInfo, ref string connectionsInfo)
        {
            ...
            IOUtils.AddFieldValueToString(
                ref nodeInfo,
                (m_position.x.ToString() + IOUtils.VECTOR_SEPARATOR + m_position.y.ToString())
            );
        ...
        public virtual bool OnNodeInteraction(ParentNode node)
        {
            return false;
        }

        public virtual void OnConnectedOutputNodeChanges(
            int portId,
            int otherNodeId,
            int otherPortId,
            string name,
        ...
        public virtual void OnConnectedInputNodeChanges(
            int portId,
            int otherNodeId,
            int otherPortId,
            string name,
        ...
        public Rect CachedPos
        {
            get { return m_cachedPos; }
        }
        ...
        public virtual void PropagateNodeData(
            NodeData nodeData,
            ref MasterNodeDataCollector dataCollector
        )
        {
        ...
        public void SetAdditonalTitleTextOnCallback(
            string compareTitle,
            Action<ParentNode, string> callback
        )
        {
        ...
        public void SetAdditonalTitleTextOnCallback(
            string compareTitle,
            string compareTitleFormat,
            Action<ParentNode, string> callback
        )
        {
            if (
                !m_previousAdditonalTitle.Equals(compareTitle)
                || !m_previousAdditonalTitleFormat.Equals(compareTitleFormat)
            )
            {
                m_previousAdditonalTitle = compareTitle;
        ...
        public virtual void SetClippedTitle(
            string newText,
            int maxSize = 170,
            string endString = "..."
        )
        ...
        public virtual void SetClippedAdditionalTitle(
            string newText,
            int maxSize = 170,
            string endString = "..."
        )
        ...
        public virtual int VersionConvertInputPortId(int portId)
        {
            return portId;
        }

        public virtual int VersionConvertOutputPortId(int portId)
        {
            return portId;
        }

        public virtual string DataToArray
        {
        ...
        public GUIContent AdditonalTitleContent
        {
            get { return m_additionalContent; }
        }
        ...
        {
            get { return m_nodeAttribs; }
        }
        public bool ReorderLocked
        {
            get { return m_reorderLocked; }
        }
        public bool RequireMaterialUpdate
        {
            get { return m_requireMaterialUpdate; }
        }
        public bool RMBIgnore
        {
            get { return m_rmbIgnore; }
        }
        ...
        {
            get { return m_isMoving > 0; }
        }
        public bool MovingInFrame
        {
            get { return m_movingInFrame; }
            set { m_movingInFrame = value; }
        }
        public bool SizeIsDirty
        {
            get { return m_sizeIsDirty; }
            set { m_sizeIsDirty = value; }
        }
        public int Category
        {
        ...
        public virtual void RenderNodePreview()
        {
            ...
            if (!Preferences.User.DisablePreviews)
            {
                ...
                RenderTexture beforeMask = RenderTexture.GetTemporary(
                    Constants.PreviewSize,
                    Constants.PreviewSize,
                    0,
                    Constants.PreviewFormat,
                ...
                for (int i = 0; i < count; i++)
                {
                    if (i == 0)
                    {
                        ...
                        if (
                            m_outputPorts[i].DataType == WirePortDataType.FLOAT3x3
                            || m_outputPorts[i].DataType == WirePortDataType.FLOAT4x4
                        )
                        {
                            m_outputPorts[i]
                                .MaskingMaterial.SetTexture(
                                    m_cachedMainTexId,
                        ...
                        {
                            m_outputPorts[i]
                                .MaskingMaterial.SetTexture(m_cachedMainTexId, beforeMask);
                        }
                        ...
                        Graphics.Blit(
                            null,
                            m_outputPorts[i].OutputPreviewTexture,
                            m_outputPorts[i].MaskingMaterial,
                            0
                        );
                    }
                    else
                    {
                        m_outputPorts[i]
                            .MaskingMaterial.SetTexture(m_cachedMaskTexId, PreviewTexture);
        ...
        public string GenerateClippedTitle(
            string original,
            int maxSize = 170,
            string endString = "..."
        )
        ...
        {
            get { return m_graphDepth; }
        }

        public PrecisionType CurrentPrecisionType
        {
            get
            {
                return m_currentPrecisionType == PrecisionType.Inherit
                    ? ContainerGraph.CurrentPrecision
        ...
        public Shader PreviewShader
        {
            get
            {
                ...
                {
                    m_previewShader = AssetDatabase.LoadAssetAtPath<Shader>(
                        AssetDatabase.GUIDToAssetPath(m_previewShaderGUID)
                    );
                }
        ...
        {
            get { return m_dropdownRect; }
        }

        public virtual bool Contains(Vector2 pos)
        {
            return m_globalPosition.Contains(pos);
        }
        ...
        public bool IsNodeBeingCopied
        {
            get { return m_isNodeBeingCopied; }
            set { m_isNodeBeingCopied = value; }
        }
        ...
        public virtual ParentNode ExecuteStubCode()
        {
            return this;
        }
        ...
        {
            get { return m_wasDeprecated; }
            set { m_wasDeprecated = value; }
        }
        public bool Alive
        {
            get { return m_alive; }
        ...
        public string TypeName
        {
            ...
            {
                if (m_nodeAttribs != null)
                    return m_nodeAttribs.Name;
                return GetType().ToString();
            }
        }
        public bool PreviewIsDirty
        {
            set { m_previewIsDirty = value; }
            get { return m_previewIsDirty; }
        }
        protected bool FinishPreviewRender
        {
            get { return m_finishPreviewRender; }
            set { m_finishPreviewRender = value; }
        }
        public virtual bool IsStubNode
        {
            get { return false; }
        }
    }
