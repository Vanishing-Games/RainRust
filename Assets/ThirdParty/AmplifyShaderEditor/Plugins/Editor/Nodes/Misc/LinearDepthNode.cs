    ...
    public sealed class LinearDepthNode : ParentNode
    {
        ...
        void UpdateFromOption()
        {
            ...
            SetAdditonalTitleText(
                string.Format(Constants.SubTitleSpaceFormatStr, LinearModeLabels[m_currentOption])
            );
        }
        ...
        public override void DrawProperties()
        {
            base.DrawProperties();
            EditorGUI.BeginChangeCheck();
            m_currentOption = EditorGUILayoutPopup(
                ViewSpaceLabel,
                m_currentOption,
                LinearModeLabels
            );
            if (EditorGUI.EndChangeCheck())
            {
                SetAdditonalTitleText(
                    string.Format(
                        ...
                        LinearModeLabels[m_currentOption]
                    )
                );
            }
        }
        ...
        )
        {
            ...
            {
                value = string.Format(
                    ((m_currentOption == 0) ? LinearEyeFuncSRPFormat : Linear01FuncSRPFormat),
                    value
                );
            }
            else
            {
                value = string.Format(
                    ((m_currentOption == 0) ? LinearEyeFuncFormat : Linear01FuncFormat),
                    value
                );
            }
