...
namespace AmplifyShaderEditor
{
    ...
    public class CustomExpressionInputItem
    {
        ...
        public CustomExpressionInputItem(
            PrecisionType precision,
            VariableQualifiers qualifier,
            string customType,
            bool isVariable,
            bool foldoutFlag,
            string foldoutLabel
    ...
    public class CustomExpressionDependency
    {
        ...
        public CustomExpressionDependency()
        {
            DependencyArrayIdx = DependencyNodeId = -1;
        }

        public CustomExpressionDependency(string id)
        {
            DependencyNodeId = Convert.ToInt32(id);
    ...
    public sealed class CustomExpressionNode : ParentNode
    {
        ...
        private const string CustomExpressionInfo =
            "Creates a custom expression or function according to how code is written on text area.\n\n"
        ...
        private readonly string[] QualifiersStr = { "In", "Out", "InOut" };
        ...
        private readonly Dictionary<WirePortDataType, int> WireToIdx = new Dictionary<
            WirePortDataType,
            int
        >
        {
        ...
        [SerializeField]
        private List<CustomExpressionDependency> m_dependencies =
            new List<CustomExpressionDependency>();
        ...
        protected override void CommonInit(int uniqueId)
        {
            ...
            m_items.Add(
                new CustomExpressionInputItem(
                    PrecisionType.Inherit,
                    VariableQualifiers.In,
                    string.Empty,
                    false,
                    true,
                    string.Empty /*"[0]"*/
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3)
            {
                if (
                    ...
                    && (
                        (
                            !ContainerGraph.ParentWindow.MouseInteracted
                ...
                if (m_isEditing || m_startEditing)
                {
                    ...
                    m_customExpressionName = EditorGUITextField(
                        m_titleClickArea,
                        string.Empty,
                        m_customExpressionName,
        ...
        string WrapCodeInFunction(bool isTemplate, string functionName, bool expressionMode)
        {
            ...
            string returnType =
                (m_mode == CustomExpressionMode.Call || m_voidMode)
                    ? "void"
                    : UIUtils.PrecisionWirePortToCgType(
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                string qualifier =
                    m_items[i].Qualifier == VariableQualifiers.In
                        ? string.Empty
                        : UIUtils.QualifierToCg(m_items[i].Qualifier) + " ";
                ...
                if (m_inputPorts[portIdx].DataType == WirePortDataType.OBJECT)
                    ...
                    declaration = UIUtils.PrecisionWirePortToTypeValue(
                        precision,
                        m_inputPorts[portIdx].DataType,
                        m_inputPorts[portIdx].Name
                    );
            ...
            UIUtils.ShaderIndentLevel++;
            {
                ...
                for (int i = 0; i < codeLines.Length; i++)
                {
                    ...
                    {
                        functionBody +=
                            ((i == 0 && expressionMode) ? string.Empty : UIUtils.ShaderIndentTabs)
                            + codeLines[i]
                            + (
                                ((i == codeLines.Length - 1) && expressionMode)
                                    ? string.Empty
                                    : "\n"
                            );
                    }
        ...
        string GetFileAssetRelativePath()
        {
            ...
            if (currentShader != null)
            {
                ...
                UIUtils.ShowMessage(
                    UniqueId,
                    "Invalid path found in Custom Expression File Asset: " + fileAssePath,
        ...
        {
            EditorGUI.BeginChangeCheck();
            m_customExpressionName = EditorGUILayoutTextField(
                ExpressionNameLabelStr,
                m_customExpressionName
            );
            if (EditorGUI.EndChangeCheck())
            {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (CheckCallMode())
                    UIUtils.ShowMessage(
                        UniqueId,
                        "Call Mode cannot have return over is code.\nFalling back to Create Mode"
                    );
                SetupCallMode();
            ...
            {
                m_fileAsset =
                    EditorGUILayoutObjectField("Source", m_fileAsset, typeof(TextAsset), false)
                    as TextAsset;
            ...
            if (m_mode == CustomExpressionMode.Create || m_mode == CustomExpressionMode.File)
            {
                ...
                EditorGUI.BeginChangeCheck();
                m_outputTypeIdx = EditorGUILayoutPopup(
                    OutputTypeStr,
                    m_outputTypeIdx,
                    AvailableOutputWireTypesStr
                );
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    {
                        m_outputPorts[0]
                            .ChangeType(AvailableOutputWireTypes[m_outputTypeIdx], false);
                    }
                }
            }
            NodeUtils.DrawNestedPropertyGroup(
                ref m_dependenciesFoldout,
                "Dependencies",
                DrawDependencies,
                DrawDependenciesAddRemoveInputs
            );
        }
        ...
        void DrawItemsAddRemoveInputs()
        {
            ...
            // Add new port
            if (
                GUILayoutButton(string.Empty, UIUtils.PlusStyle, GUILayout.Width(ButtonLayoutWidth))
            )
            {
            ...
            if (
                GUILayoutButton(
                    string.Empty,
                    UIUtils.MinusStyle,
                    GUILayout.Width(ButtonLayoutWidth)
                )
            )
            {
        ...
        void DrawDependencies()
        {
            ...
            {
                m_dependenciesReordableList = new ReorderableList(
                    m_dependencies,
                    typeof(CustomExpressionDependency),
                    true,
                    false,
                    false,
                    false
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        if (m_dependencies[index] != null)
                        {
                            ...
                            Rect popupPos = new Rect(
                                rect.x,
                                rect.y,
                                rect.width - 2 * Constants.PlusMinusButtonLayoutWidth,
                                EditorGUIUtility.singleLineHeight
                            );
                            Rect buttonPlusPos = new Rect(
                                rect.x + rect.width - 2 * Constants.PlusMinusButtonLayoutWidth,
                                rect.y - 2,
                                Constants.PlusMinusButtonLayoutWidth,
                                Constants.PlusMinusButtonLayoutWidth
                            );
                            Rect buttonMinusPos = new Rect(
                                rect.x + rect.width - Constants.PlusMinusButtonLayoutWidth,
                                rect.y - 2,
                                Constants.PlusMinusButtonLayoutWidth,
                                Constants.PlusMinusButtonLayoutWidth
                            );
                            EditorGUI.BeginChangeCheck();
                            m_dependencies[index].DependencyArrayIdx = EditorGUIPopup(
                                popupPos,
                                string.Empty,
                                m_dependencies[index].DependencyArrayIdx,
                                UIUtils
                                    .CurrentWindow
                                    .OutsideGraph
                                    .CustomExpressionOnFunctionMode
                                    .NodesArr
                            );
                            if (EditorGUI.EndChangeCheck())
                            {
                                m_dependencies[index].DependencyNodeId = UIUtils
                                    .CurrentWindow.OutsideGraph.CustomExpressionOnFunctionMode.GetNode(
                                        m_dependencies[index].DependencyArrayIdx
                                    )
                                    .UniqueId;
        ...
        void DrawReordableInputs()
        {
            ...
            {
                m_itemReordableList = new ReorderableList(
                    m_items,
                    typeof(CustomExpressionInputItem),
                    true,
                    false,
                    false,
                    false
                )
                {
                    ...
                    elementHeightCallback = (int index) =>
                    {
                        ...
                        if (m_items[index].FoldoutFlag)
                        {
                            ...
                            if (
                                m_inputPorts[m_firstAvailablePort + index].DataType
                                == WirePortDataType.OBJECT
                            )
                                size += lineHeight;
                            ...
                            if (list.index > m_lastIndex)
                            {
                                for (int i = m_lastIndex; i <= list.index; i++)
                                {
                                    if (m_items[i].Qualifier != VariableQualifiers.In)
                                    {
                                        ...
                                        m_outputPortsDict[oldOutputPortId]
                                            .ChangePortId(
                                                CreateOutputId(m_inputPorts[portIdx].PortId)
                                            );
                                    }
                                }
                            }
                    ...
                    drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                    {
                        if (m_items[index] != null)
                        {
                            ...
                            m_items[index].FoldoutFlag = EditorGUIFoldout(
                                foldoutRect,
                                m_items[index].FoldoutFlag, /*m_items[ index ].FoldoutLabel + " - " +*/
                                m_inputPorts[portIdx].Name
                            );
                            if (m_items[index].FoldoutFlag)
                            {
                                ...
                                //Qualifier
                                rect.y += lineSpacing;
                                VariableQualifiers newQualifier =
                                    (VariableQualifiers)EditorGUIPopup(
                                        rect,
                                        InputQualifierStr,
                                        (int)m_items[index].Qualifier,
                                        QualifiersStr
                                    );
                                if (newQualifier != m_items[index].Qualifier)
                                {
                                    ...
                                    m_inputPorts[portIdx].Visible =
                                        newQualifier != VariableQualifiers.Out;
                                    m_sizeIsDirty = true;
                                    RecalculateInOutOutputPorts();
                                }
                                ...
                                // Precision
                                rect.y += lineSpacing;
                                m_items[index].Precision = (PrecisionType)EditorGUIPopup(
                                    rect,
                                    PrecisionContent.text,
                                    (int)m_items[index].Precision,
                                    PrecisionLabelsExtraLocal
                                );
                                // Type
                                rect.y += lineSpacing;
                                int typeIdx = WireToIdx[m_inputPorts[portIdx].DataType];
                                EditorGUI.BeginChangeCheck();
                                typeIdx = EditorGUIPopup(
                                    rect,
                                    InputTypeStr,
                                    typeIdx,
                                    AvailableWireTypesStr
                                );
                                if (EditorGUI.EndChangeCheck())
                                {
                                    // actual type is need in order for texture array and sampler state to fallback correctly
                                    m_inputPorts[portIdx]
                                        .ChangeType(AvailableWireTypes[typeIdx], false);
                                    if (typeIdx == 5 || typeIdx == 6)
                                    {
                                        m_inputPorts[portIdx].Matrix4x4InternalData =
                                            Matrix4x4.identity;
                                    }

                                    if (m_items[index].Qualifier != VariableQualifiers.In)
                                    {
                                        OutputPort currOutPort = GetOutputPortByUniqueId(
                                            CreateOutputId(m_inputPorts[portIdx].PortId)
                                        );
                                ...
                                {
                                    rect.y += lineSpacing;
                                    m_items[index].CustomType = EditorGUITextField(
                                        rect,
                                        CustomTypeStr,
                                        m_items[index].CustomType
                                    );
                                }
                                ...
                                EditorGUI.BeginChangeCheck();
                                {
                                    m_inputPorts[portIdx].Name = EditorGUITextField(
                                        rect,
                                        InputNameStr,
                                        m_inputPorts[portIdx].Name
                                    );
                                ...
                                if (EditorGUI.EndChangeCheck())
                                {
                                    ...
                                    m_inputPorts[portIdx].Name = UIUtils.RemoveInvalidCharacters(
                                        m_inputPorts[portIdx].Name
                                    );
                                ...
                                if (m_mode == CustomExpressionMode.Call)
                                {
                                    //Is Unique
                                    rect.y += lineSpacing;
                                    m_items[index].IsVariable = EditorGUIToggle(
                                        rect,
                                        IsVariableStr,
                                        m_items[index].IsVariable
                                    );
                                }
                                // Port Data
                                if (!m_inputPorts[portIdx].IsConnected)
                                {
                                    rect.y += lineSpacing;
                                    m_inputPorts[portIdx]
                                        .ShowInternalData(rect, this, true, InputValueStr);
                                }
                            ...
                            else
                            {
                                //Buttons
                                rect.x +=
                                    IdentationAdjust + rect.width - 2 * AddRemoveButtonLayoutWidth;
                                rect.width = AddRemoveButtonLayoutWidth;
                                if (GUI.Button(rect, string.Empty, UIUtils.PlusStyle))
                                {
                                    m_actionType = ReordableAction.Add;
        ...
        void RecalculateInOutOutputPorts()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (m_items[i].Qualifier != VariableQualifiers.In)
                {
                    m_outputPorts[outputId]
                        .ChangeProperties(
                            m_inputPorts[idx].Name,
                            m_inputPorts[idx].DataType,
                            false
                        );
        ...
        public string DealWithFileMode(
            int outputId,
        ...
        )
        {
            ...
            {
                m_fileAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(
                    AssetDatabase.GUIDToAssetPath(m_fileGUID)
                );
            }
            ...
            Dictionary<int, CustomExpressionNode> examinedNodes =
                new Dictionary<int, CustomExpressionNode>();
            for (int i = 0; i < dependenciesCount; i++)
            {
                CustomExpressionNode node =
                    m_containerGraph.GetNode(m_dependencies[i].DependencyNodeId)
                    as CustomExpressionNode;
                if (node == null)
                {
                    node =
                        UIUtils.CurrentWindow.OutsideGraph.GetNode(
                            m_dependencies[i].DependencyNodeId
                        ) as CustomExpressionNode;
                }
            ...
            if (m_voidMode)
            {
                ...
                RegisterLocalVariable(
                    0,
                    string.Format(Constants.CodeWrapper, mainData),
                    ref dataCollector,
                    localVarName
                );
            }
            ...
            if (m_precisionSuffix)
            {
                switch (CurrentPrecisionType)
                {
                    ...
                    case PrecisionType.Float:
                        precisionSuffix = "_float";
                        break;
                    case PrecisionType.Half:
                        precisionSuffix = "_half";
                        break;
                }
            }
            ...
            {
                if (
                    UIUtils.CurrentWindow.OutsideGraph.SamplingMacros
                ...
                else
                {
                    string result =
                        (m_inputPorts[i].IsConnected)
                            ...
                            : m_inputPorts[i].InternalData.ToString();
                    string inputLocalVar = string.Format(
                        Constants.CustomTypeLocalValueDecWithoutIdent,
                        m_items[idx].CustomType,
                        inputPortLocalVar,
                        result
                    );
                ...
                {
                    OutputPort currOutputPort = GetOutputPortByUniqueId(
                        CreateOutputId(m_inputPorts[i].PortId)
                    );
        ...
        )
        {
            ...
            if (string.IsNullOrEmpty(m_code))
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    string.Format(
                        "Custom Expression '{0}' need to have code associated",
                        m_customExpressionName
                    ),
                    MessageSeverity.Warning
                );
            ...
            if (
                !expressionMode
                && !codeContainsReturn
                && m_mode == CustomExpressionMode.Create
                && !m_voidMode
            )
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    string.Format(
                        "Custom Expression '{0}' has a non-void return type but no return instruction was detected",
                        m_customExpressionName
                    ),
                ...
                if (outputId != 0)
                    UIUtils.ShowMessage(
                        UniqueId,
                        string.Format(
                            "Attempting to get value on Custom Expression '{0}' from inexisting '{1}' inout/out variable",
                            m_customExpressionName,
                            m_outputPorts[outputId].Name
                        ),
                        MessageSeverity.Error
                    );
            ...
            if (count > 0)
            {
                ...
                if (codeContainsReturn)
                {
                    ...
                    for (int i = m_firstAvailablePort; i < count; i++)
                    {
                        ...
                        if (m_inputPorts[i].DataType != WirePortDataType.OBJECT)
                        {
                            string result = m_inputPorts[i]
                                .GeneratePortInstructions(ref dataCollector);
                            if (
                                !m_inputPorts[i].IsConnected
                                && m_inputPorts[i].DataType == WirePortDataType.SAMPLERSTATE
                            )
                            {
                                result = GeneratorUtils.GenerateSamplerState(
                                    ref dataCollector,
                                    UniqueId,
                                    DefaultSamplerStateStr,
                                    VariableMode.Create
                                );
                            }
                            dataCollector.AddLocalVariable(
                ...
                else
                {
                    ...
                    if (m_mode == CustomExpressionMode.Call || m_voidMode)
                    {
                        for (int i = m_firstAvailablePort; i < count; i++)
                        {
                            int idx = i - m_firstAvailablePort;
                            if (
                                !m_items[idx].IsVariable
                                || m_items[idx].Qualifier != VariableQualifiers.In
                                || !m_inputPorts[i].IsConnected
                            )
                            {
                                string inputPortLocalVar = m_inputPorts[i].Name + OutputId;
                                string nameToReplaceRegex = string.Format(
                                    VarRegexReplacer,
                                    m_inputPorts[i].Name
                                );
                                localCode = Regex.Replace(
                                    localCode,
                                    nameToReplaceRegex,
                                    inputPortLocalVar,
                                ...
                                if (m_inputPorts[i].IsConnected)
                                {
                                    ...
                                    if (m_inputPorts[i].DataType == WirePortDataType.OBJECT)
                                    {
                                        dataCollector.AddLocalVariable(
                                            UniqueId,
                                            m_items[idx].CustomType + " " + inputPortLocalVar,
                                            result + ";"
                                        );
                                    }
                                    else
                                ...
                                else
                                {
                                    if (m_inputPorts[i].DataType == WirePortDataType.OBJECT)
                                    {
                                        dataCollector.AddLocalVariable(
                                            UniqueId,
                                            m_items[idx].CustomType + " " + inputPortLocalVar,
                                            m_inputPorts[i].WrappedInternalData + ";"
                                        );
                                    }
                                    else
                                    {
            ...
            else
            {
                ...
                else
                {
                    RegisterLocalVariable(
                        0,
                        string.Format(Constants.CodeWrapper, m_code),
                        ref dataCollector,
                        localVarName
                    );
                }
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            if (m_outputTypeIdx >= AvailableWireTypes.Length)
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Sampler types were removed as a valid output custom expression type"
                );
                m_outputTypeIdx = 1;
            }
            UpdateVoidMode();
            ...
            if (UIUtils.CurrentShaderVersion() > 12001)
            {
                ...
                {
                    m_mode = (CustomExpressionMode)
                        Enum.Parse(typeof(CustomExpressionMode), GetCurrentParam(ref nodeParams));
                }
                else
                {
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    WirePortDataType type = (WirePortDataType)
                        Enum.Parse(typeof(WirePortDataType), GetCurrentParam(ref nodeParams));
                    string internalData = GetCurrentParam(ref nodeParams);
                    VariableQualifiers qualifier = VariableQualifiers.In;
                    if (UIUtils.CurrentShaderVersion() > 12001)
                    {
                        qualifier = (VariableQualifiers)
                            Enum.Parse(typeof(VariableQualifiers), GetCurrentParam(ref nodeParams));
                    }
                    ...
                    if (UIUtils.CurrentShaderVersion() > 15607)
                    {
                        precision = (PrecisionType)
                            Enum.Parse(typeof(PrecisionType), GetCurrentParam(ref nodeParams));
                    }
                    ...
                    else
                    {
                        ...
                        m_inputPorts[m_inputPorts.Count - 1].Visible =
            ...
            if (UIUtils.CurrentShaderVersion() > 15403)
            {
                ...
                {
                    m_dependencies.Add(
                        new CustomExpressionDependency(GetCurrentParam(ref nodeParams))
                    );
                }
            }
        ...
        public void CheckDependencies(
            ref MasterNodeDataCollector dataCollector,
            ref Dictionary<int, CustomExpressionNode> examinedNodes
        )
        {
            if (
                !examinedNodes.ContainsKey(UniqueId)
                && m_mode == CustomExpressionMode.Create
                && !m_generateUniqueName
            )
            {
                ...
                for (int d = 0; d < dependencyCount; d++)
                {
                    ...
                    {
                        CustomExpressionNode dNode =
                            m_containerGraph.GetNode(m_dependencies[d].DependencyNodeId)
                            as CustomExpressionNode;
                        ...
                        if (dNode == null)
                        {
                            dNode =
                                UIUtils.CurrentWindow.OutsideGraph.GetNode(
                                    m_dependencies[d].DependencyNodeId
                                ) as CustomExpressionNode;
                        }
                ...
                dataCollector.AddFunction(
                    ExpressionName,
                    EncapsulatedCode(dataCollector.IsTemplate)
                );
            ...
            set
            {
                if (m_mode != value)
                {
                    ...
                    if (m_mode == CustomExpressionMode.Call)
                    {
                        ...
                        UIUtils.CurrentWindow.OutsideGraph.CustomExpressionOnFunctionMode.RemoveNode(
                            this
                        );
                    }
                    ...
                    {
                        UIUtils.CurrentWindow.OutsideGraph.CustomExpressionOnFunctionMode.AddNode(
                            this
                        );
                    }
                }
        ...
        {
            get { return m_customExpressionName; }
        }
        ...
        public override void RefreshExternalReferences()
        {
            ...
            for (int i = 0; i < dependencyCount; i++)
            {
                m_dependencies[i].DependencyArrayIdx =
                    UIUtils.CurrentWindow.OutsideGraph.CustomExpressionOnFunctionMode.GetNodeRegisterIdx(
                        m_dependencies[i].DependencyNodeId
                    );
            }
            //Fixing bug where user could set main output port as OBJECT
            if (
                m_outputPorts[0].DataType == WirePortDataType.OBJECT
                && (m_voidMode || m_mode == CustomExpressionMode.Call)
            )
            {
                m_outputPorts[0].ChangeType(m_inputPorts[0].DataType, false);
            }
        ...
        {
            UIUtils.CurrentWindow.OutsideGraph.CustomExpressionOnFunctionMode.UpdateDataOnNode(
                UniqueId,
                m_customExpressionName
            );
        }

        public List<CustomExpressionDependency> Dependencies
        {
