        ...
        )
        {
            ...
            if (dataCollector.IsTemplate)
            {
                if (
                    ...
                    && !string.IsNullOrEmpty(m_matrixNameHDRP)
                )
                {
                    ...
                    dataCollector.AddLocalVariable(
                        UniqueId,
                        string.Format("({0}).xyz", varName),
                        string.Format("GetCameraRelativePositionWS(({0}).xyz);", varName)
                    );
                    value = varName;
                    ...
                    dataCollector.TemplateDataCollectorInstance.CurrentSRPType
                        == TemplateSRPType.URP
                    && !string.IsNullOrEmpty(m_matrixNameURP)
                )
                {
            ...
            RegisterLocalVariable(
                0,
                string.Format("mul({0},{1})", matrixName, value),
                ref dataCollector,
                "transform" + OutputId
            );
            return GetOutputVectorItem(
