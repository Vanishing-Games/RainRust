    ...
    public sealed class RegisterLocalVarNode : ParentNode
    {
        ...
        void UpdateTitle()
        {
            SetAdditonalTitleText(
                string.Format(Constants.SubTitleVarNameFormatStr, m_variableName)
            );
        }
        ...
        public override void OnNodeLogicUpdate(DrawInfo drawInfo)
        {
            ...
            if (
                m_editingTimestampFlag
                && (EditorApplication.timeSinceStartup - m_editingTimestamp) > MaxEditingTimestamp
            )
            {
        ...
        void CheckAndChangeName()
        {
            ...
            bool isLocalVarNameAvailable =
                m_containerGraph.ParentWindow.DuplicatePrevBufferInstance.IsLocalvariableNameAvailable(
                    m_variableName
                );
            if (!isNumericName && isLocalVarNameAvailable)
            {
                m_containerGraph.ParentWindow.DuplicatePrevBufferInstance.ReleaseLocalVariableName(
                    UniqueId,
                    m_oldName
                );
                m_containerGraph.ParentWindow.DuplicatePrevBufferInstance.RegisterLocalVariableName(
                    UniqueId,
                    m_variableName
                );
                m_oldName = m_variableName;
            ...
            else
            {
                if (isNumericName)
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        string.Format(
                            "Local variable name '{0}' cannot start or be numerical values. Reverting back to '{1}'.",
                            m_variableName,
                            m_oldName
                        ),
                ...
                else if (!isLocalVarNameAvailable)
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        string.Format(
                            "Local variable name '{0}' already being used. Reverting back to '{1}'.",
                            m_variableName,
                            m_oldName
                        ),
                        MessageSeverity.Warning
            ...
            if (m_registeredGetLocalVars.Count > 0)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    EditorGUILayout.LabelField(
                        string.Format(GetLocalVarLabel, m_registeredGetLocalVars[i].UniqueId)
                    );
                    if (GUILayout.Button("\u25BA", "minibutton", GUILayout.Width(17)))
                    {
                        m_containerGraph.ParentWindow.FocusOnNode(
                            m_registeredGetLocalVars[i],
                            0,
                            false
                        );
                    }
        ...
        public List<GetLocalVarNode> NodeReferences
        {
            get { return m_registeredGetLocalVars; }
        }
    }
}
