...
namespace AmplifyShaderEditor
{
    ...
    public sealed class WireNode : ParentNode
    {
                ...
                for (int i = 0; i < m_outputPorts[portId].ExternalReferences.Count; i++)
                {
                    if (m_outputPorts[portId].ExternalReferences[i].PortId != otherPortId)
                    {
                        UIUtils.DeleteConnection(
                            true,
                            m_outputPorts[portId].ExternalReferences[i].NodeId,
                            m_outputPorts[portId].ExternalReferences[i].PortId,
                            false,
                            true
                        );
                    }
        ...
        public override void OnConnectedInputNodeChanges(
            int portId,
            int otherNodeId,
            int otherPortId,
            string name,
        ...
        public override void OnNodeLayout(DrawInfo drawInfo)
        {
            ...
            // Input Ports
            {
                ...
                m_currInputPortPos.position =
                    m_globalPosition.center - m_currInputPortPos.size * 0.5f;
                int inputCount = m_inputPorts.Count;
            ...
            // Output Ports
            {
                ...
                m_currOutputPortPos.position =
                    m_globalPosition.center - m_currOutputPortPos.size * 0.5f;
                    ...
                    if (ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD4)
                    {
                        if (m_outputPorts[i].Locked)
                            ...
                            GUI.color = m_outputPorts[i].HasCustomColor
                                ...
                                : UIUtils.GetColorForDataType(m_visualDataType, true, false);
        ...
        bool TestIfValid()
        {
            ...
            {
                if (
                    !m_containerGraph.ParentWindow.WireReferenceUtils.InputPortReference.IsValid
                    || m_containerGraph.ParentWindow.WireReferenceUtils.InputPortReference.IsValid
                        && m_containerGraph
                            .ParentWindow
                            .WireReferenceUtils
            ...
            {
                if (
                    !m_containerGraph.ParentWindow.WireReferenceUtils.OutputPortReference.IsValid
                    || m_containerGraph.ParentWindow.WireReferenceUtils.OutputPortReference.IsValid
                        && m_containerGraph
                            .ParentWindow
                            .WireReferenceUtils
        ...
        public Vector3 TangentDirection
        {
            get
            {
                ...
                {
                    otherInputNode = m_containerGraph.GetNode(
                        m_outputPorts[0].ExternalReferences[0].NodeId
                    );
                    otherInputPort = otherInputNode.GetInputPortByUniqueId(
                        m_outputPorts[0].ExternalReferences[0].PortId
                    );
                }

                if (m_inputPorts[0].ConnectionCount > 0)
                {
                    otherOutputNode = m_containerGraph.GetNode(
                        m_inputPorts[0].ExternalReferences[0].NodeId
                    );
                    otherOutputPort = otherOutputNode.GetOutputPortByUniqueId(
                        m_inputPorts[0].ExternalReferences[0].PortId
                    );
                }
                ...
                float mag =
                    Constants.HORIZONTAL_TANGENT_SIZE
        ...
        public WireReference FindNewValidInputNode(WireNode current)
        {
            ...
            {
                ParentNode node = m_containerGraph.GetNode(
                    current.InputPorts[0].ExternalReferences[0].NodeId
                );
                if (node != null)
                {
        ...
        public WireReference FindNewValidOutputNode(WireNode current)
        {
            ...
            {
                ParentNode node = m_containerGraph.GetNode(
                    current.OutputPorts[0].ExternalReferences[0].NodeId
                );
        ...
        public override void Rewire()
        {
            ...
            {
                ContainerGraph.ParentWindow.ConnectInputToOutput(
                    inputReference.NodeId,
                    inputReference.PortId,
