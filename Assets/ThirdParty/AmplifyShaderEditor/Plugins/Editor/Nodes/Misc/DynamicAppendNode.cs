...
namespace AmplifyShaderEditor
{
    ...
    public sealed class DynamicAppendNode : ParentNode
    {
        ...
        private readonly string[] m_outputValueTypes = { "Vector2", "Vector3", "Vector4", "Color" };
        ...
        void NewUpdateBehaviorConn(int portId, bool onLoading)
        {
            ...
            int channelsRequired = UIUtils.GetChannelsAmount(
                onLoading ? inputPort.DataType : inputPort.ConnectionType(0)
            );
                ...
                if (m_inputPorts[i].Visible)
                {
                    ...
                    for (int j = 0; j < usedChannels; j++)
                    {
                        if (channel < Constants.ChannelNamesVector.Length)
                            name += isColor
                                ? Constants.ChannelNamesColor[channel++]
                                : Constants.ChannelNamesVector[channel++];
                    }
        ...
        void UpdatePortTypes()
        {
            ...
            {
                int channelsRequired = m_inputPorts[usedChannels].IsConnected
                    ? UIUtils.GetChannelsAmount(m_inputPorts[usedChannels].DataType)
                    : 0;
                if (channelsRequired > 0)
                {
                    ...
                    else
                    {
                        ...
                        WirePortDataType newType = UIUtils.GetWireTypeForChannelAmount(
        ...
        )
        {
            ...
            if (
                (m_containerGraph.IsLoading || m_isNodeBeingCopied)
                && UIUtils.CurrentShaderVersion() < 13206
            )
                return;
        ...
        public override void OnInputPortDisconnected(int portId)
        {
            ...
            if (
                (UIUtils.IsLoading || m_isNodeBeingCopied)
                && UIUtils.CurrentShaderVersion() < 13206
            )
                return;
        ...
        void SetupPorts()
        {
            ...
            {
                case 0:
                    m_selectedOutputType = WirePortDataType.FLOAT2;
                    break;
                case 1:
                    m_selectedOutputType = WirePortDataType.FLOAT3;
                    break;
                case 2:
                    m_selectedOutputType = WirePortDataType.FLOAT4;
                    break;
                case 3:
                    m_selectedOutputType = WirePortDataType.COLOR;
                    break;
            }
            UpdatePortTypes();
            RenamePorts();
        ...
        public override void Draw(DrawInfo drawInfo)
        {
            ...
            if (m_dropdownEditing)
            {
                ...
                m_selectedOutputTypeInt = EditorGUIPopup(
                    m_dropdownRect,
                    m_selectedOutputTypeInt,
                    m_outputValueTypes,
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            m_selectedOutputType = (WirePortDataType)
        ...
        public override void ReadFromDeprecated(ref string[] nodeParams, Type oldType = null)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                m_inputPorts[i].FloatInternalData = Convert.ToSingle(
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (UIUtils.CurrentShaderVersion() < 13206)
            {
                ...
                for (int i = 0; i < 4 && availableChannel < 4; i++)
                {
                    ...
                    if (
                        m_inputPorts[i].IsConnected /*&& availableChannel != i*/
                    )
                    {
                ...
                if (reroutes.Count > 0)
                {
                    ...
                    {
                        int nodeId = m_inputPorts[reroutes[i].OldPortId]
                            .ExternalReferences[0]
                            .NodeId;
                        int portId = m_inputPorts[reroutes[i].OldPortId]
                            .ExternalReferences[0]
                            .PortId;
                        ...
                        m_containerGraph.DeleteConnection(
                            true,
                            UniqueId,
                            reroutes[i].OldPortId,
                            false,
                            false,
                            false
                        );
                        m_containerGraph.CreateConnection(
                            UniqueId,
                            reroutes[i].NewPortId,
                            nodeId,
                            portId,
                            false
                        );
                ...
                for (; currChannelIdx < availableChannel; currChannelIdx++)
                {
                    ...
                    {
                        int channelsAmount = UIUtils.GetChannelsAmount(
                            m_inputPorts[currChannelIdx].DataType
                        );
        ...
        void CalculatePreviewData()
        {
            ...
            {
                default:
                    m_maskValue = Vector4.zero;
                    break;
                case WirePortDataType.INT:
                case WirePortDataType.FLOAT:
                    m_maskValue = new Vector4(1, 0, 0, 0);
                    break;
                case WirePortDataType.FLOAT2:
                    m_maskValue = new Vector4(1, 1, 0, 0);
                    break;
                case WirePortDataType.FLOAT3:
                    m_maskValue = new Vector4(1, 1, 1, 0);
                    break;
                case WirePortDataType.FLOAT4:
                case WirePortDataType.COLOR:
                    m_maskValue = Vector4.one;
                    break;
            }
                ...
                case WirePortDataType.FLOAT:
                    {
                        switch (m_inputPorts[1].DataType)
                        {
                            ...
                            case WirePortDataType.INT:
                                {
                                    if (
                                        m_inputPorts[2].DataType == WirePortDataType.FLOAT
            ...
            if (m_previewMaterialPassId == -1)
            {
                ...
                if (DebugConsoleWindow.DeveloperMode)
                {
                    UIUtils.ShowMessage(
                        UniqueId,
                        "Could not find pass ID for append",
