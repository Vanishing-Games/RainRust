...
namespace AmplifyShaderEditor
{
    ...
    public class GetLocalVarNode : ParentNode
    {
        ...
        private void OnReorderEventComplete()
        {
            ...
            {
                m_referenceId = m_containerGraph.LocalVarNodes.GetNodeRegisterIdx(
                    m_currentSelected.UniqueId
                );
            }
        }
        ...
        public override void SetPreviewInputs()
        {
            ...
            if (m_currentSelected != null)
            {
                ...
                PreviewMaterial.SetTexture(
                    m_cachedPropertyId,
                    m_currentSelected.OutputPorts[0].OutputPreviewTexture
                );
            }
        }
        ...
        public override void DrawProperties()
        {
            ...
            m_referenceId = EditorGUILayoutPopup(
                Constants.AvailableReferenceStr,
                m_referenceId,
                UIUtils.LocalVarNodeArr()
            );
        ...
        void CheckForLoops()
        {
            if (
                CurrentSelected != null
                && UIUtils.DetectNodeLoopsFrom(CurrentSelected, new Dictionary<int, int>())
            )
            {
        ...
        void UpdateFromSelected()
        {
            ...
            if (m_currentSelected != null)
            {
                ...
                SetAdditonalTitleText(
                    string.Format(Constants.SubTitleVarNameFormatStr, m_currentSelected.DataToArray)
                );
            ...
            if (m_referenceId > -1)
            {
                ...
                if (m_currentSelected != null)
                {
                    if (m_currentSelected.OutputPorts[0].DataType != m_outputPorts[0].DataType)
                    {
                        m_outputPorts[0].Locked = false;
                        m_outputPorts[0]
                            .ChangeType(m_currentSelected.OutputPorts[0].DataType, false);
                    }
        ...
        )
        {
            if (m_currentSelected != null)
            {
                return m_currentSelected.GenerateShaderForOutput(
                    outputId,
                    ref dataCollector,
                    ignoreLocalvar
                );
            ...
            else
            {
                UIUtils.ShowMessage(
                    UniqueId,
                    "Get Local Var node without reference. Attempting to access inexistant local variable.",
        ...
        public override void RefreshExternalReferences()
        {
            ...
            if (UIUtils.CurrentShaderVersion() > 15)
            {
                ...
                if (
                    CurrentSelected == null
                    && UIUtils.CurrentShaderVersion() > 15500
                    && !string.IsNullOrEmpty(m_registerLocalVarName)
                )
                {
                    CurrentSelected = m_containerGraph.LocalVarNodes.GetNodeByDataToArray(
                        m_registerLocalVarName
                    );
        ...
        )
        {
            ...
            if (m_activeConnections == 1)
            {
                if (m_currentSelected != null)
                {
                    m_currentSelected.ActivateNode(
                        signalGenNodeId,
                        signalGenPortId,
                        signalGenNodeType
                    );
