    ...
    public sealed class FresnelNode : ParentNode
    {
        ...
        public override void SetPreviewInputs()
        {
            ...
            if (m_normalType == NormalType.TangentNormal && m_normalVecPort.IsConnected)
                ...
                (m_normalType == NormalType.WorldNormal || m_normalType == NormalType.HalfVector)
                && m_normalVecPort.IsConnected
                && !m_viewVecPort.IsConnected
            )
                m_previewMaterialPassId = 1;
            else if (
                m_normalType == NormalType.HalfVector
                && !m_normalVecPort.IsConnected
                && !m_viewVecPort.IsConnected
            )
        ...
        public override void DrawProperties()
        {
            ...
            if (!m_biasPort.IsConnected && m_biasPort.Visible)
                m_biasPort.FloatInternalData = EditorGUILayoutFloatField(
                    m_biasPort.Name,
                    m_biasPort.FloatInternalData
                );
            if (!m_scalePort.IsConnected && m_scalePort.Visible)
                m_scalePort.FloatInternalData = EditorGUILayoutFloatField(
                    m_scalePort.Name,
                    m_scalePort.FloatInternalData
                );
            if (!m_powerPort.IsConnected && m_powerPort.Visible)
                m_powerPort.FloatInternalData = EditorGUILayoutFloatField(
                    m_powerPort.Name,
                    m_powerPort.FloatInternalData
                );
        ...
        )
        {
            ...
            if (m_viewType == ViewType.ViewDir)
            {
                if (m_viewVecPort.IsConnected)
                    ...
                    viewdir = GeneratorUtils.GenerateViewDirection(
                        ref dataCollector,
                        UniqueId,
            ...
            else
            {
                if (m_viewVecPort.IsConnected)
                    ...
                    viewdir = GeneratorUtils.GenerateWorldLightDirection(
                        ref dataCollector,
                        UniqueId,
            ...
            if (m_normalType == NormalType.WorldNormal || m_normalType == NormalType.TangentNormal)
            {
                if (m_normalVecPort.IsConnected)
                {
                    ...
                    if (dataCollector.IsFragmentCategory)
                    {
                        ...
                        if (m_normalType == NormalType.TangentNormal)
                        {
                            ...
                            {
                                normal = dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                                    UniqueId,
                                    CurrentPrecisionType,
                                    normal,
                                    OutputId
                    ...
                    else
                    {
                        if (m_normalType == NormalType.TangentNormal)
                        {
                            string wtMatrix = GeneratorUtils.GenerateWorldToTangentMatrix(
                                ref dataCollector,
                                UniqueId,
                                CurrentPrecisionType
                            );
                ...
                else
                {
                    ...
                    if (dataCollector.IsTemplate)
                        normal = dataCollector.TemplateDataCollectorInstance.GetWorldNormal(
                            CurrentPrecisionType,
                            normalize: (dataCollector.DirtyNormal && m_normalizeVectors)
                        );
                    else
                        normal = GeneratorUtils.GenerateWorldNormal(
                            ref dataCollector,
                            UniqueId,
                            (dataCollector.DirtyNormal && m_normalizeVectors)
                        );
            ...
            else
            {
                ...
                if (!m_normalVecPort.IsConnected)
                {
                    string halfView = GeneratorUtils.GenerateViewDirection(
                        ref dataCollector,
                        UniqueId,
                        space: ViewSpace.World
                    );
                    string halfLight = GeneratorUtils.GenerateWorldLightDirection(
                        ref dataCollector,
                        UniqueId,
                        CurrentPrecisionType
                    );
            ...
            dataCollector.AddLocalVariable(
                ...
                CurrentPrecisionType,
                WirePortDataType.FLOAT,
                fresnelNDotVLocalVar,
                fresnelNDotVLocalValue
            );
            ...
            switch (m_fresnelType)
            {
                ...
                case FresnelType.Standard:
                    {
                        string powOp = m_safePower
                            ? string.Format(
                                "pow( max( 1.0 - {0} , 0.0001 ), {1} )",
                                fresnelNDotVLocalVar,
                                power
                            )
                ...
                case FresnelType.SchlickIOR:
                    {
                        ...
                        string iorPowOp = m_safePower
                            ? string.Format(
                                "pow( max( ( 1 - {0} ) / ( 1 + {0} ) , 0.0001 ), 2 )",
                                iorVarName
                            )
                        ...
                        string fresnelPowOp = m_safePower
                            ? string.Format(
                                "pow( max( 1.0 - {0} , 0.0001 ), 5 )",
                                fresnelNDotVLocalVar
                            )
        ...
        public override void ReadFromString(ref string[] nodeParams)
        {
            ...
            {
                m_fresnelType = (FresnelType)
                    Enum.Parse(typeof(FresnelType), GetCurrentParam(ref nodeParams));
                m_normalType = (NormalType)
                    Enum.Parse(typeof(NormalType), GetCurrentParam(ref nodeParams));
                m_viewType = (ViewType)
                    Enum.Parse(typeof(ViewType), GetCurrentParam(ref nodeParams));
