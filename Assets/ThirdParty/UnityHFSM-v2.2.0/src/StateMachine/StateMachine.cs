...
namespace UnityHFSM
{
            ...
            IActionable<TEvent>
    {
        ...
        private static readonly List<TransitionBase<TStateId>> noTransitions = new List<
            TransitionBase<TStateId>
        >(0);
        private static readonly Dictionary<
            TEvent,
            List<TransitionBase<TStateId>>
        > noTriggerTransitions = new Dictionary<TEvent, List<TransitionBase<TStateId>>>(0);
        ...
        private Dictionary<TEvent, List<TransitionBase<TStateId>>> activeTriggerTransitions =
        ...
        private readonly Dictionary<
            TEvent,
            List<TransitionBase<TStateId>>
        > triggerTransitionsFromAny = new Dictionary<TEvent, List<TransitionBase<TStateId>>>();
        ...
        public void AddTwoWayTransition(TransitionBase<TStateId> transition)
        {
            ...
            ReverseTransition<TStateId> reverse = new ReverseTransition<TStateId>(
                transition,
                false
            );
        ...
        /// <param name="transition">The transition instance. The "from" and "to" fields can be
        /// 	left empty, as they have no meaning in this context.</param>
        public void AddExitTriggerTransitionFromAny(
            TEvent trigger,
        ...
        public StateMachine<string, string, string> this[TStateId name]
        {
            get
            {
                ...
                StateMachine<string, string, string> subFsm =
                    state as StateMachine<string, string, string>;

                if (subFsm == null)
                {
                    throw UnityHFSM.Exceptions.Common.QuickIndexerMisusedForGettingState(
                        this,
                        name.ToString()
                    );
                }
        ...
        {
            return stateBundlesByName
                .Values.Where(bundle => bundle.state != null)
        ...
        {
            return stateBundlesByName
                .Values.Where(bundle => bundle.transitions != null)
        ...
        public IReadOnlyDictionary<
            TEvent,
            IReadOnlyList<TransitionBase<TStateId>>
        > GetAllTriggerTransitions()
        {
            ...
            foreach (var bundle in stateBundlesByName.Values)
            {
                ...
                foreach (
                    (
                        TEvent trigger,
                        List<TransitionBase<TStateId>> transitions
                    ) in bundle.triggerToTransitions
                )
                {
                    if (
                        !transitionsByEvent.TryGetValue(
                            trigger,
            ...
            foreach (
                (TEvent trigger, List<TransitionBase<TStateId>> transitions) in transitionsByEvent
            )
            {
