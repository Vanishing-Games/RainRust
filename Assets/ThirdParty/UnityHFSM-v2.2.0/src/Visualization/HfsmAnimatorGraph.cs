    ...
    public static class HfsmAnimatorGraph
    {
        ...
        private class StateMachinePositionInformation
        {
            ...
            private readonly Dictionary<
                string,
                StateMachinePositionInformation
            > childStateMachines = new Dictionary<string, StateMachinePositionInformation>();
            ...
            public static StateMachinePositionInformation ExtractFromAnimator(
            ...
            public void Apply(AnimatorStateMachine animator)
            {
                ...
                animator.stateMachines = animator
                    .stateMachines.Select(stateMachine =>
                    {
                ...
                foreach (var stateMachine in animator.stateMachines)
                {
                    ...
                    if (
                        childStateMachines.TryGetValue(
                            name,
                            out StateMachinePositionInformation positionInformation
                        )
                    )
                    {
        ...
        private class AnimatorGraphGenerator : IStateMachineHierarchyVisitor
        {
            ...
            > animatorStateMachines = new Dictionary<StateMachinePath, AnimatorStateMachine>();
            ...
            public void VisitRegularState<TStateId>(
            ...
            )
            {
                ...
                foreach (
                    var transition in fsm.GetAllTriggerTransitions().Values.SelectMany(list => list)
                )
                {
                    AddTransition(animator, fsmPath, transition);
                }
                ...
                foreach (
                    var transition in fsm.GetAllTriggerTransitionsFromAny()
            ...
            private void AddExitTransition(AnimatorStateMachine animator, StateMachinePath fromPath)
            {
                if (animatorStates.TryGetValue(fromPath, out AnimatorState state))
                {
                    ...
                    animatorStateMachines.TryGetValue(
                        fromPath,
                        out AnimatorStateMachine childStateMachine
                    )
                )
                {
                    SetupAnimatorTransition(
                        animator.AddStateMachineExitTransition(childStateMachine)
                    );
                }
            }
            ...
            private void AddTransition(
                AnimatorStateMachine animator,
                StateMachinePath fromPath,
                StateMachinePath toPath
            )
            {
            ...
            )
            {
                ...
                {
                    Debug.LogWarning(
                        "Exit transitions from any are currently not supported in the animator graph."
                    );
                    return;
                }
        ...
        /// </returns>
        public static (AnimatorController, IPreviewer) CreateAnimatorFromStateMachine<
            TOwnId,
            TStateId,
            TEvent
        >(
        ...
        )
        {
            if (fsm.GetAllStates().Count == 0)
            {
                throw new InvalidOperationException(
                    UnityHFSM.Exceptions.ExceptionFormatter.Format(
            ...
            var animator = AssetDatabase.LoadAssetAtPath<AnimatorController>(
                fullPathToDebugAnimator
            );
            StateNamingInformation stateNamingInformation;
            if (animator == null)
            {
                (animator, stateNamingInformation) = CreateNewAnimatorFromStateMachine(
                    fsm,
                    fullPathToDebugAnimator
                );
            }
        ...
        private static (
            AnimatorController,
            StateNamingInformation
        ) CreateNewAnimatorFromStateMachine<TOwnId, TStateId, TEvent>(
            StateMachine<TOwnId, TStateId, TEvent> fsm,
            string outputPath
        )
        {
        ...
        private static StateNamingInformation UpdateExistingAnimatorFromStateMachine<
            TOwnId,
            TStateId,
            TEvent
        >(AnimatorController animator, StateMachine<TOwnId, TStateId, TEvent> fsm)
        {
            ...
            {
                layout = StateMachinePositionInformation.ExtractFromAnimator(
                    animator.layers[0].stateMachine
                );
            }
            ...
            animator.AddParameter(
                AnimatorGraphGenerator.dummyProperty,
