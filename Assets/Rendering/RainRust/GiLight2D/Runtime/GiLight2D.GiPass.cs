...
namespace GiLight2D
{
    public partial class GiLight2D
    {
        private class GiPass : ScriptableRenderPass
        {
            ...
            public void Init()
            {
                renderPassEvent = _owner.m_Event;
                ...
                _bounce = new RenderTargetFlip(
                    new RenderTarget().Allocate($"{nameof(_bounce)}_a"),
                    new RenderTarget().Allocate($"{nameof(_bounce)}_b")
                );
                _bounceTmp = new RenderTarget().Allocate(nameof(_bounceTmp));
                _alpha = new RenderTarget().Allocate(nameof(_alpha));
                _jfa = new RenderTargetFlip(
                    new RenderTarget().Allocate($"{nameof(_jfa)}_a"),
                    new RenderTarget().Allocate($"{nameof(_jfa)}_b")
                );
                _pp = new RenderTargetPostProcess(
                    new RenderTarget().Allocate($"{nameof(_pp)}_a"),
                    new RenderTarget().Allocate($"{nameof(_pp)}_b")
                );
                _output = new RenderTarget().Allocate(_owner._output._globalTexture);

                _rlp = new RendererListParams(
                    new CullingResults(),
                    new DrawingSettings(),
                    new FilteringSettings(RenderQueueRange.all, _owner._mask)
                );
            ...
            )
            {
                ...
                var piercing =
                    Mathf.LerpUnclamped(0f, 7f, _owner._traceOptions._piercing)
                    / (800f / Mathf.Max(bounceRes.x, bounceRes.y));
                _owner.m_GiMat.SetVector(
                    s_AspectId,
                    new Vector4(
                        aspectRatioX,
                        aspectRatioY,
                        piercing * aspectRatioX / bounceRes.x,
                        piercing / bounceRes.y
                    )
                );
                _owner.m_GiMat.SetFloat(s_SamplesId, _owner._rays);
                _owner.m_GiMat.SetFloat(s_IntensityId, _owner._intensity.Value);
                _owner.m_GiMat.SetFloat(
                    s_PowerId,
                    _owner._distance / (camera.orthographicSize * 2f)
                );

                _owner._distMat.SetVector(
                    s_AspectId,
                    new Vector4(aspectRatioX, aspectRatioY, 0, 0)
                );
                ...
                if (_owner._depthStencil)
                {
                    ...
                    desc.depthStencilFormat = renderingData
                        .cameraData
                ...
                if (_owner.HasGiBorder)
                {
                    ...
                    var planes = GeometryUtility.CalculateFrustumPlanes(
                        cullingParameters.cullingMatrix
                    );
                    for (var n = 0; n < 6; n++)
                ...
                cmd.ClearRenderTarget(
                    _owner._depthStencil ? RTClearFlags.All : RTClearFlags.Color,
                    Color.clear,
                    1f,
                    0
                );
                ...
                _rlp.drawSettings = CreateDrawingSettings(
                    k_ShaderTags,
                    ref renderingData,
                ...
                if (_owner.HasGiBorder)
                {
                    RenderingUtils.SetViewAndProjectionMatrices(
                        ...
                        cameraData.GetGPUProjectionMatrix(),
                        false
                    );
                }
                ...
                switch (_owner._noiseOptions._noise)
                {
                    case NoiseSource.None:
                        {
                            _owner.m_GiMat.SetTexture(s_NoiseTexId, Texture2D.blackTexture);
                        }
                        break;
                    case NoiseSource.Texture:
                        {
                            ...
                            if (_owner._noiseOptions._orthoRelative.Enabled)
                            {
                                ...
                                var scaleX =
                                    camWidth
                                    / (_owner._noiseOptions._orthoRelative.Value * 2f * aspect);
                                var scaleY =
                                    camHeight / (_owner._noiseOptions._orthoRelative.Value * 2f);

                                xOffset +=
                                    ((camera.transform.position.x % camWidth) / camWidth) * scaleX
                                    - (scaleX - 1f) * camWidth * .5f / camWidth;
                                yOffset +=
                                    ((camera.transform.position.y % camHeight) / camHeight) * scaleY
                                    - (scaleY - 1f) * camHeight * .5f / camHeight;
                            ...
                            _owner.m_GiMat.SetVector(
                                s_NoiseTilingOffsetId,
                                new Vector4(xTiling, yTiling, xOffset, yOffset)
                            );
                            _owner.m_GiMat.SetTexture(s_NoiseTexId, k_Noise);
                        }
                        break;
                    case NoiseSource.Shader:
                        {
                            _owner.m_GiMat.SetVector(
                                s_NoiseTilingOffsetId,
                                new Vector4(Random.value, Random.value, 0, 0)
                            );
                        }
                        break;
                    default:
                ...
                if (_owner._traceOptions._enable)
                {
                    ...
                    cmd.SetGlobalFloat(
                        s_IntensityBounceId,
                        _owner._traceOptions._intencity * _owner._traceOptions._scales[0]
                    );
                    ...
                    cmd.DrawMesh(k_ScreenMesh, Matrix4x4.identity, _owner.m_GiMat, 0, 1);
                    ...
                    for (var n = 1; n < _owner._traceOptions._bounces; n++)
                    {
                        ...
                        cmd.SetGlobalFloat(
                            s_IntensityBounceId,
                            _owner._traceOptions._intencity * _owner._traceOptions._scales[n]
                        );
                ...
                _pp.Setup(cmd, in desc, output, passes, _owner.m_GiMat);

                if (_hasBlurImpact())
                {
                    var step = new Vector4(
                        _owner._blurOptions._step * (desc.width / desc.height),
                        _owner._blurOptions._step,
                        0,
                        0
                    );
