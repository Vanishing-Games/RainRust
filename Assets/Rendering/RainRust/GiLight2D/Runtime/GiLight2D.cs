...
namespace GiLight2D
{
    public partial class GiLight2D : ScriptableRendererFeature
    {
        ...
        [SerializeField]
        private RenderPassEvent m_Event = RenderPassEvent.BeforeRenderingOpaques;
        ...
        private Optional<LayerMask> m_volume = new Optional<LayerMask>(false);
        ...
        [SerializeField]
        [Tooltip(
            "Attach depth stencil buffer for Gi rendering. Allows stencil mask interaction and z culling"
        )]
        ...
        private Optional<RangeFloat> _distOffset = new Optional<RangeFloat>(
            new RangeFloat(new Vector2(-.01f, .1f), .0f),
            false
        );
        ...
        [SerializeField]
        [Tooltip(
            "Additional orthographic camera space, to make objects visible outside of the camera frame"
        )]
        private Optional<RangeFloat> _border = new Optional<RangeFloat>(
            new RangeFloat(new Vector2(.0f, 3f), 0f),
            false
        );
        ...
        private bool m_RunInSceneView;

        [SerializeField]
        private ShaderCollection _shaders = new ShaderCollection();

        private GiPass m_GiPass;

        private Material m_GiMat;
        ...
        private RenderTextureDescriptor _rtDesc = new RenderTextureDescriptor(
            0,
            0,
            GraphicsFormat.None,
            0,
        ...
        {
            get => _scaleMode._ratio;
            set => _scaleMode._ratio = value;
        }
        public bool Blur
        {
            get => _blurOptions._enable;
            set => _blurOptions._enable = value;
        }
        public float BlurStep
        {
            get => _blurOptions._step;
            set => _blurOptions._step = value;
        }
        ...
        public int BounceCount
        {
            set
            {
                ...
                if (isEnabled != _traceOptions._enable)
                {
                    ...
                    if (isEnabled)
                    {
                        m_GiMat.EnableKeyword("RAY_BOUNCES");
                    }
                    else
                    {
                        m_GiMat.DisableKeyword("RAY_BOUNCES");
                    }
                }
        ...
        internal class BlurOptions
        {
            ...
            [Tooltip(
                "Blur distance in uv coords, if disabled step will be set to one pixel per sample"
            )]
            [Range(0, 0.003f)]
            public float _step;
        }
        ...
        internal class NoiseOptions
        {
            ...
            [Tooltip(
                "Noise would be scaled and translated relative to the camera world position and ortho size"
            )]
        ...
        public enum ScaleMode
        {
            ...
            [Tooltip(
                "Scale always has fixed frame height, but aspect still will be taken from the camera"
            )]
            Fixed,
        }
        ...
        {
            m_GiPass = new GiPass() { _owner = this };
            m_GiPass.Init();
            ...
            SetNoiseState(_noiseOptions._noise);

            m_stack = m_volume.Enabled
                ? VolumeManager.instance.CreateStack()
            ...
            if (k_ScreenMesh == null)
            {
                ...
                InitScreenMesh(k_ScreenMesh, Matrix4x4.identity);
            }

            if (k_ShaderTags == null)
            {
                k_ShaderTags = new List<ShaderTagId>(
                    new[]
                    {
        ...
        )
        {
            ...
            ApplyFromPostProcess();

            SetupDesc(in renderingData);

            if (_outputOverride != DebugOutput.None)
            {
                m_GiPass.renderPassEvent = RenderPassEvent.AfterRenderingPostProcessing;
            }
            else
            {
                m_GiPass.renderPassEvent = m_Event;
            }

            renderer.EnqueuePass(m_GiPass);
        }

        private void ApplyFromPostProcess()
        {
            if (m_volume.Enabled)
                VolumeManager.instance.Update(m_stack, null, m_volume.Value);

            var settings = m_stack.GetComponent<GiLightVol>();
        ...
        private void InitMaterials()
        {
            ...
            m_GiMat = new Material(_shaders._gi);
        ...
        private void SetupDesc(in RenderingData renderingData)
        {
            ...
            _rtRes = _scaleMode._scaleMode switch
            {
                ...
                ScaleMode.Fixed => new Vector2Int(
                    Mathf.FloorToInt(
                        (camDesc.width / (float)camDesc.height)
                            * _scaleMode._height
                            * _scaleMode._ratio
                    ),
            ...
            m_GiMat.SetVector(s_ScaleId, new Vector4(uvScale, uvScale, 1f, 1f));
        ...
        private void InitNoise()
        {
            ...
            try
            {
                ...
                if (_noiseOptions._pattern == NoiseTexture.Texture)
                {
                    ...
                    _noiseTiling.x =
                        width > k_Noise.width
                            ? width / (float)k_Noise.width
                            : k_Noise.width / (float)width;
                    _noiseTiling.y =
                        height > k_Noise.height
                            ? height / (float)k_Noise.height
                            : k_Noise.height / (float)height;
                    return;
                }
                ...
                k_Noise.filterMode = _noiseOptions._bilinear
                    ? FilterMode.Bilinear
                    : FilterMode.Point;
        ...
        private void SetNoiseState(NoiseSource value)
        {
            ...
            switch (_noiseOptions._noise)
            {
                ...
                case NoiseSource.Texture:
                    m_GiMat.DisableKeyword("TEXTURE_RANDOM");
                    break;
                case NoiseSource.Shader:
                    m_GiMat.DisableKeyword("FRAGMENT_RANDOM");
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
            ...
            switch (_noiseOptions._noise)
            {
                case NoiseSource.None:
                case NoiseSource.Texture:
                    m_GiMat.EnableKeyword("TEXTURE_RANDOM");
                    break;
                case NoiseSource.Shader:
                    m_GiMat.EnableKeyword("FRAGMENT_RANDOM");
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
        ...
        private void _setSteps(RaySteps steps)
        {
            ...
            {
                case RaySteps.N4:
                    m_GiMat.DisableKeyword("STEPS_4");
                    break;
                case RaySteps.N6:
                    m_GiMat.DisableKeyword("STEPS_6");
                    break;
                case RaySteps.N8:
                    m_GiMat.DisableKeyword("STEPS_8");
                    break;
                case RaySteps.N12:
                    m_GiMat.DisableKeyword("STEPS_12");
                    break;
                case RaySteps.N16:
                    m_GiMat.DisableKeyword("STEPS_16");
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
            ...
            {
                case RaySteps.N4:
                    m_GiMat.EnableKeyword("STEPS_4");
                    break;
                case RaySteps.N6:
                    m_GiMat.EnableKeyword("STEPS_6");
                    break;
                case RaySteps.N8:
                    m_GiMat.EnableKeyword("STEPS_8");
                    break;
                case RaySteps.N12:
                    m_GiMat.EnableKeyword("STEPS_12");
                    break;
                case RaySteps.N16:
                    m_GiMat.EnableKeyword("STEPS_16");
                    break;
                default:
                    throw new ArgumentOutOfRangeException();
            }
        ...
        private void _setAlpha(Alpha alpha)
        {
            ...
            {
                case Alpha.One:
                    m_GiMat.DisableKeyword("ONE_ALPHA");
                    break;
                case Alpha.Mask:
                    m_GiMat.DisableKeyword("OBJECTS_MASK_ALPHA");
                    break;
                case Alpha.Blend:
                    m_GiMat.DisableKeyword("NORMALIZED_ALPHA");
                    break;
                default:
            ...
            {
                case Alpha.One:
                    m_GiMat.EnableKeyword("ONE_ALPHA");
                    break;
                case Alpha.Mask:
                    m_GiMat.EnableKeyword("OBJECTS_MASK_ALPHA");
                    break;
                case Alpha.Blend:
                    m_GiMat.EnableKeyword("NORMALIZED_ALPHA");
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(alpha), alpha, null);
            }
            ...
            {
                m_GiMat.SetInt(s_SrcMode, (int)BlendMode.SrcAlpha);
                m_GiMat.SetInt(s_DstMode, (int)BlendMode.OneMinusSrcAlpha);
            }
            else
            {
                m_GiMat.SetInt(s_SrcMode, (int)BlendMode.One);
                m_GiMat.SetInt(s_DstMode, (int)BlendMode.Zero);
            }
        }

        private static void InitScreenMesh(Mesh mesh, Matrix4x4 mat)
        {
