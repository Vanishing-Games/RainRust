...
{
    public class OdinAssetTableCollectionEditor
        : OdinTableCollectionEditor<AssetTableCollection, AssetTable, AssetTableEntry>
    {
        ...
        public OdinAssetTableCollectionEditor(
        ...
        {
            this.setEntryAssetTypeAction =
                OdinLocalizationReflectionValues.Create_AssetTableCollection_SetEntryAssetType_PrivateMethod_Delegate(
                    this.Collection
                );

            this.removeEntryAssetTypeAction =
                OdinLocalizationReflectionValues.Create_AssetTableCollection_RemoveEntryAssetType_PrivateMethod_Delegate(
                    this.Collection
                );
            ...
            this.OnAssetTableEntryRemoved = (collection, table, entry, guid) =>
            {
                ...
                SharedTableData.SharedTableEntry sharedEntry = table.SharedData.GetEntry(
                    entry.KeyId
                );
                    ...
                    switch (table.Type)
                    {
                        ...
                        case OdinGUITable<AssetTable>.GUITableType.Default:
                            ...
                            if (
                                ...
                                && EditorWindow.focusedWindow == this.RelatedWindow
                            )
                            {
                                switch (Event.current.keyCode)
                                {
                                    ...
                                    case KeyCode.Backspace:
                                        ...
                                        this.AssignObjectToSharedEntry(
                                            sharedEntry,
                                            table.Asset,
                                            null
                                        );
        ...
        )
        {
            ...
            if (!isDraggingControls && !(asset is DefaultAsset))
            {
                ...
                var dropValue =
                    DragAndDropUtilities.DragAndDropZone(
                        fullRect,
                        asset,
                        entryAssetType,
                        true,
            ...
            else if (!hasThumbnail)
            {
                GUI.Label(
                    rect.AlignMiddle(20),
                    GUIHelper.TempContent(
                        $"{asset.name} ({entryAssetType.Name})",
                ...
                if (entryAssetType != typeof(UnityEngine.Object))
                {
                    ...
                    GUI.Label(
                        openInspectorRect,
                        GUIHelper.TempContent(string.Empty, "Inspect Object")
                    );
                    GUI.Label(
                        openExplorerRect,
                        GUIHelper.TempContent(string.Empty, "Show In Explorer")
                    );
            ...
            if (
                !isDraggingControls
                && dragAndDropId != 0
                && DragAndDropUtilities.IsDragging
                && DragAndDropUtilities.HoveringAcceptedDropZone == dragAndDropId
            )
            {
                GUI.DrawTexture(
                    fullRect,
        ...
        private void ShowObjectPickerForEntry(AssetTableEntry entry)
        {
            ...
            Type objType =
                this.UserSpecifiedCollectionType ?? this.Collection.GetEntryAssetType(entry.KeyId);
        ...
        private void HandleObjectPickerUpdates()
        {
            ...
            {
                if (
                    OdinObjectSelector.IsReadyToClaim(
                        this.RelatedWindow,
                        OdinObjectSelectorIds.LOCALIZATION_EDITOR
                    )
                )
                ...
                else
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    var changedValue = OdinObjectSelector.GetChangedObject<UnityEngine.Object>(
                        null,
                        this.RelatedWindow,
                        OdinObjectSelectorIds.LOCALIZATION_EDITOR
                    );
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        {
                            this.pickedEntry.Guid = AssetDatabase.AssetPathToGUID(
                                AssetDatabase.GetAssetPath(changedValue)
                            );
                        }
                    }
                }
            ...
            {
                Debug.LogError(
                    $"{nameof(NullReferenceException)}: The asset '{selectedObj ?? "<ASSET NOT FOUND>"}' is NULL.\n{exception}"
                );
            }
#else
...
#if true
        private AssetTableEntry AssignObjectToSharedEntry(
            SharedTableData.SharedTableEntry sharedEntry,
            AssetTable table,
        ...
        )
        {
            ...
            if (obj == null)
            {
                ...
                entry =
                    table.GetEntry(sharedEntry.Id) ?? table.AddEntry(sharedEntry.Id, string.Empty);
            }
            else
            {
        ...
        private void AssignObjectToSelectorEntry(UnityEngine.Object obj)
        {
            ...
            try
            {
                ...
                if (obj == null)
                {
                    this.CustomRemoveAssetFromTable(
                        (AssetTable)this.pickedEntry.Table,
                        this.pickedEntry.SharedEntry.Id,
                        true
                    );
                    ...
                    this.pickedEntry =
                        ...
                        ?? table.AddEntry(sharedEntry.Id, string.Empty);
                }
                else
                {
                    this.Collection.AddAssetToTable(
                        (AssetTable)this.pickedEntry.Table,
                        this.pickedEntry.SharedEntry.Id,
                        obj,
                        true
        ...
        public void CustomRemoveAssetFromTable(
            AssetTable table,
        ...
        {
            if (
                !OdinLocalizationReflectionValues.HasAPIForCustomUndo
                || !OdinLocalizationConfig.Instance.useCustomUndoHandlingForAssetCollections
            )
            {
            ...
            {
                Undo.RecordObjects(
                    new UnityEngine.Object[] { table, table.SharedData },
            ...
            AddressableAssetSettings addressableAssetSettings =
                OdinLocalizationEditorSettings.GetAddressableAssetSettings(false);
            ...
            ReadOnlyCollection<AssetTableCollection> assetTableCollections =
                LocalizationEditorSettings.GetAssetTableCollections();

            foreach (AssetTableCollection collection in assetTableCollections)
            {
                if (
                    collection.GetTable(table.LocaleIdentifier)
                        is AssetTable tableWithMatchingLocaleId
            ...
            AddressableAssetEntry assetEntry = addressableAssetSettings.FindAssetEntry(
                removedAssetGuid
            );
            ...
            if (assetEntry != null)
            {
                ...
                var assetLabel = (string)
                    OdinLocalizationReflectionValues.FormatAssetLabelMethod.Invoke(
                        null,
                        new object[] { table.LocaleIdentifier }
                    );

                assetEntry.SetLabel(assetLabel, false);

                OdinLocalizationReflectionValues.UpdateAssetGroupMethod.Invoke(
                    this.Collection,
                    new object[] { addressableAssetSettings, assetEntry, createUndo }
                );
            }

            OdinLocalizationEvents.RaiseAssetTableEntryRemoved(
                this.Collection,
                table,
                tableEntry,
                removedAssetGuid
