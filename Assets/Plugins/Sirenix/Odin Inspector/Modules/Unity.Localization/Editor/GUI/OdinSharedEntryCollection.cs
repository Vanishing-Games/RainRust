...
namespace Sirenix.OdinInspector.Modules.Localization.Editor
{
    public class OdinSharedEntryCollection
    {
        ...
        public List<SharedTableData.SharedTableEntry> Entries =>
            this.IsSorted ? this.SortedEntries : this.Collection.SharedData.Entries;
        ...
        public bool IsVisible(SharedTableData.SharedTableEntry sharedEntry)
        {
            return !this.IsSearching
                || (this.IsSearching && this.FilteredEntries.Contains(sharedEntry));
        }
                        ...
                        switch (table.Asset)
                        {
                            case AssetTable assetTable:
                                ...
                                for (var j = 0; j < this.Length; j++)
                                {
                                    ...
                                    Type assetType = assetCollection.GetEntryAssetType(
                                        sharedEntry.Id
                                    );

                                    UnityEngine.Object asset = OdinLocalizationAssetCache.Get(
                                        sharedEntry,
                                        assetTable,
                                        assetType
                                    );
        ...
        public void SortByKeys(bool preserveCurrentOrder)
        {
            switch (this.CurrentSortOrderState)
            {
                case SortOrderState.Ascending:
                    ...
                    {
                        List<SharedTableData.SharedTableEntry> result = this
                            .Collection.SharedData.Entries.OrderBy(
                                entry => entry.Key,
                                this.stringComparer
                            )
                ...
                case SortOrderState.Descending:
                    ...
                    {
                        List<SharedTableData.SharedTableEntry> result = this
                            .Collection.SharedData.Entries.OrderByDescending(
                                entry => entry.Key,
                                this.stringComparer
                            )
                            .ThenBy(this.GetOrderIndex)
        ...
        public void SortByAssetTable(
            AssetTableCollection collection,
            AssetTable table,
            bool preserveCurrentOrder
        )
        {
            switch (this.CurrentSortOrderState)
            {
            ...
            switch (this.CurrentSortOrderState)
            {
                case SortOrderState.Ascending:
                    ...
                    if (preserveCurrentOrder)
                    {
                        List<SharedTableData.SharedTableEntry> result = this
                            .Collection.SharedData.Entries.OrderBy(
                                entry => GetStringFromEntry(entry, table),
                                this.stringComparer
                            )
                            .ThenBy(this.GetOrderIndex)
                            .ToList();
        ...
        private static string GetStringFromEntry(
        ...
        public void MoveEntry(int from, int to)
        {
            ...
            OdinLocalizationEvents.RaiseTableEntryModified(
                this.Collection.SharedData.Entries[from]
            );
