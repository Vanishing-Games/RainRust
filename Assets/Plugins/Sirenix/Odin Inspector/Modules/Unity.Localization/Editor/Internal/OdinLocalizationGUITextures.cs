    ...
    public static class OdinLocalizationGUITextures
    {
        ...
        public static Texture2D LeftToRightFade
        {
            get
            {
                ...
                leftToRightFade = new Texture2D(32, 32) { hideFlags = HideFlags.HideAndDontSave };
                ...
                for (var x = 0; x < 32; x++)
                {
                    for (var y = 0; y < 32; y++)
                    {
                        pixels[index++] = Color.Lerp(
                            Color.white,
                            Color.clear,
                            Mathf.Pow(y / 31.0f, FADE_STRENGTH)
                        );
                    }
                }
        ...
        public static Texture2D TopToBottomFade
        {
            get
            {
                ...
                topToBottomFade = new Texture2D(32, 32) { hideFlags = HideFlags.HideAndDontSave };
                ...
                for (var x = 0; x < 32; x++)
                {
                    for (var y = 0; y < 32; y++)
                    {
                        pixels[index++] = Color.Lerp(
                            Color.white,
                            Color.clear,
                            Mathf.Pow(1.0f - x / 31.0f, FADE_STRENGTH)
                        );
                    }
                }
        ...
        public static Texture2D BottomToTopFade
        {
            get
            {
                ...
                bottomToTopFade = new Texture2D(32, 32) { hideFlags = HideFlags.HideAndDontSave };
                ...
                for (var x = 0; x < 32; x++)
                {
                    for (var y = 0; y < 32; y++)
                    {
                        pixels[index++] = Color.Lerp(
                            Color.white,
                            Color.clear,
                            Mathf.Pow(x / 31.0f, FADE_STRENGTH)
                        );
                    }
                }
