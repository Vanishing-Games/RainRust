...
namespace Sirenix.OdinInspector.Modules.Localization.Editor
{
    public class OdinLocalizationCreateTableMenu
    {
        ...
        private string FolderPath =>
            string.IsNullOrEmpty(this.Folder) ? "Assets" : $"Assets/{this.Folder}";
        ...
        [InfoBox(
            "The directory is not found, this will create a new directory on creation.",
            visibleIfMemberName: nameof(ShowFolderInfoBox)
        )]
        ...
        public void Create()
        {
            ...
            switch (this.Type)
            {
                ...
                case TableCollectionType.AssetTableCollection:
                    LocalizationEditorSettings.CreateAssetTableCollection(
                        this.Name,
                        this.FolderPath,
                        collectionLocales
                    );
        ...
        private bool ValidateName(string name)
        {
            ...
            bool isTableNameValid = OdinLocalizationEditorSettings.IsTableNameValid(
                collectionType,
                name,
                out string localizationErrorMsg
            );
    ...
    public class OdinLocalizationEditorWindow : OdinMenuEditorWindow, IDisposable
    {
        ...
        public class WindowState : IDisposable
        {
            ...
            public void Save()
            {
                ...
                EditorPrefs.SetFloat(
                    $"{EditorPrefsKey}_RightMenuTopHeight",
                    this.RightMenuTopPanelHeight
                );
                EditorPrefs.SetFloat(
                    $"{EditorPrefsKey}_LastOpenRightMenuWidth",
                    this.LastOpenRightMenuWidth
                );
            }
            ...
            public void Load()
            {
                ...
                this.RightMenuTopPanelHeight = EditorPrefs.GetFloat(
                    $"{EditorPrefsKey}_RightMenuTopHeight"
                );
                this.LastOpenRightMenuWidth = EditorPrefs.GetFloat(
                    $"{EditorPrefsKey}_LastOpenRightMenuWidth"
                );
            }
        ...
        protected override void OnImGUI()
        {
            if (LocalizationEditorSettings.ActiveLocalizationSettings == null)
            {
                ...
                else
                {
                    ...
                    SirenixEditorGUI.DrawRoundRect(
                        popupPosition.AlignBottom(32 + 12 + 8 + 6),
                ...
                GUIStyle labelStyle = EditorGUIUtility.isProSkin
                ...
                if (EditorGUIUtility.isProSkin)
                {
                    GUI.Label(
                        popupPosition,
                        "No Localization Settings found in project.",
                        labelStyle
                    );
                }
                else
        ...
        protected override OdinMenuTree BuildMenuTree()
        {
            ...
            {
                Config = { AutoHandleKeyboardNavigation = false, DrawSearchToolbar = true },
                DefaultMenuStyle =
                {
                    Height = 28,
                    ...
                    case SelectionChangedType.ItemAdded:
                        ...
                        switch (tree.Selection.SelectedValue)
                        {
                            case OdinAssetTableCollectionEditor assetCollection:
                            {
                                ...
                                if (
                                    assetCollection.SelectionType
                                        == OdinTableSelectionType.TableEntry
                                    && this.State.CurrentTopTab == RightMenuTopTabs.Metadata
                                )
                                {
                                    assetCollection.UpdateMetadataViewForEntry(
                                        assetCollection.CurrentSelectedEntry
                                    );
                                }
                            ...
                            case OdinStringTableCollectionEditor stringCollection:
                            {
                                ...
                                if (
                                    stringCollection.SelectionType
                                        == OdinTableSelectionType.TableEntry
                                    && this.State.CurrentTopTab == RightMenuTopTabs.Metadata
                                )
                                {
                                    stringCollection.UpdateMetadataViewForEntry(
                                        stringCollection.CurrentSelectedEntry
                                    );
                                }
            ...
            for (var i = 0; i < collectionGUIDs.Length; i++)
            {
                ...
                AssetTableCollection assetTableCollection =
                    LocalizationEditorSettings.GetAssetTableCollection(
                        collection.TableCollectionNameReference
                    );

                if (assetTableCollection != null)
                {
                    var guiCollection = new OdinAssetTableCollectionEditor(
                        assetTableCollection,
                        this,
                        this.State
                    );
                ...
                {
                    var guiCollection = new OdinStringTableCollectionEditor(
                        stringTableCollection,
                        this,
                        this.State
                    );
