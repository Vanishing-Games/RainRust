...
namespace Sirenix.OdinInspector.Modules.Localization.Editor.Configs
{
    ...
    public class OdinLocalizationConfig : GlobalConfig<OdinLocalizationConfig>
    {
            ...
            protected override void DrawPropertyLayout(GUIContent label)
            {
                SirenixEditorGUI.BeginBox(string.Empty);
                {
                    SirenixEditorGUI.BeginBoxHeader();
                    {
                        ...
                        {
                            GUILayout_Internal.BeginColumn(
                                LayoutSize.Pixels(EditorGUIUtility.labelWidth + 6.0f)
                            );
                            {
                                this.Property.State.Expanded = EditorGUILayout.Foldout(
                    ...
                    bool toggle =
                        this.ValueEntry.ValueState != PropertyValueState.NullReference
                        && this.Property.State.Expanded;
        ...
        [BoxGroup("Syntax Highlighting")]
        public ThemeColor placeholderColor = new ThemeColor(
            new Color(0.743147f, 0.9433962f, 0.9242815f),
            new Color(0, 0.5882353f, 0.5333334f)
        );

        [EnableIf(nameof(useSyntaxHighlighter))]
        [BoxGroup("Syntax Highlighting")]
        public ThemeColor selectorColor = new ThemeColor(
            new Color(1.0f, 0.7727525f, 0.3632075f),
            new Color(1, 0.6470588f, 0)
        );

        [EnableIf(nameof(useSyntaxHighlighter))]
        [BoxGroup("Syntax Highlighting")]
        public ThemeColor formatterColor = new ThemeColor(
            new Color(0.9921569f, 0.9855571f, 0.8823529f),
            new Color(0.9607843f, 0.9607843f, 0.8627451f)
        );
        ...
        public void Reset()
        {
            if (
                !EditorUtility.DisplayDialog(
                    "Odin Localization Config",
                    "Are you certain you want to reset your Localization config?",
                    "Yes",
                    "No"
                )
            )
            ...
            this.useCustomUndoHandlingForAssetCollections =
                OdinLocalizationReflectionValues.HasAPIForCustomUndo;

            this.assetRowHeight = 128;

            this.useSyntaxHighlighter = true;
            this.placeholderColor = new ThemeColor(
                new Color(0.743147f, 0.9433962f, 0.9242815f),
                new Color(0, 0.5882353f, 0.5333334f)
            );
            this.selectorColor = new ThemeColor(
                new Color(1.0f, 0.7727525f, 0.3632075f),
                new Color(1, 0.6470588f, 0)
            );
            this.formatterColor = new ThemeColor(
                new Color(0.9921569f, 0.9855571f, 0.8823529f),
                new Color(0.9607843f, 0.9607843f, 0.8627451f)
            );
