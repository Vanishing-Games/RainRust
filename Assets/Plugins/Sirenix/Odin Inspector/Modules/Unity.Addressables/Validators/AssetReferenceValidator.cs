...
[assembly: RegisterValidationRule(
    typeof(AssetReferenceValidator),
    Description = "This validator provides robust integrity checks for your asset references within Unity. "
...
namespace Sirenix.OdinInspector.Modules.Addressables.Editor
{
    ...
    {
        [Tooltip(
            "If true and the AssetReference is not marked with the Optional attribute, "
        ...
        protected override void Validate(ValidationResult result)
        {
            ...
            var addressableAssetEntry =
                AddressableAssetSettingsDefaultObject.Settings.FindAssetEntry(
            ...
            {
                result
                    .AddError("Assigned item is not addressable.")
                    .WithFix<MakeAddressableFixArgs>(
                        "Make Addressable",
            ...
            {
                if (
                    OdinAddressableUtility.ValidateAssetReferenceRestrictions(
                        restrictions,
                        mainAsset,
                ...
                )
                {
                    if (failedRestriction is AssetReferenceUILabelRestriction labelRestriction)
                    {
                        result
                            ...
                            .WithFix<AddLabelsFixArgs>(
                                "Add Labels",
                                args => SetLabels(mainAsset, args.AssetLabels)
                            );
                    }
                    else
                    {
            ...
            {
                var subObjects = OdinAddressableUtility.EnumerateAllActualAndVirtualSubAssets(
                    mainAsset,
                    assetPath
                );
        ...
        private static void SetLabels(UnityEngine.Object obj, List<AssetLabel> assetLabels)
        {
            ...
            settings.SetDirty(
                AddressableAssetSettings.ModificationEvent.LabelAdded,
                entry,
                false,
                true
            );
            ...
            private static IEnumerable<ValueDropdownItem> GetGroups()
            {
                return !AddressableAssetSettingsDefaultObject.SettingsExists
                    ...
                    : AddressableAssetSettingsDefaultObject
                        .Settings.groups.Where(group => !group.ReadOnly)
        ...
        private class AddLabelsFixArgs
        {
            ...
            private void Draw()
            {
                var togglesRect = EditorGUILayout.GetControlRect(
                    false,
                    Mathf.CeilToInt(this.AssetLabels.Count / 2f) * 20f
                );
                ...
                if (
                    SirenixEditorGUI.SDFIconButton(
                        buttonsRect,
                        "Open Addressables Labels",
                        SdfIconType.TagsFill
                    )
                )
                {
