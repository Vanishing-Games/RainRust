            ...
            foreach (var addressableAssetGroup in addressableAssetSettings.groups)
            {
                ...
                foreach (var addressableAssetEntry in addressableAssetGroup.entries)
                {
                    ...
                    foreach (var dependencyAssetPath in dependencyAssetPaths)
                    {
                        ...
                        if (isAddressable)
                            continue;
                        if (!IsInsideResourcesFolder(dependencyAssetPath))
                            continue;

                        result
                            .Add(
                                this.ValidatorSeverity,
                            ...
                            .WithFix<FixArgs>(
                                args =>
                                {
                                    if (args.FixChoice == FixChoice.Ignore)
                                    {
                                        ...
                                        var data =
                                            RuleConfig.Instance.GetRuleData<CheckResourcesToAddressableDuplicateDependenciesValidator>(
                                                sourceType
                                            );
                                    ...
                                    if (!ValidNewFolder(args.NewFolder, out _))
                                        return;

                                    if (!AssetDatabase.IsValidFolder(args.NewFolder))
                                    {
                                        Directory.CreateDirectory(
                                            new DirectoryInfo(args.NewFolder).FullName
                                        );
                                        AssetDatabase.Refresh();
                                    }

                                    var newPath =
                                        $"{args.NewFolder}/{Path.GetFileName(dependencyAssetPath)}";
