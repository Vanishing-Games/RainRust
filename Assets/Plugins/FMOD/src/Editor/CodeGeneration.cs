...
namespace FMODUnity
{
    ...
    {
        public static void GenerateStaticPluginRegistration(
            string filePath,
            Platform platform,
            Action<string> reportError
        )
        {
            List<string> validatedPlugins = ValidateStaticPlugins(
                platform.StaticPlugins,
                reportError
            );

            using (StreamWriter file = new StreamWriter(filePath))
            {
                WriteStaticPluginRegistration(
                    file,
                    platform.IsFMODStaticallyLinked,
                    validatedPlugins
                );
            }
        }

        private static void WriteStaticPluginRegistration(
            StreamWriter file,
            bool isFMODStaticallyLinked,
        ...
        )
        {
            ...
            if (pluginFunctions.Any())
            {
                ...
                foreach (string pluginFunction in pluginFunctions)
                {
                    ...
                    file.WriteLine(
                        "            reportResult(result, \"Registering static plugin '{0}'\");",
                        pluginFunction
                    );
                }
            }
        ...
        private static List<string> ValidateStaticPlugins(
            List<string> staticPlugins,
            Action<string> reportError
        )
        {
            ...
            for (int i = 0; i < staticPlugins.Count; ++i)
            {
                ...
                {
                    reportError(
                        string.Format("Static plugin {0} has no name and will be ignored.", i + 1)
                    );
                }
        ...
        private static bool IsValidFunctionName(string name, Action<string> reportError)
        {
            if (!(char.IsLetter(name[0]) || name[0] == '_'))
            {
                reportError(
                    string.Format(
                        "Plugin name '{0}' is not valid. Names must start with a letter or an underscore ('_').",
                        name
                    )
                );
                return false;
