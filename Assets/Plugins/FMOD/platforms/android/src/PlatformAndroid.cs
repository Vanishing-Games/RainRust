...
namespace FMODUnity
{
    ...
    public class PlatformAndroid : Platform
    {
        ...
        {
            yield return new FileRecord("fmod.jar").WithAbsoluteVersion(
                FileLayout.Release_1_10,
                "Plugins/Android/fmod.jar"
            );

            foreach (string architecture in Architectures)
            {
                yield return new FileRecord(
                    string.Format("{0}/libfmod{1}.so", architecture, suffix)
                );
                yield return new FileRecord(
                    string.Format("{0}/libfmodstudio{1}.so", architecture, suffix)
                );
            }
        }
        ...
        )
        {
            foreach (string architecture in Architectures)
            {
                ...
                yield return new FileRecord(
                    string.Format("{0}/libresonanceaudio.so", architecture)
                );
            }
        }
        ...
        private static OutputType[] sValidOutputTypes =
        {
            new OutputType()
            {
                displayName = "Java Audio Track",
                outputType = FMOD.OUTPUTTYPE.AUDIOTRACK,
            },
