...
namespace LDtkUnity.Editor
{
    ...
    internal sealed class LDtkTocFieldFactory
    {
        ...
        {
            public EntityFieldDefsByIdentifier(int capacity)
                : base(capacity) { }
        }

        public LDtkTocFieldFactory(
            LdtkJson json,
        ...
        public void IndexEntitiesAndFieldsByIdentifiers()
        {
            ...
            _entityIdentifierToEntityDefinition =
                new Dictionary<string, LDtkDefinitionObjectEntity>();

            foreach (EntityDefinition entityDef in _json.Defs.Entities)
            {
                //skip caching any that don't export to toc
                if (!entityDef.ExportToToc)
                    continue;

                _entityIdentifierToEntityDefinition.Add(
                    entityDef.Identifier,
                    _importer.DefinitionObjects.GetObject<LDtkDefinitionObjectEntity>(entityDef.Uid)
                );

                EntityFieldDefsByIdentifier fields = new EntityFieldDefsByIdentifier(
                    entityDef.FieldDefs.Length
                );
                foreach (FieldDefinition fieldDef in entityDef.FieldDefs)
                {
                    //skip caching fields that don't export to toc
                    if (!fieldDef.ExportToToc)
                        continue;
        ...
        private LDtkField[] GetFields(string entityIdentifier, Dictionary<string, object> srcFields)
        {
            ...
            {
                FieldDefinition def = GetFieldDefByFieldIdentifier(
                    entityIdentifier,
                    fieldIdentifier
                );
        ...
        /// </summary>
        private FieldDefinition GetFieldDefByFieldIdentifier(
            string entityIdentifier,
            string fieldIdentifier
        )
        {
            if (
                _entityIdentifierToFieldDefs.TryGetValue(
                    entityIdentifier,
                    out EntityFieldDefsByIdentifier fields
                )
            )
            {
                ...
                LDtkDebug.LogError(
                    $"The field identifier \"{fieldIdentifier}\" does not exist in the entity \"{entityIdentifier}\""
                );
            }
            else
            {
                LDtkDebug.LogWarning(
                    $"The entity identifier \"{entityIdentifier}\" does not exist in the table of contents"
                );
            }
