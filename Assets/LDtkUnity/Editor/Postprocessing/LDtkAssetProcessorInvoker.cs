...
namespace LDtkUnity.Editor
{
    ...
    internal static class LDtkAssetProcessorInvoker
    {
        ...
        public static void AddPreProcessProject(
            LDtkAssetProcessorActionCache cache,
            LdtkJson projectJson,
            string projectName
        ) =>
            AddPreprocessActions(
                cache,
                LDtkPreprocessor.METHOD_PROJECT,
                new object[] { projectJson, projectName }
            );

        public static void AddPreProcessLevel(
            LDtkAssetProcessorActionCache cache,
            Level levelJson,
            LdtkJson projectJson,
            string projectName
            ...
            AddPreprocessActions(
                cache,
                LDtkPreprocessor.METHOD_LEVEL,
                new object[] { levelJson, projectJson, projectName }
            );

        public static void AddPostProcessProject(
            LDtkAssetProcessorActionCache cache,
            ...
            AddPostprocessAction(
                cache,
                importer,
                LDtkPostprocessor.METHOD_PROJECT,
                new object[] { projectObj }
            );

        public static void AddPostProcessLevel(
            LDtkAssetProcessorActionCache cache,
            LDtkJsonImporter importer,
            GameObject levelObj,
            LdtkJson projectJson
            ...
            AddPostprocessAction(
                cache,
                importer,
                LDtkPostprocessor.METHOD_LEVEL,
                new object[] { levelObj, projectJson }
            );

        private static void AddPreprocessActions(
            LDtkAssetProcessorActionCache cache,
            string methodName,
            object[] args
        )
        {
            ...
            {
                cache.AddProcessAction(
                    preprocessor.GetPreprocessOrder(),
                    () =>
                    {
        ...
        private static void AddPostprocessAction(
            LDtkAssetProcessorActionCache cache,
            LDtkJsonImporter importer,
            string methodName,
            object[] args
        )
        {
            ...
            {
                cache.AddProcessAction(
                    postprocessor.GetPostprocessOrder(),
                    () =>
                    {
        ...
        private static void TryInstantiatePreprocessors()
        {
            ...
            TypeCache.TypeCollection preprocessors =
                TypeCache.GetTypesDerivedFrom<LDtkPreprocessor>();
            foreach (Type pre in preprocessors)
            {
                try
                {
                    LDtkPreprocessor preprocessor =
                        Activator.CreateInstance(pre) as LDtkPreprocessor;
        ...
        private static void TryInstantiatePostprocessors()
        {
            ...
            TypeCache.TypeCollection postprocessors =
                TypeCache.GetTypesDerivedFrom<LDtkPostprocessor>();
            foreach (Type post in postprocessors)
            {
                try
                {
                    LDtkPostprocessor assetPostprocessor =
                        Activator.CreateInstance(post) as LDtkPostprocessor;
        ...
        private static bool InvokeMethodIfAvailable(
            object processor,
            string methodName,
            object[] args
        )
        {
            ...
            {
                int length =
                    "System.Reflection.TargetInvocationException: Exception has been thrown by the target of an invocation. ---> ".Length;
                ...
                if (gameObject)
                {
                    ...
                    string detail = parent
                        ? $"level \"{gameObject.name}\" of project \"{parent.name}\""
                        : $"{gameObject.name}";
                ...
                LDtkDebug.LogError(
                    $"An exception occurred while pre/post processing \"{targetType.Name}.{methodName}\" on {gameObjectContext}\n{s}",
                    script
                );
