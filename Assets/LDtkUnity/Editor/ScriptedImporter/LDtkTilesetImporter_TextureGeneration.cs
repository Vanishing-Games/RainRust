...
namespace LDtkUnity.Editor
{
    ...
    internal sealed partial class LDtkTilesetImporter
    {
        ...
        private bool PrepareAndGenerateTexture(
            TextureImporterPlatformSettings platformSettings,
            out TextureGenerationOutput output
        )
        {
            ...
            {
                Logger.LogError(
                    $"Didn't generate the texture and sprites for \"{AssetName}\" because the source texture's TextureType is not \"Sprite\"."
                );
                return false;
            }
            ...
            int innerLoopBatchCount = Mathf.Max(
                1,
                (tilesLength / System.Environment.ProcessorCount) + 1
            );
            ...
            SpriteImportData[] sprites = new SpriteImportData[
                validRects.Count + _additionalTiles.Count
            ];
        ...
        private Texture2D LoadTextureAndMakeCopy()
        {
            ...
            if (_srcAsepriteImporter)
            {
                ...
                {
                    Logger.LogError(
                        $"Failed to load the aseprite sprite for \"{AssetName}\". Either the Aseprite file failed to import, or the aseprite file's import settings are configured to not generate a sprite."
                    );
                    return null;
                }
        ...
        private Texture2D GenerateTextureFromAseprite(Sprite sprite)
        {
            ...
            croppedTexture.SetPixels(
                0,
                _json.PxHei - (int)sprite.rect.height,
        ...
        private static readonly int[] MaxSizes = new[]
        {
            32,
            64,
            128,
        ...
        private bool HasTextureIssue(TextureImporterPlatformSettings platformSettings)
        {
            ...
            AssetImporter importer =
                _srcAsepriteImporter != null
                    ? (AssetImporter)_srcAsepriteImporter
                    : _srcTextureImporter;
#else
            ...
            if (
                platformSettings.maxTextureSize < _json.PxWid
                || platformSettings.maxTextureSize < _json.PxHei
            )
            {
                ...
                issue = true;
                Logger.LogError(
                    $"The texture at \"{importer.assetPath}\" maxTextureSize needs to at least be {resolution}.\n(From {assetPath})",
                    importer
                );
            ...
            if (platformSettings.format != TextureImporterFormat.RGBA32)
            {
                ...
                //platformSettings.format = TextureImporterFormat.RGBA32;
                Logger.LogError(
                    $"The texture at \"{importer.assetPath}\" needs to have a compression format of {TextureImporterFormat.RGBA32}\n(From {assetPath})",
                    importer
                );
            }
    ...
    internal struct DetectUsedTilePixels : IJobFor
    {
        ...
        [WriteOnly]
        public NativeArray<bool> TileIdsWithPixels;

        [ReadOnly]
        private readonly int _cWid;

        [ReadOnly]
        private readonly int _cHei;
        ...
        [ReadOnly]
        private readonly int _defPxWid;

        [ReadOnly]
        private readonly int _defPxHei;
