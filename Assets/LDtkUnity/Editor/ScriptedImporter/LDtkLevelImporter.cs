...
namespace LDtkUnity.Editor
{
    ...
    [ScriptedImporter(
        LDtkImporterConsts.LEVEL_VERSION,
        LDtkImporterConsts.LEVEL_EXT,
        LDtkImporterConsts.LEVEL_ORDER
    )]
    internal sealed class LDtkLevelImporter : LDtkJsonImporter<LDtkLevelFile>
    {
        ...
        private static readonly Dictionary<LDtkProjectImporter, LdtkJson> Jsons =
            new Dictionary<LDtkProjectImporter, LdtkJson>();
        ...
        {
            string projectPath = new LDtkRelativeGetterProjectImporter().GetPath(
                assetPath,
                assetPath
            );
        ...
        private void BuildLevel()
        {
            ...
            LDtkAssetProcessorInvoker.AddPreProcessLevel(
                preAction,
                _levelJson,
                _projectJson,
                AssetName
            ...
            ImportContext.AddObjectToAsset(
                "levelRoot",
                levelRoot,
                LDtkIconUtility.LoadLevelFileIcon()
            );
        ...
        private bool DeserializeAndAssign()
        {
            ...
            if (_projectImporter == null)
            {
                ...
                Logger.LogError(
                    $"Tried to build level {levelName}, but the project importer was not found"
                );
                return false;
            }
            ...
            {
                Logger.LogError(
                    "Tried to build level, but the level json ScriptableObject was null"
                );
                return false;
            }
