...
namespace LDtkUnity.Editor
{
    internal static class LDtkJsonDigger
    {
        ...
        public static bool GetTilesetTextureRelPaths(
            string projectPath,
        ...
        public static bool GetSeparateLevelDependencies(
            string path,
            ref DugDependencyDataLevel result
        ...
        /// </summary>
        public static bool GetUsedFieldTiles(
            string levelOrProjectPath,
            ref List<FieldInstance> result
        ) => DigIntoJson(levelOrProjectPath, GetUsedFieldTilesReader, ref result);

        public static bool GetUsedTilesetSprites(
            string levelPath,
            ref Dictionary<string, HashSet<int>> result
        ...
        private static bool DigIntoJson<T>(string path, JsonDigAction<T> digAction, ref T result)
        {
            ...
            {
                LDtkDebug.LogError(
                    $"Couldn't locate the file to dig into the json for at path: \"{path}\""
                );
                return false;
            }
            ...
            LDtkDebug.LogError(
                $"Issue digging into the json for {path} when doing {digAction.Method.Name}"
            );
            return false;
        }
        ...
        private static bool GetUsedBackgroundsReader(
        ...
        private static bool GetTilesetRelPathsReader(
            ref JsonReader reader,
            ref HashSet<string> textures
        )
        {
            while (reader.Read())
            {
        ...
        private static bool GetTilesetDefNamesReader(
            ref JsonReader reader,
            ref HashSet<string> textures
        )
        {
        ...
        private static bool GetUsedEntitiesReader(
            ref JsonReader reader,
            ref HashSet<string> entities
        )
        {
        ...
        private static bool GetSeparateLevelDependenciesReader(
            ref JsonReader reader,
            ref DugDependencyDataLevel result
        )
        {
            ...
            while (reader.CanRead())
            {
                ...
                if (propName == "intGridCsv" && reader.ReadIsNameSeparator())
                {
                    ...
                    while (reader.IsInArray(ref depth))
                    {
                        ...
                        string formattedString = LDtkKeyFormatUtil.IntGridValueFormat(
                            recentIntGridIdentifier,
                            intGridValue.ToString()
                        );
                ...
                if (propName == "entityInstances" && reader.ReadIsNameSeparator())
                {
                    ...
                    while (reader.IsInArray(ref arrayDepth))
                    {
                        ...
                        while (reader.IsInObject(ref objectDepth))
                        {
                            ...
                            if (
                                reader.ReadString() == "__identifier"
        ...
        private static bool GetUsedTilesetDefsReader(
            ref JsonReader reader,
            ref HashSet<string> relPaths
        )
        {
        ...
        private static bool GetUsedIntGridValuesReader(
            ref JsonReader reader,
            ref HashSet<string> result
        )
        {
        ...
        private static bool GetUsedTilesetSpritesReader(
            ref JsonReader reader,
            ref Dictionary<string, HashSet<int>> usedTileIds
        )
        {
            ...
            while (reader.Read())
            {
                ...
                while (reader.ReadIsInArray(ref layerInstancesArrayDepth)) //endarray handled automatically if array is empty
                {
                    ...
                    while (reader.IsInObject(ref layerInstanceObjectDepth))
                    {
                        ...
                        if (type != "IntGrid" && type != "Tiles" && type != "AutoLayer")
                        {
                        ...
                        {
                            case "IntGrid":
                                arrayToSearch = "autoLayerTiles";
                                break;
                            case "Tiles":
                                arrayToSearch = "gridTiles";
                                break;
                            case "AutoLayer":
                                arrayToSearch = "autoLayerTiles";
                                break;
                            default:
                                LDtkDebug.LogError(
                                    $"Expected type wasn't properly encountered {type}"
                                );
                                break;
                        }
        ...
        private static bool GetUsedFieldTilesReader(
            ref JsonReader reader,
            ref List<FieldInstance> result
        )
        {
