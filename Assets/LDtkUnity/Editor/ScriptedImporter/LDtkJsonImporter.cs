...
namespace LDtkUnity.Editor
{
    internal abstract class LDtkJsonImporter : ScriptedImporter
    {
        ...
        [SerializeField]
        private bool _reimportOnDependencyChange = true;

        private readonly Dictionary<TilesetDefinition, LDtkTilesetImporter> _importersForDefs =
            new Dictionary<TilesetDefinition, LDtkTilesetImporter>();
        ...
        public bool IsVersionOutdated(string projectPath = null)
        {
            ...
            {
                projectPath =
                    this is LDtkProjectImporter
                        ? assetPath
                        : new LDtkRelativeGetterProjectImporter().GetPath(assetPath, assetPath);
            }
        ...
        public static bool CheckOutdatedJsonVersion(
            string jsonVersion,
            string assetName,
            LDtkDebugInstance projectCtx = null
        )
        {
            ...
            {
                LDtkDebug.LogError(
                    $"This json asset \"{assetName}\" couldn't parse it's version \"{jsonVersion}\", post an issue to the developer",
                    projectCtx
                );
                return false;
            }

            Version minimumRecommendedVersion = new Version(
                LDtkImporterConsts.MINIMUM_JSON_VERSION
            );
            if (version < minimumRecommendedVersion)
            {
                LDtkDebug.LogError(
                    $"The version of the project \"{assetName}\" is outdated. It's a requirement to update your project to the latest supported version. ({version} < {minimumRecommendedVersion})",
                    projectCtx
                );
                return false;
            }
        ...
        public LDtkProjectImporter GetProjectImporter()
        {
            ...
            projectImporter = getter.GetRelativeAsset(
                assetPath,
                assetPath,
                (path) => (LDtkProjectImporter)GetAtPath(path)
            );
            return projectImporter;
        }
        ...
        public Sprite GetAdditionalSprite(
        ...
        )
        {
            ...
            Logger.LogError(
                $"Failed to load an additional sprite at id \"{id}\" from \"{def.Identifier}\""
            );
            return null;
        }

        protected Dictionary<int, LDtkArtifactAssetsTileset> MakeTilesetDict(
            LDtkProjectImporter project,
            LdtkJson json
        )
        {
            //construct a dictionary to get artifacts by tileset uid
            Dictionary<int, LDtkArtifactAssetsTileset> artifacts =
                new Dictionary<int, LDtkArtifactAssetsTileset>();
        ...
        internal LDtkArtifactAssetsTileset LoadTilesetArtifacts(
            LDtkProjectImporter project,
            TilesetDefinition def
        )
        {
            ...
            {
                Logger.LogWarning(
                    $"The tileset file \"{tilesetImporter.AssetName}\" ({tilesetImporter._pixelsPerUnit}) doesn't have the same pixels per unit as it's project \"{AssetName}\" ({project.PixelsPerUnit}). Ensure they match."
                );
            }
        ...
        private LDtkTilesetImporter LoadAndCacheTilesetImporter(TilesetDefinition def)
        {
            ...
            {
                Logger.LogError(
                    $"Failed to find the required tileset file at \"{path}\". Ensure that LDtk exported a tileset file through a custom command. If the command wasn't configured yet, check the project inspector for more info."
                );
            ...
            {
                Logger.LogError(
                    $"Failed to load the tileset importer at \"{path}\", but the file exists. The tileset file may have failed to import?"
                );
                _importersForDefs.Add(def, null);
                return null;
            }
        ...
        public static string TilesetImporterPath(string importerPath, string tilesetDefIdentifier)
        {
            ...
            if (ext != ".ldtk")
            {
                importerPath = new LDtkRelativeGetterProjectImporter().GetPath(
                    importerPath,
                    importerPath
                );
            }
            ...
            {
                LDtkDebug.LogError(
                    $"Issue formulating a tileset definition path; Path was invalid for: \"{importerPath}\""
                );
                return null;
            }

            return Path.Combine(directoryName, projectName, tilesetDefIdentifier)
                + '.'
                + LDtkImporterConsts.TILESET_EXT;
        }
        ...
        protected void FailImport()
        {
            ...
            if (this is LDtkProjectImporter)
            {
                ImportContext.AddObjectToAsset(
                    "failedImport",
                    failedAsset,
                    LDtkIconUtility.LoadProjectFileIcon(true)
                );
            }
            if (this is LDtkLevelImporter)
            {
                ImportContext.AddObjectToAsset(
                    "failedImport",
                    failedAsset,
                    LDtkIconUtility.LoadLevelFileIcon(true)
                );
            }
            if (this is LDtkTilesetImporter)
            {
                ImportContext.AddObjectToAsset(
                    "failedImport",
                    failedAsset,
                    LDtkIconUtility.LoadTilesetFileIcon(true)
                );
            }
    ...
    internal abstract class LDtkJsonImporter<T> : LDtkJsonImporter
        where T : ScriptableObject, ILDtkJsonFile
    {
        ...
        public static TJson FromJson<TJson>(string path, LDtkDebugInstance debug = null)
        {
            ...
            {
                LDtkDebug.LogError(
                    $"Could not find the json file to deserialize at \"{path}\"",
                    debug
                );
                return default;
            }
