...
namespace LDtkUnity.Editor
{
    internal sealed class PathDrawer
    {
        ...
        public PathDrawer(
            ...
            string originalPath,
            string folderButtonTooltip,
            string extension = "",
            string filePanelDescription = "Select location"
        )
        {
        ...
        public string DrawPathField()
        {
            ...
            string destinationEnumPath = EditorUtility.SaveFilePanel(
                _filePanelDescription,
            ...
            if (
                !string.IsNullOrEmpty(destinationEnumPath)
                && AssetDatabase.IsValidFolder(
                    LDtkPathUtility.DirectoryOfAssetPath(destinationEnumPath)
                )
            )
            {
            ...
            else
            {
                LDtkDebug.LogWarning(
                    "Export: Cannot specify within a folder outside of the Unity project"
                );
            }
        ...
        public string DrawFolderField()
        {
            ...
            string destinationPath = EditorUtility.OpenFolderPanel(
                _filePanelDescription,
                defaultRelPath,
                ""
            );
            ...
            {
                LDtkDebug.LogWarning(
                    $"Export: Cannot specify a folder outside of the Unity project\n{destinationPath}"
                );
            }
        ...
        private bool DrawFieldAndButton(string defaultRefPath)
        {
            ...
            {
                Rect rect = LDtkEditorGUI.PropertyFieldWithDefaultText(
                    _pathProp,
                    _labelContent,
                    defaultRefPath
                );
