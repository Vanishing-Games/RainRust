...
namespace LDtkUnity.Editor
{
    internal sealed class LDtkNativeExportWindow : EditorWindow
    {
        ...
        private readonly GUIContent _headerContent = new GUIContent()
        {
            ...
            tooltip =
                "If you feel like uninstalling the LDtkToUnity package but want to maintain the LDtk project you Unity, you can export a prefab and assets to a folder.",
        };
        ...
        private void OnGUI()
        {
            ...
            _pathDrawer = new PathDrawer(
                ...
                content,
                pathToObject,
                "The path to export the prefab and related assets to."
            );
            ...
            if (!AssetDatabase.IsValidFolder(exportPath))
            {
                EditorGUILayout.LabelField(
                    "Specify a folder path inside of the unity project. (Starting from Assets)"
                );
                EditorGUILayout.LabelField(
                    "Leave empty to export relative to the LDtk project's directory"
                );
                return;
            }
            ...
            LDtkProjectImporter assetImporter = (LDtkProjectImporter)
                AssetImporter.GetAtPath(pathToObject);
            LDtkArtifactAssets artifactAssets = AssetDatabase.LoadAssetAtPath<LDtkArtifactAssets>(
                pathToObject
            );
        ...
        private static bool TryInvalidPass(GameObject obj)
        {
            ...
            {
                EditorGUILayout.LabelField(
                    "This GameObject is not a valid imported prefab from the Project window"
                );
                invalid = true;
            }
