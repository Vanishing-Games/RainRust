...
namespace LDtkUnity.Editor
{
    internal sealed class LDtkBuilderIntGridValue : LDtkBuilderLayer
    {
        ...
        public void BuildIntGridValues()
        {
            ...
            Dictionary<IntGridValueDefinition, TileBase> defToTile = new Dictionary<
                IntGridValueDefinition,
                TileBase
            >(defsLength);
            Dictionary<IntGridValueDefinition, TilemapKey> defToKey = new Dictionary<
                IntGridValueDefinition,
                TilemapKey
            >(defsLength);
            ...
            for (int i = 0; i < defsLength; i++)
            {
                ...
                string formatString = LDtkKeyFormatUtil.IntGridValueFormat(
                    layerDef,
                    intGridValueDef
                );
                ...
                {
                    LDtkDebug.LogError(
                        "Issue loading a IntGridTile. This is always expected to exist"
                    );
                    continue;
                }
                ...
                TilemapKey key = tile is LDtkIntGridTile intGridTile
                    ? new TilemapKey(
                        intGridTile.TilemapTag,
                        intGridTile.TilemapLayerMask,
                        intGridTile.PhysicsMaterial
                    )
            ...
            for (int i = 0; i < Layer.IntGridCsv.Length; i++)
            {
                ...
                {
                    LDtkDebug.LogError(
                        $"Can't build IntGrid value when trying to access a IntGridValue definition due to OutOfBoundsException. Tried index \"{index}\" of array length \"{defsLength}\". "
                ...
                TilemapTilesBuilder tilemapToBuildOn = GetTilemapToBuildOn(
                    defToKey[intGridValueDef]
                );
                LDtkProfiler.EndSample();
