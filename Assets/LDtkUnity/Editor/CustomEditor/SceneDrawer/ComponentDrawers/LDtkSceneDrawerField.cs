        ...
        public static ILDtkHandleDrawer DrawField(LDtkFieldDrawerData data)
        {
            ...
            switch (data.FieldMode)
            {
                ...
                case EditorDisplayMode.NameAndValue: //display identifier then value (enum value could show image) //todo
                    ...
                    return new LDtkFieldDrawerValue(
                        data.Fields.transform.position + Vector3.up,
                        data.Identifier
                    );
                ...
                case EditorDisplayMode.Points:
                    return new LDtkFieldDrawerPoints(
                        data.Fields,
                        data.Identifier,
                        data.FieldMode,
                        data.LocalMiddleCenter,
                        data.PixelsPerUnit
                    );

                case EditorDisplayMode.RadiusPx:
                case EditorDisplayMode.RadiusGrid:
                    return new LDtkFieldDrawerRadius(
                        data.Fields,
                        data.Identifier,
                        data.FieldMode,
                        data.GridSize,
                ...
                case EditorDisplayMode.RefLinkBetweenPivots:
                    return new LDtkFieldDrawerEntityRef(
                        data.Fields,
                        data.Identifier,
                        data.FieldMode,
                        data.PixelsPerUnit,
