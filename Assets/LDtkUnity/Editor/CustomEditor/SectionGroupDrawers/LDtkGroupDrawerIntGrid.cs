...
{
    internal sealed class LDtkGroupDrawerIntGrid
        : LDtkGroupDrawer<IntGridValueDefinition, LayerDefinition, LDtkDrawerIntGridValue>
    {
        private readonly LDtkDrawerIntGridValueIterator _intGridValueIterator;

        public LDtkGroupDrawerIntGrid(
        ...
        private LDtkDrawerIntGridValue GetIntGridValueDrawer(IntGridValueDefinition intGridValueDef)
        {
            ...
            {
                LDtkDebug.LogError(
                    "Array index out of bounds, the serialized array likely wasn't constructed properly for IntGrid layer"
                );
                return null;
            }

            SerializedProperty valueObj = ArrayProp.GetArrayElementAtIndex(
