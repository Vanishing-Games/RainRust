...
namespace LDtkUnity.Editor
{
    ...
    /// </summary>
    internal sealed class LDtkGroupDrawerEntity
        : LDtkGroupDrawer<EntityDefinition, EntityDefinition, LDtkDrawerEntity>
    {
        private readonly List<LDtkDrawerEntity> _drawers;

        public LDtkGroupDrawerEntity(
            EntityDefinition data,
            SerializedProperty arrayProp,
            string tag,
            List<LDtkDrawerEntity> drawers
        )
            : base(data, arrayProp)
        {
            _drawers = drawers;
