...
namespace LDtkUnity.Editor
{
    ...
    internal sealed class LDtkLevelFileEditor : LDtkJsonFileEditor<Level>
    {
        ...
        private void InitValues()
        {
            ...
            _entityCount = layers
                .Where(p => p.IsEntitiesLayer)
                .SelectMany(p => p.EntityInstances)
                .Count();
            _gridTileCount = layers.Where(p => p.IsTilesLayer).SelectMany(p => p.GridTiles).Count();
            _autoTileCount = layers
                .Where(p => p.IsAutoLayer)
                .SelectMany(p => p.AutoLayerTiles)
                .Count();
            _intGridValueCount = layers
                .Where(p => p.IsIntGridLayer)
                .Select(p => p.IntGridValueCount)
                .Sum();
        ...
        protected override void DrawInspectorGUI()
        {
            ...
            DrawCountOfItems(
                ...
                "Int Grid Value",
                "Int Grid Values",
                LDtkIconUtility.LoadIntGridIcon()
            );
            ...
            DrawCountOfItems(
                ...
                "Auto Tile",
                "Auto Tiles",
                LDtkIconUtility.LoadAutoLayerIcon()
            );
        }
    }
}
