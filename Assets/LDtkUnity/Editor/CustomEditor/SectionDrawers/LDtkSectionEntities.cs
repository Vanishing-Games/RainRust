...
namespace LDtkUnity.Editor
{
    internal sealed class LDtkSectionEntities : LDtkSectionDataDrawer<EntityDefinition>
    {
        ...
        public LDtkSectionEntities(LDtkImporterEditor editor, SerializedObject serializedObject)
            : base(editor, serializedObject) { }

        protected override void GetDrawers(
            EntityDefinition[] defs,
            List<LDtkContentDrawer<EntityDefinition>> drawers
        )
        {
        ...
        private LDtkDrawerEntity GetDrawerForEntity(EntityDefinition[] defs, int i)
        {
            ...
            {
                LDtkDebug.LogError(
                    "Array index out of bounds, the serialized array likely wasn't constructed properly"
                );
                return null;
            }
        ...
        private void TryAddTaggedGroup(
            EntityDefinition[] defs,
            List<LDtkContentDrawer<EntityDefinition>> drawers
        )
        {
            //add all other tagged groups
            List<string> uniqueTags = defs.SelectMany(p => p.Tags)
                .Distinct()
                .OrderBy(p => p)
                .ToList();
            foreach (string tag in uniqueTags)
            {
                List<LDtkDrawerEntity> groupDrawers = CreateGroup(
                    defs,
        ...
        private void TryAddUntaggedGroup(
            EntityDefinition[] defs,
            List<LDtkContentDrawer<EntityDefinition>> drawers
        )
        {
        ...
        private void AddGroup(
            List<LDtkContentDrawer<EntityDefinition>> drawers,
            string tag,
            List<LDtkDrawerEntity> groupDrawers
        )
        {
            LDtkGroupDrawerEntity group = new LDtkGroupDrawerEntity(
                null,
                ArrayProp,
                tag,
                groupDrawers
