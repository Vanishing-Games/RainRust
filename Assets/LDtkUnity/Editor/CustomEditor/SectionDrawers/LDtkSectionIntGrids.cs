...
namespace LDtkUnity.Editor
{
    internal sealed class LDtkSectionIntGrids : LDtkSectionDataDrawer<LayerDefinition>
    {
        ...
        protected override string GuiTooltip =>
            "Assign a tile to be used in place of an IntGridValue. If you assign Int Grid tiles, you'll be able to separate tilemaps by Tag/Layer/PhysicsMaterial. Make some with the button to the right of this text.";
        ...
        public LDtkSectionIntGrids(LDtkImporterEditor editor, SerializedObject serializedObject)
        ...
        protected override void GetDrawers(
            LayerDefinition[] defs,
            List<LDtkContentDrawer<LayerDefinition>> drawers
        )
        {
            ...
            LDtkDrawerIntGridValueIterator intGridValueIterator =
                new LDtkDrawerIntGridValueIterator();

            foreach (LayerDefinition def in defs)
            {
                LDtkGroupDrawerIntGrid intGridDrawer = new LDtkGroupDrawerIntGrid(
                    def,
                    ArrayProp,
        ...
        {
            return defs.SelectMany(def =>
                    def.IntGridValues.Select(value =>
                        LDtkKeyFormatUtil.IntGridValueFormat(def, value)
                    )
                )
                .ToArray();
        }
        ...
        protected override void DrawDropdownContent()
        {
            ...
            LDtkAssetCreator.CreateAssetButton(
                buttonRect,
                _buttonContent,
                $"New {nameof(LDtkIntGridTile)}.asset",
                ScriptableObject.CreateInstance<LDtkIntGridTile>,
                true
            );
