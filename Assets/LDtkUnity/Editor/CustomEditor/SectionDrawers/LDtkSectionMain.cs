...
namespace LDtkUnity.Editor
{
    internal sealed class LDtkSectionMain : LDtkSectionDrawer
    {
        ...
        private static readonly GUIContent GeometryType = new GUIContent
        {
            text = "Geometry Type",
            tooltip =
                "Specifies the type of geometry the Composite Collider generates. Outlines is like an EdgeCollider2D, Polygons is like a PolygonCollider2D",
        };
        ...
        private static readonly GUIContent UseParallax = new GUIContent
        {
            text = "Use Parallax",
            tooltip =
                "Adds components to layer GameObjects that will try to mimic the parallax motion seen in LDtk.",
        };
        private static readonly GUIContent ScaleEntities = new GUIContent
        {
            text = "Scale Entities",
            tooltip =
                "Apply a scale factor to entity prefab instances if they were resized in LDtk.",
        };

        protected override string GuiText => "Main";
        protected override string GuiTooltip =>
            "This is the importer menu.\n" + "Configure all of your custom settings here.";
        ...
        public LDtkSectionMain(LDtkImporterEditor editor, SerializedObject serializedObject)
        ...
        protected override void DrawDropdownContent()
        {
            ...
            {
                SerializedProperty compositeProp = DrawField(
                    UseCompositeCollider,
                    LDtkProjectImporter.USE_COMPOSITE_COLLIDER
                );
        ...
        private void DrawCustomLevelField()
        {
            ...
            {
                IEnumerable<string> levelFields = _data.Defs.LevelFields.Select(field =>
                    field.Identifier
                );
            ...
            SerializedProperty levelPrefabProp = DrawField(
                levelContent,
                LDtkProjectImporter.CUSTOM_LEVEL_PREFAB
            );
