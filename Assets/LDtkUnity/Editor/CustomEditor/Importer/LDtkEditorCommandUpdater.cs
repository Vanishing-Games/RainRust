    ...
    internal sealed class LDtkEditorCommandUpdater
    {
        ...
        public void TryDrawFixButton(LdtkJson data)
        {
            ...
            if (!IsInstalled(out var installReason))
            {
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    EditorGUILayout.HelpBox(
                        $"The importer requires an app installed to the Library folder. This is a one time process.\nReason: {installReason}",
                        MessageType.Error
                    );
                    ...
                    GUIContent installContent = new GUIContent()
                    {
                        text = "Install",
                        tooltip =
                            "This will extract from a zip file in this package into the Library folder.",
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUIUtility.SetIconSize(Vector2.one * 32);
                EditorGUILayout.HelpBox(
                    $"This project needs a command that should run this after saving:\n{Command}\nReason: {reason}",
                    MessageType.Error
                );
        ...
        private void ToClipboard()
        {
            ...
            LDtkDebug.Log(
                $"Copied to clipboard: \"{Command}\". Paste this as a new custom command in LDtk then save!"
            );
        }
        ...
        public void TryModifyProjectWithCommand()
        {
            ...
            else
            {
                LDtkDebug.LogWarning(
                    $"Failed to modify \"{ProjectName}\" with the custom command, likely due to foreign custom commands already existing. Try taking the manual approach instead via the clipboard."
                );
            }
        }

        public static bool ModifyProjectWithCommand(
            string projectPath,
            string exeRelPath,
            string arg
        )
        {
            ...
            const string after =
                @"""customCommands"": [{ ""command"": ""_PATH"", ""when"": ""AfterSave"" }],";
        ...
        public bool IsInstalled(out string reason)
        {
            if (File.Exists(LDtkTilesetExporterUtil.PathToExe()))
            {
                ...
                {
                    reason =
                        $"The app's version ({old}) does not match the required one ({required})";
                    return false;
                }
        ...
        public bool HasCustomCommand(LdtkJson data, out string reason)
        {
            ...
            foreach (LdtkCustomCommand command in commands)
            {
                if (command.Command == Command)
                {
                    ...
                    {
                        reason =
                            "The command exists, but the timing is not set to \"Run after saving\"";
                        return false;
                    }
