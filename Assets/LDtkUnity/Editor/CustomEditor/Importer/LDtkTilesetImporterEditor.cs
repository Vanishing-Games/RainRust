...
namespace LDtkUnity.Editor
{
    ...
    internal sealed class LDtkTilesetImporterEditor : LDtkSubImporterEditor
    {
        ...
        private static readonly GUIContent PpuContent = new GUIContent
        {
            ...
            tooltip =
                "The pixels per unit for this tileset. It should match with the source project importer's pixels per unit.",
        };
        private static readonly GUIContent OverrideTextureContent = new GUIContent
        {
            text = "Override Texture",
            tooltip =
                "Choose a different texture, useful to make tile sprites of an increased resolution.\n"
        ...
        public override void OnEnable()
        {
            ...
            _overrideTextureProp = serializedObject.FindProperty(
            ...
            _projectAsset =
                AssetDatabase.LoadMainAssetAtPath(_projectImporter.assetPath) as GameObject;
            _tilesetDef = LDtkTilesetImporter.FromJson<LDtkTilesetDefinitionWrapper>(
                _importer.assetPath
            );
        }
                ...
                if (_projectImporter)
                {
                    if (_projectImporter.PixelsPerUnit != _importer._pixelsPerUnit)
                    {
                        EditorGUILayout.HelpBox(
                            $"This doesn't have the same pixels per unit as it's project \"{_projectImporter.AssetName}\" ({_projectImporter.PixelsPerUnit}). Ensure they match.",
                            MessageType.Warning
                        );
                    }
                }
            ...
            using (new EditorGUI.DisabledScope(targets.Length != 1))
            {
                ...
                if (button)
                {
                    if (HasModified())
                    {
                        ...
                        var dialogText =
                            $"Unapplied import settings for \'{_importer.assetPath}\'.\nApply and continue to sprite editor or cancel.";
                        if (
        ...
        private void DrawRedirect()
        {
            ...
            if (
                LDtkTilesetImporter.IsResolutionMultiple(
                    overrideTexture.width,
                    overrideTexture.height,
                    _tilesetDef.Def.PxWid,
                    _tilesetDef.Def.PxHei,
                    out var multiplier
                )
            )
            {
                EditorGUILayout.HelpBox(
                    $"Using multiple of {multiplier}.",
                    MessageType.None,
                    false
                );
                return;
            ...
            EditorGUILayout.HelpBox(
                "The resolution of the override texture must be a multiple of the original texture.",
                MessageType.Error,
                true
            );
        }
