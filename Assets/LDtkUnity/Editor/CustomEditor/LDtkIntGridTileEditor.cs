...
namespace LDtkUnity.Editor
{
    ...
    internal sealed class LDtkIntGridTileEditor : LDtkEditor
    {
        ...
        private static readonly GUIContent PhysicsMaterialLabel = new GUIContent
        {
            ...
            tooltip =
                "Sets the physics material of this tile's tilemap.\n"
                + "If tiles have the same tag, layer and physics material, then they will be grouped in the same tilemap and can merge colliders if using a composite collider.",
        };
        ...
        private void OnEnable()
        {
            ...
            _propPhysicsMaterial = serializedObject.FindProperty(
                LDtkIntGridTile.PROPERTY_PHYSICS_MATERIAL
            );
            _propColliderType = serializedObject.FindProperty(
                LDtkIntGridTile.PROPERTY_COLLIDER_TYPE
            );
            _propCustomPhysicsSprite = serializedObject.FindProperty(
                LDtkIntGridTile.PROPERTY_CUSTOM_PHYSICS_SPRITE
            );
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                EditorGUILayout.HelpBox(
                    "After finished editing, Save Project to reimport all LDtk projects that use this tile",
                    MessageType.None
                );
            ...
            if (
                _propColliderType.enumValueIndex == (int)Tile.ColliderType.Sprite
                || serializedObject.isEditingMultipleObjects
            )
            {
                EditorGUILayout.PropertyField(_propCustomPhysicsSprite, SpriteLabel);
                if (
                    _propCustomPhysicsSprite.objectReferenceValue != null
                    && !serializedObject.isEditingMultipleObjects
                )
                {
            ...
            if (
                _propGameObject.objectReferenceValue != null
                && !serializedObject.isEditingMultipleObjects
            )
            {
