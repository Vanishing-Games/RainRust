    ...
    internal sealed class LDtkAssetModificationProcessor : UnityEditor.AssetModificationProcessor
    {
        ...
        private static AssetMoveResult OnWillMoveAsset(string sourcePath, string destinationPath)
        {
            try
            {
                ...
                if (
                    ext != LDtkImporterConsts.PROJECT_EXT
                    && ext != LDtkImporterConsts.LEVEL_EXT
                    && ext != LDtkImporterConsts.TILESET_EXT
                )
                {
                    return AssetMoveResult.DidNotMove;
                }
                    ...
                    switch (ext)
                    {
                        case LDtkImporterConsts.PROJECT_EXT:
                            LDtkDebug.LogWarning(
                                "Did not rename the LDtk project. You should instead rename the project inside of the LDtk editor's project settings."
                            );
                            break;

                        case LDtkImporterConsts.LEVEL_EXT:
                            LDtkDebug.LogWarning(
                                "Did not rename the LDtk level. Renaming the level should be decided by the project inside the LDtk editor."
                            );
                            break;

                        case LDtkImporterConsts.TILESET_EXT:
                            LDtkDebug.LogWarning(
                                "Did not rename the LDtk tileset. The tilesets name is determined by the project's export process."
                            );
                            break;
                    }
