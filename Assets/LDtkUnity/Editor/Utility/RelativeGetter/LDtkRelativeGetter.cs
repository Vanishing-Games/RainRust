...
{
    internal abstract class LDtkRelativeGetter<TData, TAsset>
        where TAsset : Object
    {
        ...
        /// </summary>
        public virtual TAsset GetRelativeAsset(
            TData def,
            string relativeTo,
        ...
        private TAsset GetAssetRelativeToAssetPath(
            string assetPath,
            string relPath,
        ...
        )
        {
            ...
            if (loadedAsset != null)
            {
                ...
                LDtkProfiler.EndSample();
                LDtkDebug.LogError(
                    $"An asset was successfully loaded but was not the right type \"{typeof(TAsset).Name}\" at \"{fullPath}\""
                );
                return null;
            }
        ...
        private static void LogFailIsAssetRelativeToAssetPathExists(string assetsPath)
        {
            ...
            {
                LDtkDebug.LogError(
                    $"File DOES exist but could not load the asset at \"{assetsPath}\". "
            ...
            LDtkDebug.LogError(
                $"Could not find an asset in the path \"{assetsPath}\". "
