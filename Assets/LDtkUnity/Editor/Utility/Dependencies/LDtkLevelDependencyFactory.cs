...
namespace LDtkUnity.Editor
{
    ...
    internal static class LDtkLevelDependencyFactory
    {
        public static string[] GatherLevelDependencies(string levelPath)
        {
            ...
            else
            {
                LDtkDebug.LogWarning(
                    $"Could not find the level's project configuration file from {levelPath}. This will make levels not update properly when the project is changed in the importer inspector."
                );
            }
            ...
            List<ParsedMetaData> allSerializedAssets =
                LDtkDependencyUtil.GetMetaDatasForDependencies(projectLines);
        ...
        private static void DependOnLevelBackground(
            DugDependencyDataLevel depends,
            string projectPath,
