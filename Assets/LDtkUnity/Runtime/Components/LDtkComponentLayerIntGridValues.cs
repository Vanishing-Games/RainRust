...
namespace LDtkUnity
{
    ...
    {
        [SerializeField]
        internal IntGridValuePositions[] _values;
        ...
        internal void OnImport(LDtkDefinitionObjectsCache cache, LayerInstance instance)
        {
            ...
            Dictionary<int, IntGridValuePositions> valuePositions = new Dictionary<
                int,
                IntGridValuePositions
            >(instance.CWid * instance.CHei);

            int index = 0;

            Dictionary<int, IntGridValueDefinition> defs = new Dictionary<
                int,
                IntGridValueDefinition
            >(instance.Definition.IntGridValues.Length);
            ...
            for (int y = 0; y < instance.CHei; y++)
            {
                ...
                for (int x = 0; x < instance.CWid; x++)
                {
                    ...
                    if (intGridValue != 0)
                    {
                        ...
                        {
                            LDtkDefinitionObjectIntGridValue value =
                                new LDtkDefinitionObjectIntGridValue();
        ...
        public LDtkDefinitionObjectIntGridValue GetValueDefinition(Vector3Int coord)
        {
            ...
            return _valueOfPositionDict.TryGetValue(coord, out LDtkDefinitionObjectIntGridValue def)
                ? def
                : null;
        }
        ...
        private void TryCacheValueOfPositionDict()
        {
            ...
            _valueOfPositionDict = new Dictionary<Vector3Int, LDtkDefinitionObjectIntGridValue>(
                _values.Sum(p => p._positions.Count)
            );
        ...
        public Vector3Int[] GetPositionsOfValueDefinition(LDtkDefinitionObjectIntGridValue value)
        {
            ...
            {
                LDtkDebug.LogError(
                    "Argument null when trying to get IntGrid positions",
                    gameObject
                );
                return null;
            }
        ...
        public Vector3Int[] GetPositionsOfValue(int value)
        {
            ...
            {
                _positionsOfValueDict = _values.ToDictionary(
                    obj => obj._value.Value,
                    obj => obj._positions.ToArray()
                );
            }
