...
namespace LDtkUnity
{
    ...
    internal class LDtkFieldDrawerData : LDtkSceneDrawerBase
    {
        ...
        [SerializeField]
        private EditorDisplayMode _fieldMode;
        ...
        public Vector3 LocalMiddleCenter =>
            (Vector2)_fields.transform.position
            + _middleCenter * (Vector2)_fields.transform.lossyScale;

        public LDtkFieldDrawerData(
            LDtkFields fields,
            Color smartColor,
            EditorDisplayMode fieldMode,
            string identifier,
            int gridSize,
            int pixelsPerUnit,
            Vector3 middleCenterWorldPos
        )
            : base(identifier, smartColor)
        {
            _fields = fields;
