...
namespace LDtkUnity
{
    ...
    public class LDtkFieldElement
    {
        ...
        [SerializeField]
        private LDtkReferenceToAnEntityInstance _entityRef = null;
        ...
        {
            if (def.IsInt)
                return LDtkFieldType.Int;
            if (def.IsFloat)
                return LDtkFieldType.Float;
            if (def.IsBool)
                return LDtkFieldType.Bool;
            if (def.IsString)
                return LDtkFieldType.String;
            if (def.IsMultilines)
                return LDtkFieldType.Multiline;
            if (def.IsFilePath)
                return LDtkFieldType.FilePath;
            if (def.IsColor)
                return LDtkFieldType.Color;
            if (def.IsEnum)
                return LDtkFieldType.Enum;
            if (def.IsPoint)
                return LDtkFieldType.Point;
            if (def.IsEntityRef)
                return LDtkFieldType.EntityRef;
            if (def.IsTile)
                return LDtkFieldType.Tile;
            return LDtkFieldType.None;
        }

        internal int TryGetInt(out bool success) =>
            ValidateValue(_int, LDtkFieldType.Int, out success);

        public int GetInt() => ValidateValue(_int, LDtkFieldType.Int);

        internal float TryGetFloat(out bool success) =>
            ValidateValue(_float, LDtkFieldType.Float, out success);

        public float GetFloat() => ValidateValue(_float, LDtkFieldType.Float);

        internal bool TryGetBool(out bool success) =>
            ValidateValue(_bool, LDtkFieldType.Bool, out success);

        public bool GetBool() => ValidateValue(_bool, LDtkFieldType.Bool);

        internal string TryGetString(out bool success) =>
            ValidateValue(_string, LDtkFieldType.String, out success);

        public string GetString() => ValidateValue(_string, LDtkFieldType.String);

        internal string TryGetMultiline(out bool success) =>
            ValidateValue(_string, LDtkFieldType.Multiline, out success);

        public string GetMultiline() => ValidateValue(_string, LDtkFieldType.Multiline);

        internal string TryGetFilePath(out bool success) =>
            ValidateValue(_string, LDtkFieldType.FilePath, out success);

        public string GetFilePath() => ValidateValue(_string, LDtkFieldType.FilePath);

        internal Color TryGetColor(out bool success) =>
            ValidateValue(_color, LDtkFieldType.Color, out success);

        public Color GetColor() => ValidateValue(_color, LDtkFieldType.Color);

        internal TEnum TryGetEnum<TEnum>(out bool success)
            where TEnum : struct
        {
            ...
            LDtkDebug.LogError(
                $"C# enum \"{type.Name}\" does not define enum value \"{_string}\". Possible values are \"{joined}\""
            );
            return default;
        }

        public TEnum GetEnum<TEnum>()
            where TEnum : struct => TryGetEnum<TEnum>(out bool _);
        ...
        internal LDtkReferenceToAnEntityInstance TryGetEntityReference(out bool success) =>
            ValidateValue(_entityRef, LDtkFieldType.EntityRef, out success);

        public LDtkReferenceToAnEntityInstance GetEntityReference() =>
            TryGetEntityReference(out bool _);

        public Sprite TryGetTile(out bool success) =>
            ValidateValue(_sprite, LDtkFieldType.Tile, out success);
        ...
        private T ValidateValue<T>(in T data, LDtkFieldType type, out bool success)
        {
            ...
            LDtkDebug.Assert(
                success,
                $"Trying to get improper type \"{type}\" instead of \"{_type}\""
            );
            return data;
