...
namespace LDtkUnity
{
    ...
    public sealed class LDtkDefinitionObjectLayer : LDtkDefinitionObject<LayerDefinition>, ILDtkUid
    {
        ...
        [field: SerializeField]
        public LDtkDefinitionObjectLayer AutoSourceLayerDef { get; private set; }

        [field: Tooltip("Opacity of the layer (0 to 1.0)")]
        [field: SerializeField]
        public float DisplayOpacity { get; private set; }
        ...
        [field: SerializeField]
        public LDtkDefinitionObjectIntGridValue[] IntGridValues { get; private set; }

        [field: Tooltip("Group informations for IntGrid values")]
        [field: SerializeField]
        public LDtkDefinitionObjectIntGridValueGroup[] IntGridValuesGroups { get; private set; }

        [field: Tooltip(
            "Parallax factor (from -1 to 1 on both axis, defaults to 0) which affects the scrolling speed of this layer, creating a fake 3D (parallax) effect."
        )]
        [field: SerializeField]
        ...
        [field: SerializeField]
        public LDtkDefinitionObjectTileset TilesetDef { get; private set; }
        ...
        [field: SerializeField]
        public LDtkDefinitionObjectAutoLayerRuleGroup[] AutoRuleGroups { get; private set; }

        [field: SerializeField]
        public LDtkDefinitionObjectLayer AutoTilesKilledByOtherLayer { get; private set; }

        [field: SerializeField]
        public LDtkDefinitionObjectField BiomeField { get; private set; }

        [field: Tooltip("Allow editor selections when the layer is not currently active.")]
        [field: SerializeField]
        public bool CanSelectWhenInactive { get; private set; }
        ...
        [field: SerializeField]
        public Vector2Int GuideGridSize { get; private set; }

        [field: SerializeField]
        public bool HideFieldsWhenInactive { get; private set; }
        ...
        [field: SerializeField]
        public Vector2 TilePivot { get; private set; }

        [field: Tooltip(
            "Type of the layer as Haxe Enum Possible values: `IntGrid`, `Entities`, `Tiles`, `AutoLayer`"
        )]
        [field: SerializeField]
        public TypeEnum LayerDefinitionType { get; private set; }
        ...
        [field: SerializeField]
        public string[] UiFilterTags { get; private set; }

        [field: Tooltip("Asynchronous rendering option for large/complex layers")]
        [field: SerializeField]
        public bool UseAsyncRender { get; private set; }
        ...
        {
            Type = def.Type;
            AutoSourceLayerDef = cache.GetObject<LDtkDefinitionObjectLayer>(
                def.AutoSourceLayerDefUid
            );
            DisplayOpacity = def.DisplayOpacity;
            GridSize = def.GridSize;
            Identifier = def.Identifier;
            IntGridValues = def
                .IntGridValues.Select(p => new LDtkDefinitionObjectIntGridValue())
                .ToArray();
            IntGridValuesGroups = def
                .IntGridValuesGroups.Select(p => new LDtkDefinitionObjectIntGridValueGroup())
                .ToArray();
            ...
            //internal
            AutoRuleGroups = def
                .AutoRuleGroups.Select(p =>
                    cache.GetObject<LDtkDefinitionObjectAutoLayerRuleGroup>(p.Uid)
                )
                .ToArray();
            AutoTilesKilledByOtherLayer = cache.GetObject<LDtkDefinitionObjectLayer>(
                def.AutoTilesKilledByOtherLayerUid
            );
        ...
        {
            LDtkDefinitionObjectIntGridValue valueObj = IntGridValues.FirstOrDefault(p =>
                p.Value == value
            );
            if (valueObj == null)
            {
                LDtkDebug.LogError(
                    $"Didn't GetGroupOfValue because the value of \"{value}\" doesn't exist"
                );
                return null;
            }
