...
namespace LDtkUnity
{
    ...
    internal class LDtkDefinitionObjectsCache
    {
        ...
        public void InitializeFromProject(
            Definitions defs,
            Dictionary<int, LDtkArtifactAssetsTileset> tilesets
        )
        {
        ...
        public void InitializeFromLevel(
            List<LDtkDefinitionObject> defs,
            Dictionary<int, LDtkArtifactAssetsTileset> tilesets
        )
        {
        ...
        {
            _defsDict = new Dictionary<int, LDtkDefinitionObject>(
                defs.Entities.Length
                    + defs.Enums.Length
                    + defs.ExternalEnums.Length
                    + defs.Layers.Length
                    + defs.LevelFields.Length
                    + defs.Tilesets.Length
            );
            foreach (EntityDefinition def in defs.Entities)
            {
                _defsDict.Add(
                    def.Uid,
                    ScriptableObject.CreateInstance<LDtkDefinitionObjectEntity>()
                );
                ...
                foreach (FieldDefinition field in def.FieldDefs)
                {
                    _defsDict.Add(
                        field.Uid,
                        ScriptableObject.CreateInstance<LDtkDefinitionObjectField>()
                    );
                }
            }
            ...
            void AddEnums(EnumDefinition[] enums)
            {
                foreach (EnumDefinition def in enums)
                {
                    _defsDict.Add(
                        def.Uid,
                        ScriptableObject.CreateInstance<LDtkDefinitionObjectEnum>()
                    );
                }
            }
            ...
            foreach (LayerDefinition def in defs.Layers)
            {
                _defsDict.Add(
                    def.Uid,
                    ScriptableObject.CreateInstance<LDtkDefinitionObjectLayer>()
                );
                ...
                foreach (AutoLayerRuleGroup group in def.AutoRuleGroups)
                {
                    _defsDict.Add(
                        group.Uid,
                        ScriptableObject.CreateInstance<LDtkDefinitionObjectAutoLayerRuleGroup>()
                    );
                    foreach (AutoLayerRuleDefinition rule in group.Rules)
                    {
                        _defsDict.Add(
                            rule.Uid,
                            ScriptableObject.CreateInstance<LDtkDefinitionObjectAutoLayerRule>()
                        );
                    }
                }
            }
            ...
            foreach (TilesetDefinition def in defs.Tilesets)
            {
                _defsDict.Add(
                    def.Uid,
                    ScriptableObject.CreateInstance<LDtkDefinitionObjectTileset>()
                );
            }
        }
            ...
            where T : ScriptableObject
        {
            ...
            {
                _logger.LogError(
                    $"Failed to get a \"{typeof(T).Name}\" of uid: {uid}. This is likely from a broken json structure"
                );
                return null;
            }
            ...
            _logger.LogError(
                $"Failed to get a \"{typeof(T).Name}\" of uid {uid} due to a type mismatch with {obj.GetType().Name}"
            );
            return null;
        }
        ...
        public Sprite GetSpriteForTilesetRectangle(TilesetRectangle rectangle)
        {
            ...
            if (!_allSprites.TryGetValue(rectangle.TilesetUid, out var sprites))
            {
                ...
                _logger.LogError(
                    $"Problem getting sprite for TilesetRectangle def uid \"{tilesetDef.Identifier}\": Couldn't get the dictionary for the tileset uid"
                );
                return null;
            }
            ...
            {
                _logger.LogError(
                    $"Problem getting sprite for TilesetRectangle def uid {rectangle.TilesetUid}: Couldn't get the sprite from the dictionary for the Rect {rectangle.UnityRect} out of {sprites.Count} possible rects"
                );
                return null;
            }
